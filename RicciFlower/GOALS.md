...
- Section 6 evolution interfaces are largely available; the remaining
  Ricci-norm heat producer is now the lower-layer `ricciHeatSmooth` frontier.
...
- Section 9 local preservation algebra exists; analytic tensor-WMP producers
  remain explicit.
- Section 10.4 has a checked consumer stack; `ricciDataSmooth` now only carries
  the trace-free Laplacian identity, consuming lower Ricci-norm heat data.
...
Ricci-norm heat producer `ricciHeatSmooth`; Section 10 should consume it
rather than add endpoint hypotheses.
...
the Section 6 `ricciHeatSmooth` producer, the Section 10 Laplacian part of
`ricciDataSmooth`, quotient evolution, and pinching estimates.
...
1. Prove `ricciHeatSmooth` at the Ricci-flow layer, then the Laplacian part of
   `ricciDataSmooth`.
