...
This module contains the public pointwise APIs `nabla0SFun` / `nablaRSFun`, bundled regularity
wrappers, and the current explicit regularity frontiers.
-/
...
/-- Fixed-chart representative theorem for `nabla0SFun`.

The intended proof is by slot evaluation: evaluate the moving-center tensor on
fixed-chart tangent-constant slots, evaluate `fixedChartNabla0SModel` on the
same model slots, and compare the two scalar derivation formulas. -/
theorem nabla0SFun_fixedChart_eventuallyEq (s : ℕ)
    (cov : CovariantDerivative I E (TangentSpace I : M → Type _))
    ...
    (fun x : M =>
      ((trivializationAt (Tensor0SModel s 𝕜 E)
        ...
        (⟨x, nabla0SFun (𝕜 := 𝕜) (E := E) (H := H) (I := I) (M := M)
          s cov X α x⟩ :
          TotalSpace (Tensor0SModel s 𝕜 E) (fun x : M => Tensor0SSpace s I x))).2)
      =ᶠ[𝓝 x₀]
    fun x : M =>
      fixedChartNabla0SModel (𝕜 := 𝕜) (E := E) (H := H)
        (I := I) (M := M) (n := (⊤ : WithTop ℕ∞)) s cov X α x₀
  ...
  change
    tensor0SModelAt (𝕜 := 𝕜) (E := E) (H := H) (I := I) (M := M)
      ...
      fixedChartNabla0SModel (𝕜 := 𝕜) (E := E) (H := H)
        (I := I) (M := M) (n := (⊤ : WithTop ℕ∞)) s cov X α x₀
        (extChartAt I x₀ x) slots
  rw [tensor0SModelAt_apply]
  -- Remaining frontier: prove the moving-side derivation formula for
  -- `nabla0SFun` evaluated on the fixed-chart tangent-constant slots.
  -- Unfolding `nabla0SFun` here immediately exposes the moving-center chart
  -- used at `x`, so the next useful theorem is a genuine derivation formula,
  -- not an arbitrary-Γ chart-change law.
  sorry

/-- Once fixed-chart eventual equality is known, the `(0,s)` tensor covariant
derivative is a smooth bundled section. -/
theorem nabla0S_reg_of_fixedChart_eventuallyEq (s : ℕ)
    (cov : CovariantDerivative I E (TangentSpace I : M → Type _))
  ...
  letI := tensor0SBundle_topology (𝕜 := 𝕜) (E := E) (H := H) (I := I) (M := M) s
  haveI := tensor0SBundle_smooth (𝕜 := 𝕜) (E := E) (H := H) (I := I)
    (M := M) (n := (⊤ : WithTop ℕ∞)) s
  ...
  have hx₀ : x₀ ∈ e.baseSet := by
    simpa [e] using
      (mem_baseSet_trivializationAt (Tensor0SModel s 𝕜 E)
        (Bundle.continuousMultilinearMap 𝕜 s E (TangentSpace I : M → Type _)) x₀)
  refine contMDiffAt_section_of_chart_model_eventuallyEq (I := I)
    (F := Tensor0SModel s 𝕜 E)
    (V := fun x : M => Tensor0SSpace s I x)
    (A := fixedChartNabla0SModel (𝕜 := 𝕜) (E := E) (H := H)
      (I := I) (M := M) (n := (⊤ : WithTop ℕ∞)) s cov X α x₀)
    (e := e) hx₀ ?_ ?_
  · have hmn : (⊤ : WithTop ℕ∞) + 1 ≤ (⊤ : WithTop ℕ∞) := by
      simp
    exact (fixedChartNabla0SModel_contDiffWithinAt
      (𝕜 := 𝕜) (E := E) (H := H) (I := I) (M := M)
      (n := (⊤ : WithTop ℕ∞)) s cov hcov X α x₀ hmn).contMDiffWithinAt
  · simpa [e] using
...
theorem nabla0S_reg (s : ℕ)
    ...
    (α : Tensor0SField (𝕜 := 𝕜) (E := E) (H := H) (I := I) (M := M)
      (n := (⊤ : WithTop ℕ∞)) s) :
    Nabla0SRegular (𝕜 := 𝕜) (E := E) (H := H) (I := I) (M := M) s cov X α := by
  exact nabla0S_reg_of_fixedChart_eventuallyEq
    (𝕜 := 𝕜) (E := E) (H := H) (I := I) (M := M)
    s cov hcov X α
