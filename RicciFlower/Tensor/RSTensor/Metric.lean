...
/-- The CLM-valued function `b ↦ clmAt(e₀, b) : E →L[ℝ] E` is `C^n`-smooth at any
point in `baseSet`.

For the tangent bundle, `clmAt(e₀, b)` equals `mfderiv (extChartAt I x₀) b` on
`(chartAt H x₀).source = baseSet`, by `TangentBundle.continuousLinearMapAt_trivializationAt`.
Smoothness of this `mfderiv`-as-CLM is then handled via Mathlib's
`ContMDiffAt.mfderiv_const`, decoupled from the `inTangentCoordinates` wrapping using
`contMDiffOn_coordChangeL` for the source-side coordinate change. -/
theorem contMDiffWithinAt_trivializationAt_clmAt {x₀ x : M}
  ...
  -- Roadmap for filling this sorry:
  -- (a) Prove `b ↦ (trivializationAt E (TangentSpace I) x₀).symmL ℝ b` is `C^n` as a
  --     CLM-valued function on `baseSet`, using:
  --       * smoothness of `e₀.symm : M × E → TotalSpace`
  --         (from `(trivializationAt E (TangentSpace I) x₀).contMDiffOn_symm`),
  --       * `contMDiffOn_id.prodMk_space contMDiffOn_const` to lift fixed `w : E`,
  --       * the finite-dimensional CLM characterisation
  --         (`contDiffOn_clm_apply` in `Analysis.Calculus.ContDiff.FiniteDimension`,
  --         transported to manifolds via `contMDiffWithinAt_iff_contDiffWithinAt`).
  -- (b) Use `Trivialization.continuousLinearMapAt_symmL` and
  --     `Trivialization.symmL_continuousLinearMapAt` to identify
  --     `e₀.continuousLinearMapAt ℝ b = Ring.inverse (e₀.symmL ℝ b)` on `baseSet`.
  -- (c) Apply `contDiffAt_ring_inverse` (smoothness of `Ring.inverse` at invertible
  --     elements of the Banach algebra `E →L[ℝ] E`) and compose with (a).
  -- An equivalent, more direct route uses `TangentBundle.continuousLinearMapAt_trivializationAt`
  -- to rewrite as `mfderiv (extChartAt I x₀)`, then `ContMDiffAt.mfderiv_const` together
  -- with `inTangentCoordinates_eq` and `contMDiffOn_coordChangeL` to peel off the
  -- source-side coordinate change.
  sorry

/-- For each `v : E`, the pointwise evaluation `b ↦ clmAt(e₀, b) v` is `C^n` on `baseSet`.
This is obtained by applying the CLM-valued smoothness
`contMDiffWithinAt_trivializationAt_clmAt` to the constant section `b ↦ v`. -/
lemma contMDiffOn_trivializationAt_clmAt_apply {x₀ : M} (v : E) :
  ...
  intro x hx
  exact (contMDiffWithinAt_trivializationAt_clmAt (n := n) hx).clm_apply
