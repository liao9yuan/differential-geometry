import RicciFlower.RicciFlow.Evolution.ShiftedReaction
import RicciFlower.RicciFlow.Evolution.MetricVariationBounds
...
noncomputable def metricData_sol0
  ...
  ricci_eq := by
    ...
    simp [twoTensorSecToFamily, SolutionOn.ricci, SolutionFamily.ricci,
      metricCurvData, Curvature.metricCurvData]
...
theorem shiftNRaw_barrier
  ...
  have hrealSec :
    ...
    intro X Y
    rw [rawSym2_eq_of_symm (I := I) (M := M) hsym X Y]
    change Bsec t x (Realized.vec2 (I := I) X Y) = Araw x X Y
    rw [show Bsec t x (Realized.vec2 (I := I) X Y) =
...
private theorem ricciEnd_repr_basis
  ...
  simp [DimensionThree.ricciEnd_inner]
...
private theorem ricciQuadAt_comp_basis
    ...
    ricciQuadAt (I := I) (M := M) g Ric
      ...
      Realized.ricciQuadraticAt (I := I) basis gInv Ric i j := by
    ...
    ((tensor0S_curry (I := I) (𝕜 := Real) (M := M) 1 x Ric)
        (∑ k : Idx,
          basis.repr (DimensionThree.ricciEndAt (I := I) g Ric (basis i)) k •
            basis k))
        (fun _ : Fin 1 => basis j) =
      Realized.ricciQuadraticAt (I := I) basis gInv Ric i j
  rw [map_sum]
  simp [Realized.metricTrace_tensor0S_curry_apply_cons, finCons1_eq_vec2,
    ricciEnd_repr_basis (I := I) (M := M) basis gInv hinv Ric,
    Realized.ricciQuadraticAt, Realized.oneUp02CompAt, smul_eq_mul, mul_comm]
...
private theorem rm04ContrAt_comp_basis
    ...
    rm04RicciContrAt (I := I) (M := M) g Rm04 Ric
        (vec2 (I := I) (basis i) (basis j)) =
      Realized.rm04RicciContractionAt (I := I) basis Rm04 gInv Ric i j := by
  ...
  unfold Realized.rm04RicciContractionAt Realized.raised02CompAt
  change
...
private theorem traceData_metricTrace
  ...
  have hcurv :
      DimensionThree.AlgebraicCurvatureSymmetries3
        (DimensionThree.standardRmCompAt
  ...
  have hRicFirst :
      Realized.RicciRealizesRm04FirstTraceAt (I := I) (S.ricci t x)
        (S.base.rm04 t x) DimensionThree.delta3 basis := by
    have hinv :
    ...
    have hInvSym :
      ...
      · have hji : j ≠ i := fun h => hij h.symm
        simp [hij, hji]
    exact Realized.ricciFirstTraceAt_of_rm13
      (I := I) (S.base.metric t) basis DimensionThree.delta3 hinv
      (S.ricci t x) (S.base.rm13 t x) (S.base.rm04 t x)
      hRic13 hLowerAt hInvSym
  have hScalarTrace :
      Realized.ScalarRealizesRicciTraceAt (I := I)
        (metricTracePair0SAt (I := I) (S.base.metric t) (S.ricci t x))
        (S.ricci t x) DimensionThree.delta3 basis := by
    have hinv :
        MetricInverseInBasis (I := I) (S.base.metric t) x basis
          DimensionThree.delta3 :=
      DimensionThree.orthonormal_invBasis3 (I := I) (S.base.metric t)
...
theorem pinchSec_quad
    ...
    twoTensorSecToFamily (I := I) (M := M)
        (pinchSec (I := I) S delta) t x v v =
      S.ricci t x (Realized.vec2 (I := I) v v) -
...
theorem pinchSec_quad_deriv
    ...
    (hRic :
      HasDerivWithinAt
        (fun s : Real => S.ricci s x (Realized.vec2 (I := I) v v))
        ricDt K t)
    (hScalar :
  ...
  have hsub :
      HasDerivWithinAt
        (fun s : Real =>
          S.ricci s x (Realized.vec2 (I := I) v v) -
            delta * (S.scalar s x * (S.family.metric s).inner x v v))
        (ricDt -
          delta * (scalarDt * (S.family.metric t).inner x v v +
            S.scalar t x * metricDt)) K t :=
...
theorem ricciQuadDeriv_coord
    ...
    HasDerivWithinAt
      ...
      (∑ i : Coordinates.CoordinateIdx (𝕜 := Real) E,
        ∑ j : Coordinates.CoordinateIdx (𝕜 := Real) E,
          (Coordinates.coordinateFrameAt_toBasis (I := I) x).coord i v *
            (Coordinates.coordinateFrameAt_toBasis (I := I) x).coord j v *
      ...
      D.carrier (t : Real) := by
  classical
  let b := Coordinates.coordinateFrameAt_toBasis (I := I) x
  let frame := Coordinates.coordinateFrameAt (I := I) x
  let rhs : Coordinates.CoordinateIdx (𝕜 := Real) E →
  ...
  have hsum_eval : ∀ s : Real,
    ...
    intro s
    have h :=
      Coordinates.tensor0S_two_eval_coordFrame_sum (I := I)
        (M := M) (x₀ := x) (Ax := S.ricci s x) v v
    rw [vec2_self_eq_const (I := I) (M := M) v]
    simpa [b, frame, Realized.vec2, Curvature.vec2, ricciCompInFrame,
  ...
  have hsum_deriv :
    ...
    simpa [rhs, b, frame, mul_assoc] using
      (HasDerivWithinAt.fun_sum
        (u := (Finset.univ :
          Finset (Coordinates.CoordinateIdx (𝕜 := Real) E)))
        (A := fun i s =>
          ∑ j : Coordinates.CoordinateIdx (𝕜 := Real) E,
            b.coord i v * b.coord j v *
              ricciCompInFrame (I := I) S frame s x i j)
            ...
            (Coordinates.coordinateFrameAt_toBasis (I := I) x).coord i v *
              (Coordinates.coordinateFrameAt_toBasis (I := I) x).coord j v *
                ricciEvolutionRHSInFrame
                  ...
                  (t : Real) x i j) -
        delta *
          ((Realized.laplacianAt (I := I) (flowG (I := I) S) (t : Real)
                (S.scalar (t : Real)) x +
...
noncomputable def pinchMetricDerivs
  ...
  simpa [SolutionFamily.connection] using
    metricDerivsZero (I := I)
      (LeviCivita.leviCivitaConnectionOfMetric (I := I) (S.base.metric t))
      (S.base.metric t)
      (LeviCivita.leviCivitaConnectionOfMetric_isMetricCompatible
        (I := I) (S.base.metric t))
...
theorem scalarHessTrace_eq_lap
  ...
  simpa [Realized.laplacianAt, flowG, SolutionOn.scalar, SolutionFamily.scalar,
    metricScalarAt, Curvature.metricScalarAt, SolutionFamily.ricciAt,
...
theorem ricciRoughTrace_coord
  ...
  have hnabla : ∀ y a i j,
      ricciNablaWMP (I := I) S t y
          (Realized.vec3 (I := I)
            ...
            (Coordinates.coordinateFrameAt (I := I) x i y)
            (Coordinates.coordinateFrameAt (I := I) x j y)) =
        nablaRicComp (I := I) S
          (Coordinates.coordinateFrameAt (I := I) x) t y a i j := by
    intro y a i j
  ...
  have hcomp :
      ∀ i j : Coordinates.CoordinateIdx (𝕜 := Real) E,
        roughA
            (Realized.vec2 (I := I) (frame i x) (frame j x)) =
          coordRoughRic (I := I) S x (coordNab2Ric (I := I) S x)
            t x i j := by
    intro i j
      ...
      ∀ i j : Coordinates.CoordinateIdx (𝕜 := Real) E,
        roughA
          ...
          coordRoughRic (I := I) S x (coordNab2Ric (I := I) S x)
            t x i j := by
    intro i j
    simpa [Realized.vec2, Curvature.vec2] using hcomp i j
  have hsum :=
    Coordinates.tensor0S_two_eval_coordFrame_sum (I := I)
      (M := M) (x₀ := x) (Ax := roughA) v v
  rw [← roughLap0STensor_apply (I := I) (S.base.metric t)
      (ricciNabla2WMP (I := I) S t x) (Realized.vec2 (I := I) v v)]
  rw [vec2_self_eq_const (I := I) (M := M) v]
...
theorem scalarMetric_trace
  ...
  have h := pinchRough_hessMetric (I := I) S t basis gInv hinv
    (scalarHessSec (I := I) S t x) (Realized.vec2 (I := I) v v)
  simpa [roughLap0STensor_apply, metricTensorField_apply,
    Realized.vec2, RicciFlower.Curvature.vec2] using h
...
theorem pinchNab2Model_trace
    ...
    (v : TangentSpace I x) :
    metricTraceFirstTwo0SAt (I := I) (S.base.metric t)
        (pinchNab2Model (I := I) S delta t x) (Realized.vec2 (I := I) v v) =
      metricTraceFirstTwo0SAt (I := I) (S.base.metric t)
          (ricciNabla2WMP (I := I) S t x) (Realized.vec2 (I := I) v v) -
        delta *
          (metricTraceFirstTwo0SAt (I := I) (S.base.metric t)
...
theorem pinchHeat_coord
  ...
  rw [pinchNab2Model_trace (I := I) S delta t
    (Coordinates.coordinateFrameAt_toBasis (I := I) x)
    (fun a b : Coordinates.CoordinateIdx (𝕜 := Real) E =>
  ...
  ricciCoordQuadRHS (I := I) S t x v -
    delta *
      ((Realized.laplacianAt (I := I) (flowG (I := I) S) t
        ...
        S.scalar t x *
          ((-2 : Real) * S.ricciAt t x (Realized.vec2 (I := I) v v)))
...
private theorem ricciActualReactAt_eq_reaction_basis
  ...
  have hsym : DimensionThree.RicciSymAt (I := I) (S.ricci t x) := by
...
private theorem ricciCoordReact_eq_actual
    ...
    ricciCoordReact (I := I) S t x v =
      ricciActualReactAt (I := I) S t x (Realized.vec2 (I := I) v v) := by
  classical
  let b := Coordinates.coordinateFrameAt_toBasis (I := I) x
  let frame := Coordinates.coordinateFrameAt (I := I) x
  let gInvAt : Coordinates.CoordinateIdx (𝕜 := Real) E ->
      Coordinates.CoordinateIdx (𝕜 := Real) E -> Real :=
  ...
  have hbasis :
      ∀ i : Coordinates.CoordinateIdx (𝕜 := Real) E,
        b i = frame i x := by
    intro i
    simp [b, frame, Coordinates.coordinateFrameAt_toBasis_apply]
  have hcomp :
      ∀ i j : Coordinates.CoordinateIdx (𝕜 := Real) E,
        ricciActualReactAt (I := I) S t x
            (Realized.vec2 (I := I) (frame i x) (frame j x)) =
    ...
    calc
      ...
      _ =
        (-2 : Real) *
            Realized.rm04RicciContractionAt (I := I) b (S.base.rm04 t x)
              gInvAt (S.ricci t x) i j -
          2 * Realized.ricciQuadraticAt (I := I) b gInvAt
              (S.ricci t x) i j := by
      ...
      _ =
          ...
          simp [rmRicciContractionCompInFrame, raisedRicciCompInFrame,
            ...
            Realized.rm04Comp, RicciFlower.Curvature.rm04Comp,
            Realized.rm04RicciContractionAt, Realized.raised02CompAt,
  ...
  let c : Coordinates.CoordinateIdx (𝕜 := Real) E ->
      Coordinates.CoordinateIdx (𝕜 := Real) E -> Real :=
    fun i j =>
      (Coordinates.coordinateFrameAt_toBasis (I := I) x).coord i v *
        (Coordinates.coordinateFrameAt_toBasis (I := I) x).coord j v
  let L : Coordinates.CoordinateIdx (𝕜 := Real) E ->
      Coordinates.CoordinateIdx (𝕜 := Real) E -> Real :=
    fun i j => coordRoughRic (I := I) S x (coordNab2Ric (I := I) S x) t x i j
  let R : Coordinates.CoordinateIdx (𝕜 := Real) E ->
      Coordinates.CoordinateIdx (𝕜 := Real) E -> Real :=
    fun i j =>
      rmRicciContractionCompInFrame
        (I := I) S S.base.rm04 (coordInv (I := I) S x)
        (Coordinates.coordinateFrameAt (I := I) x) t x i j
  let Q : Coordinates.CoordinateIdx (𝕜 := Real) E ->
      Coordinates.CoordinateIdx (𝕜 := Real) E -> Real :=
    fun i j =>
...
theorem shiftNRaw_pinchCoordReact
  ...
  · obtain ⟨nb⟩ :=
    ...
    simp [pinchCoordReact, metricTensorField_apply, smul_eq_mul,
      SolutionOn.scalar_eq_metricTrace, vec2, Curvature.vec2]
    ring
