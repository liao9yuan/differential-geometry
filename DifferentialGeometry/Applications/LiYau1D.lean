...
class HeatEquation1D
  ...
  hessian_norm_eq_laplacian_sq : ∀ t, tensorNormSq metric (hessianForm metric conn (f t)) = laplacian metric.toNonDegenerateMetric.toAbstractMetricTensor conn (f t) * laplacian metric.toNonDegenerateMetric.toAbstractMetricTensor conn (f t)
  t_val : Time → R
...
theorem laplacian_f_evolution
  ...
  2 * tensorNormSq metric (hessianForm metric conn (f t)) := by
  dsimp [L]
  ...
  rw [h_lap_add]
  have h_bochner : laplacian metric.toNonDegenerateMetric.toAbstractMetricTensor conn (metric.g (grad metric (f t)) (grad metric (f t))) = 2 * tensorNormSq metric (hessianForm metric conn (f t)) + 2 * Rc conn (grad metric (f t)) (grad metric (f t)) + 2 * metric.g (grad metric (f t)) (grad metric (laplacian metric.toNonDegenerateMetric.toAbstractMetricTensor conn (f t))) := bochner_identity metric conn (f t)
  rw [h_bochner]
  have h_rc : Rc conn (grad metric (f t)) (grad metric (f t)) = 0 := heat.flat_Rc _ _
  rw [h_rc]
  change laplacian metric.toNonDegenerateMetric.toAbstractMetricTensor conn (laplacian metric.toNonDegenerateMetric.toAbstractMetricTensor conn (f t)) + (2 * tensorNormSq metric (hessianForm metric conn (f t)) + 2 * 0 + 2 * metric.g (grad metric (f t)) (grad metric (laplacian metric.toNonDegenerateMetric.toAbstractMetricTensor conn (f t)))) - laplacian metric.toNonDegenerateMetric.toAbstractMetricTensor conn (laplacian metric.toNonDegenerateMetric.toAbstractMetricTensor conn (f t)) - 2 * metric.g (grad metric (f t)) (grad metric (laplacian metric.toNonDegenerateMetric.toAbstractMetricTensor conn (f t))) = 2 * tensorNormSq metric (hessianForm metric conn (f t))
  ring
...
theorem Q_evolution
  ...
  rw [h_tmul]
  have h_lap_evol : L metric conn f (fun s => laplacian metric.toNonDegenerateMetric.toAbstractMetricTensor conn (f s)) t = 2 * tensorNormSq metric (hessianForm metric conn (f t)) := laplacian_f_evolution metric conn u f inv_u t
  rw [h_lap_evol]
  have h_1d : tensorNormSq metric (hessianForm metric conn (f t)) = laplacian metric.toNonDegenerateMetric.toAbstractMetricTensor conn (f t) * laplacian metric.toNonDegenerateMetric.toAbstractMetricTensor conn (f t) := heat.hessian_norm_eq_laplacian_sq t
  rw [h_1d]
