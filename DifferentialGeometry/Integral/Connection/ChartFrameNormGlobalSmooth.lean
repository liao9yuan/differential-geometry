...
/-! ## Stage B: the second headline -/

/-- **Second headline**: the global section agrees with `smoothOrthoFrame g b i`
on a neighbourhood of each `b` in the chart-α partition-of-unity tsupport
intersected with the chart-α Levi-Civita good set, under the locally-constant
chart predicate and the chart-equation `chartAt H b = chartAt H α`.

The chart-equation `chartAt H b = chartAt H α` is the genuine mathematical
hypothesis bridging the centre-`b` Gram-Schmidt frame to the centre-`α`
Gram-Schmidt frame. The locally-constant chart predicate at `b` would yield
such an equation only inside the open `{y | chartAt H y = chartAt H α}`, which
is not guaranteed to contain every point of the chart-α POU tsupport; we
therefore require the equation as an explicit hypothesis. -/
theorem chartFrameNormGlobalSmooth_eventuallyEq_smoothOrthoFrame
    ...
    (hb_chart : chartAt H b = chartAt H α) :
    (smoothOrthoFrame (I := I) g b i :
        Π y : M, TangentSpace I y) =ᶠ[𝓝 b]
      (chartFrameNormGlobalSmooth (I := I) (M := M) g α i).toFun := by
  classical
  -- Set up the three nbhds whose intersection witnesses the eventually-equality.
  -- (1) The nbhd `V₁` from the global-bump data: on V₁, the global section equals
  -- chartFrameNorm g α i.
  have hb_pou : b ∈ tsupport (fun x : M =>
      ((chartAtlasPOU I M α : C^∞⟮I, M; ℝ⟯) : M → ℝ) x) := hb.1
  have hb_V₁ : b ∈ globalBumpV₁ (I := I) (M := M) α :=
    pouTsupport_subset_globalBumpV₁ (I := I) (M := M) α hb_pou
  have hV₁_open : IsOpen (globalBumpV₁ (I := I) (M := M) α) :=
    globalBumpV₁_isOpen (I := I) (M := M) α
  have hV₁_nhds : globalBumpV₁ (I := I) (M := M) α ∈ 𝓝 b :=
    hV₁_open.mem_nhds hb_V₁
  -- (2) The nbhd `smoothOrthoFrameNbhd b`.
  have hNb_nhds : smoothOrthoFrameNbhd (I := I) (M := M) b ∈ 𝓝 b :=
    smoothOrthoFrameNbhd_mem_nhds (I := I) (M := M) b
  -- (3) The nbhd `U_b` from h_atlas at `b`, on which `chartAt H y = chartAt H b`.
  have hUb_nhds :
      {y : M | chartAt H y = chartAt H b} ∈ 𝓝 b := h_atlas b
  -- Intersection.
  set W : Set M :=
    globalBumpV₁ (I := I) (M := M) α ∩
      smoothOrthoFrameNbhd (I := I) (M := M) b ∩
      {y : M | chartAt H y = chartAt H b} with hW_def
  have hW_nhds : W ∈ 𝓝 b := by
    refine Filter.inter_mem (Filter.inter_mem hV₁_nhds hNb_nhds) hUb_nhds
  refine Filter.eventually_of_mem hW_nhds ?_
  intro y hy
  obtain ⟨⟨hy_V₁, hy_Nb⟩, hy_Ub⟩ := hy
  -- On `smoothOrthoFrameNbhd b`, smoothOrthoFrame g b i y = chartFrameNorm g b i y.
  have hLHS_step : smoothOrthoFrame (I := I) g b i y =
      chartFrameNorm (I := I) g b i y :=
    smoothOrthoFrame_eq_on_nbhd (I := I) g b i hy_Nb
  -- Using chartAt H y = chartAt H b (from h_atlas) and chartAt H b = chartAt H α (from hyp),
  -- chartAt H y = chartAt H α. Hence chartFrameNorm g b i y = chartFrameNorm g α i y
  -- (the chart-center change preserves the frame at every fixed evaluation point).
  have hba : chartAt H b = chartAt H α := hb_chart
  have hFrame_eq : chartFrameNorm (I := I) g b i y =
      chartFrameNorm (I := I) g α i y :=
    chartFrameNorm_eq_of_chartAt_eq (I := I) (H := H) (M := M) g hba i y
  -- On V₁, the global section equals chartFrameNorm g α i.
  have hGlobal_eq : (chartFrameNormGlobalSmooth (I := I) (M := M) g α i).toFun y =
      chartFrameNorm (I := I) g α i y := by
    have hψ : globalBumpψ (I := I) (M := M) α y = 1 :=
      globalBumpψ_eq_one_on_closure_V₁ (I := I) (M := M) α (subset_closure hy_V₁)
    rw [chartFrameNormGlobalSmooth_toFun_apply, hψ, one_smul]
  -- Chain the equalities. `hy_Ub` is incidental (the chart-equation comes
  -- from `hb_chart`); we keep the nbhd `U_b` in the intersection for
  -- alignment with the locally-constant chart strategy.
  -- Goal: (smoothOrthoFrame g b i) y = (chartFrameNormGlobalSmooth g α i).toFun y.
  -- LHS = chartFrameNorm g b i y = chartFrameNorm g α i y = RHS.
  change (smoothOrthoFrame (I := I) g b i :
      Π y : M, TangentSpace I y) y =
