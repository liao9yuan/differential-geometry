...
/-! ## Per-point local op-norm bound on `(triv α).symmL`
...
`v : TensorRSModel r s ℝ E`,
  ...
  `‖(triv α).symmL b v‖ ≤ M_{y₀} · ‖v‖`.

The constant arises as the product of:
* `D_{y₀} := 1 + ‖coordChangeL α y₀ y₀‖`, a continuity-based upper bound on the
  `coordChangeL` op-norm in a sufficiently small neighbourhood of `y₀`;
* `C_{y₀}` from `eventually_norm_symmL_trivializationAt_lt y₀` applied to the
  `(r, s)`-tensor bundle, which under `[IsContinuousRiemannianBundle …]` bounds
  `‖(triv y₀).symmL b‖_op` for `b` near `y₀`. -/
...
private lemma exists_W_and_constant_symmL
  ...
  -- `b ↦ ‖coordChangeL α y₀ b‖` is continuous on `(chartAt H y₀).source ∩
  -- (chartAt H α).source`, hence locally bounded near `y₀`. Note: we use
  -- coordChange FROM α TO y₀ here (inverse-direction factorisation).
  ...
  -- The inverse trivialisation `(triv y₀).symmL b` op-norm is `< C₁` on V₁.
  ...
  -- Factorisation: `eα.symmL b v = ey₀.symmL b (coordChangeL eα ey₀ b v)`.
  ...
  -- The key factorisation. We derive it from `coordChangeL_apply`:
  -- `coordChangeL eα ey₀ b v = ey₀.clmAt b (eα.symmL b v)`. Apply `ey₀.symmL b`:
  -- `ey₀.symmL b (coordChangeL eα ey₀ b v) = eα.symmL b v` (by `symmL_clmAt`).
  ...
  -- Bound the norm of `coordChangeL eα ey₀ b v` by `‖coordChangeL eα ey₀ b‖ * ‖v‖`.
  have h_norm_cc_v :
      ‖(eα.coordChangeL ℝ ey₀ b
  ...
  -- Bound the norm of the inverse trivialisation applied to that.
  have h_norm_total :
      ‖(eα.symmL ℝ b v : TensorRSSpace r s I b)‖ ≤
        ‖ey₀.symmL ℝ b‖ *
        ‖(eα.coordChangeL ℝ ey₀ b
...
theorem tensorRSChartFiberFromModel_opNorm_isBounded_on_compact_unconditional
  ...
  letI : Bundle.RiemannianBundle (fun b : M => TensorRSSpace r s I b) :=
    Tensor0SBundle.tensorRS_riemannianBundle (I := I) (M := M) g r s
  -- For each `y₀ ∈ M`, define `W` to be the open nbhd from
  -- `exists_W_and_constant_symmL` when `y₀ ∈ (chartAt H α).source`, or `∅`
  -- otherwise.
  let W : M → Set M := fun y₀ =>
