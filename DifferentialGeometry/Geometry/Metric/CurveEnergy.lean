...
/-- The fixed-metric energy of a manifold-valued curve on an oriented interval. -/
...
/-- Fixed-metric curve energy decreases when the oriented interval is restricted. -/
...
/-- Arc length is bounded by the square root of time times the square root of energy. -/
...
/-- Endpoint Riemannian distance is bounded by the energy of a `C¹` curve. -/
...
/-- A uniform energy budget gives the endpoint estimate used for `1 / 2`-Hölder control. -/
