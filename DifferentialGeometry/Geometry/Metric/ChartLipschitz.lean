...
/-!
# Intrinsic Lipschitz control of manifold charts

This file compares a canonical chart with the intrinsic distance of a
continuous Riemannian metric.  It complements Mathlib's local estimate in the
opposite direction for the inverse chart.
-/
...
variable {M : Type*} [PseudoEMetricSpace M] [ChartedSpace H M]
  [IsManifold I 1 M] [RegularSpace M]
...
theorem extChart_lip_at (q : M) :
  ...
  have hyρ : riemannianEDist I q y < (ρ : ℝ≥0∞) := by
    simpa only [s, Set.mem_setOf_eq] using hy
  have hzρ : riemannianEDist I q z < (ρ : ℝ≥0∞) := by
    simpa only [s, Set.mem_setOf_eq] using hz
...
/-- A fixed canonical chart is locally Lipschitz on its source for the
intrinsic Riemannian distance. -/
theorem extChart_lip (p : M) :
    LocallyLipschitzOn (extChartAt I p).source (extChartAt I p) := by
  intro q hq
  obtain ⟨K, s, hs, hKs⟩ := extChart_lip_at (I := I) q
...
/-- On a compact subset of one chart source, the chart has one global
intrinsic Lipschitz constant. -/
theorem extChart_lip_cpt (p : N) {K : Set N} (hK : IsCompact K)
    (hKsrc : K ⊆ (extChartAt I p).source) :
    ∃ C : ℝ≥0, LipschitzOnWith C (extChartAt I p) K :=
  ((extChart_lip (I := I) p).mono hKsrc).exists_lipschitzOnWith_of_compact hK
