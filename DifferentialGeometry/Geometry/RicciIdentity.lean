...
lemma nabla_sub_left (conn : AffineConnection R V) (X Y Z : V) : conn.nabla (X - Y) Z = conn.nabla X Z - conn.nabla Y Z := by
...
lemma secondCovDerivCommutator_expand (conn : AffineConnection R V) (X Y Z : V) :
...
theorem ricci_identity (conn : AffineConnection R V) [TorsionFree conn] (X Y Z : V) :
