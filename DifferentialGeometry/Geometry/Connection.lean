...
def koszul_connection [Invertible (2 : R)] [LieBracket V] [DerivationRules R V]
  (metric : MetricDuality R V) : AffineConnection R V where
  ...
  nabla_smul_left c X Y := by
    have eq_func : (fun Z : V => (action (c • X) (metric.g Y Z) + action Y (metric.g Z (c • X)) - action Z (metric.g (c • X) Y) - metric.g (c • X) (bracket Y Z) + metric.g Y (bracket Z (c • X)) + metric.g Z (bracket (c • X) Y)) * ⅟2)
      ...
      have h6a : metric.g Z (bracket (c • X) Y) = c * metric.g Z (bracket X Y) - action Y c * metric.g Z X := by rw [h6, metric.symm Z _, metric_sub_left (metric:=metric.toNonDegenerateMetric.toMetricTensor), metric.bilinear_smul_left, metric.bilinear_smul_left, metric.symm (bracket _ _) Z, metric.symm X Z]
  ...
  leibniz c X Y := by
    have eq_func : (fun Z : V => (action X (metric.g (c • Y) Z) + action (c • Y) (metric.g Z X) - action Z (metric.g X (c • Y)) - metric.g X (bracket (c • Y) Z) + metric.g (c • Y) (bracket Z X) + metric.g Z (bracket X (c • Y))) * ⅟2)
      ...
      have h4a : metric.g X (bracket (c • Y) Z) = c * metric.g X (bracket Y Z) - action Z c * metric.g X Y := by rw [h4, metric.symm X _, metric_sub_left (metric:=metric.toNonDegenerateMetric.toMetricTensor), metric.bilinear_smul_left, metric.bilinear_smul_left, metric.symm (bracket _ _) X, metric.symm Y X]
...
instance metric_compat_koszul [Invertible (2 : R)] (metric : MetricDuality R V) : MetricCompatible (koszul_connection metric) metric.toNonDegenerateMetric.toMetricTensor where
  compat X Y Z := by
    ...
    have m1 : metric.g X (bracket Z Y) = - metric.g X (bracket Y Z) := by rw [b1, metric.symm X _, metric_neg_left (metric:=metric.toNonDegenerateMetric.toMetricTensor), metric.symm _ X]
    have m2 : metric.g Z (bracket Y X) = - metric.g Z (bracket X Y) := by rw [b2, metric.symm Z _, metric_neg_left (metric:=metric.toNonDegenerateMetric.toMetricTensor), metric.symm _ Z]
    have m3 : metric.g Y (bracket X Z) = - metric.g Y (bracket Z X) := by rw [b3, metric.symm Y _, metric_neg_left (metric:=metric.toNonDegenerateMetric.toMetricTensor), metric.symm _ Y]
...
instance torsion_free_koszul [Invertible (2 : R)] (metric : MetricDuality R V) : TorsionFree (koszul_connection metric) where
  torsion_zero X Y := by
    ...
    have eq_func : (fun Z : V => (action X (metric.g Y Z) + action Y (metric.g Z X) - action Z (metric.g X Y) - metric.g X (bracket Y Z) + metric.g Y (bracket Z X) + metric.g Z (bracket X Y)) * ⅟2 - (action Y (metric.g X Z) + action X (metric.g Z Y) - action Z (metric.g Y X) - metric.g Y (bracket X Z) + metric.g X (bracket Z Y) + metric.g Z (bracket Y X)) * ⅟2) = (fun Z : V => metric.g (bracket X Y) Z) := by
      ...
      have m1 : metric.g Y (bracket X Z) = - metric.g Y (bracket Z X) := by rw [b1, metric.symm Y _, metric_neg_left (metric:=metric.toNonDegenerateMetric.toMetricTensor), metric.symm _ Y]
      have m2 : metric.g X (bracket Z Y) = - metric.g X (bracket Y Z) := by rw [b2, metric.symm X _, metric_neg_left (metric:=metric.toNonDegenerateMetric.toMetricTensor), metric.symm _ X]
      have m3 : metric.g Z (bracket Y X) = - metric.g Z (bracket X Y) := by rw [b3, metric.symm Z _, metric_neg_left (metric:=metric.toNonDegenerateMetric.toMetricTensor), metric.symm _ Z]
      rw [s1, s2, s3, m1, m2, m3]
    ...
    have hg : metric.g (nabla X Y - nabla Y X) Z = metric.g (nabla X Y) Z - metric.g (nabla Y X) Z := by
      ...
      rw [hsub, metric.bilinear_add_left, metric_neg_left (metric:=metric.toNonDegenerateMetric.toMetricTensor), ← sub_eq_add_neg]
    rw [hg]
...
theorem levi_civita_exists_unique [Invertible (2 : R)] (metric : MetricDuality R V) :
  ∃ (conn : AffineConnection R V),
    (MetricCompatible conn metric.toNonDegenerateMetric.toMetricTensor ∧ TorsionFree conn) ∧
    (∀ (conn' : AffineConnection R V), MetricCompatible conn' metric.toNonDegenerateMetric.toMetricTensor ∧ TorsionFree conn' → conn' = conn) := by
  ...
  · intro conn' h
    ...
    have h_nabla : conn'.nabla = (koszul_connection metric).nabla := by
      ...
      apply g_inj
      intro Z
      have h1 : 2 * metric.g (conn'.nabla X Y) Z = action X (metric.g Y Z) + action Y (metric.g X Z) - action Z (metric.g X Y) - metric.g X (bracket Y Z) + metric.g Y (bracket Z X) + metric.g Z (bracket X Y) := levi_civita_uniqueness conn' metric.toNonDegenerateMetric.toMetricTensor X Y Z
