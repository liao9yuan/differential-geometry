...
/-- A bounded-speed geodesic with a compact endpoint tail extends past that endpoint. -/
...
omit [NeZero (Module.finrank ℝ E)] [T2Space M] [SigmaCompactSpace M]
    [RiemannianBundle (fun x : M => TangentSpace I x)] [PseudoEMetricSpace M]
    [IsRiemannianManifold I M] [CompleteSpace M] in
/-- A local geodesic continues to a fixed finite horizon when every compatible
endpoint strictly below that horizon admits continuation. -/
theorem geo_Ioo_extend_to
