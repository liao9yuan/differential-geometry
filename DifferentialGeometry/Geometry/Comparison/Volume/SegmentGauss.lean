...
model chart-basis Jacobi frame relates to the `gₓ`-adapted launch frame; the
launch-basis Gram determinant supplies the `√det gₓ` constant. -/
...
/-- Endpoint intrinsic-Jacobi density changes by the absolute determinant when
the launch basis is changed. -/
...
/-- The transverse intrinsic-Jacobi density at time `t` is `|t|^d` times
the endpoint density launched from `t • u` in the same transverse frame. -/
