...
theorem expJac_integrable
  ...
  change Integrable
    (fun v : E => expJacDensity (I := I) g hEnorm x v * f (F v)) μK
  apply hweighted.congr
  filter_upwards with v
  simp only [J, ENNReal.toReal_ofReal
    (expJacDensity_nonneg (I := I) g hEnorm x v), mul_comm]
...
theorem expJac_integral
  ...
  simp only [J, F, ENNReal.toReal_ofReal
    (expJacDensity_nonneg (I := I) g hEnorm x v), smul_eq_mul]
...
theorem segInt_lintegral
    ...
    (∫⁻ y in
        (fun v : E => expMapIntrinsic (I := I) g hEnorm x
          (show TangentSpace I x from v)) ''
            (show Set E from SegInt (I := I) g hEnorm x),
        f y ∂(riemannianVolumeMeasure (I := I) (M := M) g)) =
      ∫⁻ v in (show Set E from SegInt (I := I) g hEnorm x),
        f (expMapIntrinsic (I := I) g hEnorm x
          (show TangentSpace I x from v)) *
          ENNReal.ofReal (expJacDensity (I := I) g hEnorm x v)
        ∂(modelHaar (E := E)) := by
  exact expJac_lintegral (I := I) g hEnorm x
    (measurableSet_segInt (I := I) g hEnorm x)
    (exp_inj_segInt (I := I) g hEnorm x) f hf
...
theorem segInt_int_polar
    ...
    (∫ y in
      ...
      ∫ u : Metric.sphere (0 : E) 1,
        ∫ r : Ioi (0 : ℝ),
          (show Set E from SegInt (I := I) g hEnorm x).indicator
            (fun v : E => expJacDensity (I := I) g hEnorm x v *
              f (expMapIntrinsic (I := I) g hEnorm x
                (show TangentSpace I x from v)))
            (r.1 • u.1)
  ...
  let F : E → M := fun v =>
    expMapIntrinsic (I := I) g hEnorm x
      (show TangentSpace I x from v)
  let D : E → ℝ := fun v => expJacDensity (I := I) g hEnorm x v
  let K : Set E := show Set E from SegInt (I := I) g hEnorm x
