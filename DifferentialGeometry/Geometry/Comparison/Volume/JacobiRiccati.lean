...
omit [NeZero (Module.finrank ℝ E)]
    [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- Equality in the mean-square bound holds exactly when the shape matrix is scalar. -/
theorem mean_sq_eq_iff
  ...
  letI : Nonempty (Fin (Module.finrank ℝ E - 1)) := ⟨⟨0, hd⟩⟩
...
omit [SigmaCompactSpace M] in
/-- Equality in the mean Riccati bound holds exactly when its Ricci and shape defects vanish. -/
