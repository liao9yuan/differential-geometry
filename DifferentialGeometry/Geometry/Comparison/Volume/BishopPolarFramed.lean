...
/-- At the origin, the framed normal-coordinate density is the fixed Gram
density of the chart-model basis. -/
@[simp] theorem framedDens_zero
    (g : SmoothRiemannianMetric I M) (p : M) :
...
omit [Nontrivial E] [T2Space M] [SigmaCompactSpace M]
  [NeZero (Module.finrank ℝ E)] in
/-- The framed density at the pole exactly converts chart-model Haar measure
to the canonical volume measure. -/
theorem framedDens_haar
  ...
  have hdet :
    ...
    simpa only [basisGram, Matrix.of_apply, L, ContinuousLinearMap.id_apply] using h
  have hdensity :
...
omit [Nontrivial E] [T2Space M] [SigmaCompactSpace M]
  [NeZero (Module.finrank ℝ E)] in
/-- The normal-coordinate density at the pole converts chart-model Haar
measure to the pushforward of canonical volume by the normal frame. -/
