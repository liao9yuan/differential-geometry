...
theorem intrDen_deriv_le
  ...
  have hW : ∀ i j, jacobiWronskian (I := I) g γ (V i) (V j) t = 0 := by
    ...
    exact wronskian_eq_zero (I := I) (n := (2 : WithTop ℕ∞)) (by norm_num)
      ...
      (fun s _ ↦ by simpa only [γ, V] using
        intrinsic_jacobi (I := I) g hEnorm p (u : E) (v i : E) s)
      (fun s _ ↦ by simpa only [γ, V] using
        intrinsic_jacobi (I := I) g hEnorm p (u : E) (v j : E) s)
      (by simp [V]) (by simp [V]) t ⟨ht.1.le, ht.2.le⟩
  let R : Real → Real := fun r ↦
    curveDensity (I := I) g γ V r / hypDensity 0 d r
  ...
  have hanti : AntitoneOn R (Ioo (0 : Real) b) := by
    simpa only [R, γ, V, d, zero_mul] using
      intrRatioOfFrame (I := I) g hEnorm p u 0 b (by norm_num)
        hd hu v hON hperp hno hRic0
  have hRderiv : HasDerivAt R
      (R t * (curveMean (I := I) g γ V t - hypMeanCurv 0 d t)) t := by
    simpa only [R] using
  ...
  have hRnonpos :
      R t * (curveMean (I := I) g γ V t - hypMeanCurv 0 d t) ≤ 0 := by
  ...
  have hRpos : 0 < R t := by
    exact div_pos (curveDensity_pos (I := I) g γ V t hLI)
      (hypDensity_pos (q := (0 : Real)) (d := d) (r := t) (by norm_num) ht.1)
  have hmeanModel :
      curveMean (I := I) g γ V t ≤ hypMeanCurv 0 d t := by
    nlinarith
  have hmodel : hypMeanCurv 0 d t ≤ (d : Real) / t := by
    simpa only [add_zero, mul_zero] using
      hypMeanCurv_le (q := (0 : Real)) (r := t) d (by norm_num) ht.1
  ...
  have hJderiv : HasDerivAt (curveDensity (I := I) g γ V)
    ...
    simpa only [curveMean] using
      hasDerivAt_symmDen (I := I) (n := (2 : WithTop ℕ∞)) (by norm_num)
        g γ V t hγ hVdiff
        (curveGram_det_pos (I := I) g γ V t hLI) hW
