...
/-- On a complete Riemannian manifold, the volume of an intrinsic ball depends
continuously on its positive radius. -/
theorem segBall_vol_cont [RiemannianBundle (fun x : M => TangentSpace I x)]
  ...
  have hq_cont : Continuous q := by
    exact Real.continuous_sqrt.comp
      (by simpa only [q] using continuous_gInner_self (I := I) g x)
  ...
  have hlim : ∀ᵐ v ∂modelHaar (E := E),
    ...
    have hq_ne : q v ≠ R := by
      simpa only [level, mem_setOf_eq] using hv
  ...
  have hR_integral :
    ...
    simpa only [F, A, D, q, S, gBall] using harea.symm
