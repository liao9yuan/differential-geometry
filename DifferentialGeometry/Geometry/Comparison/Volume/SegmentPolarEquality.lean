...
/-- Equality in the endpoint transverse-density comparison propagates along the
radial segment and forces equality in both Riccati defects. -/
theorem transDens_eq_rigid
  ...
  have hspeed (t : ℝ) : g.inner (γ t) (curveVelocity (I := I) γ t)
    ...
    calc
      _ = g.inner x v v := by simpa only [γ, curveVelocity] using
  ...
  have hJ (t : ℝ) (i : Fin d) : IsJacobiAt (I := I) g γ (V i) t := by
    simpa only [γ, V] using intrinsic_jacobi (I := I) g hEnorm x (v : E) (w i : E) t
  ...
  have hLI : ∀ t ∈ Set.Ioo (0 : ℝ) 1, LinearIndependent ℝ fun i => V i t := by
    ...
    have hscaled : LinearIndependent ℝ fun i => t • w i := by
      simpa only [Pi.smul_apply] using hwLI.units_smul (fun _ => Units.mk0 t ht.1.ne')
    have hmapped := hscaled.map' L.toLinearMap (LinearMap.ker_eq_bot.mpr hLinj)
    have he : (fun i => V i t) = fun i => L (t • (w i : E)) := by
      funext i
      simpa only [L, V] using intrinsic_jacobi_at (I := I) g hEnorm x (v : E) (w i : E) t
    rwa [he]
