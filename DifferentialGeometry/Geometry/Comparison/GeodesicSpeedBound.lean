...
omit [Module.Finite ℝ E] [NeZero (Module.finrank ℝ E)]
  ...
  [CompleteSpace M] in
/-- A curve with uniformly bounded speed is Cauchy at a finite right endpoint. -/
...
theorem curve_exists_limit_of_bounded_speed
  ...
  haveI : (nhdsWithin b (Set.Iio b)).NeBot := nhdsLT_neBot b
...
omit [Module.Finite ℝ E] [NeZero (Module.finrank ℝ E)]
  [I.Boundaryless] [IsManifold I ∞ M] [T2Space M] [SigmaCompactSpace M]
  [CompleteSpace M] in
/-- A bounded-speed curve whose tail lies in a compact set converges inside that set. -/
