...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Restrict a Calabi tail to a shorter comparison interval. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- A Calabi tail whose total radial reach has a strict radius margin stays in
the corresponding metric ball throughout its comparison interval. -/
theorem CalabiTailData.mem_eball
  ...
  have hseg :
    ...
    simpa only [intrinsicGeodesic_zero, tail.u_norm, sub_zero] using hdist
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- A minimizing Calabi tail can be shortened so that its full comparison
interval stays below any prescribed radius strictly larger than the endpoint
distance. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Build the Calabi upper support from an explicit tail, assuming the Ricci
lower bound only along that tail's intrinsic geodesic. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Produce Calabi upper-support data from a Ricci lower bound on a ball that
strictly contains the endpoint. -/
