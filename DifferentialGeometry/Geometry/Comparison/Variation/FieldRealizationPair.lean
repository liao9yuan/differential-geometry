...
/-- Two smooth tangent fields along one curve are realized by variations made
from a common compactly supported flow. Pointwise equal fields give equal
variation curves, and a zero field value gives a stationary variation curve. -/
theorem exists_var_pair
  ...
  have hXc : IsCompact (tsupport X) := by
    simpa only [X] using hpsic.smul_right
  ...
  have hsmooth (Q : Real → E)
    ...
    simpa only [f] using hproj.comp hlift
  ...
  have hfield (Q : Real → E)
    ...
    have hc0 : c 0 = init Q t := by
      simp only [c, curveAt_zero]
    rw [hc0] at hvel
    simpa only [f, c, init] using hvel
  have hstationary (Q : Real → E) (t : Real) (hQt : Q t = 0) :
    ...
    have hconst : IsMIntegralCurve (fun _ : Real ↦ init Q t) X := by
      intro s
      simpa only [hXt, ContinuousLinearMap.smulRight_zero] using
        (hasMFDerivAt_const (c := init Q t) (x := s)
