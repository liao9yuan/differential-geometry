...
/-!
# Smooth curves contained in a prescribed neighborhood

This file constructs a globally defined smooth curve with prescribed initial
position and velocity whose entire image remains in a given open neighborhood.
-/
...
/-- A tangent vector is the initial velocity of a global smooth curve contained
in any prescribed open neighborhood of its base point. -/
theorem exists_smooth_curve
  ...
  have hline : HasMFDerivAt 𝓘(Real, Real) 𝓘(Real, E)
    ...
    have hsmul : HasDerivAt (fun u : Real => θ u • w) w 0 := by
      simpa using hθ'0.smul_const w
    have hderiv : HasDerivAt (fun u : Real => φ + θ u • w) w 0 := by
      simpa only [zero_add] using (hasDerivAt_const (0 : Real) φ).add hsmul
  ...
  have hsymmD : HasMFDerivAt 𝓘(Real, E) I (extChartAt I y).symm φ
    ...
    have heq : mfderivWithin 𝓘(Real, E) I (extChartAt I y).symm univ φ =
        ContinuousLinearMap.id Real E := by
      rw [← I.range_eq_univ]
      simpa only [φ] using
        (mfderivWithin_range_extChartAt_symm (I := I) (x := y))
    rw [← hasMFDerivWithinAt_univ]
    exact heq ▸ hdiff.hasMFDerivWithinAt
  have hηderiv : HasMFDerivAt 𝓘(Real, Real) I η 0
      ((1 : Real →L[Real] Real).smulRight w) := by
    have hsymmD' : HasMFDerivAt 𝓘(Real, E) I (extChartAt I y).symm
        (φ + θ 0 • w) (ContinuousLinearMap.id Real E) := by
      have hbase : φ + θ 0 • w = φ := by
        rw [hθ0, zero_smul, add_zero]
      rw [hbase]
      exact hsymmD
    have hcomp := hsymmD'.comp 0 hline
    simpa only [η, Function.comp_def, ContinuousLinearMap.id_comp] using hcomp
