...
/-!
# Compactly supported variations in one chart

This file realizes a compactly supported model-space field along a curve as
the variation field of a globally smooth two-parameter variation.  The
construction is local in the manifold chart but global in both real
parameters, using a bounded sine reparametrization in the variation variable.
-/
...
/-- The velocity at zero of a chart-affine slice is the inverse
trivialization of its model-space direction. -/
...
omit [I.Boundaryless] in
/-- A chart formula glued to the original curve off the support of its
model-space field is a smooth variation. -/
...
/-- Every compactly supported smooth model-space field contained in one chart
is the transverse field of a globally smooth variation. -/
  ...
  · intro t
    ...
    · rw [show (fun u : Real => f u t) =
      ...
      have hvel := chartVar_velocity c (extChartAt I c (γ t)) (V t) η
          ...
          (fun u => hmem u t htU) (by simpa only [hleft] using htU)
      rw [hleft] at hvel
      exact hvel
    · have hV0 : V t = 0 := by
