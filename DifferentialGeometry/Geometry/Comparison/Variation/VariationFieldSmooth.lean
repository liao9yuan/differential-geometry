...
/-- A variation that is smooth near `(0, t)` has a smooth variation field at `t`. -/
theorem varField_smoothAt
  ...
  simpa only [fSwap, Function.comp_apply] using hvel.comp t hincl

/-- Pointwise local smoothness near `(0, t)` makes the variation field smooth on a set. -/
...
/-- A globally smooth variation has a globally smooth variation field. -/
