...
/-- A smooth tangent field along a curve, vanishing at two endpoints, is the
variation field of a globally defined smooth fixed-endpoint variation on the
  ...
  have hstationary : ∀ t, V t = 0 → ∀ u,
    ...
    have hconst : IsMIntegralCurve (fun _ : Real => init t) X := by
      ...
      simpa only [hXt, ContinuousLinearMap.smulRight_zero] using
        (hasMFDerivAt_const (c := init t) (x := s)
