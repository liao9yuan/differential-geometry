...
omit [InnerProductSpace ℝ E] [T2Space M] [SigmaCompactSpace M] in
/-- A pointwise `C²` tangent field has a pointwise `C¹` covariant derivative
...
theorem cov_snd_mdiff_at
  ...
  have heq :
    ...
    have hslice : ContMDiffAt (modelWithCornersSelf Real Real) I 2
      ...
      simpa only [F] using hqF.comp q.2 hincl
...
omit [InnerProductSpace ℝ E] [T2Space M] [SigmaCompactSpace M] in
/-- Pointwise `C²` regularity makes the second covariant derivative field
differentiable along the second parameter. -/
theorem cov_snd_diff_at
  ...
  have hslice := hcoord.comp t
    (contDiff_const.prodMk contDiff_id).contDiffAt
  simpa only [chartRepAt] using hslice.differentiableAt (by norm_num)

omit [InnerProductSpace ℝ E] [T2Space M] [SigmaCompactSpace M] in
/-- Pointwise `C²` regularity makes the second covariant derivative field
differentiable along the first parameter. -/
...
omit [InnerProductSpace ℝ E] [SigmaCompactSpace M] in
/-- Covariant derivatives in the two parameter directions commute up to
curvature under pointwise `C²` regularity of the tangent field. -/
theorem cov_commute_at
  ...
  have hF2 : ContDiffAt Real 2
    ...
    have hcomp : ContMDiffAt
        ...
        (fun q : Real × Real ↦ extChartAt I β (f q.1 q.2)) (s, t) := by
      simpa only [F, Function.comp_apply] using hext.comp (s, t) hF
    rw [← contMDiffAt_iff_contDiffAt, modelWithCornersSelf_prod,
      ← chartedSpaceSelf_prod]
    exact hcomp
  have hfixed :=
  ...
  have hcurveL : ContMDiffAt (modelWithCornersSelf Real Real) I 2
    ...
    simpa only [F] using hF.comp s hincl
  have hcurveR : ContMDiffAt (modelWithCornersSelf Real Real) I 2
    ...
    have hincl : ContMDiffAt (modelWithCornersSelf Real Real)
        ...
        (fun v : Real ↦ (s, v)) t :=
      (contMDiff_const.prodMk contMDiff_id).contMDiffAt
    simpa only [F] using hF.comp t hincl
  ...
  have hrepOuterL :
    ...
    have hslice : ContMDiffAt (modelWithCornersSelf Real Real) I 2
      ...
      simpa only [F] using huF.comp t hincl
  ...
  have hrepOuterR :
    ...
    have hslice : ContMDiffAt (modelWithCornersSelf Real Real) I 2
      ...
      simpa only [F] using hvF.comp s hincl
