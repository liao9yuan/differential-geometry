...
theorem intrPull_rm04
    ...
    letI : SigmaCompactSpace (intrPullBall (E := E) R) :=
      isSigmaCompact_iff_sigmaCompactSpace.mp
        (Geometry.isSigmaCompact_of_isOpen
          𝓘(Real, E) (intrPullBall (E := E) R).isOpen)
    Integral.Connection.metricRm04StdAt
        (I := 𝓘(Real, E)) (M := intrPullBall (E := E) R)
        (intrPullMetric (I := I) g hEnorm p hloc) z X Y Z W =
      Integral.Connection.metricRm04StdAt (I := I) (M := M) g
        (intrinsicFramedExp (I := I) g hEnorm p (z : E))
        (mfderiv 𝓘(Real, E) I
...
theorem intrPull_quad_le
    ...
    gPull.inner z
        (Integral.Connection.riemannOp
          (Integral.Connection.LeviCivita (I := 𝓘(Real, E)) gPull)
          z J V V)
        J ≤
  ...
  obtain ⟨basis, hON⟩ :=
    Integral.Connection.exists_gOrthonormalBasis (I := I) g q
  have hquad :=
  ...
  calc
    ...
    _ = Integral.Connection.metricRm04StdAt
          (I := 𝓘(Real, E)) (M := intrPullBall (E := E) R)
          gPull z J V V J := by
    ...
    _ = g.inner q dJ
          (Integral.Connection.riemannOp
            (Integral.Connection.LeviCivita (I := I) g)
            q dJ dV dV) := by
      rw [Integral.Connection.rm04_eq_inner]
    _ = g.inner q
          (Integral.Connection.riemannOp
