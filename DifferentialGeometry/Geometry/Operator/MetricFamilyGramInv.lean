...
/-- A fixed-chart Gram operator is invertible at every regular chart point.
The inverse follows from the metric's positive lower bound, not from supplied
inverse data. -/
...
/-- The inverse fixed-chart Gram operator is jointly smooth on every regular
time-coordinate set inside one chart. -/
...
/-- The inverse fixed-chart Gram operator varies continuously on every regular
time-coordinate set inside one chart. -/
theorem chartGramInv_cont {D : RealTimeInterval}
  ...
  simpa only [Function.comp_apply] using hinv.continuousWithinAt.comp
