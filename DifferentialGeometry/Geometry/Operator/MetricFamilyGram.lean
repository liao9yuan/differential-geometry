...
/-- The Hilbert-space operator representing a metric in one fixed manifold chart. -/
...
/-- The fixed-chart Gram operator represents the metric after transporting both
arguments from the chart model space to the tangent fiber. -/
...
/-- The scalar Gram form is invariant under a change of tangent coordinates. -/
...
/-- Every fixed-chart Gram operator is self-adjoint. -/
...
/-- Every fixed-chart Gram operator has nonnegative quadratic form. -/
...
theorem chartGramOp_cont {D : RealTimeInterval}
  ...
  have hbilin : ContinuousOn
    ...
    exact continuousOn_finset_sum _ fun j _ =>
      continuousOn_finset_sum _ fun k _ => (hentry j k).smul continuousOn_const
  exact (IsCoercive.gramCLM (F := E)).continuous.comp_continuousOn hbilin

/-- Fixed-chart metric operators converge uniformly along uniformly convergent
coordinate curves whose values eventually remain in one compact chart set. -/
theorem chartGramOp_unif {D : RealTimeInterval}
    ...
    (hKc : IsCompact K) {P idx : Type*} [UniformSpace P]
...
/-- Fixed-chart metric operators are uniformly norm-bounded on compact
time-coordinate sets. -/
...
/-- Fixed-chart metric operators have a uniform positive quadratic lower bound
on compact time-coordinate sets contained in the chart target. -/
theorem chartGramOp_lower {D : RealTimeInterval}
  ...
  · letI := hE
    ...
    haveI : ProperSpace E := FiniteDimensional.proper Real E
