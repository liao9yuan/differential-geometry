...
private lemma chartGramOp_eq_sum {D : RealTimeInterval}
  ...
  simpa only [chartGramOnE_def, chartCoord_def] using
    (chartGramBilin_apply (I := I) (M := M) (G.metric z.1) p
      ((extChartAt I p).symm z.2) u v)

/-- The spatial derivative of a fixed-chart metric Gram operator, paired twice
with one vector, is twice its Christoffel contraction pairing. -/
theorem chartGram_spatial {D : RealTimeInterval}
  ...
  have hline : HasDerivAt line w 0 := by
    ...
    simpa only [Pi.add_apply, id_eq, one_smul, zero_add] using
      (hasDerivAt_const (x := (0 : Real)) y).add
        ((hasDerivAt_id (x := (0 : Real))).smul_const w)
