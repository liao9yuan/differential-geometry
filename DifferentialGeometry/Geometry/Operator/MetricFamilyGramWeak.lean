...
/-- Fixed-chart kinetic energy is lower semicontinuous under uniform convergence
of the coordinate curves and weak convergence of their time derivatives. -/
theorem chartKin_liminf {D : RealTimeInterval}
  ...
  have hAcont (n : ℕ) : ContinuousOn (A n) (Icc (0 : Real) L) := by
    ...
    exact (chartGramOp_cont (I := I) hG hJreg alpha hKchart).comp
      (hpair n) fun r hr ↦ ⟨⟨r, hr, rfl⟩, huK n ⟨r, hr⟩⟩
  have hALimCont : ContinuousOn ALim (Icc (0 : Real) L) := by
    apply ContinuousOn.const_smul
    exact (chartGramOp_cont (I := I) hG hJreg alpha hKchart).comp
      hpairLim fun r hr ↦ ⟨⟨r, hr, rfl⟩, huLimK ⟨r, hr⟩⟩
...
/-- A uniform fixed-chart kinetic-energy bound controls the corresponding
`timeH1` norms when all coordinate values remain in one compact chart set. -/
...
/-- A fixed-chart kinetic-energy bound yields a subsequence with weakly
convergent derivatives and uniformly convergent continuous representatives. -/
...
/-- Finitely many fixed-chart kinetic-energy bounds yield one common
subsequence with weakly convergent derivatives and uniformly convergent
continuous representatives in every chart. -/
