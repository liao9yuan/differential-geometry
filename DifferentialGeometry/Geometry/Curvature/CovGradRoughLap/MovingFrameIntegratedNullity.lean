...
/-- **Per-summand integrability of the covariant Leibniz integrand.** For smooth `(r, s)`-tensor
sections `W`, `Z` and a smooth tangent vector field `V`, the combined covariant Leibniz integrand
`⟨∇_V W, Z⟩ + ⟨W, ∇_V Z⟩ + ⟨W, Z⟩ · divᵍ V` is integrable against the Riemannian volume measure — it is
continuous (the directional metric-compatibility tangent action of the smooth inner-product scalar,
plus the product of two smooth scalars) and `M` is closed, so it has compact support. -/
private lemma integrable_covDeriv_leibniz_integrand
      ...
      (fun x : M => tensorInnerPointwise_0s (I := I) (M := M) (r + s) g x
            ...
            + tensorInnerPointwise_0s (I := I) (M := M) (r + s) g x
              (Tensor0SSpace.toModel
                (liftedTensorSection (I := I) (M := M) g r s W.toSection x))
              (Tensor0SSpace.toModel
                (loweredCovDerivAlongVF (I := I) (M := M) g r s Z.toSection V x))) := by
    have hact : Continuous
        (tangentSectionAction (I := I) V
          (tensorInnerScalar (I := I) (M := M) g r s W.toSection Z.toSection)) :=
      (tangentSectionAction_contMDiff (I := I) V hWZ).continuous
    refine hact.congr (fun x => ?_)
    have h := tangentSectionAction_tensorInnerScalar
      (I := I) (M := M) g r s W.toSection Z.toSection V x
    rw [h]
    rfl
  have hC_cont : Continuous
      (fun x : M => tensorInnerScalar (I := I) (M := M) g r s W.toSection Z.toSection x
        * divergence_g (I := I) g V x) :=
    hWZ.continuous.mul (divergence_g_contMDiff (I := I) g V).continuous
  exact (hAB_cont.add hC_cont).integrable_of_hasCompactSupport
    (HasCompactSupport.of_compactSpace _)

/-- **Integrated moving-frame nullity producer (from the raw frame-divergence representation, via the
covariant-IBP engine).** For a closed smooth Riemannian manifold `(M, g)`, covariant rank `s`, smooth
compactly-supported `(0, s)`-tensor `S`, a finite frame-index family `ι`, smooth tangent vector fields
`V i` and smooth `(0, s + 1)`-tensor sections `W i`, and an abstract differentiated-curvature trace
section `Gcd : SmoothCcTensor g 0 (s + 1)`, suppose the moving-frame remainder pairing against
`∇S := covGrad g 0 s S` has the **frame-divergence representation** — its pointwise integrand agrees
almost everywhere with the frame-summed covariant Leibniz integrand

```
⟨Curv S − GcurvSection g s S − Gcd, ∇S⟩(x)
  =ᵐ ∑ᵢ ( ⟨∇_{V i}(W i), ∇S⟩ + ⟨W i, ∇_{V i}(∇S)⟩ + ⟨W i, ∇S⟩ · divᵍ (V i) )(x).
```

Then the **integrated moving-frame nullity** holds: `⟨Curv S − GcurvSection g s S − Gcd, ∇S⟩_{L²} = 0`.

This is the integrated-nullity producer in its most primitive form — the raw structural identity it
consumes is exactly the frame-summed covariant integration by parts the gauge-glued `Gcd` construction
exhibits (the moving-frame / frame-bracket remainder telescopes term-by-term, over the frame and over
the gradient-slot directions, into the covariant Leibniz integrand of honest smooth `∇S`-order
fields). The proof integrates the representation, swaps the finite frame sum past the integral
(per-summand integrability `integrable_covDeriv_leibniz_integrand`), and kills each summand by the
frame-summed covariant-IBP engine `integral_frameSummed_covDeriv_combined_eq_zero` — each combined
Leibniz integrand integrates to zero over the closed manifold, so the frame sum, and hence the
remainder pairing, vanishes. The representation is a genuine mathematical hypothesis (it is *false*
for an arbitrary `Gcd` / arbitrary frame data — it holds exactly when the frame data exhibits the
genuine bracket remainder as a total covariant divergence), not the conclusion. -/
theorem movingFrameNullity_of_frameDivergenceDatum
    (g : SmoothRiemannianMetric I M) (s : ℕ) (S : SmoothCcTensor g 0 s)
    {ι : Type*} [Fintype ι]
    (V : ι → Cₛ^∞⟮I; E, (TangentSpace I : M → Type _)⟯)
    (W : ι → SmoothCcTensor g 0 (s + 1)) (Gcd : SmoothCcTensor g 0 (s + 1))
    (hrep : (fun x : M => tensorInnerPointwise (I := I) (M := M) g 0 (s + 1) x
              ((pointwiseTensorCurv (I := I) (M := M) g s S -
                  GcurvSection (I := I) (M := M) g s S - Gcd).toFun x)
              ((covGrad (I := I) (M := M) g 0 s S).toFun x))
        =ᵐ[riemannianVolumeMeasure (I := I) (M := M) g]
      (fun x : M => ∑ i, (tensorInnerPointwise_0s (I := I) (M := M) (0 + (s + 1)) g x
              (Tensor0SSpace.toModel
  ...
  rw [MeasureTheory.integral_congr_ae hrep]
  rw [MeasureTheory.integral_finset_sum (Finset.univ)
    (fun i _ => integrable_covDeriv_leibniz_integrand (I := I) (M := M) g 0 (s + 1)
      (W i) (covGrad (I := I) (M := M) g 0 s S) (V i))]
  exact integral_frameSummed_covDeriv_combined_eq_zero (I := I) (M := M) g 0 (s + 1)
