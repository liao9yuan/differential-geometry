...
set_option linter.unusedSectionVars false in
/-- **The frame-summed differentiated-curvature trace value of the differentiated-curvature section
(the differentiated analog of `pureRGenuineDiffOp_zero_succ_toSection_unit_eval`, the posited
moving-frame value primitive).**  For a closed smooth Riemannian manifold `(M, g)`, covariant rank
`s`, smooth compactly-supported `(0, s)`-tensor `S`, smooth read direction `Vb`, point `x`, and
trailing tuple `m`, the slot-`0` `Vb`-read of the differentiated-curvature trace section
`genuineDiffCurvSection g s S = appCc (covGrad (Φ₀ s)) S` (the `(∇R) S` field) is the orthonormal
moving-frame trace (over `Bᵢ := smoothOrthoFrame g x i`, orthonormal at `x`) of the per-frame
differentiated-curvature carrier `R(∇_{Bᵢ} Vb, Bᵢ) S + R(Vb, ∇_{Bᵢ} Bᵢ) S + ∇_{Bᵢ}(R(Bᵢ, Vb) S)`
read on the unit:
```
toModel((∇R) S)(x)(unit)(cons (Vb x) m)
  = ∑ᵢ toModel( R(∇_{Bᵢ} Vb, Bᵢ) S + R(Vb, ∇_{Bᵢ} Bᵢ) S + ∇_{Bᵢ}(R(Bᵢ, Vb) S) )(x)(unit)(m).
```
This is the covariant-derivative moving-frame trace of the curvature operator on the gradient slot:
where `pureRGenuineDiffOp_zero_succ_toSection_unit_eval` reads the order-`0` curvature trace
`∑ᵢ R(Bᵢ, ·)(slot0_{Bᵢ} S)` of the *frozen* frame, the covariant derivative `covGrad (Φ₀ s)` of the
curvature operator field adds one `∇` and splits, by the section-level B-rule inversion
`covGrad_pureRGenuineDiffOp_unit_eval_eq_genuineDiffCurv_add_spectator`, into the genuine
differentiated-curvature trace `∑ᵢ (∇R)(Bᵢ, Bᵢ, Vb) S` (the two frame-derivative curvature carriers
`R(∇_{Bᵢ} Vb, Bᵢ) S + R(Vb, ∇_{Bᵢ} Bᵢ) S`) plus the group-`2` connection-correction
`∑ᵢ ∇_{Bᵢ}(R(Bᵢ, Vb) S)`.  The expansion is frame-balanced: the curvature direction `Bᵢ` and the
covariant-derivative direction `Bᵢ` are the *same* trace direction (diagonal), summed against the
fixed read direction `Vb`, mirroring the proven order-`0` value form — never a linear-in-frame
nullity.

**Posited (frame-summed second-Bianchi Leibniz primitive).**  The frame-free per-direction value of
the carrier is `covGrad_pureRGenuineDiffOp_unit_eval_eq_genuineDiffCurv_add_spectator`
(`∇_{Vb}(pureR-trace) − Φ₀(∇_{Vb} S)`, sorry-free); its identification with the diagonal *moving*
orthonormal-frame trace of the per-frame differentiated curvature is the pointwise frame-summed
covariant Leibniz / second-Bianchi reading of `∇_{Vb}(∑ᵢ R(Bᵢ, ·) slot0_{Bᵢ} S)`, which the library
otherwise carries only in integrated form.  It is the differentiated companion of the posited
order-`0` curvature-coefficient base spec `exists_pureRGenuineDiffOp_base_appCc`; consumers transitively
depend on `sorryAx` through it. -/
theorem genuineDiffCurvSection_slot0_unit_eval_frameTrace
    (g : SmoothRiemannianMetric I M) (s : ℕ) (S : SmoothCcTensor g 0 s)
        ...
        (tensor0S_curry (I := I) (M := M) (𝕜 := ℝ) s x
          ((show Tensor0SSpace 0 I x →L[ℝ] Tensor0SSpace (s + 1) I x from
            (appCc (I := I) (M := M) g s (s + 1)
              (covGrad (I := I) (M := M) g s s
                (curvOpField (I := I) (M := M) g s)) S).toSection x)
        ...
        Tensor0SSpace.toModel
          ((show Tensor0SSpace 0 I x →L[ℝ] Tensor0SSpace s I x from
              riemannOp (tensorCov (I := I) g 0 s) x
                ((LeviCivita (I := I) g).toFun Vb x (smoothOrthoFrame (I := I) g x i x))
              ...
              covApply (tensorCov (I := I) g 0 s) (smoothOrthoFrame (I := I) g x i)
                (fun y : M => riemannSec (tensorCov (I := I) g 0 s)
                  (smoothOrthoFrame (I := I) g x i) Vb (fun z : M => S.toSection z) y) x)
            (unitZeroSec (I := I) (M := M) x))
          m := by
  sorry
