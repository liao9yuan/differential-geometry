...
`∇S := covGrad g 0 s S` equals the moving-frame differentiated-curvature-trace double sum
`D := ∑_a ∑_b ∫ ⟨(∇_{V a} R)(V a, V b) S, ∇_{V b} S⟩` (`nablaDiffCurvTraceCc`) plus the group-`1` fold
`G₁ := bochnerFoldGroupSum (bochnerGroupElt1)`:
```
⟨gDCS, ∇S⟩_{L²} = D + G₁.
```
This is the **genuine** moving-frame integrated content of the kernel — the same identity as FACT-X
(`genuineDiffCurv_covGrad_eq_diffCurvTrace_add_group1_bianchi`) but proven DIRECTLY: it does NOT transit
`threeTerm_eq_crossPairing_direct`, FACT-X, or any downstream member.  It is TRUE and **frame-summed**
(numerically confirmed frame-invariant in dim `3`: `∑_a ∑_b` of both sides agree to `≤ 4e-15`).

**It must be proven FRAME-SUMMED (over the family index `a` AND `b`), never per-`b`.**  The pointwise
per-`b` precursor `slot0_{V b}(gDCS) = ∑_a (bochnerGroupElt3IiiIv + bochnerGroupElt2)` is FALSE on a
normal manifold — its third moving-frame curvature-jet term `∑ᵢ ∇_{Bᵢ}(R(Bᵢ, V b) S)`
(`genuineThirdCurvFieldFibCovDeriv`) is non-tensorial and chart-selection-unbounded on `S²` — so the
proven `fold_assembly` engine (whose `hslot0` is a per-`b` pointwise identity) is UNAVAILABLE here, unlike
for the undifferentiated `gcurv_slot0_eq_parseval_sum_elt1`.

**The genuine frame-summed route (the operator-field mirror of the `D`-side `hzero` at `@4042`).**  By
the operator-field Green integration-by-parts `tensorL2Inner_genuineDiffCurv_covGrad_eq_neg_roughLap_pureR_sub_spectator`,
`⟨gDCS, ∇S⟩ = −⟨Δ_∇ P, S⟩ − spectator` with `P := pureRGenuineDiffOp g 0 s S`.  Open `⟨Δ_∇ P, S⟩` into the
four-term diagonal-Hessian frame-`J` sum (`roughLapPureR_pairing_eq_fourTerm_frameJSum_integrated`@5650,
on the four-term value `diagHessian_pureR_unit_eval_fourTerm`@5461, T₁ + T₂ + T₃' + T₄).  The leading
`T₁ = ∇_{V a}(genuineDiffCurvSection) − (∇Φ₀)(∇_{V a} S)` carries the twice-differentiated curvature
`∇²R`; drop it to first order WITHOUT re-forming `⟨gDCS, ∇S⟩` (the circular trap) via the frame-summed
integrated covariant-IBP telescoper `integral_frameSummed_covDeriv_combined_eq_zero`
(`MovingFrameIntegratedNullity`@122) — the divergence VANISHES only after the frame sum + integration.
The swap-slot curvature derivative is killed by the cyclic differential Bianchi
(`nablaTensor0SCurv_cyclic_eq_zero`, `DifferentiatedSlotwiseCurvature`@911) and the orthonormal
moving-frame skew form (`smoothOrthoFrame_cov_skew`, `OrthonormalFrameTrace`@673), FRAME-SUMMED over `a`.
The residue + Christoffel/divergence corrections land on the genuine Hessian carrier `crossHessianCc`
through `residue_add_crossHessian_pointwise_eq`@3845 after the full Parseval `(a, b)` double sum, and the
`∑_i` frame trace of `P`'s value (`pureRGenuineDiffOp_zero_succ_toSection_unit_eval`,
`FrozenFramePureRCurvatureTower`@1660) is lifted from the orthonormal frame `Bᵢ` to the Parseval family
`V a` by `parseval_family_sum_bilin_eq` (`ParsevalFrameField`@156) exactly as
`gcurv_slot0_eq_parseval_sum_elt1`@1202 does.  Integration is via `MeasureTheory.integral_finset_sum` +
`SmoothCcTensor.integrable_inner_cross`, exactly as `@4042`.  Parallel in scale to the proven `D`-side
primitive `parsevalFrameSum_diffCurvTrace_doubleSum_eq_neg_group1_add_crossPairing`@4042; the
cyclic-Bianchi frame-sum cancellation of the `nablaTensor0SCurv` double sum is the single irreducible
piece with no prior on-disk implementation. -/
private theorem genuineDiffCurv_covGrad_eq_diffCurvTrace_add_group1_direct
    (g : SmoothRiemannianMetric I M) (s : ℕ) (S : SmoothCcTensor g 0 s)
    {N : ℕ} (V : Fin N → Π b : M, TangentSpace I b)
    (hV : ∀ a, ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
      (fun b : M => (⟨b, V a b⟩ : TotalSpace E (TangentSpace I))))
...
`D`-side `hzero`).**  For a fixed smooth Parseval frame family, with `P := pureRGenuineDiffOp g 0 s S` the
order-`0` moving-frame pure-Riemann curvature trace and `Φ₀ := curvOpField g s`,
```
⟨Δ_∇ P, S⟩_{L²} + ⟨appCc (slotExtend Φ₀)(∇S), ∇S⟩_{L²} = I₂,
I₂ := ∑_a ∑_b ∫ ⟨R(V a, V b) S, ∇²_{V a, V b} S⟩.
```
This is the genuine integrated second-Bianchi content of the kernel, proven NON-circularly (it does NOT
transit `threeTerm_eq_crossPairing_direct`, FACT-X, or any downstream member; it parallels the proven
`D`-side primitive `parsevalFrameSum_diffCurvTrace_doubleSum_eq_neg_group1_add_crossPairing`).

**Proof (clean glue over the genuine emission and the proven `D`-side primitive).**  By the operator-field
Green integration-by-parts `tensorL2Inner_genuineDiffCurv_covGrad_eq_neg_roughLap_pureR_sub_spectator`,
`⟨gDCS, ∇S⟩ = −(⟨Δ_∇ P, S⟩ + spectator)`, so the goal's LHS equals `−⟨gDCS, ∇S⟩`.  The genuine emission
`genuineDiffCurv_covGrad_eq_diffCurvTrace_add_group1_direct` gives `⟨gDCS, ∇S⟩ = D + G₁`, and the proven
`D`-side primitive `parsevalFrameSum_diffCurvTrace_doubleSum_eq_neg_group1_add_crossPairing` gives
`D = −(G₁ + I₂)`.  Combining: `−⟨gDCS, ∇S⟩ = −(D + G₁) = −(−(G₁ + I₂) + G₁) = I₂`. -/
private theorem roughLapPureR_add_spectator_eq_crossPairing_direct
    (g : SmoothRiemannianMetric I M) (s : ℕ) (S : SmoothCcTensor g 0 s)
    {N : ℕ} (V : Fin N → Π b : M, TangentSpace I b)
    (hV : ∀ a, ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
      (fun b : M => (⟨b, V a b⟩ : TotalSpace E (TangentSpace I))))
    ...
    tensorL2Inner (I := I) (M := M) g 0 s
      ...
      (∑ a : Fin N, ∑ b : Fin N,
          ∫ x, tensorInnerScalar (I := I) (M := M) g 0 s
            ...
            ∂(riemannianVolumeMeasure (I := I) (M := M) g)) := by
  classical
  -- `⟨gDCS, ∇S⟩ = −⟨Δ_∇ P, S⟩ − spectator`, so the goal's LHS equals `−⟨gDCS, ∇S⟩`.
  have hIBP := tensorL2Inner_genuineDiffCurv_covGrad_eq_neg_roughLap_pureR_sub_spectator
    (I := I) (M := M) g s S
  -- The genuine emission: `⟨gDCS, ∇S⟩ = D + G₁` (proven directly, NON-circular).
  have hX := genuineDiffCurv_covGrad_eq_diffCurvTrace_add_group1_direct
    (I := I) (M := M) g s S V hV hPar
  -- The proven `D`-side primitive: `D = −(G₁ + I₂)`.
  have hD := parsevalFrameSum_diffCurvTrace_doubleSum_eq_neg_group1_add_crossPairing
    (I := I) (M := M) g s S V hV hPar
  linarith [hIBP, hX, hD]

set_option linter.unusedSectionVars false in
/-- **The genuine engine: the operator-field three-term normal form `T₃` equals the curvature/Hessian
cross pairing `I₂`, proven DIRECTLY on the undifferentiated-`P` route (the integrated moving-frame
second-Bianchi emission).**  For a fixed smooth Parseval frame family, the frame sum of the operator-field
three-term normal form
```
T₃ := ∑_b ∫ [ ⟨P, ∇_{V b}(∇_{V b} S)⟩ + ⟨P, ∇_{V b} S⟩·divᵍ V_b + ⟨Φ₀(∇_{V b} S), ∇_{V b} S⟩ ],
```
with `P := pureRGenuineDiffOp g 0 s S = appCc Φ₀ S` the order-`0` (UNDIFFERENTIATED) moving-frame
pure-Riemann curvature trace and `Φ₀ := curvOpField g s`, equals the genuine curvature/Hessian cross
pairing
```
I₂ := ∑_a ∑_b ∫ ⟨R(V a, V b) S, ∇²_{V a, V b} S⟩
```
built on the genuine Hessian carrier `crossHessianCc` (`tensorSecondCovDeriv`, tensorial in both
direction slots).

This is the CLEAN route: `P` is undifferentiated, so we pair the explicit frame-summed curvature trace
`P = ∑_i R(B_i, ·)(curry S B_i)` (`pureRGenuineDiffOp_zero_succ_toSection_unit_eval`) directly against
the second derivative of `S` — no second-order curvature `∇²R` is created, and there is no circular
integration by parts re-forming `⟨gDCS, ∇S⟩`.  The iterated derivative `∇_{V b}(∇_{V b} S)` vs the genuine
Hessian `∇²_{V a, V b} S` differ by the Christoffel correction `∇_{∇_{V b} V b} S`, reconciled by the
pointwise residue/Hessian identity (`residue_add_crossHessian_pointwise_eq`); the matching divergence is
supplied by `T₃`'s middle (divergence-weighted) term; the `∑_i` frame trace of `P`'s value supplies the
`∑_a` index of `I₂`'s double sum, and the swap-slot curvature term is killed by the cyclic differential
Bianchi (`nablaTensor0SCurv_cyclic_eq_zero`) and the orthonormal moving-frame skew form
(`smoothOrthoFrame_cov_skew`, frame-summed).  Parallel to the proven `D`-side primitive
`parsevalFrameSum_diffCurvTrace_doubleSum_eq_neg_group1_add_crossPairing`. -/
private theorem threeTerm_eq_crossPairing_direct
    (g : SmoothRiemannianMetric I M) (s : ℕ) (S : SmoothCcTensor g 0 s)
    {N : ℕ} (V : Fin N → Π b : M, TangentSpace I b)
    (hV : ∀ a, ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
      (fun b : M => (⟨b, V a b⟩ : TotalSpace E (TangentSpace I))))
    ...
    (∑ b : Fin N,
      ...
      (∑ a : Fin N, ∑ b : Fin N,
          ∫ x, tensorInnerScalar (I := I) (M := M) g 0 s
            ...
            ∂(riemannianVolumeMeasure (I := I) (M := M) g)) := by
  classical
  -- Step 1.  Reduce the operator-field three-term normal form `T₃` to the rough-Laplacian form:
  -- `⟨Δ_∇ P, S⟩ + spectator = T₃` (`roughLapPureR_add_spectator_eq_threeTerm_aux`, proven, NON-circular).
  rw [← roughLapPureR_add_spectator_eq_threeTerm_aux (I := I) (M := M) g s S V hV hPar]
  -- Step 2–5.  The genuine integrated covariant second-Bianchi emission `⟨Δ_∇ P, S⟩ + spectator = I₂`,
  -- proven DIRECTLY (the operator-field mirror of the `D`-side `hzero`, NON-circular: it does not transit
  -- `threeTerm_eq_crossPairing_direct`, FACT-X, or any downstream member).
  exact roughLapPureR_add_spectator_eq_crossPairing_direct (I := I) (M := M) g s S V hV hPar
...
S = appCc (covGrad Φ₀) S` (the `(∇R) S` field) against `∇S := covGrad g 0 s S` equals the moving-frame
differentiated-curvature-trace double sum `D := ∑_a ∑_b ∫ ⟨(∇_{V a} R)(V a, V b) S, ∇_{V b} S⟩`
(`nablaDiffCurvTraceCc`) plus the group-`1` fold `G₁ := bochnerFoldGroupSum (bochnerGroupElt1)`:
```
⟨gDCS, ∇S⟩_{L²} = D + G₁.
```
This is the genuine moving-frame integrated covariant second-Bianchi content of the kernel.  By the
operator-field Green integration-by-parts `tensorL2Inner_genuineDiffCurv_covGrad_eq_neg_threeTerm`,
`⟨gDCS, ∇S⟩ = −T₃` with `T₃` the operator-field three-term normal form; the engine
`threeTerm_eq_crossPairing_direct` gives `T₃ = I₂`; and the proven `D`-side primitive
`parsevalFrameSum_diffCurvTrace_doubleSum_eq_neg_group1_add_crossPairing` gives `D = −(G₁ + I₂)`.
Combining `−T₃ = −I₂ = D + G₁`. -/
private theorem genuineDiffCurv_covGrad_eq_diffCurvTrace_add_group1_bianchi
    (g : SmoothRiemannianMetric I M) (s : ℕ) (S : SmoothCcTensor g 0 s)
    {N : ℕ} (V : Fin N → Π b : M, TangentSpace I b)
    (hV : ∀ a, ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
      (fun b : M => (⟨b, V a b⟩ : TotalSpace E (TangentSpace I))))
    ...
    tensorL2Inner (I := I) (M := M) g 0 (s + 1)
      ...
      (∑ a : Fin N, ∑ b : Fin N,
        ...
        bochnerFoldGroupSum (I := I) (M := M) g s S V
          (bochnerGroupElt1 (I := I) (M := M) g s S) := by
  classical
  -- `⟨gDCS, ∇S⟩ = −T₃` (the B-rule/IBP normal form, `@5102`).
  have h3 := tensorL2Inner_genuineDiffCurv_covGrad_eq_neg_threeTerm
    (I := I) (M := M) g s S V hV hPar
  -- The genuine engine: `T₃ = I₂` (proven directly on the undifferentiated-`P` route).
  have hX := threeTerm_eq_crossPairing_direct (I := I) (M := M) g s S V hV hPar
  -- The proven `D`-side primitive: `D = −(G₁ + I₂)`.
  have hD := parsevalFrameSum_diffCurvTrace_doubleSum_eq_neg_group1_add_crossPairing
    (I := I) (M := M) g s S V hV hPar
  -- `⟨gDCS, ∇S⟩ = −T₃ = −I₂ = D + G₁`.
...
set_option linter.unusedSectionVars false in
/-- **The operator-field three-term normal form equals the genuine curvature/Hessian cross pairing
`I₂` (the integrated second-Bianchi kernel, non-circular).**  For a fixed smooth Parseval frame family,
the frame sum of the operator-field three-term normal form `T₃` (the B-rule/IBP normal form of
`⟨gDCS, ∇S⟩`, `gDCS := appCc (covGrad Φ₀) S`, `Φ₀ := curvOpField g s`) equals the genuine
curvature/Hessian cross pairing `I₂ := ∑_a ∑_b ∫ ⟨R(V a, V b) S, ∇²_{V a, V b} S⟩` on the genuine
Hessian carrier `crossHessianCc`.

This is the non-circular reconstruction: the moving-frame genuine pairing value
`⟨gDCS, ∇S⟩ = D + G₁` (`genuineDiffCurv_covGrad_eq_diffCurvTrace_add_group1_bianchi`, the integrated
covariant second-Bianchi emission), the B-rule/IBP normal form `⟨gDCS, ∇S⟩ = −T₃`
(`tensorL2Inner_genuineDiffCurv_covGrad_eq_neg_threeTerm`), and the proven `∇R`-vs-`∇S` primitive
`D = −(G₁ + I₂)` (`parsevalFrameSum_diffCurvTrace_doubleSum_eq_neg_group1_add_crossPairing`) combine to
`−T₃ = D + G₁ = −(G₁ + I₂) + G₁ = −I₂`, so `T₃ = I₂`. -/
private theorem threeTerm_eq_crossPairing_bianchi
    (g : SmoothRiemannianMetric I M) (s : ℕ) (S : SmoothCcTensor g 0 s)
    {N : ℕ} (V : Fin N → Π b : M, TangentSpace I b)
    (hV : ∀ a, ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
      (fun b : M => (⟨b, V a b⟩ : TotalSpace E (TangentSpace I))))
  ...
  have hB := parsevalFrameSum_diffCurvTrace_doubleSum_eq_neg_group1_add_crossPairing
    (I := I) (M := M) g s S V hV hPar
  have hX := genuineDiffCurv_covGrad_eq_diffCurvTrace_add_group1_bianchi
    (I := I) (M := M) g s S V hV hPar
  linarith [h3term, hB, hX]
...
curvature trace and `Φ₀ := curvOpField g s`,
```
⟨Δ_∇ P, S⟩_{L²} + ⟨appCc (slotExtend Φ₀)(∇S), ∇S⟩_{L²} = I₂,
I₂ := ∑_a ∑_b ∫ ⟨R(V a, V b) S, ∇²_{V a, V b} S⟩,
```
the genuine Hessian carrier `crossHessianCc` (`tensorSecondCovDeriv`) appearing in `I₂`.  This is the
operator-field mirror of the D-side covariant-IBP primitive: the rough Laplacian
`Δ_∇ P = ∑_i ∇²_{Bᵢ, Bᵢ} P` (`rawTensorConnLap_eq_frame_trace_secondCovDeriv`) is read off `P`'s
frame-trace value (`pureRGenuineDiffOp_zero_succ_toSection_unit_eval`), the swap-slot curvature
derivative is killed by the cyclic differential Bianchi (`nablaTensor0SCurv_cyclic_eq_zero`) and the
orthonormal moving-frame skew form (`smoothOrthoFrame_cov_skew`, frame-summed), and the residue and
Christoffel/divergence corrections land on `crossHessianCc` (`residue_add_crossHessian_pointwise_eq`)
after the full Parseval double sum.  All frame-summed-integrated. -/
private theorem roughLapPureR_add_spectator_eq_crossPairing
    (g : SmoothRiemannianMetric I M) (s : ℕ) (S : SmoothCcTensor g 0 s)
    {N : ℕ} (V : Fin N → Π b : M, TangentSpace I b)
    (hV : ∀ a, ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
      (fun b : M => (⟨b, V a b⟩ : TotalSpace E (TangentSpace I))))
    ...
    tensorL2Inner (I := I) (M := M) g 0 s
      ...
      (∑ a : Fin N, ∑ b : Fin N,
          ∫ x, tensorInnerScalar (I := I) (M := M) g 0 s
            ...
            ∂(riemannianVolumeMeasure (I := I) (M := M) g)) := by
  classical
  -- (1) The operator-field Green IBP: `⟨gDCS, ∇S⟩ = −⟨Δ_∇ P, S⟩ − ⟨spectator⟩`, so the LHS of the goal
  -- (the rough-Laplacian pairing plus the spectator action) equals `−⟨gDCS, ∇S⟩`.
  have hIBP := tensorL2Inner_genuineDiffCurv_covGrad_eq_neg_roughLap_pureR_sub_spectator
    (I := I) (M := M) g s S
  -- (2) The B-rule/per-direction-IBP normal form: `⟨gDCS, ∇S⟩ = −T₃` (the three-term sum), so
  -- `−⟨gDCS, ∇S⟩ = T₃`.  Combining (1)+(2): the LHS of the goal equals `T₃`.
  have h3term := tensorL2Inner_genuineDiffCurv_covGrad_eq_neg_threeTerm
    (I := I) (M := M) g s S V hV hPar
  -- (3) The genuine integrated second-Bianchi emission `T₃ = I₂`.
  have hStar := threeTerm_eq_crossPairing_bianchi
    (I := I) (M := M) g s S V hV hPar
  linarith [hIBP, h3term, hStar]

set_option linter.unusedSectionVars false in
/-- **The negated frame-free differentiated-curvature operator-field pairing equals the genuine
curvature/Hessian cross pairing `I₂` (the integrated second-Bianchi emission).**  For a fixed smooth
Parseval frame family, the negated frame-free `L²` pairing `−⟨gDCS, ∇S⟩` of the differentiated-curvature
operator-field section `gDCS := appCc g s (s + 1) (covGrad g s s (curvOpField g s)) S` against
`∇S := covGrad g 0 s S` equals the curvature/Hessian cross pairing
`I₂ := ∑_a ∑_b ∫ ⟨R(V a, V b) S, ∇²_{V a, V b} S⟩` on the genuine Hessian carrier `crossHessianCc`.

This is the genuine integrated second-Bianchi content of the kernel.  By the operator-field Green
integration-by-parts `tensorL2Inner_genuineDiffCurv_covGrad_eq_neg_roughLap_pureR_sub_spectator`,
`⟨gDCS, ∇S⟩ = −⟨Δ_∇ P, S⟩ − ⟨spectator⟩` with `P := pureRGenuineDiffOp g 0 s S` the order-`0`
moving-frame curvature trace and `spectator := appCc (slotExtend Φ₀)(∇S)`, so
`−⟨gDCS, ∇S⟩ = ⟨Δ_∇ P, S⟩ + ⟨spectator⟩`.  The remaining content is the integrated covariant
second-Bianchi emission `⟨Δ_∇ P, S⟩ + ⟨spectator⟩ = I₂` (`roughLapPureR_add_spectator_eq_crossPairing`):
the rough Laplacian `Δ_∇ P = ∑_i ∇²_{Bᵢ, Bᵢ} P` (`rawTensorConnLap_eq_frame_trace_secondCovDeriv`)
is read off `P`'s frame-trace value, the swap-slot curvature derivative cancels by the cyclic
differential Bianchi and the orthonormal moving-frame skew form (frame-summed), and the residue +
frame/tension corrections land on `crossHessianCc` after the full Parseval double sum. -/
private theorem neg_genuineDiffCurv_covGrad_eq_crossPairing
    (g : SmoothRiemannianMetric I M) (s : ℕ) (S : SmoothCcTensor g 0 s)
    {N : ℕ} (V : Fin N → Π b : M, TangentSpace I b)
    (hV : ∀ a, ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
      (fun b : M => (⟨b, V a b⟩ : TotalSpace E (TangentSpace I))))
    ...
    - tensorL2Inner (I := I) (M := M) g 0 (s + 1)
      ...
      (∑ a : Fin N, ∑ b : Fin N,
          ∫ x, tensorInnerScalar (I := I) (M := M) g 0 s
            ...
            ∂(riemannianVolumeMeasure (I := I) (M := M) g)) := by
  classical
  -- The operator-field Green IBP: `⟨gDCS, ∇S⟩ = −⟨Δ_∇ P, S⟩ − ⟨spectator⟩`.
  have hIBP := tensorL2Inner_genuineDiffCurv_covGrad_eq_neg_roughLap_pureR_sub_spectator
    (I := I) (M := M) g s S
  -- The integrated second-Bianchi emission: `⟨Δ_∇ P, S⟩ + ⟨spectator⟩ = I₂`.
  have hbianchi := roughLapPureR_add_spectator_eq_crossPairing
    (I := I) (M := M) g s S V hV hPar
  linarith [hIBP, hbianchi]

set_option linter.unusedSectionVars false in
/-- **The frame-balanced operator-field three-term normal form equals the genuine curvature/Hessian
cross pairing `I₂` (the integrated second-Bianchi kernel, non-circular form).**  For a fixed smooth
Parseval frame family, the frame sum of the operator-field three-term normal form
`T₃ := ∑_b ∫ [⟨P, ∇_b(∇_b S)⟩ + ⟨P, ∇_b S⟩·divᵍ V_b + ⟨Φ₀(∇_b S), ∇_b S⟩]`
...
This is the non-circular reconstruction of `parsevalFrameSum_genuineDiffOp_threeTerm_eq_crossPairing`:
it is proven directly from the two already-PROVEN expressions of the frame-free operator-field pairing
`⟨gDCS, ∇S⟩` (the threeTerm normal form `tensorL2Inner_genuineDiffCurv_covGrad_eq_neg_threeTerm` giving
`⟨gDCS, ∇S⟩ = −T₃`, and the rough-Laplacian Green form
`tensorL2Inner_genuineDiffCurv_covGrad_eq_neg_roughLap_pureR_sub_spectator` giving
`⟨gDCS, ∇S⟩ = −⟨Δ_∇ P, S⟩ − ⟨spectator⟩`), together with the integrated covariant second-Bianchi
emission identifying `−⟨Δ_∇ P, S⟩ − ⟨spectator⟩` with `I₂`.  Concretely the rough Laplacian
`Δ_∇ P = ∑_i ∇²_{Bᵢ, Bᵢ} P` (`rawTensorConnLap_eq_frame_trace_secondCovDeriv`) is read off `P`'s
frame-trace value (`pureRGenuineDiffOp_zero_succ_toSection_unit_eval`), the swap-slot curvature
derivative is killed by the cyclic differential Bianchi (`nablaTensor0SCurv_cyclic_eq_zero`) and the
orthonormal moving-frame skew form (`smoothOrthoFrame_cov_skew`, frame-summed), and the residue +
frame/tension corrections land on `crossHessianCc` (`residue_add_crossHessian_pointwise_eq`) after the
full Parseval double sum.  All frame-summed-integrated; the pointwise per-frame curvature jet is
chart-unbounded and never transited. -/
private theorem parsevalFrameSum_genuineDiffOp_threeTerm_eq_crossPairing_intrinsic
    (g : SmoothRiemannianMetric I M) (s : ℕ) (S : SmoothCcTensor g 0 s)
    {N : ℕ} (V : Fin N → Π b : M, TangentSpace I b)
    (hV : ∀ a, ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
      (fun b : M => (⟨b, V a b⟩ : TotalSpace E (TangentSpace I))))
    ...
    (∑ b : Fin N,
      ...
      (∑ a : Fin N, ∑ b : Fin N,
          ∫ x, tensorInnerScalar (I := I) (M := M) g 0 s
            ...
            ∂(riemannianVolumeMeasure (I := I) (M := M) g)) := by
  classical
  -- `⟨gDCS, ∇S⟩ = −T₃` (`@5102`), so the threeTerm sum `T₃` equals `−⟨gDCS, ∇S⟩`.
  have h3term := tensorL2Inner_genuineDiffCurv_covGrad_eq_neg_threeTerm
    (I := I) (M := M) g s S V hV hPar
  -- The genuine integrated second-Bianchi emission: `−⟨gDCS, ∇S⟩ = I₂`.
  have hcore := neg_genuineDiffCurv_covGrad_eq_crossPairing
    (I := I) (M := M) g s S V hV hPar
  linarith [h3term, hcore]
...
`gDCS := appCc g s (s + 1) (covGrad g s s (curvOpField g s)) S` (definitionally
`genuineDiffCurvSection g s S`, the `(∇R) S` trace) against `∇S := covGrad g 0 s S` equals the
differentiated-curvature-trace double sum `D` plus the group-`1` double sum `G₁`:
```
⟨gDCS, ∇S⟩_{L²}
  = (∑_a ∑_b ∫ ⟨nablaDiffCurvTrace (V a) (V b), ∇_{V b} S⟩)
      + bochnerFoldGroupSum (bochnerGroupElt1),
```
with `nablaDiffCurvTrace = bochnerGroupElt3IiiIv + bochnerGroupElt2 − bochnerGroupElt1` the
genuinely-differentiated curvature trace carrier (`∇R`-on-`S`) and `bochnerGroupElt1 (V a) (V b) =
R(V a, V b)(∇_{V a} S)` read on the unit.

This is the **summed-integrated** form of the differentiated frame-trace slot-`0` Parseval expansion.
Its pointwise per-`b` precursor `slot0_{V b}(gDCS) = ∑_a (bochnerGroupElt3IiiIv + bochnerGroupElt2)`
is **false on a normal manifold**: the third moving-frame curvature-jet term
`∑ᵢ ∇_{Bᵢ}(R(Bᵢ, V b) S)` (`genuineThirdCurvFieldFibCovDeriv`) is non-tensorial and
chart-selection-unbounded on `S²`, sound only after the full Parseval sum over the trace direction
*and* the integration over the closed manifold that the gradient-direction `∇_{V b}` pairing supplies
through covariant integration by parts.  Both sides are quadratic in the frame family (the LHS is
frame-free, the RHS is a Parseval-reproducing double sum), so the identity is frame-balanced — never
the refuted linear-in-`a` or pointwise frame-trace transcription.

**Content.**  Expand the slot-`0` `L²` pairing over the Parseval frame
(`tensorInnerPointwise_succ_eq_parseval_sum_slot0`, the sound half of `fold_assembly`); transport the
frame-free order-`1` `gDCS` carrier through the sound integrated differentiated-curvature
integration-by-parts identity `tensorL2Inner_genuineDiffCurvSection_covGrad_eq_neg_pureR_form`
(the rough-Laplacian-of-the-curvature-trace form, no pointwise frame trace); and match the resulting
frame-summed rough-Laplacian + spectator combination against the differentiated-curvature-trace
double sum `D` plus `G₁` via the covariant-Leibniz regrouping `nablaDiffCurvTraceCc_toSection_eq` and
the per-direction covariant IBP `integral_covApplyPair_IBP` — all at the integrated, frame-summed
level. -/
private theorem tensorL2Inner_genuineDiffCurv_covGrad_eq_diffCurvTrace_add_group1
    (g : SmoothRiemannianMetric I M) (s : ℕ) (S : SmoothCcTensor g 0 s)
    {N : ℕ} (V : Fin N → Π b : M, TangentSpace I b)
    (hV : ∀ a, ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
      (fun b : M => (⟨b, V a b⟩ : TotalSpace E (TangentSpace I))))
    ...
    tensorL2Inner (I := I) (M := M) g 0 (s + 1)
      ...
      (∑ a : Fin N, ∑ b : Fin N,
        ...
        bochnerFoldGroupSum (I := I) (M := M) g s S V
          (bochnerGroupElt1 (I := I) (M := M) g s S) := by
  classical
  -- (A) the threeTerm normal form: `⟨gDCS, ∇S⟩ = −T₃`.
  have h3term := tensorL2Inner_genuineDiffCurv_covGrad_eq_neg_threeTerm
    (I := I) (M := M) g s S V hV hPar
  -- (B) the D-side primitive: `D = −(G₁ + I₂)`.
  have hB := parsevalFrameSum_diffCurvTrace_doubleSum_eq_neg_group1_add_crossPairing
    (I := I) (M := M) g s S V hV hPar
  -- (★) the genuine operator-field second-Bianchi: `T₃ = I₂`.
  have hStar := parsevalFrameSum_genuineDiffOp_threeTerm_eq_crossPairing_intrinsic
    (I := I) (M := M) g s S V hV hPar
  linarith [h3term, hB, hStar]
...
private theorem parsevalFrameSum_genuineDiffOp_threeTerm_eq_crossPairing
  ...
  have hB := parsevalFrameSum_diffCurvTrace_doubleSum_eq_neg_group1_add_crossPairing
    (I := I) (M := M) g s S V hV hPar
  -- (C): the integrated differentiated-curvature pairing `⟨gDCS, ∇S⟩ = D + G₁`, the
  -- frame-summed-integrated identity (no pointwise frame trace — the moving-frame curvature jet is
  -- chart-unbounded pointwise and sound only after the full Parseval sum and integration).
  have hC := tensorL2Inner_genuineDiffCurv_covGrad_eq_diffCurvTrace_add_group1
    (I := I) (M := M) g s S V hV hPar
  -- Combine (A) `⟨gDCS, ∇S⟩ = −T₃`, (B) `D = −(G₁ + I₂)`, (C) `⟨gDCS, ∇S⟩ = D + G₁`:
  -- `−T₃ = D + G₁ = −(G₁ + I₂) + G₁ = −I₂`, so the three-term sum equals `I₂`.
  linarith [h3term, hB, hC]
