...
theorem expMapIntrinsic_surjective_dist
  ...
  · -- `riemannianEDist I p q = 0 ⟹ p = q` (point separation); take `v := 0`.
    ...
    · -- `expMapIntrinsic g hEnorm p 0 = p = q`.  The first equality is the
      -- constant-geodesic identity (the intrinsic geodesic at zero launch
      -- velocity is constant), a geodesic-uniqueness fact recorded as the
      -- residual below; combined with `p = q` it gives the endpoint.
      have hexp0 : expMapIntrinsic (I := I) g hEnorm p 0 = p := by
        -- RESIDUAL: `intrinsicGeodesic g hEnorm p 0` is constant `= p` by
        -- geodesic uniqueness against `IsGeodesicAt.const` (the constant lift
        -- `fun _ => ⟨p, 0⟩` and the intrinsic geodesic share initial data at
        -- `0`; global agreement on `[0, 1]` propagates by connectedness).  This
        -- is the same `IsGeodesic`-level uniqueness propagation flagged for the
        -- positive case; not currently exposed as a standalone lemma.
        sorry
      rw [hexp0, hpq]
    · -- `√(g_p(0, 0)) = 0 = r`.
  ...
  · -- POSITIVE CASE: the ray/sphere propagation produces `v := r • u`.
    -- See the docstring decomposition; the propagation `sup A = r` re-based at
    -- the moving foot point is the single remaining geometric input.
    sorry
