...
theorem cov_map_germ
    ...
    mfderiv (modelWithCornersSelf Real E) I c.hom (z : E)
        (((Integral.Connection.metricCov
          (I := modelWithCornersSelf Real E) (M := E)
          (c.totalMetric g)).toFun (fun u : E => V u) (z : E)) v) =
      ((Integral.Connection.metricCov (I := I) (M := M) g).toFun
  ...
  have hEqOpen :
      (fun y : W =>
        Integral.Connection.restrictOpenTangentSection (I := I) W Z y) =ᶠ[
          nhds (Phi z)]
        (fun y : W => PW y) := by
    ...
    rw [← hau]
    simp only [PW, VU,
      Integral.Connection.restrictOpenTangentSection_apply,
  ...
  have hfield :
    ...
    simpa only [Integral.Connection.restrictOpenTangentSection] using hEqOpen
