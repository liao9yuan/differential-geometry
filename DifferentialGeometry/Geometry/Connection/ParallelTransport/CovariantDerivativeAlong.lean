...
omit [NeZero (Module.finrank ℝ E)] in
/-- Reparameterizing both a curve and a section along it multiplies the
covariant derivative by the scalar parameter derivative. -/
...
/-- The covariant derivative computed in any chart containing the curve point
agrees with the intrinsic covariant derivative under pointwise regularity. -/
theorem covDeriv_chartAt [I.Boundaryless]
  ...
  have huα_diff : DifferentiableAt ℝ uα t := by
    ...
    simpa only [uα, chartCurve] using hcomp
...
/-- Compatibility form of `covDeriv_chartAt` for a globally smooth curve. -/
...
omit [NeZero (Module.finrank ℝ E)] in
omit [Module.Finite ℝ E] in
/-- Differentiability of a tangent field in the curve-point chart gives
differentiability in any fixed chart containing the curve point. -/
...
omit [NeZero (Module.finrank ℝ E)] in
omit [Module.Finite ℝ E] in
/-- Differentiability of a tangent field in one fixed chart gives
differentiability in the curve-point chart. -/
theorem chartRep_diff_base [I.Boundaryless]
  ...
  have huβ_diff : DifferentiableAt ℝ uβ t := by
    ...
    rw [mdifferentiableAt_iff_differentiableAt] at hcomp
    simpa only [uβ, chartCurve] using hcomp
  have hU_nhds :
