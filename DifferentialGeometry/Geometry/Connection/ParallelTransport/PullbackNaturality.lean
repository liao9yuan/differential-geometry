...
variable {N : Type uN} [TopologicalSpace N] [ChartedSpace G N] [IsManifold J ∞ N]

omit [FiniteDimensional ℝ E] [CompleteSpace E] [NeZero (Module.finrank ℝ E)] in
private theorem deriv_repr_comp_nat
  ...
  have hu_hd : HasDerivAt u (deriv u t) t := by
    have hchart : MDifferentiableAt (modelWithCornersSelf ℝ ℝ)
        (modelWithCornersSelf ℝ E)
        (fun s => extChartAt I a (gamma s)) t :=
      (mdifferentiableAt_extChartAt (I := I) (mem_chart_source H (gamma t))).comp t hgamma
    exact (mdifferentiableAt_iff_differentiableAt.mp
      (by simpa only [u, a, chartCurve] using hchart)).hasDerivAt
  have hxu : u t = extChartAt I a (gamma t) := by
    rw [hu_def, chartCurve]
  ...
  have hsrc_nhds : gamma ⁻¹' (extChartAt I a).source ∈ nhds t := by
...
/-- Covariant differentiation of the restriction of a smooth ambient section
agrees with intrinsic covariant differentiation in the curve velocity. -/
theorem covAlong_sec
    [SigmaCompactSpace M] [T2Space M] [BoundarylessManifold I M]
    ...
    covDerivAlong (I := I) g gamma (fun r => Y (gamma r)) t =
      (LeviCivita (I := I) g) (fun x : M => Y x) (gamma t)
        ((mfderiv (modelWithCornersSelf ℝ ℝ) I gamma t : ℝ →L[ℝ] _) (1 : ℝ)) := by
  classical
  set a : M := gamma t with ha_def
  set v : TangentSpace I a :=
    (mfderiv (modelWithCornersSelf ℝ ℝ) I gamma t : ℝ →L[ℝ] _) (1 : ℝ) with hv_def
  have hgood : a ∈ chartLeviCivitaGoodSet (I := I) a :=
    self_mem_chartLeviCivitaGoodSet (I := I) (α := a)
  ...
  have hvel : trivToE (I := I) a a v =
    ...
    have hbridge :=
      MFDerivAlongCurve.chartCoord_mfderiv_along_curve_eq_fderiv_of_mdifferentiableAt
        (I := I) (M := M) (γ := gamma) hgamma a (t := t) hsrc
    rw [trivToE]
    rw [show v =
      (mfderiv (modelWithCornersSelf ℝ ℝ) I gamma t : ℝ →L[ℝ] _) (1 : ℝ) from rfl]
    rw [hbridge]
  ...
  rw [show (LeviCivita (I := I) g) (fun x : M => Y x) (gamma t)
        ((mfderiv (modelWithCornersSelf ℝ ℝ) I gamma t : ℝ →L[ℝ] _) (1 : ℝ)) =
  ...
  have hderiv :
    ...
    rw [hvel]
    exact deriv_repr_comp_nat (I := I) gamma Y t hgamma
  rw [hderiv, hchris]
...
private theorem mfderiv_from_chart_nat
    [I.Boundaryless] [IsManifold I 1 M]
    (f : M → F) (a p : M) (hp : p ∈ (chartAt H a).source)
    (hf : MDifferentiableAt I (modelWithCornersSelf ℝ F) f p) (v : E) :
    mfderiv I (modelWithCornersSelf ℝ F) f p (trivFromE (I := I) a p v) =
      fderiv ℝ (writtenInExtChartAt I (modelWithCornersSelf ℝ F) a f)
  ...
  have hrange : Set.range I ∈ nhds z := by
    ...
    exact univ_mem
  have hsymmDiff :
      MDifferentiableWithinAt (modelWithCornersSelf ℝ E) I (extChartAt I a).symm
        (Set.range I) z := by
    simpa [z] using mdifferentiableWithinAt_extChartAt_symm (I := I) hzTarget
  have hfUniv :
      MDifferentiableWithinAt I (modelWithCornersSelf ℝ F) f Set.univ
  ...
  have hmaps :
    ...
    intro y hy
    simp
  have huniq : UniqueMDiffWithinAt (modelWithCornersSelf ℝ E) (Set.range I) z := by
    exact (I.uniqueDiffOn.uniqueDiffWithinAt
      (by exact extChartAt_target_subset_range (I := I) a hzTarget)).uniqueMDiffWithinAt
  have hchain :=
    mfderivWithin_comp (I := modelWithCornersSelf ℝ E) (I' := I)
      (I'' := modelWithCornersSelf ℝ F)
      (x := z) (g := f) (f := (extChartAt I a).symm)
      hfUniv hsymmDiff hmaps huniq
  have hchainApply :
      fderivWithin ℝ (writtenInExtChartAt I (modelWithCornersSelf ℝ F) a f)
          (Set.range I) z v =
        mfderiv I (modelWithCornersSelf ℝ F) f p
          ((mfderivWithin (modelWithCornersSelf ℝ E) I (extChartAt I a).symm
            (Set.range I) z) v) := by
  ...
  have hwithin :
      fderivWithin ℝ (writtenInExtChartAt I (modelWithCornersSelf ℝ F) a f)
          (Set.range I) z v =
        fderiv ℝ (writtenInExtChartAt I (modelWithCornersSelf ℝ F) a f) z v := by
...
omit [FiniteDimensional ℝ E] [CompleteSpace E] [NeZero (Module.finrank ℝ E)]
    [FiniteDimensional ℝ F] [CompleteSpace F] [NeZero (Module.finrank ℝ F)] in
private theorem triv_mfderiv_nat
  ...
  have hext : MDifferentiableAt J (modelWithCornersSelf ℝ F) (extChartAt J b) (Phi p) :=
    mdifferentiableAt_extChartAt (I := J) hPhip
  have htriv :
      trivToE (I := J) b (Phi p) =
        mfderiv J (modelWithCornersSelf ℝ F) (extChartAt J b) (Phi p) :=
    TangentBundle.continuousLinearMapAt_trivializationAt (I := J) hPhip
  have hcomp := mfderiv_comp_apply (I := I) (I' := J)
    (I'' := modelWithCornersSelf ℝ F)
    (g := extChartAt J b) (f := (Phi : M → N)) (x := p)
    hext hPhi (trivFromE (I := I) a p v)
  rw [htriv]
  have hfixed := mfderiv_from_chart_nat (I := I)
    (f := fun y : M => extChartAt J b (Phi y)) a p hp
    (hext.comp p hPhi) v
  calc
    mfderiv J (modelWithCornersSelf ℝ F) (extChartAt J b) (Phi p)
          (mfderiv I J (Phi : M → N) p (trivFromE (I := I) a p v)) =
        mfderiv I (modelWithCornersSelf ℝ F) (fun y : M => extChartAt J b (Phi y)) p
...
omit [FiniteDimensional ℝ E] [CompleteSpace E] [NeZero (Module.finrank ℝ E)]
    [FiniteDimensional ℝ F] [CompleteSpace F] [NeZero (Module.finrank ℝ F)] in
private theorem chartRep_map_ev
    [I.Boundaryless] [J.Boundaryless]
    [IsManifold I 1 M] [IsManifold J 1 N]
  ...
  simpa [writtenInExtChartAt, chartCurve, Function.comp_def] using
    (triv_mfderiv_nat (I := I) (J := J) Phi (gamma t) (gamma s)
      (Phi (gamma t)) hs hPhis w)

omit [FiniteDimensional ℝ E] [CompleteSpace E] [NeZero (Module.finrank ℝ E)]
    [FiniteDimensional ℝ F] [CompleteSpace F] [NeZero (Module.finrank ℝ F)] in
private theorem deriv_fderiv_zero
    (psi : E → F) (u w : ℝ → E) (t : ℝ)
...
omit [FiniteDimensional ℝ E] [CompleteSpace E] [NeZero (Module.finrank ℝ E)]
    [FiniteDimensional ℝ F] [CompleteSpace F] [NeZero (Module.finrank ℝ F)] in
/-- Mapping a differentiable vector field along a differentiable curve by a
diffeomorphism preserves differentiability of its chart representative. -/
theorem chartRep_map_diff
    [I.Boundaryless] [J.Boundaryless]
    [IsManifold I 1 M] [IsManifold J 1 N]
    (Phi : M ≃ₘ⟮I, J⟯ N) (gamma : ℝ → M)
    (V : ∀ s, TangentSpace I (gamma s)) (t : ℝ)
  ...
  have hu : DifferentiableAt ℝ u t := by
    have hchart : MDifferentiableAt (modelWithCornersSelf ℝ ℝ)
      ...
      (mdifferentiableAt_extChartAt (I := I) (mem_chart_source H (gamma t))).comp t hgamma
    exact mdifferentiableAt_iff_differentiableAt.mp
      (by simpa only [u, a, chartCurve] using hchart)
  ...
  have hright : DifferentiableAt ℝ (fun s => fderiv ℝ psi (u s) (w s)) t :=
    hDpsi.clm_apply hV
  have hev := chartRep_map_ev (I := I) (J := J) Phi gamma V t
    hgamma.continuousAt
...
private theorem covAlong_map_zero
    [I.Boundaryless] [J.Boundaryless]
    [SigmaCompactSpace M] [T2Space M] [BoundarylessManifold I M]
  ...
  have hrepEv :
    ...
    simpa [a, b, delta, W, psi, u, w] using
      (chartRep_map_ev (I := I) (J := J) Phi gamma V t hgamma.continuousAt)
  have hrepDeriv :
      deriv (chartRepAt (I := J) delta W t) t =
        fderiv ℝ psi (u t) (deriv w t) := by
    rw [hrepEv.deriv_eq]
    exact deriv_fderiv_zero psi u w t hpsi hu hV hw0
  have hsourceChart :
  ...
  have hleftCoord :
    ...
    simpa [psi, u, a, b, chartCurve, writtenInExtChartAt, Function.comp_def] using
      (triv_mfderiv_nat (I := I) (J := J) Phi a a b haSrc hbSrc (deriv w t))
  have hrightCoord :
...
private theorem covAlong_map_nat
    [SigmaCompactSpace M] [T2Space M] [BoundarylessManifold I M]
    [SigmaCompactSpace N] [T2Space N] [BoundarylessManifold J N]
    [IsManifold I 1 M] [IsManifold I ((∞ : WithTop ℕ∞) + 1) M]
    [IsManifold J 1 N] [IsManifold J ((∞ : WithTop ℕ∞) + 1) N]
  ...
  let v : TangentSpace I (gamma t) :=
    (mfderiv (modelWithCornersSelf ℝ ℝ) I gamma t : ℝ →L[ℝ] TangentSpace I (gamma t))
      (1 : ℝ)
  have hdelta : MDifferentiableAt (modelWithCornersSelf ℝ ℝ) J delta t :=
    Phi.contMDiff.mdifferentiableAt (by simp) |>.comp t hgamma
  have hleft := covAlong_sec
    (I := I)
    (Diffeomorph.pullbackMetricCross (I := I) (J := J) g Phi)
    gamma Y t hgamma
  have hright := covAlong_sec
    (I := J) g delta Ypush t hdelta
  have hPhi : MDifferentiableAt I J (Phi : M → N) (gamma t) :=
    Phi.contMDiff.mdifferentiableAt (by simp)
  have hvel :
      (mfderiv (modelWithCornersSelf ℝ ℝ) J delta t :
          ℝ →L[ℝ] TangentSpace J (delta t)) (1 : ℝ) =
        mfderiv I J (Phi : M → N) (gamma t) v := by
    simpa [delta, v, Function.comp_def] using
      (mfderiv_comp_apply (I := modelWithCornersSelf ℝ ℝ) (I' := I) (I'' := J)
        (g := (Phi : M → N)) (f := gamma) (x := t) hPhi hgamma (1 : ℝ))
  ...
  calc
    ...
    _ = (LeviCivita (I := J) g) (fun x : N => Ypush x) (delta t)
        ((mfderiv (modelWithCornersSelf ℝ ℝ) J delta t :
          ℝ →L[ℝ] TangentSpace J (delta t)) (1 : ℝ)) := by
      rw [hvel]
...
private theorem covAlong_nat_cross
  ...
  have hYmapdiff : DifferentiableAt ℝ
      (chartRepAt (I := J) delta Ymap t) t :=
    chartRep_map_diff (I := I) (J := J) Phi gamma Yalong t hgamma hYdiff
  have hRmapdiff : DifferentiableAt ℝ
      (chartRepAt (I := J) delta Rmap t) t :=
    chartRep_map_diff (I := I) (J := J) Phi gamma R t hgamma hRdiff
  have hYnat := covAlong_map_nat (I := I) (J := J) g Phi gamma Y t hgamma
  have hRnat := covAlong_map_zero (I := I) (J := J) g Phi gamma R t
    hgamma hRdiff hRt
...
/-- Covariant differentiation along a curve is natural under a diffeomorphism
at every time where the curve and the vector field along it are differentiable. -/
theorem covAlong_natMDiff
    ...
    [IsManifold I 1 M] [IsManifold I ((∞ : WithTop ℕ∞) + 1) M]
    [IsManifold I 1 P] [IsManifold I ((∞ : WithTop ℕ∞) + 1) P]
    (g : SmoothRiemannianMetric I P) (Phi : M ≃ₘ⟮I, I⟯ P)
    (gamma : ℝ → M) (V : ∀ s, TangentSpace I (gamma s)) (t : ℝ)
    (hgamma : MDifferentiableAt (modelWithCornersSelf ℝ ℝ) I gamma t)
    (hV : DifferentiableAt ℝ (chartRepAt (I := I) gamma V t) t) :
    mfderiv I I (Phi : M → P) (gamma t)
        (covDerivAlong (I := I) (Diffeomorph.pullbackMetric (I := I) g Phi)
          gamma V t) =
      covDerivAlong (I := I) g (fun s => Phi (gamma s))
        (fun s => mfderiv I I (Phi : M → P) (gamma s) (V s)) t := by
  have hmetric :
      Diffeomorph.pullbackMetricCross (I := I) (J := I) g Phi =
        Diffeomorph.pullbackMetric (I := I) g Phi := by
