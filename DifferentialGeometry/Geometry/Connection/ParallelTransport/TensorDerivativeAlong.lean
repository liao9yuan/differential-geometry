...
private lemma metric_conn_model
  ...
  simp only [ContinuousLinearMap.zero_apply, zero_add, hrepr_center]
...
omit [NeZero (Module.finrank Real E)] [SigmaCompactSpace M] in
/-- The derivative of a smooth covariant tensor evaluated on moving tangent
fields is its covariant derivative in the curve direction plus one covariant
derivative term for each moving slot. -/
theorem tensor_eval_deriv {s : ℕ}
  ...
  have hu : DifferentiableAt Real u t := by
    ...
    exact mdifferentiableAt_iff_differentiableAt.mp
      (by simpa only [u, x, chartCurve] using hchart)
  have hAm : DifferentiableAt Real Am (u t) := by
    ...
    have hat : MDifferentiableAt 𝓘(Real, E)
      ...
      simpa [Am, u, x, chartCurve,
        ModelWithCorners.Boundaryless.range_eq_univ (I := I)] using hmdiff
  ...
  have heval :
    ...
    congr 1
    funext a
    simpa [Vm, e, chartRepAt_apply] using
      (e.symmL_continuousLinearMapAt (R := Real) hr (V a r)).symm
  ...
  have hUcoord : e.continuousLinearMapAt Real x
    ...
    simpa only [e, x, U, u, chartCurve_def, deriv] using hbridge
