...
/-!
# Difference of covariant derivatives along a curve

This file identifies the difference of two along-curve Levi-Civita derivatives
with the intrinsic difference tensor.  The result is pointwise and does not
assume regularity of the field along the curve, since its chart derivative
cancels between the two connections.
-/
...
/-- The difference of two Levi-Civita derivatives along a raw curve is the
connection-difference tensor evaluated on the field and curve velocity. -/
theorem covAlong_diff
  ...
  have hdiff := DifferentialGeometry.PDE.DeTurck.connDiff_apply
    (I := I) g₁ g₂ Y.mdifferentiableAt
    ((mfderiv (modelWithCornersSelf Real Real) I gamma t :
      Real →L[Real] TangentSpace I (gamma t)) (1 : Real))
  simpa only [DifferentialGeometry.PDE.DeTurck.connDiff, metricCov, LeviCivita,
    hY] using hdiff.symm
