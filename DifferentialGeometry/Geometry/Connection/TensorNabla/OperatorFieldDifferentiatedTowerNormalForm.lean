...
theorem appCc_zero_left (g : SmoothRiemannianMetric I M) (r s : ℕ)
  ...
  rw [add_zero] at h
  -- h : appCc 0 W = appCc 0 W + appCc 0 W; subtract to get appCc 0 W = 0.
...
theorem normalForm_succ (g : SmoothRiemannianMetric I M)
  ...
  refine ⟨fun j => match j with
    ...
    | (k + 1) =>
        (if k + 1 < p + 1 then covGrad (I := I) (M := M) g (r + (k + 1)) (r + p) (Ψr (k + 1)) else 0)
          + Tk k, ?_⟩
  intro W
  -- LHS recursion: op (p+1) r W = ∇(op p r W) − cast(op p (r+1) (∇W)).
  ...
  -- The RHS j=0 term is G₀ = appCc (∇Ψr 0) W.
  ...
  -- The guarded gradient sum equals ∑_{k<p} G_{k+1} (the k=p term is appCc 0 = 0).
  rw [show (∑ k ∈ Finset.range (p + 1),
        appCc (I := I) (M := M) g (r + (k + 1)) (r + (p + 1))
          (if k + 1 < p + 1 then covGrad (I := I) (M := M) g (r + (k + 1)) (r + p) (Ψr (k + 1))
            else 0)
...
theorem exists_jet_bound_of_normalForm (g : SmoothRiemannianMetric I M)
  ...
  -- Step 1: rewrite `op p r W` by the normal form and pass `rfns` through the finite sum.
  rw [hΨ W, SmoothCcTensor.toSection_sum_apply]
  -- Step 2: `rfns(∑ ...) ≤ card · ∑ rfns(appCc (Ψ k) (∇^k W))`.
  ...
  -- Step 3: each summand `rfns(appCc (Ψ k) (∇^k W))(x) ≤ C k · a k`.
