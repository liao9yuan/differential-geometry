...
theorem deTurck_g0_realize_data
  ...
  have hN_coeff : ∀ s ∈ Set.Ico (0 : ℝ) T,
    ...
    intro s hs i
    -- The gate representative at the carrier inclusion is the smooth representative `T_s s`
    -- (`gateSmoothRep_carrierInclusion_eq`), whose order-`2a` Sobolev norm is `≤ Q` on the
    -- shrunk horizon (`hQcarrier`); this discharges the quantitative match-gate.
