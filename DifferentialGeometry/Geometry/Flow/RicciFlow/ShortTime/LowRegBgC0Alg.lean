...
open DifferentialGeometry.Analysis.Sobolev
  (iteratedCovGrad iteratedCovGrad_succ iteratedCovGrad_zero metricConnDiffLoweredCc
   rsDomDomCongrSection rsDomDomCongrSection_toSection)
open DifferentialGeometry.Analysis.Sobolev.TensorHilbert
open DifferentialGeometry.Analysis.Spectral
  (appCc appCc_add_left appCc_add_right appCc_assoc appCc_smul_left appCc_smul_right appCc_sub_left appCc_toSection appCcRS
   appCcRS_toSection appCcRS_zero_eq_appCc ccInputSlotSymm ccInputSymm ccOperatorFieldComp
   ccSlotSwapField ccSlotSwapField_toSection edgeLiePairFam lc0AMix lc0Kappa lc0Riem lc0VB
   lieCorr0Field operatorFieldApply permCoeff ricciAAArm ricciAAKer rsDomDomCongr slotExtend slotSwapFib slotSwapFib_apply
...
theorem amix_half_one
  ...
  rw [lc0AMixHalfRF]
  simp only [appCc]
  simp only [appCc] at hprod hconn
  conv_lhs =>
...
theorem amix_one
  ...
  simp only [appCc] at hhalf hhalf'
  rw [hhalf, hhalf']
...
theorem aa_one
  ...
  simp only [appCc] at h0 h1 h2 h3 h4 h5
...
theorem ricci_one
  ...
  have hda := LowBaseInternal.ricciDA_one (I := I) (M := M) g gm P
    (symmS (I := I) (M := M) g W)
  simp only [appCc] at hsymmInput haa hda
