...
The metricwise proof reduces the `H²` jet window of `slotInsertEndoCc g 3` and
`slotInsertEndoCc g 5` to the one of `slotInsertEndoCc g 1` through helpers that
...
private theorem insSuccPt (g : SmoothRiemannianMetric I M) (q : ℕ)
    ...
    riemannianFiberNormSq (I := I) (M := M) g (q + 1 + 1) ((q + 1 + 1) + i) x
        ((iteratedCovGrad (I := I) g (q + 1 + 1) (q + 1 + 1) i
          (slotInsertEndoCc (I := I) (M := M) g (q + 1) P)).toSection x) ≤
      (Module.finrank ℝ E : ℝ) *
        riemannianFiberNormSq (I := I) (M := M) g (q + 1) ((q + 1) + i) x
          ((iteratedCovGrad (I := I) g (q + 1) (q + 1) i
            (slotInsertEndoCc (I := I) (M := M) g q P)).toSection x) := by
  have hA :
      riemannianFiberNormSq (I := I) (M := M) g (q + 1 + 1) ((q + 1 + 1) + i) x
          ((iteratedCovGrad (I := I) g (q + 1 + 1) (q + 1 + 1) i
            (slotInsertEndoCc (I := I) (M := M) g (q + 1) P)).toSection x) =
...
private theorem insSuccSq (g : SmoothRiemannianMetric I M) (q : ℕ)
    ...
    (i : ℕ) :
    ‖iteratedCovGrad (I := I) g (q + 1 + 1) (q + 1 + 1) i
        (slotInsertEndoCc (I := I) (M := M) g (q + 1) P)‖ ^ 2 ≤
      (Module.finrank ℝ E : ℝ) *
        ‖iteratedCovGrad (I := I) g (q + 1) (q + 1) i
          (slotInsertEndoCc (I := I) (M := M) g q P)‖ ^ 2 := by
  classical
  set F : M → ℝ := fun x => (Module.finrank ℝ E : ℝ) *
    riemannianFiberNormSq (I := I) (M := M) g (q + 1) ((q + 1) + i) x
      ((iteratedCovGrad (I := I) g (q + 1) (q + 1) i
        (slotInsertEndoCc (I := I) (M := M) g q P)).toSection x) with hF
...
private theorem ins3Jet (g : SmoothRiemannianMetric I M)
    ...
    (∑ j ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g 4 4 j
        (slotInsertEndoCc (I := I) (M := M) g 3 P)‖ ^ 2) ≤
      (Module.finrank ℝ E : ℝ) ^ 2 *
        ∑ j ∈ Finset.range 3,
          ‖iteratedCovGrad (I := I) g 2 2 j
            (slotInsertEndoCc (I := I) (M := M) g 1 P)‖ ^ 2 := by
        ...
        (Module.finrank ℝ E : ℝ) *
          ∑ j ∈ Finset.range 3,
            ‖iteratedCovGrad (I := I) g 3 3 j
              (slotInsertEndoCc (I := I) (M := M) g 2 P)‖ ^ 2 :=
      insSuccJet (I := I) (M := M) g 2 P
    _ ≤ (Module.finrank ℝ E : ℝ) *
        ((Module.finrank ℝ E : ℝ) *
          ∑ j ∈ Finset.range 3,
            ‖iteratedCovGrad (I := I) g 2 2 j
              (slotInsertEndoCc (I := I) (M := M) g 1 P)‖ ^ 2) :=
...
private theorem ins5Jet (g : SmoothRiemannianMetric I M)
    ...
    (∑ j ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g 6 6 j
        (slotInsertEndoCc (I := I) (M := M) g 5 P)‖ ^ 2) ≤
        ...
        (Module.finrank ℝ E : ℝ) *
          ∑ j ∈ Finset.range 3,
            ‖iteratedCovGrad (I := I) g 5 5 j
              (slotInsertEndoCc (I := I) (M := M) g 4 P)‖ ^ 2 :=
...
private theorem dtJet (g : SmoothRiemannianMetric I M) (p : ℕ) :
  ...
  have hexp : (∑ j ∈ Finset.range 3,
      ...
      ‖iteratedCovGrad (I := I) g (p + 2) p 2
        (DeTurck.cometricDoubleTraceField (I := I) g p)‖ ^ 2 := by
...
theorem trace24_h2_lip_unif
  ...
  obtain ⟨ρ, Cinv, hρ, hCinv, hinv⟩ :=
    invCoeff_h2_lip_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨C₂, hC₂, happ₂⟩ :=
    appRS_h22_unif (I := I) (M := M) hDim gBase hΛ 4 4 2
  obtain ⟨C₄, hC₄, happ₄⟩ :=
    appRS_h22_unif (I := I) (M := M) hDim gBase hΛ 6 6 4
  ...
  let F₂ : SmoothCcTensor g 4 2 := DeTurck.cometricDoubleTraceField (I := I) g 2
  let F₄ : SmoothCcTensor g 6 4 := DeTurck.cometricDoubleTraceField (I := I) g 4
  let dEndo := gInvDiffRaisedEndoField (I := I) g gT -
    gInvDiffRaisedEndoField (I := I) g gU
  let D₂ : SmoothCcTensor g 4 4 :=
    slotInsertEndoCc (I := I) (M := M) g 3 dEndo
  let D₄ : SmoothCcTensor g 6 6 :=
    slotInsertEndoCc (I := I) (M := M) g 5 dEndo
  ...
  have hslot1 :
      slotInsertEndoCc (I := I) (M := M) g 1 dEndo =
  ...
  have hinvJet : (∑ j ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g 2 2 j
        (gInvDiffSlotCoeff (I := I) g gT -
          gInvDiffSlotCoeff (I := I) g gU)‖ ^ 2) ≤ (Cinv * N) ^ 2 := by
    simpa only [N] using
    ...
    calc
      (∑ j ∈ Finset.range 3,
          ...
          (Module.finrank ℝ E : ℝ) ^ 2 *
            ∑ j ∈ Finset.range 3,
              ‖iteratedCovGrad (I := I) g 2 2 j
                (slotInsertEndoCc (I := I) (M := M) g 1 dEndo)‖ ^ 2 :=
        ins3Jet (I := I) (M := M) g dEndo
      _ = 9 * ∑ j ∈ Finset.range 3,
            ‖iteratedCovGrad (I := I) g 2 2 j
              (gInvDiffSlotCoeff (I := I) g gT -
                gInvDiffSlotCoeff (I := I) g gU)‖ ^ 2 := by
        rw [hDim, hslot1]
        norm_num
      _ ≤ 9 * (Cinv * N) ^ 2 :=
...
theorem pairTrace_h2_lip_unif
        ...
        ∀ (T U : SmoothCcTensor g 0 2)
          ...
          (∑ j ∈ Finset.range 3,
            ‖iteratedCovGrad (I := I) g 6 2 j
              (lieCovPair (I := I) (M := M) g gT -
                lieCovPair (I := I) (M := M) g gU)‖ ^ 2) ≤
            (C * ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ)
              (T - U)‖) ^ 2 := by
  classical
  obtain ⟨ρ, Ct, hρ, hCt, htrace⟩ :=
    trace24_h2_lip_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨Ca, hCa, happ⟩ :=
    appRS_h22_unif (I := I) (M := M) hDim gBase hΛ 6 4 2
  let vol : ℝ := volCompareC (E := E) Λ *
    ((riemannianVolumeMeasure (I := I) (M := M) gBase) Set.univ).toReal
  ...
  have hfin : (∑ j ∈ Finset.range 3,
    ...
    exact (jetAdd (I := I) (M := M) g _ _).trans
    ...
    ∃ ρ B : ℝ, 0 < ρ ∧ 0 ≤ B ∧
      ∀ g : SmoothRiemannianMetric I M,
        ...
        ∀ (T : SmoothCcTensor g 0 2)
          ...
          (∑ j ∈ Finset.range 3,
            ‖iteratedCovGrad (I := I) g 6 2 j
              (lieCovPair (I := I) (M := M) g gT)‖ ^ 2) ≤ B ^ 2 := by
  classical
  obtain ⟨ρ, Ct, hρ, hCt, htrace⟩ :=
    trace24_h2_lip_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨Ca, hCa, happ⟩ :=
...
application `appCcRS g 4 6 2 (lieCovPair g gm) (monoExt g 0 2 4 (monoPerm σ) S)`:
...
private theorem monoSplit (g gT gU : SmoothRiemannianMetric I M)
    ...
    curvatureRefoldMonomialCoeffField (I := I) (M := M) g gT
        ...
        curvatureRefoldMonomialCoeffField (I := I) (M := M) g gU
...
theorem curvMono_h2_lip_unif
  ...
  obtain ⟨ρ₁, Cp, hρ₁, hCp, hpair⟩ :=
    pairTrace_h2_lip_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨ρ₂, Bp, hρ₂, hBp, hbpair⟩ :=
    pairTr_h2_bdd_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨Ca, hCa, happ⟩ :=
    appRS_h22_unif (I := I) (M := M) hDim gBase hΛ 4 6 2
  ...
  let X : SmoothCcTensor g 0 2 → SmoothCcTensor g 4 6 :=
    fun W => monoExt (I := I) (M := M) g 0 2 4 (LowBaseInternal.monoPerm σ) W
  let Q₁ : SmoothCcTensor g 4 2 :=
    appCcRS (I := I) (M := M) g 4 6 2
      (lieCovPair (I := I) (M := M) g gT -
        lieCovPair (I := I) (M := M) g gU) (X S)
  let Q₂ : SmoothCcTensor g 4 2 :=
    appCcRS (I := I) (M := M) g 4 6 2
      (lieCovPair (I := I) (M := M) g gU) (X S - X R)
  have hN : 0 ≤ N := norm_nonneg _
  ...
  have hPdiff :
      (∑ j ∈ Finset.range 3,
        ‖iteratedCovGrad (I := I) g 6 2 j
          (lieCovPair (I := I) (M := M) g gT -
            lieCovPair (I := I) (M := M) g gU)‖ ^ 2) ≤ (Cp * N) ^ 2 :=
    hpair g hEq hjet T U gT gU hTtie hUtie hT₁ hU₁
  have hPend :
      (∑ j ∈ Finset.range 3,
        ‖iteratedCovGrad (I := I) g 6 2 j
          (lieCovPair (I := I) (M := M) g gU)‖ ^ 2) ≤ Bp ^ 2 :=
  ...
  have hXdiff :
    ...
    rw [show X S - X R =
        monoExt (I := I) (M := M) g 0 2 4
          (LowBaseInternal.monoPerm σ) (S - R) from
      (monoExtSub (I := I) (M := M) g 0 2 4
        (LowBaseInternal.monoPerm σ) S R).symm]
    calc
      (∑ j ∈ Finset.range 3,
          ‖iteratedCovGrad (I := I) g 4 6 j
            (monoExt (I := I) (M := M) g 0 2 4
              (LowBaseInternal.monoPerm σ) (S - R))‖ ^ 2) ≤
          (Module.finrank ℝ E : ℝ) ^ 4 *
            ∑ j ∈ Finset.range 3,
              ‖iteratedCovGrad (I := I) g 0 2 j (S - R)‖ ^ 2 :=
        monoExtJet (I := I) (M := M) g 0 2 4 2
          (LowBaseInternal.monoPerm σ) (S - R)
...
theorem curv_pair_h2_unif
  ...
  obtain ⟨ρ, C, hρ, hC, hlip⟩ :=
    curvMono_h2_lip_unif (I := I) (M := M) hDim gBase hΛ
  refine ⟨C, hC, ?_⟩
