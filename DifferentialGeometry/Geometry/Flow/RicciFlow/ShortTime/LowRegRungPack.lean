...
def IsLowGateOrd (g : SmoothRiemannianMetric I M) (A B : ℝ) : Prop :=
  0 ≤ A ∧ 0 ≤ B ∧
    (∃ C2 Kr23 Kr13 K2 : ℝ,
      IsRung3Ord (I := I) (M := M) g C2 Kr23 Kr13 K2 ∧
        C2 * K2 ≤ A ∧ Kr23 + Kr13 ≤ B) ∧
    (∃ C3 Kr24 Kr14 K3 : ℝ,
      IsRung4Ord (I := I) (M := M) g C3 Kr24 Kr14 K3 ∧
        C3 * K3 ≤ A ∧ Kr24 + Kr14 ≤ B) ∧
    (∃ C4 Kr25 Kr15 K4 : ℝ,
      IsRung5Ord (I := I) (M := M) g C4 Kr25 Kr15 K4 ∧
        C4 * K4 ≤ A ∧ Kr25 + Kr15 ≤ B) ∧
...
theorem lowregGatePack (hDim : Module.finrank ℝ E = 3)
  ...
  obtain ⟨C3, Kr24, Kr14, K3, h3⟩ :=
    lowregRung4Pack (I := I) (M := M) hDim g
  obtain ⟨C4, Kr25, Kr15, K4, h4⟩ :=
    lowregRung5Pack (I := I) (M := M) hDim g
  obtain ⟨κ, hκ⟩ := lowregHmPack (I := I) (M := M) hDim g
