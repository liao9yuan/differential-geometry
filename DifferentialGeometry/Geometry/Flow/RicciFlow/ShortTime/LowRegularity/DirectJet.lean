import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.AllOrderJet
...
private theorem force_step_one
  ...
  have hsmooth := lowReg_force_smooth (I := I) (M := M) g g_bg hR hδ hreal hcore
      ...
      fun t => lowRegN (I := I) (M := M) g g hR hs.hδ
        ...
        (aeSetLift (zero_mem_lowerState (I := I) (M := M) g 1 hR.le)
          (maxRegDuhamelSolField (I := I) (M := M) ((1 : ℕ) : ℝ) hT hT1
            (0 : tensorHs (I := I) (M := M) g 0 2 (((1 : ℕ) : ℝ) + 2))
            fLo) t) := by
    simpa only [lowregNfun] using hs.hforce
...
private theorem direct_force_coeff
  ...
  have hsmooth := lowReg_force_smooth (I := I) (M := M) g g_bg hR hδ hreal
...
theorem lowreg_directJet (hDim : Module.finrank ℝ E = 3)
      ...
      (∀ (F : ℝ → SmoothCcTensor g 0 2) {δ' : ℝ} (hδ'_lt : δ' < 1)
        ...
        ∀ t ∈ Set.Ico (0 : ℝ) T, ∀ i,
          fc i t = tensorL2Coeff (I := I) (M := M)
              (tensorResolventL2_isCompactOperator (I := I) (M := M) g 0 2)
              (SmoothCcTensor.toL2 (g := g) (r := 0) (s := 2)
                (lowregNsec (I := I) (M := M) g (F t) hδ'_lt (hδ' t))) i) ∧
      (∀ t ∈ Set.Icc (0 : ℝ) T, ‖timeH1.toFun u t‖ ≤ Rcap) := by
  let hs := hlo.toIsLowSolveAt
  let R := lowregStateRad Ctop B1 ρ P
  have hR : 0 < R := lowregStateRad_pos hs.hCtop hs.hB1 hs.hρ hs.hP
  ...
  have hfield_mem : ∀ᵐ t ∂timeMeasure T,
    ...
    exact field_mem_lower (I := I) (M := M) g 1 hT hT1
      (by nlinarith [hR.le]) fLo hs.hball
  have hforce : fLo =ᵐ[timeMeasure T]
      fun t => lowRegN (I := I) (M := M) g g hR hs.hδ hreal
        (aeSetLift (zero_mem_lowerState (I := I) (M := M) g 1 hR.le)
          (maxRegDuhamelSolField (I := I) (M := M) ((1 : ℕ) : ℝ) hT hT1
            (0 : tensorHs (I := I) (M := M) g 0 2 (((1 : ℕ) : ℝ) + 2))
            fLo) t) := by
    simpa only [R, hreal, lowregNfun] using hs.hforce
  ...
  have hforce' : ∀ (F : ℝ → SmoothCcTensor g 0 2) {δ' : ℝ}
      ...
      ∀ t ∈ Set.Ico (0 : ℝ) T, ∀ i,
        fc i t = tensorL2Coeff (I := I) (M := M)
          (tensorResolventL2_isCompactOperator (I := I) (M := M) g 0 2)
          (SmoothCcTensor.toL2 (g := g) (r := 0) (s := 2)
            (lowregNsec (I := I) (M := M) g (F t) hδ'_lt (hδ' t))) i := by
    simpa only [lowregNsec] using hforceRaw
