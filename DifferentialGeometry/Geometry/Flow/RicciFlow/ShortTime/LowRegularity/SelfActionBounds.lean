...
private theorem revEndoAtgw (g₀ : SmoothRiemannianMetric I M) {δ₀ : ℝ} (_hδ₀ : δ₀ < 1) (s : ℕ) :
    ∃ C : ℕ → ℝ, (∀ i, 0 ≤ C i) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        (_htie : ∀ (y : M) (v w : TangentSpace I y),
          g₁.inner y v w = g₀.inner y v w + ccTensorBilinSymm (I := I) g₀ P y v w)
...
theorem lieCovCap (g₀ : SmoothRiemannianMetric I M)
    ...
    ∃ K : ℕ → ℝ, (∀ i, 0 ≤ K i) ∧
      ∀ (T : SmoothCcTensor g₀ 0 2)
        ...
        (_hP0 : ∀ x : M, gridBase (I := I) (M := M) g₀
          (convexPerturbation (I := I) g₀ T 0 s) x 0 ≤ Λ)
        (_hP1 : ∀ x : M, gridBase (I := I) (M := M) g₀
...
private lemma extSub (g₀ : SmoothRiemannianMetric I M) (X Y : SmoothCcTensor g₀ 0 4) :
  ...
  have hrec : ∀ Z : SmoothCcTensor g₀ 0 4,
    ...
    fun _ => rfl
  rw [hrec, hrec, hrec,
    DifferentialGeometry.Analysis.Spectral.slotExtend_sub_cc,
    DifferentialGeometry.Analysis.Spectral.slotExtend_sub_cc]
...
theorem lieCovJet (hDim : Module.finrank ℝ E = 3)
  ...
  have hres :
    ...
    rw [hres0, lieCovR4_eq (I := I) (M := M) g₀ T hδg hδZ s, hcurv,
      extSub (I := I) (M := M) g₀,
      DifferentialGeometry.Analysis.Spectral.rsDomDomCongrSection_sub_cc,
...
private theorem clExact (g₀ : SmoothRiemannianMetric I M) {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
  ...
  have hE : q ≤ (fr ^ 2 * Cb (i + 1)) * Combinatorics.antidiagonalTupleGrid
    ...
    have hzero : iteratedCovGrad (I := I) g₀ 3 3 (i + 1)
      ...
      DifferentialGeometry.Analysis.Spectral.iteratedCovGrad_zero_of_covGrad_zero
        (I := I) (M := M) g₀ 3 3 _
...
private theorem clCovMk (g₀ : SmoothRiemannianMetric I M) {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ K : ℕ → ℝ, (∀ i, 0 ≤ K i) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        {δ : ℝ} (_hδ_le : δ ≤ δ₀) (_hδ0 : 0 ≤ δ)
        (_hδ : gFibreOpBound (I := I) (M := M) g₀ (ccTensorBilinSymm (I := I) g₀ P) δ)
        (_hP0 : ∀ x : M, gridBase (I := I) (M := M) g₀ P x 0 ≤ 1),
