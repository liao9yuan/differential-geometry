...
private theorem perm_left_rfns
    ...
    riemannianFiberNormSq (I := I) (M := M) g d d x
      ...
      riemannianFiberNormSq (I := I) (M := M) g d d x
        (S.toSection x) := by
  have h := DifferentialGeometry.Analysis.Spectral.rfns_iteratedCovGrad_rs_eq_of_section_domDomCongr
    (I := I) (M := M) g d d ρ S
    (appCcRS (I := I) (M := M) g d d d
...
theorem dagTop_cap_unif :
  ...
  have hre : ∀ ρ : Equiv.Perm (Fin 3),
    ...
    intro ρ
    have h := DifferentialGeometry.Analysis.Spectral.rfns_iteratedCovGrad_reindexCoeffGen_eq
