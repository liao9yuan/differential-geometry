import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Background.C0One
...
open DifferentialGeometry.Analysis.Spectral
  (appCcRS appCcRS_sub_left appCcRS_sub_right ccTensorToHs permCoeff
...
theorem lowOneIntSub
    ...
    (hδZ : gFibreOpBound (I := I) (M := M) g
      (ccTensorBilinSymm (I := I) g
        (0 : SmoothCcTensor g 0 2)) δ) :
    lowOneAInt (I := I) (M := M) g T hδ_lt hδT hδZ -
        lowOneAInt (I := I) (M := M) g U hδ_lt hδU hδZ =
      lowOneADiff (I := I) (M := M)
  ...
  have hTcont :=
    jointContMDiff_toModel_continuous_slice
      ...
      (realizedSmallSet (δ := δ) (δ' := δ))
      (lowOneA_joint (I := I) (M := M) g T hδT hδZ) x
  have hUcont :=
    jointContMDiff_toModel_continuous_slice
      (I := I) g 3 2
      (lowOneA (I := I) (M := M) g U hδU hδZ)
      (realizedSmallSet (δ := δ) (δ' := δ))
      (lowOneA_joint (I := I) (M := M) g U hδU hδZ) x
  have hTint : IntervalIntegrable
      (fun s : ℝ => TensorRSSpace.toModel
        ((lowOneA (I := I) (M := M) g T hδT hδZ s).toSection x))
      MeasureTheory.volume 0 1 :=
    (hTcont.mono hSI).intervalIntegrable
  have hUint : IntervalIntegrable
      (fun s : ℝ => TensorRSSpace.toModel
        ((lowOneA (I := I) (M := M) g U hδU hδZ s).toSection x))
      MeasureTheory.volume 0 1 :=
    (hUcont.mono hSI).intervalIntegrable
  simp only [lowOneAInt, lowOneADiff, pathIntegralCoeffField_toModel,
...
theorem innerOnePairH2
    ...
    ∃ C : ℝ, 0 ≤ C ∧
      ...
      lowJetSq (I := I) (M := M) g 2
          (innerOne (I := I) (M := M) g T -
            innerOne (I := I) (M := M) g U) ≤
        (C * D2) ^ 2 := by
  obtain ⟨K, hK, happ⟩ := appH2 (I := I) (M := M) hDim g 3 3 3
  let P : SmoothCcTensor g 3 3 :=
    permCoeff (I := I) (M := M) g (finRotate 3)
  let J : ℝ := lowJetSq (I := I) (M := M) g 2 P
  let fr : ℝ := Module.finrank ℝ E
  let L : ℝ := K * fr ^ 2 * J
  let C : ℝ := Real.sqrt L
  have hJ : 0 ≤ J := jetNn (I := I) (M := M) (m := 2) g P
  ...
  have hins : lowJetSq (I := I) (M := M) g 2
      (slotInsertEndoCc (I := I) (M := M) g 2
        (symmRaiseEndo (I := I) (M := M) g D)) ≤
        fr ^ 2 * D2 ^ 2 := by
    refine (perturb_jet (I := I) (M := M) g 2 2 D hDself).trans ?_
  ...
  have hform :
    ...
    rw [innerOne, innerOne, ← appCcRS_sub_left,
      ← slotInsertEndoCc_sub, hraise]
  rw [hform]
  refine (happ _ _).trans ?_
  calc
    K * lowJetSq (I := I) (M := M) g 2
          (slotInsertEndoCc (I := I) (M := M) g 2
...
theorem innerOneBddH2
  ...
  have hins : lowJetSq (I := I) (M := M) g 2
      (slotInsertEndoCc (I := I) (M := M) g 2
        ...
        fr ^ 2 * R ^ 2 := by
    refine (perturb_jet (I := I) (M := M) g 2 2 W hWself).trans ?_
...
theorem innerActPairH2
  ...
  obtain ⟨Ci, hCi, hinPair⟩ :=
    innerOnePairH2 (I := I) (M := M) hDim g
  obtain ⟨Cib, hCib, hinBdd⟩ :=
    innerOneBddH2 (I := I) (M := M) hDim g
  obtain ⟨Ca, hCa, happ⟩ :=
  ...
  let IT : SmoothCcTensor g 3 3 := innerOne (I := I) (M := M) g T
  let IU : SmoothCcTensor g 3 3 := innerOne (I := I) (M := M) g U
  ...
  let X : SmoothCcTensor g 3 3 :=
    appCcRS (I := I) (M := M) g 3 3 3 (IT - IU) CT
  let Y : SmoothCcTensor g 3 3 :=
    appCcRS (I := I) (M := M) g 3 3 3 IU (CT - CU)
  let x : ℝ := Ca * (Ci * D2) * Cb
  ...
  have hITdiff : lowJetSq (I := I) (M := M) g 2 (IT - IU) ≤
      (Ci * D2) ^ 2 := by
    simpa only [IT, IU] using hinPair T U hT hU D2 hD2 hTU2
  have hIU : lowJetSq (I := I) (M := M) g 2 IU ≤
      (Cib * R) ^ 2 := by
    simpa only [IU] using hinBdd U hU R hR hU2
  have hCT : lowJetSq (I := I) (M := M) g 2 CT ≤ Cb ^ 2 := by
    simpa only [CT] using hbdd T gT hTtie hTnb
  have hCdiff : lowJetSq (I := I) (M := M) g 2 (CT - CU) ≤
      (Cp * N) ^ 2 := by
  ...
  have hsplit :
      innerAct (I := I) (M := M) g gT T -
          innerAct (I := I) (M := M) g gU U = X + Y := by
    simp only [innerAct, X, Y, IT, IU, CT, CU,
...
theorem aaBlkOnePairH2
      ...
      ∀ (gT gU : SmoothRiemannianMetric I M)
        ...
        lowJetSq (I := I) (M := M) g 2
            (connDiffContrInsertionField (I := I) g gT -
              connDiffContrInsertionField (I := I) g gU) ≤ OD ^ 2 →
        lowJetSq (I := I) (M := M) g 2
            (connDiffContrInsertionField (I := I) g gU) ≤ OU ^ 2 →
        lowJetSq (I := I) (M := M) g 2 ZT ≤ ZB ^ 2 →
        lowJetSq (I := I) (M := M) g 2 (ZT - ZU) ≤ ZD ^ 2 →
      lowJetSq (I := I) (M := M) g 2
          (aaBlkOne (I := I) (M := M) g gT pm ZT -
            aaBlkOne (I := I) (M := M) g gU pm ZU) ≤
        (C * P * (OD * ZB + OU * ZD)) ^ 2 := by
  obtain ⟨C4, hC4, hout⟩ :=
    appRoot_h2 (I := I) (M := M) hDim g 3 4 4
  obtain ⟨C3, hC3, hinn⟩ :=
    appRoot_h2 (I := I) (M := M) hDim g 3 3 4
  let C : ℝ := 2 * C4 * C3
  ...
  let X : SmoothCcTensor g 3 4 :=
    appCcRS (I := I) (M := M) g 3 3 4 (OT - OUf) ZT
  let Y : SmoothCcTensor g 3 4 :=
    appCcRS (I := I) (M := M) g 3 3 4 OUf (ZT - ZU)
  let x : ℝ := C3 * OD * ZB
  let y : ℝ := C3 * OU * ZD
  have hx0 : 0 ≤ x := by dsimp only [x]; positivity
  have hy0 : 0 ≤ y := by dsimp only [y]; positivity
  have hX : lowJetSq (I := I) (M := M) g 2 X ≤ x ^ 2 := by
    simpa only [X, x, OT, OUf] using
      hinn (connDiffContrInsertionField (I := I) g gT -
          connDiffContrInsertionField (I := I) g gU) ZT
        OD ZB hOD hZB hoDiff hZT
  have hY : lowJetSq (I := I) (M := M) g 2 Y ≤ y ^ 2 := by
    simpa only [Y, y, OUf] using
      hinn (connDiffContrInsertionField (I := I) g gU) (ZT - ZU)
        OU ZD hOU hZD hoU hZdiff
  have hinner :
      appCcRS (I := I) (M := M) g 3 3 4 OT ZT -
          appCcRS (I := I) (M := M) g 3 3 4 OUf ZU = X + Y := by
    simp only [X, Y, OT, OUf, appCcRS_sub_left, appCcRS_sub_right]
    module
  have hsum : lowJetSq (I := I) (M := M) g 2 (X + Y) ≤
      (2 * (x + y)) ^ 2 := by
    refine (jetAdd (I := I) (M := M) g 2 X Y).trans ?_
    calc
      2 * (lowJetSq (I := I) (M := M) g 2 X +
  ...
  have hform :
    ...
    rw [aaBlkOne, aaBlkOne, ← appCcRS_sub_right, hinner]
  rw [hform]
  have hxy0 : 0 ≤ 2 * (x + y) :=
