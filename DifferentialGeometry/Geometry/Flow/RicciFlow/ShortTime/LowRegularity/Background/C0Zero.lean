import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Background.C0Joint
...
open DifferentialGeometry.Analysis.Sobolev
  (cometricCastG0 iteratedCovGrad normSq_le_integral_of_pointwise_fiberNormSq_le_rs
    rfns_iteratedCovGrad_slotInsertEndoCc_le_endo rsDomDomCongrSection
    tensorL2Norm_sq_toFun_eq_integral_riemannianFiberNormSq_rs)
open DifferentialGeometry.Analysis.Spectral
  (appCc appCc_add_left appCc_smul_left appCc_smul_right appCc_sub_left appCcRS
    appCcRS_sub_left appCcRS_sub_right ccTensorToHs ccTensorToHs_smul edgeLiePairFam
    fullRaisedEndoField
    iteratedCovGrad_neg lc0Riem norm_iteratedCovGrad_domDomCongrSection permCoeff pureTrace
    pureTrace_toSection rfns_symmS_zero_le_fibreSmall slotExtend slotExtend_sub slotExtendIter
    symmS_eq_self_of_ccTensorBilin_symm)
open DifferentialGeometry.Geometry.Connection (slotInsertEndoCc)
open DifferentialGeometry.Geometry.Curvature
  (connDiffFib_apply_eval connDiffSection connDiffSection_toSection slotInsertEndoFib)
