...
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.EdgeDefectPair
...
theorem edge_center_pair_abs_of_carrier
  ...
  obtain ⟨delta2, hdelta2, hdelta2third, hprincipal⟩ :=
    bcD2_pair_abs_unif (I := I) (M := M) gBase hΛ he
  obtain ⟨rho, Ccorner, hrho, hCcorner, hcorner⟩ :=
    edge_corner_h4_unif (I := I) (M := M) hDim gBase hΛ
  ...
  let Yd := appCc (I := I) (M := M) g 4 2 B G
  intro hcarrier
