...
open DifferentialGeometry.Analysis.Spectral
  (appCc appCc_sub_left appCcRS appCcRS_sub_left appCcRS_sub_right
    appCcRS_zero_eq_appCc ccOperatorFieldComp fullRaisedEndoField operatorFieldApply)
...
theorem daWeightPairH2
  ...
  let X : SmoothCcTensor g 0 2 :=
    appCc (I := I) (M := M) g 2 2 (ET - EU) T
  let Y : SmoothCcTensor g 0 2 :=
    appCc (I := I) (M := M) g 2 2 EU (T - U)
  ...
  have hEU :
      lowJetSq (I := I) (M := M) g 2 EU ≤ (Be R) ^ 2 := by
    simpa only [EU] using
...
theorem daTransPairH2
  ...
  have hMono (σ : Equiv.Perm (Fin 4)) :
      lowJetSq (I := I) (M := M) g 2
        ...
        z ^ 2 := by
    simpa only [LowBaseInternal.daTransMono, WT, WU, z] using
      hmono WT WU σ (Bw R * D2)
        (mul_nonneg (hBw R hR) hD2) hWdiff
  let XA : SmoothCcTensor g 4 2 :=
    LowBaseInternal.daTransMono (I := I) (M := M)
        g gT T LowBaseInternal.daPermA -
      LowBaseInternal.daTransMono (I := I) (M := M)
        g gU U LowBaseInternal.daPermA
  let XB : SmoothCcTensor g 4 2 :=
    LowBaseInternal.daTransMono (I := I) (M := M)
        g gT T LowBaseInternal.daPermB -
      LowBaseInternal.daTransMono (I := I) (M := M)
        g gU U LowBaseInternal.daPermB
  have hXA : lowJetSq (I := I) (M := M) g 2 XA ≤ z ^ 2 := by
    simpa only [XA] using hMono LowBaseInternal.daPermA
  have hXB : lowJetSq (I := I) (M := M) g 2 XB ≤ z ^ 2 := by
    simpa only [XB] using hMono LowBaseInternal.daPermB
  have hsplit :
      LowBaseInternal.daTrans (I := I) (M := M) g gT T -
          LowBaseInternal.daTrans (I := I) (M := M) g gU U =
        XA - XB := by
    simp only [LowBaseInternal.daTrans, XA, XB]
    module
  rw [hsplit]
  refine (jetSub (I := I) (M := M) g 2 XA XB).trans ?_
  calc
...
theorem daOnePairH2
  ...
  let J0 : ℝ := lowJetSq (I := I) (M := M) g 2 C0
  ...
  have hCU :
      lowJetSq (I := I) (M := M) g 2 CU ≤ (Tu R) ^ 2 := by
    rw [show CU = (CU - C0) + C0 by module]
    refine (jetAdd (I := I) (M := M) g 2 (CU - C0) C0).trans ?_
    calc
      2 * (lowJetSq (I := I) (M := M) g 2 (CU - C0) +
          lowJetSq (I := I) (M := M) g 2 C0) ≤
        2 * ((Bt R * R) ^ 2 + S0 ^ 2) := by
          rw [hS0sq]
  ...
  let X : SmoothCcTensor g 3 2 :=
    appCcRS (I := I) (M := M) g 3 4 2 (CT - CU) DT
  let Y : SmoothCcTensor g 3 2 :=
    appCcRS (I := I) (M := M) g 3 4 2 CU (DT - DU)
