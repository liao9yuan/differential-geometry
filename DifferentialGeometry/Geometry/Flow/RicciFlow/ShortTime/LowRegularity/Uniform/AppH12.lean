...
theorem appRS_h1_unif
  ...
  obtain ⟨CΦ, hCΦ, hΦ6⟩ :=
    DifferentialGeometry.PDE.RicciFlow.h1Lp6RS_unif
      (I := I) (M := M) hDim gBase hΛ r c
  obtain ⟨CG6, hCG6, hG6⟩ :=
    DifferentialGeometry.PDE.RicciFlow.h1Lp6RS_unif
      (I := I) (M := M) hDim gBase hΛ p (r + 1)
  obtain ⟨CV, hCV, h63⟩ :=
    DifferentialGeometry.PDE.RicciFlow.fiberLp3_le_6_unif
      (I := I) (M := M) gBase Λ
  let CG3 : ℝ := CV * CG6
  let C : ℝ :=
  ...
  have hG3 : ∀ S : SmoothCcTensorH1 g p (r + 1),
    ...
    calc
      ...
      _ = CG3 * ‖S‖ := by
        dsimp [CG3]
        ring
  have hlocal := appRS_h1_of (I := I) (M := M) g p r c
  ...
  have hW3 : ∀ S : SmoothCcTensorH1 g p r,
    ...
    calc
      ...
      _ = CW3 * ‖S‖ := by
        dsimp only [CW3]
        ring
  have hlocal := appRS_h2_of (I := I) (M := M) g p r c
