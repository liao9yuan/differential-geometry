...
open DifferentialGeometry.Analysis.Spectral.LieCorr0Core
...
theorem amix_h1_unif
        ...
        ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2),
          ...
          (∑ i ∈ Finset.range 2,
            ‖iteratedCovGrad (I := I) g₀ 2 2 i
              (lc0AMix (I := I) (M := M) g₀ g₁ gBase)‖ ^ 2) ≤
            (B0 R + B1 R * A) ^ 2 := by
  classical
  have hΛ0 : 0 ≤ Λ := by linarith
  obtain ⟨Bt2, hBt2, htr2⟩ :=
    trace_h2_unif (I := I) (M := M) 2 hDim gBase hΛ0 hδ₀
  obtain ⟨Bt3, hBt3, htr3⟩ :=
    trace_h2_unif (I := I) (M := M) 3 hDim gBase hΛ0 hδ₀
  obtain ⟨Bt4, hBt4, htr4⟩ :=
    trace_h2_unif (I := I) (M := M) 4 hDim gBase hΛ0 hδ₀
  obtain ⟨BK, hBK, hkbg⟩ :=
    kappaBg_h1_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨Cq, hCq, hqprod⟩ :=
    appRS_h22_unif (I := I) (M := M) hDim gBase hΛ 2 5 3
  obtain ⟨Cn, hCn, hnprod⟩ :=
    appRS_h1_unif (I := I) (M := M) hDim gBase hΛ 2 3 6
  obtain ⟨Cm, hCm, hmprod⟩ :=
    appRS_h2_unif (I := I) (M := M) hDim gBase hΛ 2 6 4
  ...
  have hN :
    ...
    rw [h1_jet_sq (I := I) (M := M) g₀ 2 6
      (appCcRS (I := I) (M := M) g₀ 2 3 6 KBs Q)] at hsquare
  ...
  have hMid :
    ...
    rw [h1_jet_sq (I := I) (M := M) g₀ 2 4
      (appCcRS (I := I) (M := M) g₀ 2 6 4 T4 N)] at hsquare
  ...
  have hOa :
    ...
    rw [h1_jet_sq (I := I) (M := M) g₀ 2 2
      (appCcRS (I := I) (M := M) g₀ 2 4 2 T2a Mid)] at hsquare
