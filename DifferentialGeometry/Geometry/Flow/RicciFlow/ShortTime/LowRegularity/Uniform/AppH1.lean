...
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.AppGrad
...
theorem appCc_h1_unif
  ...
  obtain ⟨Crs, hCrs, happ⟩ :=
    appRS_h1_unif (I := I) (M := M) hDim gBase hΛ 0 2 2
  ...
  have hmix' :
    ...
    simpa only [Y, appCcRS_zero_eq_appCc] using hmix
  have hspec :
...
theorem appCc_h2cov_unif
  ...
  obtain ⟨K, hK⟩ := exists_curv_actions (I := I) (M := M) gBase hΛ
  obtain ⟨Cm, hCm, hmor⟩ :=
    DifferentialGeometry.PDE.RicciFlow.morreyRS_unif
      (I := I) (M := M) hDim gBase hΛ 3 2
  obtain ⟨Cg, hCg, hgrad⟩ :=
    appCc_grad_unif (I := I) (M := M) hDim gBase hΛ 1 2
  let Ch : ℝ := h2CovsumC K.rankTwo
  ...
  have hW1 :
    ...
    calc
      ...
      _ = ‖iteratedCovGrad (I := I) g 0 2 (1 + 1) U‖ :=
        icg_comp_norm (I := I) (M := M) g 2 1 1 U
  ...
  have hslotSup : ∀ x : M,
    ...
    rw [rfns_slotExtend_eq (I := I) (M := M) g 3 2 Φ x]
    rw [hDim]
    calc
      (3 : ℝ) *
          riemannianFiberNormSq (I := I) (M := M) g 3 2 x
  ...
  have hY1 :
    ...
    rw [show covGrad (I := I) (M := M) g 0 2 Y =
      ...
      dsimp [Y]
      exact covGrad_appCc_eq (I := I) (M := M) g 3 2 Φ W]
    calc
      _ ≤ ‖appCc (I := I) (M := M) g 3 3
              (covGrad (I := I) (M := M) g 3 2 Φ) W‖ +
            ‖appCc (I := I) (M := M) g 4 3
              (slotExtend (I := I) (M := M) g 3 2 Φ)
...
theorem appCc_h23_unif
  ...
  obtain ⟨Cg, hCg, hgrad⟩ :=
    appCc_grad_unif (I := I) (M := M) hDim gBase hΛ 2 2
  ...
  have hV0 :
    ...
    simpa only [V, Nat.add_zero] using
      (icg_comp_norm (I := I) (M := M) g 2 1 0 U)
  have hV1 :
  ...
  have hV2 :
    ...
    simpa only [V] using
      (icg_comp_norm (I := I) (M := M) g 2 1 2 U)
  have hVjet :
      (∑ j ∈ Finset.range 3,
  ...
  have hW1 :
    ...
    calc
      ...
      _ = ‖iteratedCovGrad (I := I) g 0 2 (2 + 1) U‖ :=
        icg_comp_norm (I := I) (M := M) g 2 2 1 U
      _ ≤ Ch * N := by norm_num at hpick ⊢; exact hpick
  have hΦsup : ∀ x : M,
  ...
  have hslotSup : ∀ x : M,
    ...
    rw [rfns_slotExtend_eq (I := I) (M := M) g 4 2 Φ x]
    rw [hDim]
    calc
      (3 : ℝ) *
          riemannianFiberNormSq (I := I) (M := M) g 4 2 x
  ...
  have hY1 :
    ...
    rw [show covGrad (I := I) (M := M) g 0 2 Y =
      ...
      dsimp [Y]
      exact covGrad_appCc_eq (I := I) (M := M) g 4 2 Φ W]
    calc
      _ ≤ ‖appCc (I := I) (M := M) g 4 3
              (covGrad (I := I) (M := M) g 4 2 Φ) W‖ +
            ‖appCc (I := I) (M := M) g 5 3
              (slotExtend (I := I) (M := M) g 4 2 Φ)
