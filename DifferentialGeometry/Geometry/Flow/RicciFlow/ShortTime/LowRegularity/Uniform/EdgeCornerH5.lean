import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.EdgeCornerH4
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.AppH12
...
open DifferentialGeometry.Analysis.Sobolev
  (iteratedCovGrad iteratedCovGrad_succ iteratedCovGrad_zero
   ...
   rfns_iteratedCovGrad_rsDomDomCongr_both_eq rsDomDomCongrSection
   rsDomDomCongrSection_toSection tensorL2Norm_sq_toFun_eq_integral_riemannianFiberNormSq_rs)
open DifferentialGeometry.Analysis.Spectral
  (appCc appCcRS_zero_eq_appCc ccTensorToHs ccTensorToHs_smul deTurckPhiMetTotal h1_jet_sq
   hsJet_le oneMinusConnLapSmooth pureTrace pureTrace_toSection
   rfns_iteratedCovGrad_slotExtend_le riemannianFiberNormSq_iteratedCovGrad_comp
...
private theorem rs_dom_iterated_norm_sq_h5
  ...
  rw [tensorL2Norm_sq_toFun_eq_integral_riemannianFiberNormSq_rs
    ...
    tensorL2Norm_sq_toFun_eq_integral_riemannianFiberNormSq_rs
      (I := I) (M := M) g r (s + i)]
  refine MeasureTheory.integral_congr_ae (Filter.Eventually.of_forall (fun x => ?_))
  have h := rfns_iteratedCovGrad_rsDomDomCongr_both_eq
...
private theorem edge_corner_inner_h1_of_jets_h5
    ...
    ∃ K : ℝ, 0 ≤ K ∧
      ∀ (T : SmoothCcTensor g 0 2) (B : SmoothCcTensor g 4 2),
        ...
        let Q20 : SmoothCcTensor g 0 4 :=
          appCc (I := I) (M := M) g 4 4 A20 HT
        let Q11L : SmoothCcTensor g 0 4 :=
          appCc (I := I) (M := M) g 5 4 A11L D3T
        let Q11R : SmoothCcTensor g 0 4 :=
          appCc (I := I) (M := M) g 5 4 A11R D3T
  ...
  obtain ⟨C20, hC20, happ20⟩ :=
    appRS_h1_unif (I := I) (M := M) hDim gBase hΛ 0 4 4
  obtain ⟨C11, hC11, happ11⟩ :=
    appRS_h2_unif (I := I) (M := M) hDim gBase hΛ 0 5 4
  ...
  have hQ11R :
      ‖(⟨Q11R⟩ : SmoothCcTensorH1 g 0 4)‖ ≤
        C11 * (Kn * CB * y) * (CJ * q) := by
    simpa only [Q11R, appCcRS_zero_eq_appCc] using
    ...
    ∃ C : ℝ, 0 ≤ C ∧
      ...
      ∀ Q20 Q11L Q11R : SmoothCcTensor g 0 4,
        let P20 : SmoothCcTensor g 0 2 :=
          appCc (I := I) (M := M) g 4 2 Tr Q20
        let P11L : SmoothCcTensor g 0 2 :=
          appCc (I := I) (M := M) g 4 2 Tr Q11L
        let P11R : SmoothCcTensor g 0 2 :=
          appCc (I := I) (M := M) g 4 2 Tr Q11R
        ...
        _ = ‖(⟨Q⟩ : SmoothCcTensorH1 g 0 4)‖ ^ 2 := by
          simpa only [Finset.sum_range_succ, Finset.sum_range_zero,
            zero_add, iteratedCovGrad_zero] using
              (h1_jet_sq (I := I) (M := M) g 0 4 Q).symm
        _ ≤ _ := le_rfl
...
private theorem three_corner_pair_h5_le
  ...
  have hspec (P : SmoothCcTensor g 0 2) :
    ...
    have hintrinsic := h1_jet_sq (I := I) (M := M) g 0 2 P
  ...
  have hshift :
    ...
    simpa only [W] using
      (smoothCcToTensorHs_add_two_norm_eq_oneMinusConnLap
        (I := I) (M := M) g 3 T).symm
  have h20 := oneMinusConnLapSmooth_pair_h3_h1
    (I := I) (M := M) g W P20
  have h11L := oneMinusConnLapSmooth_pair_h3_h1
    (I := I) (M := M) g W P11L
  have h11R := oneMinusConnLapSmooth_pair_h3_h1
    (I := I) (M := M) g W P11R
...
theorem edge_corner_h1_unif
  ...
  obtain ⟨rhoB, CB, hrhoB, hCB, hedge⟩ :=
    edge_coeff_h3_unif (I := I) (M := M) hDim gBase hΛ
  ...
  let B : SmoothCcTensor g 4 2 :=
    lieRefold2 (I := I) (M := M) g T hdelta hdeltaZ a +
      (deTurckPhiMetTotal (I := I) (M := M) g gBase gm -
        deTurckPhiMetTotal (I := I) (M := M) g gBase g) +
      (-2 * a : ℝ) • LowBaseInternal.ricciTop
        (I := I) (M := M) g gm T
  let HT : SmoothCcTensor g 0 4 :=
    iteratedCovGrad (I := I) g 0 2 2 T
  let D3T : SmoothCcTensor g 0 5 :=
...
set_option backward.isDefEq.respectTransparency false in
theorem edge_corner_pair_h5_unif
    (hDim : Module.finrank ℝ E = 3)
    (gBase : SmoothRiemannianMetric I M)
    {Λ : ℝ} (hΛ : 1 ≤ Λ) :
    ∃ rho : ℝ, 0 < rho ∧
  ...
  obtain ⟨rho, hrho, hcorner⟩ :=
    edge_corner_h1_unif (I := I) (M := M) hDim gBase hΛ
  refine ⟨rho, hrho, ?_⟩
  intro g hEq hjet
