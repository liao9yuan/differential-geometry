...
theorem low1_pair_h4_unif
    ...
    ∃ C0 C1 : ℝ, 0 ≤ C0 ∧ 0 ≤ C1 ∧
      ∀ g : SmoothRiemannianMetric I M,
        ...
        ∀ (T : SmoothCcTensor g 0 2)
          ...
          let D1 := rhsLow1PathIntegral (I := I) (M := M) g gBase T 0
  ...
  obtain ⟨B0, B1, hB0, hB1, hcoeff⟩ :=
    rhs1_path_unif (I := I) (M := M) hDim gBase hΛ
      hδ₀_nonneg hδ₀_lt
  obtain ⟨Capp, hCapp, happ⟩ :=
    appCc_h23_h2_unif (I := I) (M := M) hDim gBase hΛ
  ...
  let D1 : SmoothCcTensor g 3 2 :=
    rhsLow1PathIntegral (I := I) (M := M) g gBase T 0
      hδ₀_lt hδ hδ₀_lt hδZ
  let Y : SmoothCcTensor g 0 2 :=
    appCc (I := I) (M := M) g 3 2 D1
      (iteratedCovGrad (I := I) g 0 2 1 T)
  ...
  have hinterp : y ^ 2 ≤ x * z := by
    exact prodOfParam hx hz (fun t ht => by
      simpa only [x, y, z] using specInterp3 (I := I) (M := M) g 2 T ht)
...
theorem low1_pair_abs_unif
  ...
  obtain ⟨C0, C1, hC0, hC1, hpair⟩ :=
    low1_pair_h4_unif (I := I) (M := M) hDim gBase hΛ
      hδ₀_nonneg hδ₀_lt
  let e : ℝ := η / 2
  let D : ℝ := 2 * (C1 + 1)
  let ρ : ℝ := min 1 (η / D)
