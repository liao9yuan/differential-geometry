...
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.GridTame
...
theorem ricci0_h1_unif
  ...
  obtain ⟨Bk0, Bk1, hBk0, hBk1, hker⟩ :=
    h1_grid_unif (I := I) (M := M) hDim gBase hΛ0
      (r := 2) (s := 4) Ck hCk
  ...
  have hkernel : (∑ i ∈ Finset.range 2,
      ‖iteratedCovGrad (I := I) g₀ 2 4 i
        (linearizedRicciConnDiffOrder0KernelField (I := I) g₀ g₁)‖ ^ 2) ≤
      (Bk0 R + Bk1 R * A) ^ 2 := by
  ...
  rw [h1_jet_sq (I := I) (M := M) g₀ 2 2
    (appCcRS (I := I) (M := M) g₀ 2 4 2
      (ricciCometricFourTraceCastG0 (I := I) g₀ g₁)
      (linearizedRicciConnDiffOrder0KernelField (I := I) g₀ g₁))] at hsquare
