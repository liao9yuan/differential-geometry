...
theorem gal_arm_pair3_pair4_unif_of_mem
  ...
  obtain ⟨δ3, hδ3, hδ3third, hpair3⟩ :=
    gal_arm_pair3_unif_of_mem_below (I := I) (M := M) hDim gBase hΛ hη3
  obtain ⟨δ4, hδ4, hδ4third, hpair4⟩ :=
    gal_arm_pair4_unif_of_mem_below (I := I) (M := M) hDim gBase hΛ hη4
  let δ : ℝ := min δ3 δ4
...
theorem gal_arm_pair3_pair4_unif_of_mem_below
    (hDim : Module.finrank ℝ E = 3)
    (gBase : SmoothRiemannianMetric I M)
                  ...
                  G4 * ((∑ i ∈ F,
                    ...
                    (∑ i ∈ F,
                      tensorSobolevWeight (I := I) (M := M) i (3 : ℝ) *
                        (c i) ^ 2) ^ 2)) := by
  obtain ⟨δ3, hδ3, hδ3third, hpair3⟩ :=
    gal_arm_pair3_unif_of_mem_below (I := I) (M := M) hDim gBase hΛ one_pos
  obtain ⟨δ4, hδ4, _hδ4third, hpair4⟩ :=
    gal_arm_pair4_unif_of_mem_below (I := I) (M := M) hDim gBase hΛ one_pos
