...
theorem dla_h1_unif
        ...
        ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2),
          ...
          (∑ i ∈ Finset.range 2,
            ‖iteratedCovGrad (I := I) g₀ 2 2 i
              (deTurckLieDLaCoeffField (I := I) (M := M)
                g₀ g₁ gBase)‖ ^ 2) ≤
            (B0 R + B1 R * A) ^ 2 := by
  classical
  have hΛ0 : 0 ≤ Λ := by linarith
  obtain ⟨F, hF, hfix⟩ := connFix_grid_unif
    (I := I) (M := M) gBase hΛ
  obtain ⟨C, hC, hpt⟩ := dla_grid_of_conn
    (I := I) (M := M) hδ₀ F hF
  obtain ⟨B0, B1, hB0, hB1, hint⟩ := h1_grid_unif
  ...
  apply hint g₀ hEq hjet1 hjet2 P
    (deTurckLieDLaCoeffField (I := I) (M := M) g₀ g₁ gBase)
    R A hR hA hP htop
  intro i hi x
