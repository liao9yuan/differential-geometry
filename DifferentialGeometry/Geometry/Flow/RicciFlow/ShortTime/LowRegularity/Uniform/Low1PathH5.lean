...
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.EdgeCarrierH5
...
open DifferentialGeometry.Analysis.Spectral
  (appCc ccTensorToHs ccTensorToHs_smul
...
theorem low1_path_h3
  ...
  classical
  obtain ⟨Ca, hCa, happ⟩ :=
    appRS_h3_sup_unif (I := I) (M := M) gBase Lambda 0 3 2
  obtain ⟨Cm0, hCm0, hmor0⟩ :=
    morreyRS_unif (I := I) (M := M) hDim gBase hLambda 0 3
  obtain ⟨Cm3, hCm3, hmor3⟩ :=
    morreyRS_unif (I := I) (M := M) hDim gBase hLambda 3 2
  obtain ⟨B0, B1, hB0, hB1, hcoeff⟩ :=
    rhs1_path_unif (I := I) (M := M) hDim gBase hLambda
  ...
  obtain ⟨Kc, hKc, htow⟩ :=
    c1JetTowerQBg (I := I) (M := M) hDim g gBase
  ...
  let D1 : SmoothCcTensor g 3 2 := rhsLow1PathIntegral (I := I) (M := M)
    g gBase T 0 delta_lt hdelta delta_lt hdeltaZ
  let W : SmoothCcTensor g 0 3 := iteratedCovGrad (I := I) g 0 2 1 T
  let Y : SmoothCcTensor g 0 2 := appCc (I := I) (M := M) g 3 2 D1 W
    ...
    calc
      _ = ∑ j ∈ Finset.range 4,
        ...
        simpa only [W, Nat.reduceAdd, Nat.add_comm] using congrArg (fun z : ℝ => z ^ 2)
          (icg_comp_norm (I := I) (M := M) g 2 1 j T)
      _ ≤ ∑ j ∈ Finset.range 5,
    ...
    have hterm : ∀ i ∈ Finset.range 4,
      ...
      have hi : ‖iteratedCovGrad (I := I) g 3 2 i D1‖ ^ 2 ≤
          Kc i * (1 + ∑ j ∈ Finset.range (i + 2),
            ‖iteratedCovGrad (I := I) g 0 2 j T‖ ^ 2) := by
        simpa only [D1, delta_lt, LowBaseInternal.c1_eq] using hi0
...
theorem low1_pair_h5_unif
  ...
  let D1 : SmoothCcTensor g 3 2 := rhsLow1PathIntegral (I := I) (M := M)
    g gBase T 0 delta_lt hdelta delta_lt hdeltaZ
  let Y : SmoothCcTensor g 0 2 := appCc (I := I) (M := M) g 3 2 D1
    (iteratedCovGrad (I := I) g 0 2 1 T)
  let y : ℝ := ‖ccTensorToHs (I := I) (M := M) g 2 (3 : ℝ) T‖
  let q : ℝ := ‖ccTensorToHs (I := I) (M := M) g 2 (4 : ℝ) T‖
