import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.EdgeCoeffH3
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.EdgeSwapPair
...
private theorem fiber_app_app_le
    ...
    ∀ x : M,
      fiberLpFun g a d
          (appCcRS (I := I) (M := M) g a c d Tr
            (appCcRS (I := I) (M := M) g a b c A W)) x ≤
  ...
  have houter := riemannianFiberNormSq_compRS_le_mul
    (I := I) (M := M) g a c d x (Tr.toSection x)
      ((appCcRS (I := I) (M := M) g a b c A W).toSection x)
    ...
    calc
      _ ≤ riemannianFiberNormSq (I := I) (M := M) g c d x
        ...
        riemannianFiberNormSq (I := I) (M := M) g a c x
          ((appCcRS (I := I) (M := M) g a b c A W).toSection x) := by
            simpa only [appCcRS_toSection] using houter
      _ ≤ K ^ 2 *
...
theorem edge_corner_h4_unif
  ...
  obtain ⟨C6B, hC6B, h6B⟩ :=
    h1Lp6RS_unif (I := I) (M := M) hDim gBase hΛ 4 4
  obtain ⟨C6T, hC6T, h6T⟩ :=
    h1Lp6RS_unif (I := I) (M := M) hDim gBase hΛ 0 4
  obtain ⟨C63, hC63, h63⟩ :=
    fiberLp3_le_6_unif (I := I) (M := M) gBase Λ
  obtain ⟨Cmor, hCmor, hmor⟩ :=
    morreyRS_unif (I := I) (M := M) hDim gBase hΛ 4 3
  obtain ⟨Kcurv, hKcurv⟩ :=
  ...
  have hA11Lrfns : ∀ p : M,
    ...
    rw [show riemannianFiberNormSq (I := I) (M := M) g 5 4 p
      ...
      simpa only [A11L, n] using
        rfns_slotExtend_eq (I := I) (M := M) g 4 3 A11 p]
    calc
      _ ≤ n * (Cmor * CB * y) ^ 2 :=
        mul_le_mul_of_nonneg_left (hA11rfns p) hn
  ...
  have hA11Rrfns : ∀ p : M,
    ...
    rw [show riemannianFiberNormSq (I := I) (M := M) g 5 4 p
      ...
      simpa only [A11R, A11] using
        rfns_covGrad_slotExtend_eq (I := I) (M := M) g 4 2 B p]
    exact hA11Lrfns p
  have hA11Lcap : ∀ p : M,
      fiberLpFun g 5 4 A11L p ≤ Kn * Cmor * CB * y := by
    intro p
    have hsqrt := Real.sqrt_le_sqrt (hA11Lrfns p)
    simpa only [fiberLpFun, tensorPointwiseNorm,
  ...
  have hA11Rcap : ∀ p : M,
    ...
    intro p
    have hsqrt := Real.sqrt_le_sqrt (hA11Rrfns p)
    simpa only [fiberLpFun, tensorPointwiseNorm,
      Real.sqrt_sq (mul_nonneg
        (mul_nonneg (mul_nonneg hKn hCmor) hCB) hy)] using hsqrt
