...
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.LowBaseC2Fibre
...
theorem a2_ladder_affine_unif
            ...
            (m : ℕ),
              ‖smoothCcToTensorHs (I := I) (M := M) g (m : ℝ)
                  ((lowBaseData (I := I) (M := M) g gBase T
                    (lt_of_le_of_lt hδ_le (by norm_num)) hδ hδZ).a2
                      (I := I) (M := M) T)‖ ≤
                K * (δ / (1 - δ) ^ 2 + R) *
  ...
  obtain ⟨ρ, K, hρ, hK, hcap⟩ :=
    c2_fibre_cap_unif (I := I) (M := M) hDim gBase hΛ
  refine ⟨ρ, K, hρ, hK, ?_⟩
  intro δ hδ_le hδ0 R hR0 hRρ g hEq hjet
  obtain ⟨Kc, hKc_nn, htower⟩ := c2JetTowerQ (I := I) (M := M) hDim g gBase
  ...
  have hc2pt : ∀ x : M,
      riemannianFiberNormSq (I := I) (M := M) g 4 2 x (A.C2.toSection x) ≤
        (K * (δ / (1 - δ) ^ 2 + R)) ^ 2 := by
    simpa only [A] using
