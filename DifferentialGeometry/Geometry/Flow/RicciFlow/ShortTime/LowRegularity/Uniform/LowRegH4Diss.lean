import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.GalerkinH4
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.LowRegFatou
...
theorem lowregFatouE4D5AtBg_of_pair
    ...
    (hL2H4 : ∀ N : ℕ, ∫ t, galerkinEnergy (I := I) (M := M)
      (eigenIdxFinset (I := I) (M := M) g N)
      (lowregProjMode (I := I) (M := M) g fseq N) 4 t ∂(timeMeasure T) ≤ Bd4) :
    ∃ Φ4 Φ5 : ℝ,
      (∀ N : ℕ, ∀ t ∈ Set.Icc (0 : ℝ) T,
        galerkinEnergy (I := I) (M := M)
          (eigenIdxFinset (I := I) (M := M) g N)
      ...
      (∀ N : ℕ, ∫ t, galerkinEnergy (I := I) (M := M)
        (eigenIdxFinset (I := I) (M := M) g N)
        (lowregProjMode (I := I) (M := M) g fseq N) 5 t ∂(timeMeasure T) ≤ Φ5) := by
  classical
  ...
  have hUderiv : ∀ N, ∀ t ∈ Set.Ico (0 : ℝ) T,
    ...
    · exact lowregForceCont (I := I) (M := M) g gBase hδ hCtop hB0 hB1 hρ hP
        hreal htame N (U N) (fun j _ => hUcont N j (by assumption)) i
    · exact lowregForceMode (I := I) (M := M) g
        (lowregStateRad_pos hCtop hB1 hρ hP).le hT hT1 N fseq
        (hball N) (hnem N) i
  have hstate : ∀ N, ∀ t ∈ Set.Icc (0 : ℝ) T,
      ‖galLowView (I := I) (M := M) g 1
        (finiteEigenComboHs (I := I) (M := M) g
          (eigenIdxFinset (I := I) (M := M) g N) (U N t)
          (((1 : ℕ) : ℝ) + 2))‖ ≤ lowregStateRad Ctop B1 ρ P := by
    intro N t ht
    simpa only [U] using lowregProjMode_state_bound (I := I) (M := M) g hT hT1 N
  ...
  obtain ⟨Cseed, hCseed, hseed⟩ := lowRegSeedMass (I := I) (M := M) g gBase
  ...
  have hclosure : ∀ N : ℕ, ∀ t ∈ Set.Ico (0 : ℝ) T,
    ...
    let arm := galArmVecBg (I := I) (M := M) g gBase hRpos.le hδ hrealR F (U N t)
    let seed := lowregNfun (I := I) (M := M) g gBase hδ hCtop hB1 hρ hP hreal
    ...
    have hsplit : ∀ i ∈ F, force i = arm.coeff i + seed.coeff i := by
      ...
      rw [galForceArmBg (I := I) (M := M) g gBase hδ hδ0 hδ3 hCtop hB1 hρ hP
    ...
    calc
      _ ≤ 2 * |∑ i ∈ eigenIdxFinset (I := I) (M := M) g N,
            tensorSobolevWeight (I := I) (M := M) i (4 : ℝ) *
              (U N t i * (galArmVecBg (I := I) (M := M) g gBase hRpos.le hδ
                hrealR (eigenIdxFinset (I := I) (M := M) g N)
                (U N t)).coeff i)| +
          2 * |∑ i ∈ eigenIdxFinset (I := I) (M := M) g N,
            tensorSobolevWeight (I := I) (M := M) i (4 : ℝ) *
              (U N t i * (lowregNfun (I := I) (M := M) g gBase hδ hCtop hB1
