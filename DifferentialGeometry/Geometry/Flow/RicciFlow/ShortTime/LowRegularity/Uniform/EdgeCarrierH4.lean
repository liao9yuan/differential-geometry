import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.Low1PathPair
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Background.H2
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Background.C2Pair
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.CenterPathPairing
...
open DifferentialGeometry.Analysis.Spectral
  (appCc appCc_add_left appCcRS_zero_eq_appCc appRS_h2_h2_h2 ccTensorToHs
   ccTensorToHs_coeff ccTensorToHs_smul deTurckPhiMetTotal hs2_low2 hsJet_le lc0Riem
   oneMinusConnLapSmooth phiMetCurvCoeff smoothCcToTensorHs
...
theorem rhs_self_bg_corr_action_h2
  ...
  obtain ⟨B0, B1, hB0, hB1, hcorr⟩ :=
    rhs_self_bg_corr_h2 (I := I) (M := M) hDim g gB
  ...
  have hT2 : lowJetSq (I := I) (M := M) g 2 T ≤ R0 ^ 2 := by
  ...
  have hcoeff :
    ...
    simpa only [lowJetSq, Nat.reduceAdd, A, mul_assoc] using
      hcorr T hδ_le hδ_nonneg hδ hδZ R0 (C3 * y)
        hR0 (mul_nonneg hC3 hy) hT2 hT3sum s hs
  have hTlow : lowJetSq (I := I) (M := M) g 2 T ≤ (C2 * x) ^ 2 :=
    hjet2 T
  have hY : lowJetSq (I := I) (M := M) g 2 Y ≤ Q ^ 2 := by
    simpa only [Y, Q, appCcRS_zero_eq_appCc] using
      happ
        (LowBaseInternal.rhsSelfLow (I := I) (M := M)
            g gB T hδ hδZ s -
          LowBaseInternal.rhsSelfLow (I := I) (M := M)
...
theorem edge_carrier_h2
  ...
  have hS : lowJetSq (I := I) (M := M) g 2 S ≤
      (Qs * (1 + y ^ 2)) ^ 2 := by
    ...
    calc
      ...
      _ = Ks * (1 + y ^ 2) ^ 2 := by
        dsimp only [Ks]
        ring
  have hC : lowJetSq (I := I) (M := M) g 2 C ≤
      (Qc * (1 + y ^ 2)) ^ 2 := by
    dsimp only [C]
  ...
  have hSC : lowJetSq (I := I) (M := M) g 2 (S + C) ≤
      (Q * (1 + y ^ 2)) ^ 2 := by
    refine (jetAdd (I := I) (M := M) g 2 S C).trans ?_
    calc
      2 * (lowJetSq (I := I) (M := M) g 2 S +
          lowJetSq (I := I) (M := M) g 2 C) ≤
          2 * ((Qs * (1 + y ^ 2)) ^ 2 +
            (Qc * (1 + y ^ 2)) ^ 2) :=
...
theorem carrier_pair_abs_of_h2_poly
    (g : SmoothRiemannianMetric I M)
...
theorem edge_center_h4_unif
  ...
  have hpair := carrier_pair_abs_of_h2_poly
...
theorem edge_center_path_h4_unif
    (hDim : Module.finrank ℝ E = 3)
    (gBase : SmoothRiemannianMetric I M)
    {Λ : ℝ} (hΛ : 1 ≤ Λ) :
    ∀ {eta : ℝ}, 0 < eta →
  ...
  obtain ⟨delta2, R2, hdelta2, hdelta2third, hR2, hR2one, hpoint⟩ :=
    edge_center_h4_unif (I := I) (M := M) hDim gBase hΛ heta
  refine ⟨delta2, R2, hdelta2, hdelta2third, hR2, hR2one, ?_⟩
  intro g hEq hjet
