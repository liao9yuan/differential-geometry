import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.InvCoeffLip
...
open DifferentialGeometry.Analysis.Sobolev
  (gInvDiffRaisedEndoField gInvDiffSlotCoeff_eq_slotInsertEndoCc iteratedCovGrad
   iteratedCovGrad_zero
   normSq_le_integral_of_pointwise_fiberNormSq_le_rs
   norm_le_of_pointwise_fiberNormSq_bound_rs rfns_iteratedCovGrad_rsDomDomCongr_both_eq
   rsDomDomCongrSection
   tensorL2Norm_sq_toFun_eq_integral_riemannianFiberNormSq_rs)
open DifferentialGeometry.Analysis.Spectral
  (appCcRS appCcRS_sub_left appCcRS_sub_right ccOperatorFieldComp ccTensorToHs
   ccTensorToHs_smul gInvDiffSlotCoeff iteratedCovGrad_add pureTrace pureTrace_split
   rfns_iteratedCovGrad_slotExtend_le slotExtend slotExtendIter)
...
open DifferentialGeometry.Geometry.Connection
  (curvatureRefoldMonomialCoeffField endoSlotZeroCcTensor slotInsertEndoCc)
...
private theorem gInvDiffSlotCoeff_eq_endoSlotZero
  ...
  change gInvDiffSlotCoeff (I := I) g₀ g₁ =
...
theorem trace24_h2_lip_unif
  ...
  have hslot1 :
    ...
    rw [endoSlotZero_sub_traceLip,
      ← gInvDiffSlotCoeff_eq_endoSlotZero (I := I) (M := M) g gT,
      ← gInvDiffSlotCoeff_eq_endoSlotZero (I := I) (M := M) g gU]
