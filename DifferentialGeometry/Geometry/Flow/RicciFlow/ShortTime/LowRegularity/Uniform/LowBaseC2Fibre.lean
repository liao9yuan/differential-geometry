...
theorem c2_fibre_cap_unif
        ...
        ∀ (T : SmoothCcTensor g 0 2)
          ...
          ∀ x : M,
            riemannianFiberNormSq (I := I) (M := M) g 4 2 x
                ((lowBaseData (I := I) (M := M) g gBase T
                  (lt_of_le_of_lt hδ_le (by norm_num)) hδ hδZ).C2.toSection x) ≤
              (K * (δ / (1 - δ) ^ 2 + R)) ^ 2 := by
  classical
  ...
  let Φ := rhsRefoldTop (I := I) (M := M) g gBase T hδ hδZ
  let Ψ := LowBaseInternal.rhsSelfTop (I := I) (M := M) g T hδ hδZ
  let C := deTurckPhiMetTotal (I := I) (M := M) g gBase g
  have hSI : Set.uIcc (0 : ℝ) 1 ⊆
      realizedSmallSet (δ := δ) (δ' := δ) := by
    rw [Set.uIcc_of_le zero_le_one]
    exact Icc_subset_realizedSmallSet hδ_lt hδ_lt
  have hΦ := rhsRefoldTop_joint (I := I) (M := M) g gBase T hδ_lt hδ hδZ
  have hΨ := LowBaseInternal.selfTop_joint (I := I) (M := M) g T hδ hδZ
  ...
  rw [LowBaseInternal.c2_eq (I := I) (M := M) g gBase T hδ_lt hδ hδZ]
  ...
  let Dtop : SmoothCcTensor g 4 2 :=
    lieRefold2 (I := I) (M := M) g T hδ hδZ s +
      (-2 * s : ℝ) • LowBaseInternal.ricciTop (I := I) (M := M) g gm T
  ...
  have hsplit : Φ s + Ψ s - C = Dtop + Ddev := by
    rw [LowBaseInternal.topKernel_eq (I := I) (M := M) g gBase T hδ hδZ s]
