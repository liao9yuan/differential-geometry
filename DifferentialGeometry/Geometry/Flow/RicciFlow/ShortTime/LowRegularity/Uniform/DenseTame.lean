import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.DenseSolve
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.TameEstimate
...
theorem coreN_outer_unif
  ...
  obtain ⟨ρ, Ctop, B0, B1, hρ, hCtop, hB0, hB1, hcore⟩ :=
    coreN_tame_unif (I := I) (M := M) hDim gBase hΛ
      hδ₀_nonneg hδ₀_lt
  ...
  have hyrep : coreRep g yQ = coreRep g y := by
    ...
    rw [coreRep_spec, coreRep_spec]
  have hxN :
      coreN (I := I) (M := M) g gBase hδ₀_lt hrealQ xQ =
        coreN (I := I) (M := M) g gBase hδ₀_lt
    ...
    rw [hxrep]
  have hyN :
      coreN (I := I) (M := M) g gBase hδ₀_lt hrealQ yQ =
        coreN (I := I) (M := M) g gBase hδ₀_lt
...
theorem lowRegN_outer_unif
        ...
        ∀ {Q R : ℝ} (_hQ : 0 ≤ Q) (_hQρ : Q ≤ ρ)
          ...
          (hrealQ : ∀ T : SmoothCcTensor g 0 2,
            ‖smoothCcToTensorHs (I := I) (M := M) g
              ...
              gFibreOpBound (I := I) (M := M) g
                (ccTensorBilinSymm (I := I) g T) δ₀),
          Continuous (lowRegN (I := I) (M := M) g gBase hR hδ₀_lt
            (realizeOfLE (I := I) (M := M) g hRQ hrealQ)) ∧
          Continuous (coreN (I := I) (M := M) g gBase hδ₀_lt
            (realizeOfLE (I := I) (M := M) g hRQ hrealQ)) ∧
          ∀ u v : lowerState (I := I) (M := M) g 1 R,
            ‖lowRegN (I := I) (M := M) g gBase hR hδ₀_lt
                ...
                B1 Q *
                  ...
                  ‖tensorHsInclusion (I := I) (M := M) (g := g) (r := 0) (s := 2)
                    ...
                    ((u.1 : tensorHs (I := I) (M := M) g 0 2
                      (((1 : ℕ) : ℝ) + 2)) - v.1)‖ := by
  obtain ⟨ρ, Ctop, B0, B1, hρ, hCtop, hB0, hB1, hcore⟩ :=
    coreN_outer_unif (I := I) (M := M) hDim gBase hΛ
      hδ₀_nonneg hδ₀_lt
  refine ⟨ρ, Ctop, B0, B1, hρ, hCtop, hB0, hB1, ?_⟩
  ...
  let F : D → tensorHs (I := I) (M := M) g 0 2 ((1 : ℕ) : ℝ) :=
    coreN (I := I) (M := M) g gBase hδ₀_lt hrealR
