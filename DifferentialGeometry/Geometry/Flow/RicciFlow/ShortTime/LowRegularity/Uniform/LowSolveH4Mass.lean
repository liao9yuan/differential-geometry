...
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.LowSolveH4
...
theorem lowreg_sol_field_mass_five_of_integral_energy
  ...
  have hmass := integral_fatou_sq_mass
    ...
    (fun N t i => lowregProjMode (I := I) (M := M) g fseq N t i)
    (fun t i => perModeConv
      (TensorEigenIdx.lambda (I := I) (M := M) i)
      (fun s => (timeModeCoeff (I := I) (M := M) gforce i) s) t)
    (fun N i => lowregProjMode_cont (I := I) (M := M) g hT fseq N i)
    hconv Φ (by
      intro N
...
theorem lowreg_solve_h4_mass_unif
