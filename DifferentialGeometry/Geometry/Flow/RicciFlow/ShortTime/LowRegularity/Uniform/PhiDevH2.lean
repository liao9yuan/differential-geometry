...
theorem phi_dev_h2_unif
  ...
  have hCTH : ∀ (g₁ : SmoothRiemannianMetric I M) (i : ℕ) (x : M),
    ...
    intro g₁ i x
    simpa only [CTH] using trace_rfns_le (I := I) (M := M) g g₁ i x
        ...
        CR i * ∑ j ∈ Finset.range (i + 1),
          riemannianFiberNormSq (I := I) (M := M) g 2 (2 + j) x
            ((iteratedCovGrad (I := I) g 2 2 j
              (gInvDiffSlotCoeff (I := I) g g₁)).toSection x) := by
    intro g₁ i x
    simpa only [CR] using ricci_rfns_le (I := I) (M := M) g g₁ i x
  ...
  have hdev_eq : Dev =
      reindexCoeffGen (I := I) (M := M) g 4 2 DTHs ρA +
        reindexCoeffGen (I := I) (M := M) g 4 2 DTHs ρAT -
          (DRs + DRs) := by
    dsimp [Dev, DTHs, DRs]
    rw [phiMet_reindex (I := I) (M := M) g gBase g₁,
      phiMet_reindex (I := I) (M := M) g gBase g,
