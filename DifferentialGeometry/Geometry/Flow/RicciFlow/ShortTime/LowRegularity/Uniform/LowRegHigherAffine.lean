...
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.LowBaseGalArmAffine
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.LowRegH4Diss
...
theorem lowreg_high_rungs_affine_at_bg
    ...
    (hsol : IsBgSolveAt (I := I) (M := M) g gBase K hT hT1
      u gforce Rcap)
    {fseq : ℕ → timeL2
      (tensorHs (I := I) (M := M) g 0 2 ((1 : ℕ) : ℝ)) T}
      ...
      HasDerivWithinAt
        ...
        (-(TensorEigenIdx.lambda (I := I) (M := M) i) *
          ...
          galTameForce (I := I) (M := M) g 1
            ...
            (lowregNfun (I := I) (M := M) g gBase hsol.hδ hsol.hCtop hsol.hB1
              hsol.hρ hsol.hP hsol.hreal)
            (eigenIdxFinset (I := I) (M := M) g N)
            (lowregProjMode (I := I) (M := M) g fseq N t) i)
        (Set.Ici t) t)
    (hUinit : ∀ N i, lowregProjMode (I := I) (M := M) g fseq N 0 i = 0)
  ...
  have hclosure : ∀ N : ℕ, ∀ t ∈ Set.Ico (0 : ℝ) T,
    ...
    let arm := galArmVecBg (I := I) (M := M) g gBase hRpos.le hsol.hδ hrealR F (U N t)
    let seed := lowregNfun (I := I) (M := M) g gBase hsol.hδ hsol.hCtop hsol.hB1
      hsol.hρ hsol.hP hsol.hreal
    ...
    have hsplit : ∀ i ∈ F, force i = arm.coeff i + seed.coeff i := by
      ...
      rw [galForceArmBg (I := I) (M := M) g gBase hsol.hδ hsol.hδ0 hsol.hδ3
