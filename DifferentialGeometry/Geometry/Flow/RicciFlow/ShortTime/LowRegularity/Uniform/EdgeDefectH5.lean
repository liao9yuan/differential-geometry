...
open DifferentialGeometry.Analysis.Spectral
  (appCc appCcRS_zero_eq_appCc ccTensorToHs deTurckPhiMetTotal h1_jet_sq hsJet_le
    icg_comp_norm oneMinusConnLapSmooth
...
private theorem defect_arg_h1_fixed
  ...
  have hDT0 :
    ...
    simpa only [DT, Nat.add_zero] using
      (icg_comp_norm (I := I) (M := M) g 2 1 0 T)
  have hDT1 :
      ‖iteratedCovGrad (I := I) g 0 3 1 DT‖ =
        ‖iteratedCovGrad (I := I) g 0 2 2 T‖ := by
    simpa only [DT] using
      (icg_comp_norm (I := I) (M := M) g 2 1 1 T)
  have hDT0' : ‖DT‖ =
      ‖iteratedCovGrad (I := I) g 0 2 1 T‖ := by
  ...
  have hDT2' :
    ...
    simpa only [DT] using
      (icg_comp_norm (I := I) (M := M) g 2 1 2 T)
  have hDT2 :
      (∑ j ∈ Finset.range 2,
  ...
  have hA1 :
    ...
    calc
      _ = ‖iteratedCovGrad (I := I) g 0 3 2 R2‖ := by
        simpa only [A] using icg_comp_norm
          (I := I) (M := M) g 3 1 1 R2
  ...
  have hroot : ‖(⟨GT⟩ : SmoothCcTensorH1 g 0 4)‖ ≤
    ...
    have hsq := h1_jet_sq (I := I) (M := M) g 0 4 GT
...
theorem edge_defect_pair_h5_unif
      ...
      ∀ g : SmoothRiemannianMetric I M,
        ...
        ∃ C : ℝ, 0 ≤ C ∧
          ∀ (T : SmoothCcTensor g 0 2)
            ...
            ∀ {a : ℝ}, a ∈ Set.Icc (0 : ℝ) 1 →
            let gm := realizedFam (I := I) g T 0 hdelta hdeltaZ a
            ...
            2 * |tensorL2Inner (I := I) (M := M) g 0 2 V.toFun
                (appCc (I := I) (M := M) g 4 2 B GT).toFun| ≤
  ...
  let gm : SmoothRiemannianMetric I M :=
    realizedFam (I := I) g T 0 hdelta hdeltaZ a
  let B : SmoothCcTensor g 4 2 :=
    lieRefold2 (I := I) (M := M) g T hdelta hdeltaZ a +
      (deTurckPhiMetTotal (I := I) (M := M) g gBase gm -
        deTurckPhiMetTotal (I := I) (M := M) g gBase g) +
      ...
      _ = ‖(⟨GT⟩ : SmoothCcTensorH1 g 0 4)‖ ^ 2 := by
        simpa only [Finset.sum_range_succ, Finset.sum_range_zero, zero_add,
          iteratedCovGrad_zero] using
            (h1_jet_sq (I := I) (M := M) g 0 4 GT).symm
  ...
  have hspec :
    ...
    have hintrinsic := h1_jet_sq (I := I) (M := M) g 0 2 Y
...
theorem edge_defect_pair_abs_h5_unif
    (hDim : Module.finrank ℝ E = 3)
    (gBase : SmoothRiemannianMetric I M)
    {Λ : ℝ} (hΛ : 1 ≤ Λ) :
    ∀ {eta : ℝ}, 0 < eta →
  ...
  obtain ⟨rho, hrho, hraw⟩ :=
    edge_defect_pair_h5_unif (I := I) (M := M) hDim gBase hΛ
  refine ⟨rho, hrho, ?_⟩
  intro g hEq hjet
