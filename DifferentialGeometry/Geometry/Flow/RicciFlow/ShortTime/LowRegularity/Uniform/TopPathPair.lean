...
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.AppH1
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.ConvexJets
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.TopPathDev
...
theorem top_pair_h2_unif
  ...
  have hpair := appD2_pair_h2 (I := I) (M := M) g hact
...
theorem top_pair_h4_unif
  ...
  obtain ⟨ρ, C, hρ, hC, hpair⟩ :=
    top_pair_h2_unif (I := I) (M := M) hDim gBase hΛ
  refine ⟨ρ, C, hρ, hC, ?_⟩
...
theorem top_pair_abs_unif
  ...
  intro η hη
  obtain ⟨ρ₀, C, hρ₀, hC, hpair⟩ :=
    top_pair_h4_unif (I := I) (M := M) hDim gBase hΛ
  let D : ℝ := 2 * (C + 1)
  have hD : 0 < D := by
    dsimp only [D]
    positivity
