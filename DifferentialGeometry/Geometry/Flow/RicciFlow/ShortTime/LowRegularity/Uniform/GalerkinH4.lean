import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.A1FixedPairBg
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.LowBasePairH5
...
open DifferentialGeometry.Analysis.Spectral
  (ccTensorToHs finiteEigenComboHs finiteEigenComboHs_coeff oneMinusConnLapSmoothIter
    oneMinusConnLapSmoothIter_succ oneMinusConnLapSmoothIter_zero smoothCcToTensorHs)
...
theorem gal_arm_pair4_unif_below
    ...
    _ = 2 * |tensorL2Inner (I := I) (M := M) g 0 2
        ...
        (oneMinusConnLapSmoothIter (I := I) g 0 2 1
          ((lowBaseData (I := I) (M := M) g gBase T
            hdelta_lt hT hZ).a2 (I := I) (M := M) T +
            (lowBaseData (I := I) (M := M) g gBase T
              hdelta_lt hT hZ).a1 (I := I) (M := M) T)).toFun| := by
      rw [hdiag]
...
theorem gal_arm_pair4_unif_of_mem
    (hDim : Module.finrank ℝ E = 3)
    (gBase : SmoothRiemannianMetric I M)
    {Λ : ℝ} (hΛ : 1 ≤ Λ) :
    ∀ {eta : ℝ}, 0 < eta →
  ...
  intro eta heta
  obtain ⟨delta, hdelta, hdeltathird, hpair⟩ :=
    gal_arm_pair4_unif_of_mem_below (I := I) (M := M) hDim gBase hΛ heta
  obtain ⟨R0, hR0, hR0one, hpair0⟩ := hpair hdelta le_rfl
  exact ⟨delta, R0, hdelta, hdeltathird, hR0, hR0one, hpair0⟩
