...
theorem top_ker_grad_h3_unif
    ...
    ∃ ρ C : ℝ, 0 < ρ ∧ 0 ≤ C ∧
      ∀ g : SmoothRiemannianMetric I M,
        ...
        ∀ (T : SmoothCcTensor g 0 2)
          ...
          ∀ p : M,
          let gm := realizedFam (I := I) g T 0 hδ hδZ s
          let Φ := lieRefold2 (I := I) (M := M) g T hδ hδZ s +
            (-2 * s : ℝ) •
              LowBaseInternal.ricciTop (I := I) (M := M) g gm T
  ...
  let B : SmoothCcTensor g 4 2 :=
    lieRefold2 (I := I) (M := M) g T hδ hδZ s + P +
      (-2 * s : ℝ) •
        LowBaseInternal.ricciTop (I := I) (M := M) g gm T
  ...
  have hzero3 :
    ...
    rw [show (0 : SmoothCcTensor g 0 2) =
        (0 : ℝ) • (0 : SmoothCcTensor g 0 2) by simp,
      ccTensorToHs_smul, zero_smul, norm_zero]
  have hBjet : lowJetSq (I := I) (M := M) g 3 B ≤ (CB * y) ^ 2 := by
    simpa only [lowJetSq, B, P, gm, y] using
      hedge g hEq hjet T hT hδle hδ0 hδ hδZ hT2B hs
  have hPjet : lowJetSq (I := I) (M := M) g 3 P ≤ (CP * y) ^ 2 := by
    obtain ⟨_, h⟩ := hphi g hEq hjet T (0 : SmoothCcTensor g 0 2)
      hδlt hδ hδlt hδZ hx hT2P le_rfl hzero2 hs.1 hs.2
    simpa only [lowJetSq, P, gm, y, hzero3, add_zero] using h
  have hΦjet : lowJetSq (I := I) (M := M) g 3 Φ ≤ (Cjet * y) ^ 2 := by
    refine (jetSub (I := I) (M := M) g 3 B P).trans ?_
    calc
      2 * (lowJetSq (I := I) (M := M) g 3 B +
  ...
  have hΦeq : Φ = lieRefold2 (I := I) (M := M) g T hδ hδZ s +
      (-2 * s : ℝ) •
        LowBaseInternal.ricciTop (I := I) (M := M) g gm T := by
    dsimp only [Φ, B, P]
    module
  simpa only [hΦeq, DΦ, gm, y] using hpoint

theorem bcD2_pair_h5_unif
    (hDim : Module.finrank ℝ E = 3)
    (gBase : SmoothRiemannianMetric I M) {Λ : ℝ} (hΛ : 1 ≤ Λ) :
    ∃ ρ C4 Cmix C5 : ℝ,
  ...
  obtain ⟨ρ, Cgrad, hρ, hCgrad, hgrad⟩ :=
    top_ker_grad_h3_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨Kcurv, hKcurv⟩ :=
    exists_curv_actions (I := I) (M := M) gBase hΛ
  ...
  have hpair := appD2_pair_h3 (I := I) (M := M) g hact2 hact3
