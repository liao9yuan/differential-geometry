import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.LowBasePairH4
...
theorem gal_arm_pair3_unif_below
  ...
  have hdiag := galArmPair3_diag (I := I) (M := M) g gBase hR
  ...
  have hrhs :
      eta * ‖ccTensorToHs (I := I) (M := M) g 2 (4 : ℝ) T‖ ^ 2 +
        ...
        eta * E4 + G * (E3 + E3 ^ 2) := by
    exact add_le_add (mul_le_mul_of_nonneg_left hT4sq heta.le)
      (mul_le_mul_of_nonneg_left (add_le_add hT3sq hT3four) hG)
  change 2 * |theta * (∑ i ∈ F,
      tensorSobolevWeight (I := I) (M := M) i (3 : ℝ) *
        (c i * (galArmVecBg (I := I) (M := M) g gBase hR
          hdelta_lt hreal F c).coeff i))| ≤ eta * E4 + G * (E3 + E3 ^ 2)
  calc
