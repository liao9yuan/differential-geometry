import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.EdgeCenterCommutator
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.EdgeCornerH5
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.EdgeDefectH5
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.TopKerPair
...
open DifferentialGeometry.Analysis.Spectral
  (appCc ccTensorToHs deTurckPhiMetTotal edgeMetric edgeMetric_bal oneMinusConnLapSmooth
    phiMetCurvCoeff slotExtend)
...
theorem edge_center_pair_abs_h5_of_carrier
  ...
  obtain ⟨rhoP, C4, Cmix, C5, hrhoP, hC4, hCmix, hC5, hprincipal⟩ :=
    bcD2_pair_h5_unif (I := I) (M := M) hDim gBase hLambda
  obtain ⟨rhoD, hrhoD, hdefect⟩ :=
    edge_defect_pair_abs_h5_unif (I := I) (M := M)
      hDim gBase hLambda he
  obtain ⟨rhoK, hrhoK, hcorner⟩ :=
    edge_corner_pair_h5_unif (I := I) (M := M) hDim gBase hLambda
  ...
  let Q : SmoothCcTensor g 0 2 → SmoothCcTensor g 2 2 := fun U =>
    edgeTopPairBi (I := I) (M := M) g T U hdelta hdeltaZ
      ricciRefoldQA ricciRefoldQB lieRefoldQ lieRefoldEps s
  let Z := appCc (I := I) (M := M) g 2 2 (Q T) T
  let Cross := appCc (I := I) (M := M) g 2 2 (Q LT) T +
    appCc (I := I) (M := M) g 2 2 (Q T) LT
  ...
  let B : SmoothCcTensor g 4 2 :=
    lieRefold2 (I := I) (M := M) g T hdelta hdeltaZ s + C +
      (-2 * s : ℝ) • LowBaseInternal.ricciTop
        (I := I) (M := M) g gs T
  ...
  let Yp := appCc (I := I) (M := M) g 4 2 (B - C) HLT
  let Yd := appCc (I := I) (M := M) g 4 2 B Gcurv
  ...
  have hprincipal' :
    ...
    have hp' :
      ...
      have hYp : Yp = appCc (I := I) (M := M) g 4 2
          (lieRefold2 (I := I) (M := M) g T hdelta hdeltaZ s +
            (-2 * s : ℝ) • LowBaseInternal.ricciTop
  ...
  have hpeel := edge_center_peel (I := I) (M := M) g gBase T hTsymm
    (lt_of_le_of_lt hdelta_third (by norm_num)) hdelta hdeltaZ s hs
  have hgs : edgeMetric (I := I) (M := M) g T hdelta s = gs :=
    edgeMetric_bal (I := I) (M := M) g T
