import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.InvCoeffH2
...
open DifferentialGeometry.Analysis.Sobolev
  (gInvDiffSlotCoeff_eq_slotInsertEndoCc iteratedCovGrad iteratedCovGrad_zero
   normSq_le_integral_of_pointwise_fiberNormSq_le_rs
   norm_le_of_pointwise_fiberNormSq_bound_rs rfns_iteratedCovGrad_slotInsertEndoCc_le_endo
   tensorL2Norm_sq_toFun_eq_integral_riemannianFiberNormSq_rs)
open DifferentialGeometry.Analysis.Spectral
  (appCcRS ccTensorToHs fullRaisedEndoField gInvDiffSlotCoeff iteratedCovGrad_add
   iteratedCovGrad_neg)
open DifferentialGeometry.Analysis.Spectral.CurvatureCoefficientDifferenceJetTower
  (fullRaisedEndoField_diff_split iteratedCovGrad_slotInsert_fullRaised_id_succ_eq_zero
...
private theorem idSlotPt
    ...
    riemannianFiberNormSq (I := I) (M := M) g 2 2 x
        ((slotInsertEndoCc (I := I) (M := M) g 1
          (fullRaisedEndoField (I := I) (M := M) g g)).toSection x) ≤ 27 := by
  have h := rfns_iteratedCovGrad_slotInsertEndoCc_le_endo
    (I := I) (M := M) g 1 (fullRaisedEndoField (I := I) (M := M) g g) 0 x
  rw [hDim] at h
  ...
  have hid : riemannianFiberNormSq (I := I) (M := M) g 1 1 x
      ((slotInsertEndoCc (I := I) (M := M) g 0
        (fullRaisedEndoField (I := I) (M := M) g g)).toSection x) ≤ 9 := by
    have hb := rfns_idEndo_le (I := I) (M := M) g x
    rw [hDim] at hb
...
private theorem idSlotJet
    ...
    (∑ j ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g 2 2 j
        (slotInsertEndoCc (I := I) (M := M) g 1
          (fullRaisedEndoField (I := I) (M := M) g g))‖ ^ 2) ≤
  ...
  have h0 : ‖iteratedCovGrad (I := I) g 2 2 0
    ...
    exact norm_le_of_pointwise_fiberNormSq_bound_rs (I := I) (M := M) g 2 2
      (slotInsertEndoCc (I := I) (M := M) g 1
        (fullRaisedEndoField (I := I) (M := M) g g)) 27
  ...
  have hexp : (∑ j ∈ Finset.range 3,
      ...
      ‖iteratedCovGrad (I := I) g 2 2 2
        (slotInsertEndoCc (I := I) (M := M) g 1
          (fullRaisedEndoField (I := I) (M := M) g g))‖ ^ 2 := by
...
private theorem fullSlotSplit (g gm : SmoothRiemannianMetric I M) :
  ...
  rw [fullRaisedEndoField_diff_split (I := I) (M := M) g gm,
...
theorem invCoeff_h2_lip_unif
  ...
  obtain ⟨ρ, Cinv, hρ, hCinv, hinv⟩ :=
    inv_coeff_h2_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨Cmul, hCmul, hmul⟩ :=
    appRS_h22_unif (I := I) (M := M) hDim gBase hΛ 2 2 2
  obtain ⟨Cpt, hCpt, hpoint⟩ :=
    morreyRS_unif (I := I) (M := M) hDim gBase hΛ 2 2
  ...
  have hfull : ∀ gm : SmoothRiemannianMetric I M,
      ...
      (∑ j ∈ Finset.range 3,
        ‖iteratedCovGrad (I := I) g 2 2 j
          (slotInsertEndoCc (I := I) (M := M) g 1
            (fullRaisedEndoField (I := I) (M := M) g gm))‖ ^ 2) ≤ A ^ 2 := by
    intro gm W hW hWtie
  ...
  have hMid : (∑ j ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g 2 2 j
        (appCcRS (I := I) (M := M) g 2 2 2
          ...
          (slotInsertEndoCc (I := I) (M := M) g 1
            (fullRaisedEndoField (I := I) (M := M) g gT)))‖ ^ 2) ≤
      (Cmul * (Cp * N) * A) ^ 2 :=
  ...
  have hD : (∑ j ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g 2 2 j
        (gInvDiffSlotCoeff (I := I) g gT -
          gInvDiffSlotCoeff (I := I) g gU)‖ ^ 2) ≤ (C0 * N) ^ 2 := by
    ...
    calc
      riemannianFiberNormSq (I := I) (M := M) g 2 2 x
          ...
          Cpt ^ 2 * (∑ j ∈ Finset.range 3,
            ‖iteratedCovGrad (I := I) g 2 2 j
              (gInvDiffSlotCoeff (I := I) g gT -
                gInvDiffSlotCoeff (I := I) g gU)‖ ^ 2) := hpt
      _ ≤ Cpt ^ 2 * (C0 * N) ^ 2 :=
