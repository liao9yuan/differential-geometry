...
theorem vb_h1_unif
  ...
  obtain ⟨Bt1, hBt1, htr1⟩ :=
    trace_h2_unif (I := I) (M := M) 1 hDim gBase hΛ0 hδ₀
  ...
  obtain ⟨Cω, hCω, hωprod⟩ :=
    appRS_h22_unif (I := I) (M := M) hDim gBase hΛ 0 3 1
  obtain ⟨Cip, hCip, hiprod⟩ :=
    appRS_h22_unif (I := I) (M := M) hDim gBase hΛ 2 3 1
  obtain ⟨Cn, hCn, hnprod⟩ :=
    appRS_h1_unif (I := I) (M := M) hDim gBase hΛ 2 1 4
  ...
  let Carm : SmoothCcTensor g₀ 3 1 :=
    lc0TraceRF (I := I) (M := M) g₀ g₀ 1 ipTracePerm
  ...
  have hHterm : ∀ i ∈ Finset.range 2,
    ...
    simpa only [Hf, lc0Kappa, hDim, Nat.cast_ofNat] using h
  ...
  have hIpForm :
      Ip = appCcRS (I := I) (M := M) g₀ 2 3 1 Carm Ωs := by
  ...
  have hInn : (∑ i ∈ Finset.range 2,
    ...
    rw [h1_jet_sq (I := I) (M := M) g₀ 2 4
      (appCcRS (I := I) (M := M) g₀ 2 1 4 Hf Ip)] at hsquare
  ...
  have hOut : (∑ i ∈ Finset.range 2,
    ...
    rw [h1_jet_sq (I := I) (M := M) g₀ 2 2
      (appCcRS (I := I) (M := M) g₀ 2 4 2 T2 Inn)] at hsquare
