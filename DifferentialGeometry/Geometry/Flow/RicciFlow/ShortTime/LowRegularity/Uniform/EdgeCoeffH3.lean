import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.AlgCoeffH3
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.Uniform.AppH3
...
import DifferentialGeometry.Geometry.Flow.RicciFlow.ShortTime.LowRegularity.RicciTopFibreBound
...
private theorem endo_slot_succ_eq
  ...
  simpa only [Nat.add_assoc, Nat.add_comm, Nat.add_left_comm] using
    tsSlotInsertEndoCc_succ_eq_reindex_slotExtend (I := I) (M := M) g q P
...
private theorem endo_slot_succ_pointwise
  ...
  rw [hA]
  exact rfns_iteratedCovGrad_slotExtend_le (I := I) (M := M) g (q + 1) (q + 1)
    (endoSlotZeroCcTensor (I := I) (M := M) g q P) i x
...
private theorem double_trace_jet_four
  ...
  have h0 : ‖iteratedCovGrad (I := I) g (p + 2) p 0
    ...
    exact norm_le_of_pointwise_fiberNormSq_bound_rs (I := I) (M := M) g (p + 2) p
      ...
      ((Module.finrank ℝ E : ℝ) ^ (p + 6))
      (fun x => cometricTrace_rfns_p (I := I) (M := M) p g x)
...
theorem pure_trace_h3_unif
  ...
  obtain ⟨rho, Cf, hrho, hCf, hfull⟩ :=
    fullRaised_h3_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨Ca2, hCa2, happ2⟩ :=
    appRS_h3_sup_unif (I := I) (M := M) gBase Λ 4 4 2
  obtain ⟨Ca4, hCa4, happ4⟩ :=
    appRS_h3_sup_unif (I := I) (M := M) gBase Λ 6 6 4
  ...
  let F := fullRaisedEndoField (I := I) (M := M) g gm
  ...
  have hTr2pt : ∀ x : M,
    ...
    simpa only [Tr2, D2, n] using
      cometricTrace_rfns_p (I := I) (M := M) 2 g x
  have hTr4pt : ∀ x : M,
      riemannianFiberNormSq (I := I) (M := M) g 6 4 x
          (Tr4.toSection x) ≤ D4 := by
    intro x
    simpa only [Tr4, D4, n] using
      cometricTrace_rfns_p (I := I) (M := M) 4 g x
...
theorem pair_trace_h3_unif
        ...
        ∀ (P : SmoothCcTensor g 0 2) (gm : SmoothRiemannianMetric I M)
          ...
          (∀ x : M,
            riemannianFiberNormSq (I := I) (M := M) g 6 2 x
              ((lieCovPair (I := I) (M := M) g gm).toSection x) ≤ Cp ^ 2) ∧
          (∑ j ∈ Finset.range 4,
            ‖iteratedCovGrad (I := I) g 6 2 j
              (lieCovPair (I := I) (M := M) g gm)‖ ^ 2) ≤
                (Cj * (1 + N)) ^ 2 := by
  obtain ⟨rho, Ct, Cj, hrho, hCt, hCj, htrace⟩ :=
    pure_trace_h3_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨Ca, hCa, happ⟩ :=
    appRS_h3_sup_unif (I := I) (M := M) gBase Λ 6 4 2
  ...
  have hcomp (x : M) :
      riemannianFiberNormSq (I := I) (M := M) g 6 2 x
          ((appCcRS (I := I) (M := M) g 6 4 2 T2 T4).toSection x) ≤
  ...
  have hpoint : ∀ x : M,
      riemannianFiberNormSq (I := I) (M := M) g 6 2 x
        ((lieCovPair (I := I) (M := M) g gm).toSection x) ≤ (Ct ^ 2) ^ 2 := by
    intro x
    rw [LowBaseInternal.pairTrace_eq]
    calc
...
private theorem mono_ext_pointwise
    ...
    riemannianFiberNormSq (I := I) (M := M) g (r + w) (s + w) x
      ...
      (Module.finrank ℝ E : ℝ) ^ w *
        riemannianFiberNormSq (I := I) (M := M) g r s x
          (A.toSection x) := by
  have hperm := DifferentialGeometry.Analysis.Spectral.rfns_iteratedCovGrad_rs_eq_of_section_domDomCongr
...
theorem dag_top_h3_unif
    ...
    ∃ rho Cp Cj : ℝ, 0 < rho ∧ 0 ≤ Cp ∧ 0 ≤ Cj ∧
      ∀ g : SmoothRiemannianMetric I M,
        ...
        ∀ (P : SmoothCcTensor g 0 2) (gm : SmoothRiemannianMetric I M)
          ...
          (∀ x : M,
            riemannianFiberNormSq (I := I) (M := M) g 4 4 x
              ((LowBaseInternal.dagTopOp (I := I) (M := M) g gm).toSection x) ≤
                Cp ^ 2) ∧
          (∑ j ∈ Finset.range 4,
            ‖iteratedCovGrad (I := I) g 4 4 j
              (LowBaseInternal.dagTopOp (I := I) (M := M) g gm)‖ ^ 2) ≤
                (Cj * (1 + N)) ^ 2 := by
  classical
  obtain ⟨rho, Cf, hrho, hCf, hfull⟩ :=
    fullRaised_h3_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨Kp, hKp, hpoint⟩ :=
    LowBaseInternal.dagTop_cap_unif (I := I) (M := M)
  let n : ℝ := Module.finrank ℝ E
  ...
  have hE1 : (∑ j ∈ Finset.range 4,
    ...
    simpa only [E1, F, n, N] using
      endo_slot_succ_jet (I := I) (M := M) g 1 4
        (fullRaisedEndoField (I := I) (M := M) g gm) |>.trans
    ...
    calc
      (1 / 2 : ℝ) ^ 2 * (∑ j ∈ Finset.range 4,
        ...
        mul_le_of_le_one_left (Finset.sum_nonneg fun j _ => sq_nonneg _)
          (by norm_num)
      _ ≤ _ := hABC
  have hYeq : appCcRS (I := I) (M := M) g 3 3 3 E1
      ((1 / 2 : ℝ) •
        (permCoeff (I := I) (M := M) g (Equiv.swap (0 : Fin 3) 2) +
          permCoeff (I := I) (M := M) g (finRotate 3) -
    ...
    have hinner :
      ...
      calc
        _ = appCcRS (I := I) (M := M) g 3 3 3 E1
              (permCoeff (I := I) (M := M) g (Equiv.swap (0 : Fin 3) 2) +
                permCoeff (I := I) (M := M) g (finRotate 3)) -
            appCcRS (I := I) (M := M) g 3 3 3 E1
              (permCoeff (I := I) (M := M) g
                (Equiv.swap (1 : Fin 3) 2)) :=
          appCcRS_sub_right (I := I) (M := M) g 3 3 3 E1 _ _
        _ = _ := congrArg
          (fun Z => Z - appCcRS (I := I) (M := M) g 3 3 3 E1
            (permCoeff (I := I) (M := M) g (Equiv.swap (1 : Fin 3) 2)))
          (appCcRS_add_right (I := I) (M := M) g 3 3 3 E1 _ _)
    calc
      _ = (1 / 2 : ℝ) • appCcRS (I := I) (M := M) g 3 3 3 E1
          (permCoeff (I := I) (M := M) g (Equiv.swap (0 : Fin 3) 2) +
            permCoeff (I := I) (M := M) g (finRotate 3) -
            permCoeff (I := I) (M := M) g
              (Equiv.swap (1 : Fin 3) 2)) :=
        appCcRS_smul_right (I := I) (M := M) g 3 3 3 _ _ _
      _ = (1 / 2 : ℝ) •
          (appCcRS (I := I) (M := M) g 3 3 3 E1
              (permCoeff (I := I) (M := M) g (Equiv.swap (0 : Fin 3) 2)) +
            appCcRS (I := I) (M := M) g 3 3 3 E1
  ...
  have hconn : (∑ j ∈ Finset.range 4,
      ‖iteratedCovGrad (I := I) g 3 3 j
        (LowBaseInternal.connLowOp (I := I) (M := M) g gm)‖ ^ 2) ≤
        10 * (n * (Cf * (1 + N)) ^ 2) := by
    rw [conn_low_nf (I := I) (M := M) g gm, hYeq]
    exact (perm_left_jet_four_le (I := I) (M := M) g
      LowBaseInternal.lowPerm Y).trans hY
  have hdag : (∑ j ∈ Finset.range 4,
      ‖iteratedCovGrad (I := I) g 4 4 j
        (LowBaseInternal.dagTopOp (I := I) (M := M) g gm)‖ ^ 2) ≤
        10 * n ^ 2 * (Cf * (1 + N)) ^ 2 := by
    rw [LowBaseInternal.dagTopOp]
    calc
      _ ≤ ∑ j ∈ Finset.range 4,
          ‖iteratedCovGrad (I := I) g 4 4 j
            (slotExtend (I := I) (M := M) g 3 3
              (LowBaseInternal.connLowOp (I := I) (M := M) g gm))‖ ^ 2 :=
        perm_left_jet_four_le (I := I) (M := M) g LowBaseInternal.daPermA _
      _ ≤ n * (∑ j ∈ Finset.range 4,
          ‖iteratedCovGrad (I := I) g 3 3 j
            (LowBaseInternal.connLowOp (I := I) (M := M) g gm)‖ ^ 2) := by
        rw [Finset.mul_sum]
        apply Finset.sum_le_sum
        intro j hj
        exact slot_extend_sq (I := I) (M := M) g
          (LowBaseInternal.connLowOp (I := I) (M := M) g gm) j
      _ ≤ n * (10 * (n * (Cf * (1 + N)) ^ 2)) :=
...
theorem lie_refold_h3_unif
    ...
    ∃ rho C : ℝ, 0 < rho ∧ 0 ≤ C ∧
      ∀ g : SmoothRiemannianMetric I M,
        ...
        ∀ (T : SmoothCcTensor g 0 2)
          ...
          (∑ j ∈ Finset.range 4,
            ‖iteratedCovGrad (I := I) g 4 2 j
              (lieRefold2 (I := I) (M := M) g T hdelta hdeltaZ a)‖ ^ 2) ≤
            (C * ‖ccTensorToHs (I := I) (M := M) g 2 (3 : ℝ) T‖) ^ 2 := by
  classical
  obtain ⟨rhoP, Cp, Cj, hrhoP, hCp, hCj, hpair⟩ :=
    pair_trace_h3_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨Ca, hCa, happ⟩ :=
    appRS_h3_sup_unif (I := I) (M := M) gBase Λ 4 6 2
  obtain ⟨Cmor, hCmor, hmor⟩ :=
    morreyRS_unif (I := I) (M := M) hDim gBase hΛ 0 2
  obtain ⟨Kcurv, hKcurv⟩ :=
  ...
  have hmem : a ∈ DifferentialGeometry.PDE.DeTurck.RicciLinearization.realizedSmallSet
      (δ := delta) (δ' := delta) :=
    DifferentialGeometry.PDE.DeTurck.RicciLinearization.Icc_subset_realizedSmallSet
  ...
  let V : Fin 3 → SmoothCcTensor g 4 2 := fun i =>
    curvatureRefoldMonomialCoeffField (I := I) (M := M) g gm
      ...
      (lieRefoldQ i)
  let U : Fin 3 → SmoothCcTensor g 4 2 := fun i => lieRefoldEps i • V i
  ...
  have hU (i : Fin 3) : (∑ j ∈ Finset.range 4,
    ...
    exact hmono i
  have hrefold :
      lieRefold2 (I := I) (M := M) g T hdelta hdeltaZ a =
        a • (U 0 + U 1 + U 2) := by
...
theorem da_weight_h3_of_full
    ...
    (hFpt : ∀ y : M,
      riemannianFiberNormSq (I := I) (M := M) g 2 2 y
        ((slotInsertEndoCc (I := I) (M := M) g 1
          (fullRaisedEndoField (I := I) (M := M) g gm)).toSection y) ≤ Fq)
    (hFjet : (∑ j ∈ Finset.range 4,
      ‖iteratedCovGrad (I := I) g 2 2 j
        (slotInsertEndoCc (I := I) (M := M) g 1
          (fullRaisedEndoField (I := I) (M := M) g gm))‖ ^ 2) ≤
            (Cf * (1 + N)) ^ 2)
    (hTpt : ∀ y : M,
      riemannianFiberNormSq (I := I) (M := M) g 0 2 y (T.toSection y) ≤
        (C2 * x) ^ 2)
    ...
    (∀ y : M,
      riemannianFiberNormSq (I := I) (M := M) g 0 2 y
        ((LowBaseInternal.daWeight (I := I) (M := M) g gm T).toSection y) ≤
          (Real.sqrt Fq * (C2 * x)) ^ 2) ∧
      (∑ j ∈ Finset.range 4,
        ‖iteratedCovGrad (I := I) g 0 2 j
          (LowBaseInternal.daWeight (I := I) (M := M) g gm T)‖ ^ 2) ≤
            Kw * N ^ 2 := by
  ...
  · intro y
    ...
    calc
      _ ≤ riemannianFiberNormSq (I := I) (M := M) g 2 2 y (F.toSection y) *
        ...
        simpa only [F, LowBaseInternal.daWeight, appCcRS_zero_eq_appCc,
          appCcRS_toSection] using hc
      _ ≤ Fq * (C2 * x) ^ 2 := hm
  ...
  · have hr := happW F T (Real.sqrt Fq) (C2 * x)
    ...
    change (∑ j ∈ Finset.range 4,
      ‖iteratedCovGrad (I := I) g 0 2 j
        (appCcRS (I := I) (M := M) g 0 2 2 F T)‖ ^ 2) ≤ Kw * N ^ 2
    refine hr.trans ?_
    calc
...
theorem da_trans_h3_of_weight
    ...
    (hpairPt : ∀ y : M,
      riemannianFiberNormSq (I := I) (M := M) g 6 2 y
        ((lieCovPair (I := I) (M := M) g g).toSection y) ≤ Qp ^ 2)
    (hpairJet : (∑ j ∈ Finset.range 4,
      ‖iteratedCovGrad (I := I) g 6 2 j
        (lieCovPair (I := I) (M := M) g g)‖ ^ 2) ≤ Qj ^ 2)
    (hWpt : ∀ y : M,
      riemannianFiberNormSq (I := I) (M := M) g 0 2 y
        ((LowBaseInternal.daWeight (I := I) (M := M) g gm T).toSection y) ≤
          (Real.sqrt Fq * (C2 * x)) ^ 2)
    ...
    (∀ y : M,
      riemannianFiberNormSq (I := I) (M := M) g 4 2 y
        ((LowBaseInternal.daTrans (I := I) (M := M) g gm T).toSection y) ≤
          (4 * Mp) * x ^ 2) ∧
      (∑ j ∈ Finset.range 4,
        ‖iteratedCovGrad (I := I) g 4 2 j
          (LowBaseInternal.daTrans (I := I) (M := M) g gm T)‖ ^ 2) ≤
            (4 * Km) * N ^ 2 := by
  classical
  let W : SmoothCcTensor g 0 2 :=
    LowBaseInternal.daWeight (I := I) (M := M) g gm T
  let Wp : ℝ := Fq * C2 ^ 2
  ...
  have hZjet (e : Equiv.Perm (Fin 4)) : (∑ j ∈ Finset.range 4,
    ...
    refine (monoExtJet (I := I) (M := M) g 0 2 4 3
      (LowBaseInternal.monoPerm e) W).trans ?_
    rw [hn4eq]
    exact mul_le_mul_of_nonneg_left (by simpa only [W] using hWjet) hn4
  have hVeq (e : Equiv.Perm (Fin 4)) :
      V e = appCcRS (I := I) (M := M) g 4 6 2
        (lieCovPair (I := I) (M := M) g g) (Z e) := by
    dsimp only [V, Z, W, LowBaseInternal.daTransMono,
      LowBaseInternal.daWeight]
    rw [LowBaseInternal.curvMono_eq]
    rfl
  have hVpt (e : Equiv.Perm (Fin 4)) : ∀ y : M,
    ...
    have hc := riemannianFiberNormSq_compRS_le_mul
      (I := I) (M := M) g 4 6 2 y
        ((lieCovPair (I := I) (M := M) g g).toSection y) ((Z e).toSection y)
    rw [hVeq e]
    have hc' := (by simpa only [appCcRS_toSection] using hc)
    refine hc'.trans ?_
  ...
  have hVjet (e : Equiv.Perm (Fin 4)) : (∑ j ∈ Finset.range 4,
    ...
    rw [hVeq e]
    refine (happM (lieCovPair (I := I) (M := M) g g) (Z e)
    ...
    have hlead :
        (Real.sqrt n4 * (Real.sqrt Fq * (C2 * x))) ^ 2 *
            (∑ j ∈ Finset.range 4,
              ‖iteratedCovGrad (I := I) g 6 2 j
                (lieCovPair (I := I) (M := M) g g)‖ ^ 2) ≤
          (n4 * Wp * x ^ 2) * Qj ^ 2 := by
      calc
        _ = (n4 * Wp * x ^ 2) * (∑ j ∈ Finset.range 4,
            ‖iteratedCovGrad (I := I) g 6 2 j
              (lieCovPair (I := I) (M := M) g g)‖ ^ 2) := by
                rw [mul_pow, Real.sq_sqrt hn4, mul_pow, Real.sq_sqrt hFq]
                dsimp only [Wp]
                ring
        _ ≤ (n4 * Wp * x ^ 2) * Qj ^ 2 :=
  ...
  have htransEq : LowBaseInternal.daTrans (I := I) (M := M) g gm T =
  ...
  · intro y
    ...
    calc
      2 * riemannianFiberNormSq (I := I) (M := M) g 4 2 y
          ((V LowBaseInternal.daPermA).toSection y) +
        2 * riemannianFiberNormSq (I := I) (M := M) g 4 2 y
          ((V LowBaseInternal.daPermB).toSection y) ≤
        2 * (Mp * x ^ 2) + 2 * (Mp * x ^ 2) :=
          add_le_add
            (mul_le_mul_of_nonneg_left (hVpt LowBaseInternal.daPermA y) (by norm_num))
            (mul_le_mul_of_nonneg_left (hVpt LowBaseInternal.daPermB y) (by norm_num))
      _ = (4 * Mp) * x ^ 2 := by ring
  · rw [htransEq, sub_eq_add_neg, ← neg_one_smul ℝ (V LowBaseInternal.daPermB)]
    refine (jet_four_add (I := I) (M := M) g
      (V LowBaseInternal.daPermA) ((-1 : ℝ) • V LowBaseInternal.daPermB)).trans ?_
    rw [jet_four_smul]
    norm_num
    calc
      2 * ((∑ j ∈ Finset.range 4,
          ‖iteratedCovGrad (I := I) g 4 2 j (V LowBaseInternal.daPermA)‖ ^ 2) +
        ∑ j ∈ Finset.range 4,
          ‖iteratedCovGrad (I := I) g 4 2 j (V LowBaseInternal.daPermB)‖ ^ 2) ≤
          2 * (Km * N ^ 2 + Km * N ^ 2) :=
        mul_le_mul_of_nonneg_left (add_le_add (hVjet _) (hVjet _)) (by norm_num)
      _ = (4 * Km) * N ^ 2 := by ring

omit [BoundarylessManifold I M] in
theorem ricci_top_h3_of_trans_dag
    (g : SmoothRiemannianMetric I M)
    (gm : SmoothRiemannianMetric I M) (T : SmoothCcTensor g 0 2)
    ...
    (hdagPt : ∀ y : M,
      riemannianFiberNormSq (I := I) (M := M) g 4 4 y
        ((LowBaseInternal.dagTopOp (I := I) (M := M) g gm).toSection y) ≤
          Dp ^ 2)
    ...
    (∑ j ∈ Finset.range 4,
      ‖iteratedCovGrad (I := I) g 4 2 j
        (LowBaseInternal.ricciTop (I := I) (M := M) g gm T)‖ ^ 2) ≤
          Kr * N ^ 2 := by
  let Kr : ℝ := Cr * (Dp ^ 2 * Kt + Tp * (4 * Dj ^ 2))
  unfold LowBaseInternal.ricciTop
  refine (happR
    (LowBaseInternal.daTrans (I := I) (M := M) g gm T)
    (LowBaseInternal.dagTopOp (I := I) (M := M) g gm)
    (Real.sqrt Tp * x) Dp
...
theorem ricci_top_h3_of_weight_dag
    (g : SmoothRiemannianMetric I M)
    (gm : SmoothRiemannianMetric I M) (T : SmoothCcTensor g 0 2)
  ...
  obtain ⟨htransPt, htransJet⟩ := da_trans_h3_of_weight
...
theorem ricci_top_h3_unif
    ...
    ∃ rho C : ℝ, 0 < rho ∧ 0 ≤ C ∧
      ∀ g : SmoothRiemannianMetric I M,
        ...
        ∀ (T : SmoothCcTensor g 0 2)
          ...
          ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) T‖ ≤ rho →
          ∀ {a : ℝ}, a ∈ Set.Icc (0 : ℝ) 1 →
          let gm := DifferentialGeometry.PDE.DeTurck.RicciLinearization.realizedFam
            (I := I) g T 0 hdelta hdeltaZ a
          (∑ j ∈ Finset.range 4,
            ‖iteratedCovGrad (I := I) g 4 2 j
              (LowBaseInternal.ricciTop (I := I) (M := M) g gm T)‖ ^ 2) ≤
  ...
  obtain ⟨rhoD, Dp, Dj, hrhoD, hDp, hDj, hdag⟩ :=
    dag_top_h3_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨rhoQ, Qp, Qj, hrhoQ, hQp, hQj, hpair⟩ :=
    pair_trace_h3_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨Cw, hCw, happW⟩ :=
    appRS_h3_sup_unif (I := I) (M := M) gBase Λ 0 2 2
  obtain ⟨Cm, hCm, happM⟩ :=
    appRS_h3_sup_unif (I := I) (M := M) gBase Λ 4 6 2
  obtain ⟨Cr, hCr, happR⟩ :=
    appRS_h3_sup_unif (I := I) (M := M) gBase Λ 4 4 2
  ...
  have hpairJet : (∑ j ∈ Finset.range 4,
      ‖iteratedCovGrad (I := I) g 6 2 j
        (lieCovPair (I := I) (M := M) g g)‖ ^ 2) ≤ Qj ^ 2 := by
  ...
  obtain ⟨hWpt, hWjet⟩ := da_weight_h3_of_full
  ...
  have hr := ricci_top_h3_of_weight_dag
...
theorem edge_coeff_h3_unif
      ...
      ∀ g : SmoothRiemannianMetric I M,
        ...
        ∀ (T : SmoothCcTensor g 0 2)
          ...
          (∑ j ∈ Finset.range 4,
            ‖iteratedCovGrad (I := I) g 4 2 j
              (lieRefold2 (I := I) (M := M) g T hdelta hdeltaZ a +
                (deTurckPhiMetTotal (I := I) (M := M) g gBase gm -
  ...
  obtain ⟨rhoL, CL, hrhoL, hCL, hlie⟩ :=
    lie_refold_h3_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨rhoP, CP, hrhoP, hCP, hphi⟩ :=
    phi_dev_h3_unif (I := I) (M := M) hDim gBase hΛ
  obtain ⟨rhoR, CR, hrhoR, hCR, hricci⟩ :=
    ricci_top_h3_unif (I := I) (M := M) hDim gBase hΛ
