...
import DifferentialGeometry.Geometry.Flow.RicciFlow.Compactness.Bounds.Uniform.Lp6ToLp3
...
theorem edge_swap_h4_unif
    ...
    ∃ C : ℝ, 0 ≤ C ∧
      ∀ g : SmoothRiemannianMetric I M,
        ...
        ∀ (T : SmoothCcTensor g 0 2)
          ...
          let YTL := appCc (I := I) (M := M) g 2 2
            (edgeTopPairInt (I := I) (M := M) g T T
              (lt_of_le_of_lt hδ₀_half (by norm_num)) hδ hδZ
              qA qB q epsilon) LT
          let YLT := appCc (I := I) (M := M) g 2 2
            (edgeTopPairInt (I := I) (M := M) g T LT
              (lt_of_le_of_lt hδ₀_half (by norm_num)) hδ hδZ
              qA qB q epsilon) T
  ...
  obtain ⟨Kedge, hKedge, hedge⟩ :=
    edgeTopInt_zero_unif (I := I) (M := M)
  obtain ⟨C6two, hC6two, h6two⟩ :=
    h1Lp6RS_unif (I := I) (M := M) hDim gBase hΛ 0 2
  ...
  let YTL : SmoothCcTensor g 0 2 :=
    appCc (I := I) (M := M) g 2 2
      (edgeTopPairInt (I := I) (M := M) g T T hδ₀_lt hδ hδZ
        qA qB q epsilon) LT
  let YLT : SmoothCcTensor g 0 2 :=
    appCc (I := I) (M := M) g 2 2
      (edgeTopPairInt (I := I) (M := M) g T LT hδ₀_lt hδ hδZ
        qA qB q epsilon) T
  ...
  let QTL : SmoothCcTensor g 0 4 :=
    edgeTopPartnerInt (I := I) (M := M) g T LT V hδ₀_lt
      hδ hδZ qA qB q epsilon
  let QLT : SmoothCcTensor g 0 4 :=
    edgeTopPartnerInt (I := I) (M := M) g T T V hδ₀_lt
      hδ hδZ qA qB q epsilon
  ...
  have hcompD2 :
    ...
    exact icg_comp_norm (I := I) (M := M) g 2 2 1 T
  have hD2H1root :
  ...
  have hTcap : ∀ p : M,
    ...
    have hroot := Real.sqrt_le_sqrt hrfns
...
theorem edge_diag_h4_unif
  ...
  have hpath : Inner.inner ℝ V QTT = Inner.inner ℝ PTT D2T := by
    simpa only [QTT, PTT, D2T] using
      (edgePath_inner_bi (I := I) (M := M) g T T T V hδ₀_lt
        hδ hδZ qA qB q epsilon)
  have hcap_nonneg : 0 ≤ Cmor * C2 * R := by positivity
