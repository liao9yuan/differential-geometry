...
open DifferentialGeometry.Analysis.Spectral (ccTensorToHs edgeLiePairFam lc0AMix lc0Insert lc0Riem
  lc0VB lieCorr0Field tail_base_split)
...
private theorem ricci_de_turck_sum_jet_sq_le
...
theorem rhs_self_low_pair_h2
  ...
  obtain ⟨ρg, G0, G1, hρg, hG0, hG1, hgood⟩ :=
    LowBaseInternal.ricci_good_pair_h2 (I := I) (M := M) hDim g
  ...
  obtain ⟨ρa, W0, W1, hρa, hW0, hW1, hamix⟩ :=
    LowBaseInternal.amix_pair_h2 (I := I) (M := M) hDim g
  obtain ⟨ρr, C0, C1, hρr, hC0, hC1, hriem⟩ :=
    LowBaseInternal.riemann_pair_h2 (I := I) (M := M) hDim g
  ...
  refine (ricci_de_turck_sum_jet_sq_le (I := I) (M := M) g Y1 Y2 Y3 Y4 Y5 Zg Zl Zv Za Zr
...
private theorem rhs_self_low_pair_h3
  ...
  obtain ⟨ρg, G, hρg, hG, hgood⟩ :=
    LowBaseInternal.ricci_good_pair_h3 (I := I) (M := M) hDim g
  obtain ⟨ρl, L, hρl, hL, hlie⟩ :=
    LowBaseInternal.lie_covariant_pair_h3 (I := I) (M := M) hDim g
  obtain ⟨ρv, V, hρv, hV, hvb⟩ :=
    LowBaseInternal.vector_bundle_pair_h3 (I := I) (M := M) hDim g
  obtain ⟨ρa, W, hρa, hW, hamix⟩ :=
    LowBaseInternal.amix_pair_h3 (I := I) (M := M) hDim g
  obtain ⟨ρr, C, hρr, hC, hriem⟩ :=
    LowBaseInternal.riemann_pair_h3 (I := I) (M := M) hDim g
  ...
  refine (ricci_de_turck_sum_jet_sq_le (I := I) (M := M) g
