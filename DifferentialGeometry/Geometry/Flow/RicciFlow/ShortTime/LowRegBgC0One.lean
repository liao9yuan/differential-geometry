...
open DifferentialGeometry.Analysis.Sobolev (metricConnDiffLoweredCc)
open DifferentialGeometry.Analysis.Spectral
  (appCcRS appCcRS_sub_left appCcRS_sub_right ccTensorToHs ccTensorToHs_smul
    fullRaisedEndoField permCoeff slotExtend slotExtend_sub slotExtendIter
...
theorem amixOne_h2
  ...
  have hhalf : ∀ σlast : Equiv.Perm (Fin 4),
    ...
    let X1 : SmoothCcTensor g 3 3 :=
      appCcRS (I := I) (M := M) g 3 5 3
        (lc0TraceRF (I := I) (M := M) g gm 3
          DifferentialGeometry.Analysis.Spectral.LieCorr0Core.lieCorr0AMixPermQ) X0
    ...
    let X3 : SmoothCcTensor g 3 4 :=
      appCcRS (I := I) (M := M) g 3 6 4
        (lc0TraceRF (I := I) (M := M) g gm 4
          DifferentialGeometry.Analysis.Spectral.LieCorr0Core.lieCorr0AMixPerm1) X2
    ...
    have hx1 : lowJetSq (I := I) (M := M) g 2 X1 ≤ Z1 R := by
      ...
      simpa only [Z1] using
        mul_le_mul (mul_le_mul_of_nonneg_left
          (ht3 DifferentialGeometry.Analysis.Spectral.LieCorr0Core.lieCorr0AMixPermQ) hC1) hx0
    ...
    have hx3 : lowJetSq (I := I) (M := M) g 2 X3 ≤ Z3 R * S := by
      ...
      have hmul := mul_le_mul
        (mul_le_mul_of_nonneg_left
          (ht4 DifferentialGeometry.Analysis.Spectral.LieCorr0Core.lieCorr0AMixPerm1) hC3) hx2
  ...
  have hy0 : lowJetSq (I := I) (M := M) g 2 Y0 ≤ Q R * S :=
    hhalf DifferentialGeometry.Analysis.Spectral.LieCorr0Core.lieCorr0AMixPerm2
  have hy1 : lowJetSq (I := I) (M := M) g 2 Y1 ≤ Q R * S :=
    hhalf (lc0SwapPermRF * DifferentialGeometry.Analysis.Spectral.LieCorr0Core.lieCorr0AMixPerm2)
