...
private theorem pbLow_h2_mul
  ...
  let C : ℝ := Ca * AF
  have hC : 0 ≤ C := mul_nonneg hCa hAF
  refine ⟨C, hC, ?_⟩
  intro P A hA hP
  ...
  have hP' : (∑ j ∈ Finset.range 3,
      ‖iteratedCovGrad (I := I) g 0 2 j P‖ ^ 2) ≤ A ^ 2 := by
    simpa only [lowJetSq, Nat.reduceAdd] using hP
