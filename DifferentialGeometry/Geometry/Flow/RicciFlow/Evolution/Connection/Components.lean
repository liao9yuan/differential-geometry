...
theorem metricCovDerivOfMix
  ...
  have hCa :
      HasDerivWithinAt
        ...
        ((-2 : Real) *
          S.ricciAt (t : Real) x (DifferentialGeometry.Geometry.Curvature.vec2 Ca (frame b x)))
        D.carrier
        (t : Real) := by
  ...
  have hCb :
      HasDerivWithinAt
        ...
        ((-2 : Real) *
          S.ricciAt (t : Real) x (DifferentialGeometry.Geometry.Curvature.vec2 (frame a x) Cb))
        D.carrier
        (t : Real) := by
