...
theorem flow_end_le
  ...
  let scalarLap : Real → M → Real := fun t x =>
    DifferentialGeometry.Integral.Connection.laplacianAt (I := I) G t (S.scalar t) x
  ...
  have hevol : ScalarEvolutionEquationOn
      (D := DifferentialGeometry.Integral.Connection.RealTimeInterval.closedOpen 0 T hT)
  ...
  have hreg : ∀ U : Real, 0 < U → U < T →
    ...
    have hsubset : ∀ t : Real, t ∈ Set.Icc 0 U →
        t ∈ (DifferentialGeometry.Integral.Connection.RealTimeInterval.closedOpen 0 T hT).carrier := by
      intro t ht
