...
/-!
# Backward Ricci-flow connection variation

This file packages the coordinate Christoffel evolution of an ordinary
Ricci-flow solution as a scalar, intrinsic derivative of the Levi-Civita
connection difference.  Pairing before taking the derivative avoids exposing
the moving tangent-valued family to downstream arguments.
-/
...
/-- Under backward Ricci-flow time, the metric pairing of the connection
difference has derivative
`nabla_X Ric (Y,W) + nabla_Y Ric (X,W) - nabla_W Ric (X,Y)`.
The comparison connection and the pairing metric are frozen at the base
backward time. -/
theorem connBack_pair
  ...
  have hcoeff : ∀ i j k : CoordinateIdx (𝕜 := Real) E,
    ...
    intro i j k
    rw [hAat]
    simp only [A, ContinuousLinearMap.neg_apply, map_neg]
  ...
  have hGamma : ∀ i j k : CoordinateIdx (𝕜 := Real) E,
    ...
    simpa only [g₁, g₂, SolutionOn.family_connection, hcoeff i j k] using hsub
  have hvec := connDiffVec_hasDerivAt (I := I) g₁ g₂ frame hframe
  ...
  have hinv : MetricInverseInBasis_gen (I := I)
    ...
    simpa [b, hframe, frame, gInv, u, coordInv] using
  ...
  have hfwd := lowerHam_eq_perm (I := I) (S.base.metric (T - tau)) b
  ...
  have hspeed :
    ...
    have hham : hamSum (I := I) N (vec3 X Y W) =
        -N (vec3 X Y W) - N (vec3 Y X W) + N (vec3 W X Y) := by
      unfold hamSum
      simp only [Tensor0SSpace.add_apply, Tensor0SSpace.smul_apply,
        perm3_apply, smul_eq_mul, neg_mul, one_mul]
      have hswap :
    ...
    change lowerBilin (I := I)
        (metricTensorField (I := I) (S.base.metric (T - tau)) x) Af
          (vec3 X Y W) = hamSum (I := I) N (vec3 X Y W) at heval
    rw [lowerBilin_apply, metricTensorField_apply, hham] at heval
    have heval' :
  ...
  simpa [Function.comp_apply, g₁, g₂, N] using hpair'
...
private theorem connBack_frame_sq
  ...
  have hfixed : HasDerivAt (fun r : Real ↦ F r x₀)
    ...
    simpa only [F, q, Function.comp_apply, mul_comm] using hcomp
...
private theorem connBack_low_sq_diff
  ...
  have hframe : ∀ (r : Real), alpha r ∈ e.baseSet → ∀ i,
    ...
    intro r hr i
    simpa only [e, b, frame, coordinateFrameAt, coordinateTrivializationAt] using
      (e.localFrame_apply_of_mem_baseSet (b := b) (i := i) hr).symm
  have hc_repr : ∀ (r : Real), alpha r ∈ e.baseSet → ∀ K,
    ...
    have htriv := Tensor0SSpace.trivializationAt_apply
      (𝕜 := Real) (I := I) (x₀ := x₀) (x := alpha r) 3 hr (C r)
        (fun i ↦ b (K i))
    rw [hslots] at htriv
...
omit [SigmaCompactSpace M] in
/-- Along a differentiable curve, the backward-time connection difference
paired with three differentiable moving vectors has the expected Ricci
covariant-derivative variation after the square reparametrization. -/
theorem connBack_along_sq
  ...
  let R : Tensor0SSpace 3 I x₀ := -hamSum (I := I) N
  ...
  have hc0 : ∀ K, c K s = 0 := by
    intro K
    simp only [c, C, q]
    have hself := connDiffLowAt_self (I := I) q (alpha s)
    rw [connDiffLow_eq_lower] at hself
    rw [hself]
    rfl
  have hB0 : B s = 0 := by
  ...
  have hV : ∀ i, HasDerivAt (V i) (deriv (V i) s) s := by
    intro i
    fin_cases i
    · simpa only [V, Fin.zero_eta, ↓reduceIte] using hA.hasDerivAt
    · simpa only [V, Fin.mk_one, OfNat.one_ne_ofNat, ↓reduceIte] using hY.hasDerivAt
    · simpa only [V, Fin.mk_eq_mk, OfNat.zero_ne_ofNat, OfNat.one_ne_ofNat,
        ↓reduceIte] using hW.hasDerivAt
  ...
  have heval : (fun r ↦ B r (fun i ↦ V i r)) =ᶠ[𝓝 s]
    ...
    have hslot : ∀ i,
      ...
      intro i
      fin_cases i
      · simpa only [V, Fin.zero_eta, ↓reduceIte, chartRepAt_apply] using
          e.symmL_continuousLinearMapAt (R := Real) hr (A r)
      · simpa only [V, Fin.mk_one, OfNat.one_ne_ofNat, ↓reduceIte,
          chartRepAt_apply] using
          e.symmL_continuousLinearMapAt (R := Real) hr (Y r)
      · simpa only [V, Fin.mk_eq_mk, OfNat.zero_ne_ofNat,
          OfNat.one_ne_ofNat, ↓reduceIte, chartRepAt_apply] using
  ...
  have hB' : B' (fun i ↦ V i s) =
    ...
    have hsum := b₃.sum_repr
      ((R.compContinuousLinearMap fun _ ↦ e.symmL Real x₀) :
    ...
    have hB'eq : B' =
      ...
      simp only [ContinuousMultilinearMap.sum_apply,
        ContinuousMultilinearMap.smul_apply, smul_eq_mul] at heval
    ...
    have hslot0 : ∀ i,
      ...
      intro i
      fin_cases i
      · simpa only [V, Fin.zero_eta, ↓reduceIte, x₀] using
          symmL_chartRepAt_self (I := I) alpha A s
      · simpa only [V, Fin.mk_one, OfNat.one_ne_ofNat, ↓reduceIte, x₀] using
          symmL_chartRepAt_self (I := I) alpha Y s
      · simpa only [V, Fin.mk_eq_mk, OfNat.zero_ne_ofNat,
          OfNat.one_ne_ofNat, ↓reduceIte, x₀] using
          symmL_chartRepAt_self (I := I) alpha W s
...
omit [SigmaCompactSpace M] in
/-- The backward-time connection difference applied to two differentiable
moving vectors is differentiable after square reparametrization. -/
theorem connBack_vec_sq
  ...
  have hf : ∀ j, DifferentiableAt Real (f j) s := by
    ...
    have hV : ∀ i, HasDerivAt (V i) (deriv (V i) s) s := by
      intro i
      fin_cases i
      · simpa only [V, Fin.zero_eta, ↓reduceIte, X] using hA.hasDerivAt
      · simpa only [V, Fin.mk_one, OfNat.one_ne_ofNat, ↓reduceIte, Z] using hY.hasDerivAt
      · simpa [V] using (hasDerivAt_const s (b j))
  ...
  have hCeq : C =ᶠ[𝓝 s] fun r ↦
    ...
    rw [← (trivializationAt E (TangentSpace I : M → Type _) x₀).symmL_continuousLinearMapAt
      (R := Real) hr (C r)]
    congr 1
    change (trivializationAt E (TangentSpace I : M → Type _) x₀).continuousLinearMapAt Real
      (alpha r) (C r) = G r
