...
/-- Coordinate-chart Ricci components of a Ricci-flow solution are jointly
smooth in regular time and chart position. -/
...
omit [SigmaCompactSpace M] in
/-- Fixed-chart components of the covariant derivative of Ricci curvature are
jointly smooth on regular Ricci-flow spacetime chart domains. -/
theorem chartNablaRicci [I.Boundaryless]
  ...
  have hpart : ContDiffOn Real ∞
    ...
    simpa only [partialDeriv] using
      hfd.clm_apply (contDiffOn_const (c := chartModelBasis E d))
...
omit [SigmaCompactSpace M] in
/-- The covariant derivatives of Ricci curvature form a continuous rank-three
