...
omit [SigmaCompactSpace M] in
/-- Backward-time derivative of the coordinate-frame Christoffel symbols along
an ordinary Ricci-flow solution. -/
theorem coordGammaBack
  ...
  have hsub : HasDerivAt (fun s : Real ↦ T - s) (-1) tau :=
    by simpa only [Pi.sub_apply, id_eq, zero_sub] using
      (hasDerivAt_const tau T).sub (hasDerivAt_id tau)
  have hcomp := hforwardAt.comp tau hsub
  simpa using hcomp

omit [SigmaCompactSpace M] in
/-- Backward-time derivative of the coordinate-frame connection difference,
paired using the moving metric. -/
theorem coordConnBack
  ...
  have hsub : HasDerivAt (fun s : Real ↦ T - s) (-1) tau :=
    by simpa only [Pi.sub_apply, id_eq, zero_sub] using
      (hasDerivAt_const tau T).sub (hasDerivAt_id tau)
  have hcomp := hforwardAt.comp tau hsub
  simpa [frame, N, metricDt] using hcomp
