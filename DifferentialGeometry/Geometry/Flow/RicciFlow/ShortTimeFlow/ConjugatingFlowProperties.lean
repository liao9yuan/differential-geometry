...
`s ↦ g.inner (Φ_fam s x) (dΦ_s v) (dΦ_s w)` for a family generated by `-X`
has within-set derivative `-𝓛_{X_t} g` at the interior time `t`.  This is the
metric-compatibility chain rule
...
covariant variational equation `flow_cov_variation`, then folded into the
...
theorem flow_slot_deriv
  ...
  have hsumval : g.inner (γ t) (covDerivAlong (I := I) g γ V t) (W t)
    ...
    rw [hAval, hBval]
    exact lie_deriv_metric_neg_eq_pushforward_variation_sum (I := I) g (X t)
      ((Φ_fam t : M → M) x)
...
theorem flow_slot_pos
    ...
    (hΦode : ∀ x : M, ∀ t ∈ Set.Ioo (0 : ℝ) T,
      HasMFDerivWithinAt 𝓘(ℝ, ℝ) I (fun s : ℝ => (Φ_fam s : M → M) x)
        (Set.Ici (0 : ℝ)) t
        ((1 : ℝ →L[ℝ] ℝ).smulRight (Y t ((Φ_fam t : M → M) x))))
    (hjoint : ContMDiffOn (𝓘(ℝ, ℝ).prod I) I ∞
      (fun q : ℝ × M => (Φ_fam q.1 : M → M) q.2)
      (Set.Ioo (0 : ℝ) T ×ˢ Set.univ))
    (t : ℝ) (ht : t ∈ Set.Ioo (0 : ℝ) T) (x : M) (v w : TangentSpace I x) :
    HasDerivWithinAt
      ...
      (lieDerivMetric (I := I) g (Y t) ((Φ_fam t : M → M) x)
        (mfderiv I I (Φ_fam t : M → M) x v)
        (mfderiv I I (Φ_fam t : M → M) x w)) (Set.Ici 0) t := by
  ...
  have h := flow_slot_deriv (I := I) g (fun s => -Y s) T Φ_fam hneg hjoint
    t ht x v w
  convert h using 1
  rw [show -Y t = (-1 : ℝ) • Y t by simp,
...
theorem conjugating_flow_flat_data
  ...
  have h_push := flow_slot_deriv (I := I) (g_DT t)
    (fun s => deTurckVF (I := I) (g_DT s) g_bg) T Φ_fam hΦode hjoint t ht x v w
  obtain ⟨Q', hQ'⟩ := evalForm_joint (I := I) g_DT T Φ_fam hjoint hgram_DT
    t ht x v w
