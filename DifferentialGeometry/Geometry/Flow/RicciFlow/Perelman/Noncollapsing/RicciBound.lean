...
/-- A controlled parabolic Riemann-curvature norm gives a pointwise Ricci
quadratic lower bound on the flow metric ball. -/
theorem ricci_ge_of_rm
  ...
  have hroot :
    ...
    simpa only [FlowMetricBall.rmNormSq, SolutionFamily.rm04] using hnorm
  exact Geometry.Riemannian.BonnetMyers.ricciLowerAt_of_rm
    (I := I) (S.base.metric t) hroot v

/-- A controlled parabolic Riemann-curvature norm gives the corresponding
absolute Ricci quadratic bound at every point of the flow metric ball. -/
theorem ricci_abs_of_rm
  ...
  have hnorm : FlowMetricBall.rmNormSq S t x ≤ 1 / B.radius ^ 4 := by
    apply (le_div_iff₀ (pow_pos B.radius_pos 4)).2
    simpa only [mul_comm] using hB.2 t ht x hx
  simpa only [FlowMetricBall.rmNormSq, SolutionFamily.rm04] using
    (ricci_quad_sol (I := I) S x v hnorm)
