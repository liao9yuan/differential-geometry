...
/-!
# Curvature bounds for complete Perelman flows

This module derives the scalar-potential and metric-comparison inputs of the
complete-flow compact-localization theorem from one uniform Riemann-curvature
bound on the relevant regular spacetime slab.
-/
...
/-- A uniform curvature-square bound gives the scalar-potential lower bound
used by regularized L-action coercivity. -/
theorem lRegPot_lower_rm
  ...
  have hscalar' : |S.scalar (T - s ^ 2) x| ≤ lRmFactor E K := by
    simpa only [SolutionOn.scalar, SolutionFamily.scalar, SolutionFamily.rm04,
      metricRm04_apply, lRmFactor] using hscalar.trans
...
omit [NeZero (Module.finrank Real E)] [I.Boundaryless]
  [T2Space (TangentBundle I M)] in
/-- A uniform curvature-square bound gives the matching scalar-potential upper
bound on a regularized square-root-time interval. -/
...
omit [NeZero (Module.finrank Real E)] [I.Boundaryless]
  [T2Space (TangentBundle I M)] in
/-- A uniform curvature-square bound on a regular slab exponentially compares
the terminal metric with every square-root backward-time metric. -/
theorem lRegMetric_le_rm
  ...
  have hquad : ∀ i : Nat, ∀ t : Real, t ∈ Icc (T - b ^ 2) T →
    ...
    have hbound := twoTensorQuadBound_of_solutions (I := I)
      ...
      (hreg.trans D.regular_subset) (fun _ t ht y _ ↦ hRm t ht y)
    intro i t ht y hy w
...
/-- Uniform Riemann curvature on a regular backward slab supplies every
geometric input needed to compactly localize an action-bounded L-curve. -/
...
/-- A common action bound for curves with a common initial point, together
with uniform Riemann curvature on the backward slab, puts every curve in one
compact terminal-metric ball. -/
