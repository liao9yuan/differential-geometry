...
This file defines the scalar, fully paired linearization of the regularized
L-geodesic equation in square-root time.  All along-curve covariant derivatives
at `s` use the metric frozen at backward time `T - s ^ 2`.
-/
...
/-- The regularized L-Jacobi residual paired with a test vector.  The first
slot of `totalNabla0SFun` is the derivative slot, so the Ricci derivative term
is `(nabla_Y Ric)(A,W)`. -/
...
/-- A smooth regularized variation supplies the central curve, transverse
field, and frozen covariant-derivative regularity at a point. -/
theorem lRegVar_reg
  ...
  have hYdiff : DifferentiableAt Real
    ...
    simpa only [lVelocity, varFst] using
      variationField_chartRep_differentiableAt
        (I := I) (S.base.metric (T - s ^ 2)) f hf s
  ...
  have hVfstAll : ContMDiff
      (𝓘(Real, Real).prod 𝓘(Real, Real)) I.tangent (7 : Nat)
      (fun q : Real × Real ↦
        (TotalSpace.mk' E (E := (TangentSpace I : M → Type _))
    ...
    simpa only [Function.comp_apply, varFst] using hcomp
  have hVfst : ContMDiffAt
      (𝓘(Real, Real).prod 𝓘(Real, Real)) I.tangent 2
      (fun q : Real × Real ↦
        (TotalSpace.mk' E (E := (TangentSpace I : M → Type _))
...
private theorem lReg_ricci_deriv
  ...
  have h := tensor_eval_deriv (I := I)
    ...
    beta slots 0 hbeta (fun q ↦ by
      fin_cases q
      · simpa only [hslot0] using hA
      · simpa only [hslot1] using hW)
  convert h using 1
  congr 1
  · apply congrArg
    funext q
...
private theorem lReg_chart_diff
    ...
    (hV : ContMDiffAt 𝓘(Real, Real) I.tangent 1
      (fun r : Real ↦
        (TotalSpace.mk' E (E := (TangentSpace I : M → Type _))
  ...
  have hAt : ContMDiffAt 𝓘(Real, Real) I.tangent 1 Q t := by
  ...
  have hfiberDiff : ContDiffAt Real 1
    ...
    exact hfiber'
  simpa only [chartRepAt] using hfiberDiff.differentiableAt (by norm_num)
...
private theorem lReg_vsnd_at
    ...
    ContMDiffAt (𝓘(Real, Real).prod 𝓘(Real, Real)) I.tangent 2
      (fun q : Real × Real ↦
        (TotalSpace.mk' E (E := (TangentSpace I : M → Type _))
          (f q.1 q.2) (varSnd (I := I) f q.1 q.2) : TangentBundle I M))
      (0, s) := by
  simpa only [varSnd] using
    velocity_mdiff_at (I := I) (M := M) 2 f (0, s) hf
...
private theorem lReg_vfst_at
  ...
  have hft : ContMDiffAt (𝓘(Real, Real).prod 𝓘(Real, Real)) I 3
    ...
    simpa only [ft] using hf.comp (s, 0) hswap
  ...
  have hback : ContMDiffAt
    ...
    (contMDiff_snd.prodMk contMDiff_fst).contMDiffAt
  simpa only [Function.comp_apply, ft, varFst] using
    hvel.comp (0, s) hback
...
private theorem lReg_var_comm
  ...
  have hslice_u : ∀ᶠ u in 𝓝 (0 : Real),
    ...
    simpa only [F] using hu.comp s hincl
  have hslice_v : ∀ᶠ r in 𝓝 s,
    ...
    simpa only [F] using hr.comp 0 hincl
  ...
  have hpull : ContMDiffAt
      ...
      (0, s) := by
    simpa only [Function.comp_apply] using hext.comp (0, s) hf
...
private theorem lReg_accel_var_at
  ...
  have hcomm : ∀ᶠ r in 𝓝 s,
    ...
    filter_upwards [hlineR.eventually hfev] with r hr
    simpa only [covFst, covSnd, varFst, varSnd] using
      lReg_var_comm (I := I) g f r (by simpa only [F] using hr)
  have hsnd :
      covSndFst (I := I) g f (varSnd (I := I) f) 0 s =
...
/-- The regularized L-Euler residual paired with a tangent vector. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The regularized Euler pairing is the positive square-root-time
reparameterization of the dynamic Euler pairing. -/
  ...
  change _ =
    (S.base.metric (T - s ^ 2)).inner (sqReparam gamma s) Y
      (covDerivAlong (I := I) (S.base.metric (T - s ^ 2))
        ...
        lRegAccel S T s (sqReparam gamma s)
          (lVelocity (I := I) (sqReparam gamma) s))
  rw [hsub, lRegAccel_inner]
  simp only [sqReparam]
  ring_nf
  rfl

omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- The transverse derivative of the regularized L-Euler residual is the
regularized L-Jacobi residual plus the Euler residual paired with the
covariant derivative of the test field. -/
theorem lRegEuler_deriv
  ...
  have hbeta : ContMDiffAt 𝓘(Real, Real) I 3 beta 0 := by
    have hline : ContMDiffAt 𝓘(Real, Real)
        ...
        (fun u : Real ↦ (u, s)) 0 :=
      (contMDiff_id.prodMk contMDiff_const).contMDiffAt
    simpa only [beta] using hf.comp 0 hline
  ...
  have hAtotal : ContMDiffAt 𝓘(Real, Real) I.tangent 1
      (fun u : Real ↦
        (TotalSpace.mk' E (E := (TangentSpace I : M → Type _))
    ...
    have hline : ContMDiffAt 𝓘(Real, Real)
        ...
        (fun u : Real ↦ (u, s)) 0 :=
      (contMDiff_id.prodMk contMDiff_const).contMDiffAt
    simpa only using (hVsnd.comp 0 hline).of_le (by norm_num)
  ...
  have hcalc :=
    (hinner.sub (hdu.const_mul (2 * s ^ 2))).add
      (hric.const_mul (4 * s))
  have hcalcF : HasDerivAt F
      ((g.inner (beta 0) (covDerivAlong (I := I) g beta W 0) (B 0) +
          g.inner (beta 0) (W 0) (covDerivAlong (I := I) g beta B 0)) -
        2 * s ^ 2 *
          (hessianSec (I := I) (LeviCivita (I := I) g)
            ...
            duSec (I := I) (S.scalar (T - s ^ 2))
              (scalarSmoothOfSol (I := I) S (T - s ^ 2)) (beta 0)
              (fun _ : Fin 1 ↦ covDerivAlong (I := I) g beta W 0)) +
        4 * s *
          (totalNabla0SFun (𝕜 := Real) (I := I) 2
              (S.base.connection (T - s ^ 2)) (S.ricci (T - s ^ 2))
              (beta 0)
              (vec3 (lVelocity (I := I) beta 0) (A 0) (W 0)) +
            (S.ricci (T - s ^ 2) (beta 0)
                (vec2 (covDerivAlong (I := I) g beta A 0) (W 0)) +
              S.ricci (T - s ^ 2) (beta 0)
                (vec2 (A 0) (covDerivAlong (I := I) g beta W 0))))) 0 := by
    simpa only [F] using hcalc
  have hfun :
...
/-- Pointwise regularity and the scalar regularized L-Jacobi equation. -/
def HasLRegJacobiAt
    (S : SolutionOn (I := I) (M := M) D) (T : Real)
    (alpha : Real → M) (Y : ∀ s, TangentSpace I (alpha s))
    (s : Real) : Prop :=
...
/-- A vector field satisfies the regularized L-Jacobi equation on a set. -/
def IsLRegJacobi
    (S : SolutionOn (I := I) (M := M) D) (T : Real)
    (alpha : Real → M) (Y : ∀ s, TangentSpace I (alpha s))
    (J : Set Real) : Prop :=
...
/-- The moving-metric energy of a regularized L-Jacobi field and its
metric-dependent covariant velocity. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- Regularized L-Jacobi energy is nonnegative. -/
...
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- A locally smooth variation through regularized L-geodesics has a
regularized L-Jacobi transverse field at its central point. -/
theorem lRegVar_jacobiAt
  ...
  have hVsnd := lReg_vsnd_at (I := I) f s hf
  have hVfst := lReg_vfst_at (I := I) f s hf
  have hYtotal : ContMDiffAt 𝓘(Real, Real) I.tangent 1
      (fun r : Real ↦
        (TotalSpace.mk' E (E := (TangentSpace I : M → Type _))
    ...
    have hline : ContMDiffAt 𝓘(Real, Real)
        ...
        (fun r : Real ↦ ((0 : Real), r)) s :=
      (contMDiff_const.prodMk contMDiff_id).contMDiffAt
    simpa only using (hVfst.comp s hline).of_le (by norm_num)
  have hYdiff : DifferentiableAt Real
...
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- The variation field of a smooth family of regularized L-geodesics is a
regularized L-Jacobi field. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- The variation of the maximal regularized L-curve with respect to its
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The initial-tangent differential of the L-exponential map is the
regularized Jacobi field evaluated at square-root time. -/
...
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- Differentiating the maximal regularized L-curve along an affine line of
initial tangents gives its initial-tangent differential. -/
theorem lRegJacobi_affine
  ...
  have hline_apply :
    ...
    have hfd : HasFDerivAt line
      ...
      simpa only [line] using
        ((hasFDerivAt_id (0 : Real)).smul_const v).const_add z
    rw [hfd.fderiv]
...
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- The initial-tangent differential of the maximal regularized L-curve is a
regularized L-Jacobi field throughout its central maximal domain. -/
theorem lRegCurve_jacobi
  ...
  have hfInf : ContMDiffAt
    ...
    simpa only [f, line, z, zero_smul, add_zero] using hcomp
  have hf : ContMDiffAt
  ...
  have hcenter : (line 0, s) ∈ lRegJointDom S T x := by
    simpa only [line, z, zero_smul, add_zero, lRegJointDom] using hsdom
  have hnear : ∀ᶠ u in 𝓝 (0 : Real),
    ...
    filter_upwards [hpre] with u hu
    simpa only [lRegJointDom] using hu
  ...
  let Ya : (r : Real) → TangentSpace I (lRegCurve S T x Z r) :=
    fun r ↦ lVelocity (I := I)
      (fun u : Real ↦ lRegCurve S T x (Z + u • V) r) 0
...
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- The initial-tangent Jacobi field has the regularized initial covariant
derivative prescribed by the normalization `A(0) = 2 Z`. -/
theorem lRegJacobi_d0
  ...
  have hcentralEv : (fun r : Real ↦ F 0 r) =ᶠ[𝓝 (0 : Real)]
    ...
    simp only [F, line, z, zero_smul, add_zero]
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The initial-tangent variation field vanishes at square-root time zero. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- At square-root time zero only the second covariant derivative and curvature
terms remain in the regularized L-Jacobi residual. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- The pointwise Jacobi equation annihilates every test vector. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- The regularized L-Jacobi residual is linear under subtraction once the
first and second covariant derivatives have been identified. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- Regularized L-Jacobi fields along the same curve are closed under
subtraction on an open parameter set. -/
...
/-- Frozen covariant-derivative regularity gives differentiability of the
metric-dependent regularized L-Jacobi velocity. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The nonsingular regularized velocity equation before imposing the
L-Jacobi equation. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- A regularized L-Jacobi field satisfies the nonsingular equation for its
metric-dependent covariant velocity. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)] in
/-- Exact derivative of regularized L-Jacobi energy.  The square-root time
variable removes every singular coefficient at `s = 0`. -/
...
private noncomputable def lCovFirst3
  ...
  lCovLast3 (I := I)
    (ContinuousMultilinearMap.domDomCongr
      (Equiv.swap (0 : Fin 3) (2 : Fin 3)) N) Y A
...
private theorem lCovFirst3_apply
  ...
  rw [lCovFirst3, lCovLast3_apply]
  change N (fun i ↦ vec3 Y A W ((Equiv.swap (0 : Fin 3) (2 : Fin 3)) i)) = _
  congr 1
  funext i
  fin_cases i <;> rfl

/-- The covector form of the regularized L-Jacobi force at one time. -/
...
/-- The vector-valued regularized L-Jacobi force obtained by raising its
covector with the moving metric. -/
...
private noncomputable def lRegJacobiCovLM
  ...
  let nabWA := (lFix2LM (I := I) A).comp
    ((tensor0S_curry (I := I) (𝕜 := Real) (M := M) 2 x
      (ContinuousMultilinearMap.domDomCongr
        (Equiv.swap (0 : Fin 3) (2 : Fin 3)) N)).toLinearMap)
...
/-- At a fixed point and time, the regularized L-Jacobi force is a continuous
linear map of the field and its covariant velocity. -/
...
/-- The fixed-chart representative of the pointwise regularized L-Jacobi
force. -/
...
/-- The pointwise first-order regularized L-Jacobi operator in one fixed
chart. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)] in
/-- The paired regularized L-Jacobi equation determines the moving covariant
velocity derivative as a tangent vector. -/
...
/-- In a fixed chart, a regularized L-Jacobi field and its moving covariant
velocity satisfy the expected first-order state equation at one time. -/
theorem lRegJacobi_state
  ...
  have hqcoord : deriv (chartCurve (I := I) x0 alpha) s = a s := by
    ...
    simpa only [a, A, chartCurve, chartRepAtBase_apply, lVelocity] using hcoord.symm
  have hbase :
  ...
  have hYcov : chartCovDerivAlong (I := I) q x0 alpha y s = p s := by
    ...
    simpa only [p, P, y, q] using hcoord
...
omit [InnerProductSpace Real E] in
/-- The fixed-chart regularized L-Jacobi state derivative is the value of its
pointwise continuous linear coefficient. -/
