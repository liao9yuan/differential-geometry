...
/-!
# Pointwise reduced-density monotonicity

This file combines the canonical-ray Hamilton `K` identity with the moving
L-exponential Jacobian estimate.  The first step is the diagonal derivative
of reduced length along a strict minimizing L-ray.
-/
...
/-- Along a strict minimizing L-ray, the diagonal reduced-length derivative
is the negative Hamilton-`K` boundary quotient. -/
theorem redLength_ray_K
  ...
  have hJoint : HasFDerivAt A L (Z, tau) := by
    simpa only [A, L, Lz, c, alpha, b, endMap, y] using
  ...
  have hact : HasDerivAt (fun r : Real ↦ A (z, r)) c tau := by
    ...
    simpa [L, Lz, ContinuousLinearMap.comp_apply,
      ContinuousLinearMap.inr_apply] using hout
    ...
    calc
      lRegAction S T alpha 0 (Real.sqrt r) =
        ...
        simpa only [alpha, lExp, sqrtReparam] using
          (lLength_sqrt (I := I) S T alpha r hrpos.le).symm
...
/-- Logarithm of the standard reduced-volume density pulled back by L-exp,
relative to the source Riemannian density. -/
...
/-- Standard reduced-volume density pulled back by L-exp, relative to the
source Riemannian density. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The pulled-back reduced logarithmic density has the exact Hamilton-`K`
rate on the strict injectivity domain. -/
theorem lRedLog_hasDeriv
  ...
  field_simp [n2, htau.ne']
  ring

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The pulled-back reduced density differentiates as itself times its
logarithmic rate. -/
      ...
      (Real.exp (lRedLog S T x Z tau) *
        (laplacian (I := I) (LeviCivita (I := I)
          ...
          lK S T (lRegCurve S T x Z) (Real.sqrt tau) /
            (2 * tau * Real.sqrt tau) -
          (Module.finrank Real E : Real) / (2 * tau))) tau := by
  simpa only [lRedJac] using (lRedLog_hasDeriv S hS T x htau hZ).exp

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The logarithmic pulled-back reduced density is nonincreasing wherever the
Hamilton-`K` Laplacian comparison is realized. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The pulled-back reduced density is nonincreasing wherever the
Hamilton-`K` Laplacian comparison is realized. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The pulled-back reduced density has nonpositive derivative at every
positive time in the strict injectivity domain. -/
theorem lRedJac_deriv_le0
  ...
  have hRint (i : Fin (Module.finrank Real E)) : IntervalIntegrable
    ...
    have hsec : Continuous (fun u : K ↦
      ...
      simpa only [K] using ((hPsm i).continuousOn.mono hseg).restrict
    ...
    have heval := hS.ricciCont.eval_continuous
      ...
      (v := fun k u ↦ vec2 (P i u) (P i u) k) (by
        intro k
        fin_cases k
        · simpa only [vec2, Fin.zero_eta] using hsec
        · simpa only [vec2, Fin.mk_one] using hsec)
    have hric : ContinuousOn
        (fun s : Real ↦ S.ricciAt (T - s ^ 2) (alpha s)
          (vec2 (P i s) (P i s))) K := by
      rw [continuousOn_iff_continuous_restrict]
      simpa only [K, SolutionOn.ricci, SolutionFamily.ricci_apply,
  ...
  · intro i j
    simpa only [alpha, b, lExp, hbsq] using hON i j
  · simpa only [alpha, b] using hIint
  · simpa only [alpha, b] using hRint

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Along one strict minimizing L-ray, the pulled-back reduced density is
antitone in positive backward time. -/
