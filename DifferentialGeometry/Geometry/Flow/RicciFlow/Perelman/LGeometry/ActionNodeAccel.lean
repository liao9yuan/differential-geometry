...
/-!
# Acceleration at finite L-action nodes

The global `C¹` regularity of a strict finite minimizer makes its chart phase
continuous across every internal node.  The regularized phase equation on the
two adjacent open pieces therefore extends across the node.
-/
...
theorem lFinNode_reg
  ...
  have hrepv : (fun r ↦ chartRepAtBase (I := I) x gamma X r)
    ...
    simpa only [v, X, chartCurve, chartRepAtBase_apply, lVelocity] using hbridge
  ...
  have hveldiff : DifferentiableAt Real
    ...
    simpa only [z, X, x, chartRepAtBase_foot] using hsnd.differentiableAt
  ...
  calc
    ...
    _ = (lRegAccel S T c (gamma c) (X c) : E) := by
