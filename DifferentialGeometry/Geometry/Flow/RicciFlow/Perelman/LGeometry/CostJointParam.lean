...
/-!
# Joint parameter upper continuity of compact L-cost

This module combines terminal-time and initial-point perturbations.  Its
analytic input is convergence of the chart kinetic and scalar action terms
when the terminal forward time and the chart representative vary together on
one compact regular slab.
-/
...
private theorem lScalar_param
  ...
  · exact ae_of_all _ fun s hs ↦ by
      ...
      have hscalarAt : Tendsto
        ...
        simpa only [tDn, tD] using hS.continuous_subtype.continuousAt.tendsto.comp
...
private theorem lAction_head_param
  ...
  have htmono : Monotone t := by
    apply Fin.monotone_iff_le_succ.2
    intro i
    have hi : i = 0 := Fin.eq_zero i
    subst i
    simpa only [t, Matrix.cons_val_zero, Matrix.cons_val_one] using hac
  have hact (n : Nat) : lRegAction S (Rn n) (alpha n) a c =
    ...
    have hraw := lRegAction_chart (I := I) (m := 1) S hMet hSc (Rn n) a c t htmono
      (by simp [t]) (by simp [t]) (fun _ ↦ p) (alpha n)
      (fun i ↦ by
        have hi : i = 0 := Fin.eq_zero i
        subst i
        simpa [lSegLen, t] using v n)
      (fun i ↦ by
        have hi : i = 0 := Fin.eq_zero i
        subst i
        simpa [t] using hsrc n)
      (fun i ↦ by
        have hi : i = 0 := Fin.eq_zero i
        subst i
        simpa [lSegLen, t] using hrep n) hregn
    simpa only [Fin.sum_univ_one, lSegLen, t] using hraw
  have hactLim : lRegAction S R gamma a c =
    ...
    have hraw := lRegAction_chart (I := I) (m := 1) S hMet hSc R a c t htmono
      (by simp [t]) (by simp [t]) (fun _ ↦ p) gamma
      (fun i ↦ by
        have hi : i = 0 := Fin.eq_zero i
        subst i
        simpa [lSegLen, t] using vLim)
      (fun i ↦ by
        have hi : i = 0 := Fin.eq_zero i
        subst i
        simpa [t] using hsrcLim)
      (fun i ↦ by
        have hi : i = 0 := Fin.eq_zero i
        subst i
        simpa [lSegLen, t] using hrepLim) hregLim
    simpa only [Fin.sum_univ_one, lSegLen, t] using hraw
...
omit [NeZero (Module.finrank Real E)] [T2Space M] in
/-- Affine chart heads have convergent action when their initial endpoint and
the terminal forward time converge together inside one compact regular slab. -/
theorem chart_head_T_lim
  ...
  have hderiv : Tendsto (fun n ↦ (v n).deriv) atTop (nhds u₀.deriv) := by
    simpa only [timeH1.timeDeriv_apply] using
      (timeH1.timeDeriv E (c - a)).continuous.continuousAt.tendsto.comp hv
...
omit [NeZero (Module.finrank Real E)] in
/-- Along jointly convergent terminal forward times and initial points, every
strict action upper bound for a fixed positive-time competitor eventually
bounds the corresponding L-costs. -/
theorem lCost_lt_param
  ...
  have hrep₀ : EqOn u₀.toFun (fun r ↦ extChartAt I x (alpha r))
      (Icc (0 : Real) c) := by
    simpa only [u₀, gamma, Function.comp_apply] using
