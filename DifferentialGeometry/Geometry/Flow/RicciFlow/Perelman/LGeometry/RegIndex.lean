...
/-!
# Regularized Perelman L-index form

This file expresses the L-index form in square-root backward time.  Its
pointwise balance remains nonsingular at the initial endpoint `s = 0`.
-/
...
/-- The symmetric L-index density in square-root backward time. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] in
/-- The regularized L-index density is symmetric in its two fields. -/
...
/-- Perelman's L-index form on an oriented square-root-time interval. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] in
/-- The regularized L-index form is symmetric in its two fields. -/
...
/-- The regularized L-index depends only on its fields on the open interval;
the two endpoints do not affect the interval integral. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- At positive square-root time, the ordinary index density times the
Jacobian `2 * s` is the regularized index density. -/
theorem lRegIndexInt_sq
  ...
  have hDY : covDerivAlong (I := I) q alpha Yb s = c • DY := by
    ...
    simpa only [q, alpha, Yb, c, DY, tau, sqReparam] using hcomp
  have hDW : covDerivAlong (I := I) q alpha Wb s = c • DW := by
    ...
    have hsqderiv : deriv (fun r : Real ↦ r ^ 2) s = c := by
      ...
      norm_num
    rw [hsqderiv] at hcomp
    simpa only [q, alpha, Wb, c, DW, tau, sqReparam] using hcomp
  have hinner :
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- The ordinary L-index form equals the regularized form after the monotone
substitution `tau = s ^ 2`; the exceptional point `s = 0` is removed only
almost everywhere. -/
theorem lIndex_sq
    (S : SolutionOn (I := I) (M := M) D) (T : Real)
...
/-- The derivative pairing is twice the regularized index density plus the
regularized L-Jacobi residual. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)] in
/-- Green's identity for the regularized L-index form, valid on intervals
whose endpoints may include square-root time zero. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)] in
/-- Green's identity has no boundary term when the test field vanishes at
both square-root-time endpoints. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)] in
/-- Along a regularized L-Jacobi field, the regularized L-index form is half
the endpoint derivative pairing. -/
