...
/-!
# Positive-start regularized L-families

This file begins the Calabi suffix construction by extracting a jointly smooth
family with fixed position and varying actual velocity at an arbitrary regular
square-root time.
-/
...
/-- Near any prescribed velocity at an arbitrary regular square-root time,
regularized L-curves with the same position form a jointly smooth family. -/
theorem exists_lTailFamily
  ...
  have hvel0 : lVelocity (I := I) gamma s0 = A := by
    calc
      lVelocity (I := I) gamma s0 = W s0 := hvel hs0
      _ = A := by
        change trivFromE (I := I) x (gamma s0) (z s0).2 = A
        rw [hgamma0, hzs0]
        exact trivFromE_self_apply (I := I) x A
...
private theorem tailSeed_smooth
  ...
  have hpos : ContDiffAt Real ∞ (fun A : E ↦ F (A, s0)) A0 := by
    simpa only [Function.comp_apply] using hF.comp A0 hincl
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] in
/-- A supplied local phase flow extends a positive-start regularized L-family
across one common neighboring interval. -/
theorem lTailFamily_step_of
  ...
  have hA0W : A0 ∈ W := by
    exact ⟨hA0V0, by simpa only [seed] using hseedU⟩
  ...
  have hetaReg : ∀ A ∈ W, ∀ r ∈ K,
    ...
    have hv : HasDerivAt (fun w : Real ↦ (z w).2)
      ...
      simpa using hasFDerivAt_snd.comp_hasDerivAt r hzr
  ...
  have hetaVel : ∀ A ∈ W,
    ...
    rw [hetaGamma]
    calc
      lVelocity (I := I) gamma t = lPhaseVel (I := I) x0 z t := hphaseVel
      _ = trivFromE (I := I) x0 (alpha (A, t)) (seed A).2 := by
        change trivFromE (I := I) x0 (gamma t) (z t).2 = _
        rw [hgamma0, hzt]
      _ = lVelocity (I := I) (fun r ↦ alpha (A, r)) t := by
        rw [hseedVel]
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] in
/-- A positive-start regularized L-family extends across one common
neighboring interval while preserving its fixed initial position and actual
initial velocity. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] in
/-- A prescribed regular positive-start L-solution through a later time lies
in one jointly smooth family parametrized by its actual velocity at the start
time. -/
...
omit [InnerProductSpace Real E] [FiniteDimensional Real E]
  ...
  [IsManifold I ∞ M] in
/-- Differentiating a smooth endpoint family along an affine parameter line
gives its parameter differential. -/
theorem lTailLine_deriv
  ...
  have hslice : ContMDiffAt 𝓘(Real, E) I ∞ phi A := by
    simpa only [phi] using halpha.comp A hincl
...
omit [InnerProductSpace Real E]
  [NeZero (Module.finrank Real E)] [I.Boundaryless] [T2Space M] in
/-- For an actual-velocity parametrized family, the affine-line Jacobi field
has initial covariant derivative equal to its parameter direction. -/
...
omit [InnerProductSpace Real E] [FiniteDimensional Real E]
  [NeZero (Module.finrank Real E)] [I.Boundaryless] [T2Space M] in
/-- A smooth field along a curve on an open neighborhood of a compact tail
admits a global smooth representative agreeing on a neighborhood of the tail. -/
...
omit [InnerProductSpace Real E] in
/-- An affine velocity-line variation of a positive-start regularized L-family
is an L-Jacobi field and vanishes at the family's fixed start. -/
