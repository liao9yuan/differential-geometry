...
/-!
# Endpoint bounds for minimizing L-rays

This module packages the compact-slab bound on minimizing initial tangents
whose endpoints converge to a regular L-ray endpoint.
-/
...
/-- Near the endpoint of a positive regularized L-ray, every strict upper
bound for the ray action is eventually an upper bound for fixed-time L-cost. -/
...
/-- A sequence of minimizing initial tangents with fixed positive time is
bounded when its endpoints converge to a regular L-ray endpoint. -/
