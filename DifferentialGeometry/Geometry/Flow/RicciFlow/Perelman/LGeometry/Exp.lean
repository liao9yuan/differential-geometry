...
/-!
# Perelman's L-exponential map

This file defines the maximal square-root-time domain of regularized
L-geodesics and totalizes the L-exponential map by returning the base point
outside that domain.
-/
...
/-- A regularized L-curve on a square-root-time set, with initial velocity
normalized by `A(0) = 2 Z`. -/
def IsLRegCurveOn
...
/-- Two regularized L-curve witnesses with the same initial data agree on the
intersection of their connected open square-root-time domains. -/
theorem lRegWitness_eq
  ...
  have hUopen : IsOpen U := by
    ...
    exact heq_of_eq (by
      simpa only [za, zb, va, vb, lVelocity] using
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- Two regularized L-solutions with the same phase state at one time agree on
the overlap of their connected open domains. -/
...
theorem lRegData_congr
  ...
  have hvel : lVelocity (I := I) gamma s =
    ...
    simpa only [lVelocity] using congrArg (fun L => L (1 : Real))
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Near a regular phase seed, the regularized L-phase equation has a jointly
smooth local flow that remains in the regular chart domain. -/
theorem exists_lPhaseFlow
  ...
  have hvf : ContDiffOn Real ∞ vf Omega := by
    ...
    simpa only [vf] using
      (contDiffAt_const.prodMk hphase).contDiffWithinAt
  ...
  · intro z hz s hs
    ...
    have htime : ∀ r ∈ Set.Ioo (-epsilon) epsilon,
      ...
      · intro r hr
        ...
        simpa only [vf] using hderiv.fst
      · have hstart := hPsi0 ((0 : Real), z) hz
        exact congrArg Prod.fst hstart
    have hderiv := hPsiDeriv ((0 : Real), z) hz s hs
    simpa only [Phi, lift, vf, htime s hs] using hderiv.snd
  · rintro ⟨z, s⟩ ⟨hz, hs⟩
    have hzero : (0 : Real) ∈ Set.Ioo (-epsilon) epsilon := by
      constructor <;> linarith
    have htime : ∀ r ∈ Set.Ioo (-epsilon) epsilon,
    ...
    simpa only [Phi, lift] using hmem

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Near a regular phase state at an arbitrary square-root time, the
regularized L-phase equation has a jointly smooth local flow. -/
  ...
  · intro z hz s hs
    ...
    have htime : ∀ r ∈ Set.Ioo (-epsilon) epsilon,
      ...
      have hphi : ∀ r ∈ Set.Ioo (-epsilon) epsilon,
        ...
        simpa only [phi, vf] using hderiv.fst.sub_const s0
    ...
    have hsnd := (hderiv.comp_sub_const s s0).snd
    have htime_s : (Psi ((s0, z), s - s0)).1 = s := by
      rw [htime (s - s0) hsrel]
      ring
    simpa only [Phi, lift, vf, htime_s] using hsnd
  · rintro ⟨z, s⟩ ⟨hz, hs⟩
    rcases hs with ⟨hslo, hshi⟩
    have hzero : (0 : Real) ∈ Set.Ioo (-epsilon) epsilon := by
      constructor <;> linarith
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- On a compact set of regular phase states in one chart, the regularized
L-phase equation has a common local existence time. -/
theorem exists_lPhaseComp
  ...
  · intro z hz s hs
    ...
    have htime : ∀ r ∈ Set.Ioo (-epsilon) epsilon,
      ...
      have hphi : ∀ r ∈ Set.Ioo (-epsilon) epsilon,
        ...
        simpa only [phi, vf] using hderiv.fst.sub_const p0.1
      have hphi0 : phi 0 = 0 := by
        have hstart := hPsi0 (p0.1, z) hz
        simp only [phi]
        rw [congrArg Prod.fst hstart]
    ...
    have hsnd := (hderiv.comp_sub_const s p0.1).snd
  ...
  · rintro ⟨z, s⟩ ⟨hz, hs⟩
    ...
    have hsrel : s - p0.1 ∈ Set.Ioo (-epsilon) epsilon :=
      ⟨by linarith [hs.1], by linarith [hs.2]⟩
    have hzW : (p0.1, z) ∈ W := by simpa only [U] using hz
...
private theorem timeVel_smoothAt
  ...
  have hF' : ContMDiffAt
      ...
      (Function.uncurry (fun Z : E => fun s : Real => F (Z, s)))
      (Z0, s0) := by
...
private theorem lPhaseSeed_smooth
  ...
  have hpos : ContDiffAt Real ∞ (fun Z : E => F (Z, s0)) Z0 := by
    simpa only [Function.comp_apply] using hF.comp Z0 hincl
...
theorem lPhaseSeed_vel
  ...
  simpa only [Function.comp_apply, lVelocity] using hcoord.symm

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- Near any initial tangent vector, regularized L-curves with the same base
point form a jointly smooth family on one common square-root-time interval. -/
theorem exists_lRegFamily
  ...
  have hphase : ContDiffOn Real ∞ phase
    ...
    simpa only [phase, Function.comp_apply] using
  ...
  have hdata : ∀ s ∈ Set.Ioo (-epsilon) epsilon,
    ...
    simpa only [z, phase, input] using hPhiMap hmem
...
theorem lRegFamily_step_of
  ...
  have hZ0W : Z0 ∈ W := by
    exact ⟨hZ0V0, by simpa only [seed] using hseedU⟩
  ...
  have hetaReg : ∀ Z ∈ W, ∀ s ∈ K,
    ...
    have hvel : Set.EqOn (fun s => lVelocity (I := I) gamma s) A K := by
      ...
      have hq : HasDerivAt (fun r : Real => (z r).1) (z s).2 s := by
        ...
        simpa only [lPhaseField] using h
      simpa only [gamma, A] using
        lPhase_velocity (I := I) x0 z s hq (hdata s hs).2
    intro s hs
    have hzs := hsol s hs
    have hsdata := hdata s hs
    have hq : HasDerivAt (fun r : Real => (z r).1) (z s).2 s := by
  ...
  have hetaVel : ∀ Z ∈ W,
    ...
    have hq : HasDerivAt (fun r : Real => (z r).1) (z s0).2 s0 := by
      ...
      simpa only [z, phase, input, lPhaseField] using h
    ...
    rw [hetaGamma]
    calc
      lVelocity (I := I) gamma s0 = lPhaseVel (I := I) x0 z s0 := hphaseVel
      _ = trivFromE (I := I) x0 (alpha (Z, s0)) (seed Z).2 := by
        change trivFromE (I := I) x0 (gamma s0) (z s0).2 = _
        rw [hgamma0, hzs0]
      _ = lVelocity (I := I) (fun s => alpha (Z, s)) s0 := by
        rw [hseedVel]
  ...
  have hbetaVel : lVelocity (I := I) (fun s => beta (Z, s)) 0 = 2 • Z := by
    ...
    have hvel := congrArg (fun L => L (1 : Real))
      (heq.mfderiv_eq (I := 𝓘(Real, Real)) (I' := I))
    simpa only [lVelocity] using hvel.trans (hcurves Z (hWV hZ)).2.1
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- A jointly smooth family of regularized L-curves can be restarted at any
time in its domain and extended across one common neighboring interval. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- A regularized L-curve witness through `s0` extends to one jointly smooth
family of nearby initial velocities on a common domain through `s0`. -/
...
/-- A witness that the regularized L-curve with initial data `(x,2Z)` exists
through square-root time `s` on one connected open interval containing zero. -/
...
/-- The maximal square-root-time domain covered by regularized L-curve
witnesses with initial data `(x,2Z)`. -/
def lRegDomain
    (S : SolutionOn (I := I) (M := M) D) (T : Real)
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The maximal regularized square-root-time domain is open. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The maximal regularized square-root-time domain is preconnected. -/
...
/-- A chosen regularized curve witnessing membership in `lRegDomain`. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The chosen regularized curve comes with a connected open witness interval. -/
...
/-- The totalized maximal regularized L-curve; outside its existence domain it
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Inside its domain, the totalized regularized curve uses a chosen witness. -/
...
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- On every witness interval, the totalized regularized curve agrees with that
witness. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- On its maximal joint domain, the regularized L-curve is jointly smooth in
the initial tangent vector and square-root time. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- At a regular terminal time, the maximal totalized regularized L-curve is
jointly smooth in its initial tangent vector and square-root time at time zero. -/
...
/-- The joint maximal domain of the regularized L-curve in initial velocity
...
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- The joint maximal domain of the regularized L-curve is open. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- The maximal regularized L-curve is jointly smooth on its joint domain. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Outside its domain, the totalized regularized curve equals its base point. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The totalized regularized curve starts at its prescribed base point. -/
...
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- At a regular terminal time, the totalized regularized curve has its
prescribed normalized initial velocity. -/
...
/-- The nonnegative backward-time domain of the L-exponential map. -/
def lExpDomain
    (S : SolutionOn (I := I) (M := M) D) (T : Real)
    (x : M) (Z : TangentSpace I x) : Set Real :=
...
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- Zero backward time belongs to every regular L-exponential domain. -/
...
/-- Perelman's totalized L-exponential map, evaluated at square-root time. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Membership in the positive joint domain is positive membership in the
ordinary L-exponential domain. -/
theorem mem_lExpPosDom
    (S : SolutionOn (I := I) (M := M) D) (T : Real)
    (x : M) (Z : TangentSpace I x) (tau : Real) :
    (Z, tau) ∈ lExpPosDom S T x ↔
      0 < tau ∧ tau ∈ lExpDomain S T x Z := by
  simp only [lExpPosDom, lRegJointDom, lExpDomain, mem_setOf_eq]
  constructor
  · rintro ⟨htau, hreg⟩
...
theorem lExpPosDom_open
  ...
  simpa only [lExpPosDom, reparam] using hopen

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- The L-exponential map is jointly smooth on its positive-time joint
domain. -/
theorem lExp_smoothOn
  ...
  simpa only [lExp, reparam, Function.comp_apply] using
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The L-exponential map equals its base point at zero backward time. -/
