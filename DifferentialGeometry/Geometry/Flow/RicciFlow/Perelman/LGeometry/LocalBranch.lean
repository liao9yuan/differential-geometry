...
/-!
# Local action branches for Perelman's L-geometry

This module develops the upper-touching calculus used by the multiple-ray
part of the L-cut locus.  The geometric branch construction is built from a
local inverse of the fixed-time L-exponential map.
-/
...
/-- Two differentiable upper barriers touching the same differentiable
function at one point have the same derivative there. -/
...
/-- A function touched from above by two differentiable barriers with
different derivatives is not differentiable at the touching point. -/
...
/-- The fixed-terminal-time action of a positive regularized L-ray depends
continuously on its initial tangent. -/
...
private theorem lRayAct_smooth
  ...
  obtain ⟨eps, heps, hball⟩ := (Metric.isOpen_iff.mp hVopen) Z hZV
  ...
  have hG : ContDiffOn Real ∞
    ...
    simpa only [G] using (contDiffAt_const.mul hcomp).contDiffWithinAt
...
omit [SigmaCompactSpace M] in
/-- The local action branch obtained by lifting an endpoint through the local
inverse of the fixed-time L-exponential map. -/
def lActBranch
    (S : SolutionOn (I := I) (M := M) D) (hS : IsSolutionOn (I := I) S)
...
theorem lActBranch_smooth
  ...
  simpa only [lActBranch, hloc, Function.comp_apply] using hcomp

omit [SigmaCompactSpace M] in
/-- A local inverse action branch is continuous at its center endpoint. -/
theorem lActBranch_cont
  ...
  simpa only [lActBranch, hloc] using hcomp

omit [SigmaCompactSpace M] in
/-- At a nonconjugate positive ray, the local action branch has cotangent
derivative given by metric lowering of the terminal ray velocity. -/
theorem lActBranch_hasMFD
  ...
  have hRay : HasFDerivAt rayAct
    ...
    simpa only [rayAct, alpha, y, metricFlatMap_apply] using
      lRayAct_hasFDeriv S hS T x Z hdom
  have hRayM : HasMFDerivAt (modelWithCornersSelf Real E)
      (modelWithCornersSelf Real Real) rayAct Z
      (alpha.comp
        (mfderiv (modelWithCornersSelf Real E) I
  ...
  have hderiv :
    ...
    change mfderiv (modelWithCornersSelf Real E) I
        (fun W : E ↦ lExp S T x W tau) Z
        (mfderiv I (modelWithCornersSelf Real E)
          hloc.localInverse y Y) = Y at hright
    change alpha
        (mfderiv (modelWithCornersSelf Real E) I
          (fun W : E ↦ lExp S T x W tau) Z
          (mfderiv I (modelWithCornersSelf Real E)
  ...
  simpa only [lActBranch, hloc, rayAct, y, alpha] using hcomp'
...
private theorem lActBranch_mfd_at
  ...
  have hcancel :
    ...
    simpa only [Function.comp_apply, W, mfderiv_id] using
...
private theorem lEndVel_cov
  ...
  have hF : IsSmoothVariation (I := I) F := by
    ...
    rw [← contMDiffOn_univ]
    simpa only [F, Function.comp_apply] using (halpha.of_le (by
  ...
  have hgradEv :
    ...
    exact Filter.Eventually.of_forall fun s ↦ by
      simpa only [g, Vterm, hloc, zeta] using hgrad (eta s) (hetaU s).1
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The Hessian of a nonconjugate local action branch is the terminal metric
pairing with the covariant derivative of the induced regularized L-Jacobi
field. -/
...
omit [SigmaCompactSpace M] in
/-- Near its center, a local inverse action branch is an upper barrier for
the fixed-time L-cost. -/
...
omit [SigmaCompactSpace M] in
/-- A nonconjugate local action branch has the endpoint first-variation
derivative along every smooth curve contained in a suitable neighborhood. -/
theorem exists_branch_deriv
  ...
  have hf : IsSmoothVariation (I := I) f := by
    ...
    simpa only [Function.comp_apply, f] using
      (halpha.of_le h8inf).comp hpair.contMDiffOn
        (fun p _hp ↦ hpairVK p)
  have h0ad : (0 : Real) ∈ Set.Icc a d :=
    ⟨ha0.le, (hb0.trans hbd).le⟩
  have hbad : b ∈ Set.Icc a d :=
...
omit [SigmaCompactSpace M] in
/-- A minimizing nonconjugate ray makes its local inverse action branch touch
the fixed-time L-cost at the common endpoint. -/
theorem lActBranch_touch
  ...
  have hlen :
    ...
    simpa only [lExp, sqrtReparam] using
      lLength_sqrt (I := I) S T (lRegCurve S T x Z) tau htau
...
omit [SigmaCompactSpace M] in
/-- Two distinct nonconjugate minimizing L-rays reaching the same endpoint
force the fixed-time L-cost to be nondifferentiable there. -/
theorem lCost_nondiff_two
  ...
  exact mdifferentiableAt_iff_differentiableAt.mp
    (by simpa only [c, Function.comp_apply] using hcomp)
