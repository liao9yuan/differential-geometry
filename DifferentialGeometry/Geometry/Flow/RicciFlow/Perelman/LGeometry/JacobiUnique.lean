...
private theorem lRegJacCLM_cont
  ...
  have hu : Continuous u := by
    have h := (continuousOn_extChartAt (I := I) x0).comp_continuous hb
      (fun z ↦ hbext z)
    simpa only [u, b, chartCurve] using h
  ...
  have hsec (v : P → E) (hv : Continuous v) :
    ...
    simpa only [e, trivFromE, Trivialization.symmL_apply] using
      continuousOn_univ.mp htot
  ...
  have hRm (Z : E × E) (j : Fin (Module.finrank Real E)) :
    ...
    intro i
    fin_cases i
    · simpa only [vec4, Fin.zero_eta] using hYsec Z
    · simpa only [vec4, Fin.mk_one] using hAsec
    · simpa only [vec4, Fin.mk_eq_mk] using hAsec
    · simpa only [vec4, Fin.mk_eq_mk] using hWsec j
  have hHess (Z : E × E) (j : Fin (Module.finrank Real E)) :
    ...
    intro i
    fin_cases i
    · simpa only [vec2, Fin.zero_eta] using hYsec Z
    · simpa only [vec2, Fin.mk_one] using hWsec j
    ...
    have h := hS.ricciCont.eval_continuous (P := P) htau
      (fun z ↦ D.regular_subset (hreg z z.2)) hb (v := fun i z ↦
        vec2 (Pv Z z) (Wv j z) i) (by
          intro i
          fin_cases i
          · simpa only [vec2, Fin.zero_eta] using hPsec Z
          · simpa only [vec2, Fin.mk_one] using hWsec j)
    simpa only [SolutionOn.ricci, SolutionFamily.ricci_apply] using h
  have hCov (Z : E × E) (j : Fin (Module.finrank Real E)) :
    ...
    have h3 : Continuous (fun z : P ↦ totalNabla0SFun (𝕜 := Real) (I := I) 2
      ...
      · simpa only [vec3, Fin.mk_one] using hYsec Z
      · simpa only [vec3, Fin.mk_eq_mk] using hWsec j
    have h4 : Continuous (fun z : P ↦ totalNabla0SFun (𝕜 := Real) (I := I) 2
      ...
      intro i
      fin_cases i
      · simpa only [vec3, Fin.zero_eta] using hYsec Z
    ...
    have h5 : Continuous (fun z : P ↦ totalNabla0SFun (𝕜 := Real) (I := I) 2
      ...
      intro i
      fin_cases i
      · simpa only [vec3, Fin.zero_eta] using hWsec j
      · simpa only [vec3, Fin.mk_one] using hAsec
      · simpa only [vec3, Fin.mk_eq_mk] using hYsec Z
  ...
  have hForce (Z : E × E) : Continuous (fun z : P ↦
    ...
    have hsum : Continuous (fun z : P ↦
      ...
      refine (continuous_finset_sum _ (fun j _ ↦
    ...
    simpa only [cv, Yv, A, Pv, Wv] using hsharp
  ...
  have hGamma (i j k : Fin (Module.finrank Real E)) :
    ...
    have hcomp := hraw.comp_continuous (htau.prodMk hu)
      (fun z ↦ ⟨hreg z z.2, hu_mem z⟩)
    simpa only [Function.comp_apply, SolutionOn.family_metric] using hcomp
  have hGApp (w : E) : Continuous (fun z : P ↦
    ...
    refine (continuous_finset_sum _ (fun i _ ↦
      continuous_finset_sum _ (fun j _ ↦ ?_))).smul continuous_const
...
/-- Two regularized L-Jacobi fields with the same field and moving covariant
velocity at one time agree on a connected open time set. -/
  ...
  have hUopen : IsOpen U := by
    ...
    have hC : ContinuousOn C (Ioo c d) := by
      ...
      have hraw := lRegJacCLM_cont (I := I) S hS T x0 (Ioo c d) alpha a
        (continuousOn_iff_continuous_restrict.mp halphaI)
        (continuousOn_iff_continuous_restrict.mp haI) hsrc
    ...
    have hYr : Y r = Y' r := by
      ...
      exact congrArg (trivFromE (I := I) x0 (alpha r))
        (by simpa only [z, z', y, y'] using congrArg Prod.fst hcoord)
    have hPr : P r = P' r := by
      rw [← trivFromE_trivToE (I := I) x0 hbase (P r),
        ← trivFromE_trivToE (I := I) x0 hbase (P' r)]
      exact congrArg (trivFromE (I := I) x0 (alpha r))
        (by simpa only [z, z', p, p'] using congrArg Prod.snd hcoord)
    apply Prod.ext
