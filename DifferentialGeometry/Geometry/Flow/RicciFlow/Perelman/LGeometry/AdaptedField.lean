...
/-!
# Adapted fields for Perelman L-geometry

This file records the metric-pairing cancellation for fields adapted to the
square-root backward-time parametrization.
-/
...
private theorem movingRicciPair_smooth
  ...
  have hinv (i j : Fin (Module.finrank Real E)) : ContDiffAt Real ∞
    ...
    simpa only [base, Function.comp_apply] using hraw.comp t hbase
...
private theorem adaptCoeff_smooth
  ...
  have hcov : ContDiffOn Real ∞
      (fun s ↦ q.inner (alpha s) (U s)
        (covDerivAlong (I := I) (S.base.metric (T - s ^ 2)) alpha V s)) Omega := by
    simpa only using hcovM.contDiffOn
  have hscale : ContDiffOn Real ∞ (fun s : Real ↦ -2 * s) Omega :=
    contDiffOn_const.mul contDiffOn_id
  have hsmooth := hscale.mul hric |>.sub hcov
  simpa only [map_sub, ContinuousLinearMap.sub_apply, map_smul,
    ContinuousLinearMap.smul_apply, smul_eq_mul] using hsmooth
...
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- A terminal tangent vector extends to a square-root adapted field on an open
neighborhood of the closed parameter interval. -/
theorem exists_lAdapted
    ...
    ∃ (P : ∀ s, TangentSpace I (alpha s)) (Omega' : Set Real),
      ...
      ∀ s ∈ Omega',
        covDerivAlong (I := I) (S.base.metric (T - s ^ 2)) alpha P s =
          (-2 * s) •
            ricciSharp (I := I) (S.base.metric (T - s ^ 2)) (alpha s) (P s) := by
  classical
  ...
  have hab : -eta < b + eta := by linarith [hb, heta]
  ...
  have hzRaw := linearODESolution_contDiffOn_top hab hbmem isOpen_univ hA' hz0
  ...
  have hzDeriv : HasDerivAt z (A () s (z s)) s := by
    simpa only [z] using linearODESolution_hasDerivAt hab hbmem isOpen_univ
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The moving metric pairs two square-root adapted fields with zero
derivative. -/
theorem lAdapted_inner
    (S : SolutionOn (I := I) (M := M) D) (hS : IsSolutionOn (I := I) S)
    (T : Real) (alpha : Real → M)
    ...
    (hDV : covDerivAlong (I := I) (S.base.metric (T - s ^ 2)) alpha V s =
      (-2 * s) • ricciSharp (I := I) (S.base.metric (T - s ^ 2)) (alpha s) (V s))
    (hDW : covDerivAlong (I := I) (S.base.metric (T - s ^ 2)) alpha W s =
      (-2 * s) • ricciSharp (I := I) (S.base.metric (T - s ^ 2)) (alpha s) (W s)) :
    HasDerivAt
      (fun r : Real ↦
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The moving metric pairing of two square-root adapted fields is constant on
a closed interval. -/
theorem lAdapted_inner_eq
    (S : SolutionOn (I := I) (M := M) D) (hS : IsSolutionOn (I := I) S)
    (T : Real) (alpha : Real → M)
  ...
  have hd : ∀ s ∈ Set.Icc a b, HasDerivAt f 0 s := by
    intro s hs
    exact lAdapted_inner S hS T alpha V W s (ht s hs) (halpha s hs)
      (hV s hs) (hW s hs) (hDV s hs) (hDW s hs)
