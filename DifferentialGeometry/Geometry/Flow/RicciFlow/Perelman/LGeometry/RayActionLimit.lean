...
/-!
# Continuity of regularized L-ray action

This module proves the compact-interval continuity input used when minimizing
initial tangents converge.  All moving-fiber quantities are evaluated to real
scalars before integration.
-/
...
/-- On the positive regularized domain, L-ray action is stable under joint
convergence of the initial tangent and terminal square-root time. -/
theorem lRayAct_tendsto
  ...
  have hQcompact : IsCompact Q := by
    simpa only [Q] using hZs.isCompact_insert_range
  ...
  have hfixed : Tendsto
    ...
    · exact ae_of_all _ fun s hs ↦ by
        ...
        simpa only [F] using hAt.tendsto.comp hpair
