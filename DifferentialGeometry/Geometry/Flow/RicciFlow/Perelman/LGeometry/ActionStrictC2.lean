...
/-!
# Piecewise C2 regularity of strict finite L-action realizations

Every chart representative in a strict finite realization of a global
regularized L-action minimizer is `C²` on its closed parameter interval.
-/
...
/-- On the interior of a strict realized piece, the attained manifold curve is
`C²`; this is the manifold-valued form of `lStrict_piece_c2`. -/
theorem lStrict_piece_c2_at
  ...
  have hshift : ContDiffAt Real 2
    ...
    simpa only [r] using hur.comp s (contDiffAt_id.sub contDiffAt_const)
