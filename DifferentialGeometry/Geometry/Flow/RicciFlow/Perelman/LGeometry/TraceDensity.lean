...
/-!
# Pointwise trace of the regularized L-index density

This file contracts the regularized L-index density over a pointwise
orthonormal adapted family.  It is the purely spatial trace step before any
scalar-evolution or Hamilton-H rewrite.
-/
...
private theorem sum_rm04_diag
  ...
  have hRm := (metricCurvData (I := I) (M := M) g).h_rm04 Esec Asec Asec Esec x
...
/-- The pointwise trace of the regularized L-index density along an adapted
orthonormal family is a purely scalar spatial contraction. -/
theorem lIndexInt_trace
  ...
  have hRm :
    ...
    simpa only [g, t, x, A, SolutionFamily.rm04, SolutionOn.ricciAt,
      SolutionFamily.ricciAt] using
  ...
  have hHess :
    ...
    simpa only [g, t, x, SolutionFamily.connection] using
      (sum_hessian_diag (I := I) g x (S.scalar t)
  ...
  have hFirst :
      ∑ i, dRic (vec3 A (P i s) (P i s)) =
        nablaScalar (I := I) g
          (ContMDiffSection.exists_eq_at_gen
            (I := I) (F := E) (V := TangentSpace I) (n := (⊤ : ℕ∞)) x A).choose x := by
    simpa only [dRic, g, t, x, A, SolutionFamily.connection,
      SolutionFamily.ricci] using
      (sum_nablaRicci_first (I := I) g x A (fun i ↦ P i s) hON)
  have hDiv :
