...
/-!
# Parameter continuity of compact L-cost

This module begins the joint terminal-time/base-point continuity layer needed
for compact-slab reduced-volume bounds.  The first producer proves continuity
of the regularized action of one fixed smooth competitor as the terminal
forward time varies.
-/
...
/-- For a fixed smooth curve, the regularized Lagrangian is jointly continuous
in terminal forward time and square-root backward time on the regular region. -/
theorem lRegLag_time_cont
  ...
  have hkin : Continuous (fun q : P ↦
    ...
    simpa [timeLift, velLift, DifferentialGeometry.metricTimeBundleQuad,
      Tensor0SBundle.metricTensorField_apply] using hkin0
  ...
  simpa only [P, U, lRegLag] using hlag

/-- Regularity of a compact square-root-time segment persists when the
terminal forward time is perturbed. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [NeZero (Module.finrank Real E)] [I.Boundaryless] [CompactSpace M] in
/-- The regularized action of one fixed global `C¹` competitor is continuous
as the terminal forward time varies through a compact regular slab. -/
...
/-- Along convergent terminal forward times, every strict upper bound for the
action of a fixed positive-time competitor eventually bounds the L-cost. -/
