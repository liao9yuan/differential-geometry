...
/-!
# Lower semicontinuity of the regularized L-action

This file combines the fixed-chart realization of the manifold kinetic energy
with weak lower semicontinuity of the corresponding chart Gram quadratic form
and continuity of the scalar-curvature potential.
-/
...
/-- On a fixed chart interval, the kinetic energy of a weak limit is bounded
above by the liminf of the manifold kinetic energies of its approximants. -/
theorem lKinetic_liminf
  ...
  have hseq :
    ...
    funext n
    simpa only [ContinuousLinearMap.smul_apply, real_inner_smul_left] using
...
L-action of its manifold curve. -/
theorem lRegAction_chart
  ...
  have hLag (i : Fin m) : IntervalIntegrable (lRegLag S T gamma) volume
    ...
    simpa only [lRegLag] using (hkinInt i).add (hpotInt i)
    ...
    calc
      ...
      _ = lRegAction S T gamma (tNat 0) (tNat m) :=
        lRegAction_sum S T gamma (fun k hk ↦ by
          have hk' : k < m := hk
          simpa only [tNat, dif_pos (Nat.lt_trans hk' (Nat.lt_succ_self m)),
            dif_pos (Nat.succ_lt_succ hk')] using hLag ⟨k, hk'⟩)
...
theorem lRegAction_fin_lsc
  ...
  have hLag (i : Fin m) (n : Nat) : IntervalIntegrable
    ...
    simpa only [lRegLag] using (hkinInt i n).add (hpotInt i n)
    ...
    calc
      ...
      _ = lRegAction S T (alpha n) (tNat 0) (tNat m) :=
        lRegAction_sum S T (alpha n) (fun k hk ↦ by
          have hk' : k < m := hk
          simpa only [tNat, dif_pos (Nat.lt_trans hk' (Nat.lt_succ_self m)),
            dif_pos (Nat.succ_lt_succ hk')] using hLag ⟨k, hk'⟩ n)
