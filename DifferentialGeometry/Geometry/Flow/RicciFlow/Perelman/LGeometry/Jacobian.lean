...
/-!
# Moving L-Jacobian density

This file supplies the finite Gram-matrix calculus for a family of tangent
fields measured by the backward Ricci-flow metric.  It is the scalar bridge
between L-Jacobi fields and the Jacobian of the fixed-time L-exponential map.
-/
...
/-- Gram matrix of a finite tangent family in the backward-time metric. -/
...
/-- Derivative matrix for `lGram`, including the backward Ricci-flow metric
variation. -/
...
/-- Riemannian density of a finite tangent family in the backward-time
metric. -/
...
/-- Every entry of the moving L-Gram matrix has the expected covariant-plus-
Ricci derivative. -/
...
/-- The moving L-Jacobian density satisfies Jacobi's determinant formula at
every point where its Gram determinant is positive. -/
...
omit [NeZero (Module.finrank Real E)]
  ...
  [SigmaCompactSpace M] in
/-- A linearly independent tangent family has positive moving L-Gram
determinant. -/
...
omit [NeZero (Module.finrank Real E)]
  [FiniteDimensional Real E] [I.Boundaryless] [T2Space M]
  [SigmaCompactSpace M] in
/-- A linearly independent tangent family has positive moving L-Jacobian
density. -/
...
/-- Jacobi field obtained by differentiating the fixed-time L-exponential map
in one initial-tangent direction. -/
...
/-- Square-root-time covariant velocity of an L-exponential Jacobi field. -/
...
/-- Raw-time covariant velocity of an L-exponential Jacobi field. -/
...
/-- Covariant velocity after reading the raw L-exponential Jacobi field in
square-root time. -/
...
/-- On positive time, the regularized Jacobi velocity and the square-time raw
Jacobi velocity have the same endpoint pairing. -/
theorem lRegSq_pair
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The square-time raw Jacobi velocity has pairing equal to
`2 * sqrt tau` times the raw-time pairing. -/
theorem lExpSq_pair
    (S : SolutionOn (I := I) (M := M) D)
  ...
  simpa only [lExpSqVel, lExpFieldVel, lJacobiVel, gamma, Y] using
    sqVel_pair (I := I) (S.base.metric (T - tau)) gamma Y tau
      ((mem_lExpPosDom S T x Z tau).1 hpos).1 hJac.1 hJac.2.1 (Q : E)

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Pairing the square-root-time derivative of an L-exponential Jacobi field
with an endpoint vector gives `2 * sqrt tau` times the raw-time pairing. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- On the strict injectivity domain, the spatial reduced-length Hessian on
two L-exponential directions is the endpoint pairing with the raw Jacobi
velocity. -/
theorem redHess_lJac
  ...
  have hInv : mfderiv I 𝓘(Real, E) hloc.localInverse y YV = V := by
    have hleft :=
      (hloc.mfderivToContinuousLinearEquiv (by simp)).left_inv V
    change mfderiv I 𝓘(Real, E) hloc.localInverse y
      (mfderiv 𝓘(Real, E) I endMap Z V) = V at hleft
    simpa only [YV, lExpField, endMap] using hleft
  have hcostBranch :
...
/-- Moving target Gram matrix of the differential of the L-exponential map,
computed in the model-space basis. -/
...
/-- Derivative matrix of `lExpGram`, including the backward Ricci-flow metric
variation. -/
...
/-- Matrix of the spatial reduced-length Hessian in the image of the model
basis under `d lExp`. -/
...
/-- Matrix of the target Ricci tensor in the image of the model basis under
`d lExp`. -/
...
/-- Matrix pairing the raw Jacobi velocities with the image of the model
basis under `d lExp`. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- On the strict injectivity domain, the raw Jacobi-velocity pairing matrix
is the reduced-length Hessian matrix. -/
...
omit [NeZero (Module.finrank Real E)] [CompactSpace M] in
/-- The moving L-exponential Gram derivative is the sum of the Jacobi-
velocity matrix, its transpose, and twice the Ricci matrix. -/
...
/-- Target volume density of the differential of the L-exponential map,
relative to `modelHaar` on the initial tangent model. -/
noncomputable def lExpDensity
    (S : SolutionOn (I := I) (M := M) D) (T : Real) (x : M)
    (Z : TangentSpace I x) (tau : Real) : Real :=
  Real.sqrt (lExpGram S T x Z tau).det

/-- Gram matrix of the model-space basis in the source metric at time `T`. -/
...
/-- Source Riemannian density of `modelHaar` in the metric at time `T`. -/
noncomputable def lSrcDensity
    (S : SolutionOn (I := I) (M := M) D) (T : Real) (x : M) : Real :=
  Real.sqrt (lSrcGram S T x).det

/-- Intrinsic Jacobian of the L-exponential map from the source metric at time
`T` to the target metric at time `T - tau`. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The target Gram determinant of `d lExp` is positive on the strict
injectivity domain. -/
theorem lExpGram_pos
  ...
  let L := mfderiv 𝓘(Real, E) I f Z
  have hinj : Function.Injective L :=
    ((lInj_local (E := E) (I := I) S hS T x tau htau hZ).mfderivToContinuousLinearEquiv
      (by simp)).injective
  ...
  have hLI : LinearIndependent Real
    ...
    simpa only [Function.comp_def] using
      (chartModelBasis E).linearIndependent.map' L.toLinearMap hker
  simpa only [lExpGram, lExpField, f, L] using
    lGram_det_pos S T (fun q => lExp S T x Z q)
      (fun i q => lExpField S T x Z ((chartModelBasis E) i) q) tau hLI

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Contracting the reduced-length Hessian matrix with the inverse target
Gram matrix gives the spatial reduced-length Laplacian. -/
theorem lExpTrace_hess
  ...
  have htrace' :
    ...
    simpa only [hGram, lExpHess, Matrix.of_apply, hbasis,
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Contracting the target Ricci matrix with the inverse target Gram matrix
gives the scalar curvature at the L-exponential endpoint. -/
theorem lExpTrace_ricci
  ...
  have htrace' :
    ...
    simpa only [hGram, lExpRicci, Matrix.of_apply, hbasis, g, y] using htrace
  have hscalar := S.scalar_eq_metricTrace (I := I) (T - tau) y
  exact htrace'.trans (by simpa only [g, y] using hscalar.symm)

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Half the logarithmic target-density rate of the L-exponential map is the
spatial reduced-length Laplacian plus scalar curvature. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The target L-exponential density is positive on the strict injectivity
domain. -/
...
omit [NeZero (Module.finrank Real E)] [I.Boundaryless] [T2Space M]
  [CompactSpace M] in
/-- The fixed source density is positive. -/
  ...
  have hdet : 0 <
    ...
    DifferentialGeometry.Geometry.Riemannian.Variation.curveGram_det_pos
      (I := I) (S.base.metric T) gamma V 0
        (by simpa only [V] using (chartModelBasis E).linearIndependent)
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The intrinsic L-exponential Jacobian is positive on the strict injectivity
domain. -/
theorem lExpJac_pos
    (S : SolutionOn (I := I) (M := M) D)
    (hS : IsSolutionOn (I := I) S) (T : Real) (x : M)
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The target L-exponential density satisfies the moving determinant
derivative formula on the strict injectivity domain. -/
theorem lExpDen_hasDeriv
  ...
  simpa only [lExpDensity, lExpGram, lExpGramDeriv, gamma, Y] using hout

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The intrinsic L-exponential Jacobian has logarithmic rate equal to the
spatial reduced-length Laplacian plus scalar curvature. -/
theorem lExpJac_hasDeriv
  ...
  simpa only [lExpJac, mul_div_assoc] using hout

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The logarithm of the intrinsic L-exponential Jacobian has derivative
equal to the spatial reduced-length Laplacian plus scalar curvature. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Along a strict minimizing L-ray, the logarithmic L-exponential Jacobian
rate satisfies the Hamilton-`K` comparison. -/
