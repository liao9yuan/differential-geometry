...
/-!
# Perelman's reduced length

This module normalizes the fixed-time L-cost and records its first almost-
everywhere regularity consequence on compact positive regular time slices.
-/
...
/-- On the strict minimizing domain, the fixed-time L-cost agrees near the
endpoint with the action branch supplied by the local inverse of L-exp. -/
theorem lCost_eq_branch
  ...
  have hlen :
    ...
    simpa only [lExp, sqrtReparam] using
      lLength_sqrt (I := I) S T (lRegCurve S T x W) tau htau.le
...
/-- Near a strict minimizing L-ray endpoint, the fixed-time L-cost is smooth
...
/-- At a strict minimizing L-ray and positive backward time, the fixed-time
L-cost has cotangent derivative given by metric lowering of the terminal ray
velocity. -/
...
/-- At a strict minimizing L-ray, the spatial L-cost gradient is the terminal
velocity of the corresponding regularized ray. -/
theorem lCost_grad
    (S : SolutionOn (I := I) (M := M) D) (hS : IsSolutionOn (I := I) S)
    (T : Real) (x : M) {Z : TangentSpace I x} {tau : Real}
    (htau : 0 < tau) (hZ : Z ∈ lInjDomain S T x tau) :
  ...
  have hderiv := (lCost_hasMFD S hS T x htau hZ).mfderiv
  rw [hderiv]
  ext V
  rfl

/-- The fixed-time L-cost Hessian is bounded by twice the regularized index
of every smooth field with the prescribed endpoint value. -/
theorem lCost_hess_le
  ...
  have hJb : J b = V := by
    change lRegJacobiField S T x Z
      (mfderiv I (modelWithCornersSelf Real E) hloc.localInverse y V)
        (Real.sqrt tau) = V
    rw [← lExpJacobi_eq]
    simpa only [y] using
  ...
  have hQb : Q b = 0 := by
    change W b - J b = 0
    rw [show W b = V by simpa only [b] using hWb, hJb, sub_self]
  ...
  have hWg8 : ContMDiff (modelWithCornersSelf Real Real) I.tangent (8 : Nat)
    ...
    rw [← contMDiffOn_univ]
    simpa only [gamma, Wg, alpha] using hW.comp
  ...
  have hgeo : IsLRegCurveOn S T gamma (Set.uIcc (0 : Real) b) x Z := by
    ...
    · have hvel : lVelocity (I := I) gamma 0 =
        ...
        simpa only [lVelocity] using congrArg
          (fun L : Real →L[Real] E ↦ L 1)
          (h0germ.mfderiv_eq
...
/-- Perelman's reduced length is the fixed-time L-cost normalized by twice the
square root of backward time. -/
def redLength
    (S : SolutionOn (I := I) (M := M) D) (T : Real) (x y : M)
    (tau : Real) : Real :=
  lCost S T x y tau / (2 * Real.sqrt tau)

/-- Near a strict minimizing L-ray endpoint, reduced length is smooth on an
open neighborhood. -/
...
/-- At a strict minimizing L-ray, the spatial reduced-length Hessian is
bounded by the normalized regularized index of every smooth field with the
prescribed endpoint value. -/
...
/-- At a strict minimizing L-ray and positive backward time, reduced length
has the normalized terminal-velocity cotangent derivative. -/
...
/-- At a strict minimizing L-ray, the spatial reduced-length gradient is the
terminal regularized-ray velocity normalized by twice square-root time. -/
...
/-- At a strict minimizing L-ray, the spatial reduced-length gradient is the
ordinary backward-time velocity of that L-exponential ray. -/
...
/-- At a strict minimizing L-ray, varying backward time while holding the
terminal endpoint fixed differentiates L-cost by the Hamilton--Jacobi term. -/
theorem lCost_hasDeriv
  ...
  have hp0 : p tau = (Z, tau) := by
    simpa only [p, q, y, F, f, hloc] using
      hloc.localInverse_left_inv hloc.localInverse_mem_target
  let b : Real := Real.sqrt tau
  ...
  have hJoint : HasFDerivAt A L (Z, tau) := by
    simpa only [A, L, Lz, c, b, endMap, y] using
      lRayAct_joint S hS T x Z hdom
  have hq : HasMFDerivAt (modelWithCornersSelf Real Real) K q tau
      ((0 : Real →L[Real] E).prod (1 : Real →L[Real] Real)) := by
    simpa only [q, K] using
      (hasMFDerivAt_const (c := y) (x := tau)).prodMk
        (hasMFDerivAt_id tau)
  ...
  have hpM : HasMFDerivAt (modelWithCornersSelf Real Real) J p tau
      ((mfderiv K J hloc.localInverse (q tau)).comp
        ((0 : Real →L[Real] E).prod (1 : Real →L[Real] Real))) := by
    simpa only [p, Function.comp_apply] using
      hInv.hasMFDerivAt.comp tau hq
  ...
  have hbranch : HasDerivAt (fun r : Real ↦ A (p r)) (L v) tau := by
    have hcomp := hJointP.comp tau hpM
    have hraw := hcomp.hasFDerivAt.hasDerivAt
    simpa only [Function.comp_apply, v, eT,
      ContinuousLinearMap.comp_apply, ContinuousLinearMap.prod_apply,
      ContinuousLinearMap.zero_apply, ContinuousLinearMap.id_apply] using hraw
  have hright : mfderiv J K F (Z, tau) v = eT := by
    have hright' :=
      (hloc.mfderivToContinuousLinearEquiv (by simp)).right_inv eT
    simpa only [v, p, q, y, F, f, hp0, hloc] using hright'
  ...
  have hsndDeriv :
      mfderiv J (modelWithCornersSelf Real Real) (@Prod.snd E Real)
          (Z, tau) = ContinuousLinearMap.snd Real E Real := by
    simpa only [J] using
      (mfderiv_snd : mfderiv
        ((modelWithCornersSelf Real E).prod
          (modelWithCornersSelf Real Real))
  ...
  have hEq : (fun r : Real ↦ A (p r)) =ᶠ[nhds tau]
    ...
    calc
      lRegAction S T (lRegCurve S T x (p r).1) 0 (Real.sqrt r) =
        ...
        simpa only [lExp, sqrtReparam] using
          (lLength_sqrt (I := I) S T (lRegCurve S T x (p r).1)
            r hrpos.le).symm
...
/-- At a strict minimizing L-ray, the fixed-endpoint backward-time derivative
of reduced length satisfies Perelman's Hamilton--Jacobi identity. -/
...
/-- On the strict minimizing region, reduced length satisfies the pointwise
Hamilton--Jacobi identity in backward time. -/
...
omit [NeZero (Module.finrank Real E)] [I.Boundaryless] [T2Space M]
  [CompactSpace M] in
/-- At positive backward time, undoing the reduced-length normalization
recovers the L-cost. -/
...
omit [NeZero (Module.finrank Real E)] [I.Boundaryless] [T2Space M]
  [CompactSpace M] in
/-- At positive backward time, reduced length vanishes exactly when L-cost
vanishes. -/
...
/-- On a compact positive regular backward-time slice, reduced length is
manifold-differentiable outside a Riemannian-volume null set. -/
theorem redLength_diff_ae
  ...
  have hcomp : LipschitzOnWith (‖c‖₊ * K)
    ...
    simpa only [Function.comp_apply] using
      (lipschitzWith_smul c).lipschitzOnWith.comp hLip (mapsTo_univ _ _)
  have hfun :
      (fun q : E ↦ c • (((fun y : M ↦ lCost S T x y tau) ∘
