...
/-!
# Regularized L-phase solutions at arbitrary base times

The autonomized phase field gives a local solution through any regular
square-root time and any chart-interior phase state.
-/
...
private theorem clock_eq_at
  ...
  have hconst : ∀ s ∈ Ioo a b,
      HasDerivAt (fun q ↦ phi q - q) (0 : Real) s := by
    intro s hs
    simpa using (hphi s hs).sub (hasDerivAt_id s)
  intro s hs
  have hkey : (fun q ↦ phi q - q) s =
      (fun q ↦ phi q - q) s0 := by
    apply Convex.is_const_of_fderivWithin_eq_zero (𝕜 := Real)
      (convex_Ioo a b) (f := fun q ↦ phi q - q) (s := Ioo a b)
    · intro q hq
    ...
    · intro q hq
      ...
      have hfd : HasFDerivWithinAt (fun r ↦ phi r - r)
...
/-- The regularized L-phase ODE has a local solution through a prescribed
chart-interior state at any regular square-root base time. -/
theorem exists_lPhaseSol_at
  ...
  have hvf : ContDiffAt Real 1 vf (s0, z0) :=
  ...
  · intro s hs
    ...
    have hcderiv :
      ...
      simpa only [DifferentialGeometry.Analysis.ODE.autonomizedFlowVF, X, vf] using
        hcurveU s hsU
