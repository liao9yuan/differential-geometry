...
/-!
# Fixed-chart regularity of L-action minimizers

The genuine nonlinear weak Euler identity gives an integrable position force.
Momentum regularity and inversion of the chart Gram operator then upgrade a
fixed-chart time-`H¹` minimizer to `C¹` on its closed parameter interval.
-/
...
/-- A fixed-chart local minimizer of the genuine regularized L-action has a
continuous coordinate-velocity representative and is `C¹` on the closed time
interval, with that representative equal to its derivative within the interval. -/
theorem lChart_min_c1
  ...
  have hF : IntegrableOn F (Icc (0 : Real) L) volume := by
    simpa only [F, Pi.smul_apply] using
      Integrable.smul (2 : Real) hForce
