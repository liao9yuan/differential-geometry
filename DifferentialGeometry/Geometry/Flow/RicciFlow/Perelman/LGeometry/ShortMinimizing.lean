...
/-!
# Short-time minimizing-ray compactness

This module supplies the initial-vector compactness estimate needed when the
square-root terminal times shrink to zero.  The fixed-positive-lower-bound
estimate in `MinVecVar` does not apply in this regime.
-/
...
private theorem rayInts_shrink
  ...
  · simpa only [alpha, lRegLag, lRegSpeedSq] using
      (hkin.const_mul (1 / 2 : Real)).add hpot

omit [InnerProductSpace Real E] in
/-- Initial tangents stay bounded when their regularized L-rays live on
shrinking positive intervals and have action bounded linearly in the interval
length. -/
theorem lRegInit_shrink
  ...
  have hmetric : ∀ n,
    ...
    have hkin :
      ...
      have hraw := lRegKinetic_le (I := I) S T alpha 0 b (A * b) Cp hb.le
        (fun s hs ↦ hpot s (by
          simpa only [Set.uIcc_of_le hR] using
            (⟨hs.1, hs.2.trans hbR⟩ : s ∈ Set.Icc (0 : Real) R)) (alpha s))
  ...
  have hnorm : ∀ n, ‖Z n‖ ≤ L := by
    ...
    have hcoerc : a * ‖Z n‖ ^ 2 ≤ g.inner x (Z n) (Z n) := by
      simpa only [a] using gpCoerciveConst_le (I := I) g x (Z n)
