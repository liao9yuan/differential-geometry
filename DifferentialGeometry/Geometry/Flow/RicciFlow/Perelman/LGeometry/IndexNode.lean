...
/-! # Regularized L-index forms across a moving node -/
...
private theorem indexGreen_var
  ...
  have hYall : ContMDiff (modelWithCornersSelf Real Real) I.tangent (7 : Nat)
    ...
    simpa only [alpha, Y] using hYjoint.comp
      (contMDiff_const.prodMk contMDiff_id)
...
private theorem nodeAction_second
  ...
  have hderivEq (u : Real) : deriv L u = P0 u - P1 u + E0 u + E1 u := by
    ...
    rw [hYa] at hfirst0
    rw [hYb] at hfirst1
    simp only [map_zero, ContinuousLinearMap.zero_apply, sub_zero,
      zero_sub] at hfirst0 hfirst1
    rw [hF00] at hfirst0
    rw [hF10] at hfirst1
  ...
  have hpairs : HasDerivAt (fun u : Real ↦ P0 u - P1 u)
    ...
    have hraw' : HasDerivAt (fun u : Real ↦ P0 u - P1 u)
        (((S.base.metric (T - c ^ 2)).inner (f0 0 c)
              (covDerivAlong (I := I) (S.base.metric (T - c ^ 2))
                (fun u : Real ↦ f0 u c)
                (fun u : Real ↦
                  lVelocity (I := I) (fun v : Real ↦ f0 v c) u) 0)
              (lVelocity (I := I) (f0 0) c) +
            (S.base.metric (T - c ^ 2)).inner (f0 0 c)
              (lVelocity (I := I) (fun v : Real ↦ f0 v c) 0)
              (covDerivAlong (I := I) (S.base.metric (T - c ^ 2))
                (f0 0) (fun s : Real ↦
                  lVelocity (I := I) (fun u : Real ↦ f0 u s) 0) c)) -
          ((S.base.metric (T - c ^ 2)).inner (f1 0 c)
              (covDerivAlong (I := I) (S.base.metric (T - c ^ 2))
                (fun u : Real ↦ f1 u c)
                (fun u : Real ↦
                  lVelocity (I := I) (fun v : Real ↦ f1 v c) u) 0)
              (lVelocity (I := I) (f1 0) c) +
            (S.base.metric (T - c ^ 2)).inner (f1 0 c)
              (lVelocity (I := I) (fun v : Real ↦ f1 v c) 0)
              (covDerivAlong (I := I) (S.base.metric (T - c ^ 2))
                (f1 0) (fun s : Real ↦
                  lVelocity (I := I) (fun u : Real ↦ f1 u s) 0) c))) 0 := by
      simpa only [P0, P1] using hraw
...
omit [CompactSpace M] in
/-- Two matching smooth branches along a global fixed-endpoint minimizer have
nonnegative total regularized L-index across their common interior node. -/
theorem lIndex_sum_nonneg
  ...
  have hgeo0 : IsLRegCurveOn S T (f0 0) (uIcc a c) x Z := by
    ...
    simpa only [uIcc_of_le hac.le, uIcc_of_le hab.le] using
      (⟨hs'.1, hs'.2.trans hcb.le⟩ : s ∈ Icc a b)
  have hgeo1 : IsLRegCurveOn S T (f1 0) (uIcc c b) x Z := by
    rw [hcenter1]
    refine ⟨hgeo.1, hgeo.2.1, fun s hs ↦ hgeo.2.2 s ?_⟩
    have hs' : s ∈ Icc c b := by
      simpa only [uIcc_of_le hcb.le] using hs
    simpa only [uIcc_of_le hcb.le, uIcc_of_le hab.le] using
      (⟨hac.le.trans hs'.1, hs'.2⟩ : s ∈ Icc a b)
  ...
  have hEq0 : EqOn
    ...
    simpa only [uIcc_of_le hab.le] using
      (⟨hs'.1.le, (hs'.2.trans hcb).le⟩ : s ∈ Icc a b)
  have hEq1 : EqOn
    ...
    have hs' : s ∈ Ioo c b := by
      simpa only [uIoo_of_le hcb.le] using hs
    simpa only [uIcc_of_le hab.le] using
      (⟨(hac.trans hs'.1).le, hs'.2.le⟩ : s ∈ Icc a b)
