...
/-!
# Positive-start families spanning both sides

This file continues a supplied positive-start regularized L-family while
preserving its whole original time domain.  Applying that continuation after
first reaching time zero produces one family on a connected domain containing
zero, the positive start, and a prescribed terminal time.
-/
...
/-- A supplied positive-start L-family continues to a prescribed time while
retaining its entire original connected time domain. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] in
/-- One positive-start L-family may be chosen on a connected open time domain
containing zero, its positive start, and any prescribed terminal time. -/
