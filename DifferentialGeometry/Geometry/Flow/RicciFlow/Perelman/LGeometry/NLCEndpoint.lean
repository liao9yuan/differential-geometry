...
/-!
# Endpoint localization for regularized L-rays

This module converts a fixed terminal-metric energy budget for a regularized
L-ray into a quantitative endpoint-distance estimate and membership in a flow
metric ball.  Curvature control enters upstream, through the production of the
energy budget; containment is a conclusion here rather than a hypothesis.
-/
...
/-- On a compact regular spacetime slab, the scalar-gradient pairing admits a
uniform inverse-cubic bound at every radius below a fixed scale. -/
...
/-- Curvature control on a flow metric ball supplies the Ricci quadratic bound
needed in the regularized L-speed inequality, at every point still in the
controlled parabolic ball. -/
theorem lRegRicci_le
  ...
  have hquad := ricci_quad_sol (I := I) S (alpha s)
    (lVelocity (I := I) alpha s) hcurv
  rw [← metricRicciAt_apply_eq_ricciTensor] at hquad
  simpa only [lRegSpeedSq] using hquad

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)] in
/-- On a compact regular slab, a regularized L-ray has a quantitative speed
bound up to its first exit from a curvature-controlled flow metric ball. -/
...
private theorem lRegSpeed_fixed
  ...
  have hgr := lRegSpeed_two (I := J) S hS (time : Real) halpha
    ...
    (fun q hq ↦ by
      have hqI := hsub hq
      simpa only [a] using hgrad B.radius B.radius_pos hBrho
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- On a compact regular slab, sufficiently short parabolic-scale L-rays have
a speed bound uniform in the radius as long as they have not left the
curvature-controlled flow metric ball. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- On a compact regular slab, the terminal metric and every metric in the
slab are uniformly comparable by one internally produced constant. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- On a fixed compact regular slab, moving metrics on every shorter
parabolic-scale interval are uniformly close to the terminal metric, with an
explicit exponent that is uniform in the radius. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- A terminal-metric energy budget controls the endpoint displacement of a
regularized L-ray. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- If the terminal-metric energy radius of a regularized L-ray is strictly
smaller than a flow metric ball's radius, its endpoint lies in that ball. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- On a compact regular slab, every sufficiently short parabolic-scale L-ray
with controlled terminal source stays in the terminal half-ball and in the
corresponding curvature-controlled moving ball, uniformly over smaller radii. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- On a compact regular slab, every sufficiently short parabolic-scale
L-exponential endpoint with controlled terminal source lies in its terminal
flow metric ball, uniformly over smaller radii. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- On a compact smooth flow, a bounded ball for the exact terminal source
metric is mapped into any terminal flow-metric ball at all sufficiently small
square-root times. -/
theorem lReg_small_ball
  ...
  have hUb : (Z, b) ∈ U := by
    ...
    calc
      ...
      _ < eps := hb
  simpa only [U, curve, Set.mem_inter_iff, Set.mem_preimage] using hUb

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The L-exponential sends an exact terminal-source-metric ball into a fixed
terminal flow-metric ball for all sufficiently small backward times. -/
