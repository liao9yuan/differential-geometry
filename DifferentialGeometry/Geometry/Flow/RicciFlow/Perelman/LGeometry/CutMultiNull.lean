...
/-!
# Nullity of the multiple-ray L-cut image

This module combines chart-local Rademacher theory with the two-ray
nondifferentiability criterion to prove that the multiple-minimizer part, and
hence the whole fixed-time L-cut image, has zero Riemannian volume.
-/
...
/-- The multiple-minimizer part of the fixed-time L-cut image has zero
Riemannian volume. -/
...
/-- The fixed-time L-cut image has zero Riemannian volume. -/
theorem lCut_null
    (S : SolutionOn (I := I) (M := M) D) (hS : IsSolutionOn (I := I) S)
    (T : Real) (x : M) (tau : Real) (g : SmoothRiemannianMetric I M) :
    riemannianVolumeMeasure (I := I) (M := M) g
