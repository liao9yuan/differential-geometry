...
/-!
# Moving-metric calculus for Perelman L-geometry

This file combines the Ricci-flow time derivative of the metric with the
fixed-metric covariant product rule along a curve.  The result is stated in
backward time, so the metric-evolution contribution has sign `+2 Ric`.
-/
...
/-- Backward-time derivative of a moving metric pairing, under the pointwise
chart regularity used by the fixed-metric covariant product rule. -/
theorem lInner_deriv_chart
  ...
  have hentry : ∀ i j : Fin (Module.finrank Real E),
    ...
    have hjoint : ContMDiffAt (𝓘(Real, Real).prod 𝓘(Real, E)) 𝓘(Real, Real)
        ...
        (T - tau0, u tau0) := by
      simpa only [Function.comp_apply] using hjoint0
  ...
  have hpair : DifferentiableAt Real
    ...
    have hup : DifferentiableAt Real (fun p : Real × Real => u p.2)
      ...
      simpa only [Function.comp_apply] using
        hgamma_chart.comp (tau0, tau0) hsnd
  ...
  have hQdiff : DifferentiableAt Real Q (tau0, tau0) := by
    ...
    have hWj := (chartCoordCLM (E := E) j).differentiableAt.comp
      (tau0, tau0) hWpair
    simpa only [chartCoordCLM_apply] using (hG.mul hVi).mul hWj
  ...
  have hsub : HasDerivAt (fun tau : Real => T - tau) (-1) tau0 := by
    simpa using
      (hasDerivAt_const (x := tau0) (c := T)).sub (hasDerivAt_id (x := tau0))
  ...
  have htimeQ : HasDerivAt (fun s : Real => Q (s, tau0))
    ...
    have htime' : HasDerivAt
        ...
        (2 * S.ricciAt (T - tau0) (gamma tau0)
          (DifferentialGeometry.Geometry.Curvature.vec2 (V tau0) (W tau0))) tau0 := by
      simpa only [Function.comp_apply, mul_neg, neg_mul, neg_neg, mul_one] using htime
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)] [SigmaCompactSpace M] in
/-- Backward-time moving-metric product rule along a curve differentiable at
the evaluation time. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Moving-metric product rule after the square-root regularization of backward
time. -/
theorem lRegInner_deriv
  ...
  have hmetric : MDifferentiableAt
    ...
    simpa only [SolutionOn.family_metric] using
      (hmetric₀.mdifferentiableAt (by simp)).comp (s, s) harg
  ...
  have htimeF : HasDerivAt (fun r : Real => F (r, s))
    ...
    simpa only [F, SolutionOn.family_metric] using htime.congr_deriv (by ring)
