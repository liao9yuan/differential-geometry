...
/-!
# Weak upper barriers for reduced length

This file assembles the positive-start Calabi branch into an all-point,
product-local spacetime upper support for reduced length on complete
bounded-curvature flows.
-/
...
/-- The points where two maps agree as germs form an open set. -/
...
/-- Scalar cutoff selection for the positive-start Hamilton-tail error. -/
private theorem exists_tail_cut
  ...
  have herr : Tendsto err (𝓝[>] (0 : Real)) (nhds 0) := by
    ...
    simpa only [Function.comp_apply, err, hlim0] using hraw
...
/-- Every point of a complete bounded-curvature backward slab admits a
product-local spacetime upper support for reduced length satisfying Perelman's
backward weak differential inequality up to an arbitrary positive error. -/
theorem exists_redWeak_sup [ConnectedSpace M]
  ...
  have hOmegaSeg : Icc (0 : Real) b ⊆ Omega := by
    intro s hs
    exact ⟨by simpa only [beta] using hcenter s hs, hOmega0seg hs⟩
  ...
  have hNmem : (y, tau) ∈ N := by
    ...
    simpa only [rawMap, b] using hgood
  ...
  have hsupport : ∀ z ∈ U, ∀ rho ∈ J,
    ...
    have hpVK : p ∈ V ×ˢ Ktime := by
      simpa only [rawMap, c, p] using hqgood.2
  ...
  let Lq : Real →L[Real] E × Real :=
    (0 : Real →L[Real] E).prod (1 : Real →L[Real] Real)
  ...
  have hline : HasMFDerivAt 𝓘(Real, Real)
    ...
    have hid : HasMFDerivAt 𝓘(Real, Real) 𝓘(Real, Real)
        (fun r : Real ↦ r) b (1 : Real →L[Real] Real) := by
      simpa only [id_eq] using
        (hasMFDerivAt_id (I := 𝓘(Real, Real)) b)
    simpa only [Lq] using
      (hasMFDerivAt_const (c := y) (x := b)).prodMk hid
  have hLcomp : Lj.comp Lq = ctime • (1 : Real →L[Real] Real) := by
    ext
    ...
    simp only [Lj, Lq, ContinuousLinearMap.comp_apply,
      ContinuousLinearMap.add_apply, ContinuousLinearMap.prod_apply,
    ...
    calc
      ...
      _ = _ := by rw [map_zero, zero_add, mul_one]
  have hjointTime : HasDerivAt (fun r : Real ↦ joint (y, r))
    ...
    simpa only [Function.comp_apply, ContinuousLinearMap.smul_apply,
      ContinuousLinearMap.one_apply, smul_eq_mul, mul_one] using hraw
  ...
  have hden : HasDerivAt (fun rho : Real ↦ 2 * Real.sqrt rho)
      (1 / b) tau := by
    convert (Real.hasDerivAt_sqrt htau.ne').const_mul 2 using 1;
      simp only [b]; field_simp [hb.ne']
  ...
  have hKtail : lKTail S T beta a b = lKTail S T gamma a b := by
    ...
    have hvel : (lVelocity (I := I) beta s : E) =
        (lVelocity (I := I) gamma s : E) := by
      simpa only [beta, lVelocity] using congrArg
  ...
  have hlagEq : lag = lRegLag S T gamma b := by
    ...
    have hvelEq : (lVelocity (I := I) beta b : E) =
        (lVelocity (I := I) gamma b : E) := by
      simpa only [beta, lVelocity] using congrArg (fun L : Real →L[Real] E ↦ L 1)
        (heq.mfderiv_eq (I := 𝓘(Real, Real)) (I' := I))
    unfold lag
  ...
  have hvelEq : (S.base.metric (T - b ^ 2)).inner
    ...
    have heq := hcenter b ⟨hb.le, le_rfl⟩
    have hv : (lVelocity (I := I) beta b : E) =
        (lVelocity (I := I) gamma b : E) := by
      simpa only [beta, lVelocity] using congrArg (fun L : Real →L[Real] E ↦ L 1)
        (heq.mfderiv_eq (I := 𝓘(Real, Real)) (I' := I))
  ...
  have hPhiEqMem : {z : M |
    ...
    simpa only [Filter.EventuallyEq] using hPhiFixed
