...
/-!
# Upper continuity of compact L-cost

This module develops the endpoint-perturbation input needed to show that a
limit of minimizing L-rays is still minimizing.
-/
...
private theorem chart_tail_lim
  ...
  have hunif : TendstoUniformly
    ...
    · exact hsymm
    · simpa only [beta, beta0] using hc
  ...
  have hlim := lAction_h1_lim (I := I) S hMet hSc T c b t htmono
    ...
    (fun i ↦ by
      have hi : i = 0 := Fin.eq_zero i
      subst i
      simpa [lSegLen, t] using hrepLim)
    (fun _ ↦ K) (fun _ ↦ hKc) (fun _ ↦ hKchart)
    (fun i n r ↦ by
      have hi : i = 0 := Fin.eq_zero i
      subst i
      simpa [lSegLen, t] using hK n r)
    (fun i r ↦ by
      have hi : i = 0 := Fin.eq_zero i
      subst i
      simpa [lSegLen, t] using hK0 r)
    (fun i ↦ by
      have hi : i = 0 := Fin.eq_zero i
      subst i
      simpa [lSegLen, t] using hv)
    hcont hunif hreg
...
omit [NeZero (Module.finrank Real E)] in
/-- At a fixed positive backward time, a strict regularized-action upper
bound persists under convergence of the terminal endpoint. -/
theorem lCost_lt_event
  ...
  have hrep0 : EqOn u0.toFun
      ...
      (Icc (0 : Real) (b - c)) := by
    simpa only [u0, gamma, Function.comp_apply] using
      chartTimeH1_toFun I (sub_nonneg.mpr hcb.le) y gamma hgamma hgammaSrc
...
/-- The L-cost to a positive regularized-ray endpoint is no larger than the
action of that ray segment. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Along jointly convergent positive regularized L-rays, every strict upper
bound for the limiting ray action eventually bounds the varying-time L-cost. -/
  ...
  have hmem : ∀ᶠ n in atTop,
    ...
    exact hjoint.eventually
      ((lRegJointDom_open S hS T x).mem_nhds (by
        simpa only [lRegJointDom] using hdom))
