...
/-!
# The L-cut alternative

This module proves that a maximal minimizing L-ray either is conjugate at its
cut time or shares its cut endpoint with a second minimizing initial tangent.
-/
...
/-- At the last minimizing time of an L-ray, either the L-exponential map is
conjugate or a distinct minimizing initial tangent reaches the same endpoint. -/
theorem lCut_alt
  ...
  have hZdom : (Z, tau) ∈ lExpPosDom S T x :=
  ...
  have htauU : tau ∈ U := by
    simpa only [U, z, mem_preimage] using hZdom
  ...
  have hZdomN (n : Nat) : (Z, sigma n) ∈ lExpPosDom S T x := by
    have hmem : u (n + N) ∈ U := hNU (n + N) (by omega)
    simpa only [U, z, sigma, mem_preimage] using hmem
  ...
  have hWact (n : Nat) :
    ...
    have hcostEq :
      ...
      calc
        lRegAction S T (lRegCurve S T x (W n)) 0 (B n) =
            lLength S T (sqrtReparam (lRegCurve S T x (W n))) 0
              (sigma n) := by
          simpa only [B] using
  ...
  have hZExpLim : Tendsto
    ...
    simpa only [Function.comp_apply] using hZExpAt.tendsto.comp hZpair
