...
/-!
# Reduced-volume input for smooth noncollapsing

This file starts the ordinary smooth-flow no-local-collapsing argument with
the elementary lower bound obtained from a reduced-length bound on a
measurable subset of one time slice.
-/
...
/-- A reduced-length upper bound on a measurable slice set gives the
corresponding constant-density lower bound for reduced volume. -/
