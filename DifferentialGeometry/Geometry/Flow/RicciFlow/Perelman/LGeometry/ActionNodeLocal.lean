...
/-!
# Two-piece minimality at a chart node

Global minimality against fixed-endpoint `C¹` curves descends to an exact
comparison for any two chart `timeH1` pieces with the same outer endpoints
and a common inverse-chart node.
-/
...
/-- A global fixed-endpoint `C¹` comparison makes the realized two-piece
chart action minimal among all target-contained pieces with the same outer
endpoints and the same manifold node. -/
