...
/-!
# Intrinsic acceleration of fixed-chart L-action minimizers

The classical chart Euler equation for a positive fixed-chart minimizer is
translated back to the intrinsic regularized L-geodesic acceleration equation.
-/
...
/-- A positive fixed-chart local minimizer satisfies the intrinsic regularized
L-geodesic acceleration equation at every interior square-root time. -/
theorem lChart_min_accel
  ...
  have huShift : HasDerivAt (fun s => u.toFun (s - a)) (q r) (a + r) := by
    simpa only [har, one_smul] using hu'.scomp (a + r) hshift
  have hq' : HasDerivAt q
  ...
  have hqShift : HasDerivAt (fun s => q (s - a))
    ...
    simpa only [har, one_smul] using hq'.scomp (a + r) hshift
  ...
  have hvel : EqOn (fun s => lVelocity (I := I) alpha s) A
    ...
    have hzs : HasDerivAt (fun x : Real => (z x).1) (z s).2 s := by
      simpa only [z, one_smul] using hus.scomp s hsShift
    have hzchart : (z s).1 ∈ interior (extChartAt I p).target := by
      simpa only [z] using hchart hscc
    simpa only [alpha, A] using lPhase_velocity (I := I) p z s hzs hzchart
