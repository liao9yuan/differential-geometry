...
/-- The infimum of regularized L-action among global `C¹` curves with the
prescribed endpoint values on `[a, b]`. -/
def lRegCostC1
...
`C¹` actions and is no larger than the action of any `C¹` competitor. -/
...
omit [NeZero (Module.finrank Real E)] in
/-- The global `C¹` regularized cost is no larger than the action of any
fixed-endpoint global `C¹` competitor in the compact direct-method setting. -/
