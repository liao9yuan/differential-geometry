...
The endpoint-honest regularized minimizer is read in backward time through
the inverse square-root reparameterization.
-/
...
omit [NeZero (Module.finrank Real E)] [I.Boundaryless] [T2Space M]
  [CompactSpace M] in
/-- On nonnegative backward times, raw L-cost is the regularized global-C1
cost on the corresponding square-root-time interval. -/
...
/-- On a compact manifold, positive-time L-cost is attained by an
endpoint-honest regularized L-curve, and its raw square-root
reparameterization is no longer than every global regularized `C¹`
fixed-endpoint competitor. -/
