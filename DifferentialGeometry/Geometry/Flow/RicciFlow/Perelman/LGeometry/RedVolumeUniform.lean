...
/-!
# Uniform small-time lower bound for reduced volume

This module combines the pointwise zero-time limit, joint terminal-time and
basepoint lower semicontinuity at each fixed positive backward time, and
antitonicity in backward time.  Compactness of a time slab times the closed
manifold turns the pointwise lower bounds into one uniform positive-time
floor.
-/
...
private theorem redVol_local_floor
  ...
  have hpos : ∀ᶠ tau : Real in 𝓝[>] (0 : Real), 0 < tau := by
    simpa only [mem_Ioi] using
      (self_mem_nhdsWithin :
        ∀ᶠ tau : Real in 𝓝[>] (0 : Real), tau ∈ Ioi (0 : Real))
...
/-- On a compact interval of regular backward slabs, reduced volume has a
uniform strict lower bound at all sufficiently small positive backward times.
-/
