...
/-!
# Time regularity of the spatial reduced-length minimum

This module studies the free-endpoint minimum of reduced length.  Its
square-root-time normalized action is locally Lipschitz: a shorter minimizer
may be continued by a constant tail, while a longer minimizer may be
truncated.  Uniform curvature bounds control the scalar potential on the
discarded or appended interval.
-/
...
/-- The spatial infimum of reduced length at a fixed positive backward time. -/
def redMinVal (S : SolutionOn (I := I) (M := M) D)
    (T : Real) (x : M) (tau : Real) : Real :=
  sInf (range fun y : M ↦ redLength S T x y tau)

/-- The free-endpoint minimum of regularized L-action at square-root time. -/
...
/-- A spatial minimizer realizes `redMinVal`. -/
...
/-- On a connected complete bounded-curvature flow, the spatial reduced-length
minimum is reached by a minimizing L-exponential ray. -/
...
/-- A minimizing ray to a spatial reduced-length minimizer realizes
`redMinAct`. -/
theorem redMinAct_eq
  ...
  have hlen : lRegAction S T (lRegCurve S T x Z) 0 b =
    ...
    simpa only [lExp, sqrtReparam, Real.sqrt_sq hb.le] using
...
private theorem scalar_abs_rm
  ...
  simpa only [SolutionOn.scalar, SolutionFamily.scalar, SolutionFamily.rm04,
    metricRm04_apply] using hscalar.trans
...
private theorem lRegLag_const_le
  ...
  simp only [lVelocity, mfderiv_const, ContinuousLinearMap.zero_apply,
    ContinuousLinearMap.map_zero, mul_zero, zero_add]
...
/-- On every positive bounded-curvature slab, the free-endpoint minimum
regularized action is Lipschitz in square-root backward time. -/
...
/-- The spatial minimum of reduced length is continuous at every positive
backward time in a complete bounded-curvature slab. -/
