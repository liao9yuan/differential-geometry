...
/-!
# Two-piece node splices for the regularized L-action

This file realizes two chart `timeH1` pieces with a common manifold endpoint
as one continuous curve.  The existing finite-chart density theorem then
produces fixed-endpoint global `C¹` competitors with strong convergence on
both pieces and convergence of the complete regularized L-action.
-/
...
/-- Two `C¹` curve pieces meeting at a node admit a finite chart-`timeH1`
realization with that node retained as a subdivision point.  A zero-length
transition piece at the node keeps the two finite chart covers independent. -/
theorem exists_chartH1_join
  ...
  have ht00 : t0 0 = a := by
    simpa only [t0] using congrArg Subtype.val hs00
  have ht0last : t0 (Fin.last m0) = c := by
    simpa only [t0] using congrArg Subtype.val (hs0last m0 le_rfl)
  have ht10 : t1 0 = c := by
    simpa only [t1] using congrArg Subtype.val hs10
  have ht1last : t1 (Fin.last m1) = b := by
    simpa only [t1] using congrArg Subtype.val (hs1last m1 le_rfl)
  ...
  have hsrc0 (i : Fin m0) : MapsTo gamma0
    ...
    simpa only [p0, t0] using Classical.choose_spec (hp0 i)
  have hsrc1 (i : Fin m1) : MapsTo gamma1
      (Icc (t1 i.castSucc) (t1 i.succ)) (chartAt H (p1 i)).source := by
    simpa only [p1, t1] using Classical.choose_spec (hp1 i)
  ...
  have hv0 (j : Fin m0) : EqOn (v0 j).toFun
    ...
    simpa only [v0, Function.comp_apply] using
      chartTimeH1_toFun I (hlen0_nonneg j) (p0 j)
        (fun r ↦ gamma0 (t0 j.castSucc + r)) (hmd0 j) (hsrc0_shift j)
  let vMid : timeH1 E 0 :=
  ...
  have hvMid : EqOn vMid.toFun
    ...
    simpa only [vMid, Function.comp_apply] using
      chartTimeH1_toFun I (by norm_num) (gamma0 c) (fun _ ↦ gamma0 c)
        contMDiff_const.contMDiffOn (by
          intro _ _
          exact mem_chart_source H (gamma0 c))
  ...
  have hv1 (j : Fin m1) : EqOn (v1 j).toFun
    ...
    simpa only [v1, Function.comp_apply] using
      chartTimeH1_toFun I (hlen1_nonneg j) (p1 j)
        (fun r ↦ gamma1 (t1 j.castSucc + r)) (hmd1 j) (hsrc1_shift j)
  ...
  · rw [ht_right (Fin.last (m0 + 1 + m1)) (by
      simp only [Fin.val_last]
      omega)]
    simpa only [Fin.val_last, Nat.add_sub_cancel_left] using ht1last
...
theorem lNode_c1_dense
  ...
  have hgamma : Continuous gamma := by
    ...
    · intro s hs
      have hst : s = t 1 := by
        simpa only [frontier_Iic] using hs
