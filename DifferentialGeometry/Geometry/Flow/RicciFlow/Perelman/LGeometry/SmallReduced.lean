...
/-!
# Small-time reduced-Jacobian limit

This module combines the normalized L-exponential density and reduced-length
limits to identify the intrinsic pulled-back reduced Jacobian at backward time
zero.
-/
...
/-- Along any ray which is strictly minimizing at one later time, the
intrinsic pulled-back reduced Jacobian converges to the source-metric Gaussian
factor at square-root backward time zero. -/
...
/-- Along any ray which is strictly minimizing at one later time, the
intrinsic pulled-back reduced Jacobian converges to the source-metric Gaussian
factor in the original backward-time parameter. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- On the strict minimizing domain, the intrinsic pulled-back reduced
Jacobian is bounded above by its source-metric Gaussian zero-time limit. -/
