...
/-!
# Strict finite L-action realizations

Zero-length members of a finite chart-`H¹` realization are removed while the
original positive-segment indices and their dependent witnesses are retained.
-/
...
/-- A monotone finite chart-`H¹` realization compresses to a strict one while
retaining the original positive segment underlying every new piece. -/
