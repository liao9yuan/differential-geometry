...
/-!
# Uniform terminal action bounds for regularized L-rays

This module bounds the action of the terminal part of a compact family of
regularized L-rays linearly in the length of the terminal interval.
-/
...
private theorem rayLag_cont
  ...
  have hvel : Continuous velLift := by
    ...
    rw [continuousOn_iff_continuous_restrict] at hvelOn
    simpa only [P, U, velLift] using hvelOn
  have hbase : Continuous (fun q : P ↦ F q.1) := by
    have hbaseOn := hF.continuousOn
    rw [continuousOn_iff_continuous_restrict] at hbaseOn
    simpa only [P, U] using hbaseOn
  have hquad :=
  ...
  simpa only [P, U, F, lRegLag] using hlag

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- On a compact family of regularized L-rays, terminal action is bounded
linearly by the length of the terminal square-root-time interval. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- A bounded sequence of regularized L-rays has one uniform linear bound on
all terminal action segments inside a common compact time interval. -/
