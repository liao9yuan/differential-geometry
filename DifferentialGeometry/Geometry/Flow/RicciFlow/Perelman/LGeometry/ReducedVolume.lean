...
This file defines the ordinary fixed-manifold reduced density and reduced
volume.  It also relates the intrinsic pulled-back reduced Jacobian to the
model-coordinate density used by the manifold parametrization formula.
-/
...
/-- The reduced-volume density at a point and positive backward time. -/
...
/-- Perelman's reduced volume, valued in the extended nonnegative reals. -/
...
omit [NeZero (Module.finrank Real E)] [I.Boundaryless] [T2Space M]
  [CompactSpace M] in
/-- The reduced density is strictly positive. -/
...
private theorem exists_lExpPartial
  ...
  obtain ⟨Φ, hsource, htarget, hEq⟩ :=
    DifferentialGeometry.Geometry.Riemannian.exists_diffeo_of_injOn
...
/-- The fixed-time L-exponential partial diffeomorphism on its strict
injectivity domain. -/
...
/-- The source of `lExpPartial` is the strict minimizing domain. -/
...
/-- The target of `lExpPartial` is the strict L-exponential image. -/
...
/-- On the strict domain, `lExpPartial` agrees with the L-exponential map. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The parameter density of the fixed-time L-exponential chart is its
moving target Gram density. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- On the strict injectivity domain, the intrinsic pulled-back reduced
Jacobian is the L-exponential Jacobian times the reduced density. -/
theorem lRedJac_eq
    (S : SolutionOn (I := I) (M := M) D)
    (hS : IsSolutionOn (I := I) S) (T : Real) (x : M)
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Multiplying the intrinsic pulled-back reduced Jacobian by the fixed source
density gives the model-coordinate target density. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Under regular backward-time coverage, every point outside the strict
L-exponential image lies in the fixed-time L-cut image. -/
...
/-- The strict fixed-time L-exponential image has full moving Riemannian
volume under regular backward-time coverage. -/
theorem lExp_inj_ae
    [ConnectedSpace M]
    (S : SolutionOn (I := I) (M := M) D)
    (hS : IsSolutionOn (I := I) S) (T : Real) (x : M)
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Reduced volume is the model-space integral of the pulled-back reduced
density over the strict L-injectivity domain. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Perelman's reduced volume is antitone in positive backward time while the
corresponding backward slab remains regular. -/
