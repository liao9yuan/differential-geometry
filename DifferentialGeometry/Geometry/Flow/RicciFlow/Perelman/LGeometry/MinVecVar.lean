...
/-!
# Initial-vector bounds with varying terminal times

This module bounds the initial tangents of regularized L-rays whose positive
terminal square-root times vary in one common compact interval.
-/
...
private theorem metricRange_bdd
  ...
  have hnorm : ∀ n, ‖Z n‖ ≤ L := by
    ...
    have hcoerc : c * ‖Z n‖ ^ 2 ≤ g.inner x (Z n) (Z n) := by
      simpa only [c] using gpCoerciveConst_le (I := I) g x (Z n)
...
/-- Uniform action bounds up to varying positive terminal times in a common
regular slab uniformly bound the initial tangents of regularized L-rays. -/
    ...
    apply lRayMetric_var (I := I) S hS T x (Z n) (B n) eps C R A Cp
      ...
      (fun s hs y ↦ hpot s (by
        simpa only [Set.uIcc_of_le hR] using
          (⟨hs.1, hs.2.trans (hBR n)⟩ : s ∈ Set.Icc (0 : Real) R)) y)
      hgradC hricC (hact n)
