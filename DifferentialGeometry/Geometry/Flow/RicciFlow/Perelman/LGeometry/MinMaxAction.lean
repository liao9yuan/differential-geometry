...
/-!
# Kinetic budgets from regularized L-action

This module extracts a moving-metric kinetic budget directly from an upper
bound for the regularized L-action.  The moving tangent vectors are fully
paired by the metric before integration.
-/
...
/-- A pointwise lower bound for the scalar part converts an action budget into
a moving-metric kinetic budget. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- On a compact manifold, scalar-curvature continuity supplies a uniform
kinetic budget for every curve with a common regularized action budget. -/
