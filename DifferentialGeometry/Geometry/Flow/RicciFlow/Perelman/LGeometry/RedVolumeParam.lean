...
/-!
# Joint parameter lower semicontinuity of reduced volume

This module passes the genuine joint terminal-time/basepoint upper continuity
of compact L-cost through the reduced density and the moving Riemannian volume
measure.  The measure argument uses the finite chart partition of unity and
Fatou's lemma against the fixed model Haar measure.
-/
...
private theorem lCost_param_usc
  ...
  have hx : Tendsto (fun n ↦ (q n).2) atTop (nhds x) := by
    simpa only using continuous_snd.continuousAt.tendsto.comp hq
...
omit [NeZero (Module.finrank Real E)] in
/-- At a positive backward time with a regular compact slab, reduced volume is
lower semicontinuous in the joint terminal-time/basepoint parameter. -/
