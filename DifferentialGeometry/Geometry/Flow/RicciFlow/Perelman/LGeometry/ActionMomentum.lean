...
/-!
# Fixed-chart momentum regularity for L-action minimizers

A fixed-chart local minimizer has a continuous coordinate velocity.  Replacing
the weak position force by its continuous representative upgrades the weak
momentum equation to a closed-interval `C¹` identity.
-/
...
/-- A fixed-chart local minimizer has a closed-interval `C¹` momentum
representative, equal pointwise to twice the chart Gram operator applied to its
continuous coordinate velocity and differentiated by twice the chart force. -/
theorem lChart_mom_c1
  ...
  have hForceRep : ContinuousOn
      (lChartForceRep (I := I) S T a p u q) (Icc (0 : Real) L) :=
    lChartForceRep_cont (I := I) S hS T a p u q hreg hchart hq
  have hForceAE : lChartForce (I := I) S T a p u =ᵐ[timeMeasure L]
      lChartForceRep (I := I) S T a p u q :=
    lChartForceRep_ae (I := I) S hS T a p u q hreg hchart hqae
  let F : Real → E := fun r ↦
    (2 : Real) • lChartForceRep (I := I) S T a p u q r
  have hF : ContinuousOn F (Icc (0 : Real) L) := by
    simpa only [F] using hForceRep.const_smul (2 : Real)
  ...
  have hMomQcont : ContinuousOn momQ (Icc (0 : Real) L) := by
    simpa only [momQ] using (hAcont.clm_apply hq).const_smul (2 : Real)
