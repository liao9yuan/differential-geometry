...
/-!
# Two-piece windows in a finite L-action realization

A global fixed-endpoint comparison descends to any adjacent positive pair in a
finite chart-`H¹` realization.  All pieces outside the selected window remain
unchanged and cancel from the exact finite action sums.
-/
...
theorem lNodeWin_cmp
  ...
  let tw : Fin 3 → Real := ![t i.castSucc, t j.castSucc, t j.succ]
  let pw : Fin 2 → M := ![p i, p j]
  let vw : (k : Fin 2) → timeH1 E (lSegLen tw k) :=
    Fin.cases (by simpa only [tw, lSegLen, hmid] using v0)
      (Fin.cases (by simpa only [tw, lSegLen] using v1)
        (fun k ↦ Fin.elim0 k))
  have htw : Monotone tw := by
    apply Fin.monotone_iff_le_succ.mpr
    intro k
    fin_cases k
    · simpa only [tw, hmid] using hposi.le
    · simpa only [tw] using hposj.le
  have hvwtar : ∀ k, MapsTo (vw k).toFun
      (Icc (0 : Real) (lSegLen tw k)) (extChartAt I (pw k)).target := by
    intro k
    fin_cases k
    · simpa only [vw, tw, pw, lSegLen, hmid] using htar0
    · simpa only [vw, tw, pw, lSegLen] using htar1
  have hvwnode : (extChartAt I (pw 0)).symm
        ((vw 0).toFun (lSegLen tw 0)) =
      (extChartAt I (pw 1)).symm ((vw 1).toFun 0) := by
    simpa only [vw, tw, pw, lSegLen, hmid] using hvnode
  ...
  have hWleft : gammaW (t i.castSucc) = gamma (t i.castSucc) :=
    hW0.trans (by simpa only [vw, tw, pw, lSegLen, hmid] using hv0)
  have hWright : gammaW (t j.succ) = gamma (t j.succ) :=
    hW2.trans (by simpa only [vw, tw, pw, lSegLen] using hv2)
  ...
  have hsrcV : ∀ k, MapsTo gammaV
    ...
    · subst k
      ...
      exact hsrcW 0 (by simpa only [tw, pw] using hs)
    · by_cases hkj : k = j
      · subst k
        have hleft : t i.castSucc ≤ s :=
          hposi.le.trans (hmid.le.trans hs.1)
        rw [hgammaV_win ⟨hleft, hs.2⟩]
        exact hsrcW 1 (by simpa only [tw, pw, hmid] using hs)
  ...
  have hrepV : ∀ k, EqOn (uv k).toFun
    ...
    · subst k
      ...
      rw [hgammaV_win ⟨hs.1, hs.2.trans (hmid.le.trans hposj.le)⟩]
      simpa only [vw, tw, pw, lSegLen, hmid] using hrepW 0 hr
    · by_cases hkj : k = j
      · subst k
        ...
        rw [hgammaV_win ⟨hleft, hs.2⟩]
        simpa only [vw, tw, pw, lSegLen] using hrepW 1 hr
