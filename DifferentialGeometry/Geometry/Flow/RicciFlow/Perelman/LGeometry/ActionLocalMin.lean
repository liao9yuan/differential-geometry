...
/-!
# Local minimality of relaxed regularized L-curves

This file transfers global minimality against `C¹` curves to genuine local
minimality of a positive fixed-chart `timeH1` segment.
-/
...
-- The scalar space-time composition needs extra reduction budget in this generic manifold context.
omit [NeZero (Module.finrank Real E)] [T2Space M] [CompactSpace M] in
/-- The genuine nonlinear chart action splits into its Gram kinetic and scalar
potential integrals whenever the `timeH1` representative stays in the chart
and the backward times are regular. -/
theorem lChartAct_split
  ...
  have hAc : ContinuousOn A (Icc (0 : Real) L) := by
    ...
    exact (chartGramOp_cont (I := I) hMet hJreg p hKchart).comp hpair
  ...
  have hscal : ContinuousOn (fun r ↦
    ...
    have hτ' : Continuous τ' := by
      exact continuous_induced_rng.mpr hτc.restrict
    have hinv' : Continuous (fun r : Icc (0 : Real) L ↦
        (extChartAt I p).symm (u.toFun r.1)) := hinv.restrict
    simpa only [τ'] using hSc.continuous_subtype.comp (hτ'.prodMk hinv')
...
theorem lChartAct_local
  ...
  have hKc : IsCompact K :=
    isCompact_range (u i).continuousOn_toFun.restrict
