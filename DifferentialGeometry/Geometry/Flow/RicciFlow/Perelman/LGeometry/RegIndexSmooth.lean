...
/-- Pointwise `C²` regularity of a total tangent-bundle section gives
differentiability of its fixed-base chart representative. -/
...
omit [InnerProductSpace Real E] [FiniteDimensional Real E]
  [NeZero (Module.finrank Real E)] [I.Boundaryless] [T2Space M]
  [SigmaCompactSpace M] in
/-- A globally `C²` total tangent-bundle section has differentiable fixed-base
chart representatives at every parameter. -/
...
private theorem movingCov_contOn
  ...
  have hGamma (i j k : Fin (Module.finrank Real E)) :
    ...
    simpa only [Function.comp_apply, base] using hcomp
  ...
  have hchart : ContinuousAt
    ...
    simpa only [chartCovDerivAlong_def] using hdrep.add hchrist
...
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- The regularized L-index density of two locally `C^2` fields is integrable
on every compact parameter interval contained in their common smooth domain
and lying in regular backward time. -/
theorem lRegIndex_intOn
  ...
  have htau : Continuous tau := by
    simpa only [tau] using continuous_const.sub (hz.pow 2)
  have hbase : Continuous base := by
    simpa only [base, P, K] using
      (halpha.continuousOn.mono hseg).restrict
  have hYsec : Continuous (fun z : P ↦
      (TotalSpace.mk' E (base z) (Y z) : TangentBundle I M)) := by
    simpa only [base, P, K] using
      (hY.continuousOn.mono hseg).restrict
  have hWsec : Continuous (fun z : P ↦
      (TotalSpace.mk' E (base z) (W z) : TangentBundle I M)) := by
    simpa only [base, P, K] using
      (hW.continuousOn.mono hseg).restrict
  have hAsec : Continuous (fun z : P ↦
    ...
    have hvel : ContMDiffOn (modelWithCornersSelf Real Real) I.tangent 0
      ...
      rfl
    simpa only [base, A, P, K] using
      (hvel.continuousOn.mono hseg).restrict
  have hDYsec : Continuous (fun z : P ↦
    ...
    simpa only [base, DY, tau, P] using h.restrict
  have hDWsec : Continuous (fun z : P ↦
    ...
    have h := movingCov_contOn (I := I) S hS T alpha W hΩ hseg halpha hW
      (K := K) (by simpa only [K] using hreg)
    simpa only [base, DW, tau, P] using h.restrict
  have hInner : Continuous (fun z : P ↦
    ...
    have h := hS.smoothMetric.metricTensor_cont.eval_continuous
      ...
      (v := fun i z ↦ vec2 (DY z) (DW z) i) (by
        intro i
        fin_cases i
        · simpa only [vec2, Fin.zero_eta] using hDYsec
        · simpa only [vec2, Fin.mk_one] using hDWsec)
  ...
  have hRm : Continuous (fun z : P ↦
    ...
    intro i
    fin_cases i
    · simpa only [vec4, Fin.zero_eta] using hYsec
    · simpa only [vec4, Fin.mk_one] using hAsec
    · simpa only [vec4, Fin.mk_eq_mk] using hAsec
    · simpa only [vec4, Fin.mk_eq_mk] using hWsec
  have hHess : Continuous (fun z : P ↦
    ...
    intro i
    fin_cases i
    · simpa only [vec2, Fin.zero_eta] using hYsec
    · simpa only [vec2, Fin.mk_one] using hWsec
  have hRic1 : Continuous (fun z : P ↦
    ...
    intro i
    fin_cases i
    · simpa only [vec3, Fin.zero_eta] using hAsec
    · simpa only [vec3, Fin.mk_one] using hYsec
    · simpa only [vec3, Fin.mk_eq_mk] using hWsec
  have hRic2 : Continuous (fun z : P ↦
    ...
    intro i
    fin_cases i
    · simpa only [vec3, Fin.zero_eta] using hYsec
    · simpa only [vec3, Fin.mk_one] using hAsec
    · simpa only [vec3, Fin.mk_eq_mk] using hWsec
  have hRic3 : Continuous (fun z : P ↦
    ...
    intro i
    fin_cases i
    · simpa only [vec3, Fin.zero_eta] using hWsec
    · simpa only [vec3, Fin.mk_one] using hAsec
    · simpa only [vec3, Fin.mk_eq_mk] using hYsec
...
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- The regularized L-index density of two globally `C^2` fields is integrable
on every compact parameter interval lying in regular backward time. -/
