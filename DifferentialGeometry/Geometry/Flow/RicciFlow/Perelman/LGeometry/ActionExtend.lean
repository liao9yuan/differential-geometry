...
A closed-interval regularized curve is extended through both endpoints by the
local phase solutions determined by its one-sided chart velocities.
-/
...
private theorem regAt_of_punct
  ...
  have hqpunct : ∀ᶠ r in 𝓝[≠] c, HasDerivAt q (v r) r := by
    ...
    simpa only [q, x, lPhaseField] using hfst
  ...
  have hrep_punct :
    ...
    have hderiv : deriv ((extChartAt I x) ∘ alpha) r = v r := by
      simpa only [q, chartCurve] using hqr.deriv
  ...
  have hrep_c : chartRepAtBase (I := I) x alpha X c = v c := by
    ...
    have hderiv : deriv ((extChartAt I x) ∘ alpha) c = v c := by
      simpa only [q, chartCurve] using hqder.deriv
    rw [fderiv_apply_one_eq_deriv, hderiv] at hbridge
    simpa only [q, X, chartCurve, chartRepAtBase_apply, lVelocity] using hbridge
  have hrep :
  ...
  have hveldiff : DifferentiableAt Real
    ...
    have hv : DifferentiableAt Real v c := by
      simpa only [z] using hsnd.differentiableAt
  ...
  calc
    ...
    _ = (lRegAccel S T c (alpha c) (X c) : E) := by
      rw [hcurve.self_of_nhds, hvelEq.self_of_nhds]

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] in
/-- A regularized L-curve on a closed interval extends as a regularized
L-curve to an open neighborhood without changing its closed restriction. -/
    ...
    have hchart :=
      ((contMDiffOn_iff_target.mp (hc1.mono (fun (s : Real) (hs : s ∈ Icc a da) ↦
        ...
        (fun s hs ↦ ⟨hs, by
          ...
          exact hda_src hs⟩)
    simpa only [qa, chartCurve] using hchart.contDiffOn
    ...
    have hchart :=
      ((contMDiffOn_iff_target.mp (hc1.mono (fun (s : Real) (hs : s ∈ Icc db b) ↦
        ...
        (fun s hs ↦ ⟨hs, by
          rw [extChartAt_source]
          exact hdb_src hs⟩)
    simpa only [qb, chartCurve] using hchart.contDiffOn
  ...
  have hphaseA : ∀ᶠ r in 𝓝[≠] a,
    ...
    · have hrb : r < b := hrda.trans_le hda_le
      ...
      have hvgerm : vA =ᶠ[𝓝 r]
        ...
        simpa only [va, qa, chartCurve, chartRepAtBase_apply, lVelocity] using
          hbridge.symm
  ...
  have hphaseB : ∀ᶠ r in 𝓝[≠] b,
    ...
    · have har : a < r := lt_of_le_of_lt ha_db hdbr
      ...
      have hvgerm : vB =ᶠ[𝓝 r]
        ...
        rw [fderiv_apply_one_eq_deriv, ← derivWithin_of_mem_nhds hmem] at hbridge
        simpa only [vb, qb, chartCurve, chartRepAtBase_apply, lVelocity] using
          hbridge.symm
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] in
/-- A regularized L-curve on a closed interval extends through both endpoints
without changing its restriction to the closed interval. -/
