...
/-!
# Global smooth extensions of regularized L-rays

This file clamps square-root time inside the maximal regularized domain.  It
then globalizes the canonical regularized L-Jacobi field without changing it
on a prescribed compact positive segment.
-/
...
/-- The totalized regularized L-ray satisfies the regularized L-curve equation
on every positive compact segment contained in its maximal domain. -/
theorem lRegCurve_isReg
  ...
  · calc
      lVelocity (I := I) (lRegCurve S T x Z) 0 = (2 : Real) • Z :=
        lRegCurve_vel_zero S hS T x Z hT
      _ = Z + Z := two_smul Real Z
      _ = (2 : Nat) • Z := (two_nsmul Z).symm
  intro s hs
...
/-- A compact positive segment of a maximal regularized L-ray admits a smooth
time clamp whose whole range stays in the same maximal domain. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- A compact regularized L-ray segment admits a global smooth time clamp
whose range also stays in any prescribed open neighborhood of the segment. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- A compact regularized L-ray segment admits a global smooth time clamp that
is the identity on an open neighborhood of the whole segment. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- A compact positive regularized L-ray and its canonical initial-tangent
Jacobi field extend to one globally smooth tangent-bundle section. -/
theorem exists_lRay_smooth
  ...
  have hsmooth : ContMDiff (modelWithCornersSelf Real Real) I.tangent ∞
    ...
    exact (lRegJacobi_smooth S hS T x V).comp hpair.contMDiffOn
      (fun s _hs ↦ by simpa only [lRegJointDom] using hrho_range s)
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- A compact positive regularized L-ray and its canonical Jacobi field admit
a global smooth representative whose time clamp remains in a prescribed open
...
theorem exists_lRay_germ_in
  ...
  have hsmooth : ContMDiff (modelWithCornersSelf Real Real) I.tangent ∞
    ...
    rw [← contMDiffOn_univ]
    exact (lRegJacobi_smooth S hS T x V).comp hpair.contMDiffOn
      (fun s _hs ↦ by simpa only [lRegJointDom] using hrho_dom s)
...
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- A compact positive regularized L-ray and its canonical Jacobi field admit
a global smooth representative agreeing on a neighborhood of the segment. -/
