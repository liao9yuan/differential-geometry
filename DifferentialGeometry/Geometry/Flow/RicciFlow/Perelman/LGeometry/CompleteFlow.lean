...
/-!
# Compact localization for complete Perelman flows

This module starts the complete bounded-curvature extension of Perelman's
L-geometry.  It converts a regularized L-action bound into a fixed complete
reference-metric energy bound, then localizes the whole curve in one compact
intrinsic ball.
-/
...
/-- A reference-metric comparison and a scalar-potential lower bound convert
a regularized L-action budget into a fixed-metric energy budget. -/
...
/-- A regularized L-curve with bounded action stays in a compact ball of any
complete reference metric that is uniformly controlled by the moving metrics. -/
...
/-- A common action bound and a common initial point place an entire family of
regularized L-curves in one compact reference-metric ball. -/
