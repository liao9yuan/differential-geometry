...
/-!
# Continuation of regularized L-rays

This module continues a regularized L-ray across every compact regular
backward-time slab on a compact manifold.
-/
...
/-- Regularized L-speed is uniformly bounded while the ray remains in a
compact regular square-root-time slab. -/
  ...
  have hclosed : closure U ∩ Set.Icc (0 : Real) B ⊆ U := by
    ...
    have hvel : lVelocity (I := I) (fun r ↦ alpha (Z, r)) t0 =
      ...
      simpa only [lVelocity] using congrArg (fun L ↦ L (1 : Real))
        (heqGerm.mfderiv_eq (I := modelWithCornersSelf Real Real) (I' := I))
