...
/-!
# Joint local L-exponential coordinates

This module combines the initial-tangent and backward-time variables near a
nonconjugate point of the positive L-exponential domain.
-/
...
/-- At a nonconjugate positive-domain point, adjoining backward time makes the
L-exponential map a joint smooth local coordinate map. -/
theorem lExpTime_local
  ...
  have hZU : (z, tau) ∈ U := by simpa only [z] using hdom
    ...
    · intro hv
      ...
      have hv2 : v.2 = 0 := by
        ...
        have hsndEq :
          ...
          rfl
        rw [hsndEq] at this
        simpa only [Prod.snd_zero] using this
