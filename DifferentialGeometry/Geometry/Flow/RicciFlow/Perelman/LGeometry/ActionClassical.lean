...
/-!
# Classical fixed-chart Euler equation

This file identifies the pointwise Euler equation obtained from the fixed-chart
momentum identity with the second component of the regularized L-geodesic phase
field.  Metric and curvature expressions are compared only after scalar
pairing in the chart model space.
-/
  ...
  · rw [hcovA, hcovZ]
    ...
    have hgramA :
      ...
      simpa only [SolutionOn.family] using
        (chartGramOp_inner (I := I) S.family p (T - r ^ 2, u r)
          (q' + chartChristoffelContraction (I := I) g p
            (q r) (q r) (u r)) z).symm
    have hgramZ :
        (S.base.metric (T - r ^ 2)).inner ((extChartAt I p).symm (u r))
      ...
      simpa only [SolutionOn.family] using
        (chartGramOp_inner (I := I) S.family p (T - r ^ 2, u r) (q r)
...
private theorem lGramPair_shift
  ...
  have hU : HasDerivAt U (Q (a + r)) (a + r) := by
    simpa only [U, Q, har, one_smul] using hu'.scomp (a + r) hshift
  have hQ : HasDerivAt Q q' (a + r) := by
    simpa only [Q, har, one_smul] using hq'.scomp (a + r) hshift
  ...
  have hplus : HasDerivAt (fun x : Real => a + x) 1 r := by
    simpa using (hasDerivAt_const (x := r) a).add (hasDerivAt_id (x := r))
  dsimp only [U, Q] at habs
  simp only [har] at habs
  convert habs.comp r hplus using 1
  · funext x
    have hax : a + x - a = x := by ring
    rw [Function.comp_apply, hax]
  · rw [har, mul_one]
    congr 1
...
omit [SigmaCompactSpace M] in
/-- At a regular interior chart point, the differentiated fixed-chart momentum
equation is equivalent to the regularized L-geodesic phase equation. -/
theorem lChartEuler_iff
  ...
  have hpair (z : E) :
    ...
    have hinner : HasDerivAt (fun s => inner Real (mom s) z)
      ...
      have h := (innerSL Real z).hasFDerivAt.comp_hasDerivAt r hmom.hasDerivAt
      simpa only [Function.comp_apply, real_inner_comm] using h
