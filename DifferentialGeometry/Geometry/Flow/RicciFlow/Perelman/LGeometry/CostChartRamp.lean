...
/-!
# Fixed-chart terminal ramps for L-cost

This module realizes the affine coordinate segment between two nearby
endpoints as a time-H1 curve and bounds its regularized chart action.
-/
...
/-- The affine coordinate ramp from `y` to `z`, parameterized on a prescribed
...
/-- The continuous representative of the coordinate ramp is its affine
formula on the ramp interval. -/
...
/-- The weak derivative of a nonconstant coordinate ramp is the constant unit
vector pointing from its initial endpoint to its terminal endpoint. -/
...
omit [NeZero (Module.finrank Real E)] in
/-- A coordinate ramp starts at its first endpoint. -/
...
omit [NeZero (Module.finrank Real E)] in
/-- A coordinate ramp ends at its second endpoint. -/
...
omit [NeZero (Module.finrank Real E)] in
/-- If both endpoints lie in a convex coordinate set, the whole affine ramp
stays in that set. -/
...
omit [NeZero (Module.finrank Real E)] [I.Boundaryless] [CompactSpace M] in
/-- Uniform Gram and scalar bounds give a linear action bound for one
nonconstant affine coordinate ramp. -/
theorem lRampAct_bound
  ...
  have hscalarCont : ContinuousOn (fun r ↦
    ...
    have hτ' : Continuous τ' := continuous_induced_rng.mpr hτc.restrict
    simpa only [τ'] using
      hSc.continuous_subtype.comp (hτ'.prodMk hinv.restrict)
  ...
  have hact : lChartAct S T a p u =
    ...
    · congr 1
      · apply intervalIntegral.integral_congr_ae_restrict
        ...
        rw [hr]
        simp only [kin, A, τ, ContinuousLinearMap.smul_apply,
...
omit [NeZero (Module.finrank Real E)] [I.Boundaryless] [CompactSpace M] in
/-- If the coordinate displacement is at most `V` times the ramp duration,
the regularized chart action is bounded linearly in that duration. -/
...
/-- On a compact regular backward-time slab and a compact chart set, one pair
of Gram and scalar constants controls every positive affine ramp in the slab. -/
