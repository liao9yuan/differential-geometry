import DifferentialGeometry.Geometry.Flow.RicciFlow.Perelman.LGeometry.ActionNodeLocal
import DifferentialGeometry.Geometry.Flow.RicciFlow.Perelman.LGeometry.ActionNodePiece
...
/-!
# Same-chart momentum matching at an L-action node

Affine endpoint ramps turn two-piece minimality into equality of the coordinate
momenta when both pieces use one common manifold chart.
-/
...
omit [CompactSpace M] in
/-- Exact two-piece comparison in one chart forces the terminal momentum of
the left piece to equal the initial momentum of the right piece. -/
theorem lNode_mom_same
    ...
    (hreg : ∀ i r, r ∈ Icc (0 : Real) (lSegLen t i) →
    ...
    (hcmp : ∀ v : (i : Fin 2) → timeH1 E (lSegLen t i),
      (∀ i, MapsTo (v i).toFun
        (Icc (0 : Real) (lSegLen t i)) (extChartAt I p).target) →
      (extChartAt I p).symm ((v 0).toFun 0) =
        (extChartAt I p).symm ((u 0).toFun 0) →
      (extChartAt I p).symm ((v 1).toFun (lSegLen t 1)) =
        (extChartAt I p).symm ((u 1).toFun (lSegLen t 1)) →
      (extChartAt I p).symm ((v 0).toFun (lSegLen t 0)) =
    ...
    chartGramOp (I := I) S.family p
      ...
      chartGramOp (I := I) S.family p
        ...
        (derivWithin (u 1).toFun
          (Icc (0 : Real) (lSegLen t 1)) 0) := by
  classical
  have hL (i : Fin 2) : 0 < lSegLen t i := by
  ...
  intro z
  have hnode' : (u 0).toFun (lSegLen t 0) = (u 1).toFun 0 := by
    calc
      (u 0).toFun (lSegLen t 0) =
  ...
  obtain ⟨s0, hs0, hs0one, hbuf0⟩ :=
    timeH1.exists_line_scale (u 0) (timeH1.rampUp (lSegLen t 0) z)
      isOpen_interior (hchart 0)
  obtain ⟨s1, hs1, hs1one, hbuf1⟩ :=
    timeH1.exists_line_scale (u 1)
      (timeH1.rampDown (lSegLen t 1) (s0 • z))
      isOpen_interior (hchart 1)
  let ze : E := (s1 * s0) • z
  have hbufUp : MapsTo
      (fun x : Real × Real ↦
        (u 0).toFun x.2 +
          x.1 • (timeH1.rampUp (lSegLen t 0) ze).toFun x.2)
      (Icc (-1 : Real) 1 ×ˢ Icc (0 : Real) (lSegLen t 0))
    ...
    rw [timeH1.rampUp_smul (hL 0) s1 (s0 • z),
      ...
      timeH1.toFun_smul s1 (s0 • timeH1.rampUp (lSegLen t 0) z) hx.2,
      timeH1.toFun_smul s0 (timeH1.rampUp (lSegLen t 0) z) hx.2]
    simp only [smul_smul]
    congr 1
    ring_nf
  have hbufDown : MapsTo
      (fun x : Real × Real ↦
        (u 1).toFun x.2 +
          x.1 • (timeH1.rampDown (lSegLen t 1) ze).toFun x.2)
      (Icc (-1 : Real) 1 ×ˢ Icc (0 : Real) (lSegLen t 1))
      (interior (extChartAt I p).target) := by
    intro x hx
    ...
    rw [timeH1.rampDown_smul (hL 1) s1 (s0 • z),
      timeH1.toFun_smul s1
        (timeH1.rampDown (lSegLen t 1) (s0 • z)) hx.2]
  ...
  let phi : Real → Real := fun c ↦
    lChartAct S T (t 0) p
        (u 0 + c • timeH1.rampUp (lSegLen t 0) ze) +
      lChartAct S T (t 1) p
        (u 1 + c • timeH1.rampDown (lSegLen t 1) ze)
  have hphi : HasDerivAt phi
    ...
    simpa only [phi, sub_eq_add_neg] using hline0.add hline1
  have hphimin : IsLocalMin phi 0 := by
    ...
    let w : (i : Fin 2) → timeH1 E (lSegLen t i) := fun i ↦
      Fin.cases
        (by simpa using u 0 + c • timeH1.rampUp (lSegLen t 0) ze)
        (Fin.cases
          (by simpa using u 1 + c • timeH1.rampDown (lSegLen t 1) ze)
          (fun j ↦ Fin.elim0 j)) i
    have hwtar : ∀ i, MapsTo (w i).toFun
        (Icc (0 : Real) (lSegLen t i)) (extChartAt I p).target := by
      intro i
      fin_cases i
      · intro r hr
        change (u 0 + c • timeH1.rampUp (lSegLen t 0) ze).toFun r ∈ _
        rw [timeH1.toFun_add (u 0)
          (c • timeH1.rampUp (lSegLen t 0) ze) hr,
          timeH1.toFun_smul c (timeH1.rampUp (lSegLen t 0) ze) hr]
        exact interior_subset (hbufUp (x := (c, r)) ⟨hc, hr⟩)
      · intro r hr
        change (u 1 + c • timeH1.rampDown (lSegLen t 1) ze).toFun r ∈ _
        rw [timeH1.toFun_add (u 1)
          (c • timeH1.rampDown (lSegLen t 1) ze) hr,
          timeH1.toFun_smul c (timeH1.rampDown (lSegLen t 1) ze) hr]
        exact interior_subset (hbufDown (x := (c, r)) ⟨hc, hr⟩)
    have hw0 : (extChartAt I p).symm ((w 0).toFun 0) =
        (extChartAt I p).symm ((u 0).toFun 0) := by
      rw [show w 0 = u 0 + c • timeH1.rampUp (lSegLen t 0) ze from rfl,
    ...
    have hw2 : (extChartAt I p).symm
        ...
        (extChartAt I p).symm ((u 1).toFun (lSegLen t 1)) := by
      rw [show w 1 = u 1 + c • timeH1.rampDown (lSegLen t 1) ze from rfl,
    ...
    have hwnode : (extChartAt I p).symm
      ...
      rw [show w 0 = u 0 + c • timeH1.rampUp (lSegLen t 0) ze from rfl,
        show w 1 = u 1 + c • timeH1.rampDown (lSegLen t 1) ze from rfl,
    ...
    simpa only [Fin.sum_univ_two, w, phi, zero_smul, add_zero] using hc'
