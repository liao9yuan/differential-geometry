...
/-!
# Perelman L-geodesics

This file defines the intrinsic L-geodesic equation on positive regular
backward times.  The equation is expressed by pairing the Euler residual with
every tangent vector, avoiding a representation-dependent moving Ricci-sharp
bundle object.
-/
...
/-- The pointwise intrinsic L-geodesic equation, together with exactly the
curve regularity needed to read its covariant acceleration. -/
def HasLEquationAt
...
/-- A curve is an L-geodesic on a set when it satisfies the intrinsic
L-geodesic equation at every time in that set, and the set consists of
positive backward times corresponding to regular flow times. -/
def IsLGeodesic
    (S : SolutionOn (I := I) (M := M) D) (T : Real) (gamma : Real → M)
    (s : Set Real) : Prop :=
...
/-- A `C⁸` curve is L-critical on an interval when L-length is stationary
under every smooth variation that fixes both endpoints for every parameter. -/
def IsLCritical
    (S : SolutionOn (I := I) (M := M) D) (T : Real) (gamma : Real → M)
    (a b : Real) : Prop :=
...
/-- An L-geodesic on a set satisfies the pointwise equation at each member. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- Every time in the domain of an L-geodesic is positive. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- Every time in the domain of an L-geodesic maps to a regular forward
flow time. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- An L-geodesic is manifold-differentiable at every time in its domain. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- The chart representation of an L-geodesic's velocity is differentiable
at every time in its domain. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- The Euler residual of an L-geodesic pairs to zero with every tangent
vector at a time in its domain. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- Restricting the time set preserves the L-geodesic property. -/
...
/-- An L-geodesic is stationary for every smooth variation whose first-order
endpoint variation fields vanish. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- A sufficiently smooth L-geodesic is L-critical under fixed-endpoint
variations. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- A smooth L-critical curve satisfies the intrinsic L-geodesic equation on
the positive regular interior of its interval. -/
theorem IsLCritical.isLGeo
  ...
  · intro tau0 htau0
    ...
    have hvel : DifferentiableAt Real
      ...
      simpa only [lVelocity] using
        velocityField_chartRep_differentiableAt
          (I := I) (S.base.metric (T - tau0)) (fun _ : Real => gamma)
          hfconst tau0
    refine ⟨hmd, hvel, ?_⟩
    intro Y
    let c : M := gamma tau0
    ...
    have hV0c : HasCompactSupport V0 := by
      simpa only [V0] using hbetac.smul_right
    ...
    have htest : ∀ g : Real → Real,
      ...
      have hVgc : HasCompactSupport Vg := by
        simpa only [Vg] using hV0c.smul_left
    ...
    have hfield0 :
      ...
      change ((mfderiv 𝓘(Real, Real) I
          (fun u : Real => f0 u tau0) 0 (1 : Real)) : E) = Y
      rw [hf0vel tau0]
      simpa only [V0, hbetaone, one_smul, e, c] using hw
...
/-- The regularized L-geodesic acceleration vector at square-root time. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The square-root-time L-geodesic equation as a first-order vector field in
fixed chart position/velocity coordinates. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The manifold curve reconstructed from the position component of a fixed
chart phase path. -/
...
omit [InnerProductSpace Real E]
  [SigmaCompactSpace M] in
/-- The tangent field reconstructed from the velocity component of a fixed
chart phase path. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The regularized chart phase field is jointly smooth at regular chart
points. -/
theorem lPhaseField_smoothAt
      ...
      (∑ j : Fin (Module.finrank Real E),
        chartInvGramOnE (I := I) (S.family.metric (T - p.1 ^ 2))
          ...
          (let x := (extChartAt I x0).symm p.2.1
           extDerivFun (I := I) (S.scalar (T - p.1 ^ 2)) x
  ...
  have hinv (i j : Fin (Module.finrank Real E)) :
    ...
    simpa only [base, Function.comp_apply] using h.comp (s, z) hbase
  ...
  have hcoord (i : Fin (Module.finrank Real E)) :
      ContDiffAt Real ∞
        ...
        (s, z) := by
    simpa only [chartCoordCLM_apply] using
  ...
  have hric_comp (j : Fin (Module.finrank Real E)) :
    ...
    rw [hA, map_sum, ContinuousLinearMap.sum_apply]
    refine Finset.sum_congr rfl ?_
    intro k _
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- At a regular chart point, the autonomized regularized L-phase field has a
local integral curve through the prescribed phase state. -/
theorem exists_lPhaseCurve
    ...
    ∃ c : Real → Real × (E × E),
      c 0 = (0, z0) ∧
        IsMIntegralCurveAt (I := 𝓘(Real, Real × (E × E))) c
          (fun p : Real × (E × E) =>
  ...
  have hvf : ContDiffAt Real 1 vf (0, z0) := by
    exact contDiffAt_const.prodMk hphase
  have hsection :
      ContMDiffAt 𝓘(Real, Real × (E × E))
        (𝓘(Real, Real × (E × E)).prod 𝓘(Real, Real × (E × E))) 1
        (fun p : Real × (E × E) =>
          (⟨p, vf p⟩ : TangentBundle 𝓘(Real, Real × (E × E))
            (Real × (E × E)))) (0, z0) := by
    rw [Bundle.contMDiffAt_section]
    simpa using hvf.contMDiffAt
  simpa only [vf] using
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- At a regular chart point, the regularized L-phase ODE has a local
solution through the prescribed phase state. -/
theorem exists_lPhaseSol
  ...
  let X : Real → E × E → E × E := fun s z => lPhaseField S T x0 s z
  ...
  · intro s hs
    ...
    have hcderiv :
        HasMFDerivAt 𝓘(Real, Real)
          (𝓘(Real, Real).prod 𝓘(Real, E × E)) c s
          ((1 : Real →L[Real] Real).smulRight
            (DifferentialGeometry.Analysis.ODE.autonomizedFlowVF X (c s))) := by
      simpa only [DifferentialGeometry.Analysis.ODE.autonomizedFlowVF, X] using
        hcurveU s hsU
    ...
    have hsnd :=
      DifferentialGeometry.Analysis.ODE.autonomizedFlow_snd_hasMFDerivAt
        (I := 𝓘(Real, E × E)) X c s hcderiv
    rw [htime s hs] at hsnd
    rw [hasMFDerivAt_iff_hasFDerivAt, hasFDerivAt_iff_hasDerivAt] at hsnd
    apply hsnd.congr_deriv
    change (1 : Real) • lPhaseField S T x0 s (c s).2 = _
    rw [one_smul]
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Two local solutions of the regularized L-phase ODE with the same regular
state at a square-root time agree as germs there. -/
theorem lPhaseSol_unique_at
  ...
  have hvf : ContDiffAt Real 1 vf (s0, z0) := by
    exact contDiffAt_const.prodMk hphase
  have hsection :
      ContMDiffAt 𝓘(Real, Real × (E × E))
        (𝓘(Real, Real × (E × E)).prod 𝓘(Real, Real × (E × E))) 1
  ...
  have hc : IsMIntegralCurveAt
      (I := 𝓘(Real, Real × (E × E))) c vf s0 := by
    filter_upwards [hzsol] with s hs
    rw [hasMFDerivAt_iff_hasFDerivAt]
    simpa only [c, vf] using (hasDerivAt_id s).prodMk hs |>.hasFDerivAt
  have hc' : IsMIntegralCurveAt
      (I := 𝓘(Real, Real × (E × E))) c' vf s0 := by
    filter_upwards [hzsol'] with s hs
    rw [hasMFDerivAt_iff_hasFDerivAt]
  ...
  have heq :=
    isMIntegralCurveAt_eventuallyEq_of_contMDiffAt_boundaryless
      (I := 𝓘(Real, Real × (E × E))) hsection_c hc hc' hc0
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Two local solutions of the regularized L-phase ODE with the same regular
initial state agree as germs at square-root time zero. -/
...
/-- A differentiable chart-position path in the chart interior reconstructs
an intrinsically differentiable manifold curve. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [FiniteDimensional Real E] [T2Space M] [SigmaCompactSpace M] in
/-- Along a phase solution in the chart interior, the reconstructed phase
velocity is the actual velocity of the reconstructed manifold curve. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [FiniteDimensional Real E] [T2Space M] [SigmaCompactSpace M] in
/-- Differentiable chart position and velocity components give a genuine
differentiable reconstructed tangent field along the manifold curve. -/
...
/-- A phase solution in the chart interior reconstructs a manifold curve and
tangent field satisfying the intrinsic regularized L-acceleration equation. -/
theorem lPhase_accel
  ...
  have hq : HasDerivAt q (z s).2 s := by
    ...
    simpa only [q, lPhaseField] using h
  have hv : HasDerivAt v (lPhaseField S T x0 s (z s)).2 s := by
    simpa only [v] using hasFDerivAt_snd.comp_hasDerivAt s hz
  have htarget : ∀ᶠ r in 𝓝 s, q r ∈ (extChartAt I x0).target :=
    hq.continuousAt.eventually
      ((isOpen_extChartAt_target (I := I) x0).mem_nhds (interior_subset hpos))
  have hsource : alpha s ∈ (chartAt H x0).source := by
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- An intrinsic regularized L-acceleration solution becomes a solution of
the fixed-chart phase equation at every point contained in the chart. -/
theorem lRegCurve_phase
  ...
  have hqdiff : DifferentiableAt Real q s := by
    ...
    simpa only [q, chartCurve] using hcomp
  ...
  have hqcoord : deriv q s = v s := by
    ...
    simpa only [q, v, X, chartCurve, chartRepAtBase_apply, lVelocity] using
      hcoord.symm
  have hbase :
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- At every regular terminal time, each tangent vector determines a local
curve satisfying the intrinsic regularized L-acceleration equation, with the
Perelman normalization `A(0) = 2 Z`. -/
theorem exists_lRegCurve
  ...
  have hvel : Set.EqOn (fun s => lVelocity (I := I) alpha s) A
    ...
    have hq : HasDerivAt (fun r : Real => (z r).1) (z s).2 s := by
      ...
      simpa only [lPhaseField] using h
    simpa only [alpha, A] using
      lPhase_velocity (I := I) x z s hq hsdata.2.1
  have hzero : (0 : Real) ∈ Set.Ioo (-epsilon) epsilon := by
    constructor <;> simpa using hepsilon
  ...
  · intro s hs
    ...
    have hv : HasDerivAt (fun r : Real => (z r).2)
      ...
      simpa using hasFDerivAt_snd.comp_hasDerivAt s hzs
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- Two intrinsic regularized L-acceleration solutions with the same position
and velocity at a regular square-root time agree as germs there. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- Two intrinsic regularized L-acceleration solutions with the same initial
position and velocity agree as germs at square-root time zero. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Pairing the regularized acceleration with a test vector recovers the
gradient and Ricci terms in the square-root-time Euler equation. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The intrinsic L-Euler pairing in square-root time has the nonsingular
regularized form obtained from `A(s) = d/ds (gamma (s ^ 2))`. -/
theorem lEuler_sq
    (S : SolutionOn (I := I) (M := M) D) (T : Real) (gamma : Real → M)
  ...
  have hBcov :
    ...
    simpa only [alpha, B, sqReparam] using hcomp
  have hlin : HasDerivAt (fun r : Real => 2 * r) 2 s := by
    simpa only [zero_mul, zero_add, mul_one] using
      (hasDerivAt_const (x := s) (c := (2 : Real))).mul
        (hasDerivAt_id (x := s))
  have hAeq :
      (fun r : Real => lVelocity (I := I) alpha r) =ᶠ[𝓝 s]
        fun r : Real => (2 * r) • B r := by
    filter_upwards [eventually_gt_nhds hs] with r hr
    simpa only [alpha, B, X] using
      lVelocity_sq_pos (I := I) gamma r hr
  have hAcov :
    ...
    have hprod := covDerivAlong_smulFun (I := I) g alpha
      (fun r : Real => 2 * r) B s hlin.differentiableAt hBdiff
    rw [hlin.deriv, hBcov] at hprod
    simpa only [B] using hprod
  have hinner :
      g.inner (gamma (s ^ 2)) Y
          (covDerivAlong (I := I) g alpha
            (fun r : Real => lVelocity (I := I) alpha r) s) =
  ...
  have hric :
    ...
    have hAs : lVelocity (I := I) alpha s = (2 * s) • X (s ^ 2) := by
      simpa only [B] using hAeq.self_of_nhds
    rw [hAs]
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The intrinsic L-geodesic equation becomes the regularized acceleration
equation after the square-root reparameterization. -/
theorem HasLEquationAt.accel_sq
  ...
  apply metricFlatLinear_injective (I := I) g (gamma (s ^ 2))
  ext Y
  simp only [metricFlatLinear_apply]
  rw [g.symm (gamma (s ^ 2))
    (covDerivAlong (I := I) g alpha
      (fun r : Real => lVelocity (I := I) alpha r) s) Y]
  rw [g.symm (gamma (s ^ 2)) (lRegAccel S T s (gamma (s ^ 2)) A) Y]
