...
/-- On the inner half of an Rm-controlled flow ball, produce a Calabi distance
upper support whose comparison geodesic remains in the controlled outer ball. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- On the inner half of an Rm-controlled flow ball, produce the scaled
time-dependent Calabi distance upper support using only the outer ball. -/
