...
/-- The parameter length of one member of a finite ordered subdivision. -/
def lSegLen {m : ℕ} (t : Fin (m + 1) → Real) (i : Fin m) : Real :=
  t i.succ - t i.castSucc

omit [CompactSpace M] in
/-- Uniform convergence into buffered chart pieces eventually produces the
canonical coordinate-valued `timeH1` representatives on every piece. -/
theorem exists_chartH1_fin
  ...
  have hevent (i : Fin m) : ∀ᶠ n in atTop,
    ...
    have hgammaCont : Continuous
      ...
      (hgamma.mono (hpieceSub i)).restrict
    have hmap : MapsTo
  ...
  have hrep (i : Fin m) (n : ℕ) : EqOn (u i n).toFun
    ...
    simpa only [u, Function.comp_apply] using
      chartTimeH1_toFun I (sub_nonneg.mpr (hseg i)) (p i)
        (fun r ↦ alpha (n + N) (t i.castSucc + r))
        (hlocalMD i n) (hlocalSrc i n)
...
/-- A global regularized L-action bound yields one common chart-`H¹`
subsequence on every interval of a finite chart subdivision. -/
