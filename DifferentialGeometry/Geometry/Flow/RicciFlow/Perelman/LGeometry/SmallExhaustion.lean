...
/-!
# Small-time exhaustion of the strict L-injectivity domain

This module combines short-time minimizing-ray compactness with bounded-ball
endpoint injectivity to show that every fixed initial tangent is strictly
minimizing at all sufficiently small positive backward times.
-/
...
/-- Every fixed initial tangent lies in the strict minimizing L-injectivity
domain at all sufficiently small positive backward times. -/
theorem lInj_eventually
  ...
  have hBLim : Tendsto B atTop (𝓝 (0 : Real)) := by
    simpa only [B, Real.sqrt_zero] using
