...
/-!
# Uniform finite-chart splice bounds

This module takes finite suprema of the local chart constants supplied by the
reduced-length splice estimate.  A concrete minimizing ray can then be placed
in one chart while the resulting bound and target-set volume are independent
of that chart.
-/
...
/-- Finite chart suprema turn the local splice estimate for one concrete
minimizing ray into a chart-independent bound and a uniform positive volume
floor at a fixed forward time. -/
