...
/-!
# L-cost at unreachable endpoints

This module treats the branch where the global regularized `C¹` competitor
class is empty.  Reachability propagates back across a convex chart set by
compressing an existing curve and appending an inverse-chart affine segment.
-/
...
/-- Two regularized `C¹` pieces meeting at an interior time admit one global
`C¹` curve with the outer endpoint values. -/
...
omit [NeZero (Module.finrank Real E)] in
/-- A global `C¹` curve ending at one point of a convex chart set can be
moved to any other point of that set without changing its initial endpoint. -/
...
omit [NeZero (Module.finrank Real E)] [I.Boundaryless] [T2Space M]
  [CompactSpace M] in
/-- If no global regularized `C¹` curve reaches an endpoint at positive
square-root time, its raw L-cost is the empty infimum and hence zero. -/
...
omit [NeZero (Module.finrank Real E)] in
/-- Non-reachability by global regularized `C¹` curves persists throughout a
sufficiently small canonical-chart ball around the endpoint. -/
...
omit [NeZero (Module.finrank Real E)] in
/-- Near an unreachable chart endpoint, chart-coordinate L-cost is identically
zero and therefore Lipschitz with constant zero. -/
