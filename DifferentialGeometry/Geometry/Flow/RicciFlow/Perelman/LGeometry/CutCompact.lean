...
/-!
# Closedness of minimizing L-vectors at fixed time

This module proves the stability input used in the compactness step of the
L-cut alternative.  The terminal backward time is fixed; only the initial
tangent and hence the terminal point vary.
-/
...
/-- At a fixed positive backward time, a convergent sequence of minimizing
initial L-tangents has a minimizing limit whenever the limiting L-ray remains
in the positive regular domain. -/
theorem lMinVec_lim
  ...
  have hq : Tendsto q atTop (nhds y) := by
    simpa only [q, y] using hExpAt.tendsto.comp hpair
