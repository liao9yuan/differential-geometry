...
/-!
# Prefixes of regularized L-action minimizers

This file proves the cut-and-paste principle that a globally minimizing
regularized curve also minimizes on every proper prefix.
-/
...
/-- A global fixed-endpoint minimizer of the regularized L-action minimizes on
every proper prefix against curves that are C1 on that prefix. -/
...
/-- A closed-interval C1 curve that is no more expensive than every global C1
competitor realizes the global C1 regularized cost. -/
