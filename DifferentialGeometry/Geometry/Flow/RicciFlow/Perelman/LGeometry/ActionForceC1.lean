...
/-!
# Continuous representatives of the chart force

The weak chart force is evaluated on the raw time-`H¹` derivative.  Replacing
that derivative by a continuous representative gives a continuous force which
agrees almost everywhere with the original one.
-/
...
/-- The position covector obtained by evaluating the chart Lagrangian's
spatial derivative on a supplied coordinate-velocity representative. -/
...
/-- The Riesz vector of the position covector evaluated on a supplied
coordinate-velocity representative. -/
...
private theorem chartScalCov_basis
    ...
    chartScalCov (I := I) S p z (chartModelBasis E i) =
      ...
      extDerivFun (I := I) (S.scalar z.1) x
...
omit [NeZero (Module.finrank Real E)] [I.Boundaryless]
  [SigmaCompactSpace M] in
/-- Evaluating the chart force on a continuous velocity representative gives
a continuous force on every compact regular chart interval. -/
theorem lChartForceRep_cont
  ...
  have hcoord (i : Fin (Module.finrank Real E)) : ContinuousOn
    ...
    have hdir : ContinuousOn
      ...
      (hGramFd.clm_apply continuousOn_const).const_smul _
    have hdir' : ContinuousOn
        (fun r ↦ (1 / 2 : Real) •
          (fderiv Real (chartGramOp (I := I) S.family p)
  ...
  have hpos : ContinuousOn
    ...
    simpa only [lChartPosRep, tau] using
  ...
  have hadj : Continuous
    ...
    (ContinuousLinearMap.adjoint (E := E) (F := Real)).continuous.clm_apply
      continuous_const
  simpa only [lChartForceRep] using hadj.comp_continuousOn hpos

omit [NeZero (Module.finrank Real E)] [I.Boundaryless]
  [SigmaCompactSpace M] in
/-- A continuous velocity representative gives the same chart force almost
everywhere as the raw weak derivative. -/
