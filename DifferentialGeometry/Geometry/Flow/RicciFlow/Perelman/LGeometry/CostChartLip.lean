...
/-!
# Chart-local Lipschitz bounds for L-cost

This module converts terminal fixed-chart ramps into quantitative endpoint
comparisons for the fixed-time L-cost.
-/
...
/-- A two-piece regular curve is an honest upper competitor for fixed-time
L-cost when the fixed-endpoint `C¹` action class is bounded below. -/
...
/-- When the fixed-endpoint action class is bounded below, the L-cost to a
positive regularized-ray endpoint is no larger than the ray action. -/
theorem lCost_le_ray_bdd
  ...
  have hgammaInf : ContMDiff (modelWithCornersSelf Real Real) I ∞ gamma := by
    rw [← contMDiffOn_univ]
    simpa only [gamma, z] using
      (lRegCurve_smoothOn S hS T x).comp hpair.contMDiffOn
        (fun s _hs ↦ by simpa only [lRegJointDom, z] using hrho_range s)
...
/-- On a compact manifold, a two-piece regular curve is an honest upper
competitor for fixed-time L-cost; the node may have a velocity corner. -/
...
/-- Minimizing initial tangents reaching endpoints in a sufficiently small
coordinate neighborhood of one minimizing endpoint have uniformly bounded
model norms. -/
...
omit [CompactSpace M] in
/-- On a compact convex family of regularized L-rays lying in one chart, the
terminal chart displacement is uniformly linear in square-root time. -/
theorem lRayChart_bound
  ...
  have hQU : Q ⊆ U := fun q hq ↦ ⟨hdom hq, by
    simpa only [F, extChartAt_source] using hsrc hq⟩
  obtain ⟨V, hV⟩ :=
...
omit [CompactSpace M] in
/-- A compact family whose common terminal endpoints lie in one chart remains
in that chart on one uniform terminal square-root-time interval. -/
theorem lRayChart_tube
  ...
  have hev : ∀ᶠ s in nhds b, ∀ Z ∈ K, (Z, s) ∈ U := by
    ...
    intro Z hZ
    have hmem : (Z, b) ∈ U := ⟨by
      simpa only [lRegJointDom] using hbdom Z hZ, hbchart Z hZ⟩
  ...
  exact ⟨by simpa only [U, lRegJointDom, mem_inter_iff, F] using hmem.1,
    by simpa only [U, F] using hmem.2⟩

omit [NeZero (Module.finrank Real E)] [CompactSpace M] in
/-- The intrinsic regularized action of an inverse-chart affine ramp is its
single-piece chart action. -/
theorem lRampAct_eq
    (S : SolutionOn (I := I) (M := M) D) (hS : IsSolutionOn (I := I) S)
  ...
  have hsrc : ∀ i, MapsTo gamma (Icc (t i.castSucc) (t i.succ))
    ...
    subst i
    intro s hs
    have hs' : s ∈ Icc a b := by simpa only [t] using hs
  ...
  have hrep : ∀ i, EqOn (us i).toFun
    ...
    subst i
    intro r hr
    have hrL : r ∈ Icc (0 : Real) (b - a) := by
      simpa only [lSegLen, t] using hr
  ...
  simpa only [gamma, Fin.sum_univ_one, ps, us, lSegLen, t] using hsum

/-- Replacing the terminal part of a regularized L-ray by one inverse-chart
affine ramp gives an honest upper bound for endpoint L-cost. -/
...
/-- At every positive regular backward time, fixed-time L-cost is locally
Lipschitz in arbitrary canonical chart coordinates. -/
theorem lCost_chart_lip
  ...
  · obtain ⟨alpha, halpha, hstart, hend⟩ := hreach
    ...
    have hendCont : Continuous endMap := by
      ...
      have hendMD : ContMDiff 𝓘(Real, E) I ∞ endMap := by
        ...
        exact (lRegCurve_smoothOn S hS T x).comp hpair.contMDiffOn
          (fun Z _hZ ↦ by
            simpa only [lRegJointDom] using hbdomAll Z)
    ...
    have hZ0K : (Z0 : E) ∈ K := by
      refine ⟨⟨?_, hZ0min⟩, ?_⟩
      · simpa only [mem_closedBall_zero_iff, Real.norm_eq_abs,
          abs_norm] using hminBdd q0 (Metric.mem_ball_self hε0) Z0 hZ0min
          (by simpa only [hb2] using hZ0end)
      · refine ⟨q0, ⟨?_, ?_⟩⟩
        · exact Metric.mem_closedBall_self hρ.le
        · simpa only [endMap, hb2] using hZ0end.symm
    have hbchart : ∀ Z ∈ K,
    ...
    have hdomTail : K ×ˢ Icc (b - δ) b ⊆ lRegJointDom S T x := by
      intro q hq
      simpa only [lRegJointDom] using (htube q.1 hq.1 q.2 hq.2).1
    ...
    have hWqK : (Wq : E) ∈ K := by
      refine ⟨⟨?_, hWqmin⟩, ?_⟩
      · simpa only [mem_closedBall_zero_iff, Real.norm_eq_abs, abs_norm] using
          hminBdd q (hqρ.trans_le (min_le_left (ε0 / 2) _ |>.trans
            (half_le_self hε0.le))) Wq hWqmin (by simpa only [hb2] using hWqend)
      · refine ⟨q,
          ⟨Metric.ball_subset_closedBall hqρ, ?_⟩⟩
        simpa only [endMap, hb2] using hWqend.symm
    have hWrK : (Wr : E) ∈ K := by
      refine ⟨⟨?_, hWrmin⟩, ?_⟩
      · simpa only [mem_closedBall_zero_iff, Real.norm_eq_abs, abs_norm] using
          hminBdd r (hrρ.trans_le (min_le_left (ε0 / 2) _ |>.trans
            (half_le_self hε0.le))) Wr hWrmin (by simpa only [hb2] using hWrend)
      · refine ⟨r,
          ⟨Metric.ball_subset_closedBall hrρ, ?_⟩⟩
        simpa only [endMap, hb2] using hWrend.symm
      ...
      have hfull : lRegAction S T (lRegCurve S T x Wu) 0 b =
        ...
        calc
          ...
          _ = lCost S T x (lExp S T x Wu (b ^ 2)) (b ^ 2) := by
                simpa only [lExp, sqrtReparam] using hminEq
    ...
    simp only [Lip, NNReal.coe_mk, hb2] at hqr hrq ⊢
    rw [abs_le]
    constructor
    · have := hqr
  ...
  · obtain ⟨ε, hε, hlip⟩ := lCost_zero_lip (I := I) S hS T x b hb
    ...
    refine ⟨0, Metric.ball q0 ε,
      mem_nhdsWithin_of_mem_nhds (Metric.ball_mem_nhds q0 hε), ?_⟩
    simpa only [Function.comp_apply, hb2] using hlip
