import DifferentialGeometry.Geometry.Flow.RicciFlow.Perelman.LGeometry.ActionLower
import DifferentialGeometry.Geometry.Flow.RicciFlow.Perelman.LGeometry.ActionDensityGeom
...
/-!
# Strong approximation of the regularized L-action

This file assembles strong fixed-chart kinetic convergence and uniform scalar
convergence over a finite chart subdivision.  The geometric smooth-density
construction is added above this analytic bridge.
-/
...
private theorem timeH1_uniform
  ...
  have hnorm : Tendsto (fun n ↦ ‖u - v n‖) atTop (nhds 0) := by
    simpa only [norm_zero] using continuous_norm.tendsto (0 : timeH1 E L) |>.comp hsub
...
private theorem timeH1_deriv_lim
  ...
  simpa only [timeH1.timeDeriv_apply] using
    ((timeH1.timeDeriv E L).continuous.tendsto u).comp hv
...
theorem lAction_chart_lim
  ...
  have hsum : Tendsto
    ...
    exact tendsto_finset_sum Finset.univ fun i _ ↦ (hkin i).add (hpot i)
...
theorem lAction_c1_dense
  ...
  have hfc (i : Fin m) : IsCompact
    ...
    isCompact_range (uLim i).continuousOn_toFun.restrict
