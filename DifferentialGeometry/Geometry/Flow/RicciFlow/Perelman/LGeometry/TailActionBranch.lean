...
/-!
# Smooth positive-start tail action

This file supplies the analytic producer needed by the positive-start Calabi
branch.  A jointly smooth family of regularized L-curves has locally smooth
fixed-interval action as a function of its actual velocity parameter.
-/
...
private theorem tailVel_smooth
  ...
  have hunit : ContMDiff J J.tangent ∞
    ...
    simpa only [J, equivTangentBundleProd, Equiv.coe_fn_symm_mk] using
      hsymm.comp hpair
...
private theorem tailLag_smooth
  ...
  have hmetric : ContMDiffAt J
    ...
    simpa only [SolutionOn.family_metric] using hmetric₀.comp q harg
...
private theorem tailAct_bdry
  ...
  simpa only [hzero, map_zero, ContinuousLinearMap.zero_apply, hint,
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [I.Boundaryless] in
/-- The action of a jointly smooth positive-start family on a fixed compact
time interval is smooth near every parameter in its open parameter domain. -/
theorem lTailAct_smooth
  ...
  have hG : ContDiffOn Real ∞ (fun q : E × Real ↦ G q.1 q.2) univ := by
    ...
    simpa only [G] using (contDiffAt_const.mul hc).contDiffWithinAt
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The positive-start action of a jointly smooth L-family is jointly
differentiable in its parameter and upper endpoint. -/
theorem lTailAct_joint
  ...
  have hG : ContDiffOn Real 1
    ...
    simpa only [G] using (hlen.mul hcomp).contDiffWithinAt
  have hInt :=
  ...
  have hgammaData : ∀ s ∈ Icc a b,
    ...
    have hgammaInf : ContMDiffAt 𝓘(Real, Real) I ∞ gamma s := by
      simpa only [gamma] using halphaAt.comp s hincl
  ...
  have hAspEq : fderiv Real Asp A0 = Lsp := by
    ...
    have hendAt : ContMDiffAt 𝓘(Real, E) I ∞ endMap A0 := by
      ...
      simpa only [endMap] using halphaAt.comp A0 hincl
    ...
    have hline : HasDerivAt (fun u : Real ↦ Asp (zeta u))
      ...
      change fderiv Real Asp A0 Y = fderiv Real Asp A0
        (mfderiv 𝓘(Real, Real) 𝓘(Real, E) zeta 0 (1 : Real))
      rw [hzetaVel]
  ...
  have hfinal : HasFDerivAt Act
    ...
    rw [← hfd]
    exact hbase
  simpa only [Act, Lsp, lag, endMap] using hfinal

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Pulling the joint positive-start tail action through the endpoint-time
local inverse gives the spacetime covector with the kinetic time correction. -/
theorem lTailJoint_mfd
  ...
  have hAct : HasMFDerivAt J 𝓘(Real, Real) Act (A0, b) dAct := by
    simpa only [J, Act, dAct, g, Ab, Lb, endMap] using
      (lTailAct_joint S hS T a b hab hVopen hA0V hKopen hKconn
  ...
  have hComp : HasMFDerivAt L 𝓘(Real, Real)
    ...
    simpa only [Function.comp_apply] using
      hAct'.comp q0 hInv.hasMFDerivAt
  have hdBranch :
    ...
    have hright : mfderiv J L F (A0, b) v = w := by
      ...
      simpa only [v, hloc, q0, F] using hright'
    ...
    have hsndDeriv :
        mfderiv J 𝓘(Real, Real) (@Prod.snd E Real) (A0, b) =
          ContinuousLinearMap.snd Real E Real := by
      simpa only [J] using
        (mfderiv_snd : mfderiv
    ...
    have htime :
      ...
      change mfderiv 𝓘(Real, Real) I
          (fun r : Real ↦ alpha (A0, r)) b v.2 =
        v.2 • mfderiv 𝓘(Real, Real) I
          (fun r : Real ↦ alpha (A0, r)) b (1 : Real)
      simpa only [smul_eq_mul, mul_one] using
        (map_smul (mfderiv 𝓘(Real, Real) I
          (fun r : Real ↦ alpha (A0, r)) b) v.2 (1 : Real))
    have hsum :
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The fixed tail action, pulled back by the native endpoint local inverse,
is smooth on an open neighborhood of the central terminal point. -/
...
/-- A curvature-bounded minimizing L-ray has a smooth fixed-head upper support
for endpoint L-cost near its terminal point. -/
  ...
  have hfullCost : lRegAction S T gamma 0 b =
    ...
    calc
      ...
      _ = lCost S T x (lExp S T x Z tau) tau := by
        simpa only [gamma, lExp, sqrtReparam] using hvec.2
