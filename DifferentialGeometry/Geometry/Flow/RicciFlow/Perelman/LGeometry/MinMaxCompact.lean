...
/-!
# Compact-slab bounds for Perelman L-rays

This module produces the uniform scalar-gradient and Ricci constants used by
the Morgan--Tian min--max estimate on a compact regular time slab.
-/
...
/-- On a compact regular time slab, the spatial scalar-curvature gradient has
a uniform norm bound in the moving metric. -/
theorem lGrad_bound
  ...
  have hgram : ∀ (x₀ : M) (i j : Fin (Module.finrank Real E)),
    ...
    intro p hp
    simp only [frame, chartGramMatrix_apply, SolutionOn.family_metric]
  ...
  have hfcont : ContinuousOn f K := by
    ...
    simpa only [f, Function.comp_apply] using hcomp
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- On a compact regular time slab, the Ricci quadratic form is uniformly
bounded by the moving metric. -/
theorem lRicci_bound
  ...
  have hA : Tensor0SFamilyContinuousOnSet (I := I) (M := M) 2
      (Set.Icc t0 t1) A := by
    simpa only [A, SolutionOn.ricci, SolutionFamily.ricci_apply] using
