...
/-!
# Momentum matching across an L-action chart node

A short head of the right segment is rewritten in the left chart.  Same-chart
momentum matching and tangent-coordinate naturality then give the intrinsic
corner equation in the two original charts.
-/
...
/-- At a two-chart node of a fixed-endpoint L-action minimizer, the two
coordinate momenta agree after tangent-coordinate transport. -/
theorem lNode_mom_match
  ...
  have hcmpHead : ∀ v0 : timeH1 E (t 1 - t 0),
    ...
    exact lNodeHead_min (I := I) S hS.smoothMetric hSc T
      ...
      (by simpa only [gammaTail, Function.comp_apply] using huTail)
      hreg hmin v0 v1 hv0tar hv1tar hv0 hv2 hvnode
  let th : Fin 3 → Real := ![t 0, t 1, c]
  let uh : (i : Fin 2) → timeH1 E (lSegLen th i) :=
    Fin.cases (by simpa only [th, lSegLen] using u 0)
      (Fin.cases (by simpa only [th, lSegLen] using uHead) (fun i ↦ Fin.elim0 i))
  have hthpos : ∀ i : Fin 2, th i.castSucc < th i.succ := by
    intro i
    fin_cases i
    · simpa only [th] using hpos 0
    · simpa only [th] using h1c
  ...
  have huhchart : ∀ i, MapsTo (uh i).toFun
    ...
    intro i r hr
    fin_cases i
    · simpa only [uh, th, lSegLen] using hchart 0 hr
  ...
  have huhnode : (extChartAt I (p 0)).symm
    ...
    simpa only [uh, th, lSegLen] using hleft'.trans hright.symm
  have huhcmp : ∀ v : (i : Fin 2) → timeH1 E (lSegLen th i),
    ...
    have hv0' : (extChartAt I (p 0)).symm ((v 0).toFun 0) = gamma (t 0) := by
      rw [hv0]
      simpa only [uh, th, lSegLen, hfin0c, add_zero] using hrecover 0 0
        ⟨le_rfl, by
          simpa only [lSegLen, hfin0s] using sub_nonneg.mpr (hpos 0).le⟩
    ...
    have hhead := hcmpHead
      (by simpa only [th, lSegLen] using v 0)
      (by simpa only [th, lSegLen] using v 1)
      (by simpa only [th, lSegLen] using hvtar 0)
      (by simpa only [th, lSegLen] using hvtar 1)
      hv0' hv2' (by simpa only [th, lSegLen] using hvnode)
    simpa only [Fin.sum_univ_two, uh, th, lSegLen] using hhead
  ...
  have hmom' : chartGramOp (I := I) S.family (p 0)
    ...
    simpa only [th, uh, lSegLen] using hmom
  have hheadDeriv : derivWithin (u 1).toFun
    ...
    have hchange := chartDeriv_head I (sub_pos.mpr h1c)
      (by simpa only [lSegLen] using sub_le_sub_right hc2 (t 1))
