...
/-!
# Hamilton-K reduced-length Laplacian bound

This file substitutes the integrated Hamilton trace identity into the strict
reduced-length Laplacian comparison.
-/
...
/-- On the strict minimizing region, the reduced-length Laplacian satisfies
the explicit Hamilton-`K` comparison. -/
theorem redLength_lap_K
  ...
  have hONb : ∀ i j,
    ...
    simpa only [b, hb_sq, lExp] using hON
...
/-- The positive-start tail-action branch satisfies Hamilton's explicit
tail-`K` Laplacian comparison along the native minimizing L-ray. -/
theorem lTail_lap_K
