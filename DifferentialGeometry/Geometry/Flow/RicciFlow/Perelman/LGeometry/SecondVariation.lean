...
/-!
# Perelman L-index form

This file starts the second-variation layer in positive backward time.  The
dynamic first covariant derivative uses the metric at the evaluation time.
The paired residual is written so that its Green identity is intrinsic and
the resulting index integrand is symmetric in its two variation fields.
-/
...
private theorem cov_sq_smul
  ...
  have hBcov :
    ...
    simpa only [B, sqReparam] using hcomp
...
/-- The dynamic covariant velocity of a field along a backward-time curve. -/
...
/-- The dynamically differentiated L-Jacobi residual paired with a test
vector.  The three Ricci-derivative terms account for the connection part of
the backward-time Green formula. -/
...
private theorem lPair_second_eq
  ...
  have halpha_ev : ∀ᶠ r in nhds s,
    ...
    simpa only [alpha, sqReparam] using
      hgamma.comp (f := fun u : Real ↦ u ^ 2) r hsquare
  ...
  have hP_eq : P =ᶠ[nhds s]
    ...
    simpa only [P, alpha, Yb, DY, lJacobiVel, sqReparam] using hcomp
  ...
  have hPcov :
    ...
    simpa only [q, alpha, tau] using
      cov_sq_smul (I := I) q gamma DY P s hgamma0
  ...
  have hPinner :
    ...
    let D2 := covDerivAlong (I := I) q gamma DY tau
    have hadd :
        q.inner (alpha s) (Wb s)
            ((2 : Real) • DY tau + (2 * s) • ((2 * s) • D2)) =
          q.inner (alpha s) (Wb s) ((2 : Real) • DY tau) +
            q.inner (alpha s) (Wb s) ((2 * s) • ((2 * s) • D2)) :=
      (q.inner (alpha s) (Wb s)).map_add _ _
    have htwo :
        q.inner (alpha s) (Wb s) ((2 : Real) • DY tau) =
          2 * q.inner (alpha s) (DY tau) (Wb s) := by
      calc
        _ = 2 * q.inner (alpha s) (Wb s) (DY tau) := by
          simpa only [smul_eq_mul] using
            (q.inner (alpha s) (Wb s)).map_smul (2 : Real) (DY tau)
        _ = _ := by rw [q.symm (alpha s) (Wb s) (DY tau)]
    have hnested :
        q.inner (alpha s) (Wb s) ((2 * s) • ((2 * s) • D2)) =
          (2 * s) * ((2 * s) * q.inner (alpha s) D2 (Wb s)) := by
      calc
        _ = (2 * s) * q.inner (alpha s) (Wb s) ((2 * s) • D2) := by
          simpa only [smul_eq_mul] using
            (q.inner (alpha s) (Wb s)).map_smul
              (2 * s) ((2 * s) • D2)
        _ = (2 * s) * ((2 * s) * q.inner (alpha s) (Wb s) D2) := by
          congr 1
          simpa only [smul_eq_mul] using
            (q.inner (alpha s) (Wb s)).map_smul (2 * s) D2
        _ = _ := by rw [q.symm (alpha s) (Wb s) D2]
    change q.inner (alpha s) (Wb s)
      ((2 : Real) • DY tau + (2 * s) • ((2 * s) • D2)) = _
  ...
  simpa only [q, tau, alpha, Yb, Z, DY, A, N, C, hWs, sqReparam]
    using hsecond

omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)] in
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The dynamically differentiated L-Jacobi residual becomes the regularized
residual after the positive square-root-time reparameterization. -/
theorem lJacobiPair_sq
  ...
  have hZs :
    ...
    have hsqderiv : deriv (fun r : Real ↦ r ^ 2) s = c := by
      ...
      norm_num
    rw [hsqderiv] at hcomp
    simpa only [q, alpha, Yb, c, DY, lJacobiVel, tau, sqReparam] using hcomp
...
/-- Pointwise regularity and the dynamic L-Jacobi equation in backward time. -/
def HasLJacobiAt
    (S : SolutionOn (I := I) (M := M) D) (T : Real)
    (gamma : Real → M) (Y : ∀ tau, TangentSpace I (gamma tau))
    (tau : Real) : Prop :=
...
/-- A field satisfies the dynamic L-Jacobi equation on a backward-time set. -/
def IsLJacobi
    (S : SolutionOn (I := I) (M := M) D) (T : Real)
    (gamma : Real → M) (Y : ∀ tau, TangentSpace I (gamma tau))
    (J : Set Real) : Prop :=
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Frozen square-root-time covariant-derivative regularity implies
differentiability of the dynamically metric-dependent L-Jacobi velocity at
...
theorem lJacobiVel_sq_diff
  ...
  have hroot_eq :
    ...
    simp only [Function.comp_apply, alpha, B, tau, sqReparam,
      chartRepAt_apply]
    rw [Real.sq_sqrt hu.le]
    rfl
  simpa only [DY, tau] using hcomp.congr_of_eventuallyEq hroot_eq.symm

omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)] in
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- A regularized L-Jacobi field descends at positive square-root time to a
dynamic L-Jacobi field. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The initial-tangent differential of the L-exponential map is a dynamic
L-Jacobi field at every point of its positive joint domain. -/
theorem lExp_jacobi
    (S : SolutionOn (I := I) (M := M) D)
  ...
  have hgamma_sq : ∀ᶠ r in nhds s,
    ...
    simpa only [gamma, alpha, lExp, Function.comp_apply] using hcomp
  have hY_sq : ∀ᶠ r in nhds s,
    ...
    have heq : chartRepAt (I := I) gamma Y (r ^ 2) =
      ...
      simp only [chartRepAt_apply, Function.comp_apply]
      rw [hbase, hcurve, hYq]
    rw [heq]
    exact hcomp
  ...
  have halphaInf : ContMDiffAt (modelWithCornersSelf Real Real) I ∞ alpha s := by
    ...
    have hcomp := (lRegCurve_smooth (I := I) (M := M) S hS T x hsdom).comp
      s hparam
    simpa only [alpha, Function.comp_apply] using hcomp
...
omit [InnerProductSpace Real E] in
/-- The transverse derivative of the dynamic Euler pairing is the dynamic
L-Jacobi pairing plus the Euler residual paired with the test-field
acceleration. -/
theorem lEuler_var_full
  ...
  have hY_sq : ∀ᶠ r in nhds s,
    ...
    simpa only [gamma, Y, lVelocity, varFst] using
      variationField_chartRep_differentiableAt
        (I := I) (S.base.metric (T - s ^ 2)) f hf (r ^ 2)
  ...
  have hZ : DifferentiableAt Real
    ...
    simpa only [F, gamma, Y, sqReparam] using hZraw
  have hDY := lJacobiVel_sq_diff (I := I) S hS T gamma Y s hs ht
    hgamma_sq hY_sq (by simpa only [F, gamma, sqReparam] using hA) hZ
  have hpairRaw := lJacobiPair_sq (I := I) S hS T gamma Y s hs ht
    hgamma_sq hY_sq (by simpa only [F, gamma, sqReparam] using hA)
    hDY hZ (W 0)
  have hpair :
      c * lJacobiPair S T gamma Y (s ^ 2) (W 0) =
        lRegJacobiPair S T (F 0)
          (fun r ↦ lVelocity (I := I) (fun u ↦ F u r) 0) s (W 0) := by
    simpa only [c, F, gamma, Y, sqReparam] using hpairRaw
  have hcurve (u : Real) :
  ...
  have htail :
      c * lEulerPair S T gamma (s ^ 2) DW =
        lRegEulerPair S T (F 0) s
          (covDerivAlong (I := I) (S.base.metric (T - s ^ 2))
            (fun u ↦ F u s) W 0) := by
    simpa only [c, F, gamma, sqReparam, DW] using
  ...
  have hscale (u : Real) :
    ...
    simpa only [c, F, sqReparam] using
...
omit [InnerProductSpace Real E] in
/-- The dynamic Euler variation formula at an arbitrary positive backward
time. -/
...
omit [InnerProductSpace Real E] in
/-- At a central L-geodesic, the dynamic Euler variation has no
test-field-acceleration correction. -/
...
omit [InnerProductSpace Real E] in
/-- At a central L-geodesic, the transverse derivative of the dynamic Euler
pairing is the dynamic L-Jacobi pairing. -/
...
omit [InnerProductSpace Real E] in
/-- The integral of the weighted Euler residual has the expected transverse
derivative along a central L-geodesic. -/
theorem lEulerInt_deriv
  ...
  have hJEq : Set.EqOn (dF 0) J (Set.uIcc a b) := by
    ...
    have hW : DifferentiableAt Real
      ...
      simpa only [W, lVelocity] using
        DifferentialGeometry.Geometry.Riemannian.MFDerivAlongCurve.velocity_coord_diff
          (I := I) (fun u : Real => f u tau) 0 hslice
...
omit [InnerProductSpace Real E] in
/-- The second derivative of L-length along a fixed-endpoint variation is the
integral of the central L-Jacobi pairing. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- The squared norm of a variation field is jointly `C²` when metric time and
curve time are kept independent. -/
  ...
  have hYone : ContMDiff 𝓘(Real, Real) (I.prod 𝓘(Real, E)) (7 : ℕ)
    ...
    have hcomp := hbase.comp
      (contMDiff_id.prodMk contMDiff_const : ContMDiff 𝓘(Real, Real)
        ...
        (fun tau : Real => (tau, u)))
    simpa only [lVelocity] using hcomp
...
private theorem lVarNorm_c2
  ...
  simpa only [Q, V, Function.comp_apply] using hout
...
private theorem lVarRicci_c1
  ...
  have heq : Set.EqOn ric raw V := by
    ...
    have htimeQ : HasDerivAt (fun s : Real => Q (s, tau))
      ...
      simpa only [Q, Y, SolutionOn.family_metric, Function.comp_apply,
        mul_neg, neg_mul, neg_neg, mul_one] using htime
    have hdQEq :
        dQ (tau, tau) =
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The dynamic covariant derivative of the central variation field is
differentiable at every positive regular backward time. -/
theorem lVarJacobiVel_diff
  ...
  have hY_sq : ∀ᶠ r in nhds s,
    ...
    simpa only [gamma, Y, lVelocity, varFst] using
      variationField_chartRep_differentiableAt
        (I := I) (S.base.metric (T - tau)) f hf (r ^ 2)
...
/-- The moving-metric pairing of the covariant derivative of a variation
field with that field is `C¹` at regular backward times. -/
  ...
  have heq : Set.EqOn inner raw V := by
    ...
    have hY : DifferentiableAt Real
      ...
      have hout : DifferentiableAt Real
        ...
        simpa only [lVelocity] using
          variationField_chartRep_differentiableAt
            (I := I) (S.base.metric (T - tau)) fu hfu tau
...
/-- The pointwise symmetric L-index integrand in positive backward time. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] in
/-- The pointwise L-index integrand is symmetric in its two fields. -/
theorem lIndexInt_symm
  ...
  have hreal : Rm04RealizesConnection (I := I) g
      (LeviCivita (I := I) g) (S.base.rm04 t) := by
    simpa only [SolutionFamily.rm04, SolutionFamily.connection] using
      (metricCurvData (I := I) (M := M) g).h_rm04
  have hpair :=
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless]
  [SigmaCompactSpace M] in
/-- The diagonal L-index density in the standard second-variation form. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- Pointwise balance between the L-index density, the Jacobi residual, and
the moving-metric derivative pairing at positive backward time. -/
...
/-- Perelman's bilinear L-index form on an oriented backward-time interval. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] in
/-- The L-index form is symmetric in its two fields. -/
theorem lIndex_symm
    (S : SolutionOn (I := I) (M := M) D) (T : Real)
    (gamma : Real → M)
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless]
  [SigmaCompactSpace M] in
/-- The L-index form vanishes on a zero interval. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless]
  [SigmaCompactSpace M] in
/-- The L-index form is additive across adjacent intervals under the honest
integrability hypotheses on the two pieces. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Green's identity for the dynamic L-Jacobi residual on a positive compact
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- If the test field vanishes at both endpoints, Green's identity has no
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Along a dynamic L-Jacobi field, the L-index form is the weighted boundary
pairing. -/
...
omit [InnerProductSpace Real E] in
/-- The fixed-endpoint second variation of L-length along an L-geodesic is
twice the L-index form of its central variation field. -/
theorem lLength_second_var
  ...
  have hY : ∀ tau ∈ Set.uIcc a b,
    ...
    simpa only [gamma, Y, lVelocity, varFst] using
      variationField_chartRep_differentiableAt
        (I := I) (S.base.metric (T - tau)) f hf tau
