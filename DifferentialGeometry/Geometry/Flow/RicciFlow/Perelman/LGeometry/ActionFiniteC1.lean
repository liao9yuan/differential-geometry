...
/-!
# Global C1 regularity of finite L-action minimizers

Piecewise regularity and finite-node velocity matching combine to remove every
corner of a positive finite chart-`H¹` realization.
-/
...
private theorem lFinCurve_c1_aux
  ...
  have htNat : ∀ k, k < m + 2 → tn k < tn (k + 1) := by
    intro k hk
    let i : Fin (m + 2) := ⟨k, hk⟩
    rw [htn k (by omega), htn (k + 1) (by omega)]
    simpa only [i] using hpos i
  have hpieceNat : ∀ k, k < m + 2 →
    ...
    let i : Fin (m + 2) := ⟨k, hk⟩
    rw [htn k (by omega), htn (k + 1) (by omega)]
    simpa only [i] using hpieceFin i
  have hnodeNat : ∀ k, 0 < k → k < m + 2 →
    ...
    have hp0der : derivWithin ((extChartAt I (p i)) ∘ gamma)
      ...
      have hshift := chartDeriv_shift
        ...
        (by intro r hr; simpa only [Function.comp_apply] using hrep i hr)
    ...
    have hp1der : derivWithin ((extChartAt I (p j)) ∘ gamma)
      ...
      have hshift := chartDeriv_shift
        ...
        (by intro r hr; simpa only [Function.comp_apply] using hrep j hr)
  ...
  | succ n =>
      ...
      | zero =>
          ...
          have hleft : t (0 : Fin 1).castSucc = a := by
            simpa only using ht0
          have hright : t (0 : Fin 1).succ = b := by
            simpa only using htlast
