...
/-!
# Fixed-chart realization of the regularized kinetic energy

This file identifies the kinetic part of Perelman's regularized action with a
quadratic form on the weak derivative of a fixed-chart `timeH1` representative.
The manifold curve is required to be differentiable only almost everywhere on
...
/-- A local fixed-chart `timeH1` representative makes the manifold kinetic
density interval integrable on the represented subinterval. -/
...
/-- The global-representative kinetic integrability theorem is the local result
applied to `timeH1.slice`. -/
