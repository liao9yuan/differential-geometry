...
/-!
# The open minimizing L-injectivity domain

This module isolates the strict pre-cut part of the minimizing L-domain.  It
is the domain on which fixed-time L-exponential rays remain minimizing for a
strictly later time.
-/
...
/-- Initial L-tangents whose ray remains minimizing through a strictly later
backward time. -/
def lInjDomain
...
/-- The strict minimizing L-injectivity domain is open at every displayed
backward time. -/
theorem lInj_isOpen
  ...
  letI : ProperSpace (TangentSpace I x) := FiniteDimensional.proper Real _
  ...
  have hWExpLim : Tendsto (fun n ↦ lExp S T x (W (phi n)) rho) atTop
    ...
    simpa only [Function.comp_apply] using hWExpAt.tendsto.comp hWpair
  have hVExpLim : Tendsto (fun n ↦ lExp S T x (V (phi n)) rho) atTop
      (nhds (lExp S T x Z rho)) := by
    simpa only [Function.comp_apply] using hVExpAt.tendsto.comp hVpair
  ...
  have hEq : W (phi n) = V (phi n) := by
    ...
    exact hWend (phi n)
  apply hVnot (phi n)
  exact ⟨rho, htRho, by simpa only [hEq] using hWmin (phi n)⟩

/-- Every point of the strict minimizing domain is a nonconjugate positive
L-exponential point at the displayed backward time. -/
theorem lInj_local
    (S : SolutionOn (I := I) (M := M) D) (hS : IsSolutionOn (I := I) S)
...
/-- The fixed-time L-exponential map is injective on initial tangents whose
rays remain minimizing for a strictly later time. -/
theorem lInj_inj
    (S : SolutionOn (I := I) (M := M) D) (hS : IsSolutionOn (I := I) S)
