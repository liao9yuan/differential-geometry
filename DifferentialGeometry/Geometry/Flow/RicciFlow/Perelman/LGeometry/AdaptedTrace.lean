...
/-!
# Trace of the adapted L-index identity

This file contracts the single-field adapted-index formula over a terminal
orthonormal basis.
-/
...
/-- The finite trace of the adapted-index identity contracts its terminal
metric term to the dimension and its Ricci term to scalar curvature. -/
theorem lIndex_trace
  ...
  have hONs (s : Real) (hs : s ∈ Set.Icc (0 : Real) b)
    ...
    exact hON i j
  have hRic (s : Real) (hs : s ∈ Set.Icc (0 : Real) b) :
      ∑ i : Fin (Module.finrank Real E),
          S.ricciAt (T - s ^ 2) (alpha s) (vec2 (P i s) (P i s)) =
...
omit [InnerProductSpace Real E] in
/-- The positive-start adapted-index trace for the endpoint test fields
`W_i(s) = ((s - a) / (b - a)) P_i(s)`. -/
theorem lIndex_trace_pos
    (S : SolutionOn (I := I) (M := M) D)
    (hS : IsSolutionOn (I := I) S) (T : Real)
    (alpha : Real → M)
    (P : Fin (Module.finrank Real E) → ∀ s, TangentSpace I (alpha s))
