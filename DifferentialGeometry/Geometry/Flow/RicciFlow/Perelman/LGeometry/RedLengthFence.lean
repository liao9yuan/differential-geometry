...
/-!
# A dimension-scale reduced-length point

The spatial minimum of reduced length starts below the dimensional threshold.
Perelman's weak spacetime upper supports force it to remain below that threshold
throughout every complete bounded-curvature backward slab.
-/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- On a complete bounded-curvature backward slab, every positive interior
time has an endpoint whose reduced length is at most half the dimension. -/
