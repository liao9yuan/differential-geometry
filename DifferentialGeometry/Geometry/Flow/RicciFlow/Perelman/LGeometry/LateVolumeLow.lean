...
/-!
# Uniform late-time reduced-volume floor

A low reduced-length spatial minimizer on one fixed forward slice extends to
an earlier fixed slice. The fixed-slice volume estimate and reduced-volume
monotonicity then give one positive floor for every later terminal time and
every shorter positive backward time.
-/
...
/-- On a compact regular half-open forward slab, reduced volume has one
positive lower bound for every later terminal time, basepoint, and positive
backward time ending no earlier than the fixed initial slice. -/
