...
/-!
# Fixed-slice uniform splice bounds

This module specializes the finite-chart splice estimate to two fixed forward
times.  Rationalizing the difference of square roots supplies the uniform gap
needed on a half-open interval of terminal times.
-/
...
/-- The square-root separation associated to two fixed slices has a uniform
positive lower bound when the terminal time stays below a fixed ceiling. -/
...
/-- On two fixed forward slices, the finite-chart splice estimate has one
terminal-time-independent reduced-length bound.  The theorem consumes a
concrete minimizing ray at the later slice. -/
