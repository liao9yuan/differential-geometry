...
/-!
# The weak Euler equation for the nonlinear local L-action

This file differentiates the genuine fixed-chart L-action along affine
time-`H¹` variations.  Compact chart tubes supply all measurability and common
`L¹` domination, and an actual fixed-endpoint local minimum yields the weak
Euler identity with the native chart force.
-/
...
private theorem lWeakScal_fderiv
  ...
  simpa only [lWeakScal, f, smul_eq_mul] using hscaled

set_option maxHeartbeats 800000 in
-- Compact affine-tube composition of the joint scalar covector and its
-- evaluation on a time-H1 variation exceeds the default elaboration budget.
...
set_option maxHeartbeats 800000 in
-- The scalar spacetime continuity and chart-inverse composition remain costly
-- to elaborate in the generic manifold context.
omit [NeZero (Module.finrank Real E)] [I.Boundaryless]
  [SigmaCompactSpace M] in
private theorem lWeakScalLine_cont
  ...
  have htau' : Continuous tau' := by
    exact continuous_induced_rng.mpr htau.restrict
  have hchart' : Continuous (fun q : K ↦
      (extChartAt I p).symm (pos q.1)) := hchart.restrict
  have hscalar' : Continuous (fun q : K ↦
      S.scalar (tau q.1.2) ((extChartAt I p).symm (pos q.1))) := by
    simpa only [tau'] using
      hSc.continuous_subtype.comp (htau'.prodMk hchart')
  exact (hweight.restrict.mul hscalar').congr fun _ ↦ rfl
  ...
  have hlagInt : ∀ c ∈ Icc (-1 : Real) 1,
    ...
    have hs : ContinuousOn (lag c) (Icc (0 : Real) L) := by
      simpa only [Function.comp_apply] using
        hlagJoint.comp (continuousOn_const.prodMk continuousOn_id)
          (fun r hr ↦ ⟨hc, hr⟩)
  ...
  exact intervalIntegral.hasDerivAt_integral_of_dominated_loc_of_deriv_le
      ...
      (by
        have hs : ContinuousOn (dLag 0) (Icc (0 : Real) L) := by
          simpa only [Function.comp_apply] using
            hdJoint.comp (continuousOn_const.prodMk continuousOn_id)
...
private theorem lWeakScal_line
  ...
  · convert hparam.2.2.congr_of_eventuallyEq (Eventually.of_forall hlineEq) using 1
...
private theorem coeff_lag_meas
  ...
  simpa only [timeMeasure, uIoc_of_le hL,
    restrict_Ioc_eq_restrict_Icc, coeffLineLag] using hm

set_option maxHeartbeats 2000000 in
-- Uniform coefficient and derivative bounds on the compact affine tube make
-- this analytic producer exceed the default heartbeat budget.
...
private theorem coeff_int_deriv
  ...
  have hB_on : ∀ᵐ r ∂volume, r ∈ uIoc (0 : Real) L →
    ...
    simpa only [lWeakCoeff, lWeakCoeffD, tau] using
  ...
  have hlagInt := coeff_lag_int hL
    ...
    (fun q hq ↦ by simpa only [CBn, pos] using hCB q hq)
  ...
  have hA0b : ∀ᵐ r ∂timeMeasure L,
    ...
    filter_upwards [ae_restrict_mem measurableSet_Icc] with r hr
    simpa only [CBn, pos, zero_smul, add_zero] using
      hCB (0, r) ⟨by norm_num, hr⟩
  ...
  have hboundInt : IntervalIntegrable bound volume 0 L := by
    ...
    simpa only [bound, timeMeasure] using (huSq.add hvSq).const_mul C
...
private theorem lWeak_pos_pair
  ...
  have hscal :
      lWeakScalCov (I := I) S T a p r (u.toFun r) w =
        ∑ i : Fin (Module.finrank Real E),
          2 * (a + r) ^ 2 *
              extDerivFun (I := I) (S.scalar (T - (a + r) ^ 2))
                ((extChartAt I p).symm (u.toFun r))
                (chartBasisVecFiber (I := I) p i
                  ((extChartAt I p).symm (u.toFun r))) *
            chartCoordCLM E i w := by
    rw [lWeakScalCov, ContinuousLinearMap.smul_apply, smul_eq_mul, hcovDecomp]
    rw [Finset.mul_sum]
    apply Finset.sum_congr rfl
    intro i _
    simp only [f, y]
  ...
  · have hcoord (i : Fin (Module.finrank Real E)) :
    ...
    change (1 / 2 : Real) * inner Real
        ...
        (u.deriv r) = _
    rw [map_sum, ContinuousLinearMap.sum_apply, sum_inner]
...
set_option maxHeartbeats 800000 in
-- Compact-tube domination and two interval differentiations make this proof
-- exceed the default heartbeat budget in the generic manifold context.
omit [SigmaCompactSpace M] in
/-- The genuine nonlinear fixed-chart `L`-action is differentiable along an
affine `timeH1` line whose compact unit tube remains in the chart buffer. -/
theorem lChartAct_line
  ...
  rw [← lWeak_pos_pair (I := I) S hS T a p u r (v.toFun r)
    (hreg r hr') hx]
  simp only [lWeakCoeff, ContinuousLinearMap.smul_apply,
    real_inner_smul_left]
  ring

set_option maxHeartbeats 800000 in
-- The scaled chart tube and the local-minimum assembly are costly to elaborate
-- after expanding the two nonlinear line derivatives.
omit [SigmaCompactSpace M] in
/-- An actual fixed-endpoint local minimizer of the nonlinear fixed-chart
`L`-action satisfies the weak Euler identity with the native chart force. -/
        ...
        inner Real (lChartForce (I := I) S T a p u r) (w.toFun r) +
          inner Real
            (chartGramOp (I := I) S.family p
              (T - (a + r) ^ 2, u.toFun r) (u.deriv r))
            (w.deriv r)) 0 := by
    simpa only [Function.comp_apply, line] using hline
