...
/-!
# Small-time injectivity of the regularized L-endpoint

This module proves that the regularized L-endpoint map is injective on every
fixed bounded tangent ball for all sufficiently small positive square-root
times.  The proof uses the removable endpoint quotient and compact-uniform
convergence of its spatial derivative to the identity.
-/
    ...
    have hderiv : ∀ t ∈ Set.uIcc (0 : Real) b,
      ...
      have hcomp : HasFDerivAt (fun s : Real ↦ Phi (z, s))
        ...
        simpa only [Function.comp_apply] using
          hpat.comp t (hasFDerivAt_prodMk_right z t)
...
/-- On every bounded tangent ball, the regularized L-endpoint map is injective
at all sufficiently small positive square-root times. -/
theorem lEnd_inj_small
  ...
  have hK0 : K ×ˢ ({0} : Set Real) ⊆ U := by
    ...
    rw [show F (W, 0) = x by simp only [F, lRegCurve_zero]]
    ...
    have hchart : ContMDiffOn (𝓘(Real, E).prod 𝓘(Real, Real))
      ...
      (contMDiffOn_extChartAt (I := I) (n := 2) (x := x)).comp
        ((hFall.of_le (by decide :
          ...
          inter_subset_right
    exact (contMDiffOn_const.smul (hchart.sub contMDiffOn_const))
  ...
  have hzero : ∀ W ∈ V, Phi (W, 0) = 0 := by
    intro W _
    simp only [Phi, F, lRegCurve_zero, sub_self, smul_zero]
  have hlaunch : ∀ W ∈ V,
    ...
    have hchartvel :
      ...
      simpa only [F, lVelocity] using lRegCurve_vel_zero S hS T x W hT
    have hcurveM : ContMDiffAt (modelWithCornersSelf Real Real) I 1
      ...
      simpa only [F] using
        (lRegCurve_smoothAt S hS T x W hT).comp (0 : Real)
          (contMDiff_const.prodMk contMDiff_id).contMDiffAt |>.of_le (by norm_num)
    ...
    have hextD : HasMFDerivAt I (modelWithCornersSelf Real E)
      ...
      exact hm
    have hcurve0 : F (W, 0) = x := by simp only [F, lRegCurve_zero]
    have hextD' : HasMFDerivAt I (modelWithCornersSelf Real E)
    ...
    have hchart : HasFDerivAt (fun s : Real ↦ (extChartAt I x) (F (W, s)))
      ...
      simpa only [Function.comp_apply, ContinuousLinearMap.id_comp] using
        hasMFDerivAt_iff_hasFDerivAt.mp hm
    ...
    let v0 : E :=
      (mfderiv (modelWithCornersSelf Real Real) I
        (fun s : Real ↦ lRegCurve S T x W s) 0) 1
    have hv0 : v0 = (2 : Real) • W := by
      simpa only [v0] using hchartvel
    have heq1' : (fderiv Real Phi (W, 0)) ((0 : E), (1 : Real)) =
        (1 / 2 : Real) • v0 := by
      simpa only [F, v0] using heq1
