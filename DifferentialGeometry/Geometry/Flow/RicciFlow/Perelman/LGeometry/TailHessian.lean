...
/-!
# Positive-start tail Hessian

This file identifies the Hessian of the positive-start endpoint action branch
with the terminal covariant derivative of its parameter-variation field.
-/
...
private theorem lTail_mfd_at
  ...
  have hcancel :
    ...
    simpa only [Function.comp_apply, A, mfderiv_id] using
  ...
  simpa only [hloc, A, act, flat, endMap, Function.comp_apply] using hout
...
private theorem lTail_grad_on
  ...
  have hbranch : ContMDiffOn I 𝓘(Real, Real) ∞ branch U := by
    have hcomp := hactM.comp
      ...
      (fun _ hy ↦ hy.2)
    simpa only [branch, Function.comp_apply] using hcomp
...
private theorem lTailEnd_cov
  ...
  have hgradEv :
    ...
    exact Filter.Eventually.of_forall fun u ↦ by
      simpa only [g, branch, Vterm, hloc, zeta] using
        hgrad (eta u) (hetaU u)
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [NeZero (Module.finrank Real E)] in
/-- The fixed-time positive-start tail-action Hessian is the terminal metric
pairing with the covariant derivative of its induced parameter-variation
L-Jacobi field. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- The fixed-time positive-start tail-action Hessian is bounded by twice the
regularized L-index of every locally `C^8` comparison field with the prescribed
tail endpoint values. -/
theorem lTail_hess_le
  ...
  have hJb : J b = Y := by
    ...
    rw [hline]
    simpa only [B, hloc, y, beta] using hright
  have hQa : Q a = 0 := by
    change W a - J a = 0
    rw [hWa, hJa, sub_self]
  have hQb : Q b = 0 := by
  ...
  have hWg8 : ContMDiff 𝓘(Real, Real) I.tangent (8 : Nat)
    ...
    rw [← contMDiffOn_univ]
    simpa only [gammaG, Wg, beta] using hW.comp
  ...
  have hregGTail : ∀ s ∈ uIcc a b, T - s ^ 2 ∈ D.regular := by
    ...
    have hs' : s ∈ Icc a b := by
      simpa only [uIcc_of_le hab.le] using hs
    exact (hgeoG.2.2 s (by
      simpa only [uIcc_of_le hb0.le] using
        (⟨ha0.le.trans hs'.1, hs'.2⟩ : s ∈ Icc (0 : Real) b))).1
  ...
  have hbetaAt : ∀ s ∈ Icc a b,
    ...
    simpa only [beta] using hAlphaAt.comp s
  ...
  have hBetaMdiff : ∀ s ∈ uIcc a b, ∀ᶠ r in nhds s,
    ...
    have hBetaAt : ContMDiffAt 𝓘(Real, Real) I ∞ beta r := by
      simpa only [beta] using hAlphaAt.comp r
        (contMDiff_const.prodMk contMDiff_id).contMDiffAt
