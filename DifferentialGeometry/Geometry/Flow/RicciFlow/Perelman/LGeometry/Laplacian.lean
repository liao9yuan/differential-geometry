...
This file traces the strict-region reduced-length Hessian comparison against a
supplied terminal-orthonormal adapted family.
-/
...
/-- On the strict minimizing region, a supplied terminal-orthonormal adapted
family bounds the spatial reduced-length Laplacian by its traced regularized
index density. -/
theorem redLength_lap_le
  ...
  have hONb : ∀ i j,
    ...
    simpa only [alpha, b, hb_sq, lExp] using hON
  obtain ⟨basis, hbasis⟩ :=
    exists_basis_eq_gON (I := I) g y (fun i ↦ P i b) (by
      simpa only [g, y, alpha, hb_sq, lExp] using hONb)
  have hONbasis : ∀ i j,
      g.inner y (basis i) (basis j) = if i = j then 1 else 0 := by
    intro i j
    rw [hbasis i, hbasis j]
    simpa only [g, y, alpha, hb_sq, lExp] using hONb i j
