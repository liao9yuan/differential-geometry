...
/-!
# Integrated Hamilton trace identity

This file integrates the pointwise Hamilton trace identity along a canonical
regularized L-ray.  All scalar-path regularity and integrability inputs are
derived internally from the maximal regularized domain.
-/
...
/-- Hamilton's positive-start tail quantity in square-root time. -/
...
/-- The positive-start square-root tail agrees with Hamilton's raw-time
weighted `H` integral. -/
theorem lKTail_sq
    (S : SolutionOn (I := I) (M := M) D) (T : Real)
...
/-- Hamilton's positive-start tail converges to the full accumulated `K`
quantity as the start tends to zero from the right. -/
...
private theorem lTraceInt_data
  ...
  have halphaInf : ContMDiffOn (modelWithCornersSelf Real Real) I ∞ alpha U := by
    simpa only [alpha, z, U, Function.comp_apply] using
  ...
  have hRmd : ContMDiffOn (modelWithCornersSelf Real Real)
    ...
    simpa only [R, Function.comp_apply] using
      (scalar_joint (I := I) S hS).comp harg
        (fun s (hs : s ∈ U) ↦
...
private theorem lTracePos_data
  ...
  have hfund :
    ...
    have hWneg : IntervalIntegrable (fun s ↦ -W s)
        MeasureTheory.volume a b := by
      simpa only [Pi.neg_apply] using hWint.neg
...
omit [InnerProductSpace Real E] in
/-- Hamilton's square-root density is integrable along a canonical
regularized L-ray under the adapted trace hypotheses. -/
theorem lHamSq_int
    (S : SolutionOn (I := I) (M := M) D)
    (hS : IsSolutionOn (I := I) S) (T : Real)
    (x : M) (Z : TangentSpace I x) {b : Real}
...
omit [InnerProductSpace Real E] in
/-- The scaled trace density along a canonical regularized L-ray integrates to
the terminal scalar-curvature term and Hamilton's accumulated `K` quantity. -/
...
omit [InnerProductSpace Real E] in
/-- The positive-start scaled trace density along a canonical regularized
L-ray integrates to the terminal scalar-curvature term and Hamilton's tail. -/
