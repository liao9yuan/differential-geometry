...
/-!
# Local two-piece minimality before a chart change

A global fixed-endpoint comparison descends to the first two chart pieces while
an unchanged third piece is retained and cancelled from the action inequality.
-/
...
/-- Global fixed-endpoint minimality implies minimality of two consecutive
pieces in one chart, even when the unchanged following tail uses another
chart. -/
theorem lNodeHead_min
  ...
  let ph : Fin 2 → M := ![p, p]
  let vh : (i : Fin 2) → timeH1 E (lSegLen th i) :=
    Fin.cases (by simpa [th, lSegLen] using v0)
      (Fin.cases (by simpa [th, lSegLen] using v1) (fun i ↦ Fin.elim0 i))
  ...
  have hvhtar : ∀ i, MapsTo (vh i).toFun
      (Icc (0 : Real) (lSegLen th i)) (extChartAt I (ph i)).target := by
    intro i
    fin_cases i
    · simpa [vh, th, ph, lSegLen] using hvtar0
    · simpa [vh, th, ph, lSegLen] using hvtar1
  have hvhnode : (extChartAt I (ph 0)).symm
        ((vh 0).toFun (lSegLen th 0)) =
      (extChartAt I (ph 1)).symm ((vh 1).toFun 0) := by
    simpa [vh, th, ph, lSegLen] using hvnode
  ...
  have hjoin : gammaV c = gamma c := by
    exact hV2.trans (by
      simpa [vh, th, ph, lSegLen] using hv2)
  ...
  let uf : (i : Fin 3) → timeH1 E (lSegLen tf i) :=
    Fin.cases (by simpa [tf, lSegLen] using u0)
      (Fin.cases (by simpa [tf, lSegLen] using u1)
        (Fin.cases (by simpa [tf, lSegLen] using u2) (fun i ↦ Fin.elim0 i)))
  let vf : (i : Fin 3) → timeH1 E (lSegLen tf i) :=
    Fin.cases (by simpa [tf, lSegLen] using v0)
      (Fin.cases (by simpa [tf, lSegLen] using v1)
        (Fin.cases (by simpa [tf, lSegLen] using u2) (fun i ↦ Fin.elim0 i)))
  ...
  have hrepBase : ∀ i, EqOn (uf i).toFun
      ...
      (Icc (0 : Real) (lSegLen tf i)) := by
    intro i
    fin_cases i
    · simpa [uf, tf, pf, lSegLen] using hrep0
    · simpa [uf, tf, pf, lSegLen] using hrep1
    · simpa [uf, tf, pf, lSegLen] using hrep2
  ...
  have hdelta0 : delta a = gamma a :=
    (hdelta_head (hab.trans hbc)).trans
      (hV0.trans (by simpa [vh, th, ph, lSegLen] using hv0))
