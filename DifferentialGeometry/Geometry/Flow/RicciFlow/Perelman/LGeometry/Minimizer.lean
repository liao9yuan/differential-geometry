...
This file starts the minimizing layer for Perelman's regularized L-action.
-/
...
omit [InnerProductSpace Real E] in
/-- Every globally smooth zero-endpoint field along a regularized L-curve has
nonnegative L-index when the central curve locally minimizes regularized action
along every smooth fixed-endpoint variation. -/
theorem lRegIndex_nonneg
  ...
  have hEq : Set.EqOn
      ...
      Y (Set.uIoo a b) := by
    intro s hs
    simpa only [lVelocity] using hfield s (Set.uIoo_subset_uIcc_self hs)
