...
This module starts the global cut-domain layer by recording exactly when a
positive-domain L-exponential ray realizes the compact-category L-cost.
-/
...
/-- An initial L-tangent and positive backward time are minimizing when the
associated L-exponential ray lies in its regular domain and realizes L-cost. -/
def IsLMinVec
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The joint initial-tangent/backward-time domain on which L-exp rays realize
the L-cost to their endpoints. -/
def lMinDomain
    (S : SolutionOn (I := I) (M := M) D) (T : Real) (x : M) :
    Set (E × Real) :=
...
/-- Membership in the joint minimizing domain is the minimizing-vector
predicate. -/
...
omit [NeZero (Module.finrank Real E)] [SigmaCompactSpace M] in
/-- Every point of the minimizing L-exponential domain has positive backward
time. -/
...
/-- On a compact manifold, every endpoint covered by the compact direct
method is reached by an initial tangent in the minimizing L-exp domain. -/
...
/-- Minimizing L-exponential-domain membership is preserved when positive
backward time is decreased. -/
  ...
  · have hvec := (mem_lMinDomain S T x Z tau).1 hmin
    ...
    calc
      lLength S T (fun r : Real ↦ lExp S T x Z r) 0 sigma =
        ...
        simpa only [gamma, lExp, sqrtReparam] using
          lLength_sqrt (I := I) S T gamma sigma hsigma.le
...
/-- For a fixed initial tangent, the positive backward times on which its
L-exponential ray minimizes form an order-connected set. -/
