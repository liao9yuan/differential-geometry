...
This module lifts the pointwise source-Gaussian estimate for the pulled-back
reduced Jacobian to a global upper bound for Perelman's reduced volume.
-/
  ...
  have hred : ContinuousOn
    ...
    have hexp : ContinuousOn
      ...
      simpa only [redDensity] using
        Real.continuous_exp.comp_continuousOn hcore
    apply hexp.congr
    intro Z hZ
    dsimp only [Ψ]
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- On every regular positive backward-time slab, Perelman's reduced volume
is at most its unit source-Gaussian mass. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
/-- Perelman's reduced volume converges to one as positive backward time
tends to zero at every regular terminal time. -/
theorem redVolume_zero_lim
  ...
  have hmeas : ∀ᶠ tau in 𝓝[>] (0 : Real), Measurable (F tau) := by
    ...
    simpa only [F, Set.indicator] using
