...
/-!
# Endpoint data of regularized L-rays

This module records the endpoint first variation of regularized L-action and
the injectivity of the full endpoint phase state.  These are the geometric
inputs for separating the smooth action branches associated to distinct
nonconjugate minimizing L-rays.
-/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- The regularized Lagrangian of the maximal L-ray is jointly smooth in the
initial tangent and square-root time on its natural joint domain. -/
...
/-- Along a smooth variation whose central curve is a regularized L-ray and
whose initial endpoint is fixed, the first variation of regularized L-action
is exactly the terminal metric pairing. -/
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- The initial-tangent regularized L-action has derivative equal to terminal
metric lowering composed with the fixed-time L-exponential differential. -/
theorem lRayAct_hasFDeriv
      ...
      (((S.base.metric (T - tau)).inner (lExp S T x Z tau)
        ...
        (mfderiv (modelWithCornersSelf Real E) I
          (fun W : E ↦ lExp S T x W tau) Z :
            E →L[Real] TangentSpace I (lExp S T x Z tau))) Z := by
  ...
  have hL : Lint = L := by
    ...
    have hExpAt : ContMDiffAt (modelWithCornersSelf Real E) I ∞
      ...
      simpa only [endMap, Function.comp_apply] using
        hjoint.comp (f := fun W : E ↦ (W, tau)) (Z : E) hpairAt
    ...
    have hline : HasDerivAt (fun u : Real ↦ A (zeta u)) (Lint Y) 0 := by
      ...
      change HasDerivAt (A ∘ zeta) (Lint Y) 0
  ...
  have hIntL : HasFDerivAt A L Z := by
    rw [← hL]
    exact hInt
  simpa only [A, L, b, endMap] using hIntL

attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- The regularized L-action of an L-ray is jointly differentiable in its
initial tangent and positive backward terminal time. -/
theorem lRayAct_joint
  ...
  have hG : ContDiffOn Real 2
    ...
    simpa only [G] using (hsqrt.mul hcomp).contDiffWithinAt
  have hInt :=
    DifferentialGeometry.Analysis.Calculus.hasFDerivAt_paramInt
  ...
  have htime : HasDerivAt (fun r : Real ↦ A (Z, r)) c tau := by
    ...
    simpa only [A, lRegAction, lag, b, c, one_div, inv_mul_eq_div] using hcomp
  ...
  have hfinal : HasFDerivAt A
    ...
    rw [← hfd]
    exact hbase
  simpa only [A, Lz, c, b] using hfinal

omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- Two regularized L-rays with the same terminal position and velocity have
the same normalized initial tangent. -/
...
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- Distinct regularized L-rays reaching the same endpoint have distinct
terminal velocities. -/
