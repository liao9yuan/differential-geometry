...
/-!
# Tail of the source Gaussian

This module proves that the exactly normalized source Gaussian has vanishing
mass outside growing balls for the terminal source metric.
-/
...
/-- The exactly normalized source Gaussian has vanishing mass outside growing
closed balls for the terminal source metric. -/
...
omit [I.Boundaryless] [T2Space M]
  [SigmaCompactSpace M] in
/-- In a fixed dimension, one terminal-metric source radius bounds the tail of
every exactly normalized source Gaussian, independently of the flow, terminal
time, and basepoint. -/
