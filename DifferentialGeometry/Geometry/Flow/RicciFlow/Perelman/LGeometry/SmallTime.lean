...
/-!
# Small-time limits for Perelman's L-geometry

This module starts the Euclidean small-time normalization of reduced volume.
It first identifies the normalized action of a fixed regularized L-ray at the
singular initial time.
-/
...
/-- The action of a fixed regularized L-ray, divided by twice its terminal
square-root time, converges to the squared initial metric norm. -/
  ...
  have hcontOn : ContinuousOn lag K := by
    ...
    have hcomp : ContinuousOn
      ...
      (lRayLag_smooth S hS T x).continuousOn.comp hpairOn
        (fun s hs ↦ hs)
    simpa only [Function.comp_apply, lag, z] using hcomp
...
/-- Along a ray which is strictly minimizing at one later time, reduced
length at square-root time converges to the squared initial metric norm. -/
theorem lRedLen_sq_lim
  ...
  have hEq :
    ...
    have hlen :
      ...
      simpa only [lExp, sqrtReparam, Real.sqrt_sq hs.le] using
        lLength_sqrt (I := J) S T (lRegCurve S T x Z) (s ^ 2) hs0.le
