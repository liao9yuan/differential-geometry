...
This module extracts regularized fixed-endpoint action minimality from a
minimizing L-ray on a possibly noncompact manifold. A uniform curvature bound
supplies the lower bound needed by the infimum comparison.
-/
...
/-- A minimizing L-ray with a uniform curvature bound minimizes regularized
action among global `C¹` curves with the same square-root-time endpoints. -/
    ...
    calc
      lRegAction S T (lRegCurve S T x Z) 0 (Real.sqrt tau) =
        ...
        simpa only [lExp, sqrtReparam] using
          (lLength_sqrt (I := I) S T (lRegCurve S T x Z) tau
            htau.le).symm
