...
private theorem lVelocity_eq_of_eqOn
  ...
  simpa only [lVelocity] using congrArg (fun L => L (1 : Real)) hder
...
private theorem lRegLag_int_of_eqOn
  ...
  have hvel : lVelocity (I := I) alpha s = lVelocity (I := I) beta s := by
    simpa only [lVelocity] using congrArg (fun L => L (1 : Real)) hmf
  simp only [lRegLag]
  rw [hval, hvel]

/-- Strictly before the terminal time of a minimizing L-ray, a minimizing ray
with the same endpoint has the same initial tangent. -/
theorem lMinVec_unique_lt
  ...
  have hgammaCont : Continuous gamma := by
    ...
    · intro s hs
      have hs' : s = c := by simpa only [frontier_Iic] using hs
      subst s
  ...
  have hprefix : lRegAction S T gammaW 0 c = lRegAction S T gammaZ 0 c := by
    calc
      lRegAction S T gammaW 0 c =
        ...
        simpa only [gammaW, c, lExp, sqrtReparam] using
          (lLength_sqrt (I := I) S T gammaW sigma hsigma.le).symm
      ...
      _ = lRegAction S T gammaZ 0 c := by
        simpa only [gammaZ, c, lExp, sqrtReparam] using
          lLength_sqrt (I := I) S T gammaZ sigma hsigma.le
    ...
    calc
      lRegAction S T gammaZ 0 b =
        ...
        simpa only [gammaZ, b, lExp, sqrtReparam] using
          (lLength_sqrt (I := I) S T gammaZ tau htau.le).symm
