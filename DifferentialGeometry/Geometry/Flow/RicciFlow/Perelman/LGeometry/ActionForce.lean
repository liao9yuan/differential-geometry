...
/-!
# The local position force of the regularized L-action

This file constructs the spatial derivative of the nonlinear fixed-chart
Lagrangian as a chart covector.  Along a chart-valued `timeH1` curve its
kinetic coordinates are quadratic in the weak velocity, while its scalar
coordinates are continuous.  Consequently the force has the natural `L1`
time regularity.
-/
...
private noncomputable def lScalarDir
  ...
  extDerivFun (I := I) (S.scalar q.1) x
    (chartBasisVecFiber (I := I) p i x)

/-- The position derivative of the nonlinear chart Lagrangian as a covector on
the fixed chart model space. -/
...
/-- The Riesz vector representing the position derivative of the nonlinear
chart Lagrangian. -/
...
/-- The chart force represents the position derivative under the model inner
product. -/
...
-- Compact coefficient bounds and finite-coordinate Riesz reconstruction make
-- this single proof exceed the default heartbeat budget.
omit [NeZero (Module.finrank Real E)] [I.Boundaryless]
  [SigmaCompactSpace M] in
/-- On a positive compact chart interval, the nonlinear chart position force
along a `timeH1` curve is time integrable. -/
theorem lChartForce_int
  ...
  have hscalar (i : Fin (Module.finrank Real E)) : IntegrableOn
    ...
    have hsmooth : ContDiffOn Real ∞
      ...
      simpa only [lScalarDir, U] using chartScalarDeriv (I := I) S hS p i
  ...
  have hcov : IntegrableOn
    ...
    refine integrable_finset_sum Finset.univ
      (f := fun i r ↦
        (inner Real
            (lGramDir (I := I) S p i (tau r, u.toFun r) (u.deriv r))
            (u.deriv r) +
  ...
  have hcov' : IntegrableOn
    ...
    simpa only [lChartPosDeriv, tau] using hcov
