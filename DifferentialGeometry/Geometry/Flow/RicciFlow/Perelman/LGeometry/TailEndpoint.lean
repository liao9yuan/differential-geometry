...
/-!
# Positive-start endpoint nonsingularity

This file turns a spanning positive-start family along a minimizing regularized
L-ray into an injective terminal endpoint differential.
-/
...
/-- The affine velocity-line field of a locally smooth endpoint family is
smooth on the family's open time domain. -/
theorem lTailLine_smooth
  ...
  have hf : ∀ s ∈ K, ContMDiffAt
    ...
    simpa only [f, zero_smul, add_zero] using hcomp
...
private theorem tailEnd_injective
  ...
  have hreg0s : ∀ s ∈ uIcc (0 : Real) s0,
    ...
    exact (hgeoG.2.2 s (by
      simpa only [uIcc_of_le hb0.le] using
        (⟨hs'.1, hs'.2.trans hs0b.le⟩ : s ∈ Icc (0 : Real) b))).1
  have hregs0b : ∀ s ∈ uIcc s0 b,
    ...
    exact (hgeoG.2.2 s (by
      simpa only [uIcc_of_le hb0.le] using
        (⟨hs00.le.trans hs'.1, hs'.2⟩ : s ∈ Icc (0 : Real) b))).1
  ...
  have hY1smooth : ContMDiff 𝓘(Real, Real) I.tangent ∞
    ...
    simpa only [Y1, YgE] using hGYSmooth.add_bundle hkW
  ...
  have hk' : lRegIndex S T gammaG Y0 Y0 0 s0 +
    ...
    simpa only [Y0, Y1, YgE] using hk
  exact not_lt_of_ge hnonneg hk'

/-- A curvature-bounded minimizing regularized L-ray admits a positive-start
family whose terminal endpoint differential is injective. -/
