...
/-!
# Uniform early-slice splicing for reduced length

This module bounds fixed-chart affine ramps uniformly when their forward-time
image stays in one compact regular slab.  It then splices such a ramp onto a
concrete minimizing regularized L-ray.
-/
...
/-- On a fixed compact forward-time slab, one pair of constants controls every
affine chart ramp whose terminal time is bounded above. -/
...
/-- Uniform forward-slab constants turn a reduced-length bound on one concrete
minimizing ray into an explicit bound after a fixed-chart affine splice. -/
...
/-- A concrete minimizing ray whose endpoint lies in a chart ball gives a
positive-measure early-slice target on which reduced length has one explicit
upper bound. -/
