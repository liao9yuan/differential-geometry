...
/-!
# The L-cut locus decomposition

This module defines the fixed-time L-cut domain and decomposes its image under
the L-exponential map into conjugate and multiple-minimizer alternatives.
-/
...
/-- Initial L-tangents whose minimizing-time fiber has greatest element `tau`. -/
def lCutDomain
    (S : SolutionOn (I := I) (M := M) D) (T : Real) (x : M)
    (tau : Real) : Set E :=
  {Z | IsGreatest {sigma : Real | (Z, sigma) ∈ lMinDomain S T x} tau}

/-- The fixed-time L-exponential image of the L-cut domain. -/
def lCutImage
    (S : SolutionOn (I := I) (M := M) D) (T : Real) (x : M)
    (tau : Real) : Set M :=
  (fun Z : E ↦ lExp S T x Z tau) '' lCutDomain S T x tau

/-- Cut endpoints represented by a conjugate minimizing initial tangent. -/
def lCutConj
    (S : SolutionOn (I := I) (M := M) D) (T : Real) (x : M)
    (tau : Real) : Set M :=
...
/-- Cut endpoints represented by two distinct minimizing initial tangents. -/
def lCutMulti
    (S : SolutionOn (I := I) (M := M) D) (T : Real) (x : M)
    (tau : Real) : Set M :=
  {y | ∃ Z : E, Z ∈ lCutDomain S T x tau ∧
...
/-- A cut tangent is minimizing at its cut time but does not remain minimizing
through any strictly later time. -/
...
/-- At a fixed positive backward time, minimizing initial L-tangents form a
closed subset of the tangent model. -/
...
/-- At a fixed positive backward time, the L-cut domain is closed. -/
theorem lCutDom_closed
  ...
  rw [show lCutDomain S T x tau =
    ...
    ext Z
    simpa only [mem_setOf_eq, mem_inter_iff, mem_compl_iff] using
      (mem_lCutDomain S T x tau Z)]
  exact hslice.inter hinj

/-- At a fixed positive backward time, the L-cut domain is Borel measurable. -/
...
/-- The fixed-time L-cut image is the union of its conjugate and
multiple-minimizer parts. -/
theorem lCut_split
    (S : SolutionOn (I := I) (M := M) D) (hS : IsSolutionOn (I := I) S)
    (T : Real) (x : M) (tau : Real) :
