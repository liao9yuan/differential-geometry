...
/-!
# Negative index directions from a left extension

This file supplies the algebraic mirror of the standard interior-conjugacy
split: a zero-index Jacobi direction on a suffix is perturbed by a global test
field across the node to produce a negative broken direction.
-/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] in
/-- A zero-index suffix direction with negative cross index yields a negative
pair of broken quadratic directions for a suitable positive scale. -/
...
/-- Pairing a suffix Jacobi field with a test field pointing along its nonzero
initial covariant derivative gives negative cross index. -/
