...
/-!
# Regularized Perelman L-action

This file defines the square-root-time Lagrangian directly on a regularized
curve.  Unlike `lRegDensity`, the curve argument is already parameterized by
square-root backward time, so smooth variations may pass through `s = 0`.
-/
...
/-- The Lagrangian of a curve already parameterized by square-root backward
time. -/
...
/-- The regularized L-action on an oriented square-root-time interval. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- On a compact square-root-time interval and compact manifold, continuity of
the scalar curvature gives a uniform lower bound for the scalar part of the
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- Pointwise metric coercivity and a lower bound for the scalar term give a
coercive lower bound for the regularized L-action. -/
...
/-- On a fixed compact spacetime slab, the regularized L-action coercivity
constants are uniform over all curves. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- For a genuine Ricci-flow solution on a compact manifold, compact time-slab
control supplies the constants in the coercive lower bound for the regularized
L-action. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- A uniform upper bound for regularized L-action gives an upper bound for
the kinetic energy measured by a fixed reference metric, with constants
uniform over the curve family. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- A regularized L-action budget gives reference-metric square-root distance
control on every subinterval. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- The earlier square-time density is the direct regularized Lagrangian of
the square-reparameterized curve. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- L-length is the direct regularized action after `tau = s ^ 2`. -/
theorem lLength_reg
    (S : SolutionOn (I := I) (M := M) D) (T : Real)
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- L-length is direct regularized action after the square-root substitution
without any differentiability assumption; the zero-time exception is null. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- Reading a square-root-time curve in backward time preserves its action;
the singular inverse derivative at zero is discarded almost everywhere. -/
...
-- The compact domination proof needs extra elaboration budget.
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Differentiation of the regularized L-action under a supplied smooth
variation; the compact-interval domination bound is produced internally. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The regularized Euler residual of a smooth variation is jointly `C¹` on
the regular flow-time region, including square-root time zero. -/
theorem lRegEuler_var_c1
  ...
  have heq : Set.EqOn euler raw U := by
    ...
    have hYdiff : DifferentiableAt Real
      ...
      simpa only using variationField_chartRep_differentiableAt
        (I := I) (S.base.metric (T - p.2 ^ 2)) fp hfp p.2
    have hAdiff : DifferentiableAt Real
        (chartRepAt (I := I) (fp 0)
          (fun s : Real ↦ lVelocity (I := I) (fp 0) s) p.2) p.2 := by
      simpa only using velocityField_chartRep_differentiableAt
        (I := I) (S.base.metric (T - p.2 ^ 2)) fp hfp p.2
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The first variation of regularized L-action is the endpoint pairing minus
the integral of the regularized Euler residual. -/
...
/-- The derivative of the negative regularized Euler-residual integral along a
smooth variation is the negative integrated regularized Jacobi pairing. -/
theorem lRegEulerInt_deriv
  ...
  have hJEq : Set.EqOn (dF 0) J (Set.uIcc a b) := by
    ...
    have hneg : HasDerivAt (fun u : Real ↦ F u s) (J s) 0 := by
      simpa only [F, J, W] using hpoint.neg
...
private theorem lRegAction_jac
  ...
  have hderivEq (u : Real) : deriv L u = Eul u := by
    ...
    simp only [map_zero, ContinuousLinearMap.zero_apply, sub_self, zero_sub]
      at hshift
    rw [hfu0] at hshift
...
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- The regularized Jacobi pairing generated by a smooth variation is
continuous along a compact regularized L-curve interval. -/
...
/-- The fixed-endpoint second variation of regularized L-action along a
regularized L-curve is twice its regularized L-index form, including at
square-root time zero. -/
