...
/-!
# Euclidean Sard step for L-conjugate tangents

This module proves the fixed-target-chart Euclidean nullity statement used by
the conjugate branch of the L-cut-locus argument.  Passing this chart-coordinate
null set back to Riemannian volume is a separate measure-theoretic layer.
-/
...
/-- In one fixed target chart, the coordinate image of all L-conjugate
initial tangents landing in that chart has Euclidean measure zero. -/
theorem lConjChart_null
  ...
  have hg (Z : E) (hZ : Z ∈ s) : HasFDerivAt g (dg Z) Z := by
    ...
    simpa only [writtenInExtChartAt_model_space, ext_chart_model_space_apply]
      using hasMFDerivAt_iff_hasFDerivAt.mp hcomp
