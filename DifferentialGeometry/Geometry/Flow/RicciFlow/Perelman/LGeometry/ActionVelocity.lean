...
/-- A chart Gram coefficient along a continuous time-`H¹` curve is measurable
and uniformly bounded on a compact regular chart buffer. -/
...
/-- A continuous momentum representative yields a continuous representative
of the coordinate velocity through the inverse chart Gram operator. -/
theorem chartVel_of_mom {D : RealTimeInterval}
  ...
  rw [hhalf, one_smul]
  simpa only [ContinuousLinearMap.mul_apply, ContinuousLinearMap.one_apply] using hmul.symm

/-- A fixed-chart weak Euler identity with an integrable force gives a
continuous representative of the coordinate velocity. -/
