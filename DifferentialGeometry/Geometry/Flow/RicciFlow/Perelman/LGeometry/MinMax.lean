...
/-!
# Speed control for regularized L-rays

This module records the scalar speed-square evolution underlying the
Morgan--Tian min--max estimate.  The moving tangent fibers are fully paired
against the moving metric before any estimate is made.
-/
...
/-- Squared speed of a square-root-time curve in the corresponding backward
Ricci-flow metric. -/
...
/-- Regularized speed squared is nonnegative. -/
...
/-- Along a regularized L-ray, squared speed satisfies the scalar evolution
identity used in the Morgan--Tian min--max estimate. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- A bound for scalar-gradient and Ricci pairings gives a two-sided-time
Gronwall comparison for regularized L-speed squared. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- A positive time lower bound and a kinetic budget uniformly bound the
initial tangent of a regularized L-ray. -/
