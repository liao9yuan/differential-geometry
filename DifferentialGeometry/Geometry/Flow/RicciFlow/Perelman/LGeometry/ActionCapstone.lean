...
/-- A fixed-endpoint, action-bounded sequence has a finite-chart weak limit
whose generalized chart action is bounded by the liminf of the sequence.  The
conclusion intentionally keeps only chart weak derivatives; it does not claim
a manifold velocity or an action value for the limiting curve. -/
...
omit [NeZero (Module.finrank Real E)] in
/-- A fixed-endpoint, action-bounded sequence has a uniformly convergent
subsequence whose continuous limit satisfies the raw regularized-action liminf
bound. -/
