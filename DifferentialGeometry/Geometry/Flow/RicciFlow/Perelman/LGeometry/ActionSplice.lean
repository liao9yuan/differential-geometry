...
/-!
# One-piece chart splices for finite L-action realizations

This file realizes the replacement of one positive-length chart `timeH1`
piece by another curve with the same two endpoint traces.
-/
...
private theorem fin_seg_left {m : Nat} {i j : Fin m} (hji : j < i) :
  ...
  simpa only [Fin.val_succ, Fin.val_castSucc] using hji
...
/-- Replacing one positive member of a monotone finite chart realization by a
fixed-endpoint chart-valued `timeH1` curve produces a continuous manifold
curve realizing exactly the dependent `Function.update` family. -/
