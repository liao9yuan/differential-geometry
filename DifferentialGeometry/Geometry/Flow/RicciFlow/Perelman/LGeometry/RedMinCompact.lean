...
/-!
# Spatial minimum of reduced length on complete flows

This module removes the fixed-terminal-endpoint restriction from the compact
direct method.  An action-bounded family with one common start stays in a
single compact terminal-metric ball, so its endpoints may converge together
with the curves.  This yields attainment of the spatial minimum of reduced
length on a complete bounded-curvature flow.
-/
...
/-- A fixed-start sequence with one regularized L-action bound on a complete
...
/-- The fixed-start compact subsequence has a finite-chart weak limit whose
generalized chart action is bounded by the action liminf. -/
...
/-- On a connected complete bounded-curvature flow, reduced length attains its
spatial minimum at every positive backward time. -/
