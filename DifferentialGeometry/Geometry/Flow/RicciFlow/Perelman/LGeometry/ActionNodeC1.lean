import DifferentialGeometry.Geometry.Flow.RicciFlow.Perelman.LGeometry.ActionNodeVel
...
/-!
# C1 regularity across an L-action node

Two positive chart pieces of a fixed-endpoint L-action minimizer are individually
`C¹`, and the velocity corner equation glues them into one `C¹` manifold curve.
-/
...
/-- A two-piece fixed-endpoint L-action minimizer is `C¹` across its positive
chart node. -/
theorem lNode_c1
  ...
  have hp0der : derivWithin ((extChartAt I (p 0)) ∘ gamma)
    ...
    have hshift := chartDeriv_shift
      ...
      (by intro r hr; simpa only [Function.comp_apply] using hrep 0 hr)
  ...
  have hp1der : derivWithin ((extChartAt I (p 1)) ∘ gamma)
    ...
    have hshift := chartDeriv_shift
      ...
      (by intro r hr; simpa only [Function.comp_apply] using hrep 1 hr)
      ⟨le_rfl, sub_nonneg.mpr (hpos 1).le⟩
    simpa only [hfin1c, hfin1s, add_zero] using hshift
