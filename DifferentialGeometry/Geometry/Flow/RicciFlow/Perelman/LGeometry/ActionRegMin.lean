...
This module packages the direct-method minimizer as an endpoint-honest
regularized L-curve on a normalized closed square-root-time interval.
-/
...
/-- On a positive normalized interval, the regularized L-action infimum among
global fixed-endpoint `C¹` competitors is attained by an endpoint-honest
regularized L-curve. -/
