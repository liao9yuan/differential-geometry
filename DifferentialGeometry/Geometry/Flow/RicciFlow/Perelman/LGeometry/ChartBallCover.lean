...
/-!
# Finite coordinate-ball covers

This module packages the compactness step used after the local reduced-length
splice estimate: finitely many coordinate balls cover the closed manifold, and
their inverse-chart images have one positive volume floor for a fixed metric.
-/
...
/-- A compact boundaryless manifold admits finitely many coordinate balls with
closed balls inside chart interiors and one positive volume floor.  The cover
records both chart-source and coordinate-ball membership for each point. -/
