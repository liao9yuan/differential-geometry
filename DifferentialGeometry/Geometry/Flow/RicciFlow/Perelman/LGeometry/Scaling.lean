...
/-!
# Parabolic scaling of Perelman's L-exponential map

This file records the naturality of the regularized L-geodesic equation and
the L-exponential map under the native parabolic rescaling of a Ricci flow.
-/
...
private theorem lVelocity_mul
  ...
  · have hscale : HasMFDerivAt (modelWithCornersSelf Real Real)
        (modelWithCornersSelf Real Real)
        (fun r : Real => c * r) s (c • ContinuousLinearMap.id Real Real) := by
      rw [hasMFDerivAt_iff_hasFDerivAt]
      simpa [mul_comm] using ((hasFDerivAt_id s).const_mul c)
    have hcomp := halpha.hasMFDerivAt.comp s hscale
    calc
      lVelocity (I := I) (fun r => alpha (c * r)) s =
          ((mfderiv (modelWithCornersSelf Real Real) I alpha (c * s)).comp
            (c • ContinuousLinearMap.id Real Real)) (1 : Real) := by
        simpa only [lVelocity] using
          congrArg (fun L => L (1 : Real)) hcomp.mfderiv
      _ = c • lVelocity (I := I) alpha (c * s) := by
        simp only [ContinuousLinearMap.comp_apply, lVelocity]
        have harg : (c • ContinuousLinearMap.id Real Real) (1 : Real) = c := by
          simp
        calc
          (mfderiv (modelWithCornersSelf Real Real) I alpha (c * s))
              ((c • ContinuousLinearMap.id Real Real) (1 : Real)) =
              (mfderiv (modelWithCornersSelf Real Real) I alpha (c * s)) c :=
            congrArg _ harg
          _ = c • (mfderiv (modelWithCornersSelf Real Real) I alpha (c * s)) 1 := by
            have hmap :=
              (mfderiv (modelWithCornersSelf Real Real) I alpha (c * s)).map_smul
                c (1 : Real)
            have hc1 : c • (1 : Real) = c := by simp
            calc
              (mfderiv (modelWithCornersSelf Real Real) I alpha (c * s)) c =
                  (mfderiv (modelWithCornersSelf Real Real) I alpha (c * s))
                    (c • (1 : Real)) := congrArg _ hc1.symm
              _ = c • (mfderiv (modelWithCornersSelf Real Real) I alpha (c * s)) 1 :=
                hmap
...
/-- Regularized L-geodesic acceleration is natural under parabolic scaling. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- A regularized L-curve rescales by square-root time and inverse square-root
initial velocity under parabolic scaling. -/
theorem isLRegCurve_para
  ...
  have hregR : TR - r ^ 2 ∈ (paraInterval D t0 R hR ht0).regular := by
  ...
  have hmdiffR : MDifferentiableAt (modelWithCornersSelf Real Real) I alphaR r := by
    simpa only [alphaR] using hmdiff.comp r hparam
  have hvelAll : ∀ q : Real,
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- A regularized L-curve for a parabolically scaled flow pulls back by
square-root time to one for the original flow. -/
theorem isLRegCurve_unpara
  ...
  have hmdiffA : MDifferentiableAt (modelWithCornersSelf Real Real) I alpha s := by
    simpa only [alpha] using hmdiff.comp s hparam
  have hvelAll : ∀ q : Real,
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The maximal regularized L-domain scales by square-root time. -/
...
/-- The totalized regularized L-curve commutes with parabolic scaling. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Backward-time membership in the L-exponential domain is invariant under
parabolic scaling. -/
theorem lExpDomain_para
  ...
  simp only [lExpDomain, mem_setOf_eq]
...
attribute [-instance] Tensor0SBundle.tangentSpace_normedAddCommGroup
  Tensor0SBundle.tangentSpace_normedSpace in
omit [InnerProductSpace Real E] [SigmaCompactSpace M] in
/-- Perelman's totalized L-exponential map commutes with parabolic scaling. -/
theorem lExp_para
    (S : SolutionOn (I := I) (M := M) D)
    (hS : IsSolutionOn (I := I) S)
    (t0 R : Real) (hR : 0 < R) (ht0 : t0 ∈ D.carrier)
