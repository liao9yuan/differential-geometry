...
/-!
# The source Gaussian in Perelman's L-geometry

This module identifies the fixed source Gram matrix as positive definite and
normalizes its Gaussian density against the exact model Haar measure.
-/
...
/-- The source Gram matrix of the terminal metric is positive definite. -/
theorem lSrcGram_pd
    (S : SolutionOn (I := I) (M := M) D) (T : Real) (x : M) :
    (lSrcGram S T x).PosDef := by
  simpa only [lSrcGram,
    DifferentialGeometry.Integral.L2.gramMatrixAt] using
    DifferentialGeometry.Integral.L2.gramMatrixAt_posDef
      (I := I) (M := M) (S.base.metric T) x

omit [NeZero (Module.finrank Real E)] [I.Boundaryless] [T2Space M]
  [SigmaCompactSpace M] in
/-- The source Gram quadratic form is the terminal metric norm square. -/
    ...
    inner Real v
        ...
        ∑ i : Fin (Module.finrank Real E),
          ∑ j : Fin (Module.finrank Real E),
            v i * v j * (S.base.metric T).inner x
              ((chartModelBasis E) i) ((chartModelBasis E) j) := by
      rw [Matrix.inner_toEuclideanCLM]
      simp only [dotProduct, Matrix.mulVec, lSrcGram, Matrix.of_apply]
      refine Finset.sum_congr rfl fun i _ => ?_
      rw [Finset.mul_sum]
      refine Finset.sum_congr rfl fun j _ => ?_
      ring
    _ = (S.base.metric T).inner x w w := by
      symm
      ...
      refine Finset.sum_congr rfl fun i _ => ?_
      have hsm2 :
          (S.base.metric T).inner x (v i • (chartModelBasis E) i)
              ((chartModelBasis E) j) =
            v i * (S.base.metric T).inner x ((chartModelBasis E) i)
              ((chartModelBasis E) j) := by
        have h :
            (S.base.metric T).inner x (v i • (chartModelBasis E) i) =
              v i • (S.base.metric T).inner x ((chartModelBasis E) i) :=
          ContinuousLinearMap.map_smul
            ((S.base.metric T).inner x) (v i) ((chartModelBasis E) i)
        rw [h, ContinuousLinearMap.smul_apply, smul_eq_mul]
      rw [hsm2]
      rw [(S.base.metric T).symm x ((chartModelBasis E) i)
        ((chartModelBasis E) j)]
      ring
    _ = (S.base.metric T).inner x Z Z := by rw [hw]

/-- The normalized Gaussian density of the terminal source metric, expressed
in the exact model coordinates underlying `modelHaar`. -/
...
omit [NeZero (Module.finrank Real E)] [I.Boundaryless] [T2Space M]
  [SigmaCompactSpace M] in
/-- The source Gaussian exponent is the intrinsic terminal metric norm square. -/
...
omit [NeZero (Module.finrank Real E)] [I.Boundaryless] [T2Space M]
  [SigmaCompactSpace M] in
/-- The source Gaussian has total `modelHaar` mass one. -/
