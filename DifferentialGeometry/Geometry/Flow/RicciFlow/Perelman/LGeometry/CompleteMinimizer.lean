...
/-!
# Compactness for complete bounded-curvature L-flows

This module supplies the direct method with a single compact target for a
fixed-endpoint minimizing sequence.  The compact target is the explicit
terminal-metric ball obtained from the common action and curvature bounds.
-/
...
private def rmFactor (E : Type uE) [NormedAddCommGroup E]
    [NormedSpace Real E] [FiniteDimensional Real E] (K : Real) : Real :=
...
/-- The moving-metric squared speed of a `C¹` curve is interval integrable
on every regular compact backward-time interval. -/
theorem lRegSpeed_int_c1
  ...
  have hhalf : IntervalIntegrable
    ...
    simpa only [lRegLag, add_sub_cancel_right] using hLag.sub hpot
...
/-- The squared speed of a `C¹` curve is integrable for any fixed smooth
reference metric on a compact parameter interval. -/
theorem lRegRef_int_c1
  ...
  letI cg : ContinuousRiemannianMetric E
      (TangentSpace I : M → Type _) := gRef.toContinuousRiemannianMetric
  letI rb : RiemannianBundle (TangentSpace I : M → Type _) :=
...
/-- A bounded-curvature scalar lower bound converts a global action budget into
the finite family of chart-`H¹` bounds used by the direct method. -/
theorem lRmChartH1_fin
  ...
  have hchart (i : Fin m) (n : Nat) :
    ...
    rw [show (∫ r in (0 : Real)..lSegLen t i, (1 / 2 : Real) * inner Real
        (chartGramOp (I := I) S.family (p i)
          (T - (t i.castSucc + r) ^ 2, (u i n).toFun r) ((u i n).deriv r))
        ((u i n).deriv r)) =
        ∫ s in t i.castSucc..t i.succ,
  ...
  exact chartH1_fin (I := I) hMet p
    (fun i ↦ lSegLen t i) (fun i ↦ sub_nonneg.mpr (hseg i))
    (fun i r ↦ T - (t i.castSucc + r) ^ 2) htimeCont htimeReg
    Kcoord hKc hKchart u (fun _ ↦ B) huK hchart

/-- A fixed-endpoint sequence with one regularized L-action bound on a complete
bounded-curvature flow has a uniformly convergent subsequence. -/
...
/-- The bounded-curvature compact subsequence has a finite-chart weak limit
whose generalized chart action is bounded by the action liminf. -/
...
/-- On a complete bounded-curvature flow, every nonempty fixed-endpoint global
`C¹` competitor class on a nondegenerate interval has a regularized
L-minimizer satisfying the interior L-geodesic equation. -/
theorem exists_lRegMin_rm
  ...
  have hsubseq : Tendsto
      ...
      (nhds (sInf costs)) := by
    simpa only [hval] using hvlim.comp hchi.tendsto_atTop
...
/-- On a complete bounded-curvature flow, positive-time raw L-cost is attained
by an endpoint-honest regularized L-curve. -/
...
/-- On a complete bounded-curvature flow, every endpoint reached by a global
`C¹` competitor is reached by an initial tangent in the minimizing L-exp
domain. -/
