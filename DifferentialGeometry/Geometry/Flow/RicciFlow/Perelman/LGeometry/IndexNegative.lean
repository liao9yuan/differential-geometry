...
/-!
# Negative directions from regularized L-conjugacy

This file develops the L-specific negative-index producer used to exclude
strictly interior conjugate points on minimizing L-rays.
-/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] in
/-- A zero prefix direction with a positive cross index yields a negative
pair of broken quadratic directions for a suitable constant scale. -/
...
/-- A tangent vector at one point of a globally smooth curve extends to a
globally smooth polynomial test field vanishing at two prescribed times. -/
...
/-- A nonzero canonical regularized L-Jacobi field with a positive-time zero
has nonzero moving covariant derivative at that zero. -/
theorem lRegJacobi_d_ne
  ...
  have hvel : ∀ s ∈ J, DifferentiableAt Real
    ...
    have hcurve : ContMDiffAt 𝓘(Real, Real) I ∞
      ...
      simpa only [z] using (lRegCurve_smooth S hS T x hs).comp s hpair
  ...
  have hzeroOn : ∀ s ∈ J, Y s = 0 := by
    intro s hs
    have hsEq := heq hs
    simpa only [sub_self] using hsEq
...
/-- Pairing a regularized L-Jacobi field with a test field pointing along its
nonzero terminal covariant derivative gives positive cross index. -/
...
omit [InnerProductSpace Real E] in
/-- An interior regularized L-conjugate point produces two matching smooth
index directions with negative total index across the conjugate node. -/
theorem lIndex_neg_conj
  ...
  have hgeo : IsLRegCurveOn S T gamma (Set.uIcc (0 : Real) b) x Z := by
    ...
    · have hvel : lVelocity (I := I) gamma 0 =
        ...
        simpa only [lVelocity] using congrArg
          ...
          (h0germ.mfderiv_eq (I := 𝓘(Real, Real)) (I' := I))
  ...
  have hreg0c : ∀ s ∈ Set.uIcc (0 : Real) c,
    ...
    exact (hgeo.2.2 s (by
      simpa only [Set.uIcc_of_le hb0.le] using
        (⟨hs'.1, hs'.2.trans hcb.le⟩ : s ∈ Set.Icc (0 : Real) b))).1
  have hregcb : ∀ s ∈ Set.uIcc c b,
    ...
    exact (hgeo.2.2 s (by
      simpa only [Set.uIcc_of_le hb0.le] using
        (⟨hc0.le.trans hs'.1, hs'.2⟩ : s ∈ Set.Icc (0 : Real) b))).1
  ...
  have hA : ∀ s ∈ Set.uIcc (0 : Real) c, DifferentiableAt Real
    ...
    have hsBig : s ∈ Set.uIcc (0 : Real) b := by
      simpa only [Set.uIcc_of_le hb0.le] using
        (⟨hs'.1, hs'.2.trans hcb.le⟩ : s ∈ Set.Icc (0 : Real) b)
  ...
  have hYW : 0 < lRegIndex S T alpha J W 0 c :=
    lIndex_cross_pos (I := I) S hS T alpha J W c b hc0 hcb
      ...
      (by simpa only [P] using hWc) hPne
  ...
  have hY0smooth : ContMDiff (modelWithCornersSelf Real Real) I.tangent ∞
    ...
    simpa only [Y0, Y1, JgE] using hJgSmooth'.add_bundle hkW
  have hJg0 : Jg 0 = 0 := by
  ...
  · change JgE c + k • W c = k • W c
    rw [hJgEc, zero_add]
  · simpa only [Y0, Y1, JgE, c, b] using hk
