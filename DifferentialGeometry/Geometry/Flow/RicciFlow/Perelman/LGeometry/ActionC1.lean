...
/-!
# Integrability of the regularized Lagrangian for C1 curves

This file assembles the fixed-chart kinetic and scalar-curvature integrability
results over a finite chart subdivision of a compact parameter interval.
-/
...
/-- The regularized Lagrangian of a `C¹` curve is interval integrable on a
compact parameter interval contained in regular backward times. -/
theorem lRegLag_int_c1
  ...
  have hkinPiece (n : Nat) : IntervalIntegrable
    ...
    have hrep : EqOn us.toFun
      ...
      simpa only [us, Function.comp_apply] using
        chartTimeH1_toFun I (sub_nonneg.mpr (hseg n)) p
          (fun r : Real ↦ alpha ((t n : Real) + r)) hlocalMD hlocalSrc
  ...
  simpa only [lRegLag] using hkin.add hpot
