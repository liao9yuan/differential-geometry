...
/-!
# Base-point upper continuity of compact L-cost

This module perturbs the initial endpoint of a fixed positive-time competitor
by an affine chart ramp on a short initial square-root-time interval.
-/
...
/-- Affine chart perturbations of the initial endpoint have actions converging
to the action of the unperturbed chart head. -/
theorem chart_head_act_lim
  ...
  have hlim := lAction_h1_lim (I := I) S hMet hSc T a c t htmono
    (by simp [t]) (by simp [t]) (fun _ ↦ p) beta beta₀
    (fun i n ↦ by
...
omit [NeZero (Module.finrank Real E)] in
/-- At fixed positive backward time, a strict competitor-action upper bound
persists when the initial base point converges. -/
