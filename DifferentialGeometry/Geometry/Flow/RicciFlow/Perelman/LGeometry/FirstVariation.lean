...
/-!
# Pointwise first variation of Perelman L-density

This file differentiates the L-density in the variation parameter.  The
backward time is held fixed, so the metric is fixed in this calculation; the
moving-metric product rule in `MovingMetric` is reserved for the later
integration-by-parts step in backward time.
-/
...
/-- The squared L-speed of a smooth variation is jointly `C¹` wherever the
backward time corresponds to a regular flow time. -/
theorem lSpeedSq_contDiffOn
  ...
  have hscalar : ContMDiffAt
    ...
    simpa only [lSpeedSq] using htotal.2
...
/-- Scalar curvature along a smooth variation is jointly `C¹` wherever the
backward time corresponds to a regular flow time. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- The scalar-curvature plus speed-square part of the L-density is jointly
`C¹` wherever the backward time corresponds to a regular flow time. -/
...
private theorem lPair_c2
  ...
  have hYall : ContMDiff
    ...
    simpa only [Function.comp_apply, lVelocity] using hcomp
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- The moving-metric pairing of the variation field with the central
L-velocity is `C¹` at regular backward times. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- Pointwise variation of scalar curvature at fixed backward time. -/
theorem lScalar_var_deriv
  ...
  have hRsm : ContMDiff I 𝓘(Real, Real) (∞ : WithTop ℕ∞)
    ...
    simpa [SolutionOn.scalar, SolutionFamily.scalar] using
      metricScalar_smooth (I := I) (M := M) (S.family.metric (T - tau))
  have hR : MDifferentiableAt I 𝓘(Real, Real)
      (S.scalar (T - tau)) (f 0 tau) :=
    hRsm.contMDiffAt.mdifferentiableAt (by simp)
  have hraw : HasDerivAt
      (fun u : Real => S.scalar (T - tau) (f u tau))
      (mfderiv I 𝓘(Real, Real) (S.scalar (T - tau)) (f 0 tau)
        (mfderiv 𝓘(Real, Real) I (fun u : Real => f u tau) 0 (1 : Real))) 0 := by
    have hcomp := hR.hasMFDerivAt.comp 0 hf.hasMFDerivAt
    simpa only [Function.comp_apply, ContinuousLinearMap.comp_apply] using
      hcomp.hasFDerivAt.hasDerivAt
  apply hraw.congr_deriv
  simpa only [lVelocity] using
    (inner_gradientFun (I := I) (S.base.metric (T - tau))
      (S.scalar (T - tau)) (f 0 tau)
      (lVelocity (I := I) (fun u : Real => f u tau) 0)).symm

omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [T2Space M] [SigmaCompactSpace M] in
/-- Pointwise variation of the squared L-speed at fixed backward time. -/
...
/-- Pointwise derivative of the L-density in a smooth variation parameter. -/
theorem lDensity_deriv
  ...
  simpa only [lDensity, zero_mul, zero_add] using
    (hasDerivAt_const (x := (0 : Real)) (c := Real.sqrt tau)).mul hadd

omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Differentiation of L-length under a smooth variation on an interval whose
backward times are regular.  The domination bound is produced internally from
joint `C¹` regularity on a compact parameter rectangle. -/
theorem lLength_deriv
  ...
  have hdensJoint : ContinuousOn (fun p : Real × Real => dens p.1 p.2) U := by
    simpa only [dens] using
      (Real.continuous_sqrt.comp continuous_snd).continuousOn.mul hcore.continuousOn
  ...
  have hdDensJoint : ContinuousOn (fun p : Real × Real => dDens p.1 p.2) U := by
    simpa only [dDens] using
      (Real.continuous_sqrt.comp continuous_snd).continuousOn.mul hdcoreCont
  ...
  have hdensDeriv (u tau : Real) (htau : tau ∈ Set.uIcc a b) :
    ...
    simpa only [dens, dDens, zero_mul, zero_add] using
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- The scalar-gradient term in the L first variation is continuous on every
compact regular backward-time interval. -/
theorem lGrad_contOn
  ...
  have hdScalarCont : ContinuousOn dScalar (Set.uIcc a b) := by
    ...
    simpa only [dScalar] using
      hpartScalar.comp hmap (fun tau htau => ht tau htau)
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [SigmaCompactSpace M] in
/-- The scalar-gradient term in the L first variation is interval integrable
on every compact regular backward-time interval. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Backward-time derivative of the moving metric pairing of the variation
field with the central L-velocity. -/
theorem lPair_deriv
    (S : SolutionOn (I := I) (M := M) D) (hS : IsSolutionOn (I := I) S)
...
-- The weak-assumption moving-metric boundary rule needs extra elaboration budget.
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Derivative of the weighted boundary pairing in the L first-variation
formula at positive backward time. -/
theorem lBdry_deriv
    (S : SolutionOn (I := I) (M := M) D) (hS : IsSolutionOn (I := I) S)
    (T : Real) (gamma : Real → M)
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Integration by parts for a moving-metric pairing on a positive oriented
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The two mixed variation-velocity terms used in L integration by parts are
continuous on positive compact regular backward-time intervals. -/
theorem lCross_contOn
  ...
  have hdSpeedCont : ContinuousOn dSpeed (Set.uIcc a b) := by
    have hmap : ContinuousOn (fun tau : Real => ((0 : Real), tau))
        (Set.uIcc a b) := continuousOn_const.prodMk continuousOn_id
    simpa only [dSpeed] using hpartSpeed.comp hmap (fun tau htau => ht tau htau)
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The two mixed variation-velocity terms used in L integration by parts are
interval integrable, as a consequence of their joint `C¹` producers. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Integration by parts for the mixed variation-velocity term in the L
first-variation formula, with integrability produced from joint `C¹`
regularity. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- Perelman's first-variation formula for L-length on a positive regular
backward-time interval. -/
...
/-- The intrinsic L-geodesic Euler residual paired with a tangent vector. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] in
/-- The intrinsic L-Euler pairing is linear in its test vector. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The weighted Euler residual of a smooth variation is continuous on every
positive compact regular backward-time interval. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The weighted Euler residual of a smooth variation is jointly `C¹` at
positive regular backward times. -/
...
omit [InnerProductSpace Real E] [NeZero (Module.finrank Real E)]
  [SigmaCompactSpace M] in
/-- The L-length first variation written as the endpoint term plus the
Euler residual paired with the variation field. -/
