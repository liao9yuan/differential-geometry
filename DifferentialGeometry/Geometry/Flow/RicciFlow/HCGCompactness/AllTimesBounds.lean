...
/-- `componentRS` and the generalized `componentRS_gen` are definitionally equal.
This bridges the source proofs (written with `componentRS`) to the
DifferentialGeometry component lemmas now stated with `componentRS_gen`. -/
...
/-- Smooth-slot expansion of the third background covariant derivative of a
metric tensor.  The first slot is the new derivative direction; the remaining
four slots are the slots of `nabla^2 h`. -/
...
set_option linter.unusedDecidableInType false in
/-- Component bridge from the third HCG metric covariant derivative to the
coordinate/local-frame third covariant derivative of metric components. -/
...
metrics, while the Christoffel estimates in `DifferentialGeometry.Integral.Connection.Variation.Connection`
...
as `DifferentialGeometry.Integral.Connection.metricCovAtBase` for the canonical Levi-Civita metric family.
