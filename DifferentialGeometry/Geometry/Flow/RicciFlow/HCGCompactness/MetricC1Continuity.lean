...
theorem metricCov_smooth
  ...
  have htower :
    ...
    have hbase :
      ...
      simpa only [Tensor0SBundle.metricTensorField_apply] using
        hG.pairSmoothAt (D.regular_isOpen.mem_nhds T.2) W
    simpa only [metricCovDeriv_eq_covDerivOfField] using
      covDerivOfField_eval_contMDiffAt (I := I) (G.metric (T : Real))
  ...
  have hev' : ∀ᶠ p : Real × M in 𝓝 ((T : Real), x),
      ∀ j : Fin (a + 2), V j p.2 = frame (slots j) p.2 :=
    (continuous_snd.tendsto ((T : Real), x)).eventually hev
