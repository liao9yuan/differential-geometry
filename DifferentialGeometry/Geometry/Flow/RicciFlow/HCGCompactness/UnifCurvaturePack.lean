...
/-! ### The object-level tower identity

`rfns_ccOfField_eq` transports *norms*.  The two towers in fact agree as
objects, which transports everything else (linearity, `toL2`/`toHs`, any later
functional of the jet) through the same packaging.  What makes the object-level
statement available is that an upper-rank-zero fibre is one dimensional, so a
`(0, s)`-rank section is determined by its unit field. -/

omit [NeZero (Module.finrank ℝ E)] [CompactSpace M] [I.Boundaryless] in
/-- Every `(0,0)`-tensor is the unit `(0,0)`-tensor rescaled by its own scalar
readout. -/
...
omit [NeZero (Module.finrank ℝ E)] [CompactSpace M] [I.Boundaryless] in
/-- Evaluating a `(0, s)`-rank fibre element on an arbitrary `(0,0)`-input gives
its unit value, rescaled by the scalar readout of the input. -/
...
/-- **A `(0, s)`-rank section is determined by its unit field.**  The converse
direction of `ccUnitField`: agreeing on the parallel unit `(0,0)`-tensor at every
point is agreeing outright, because the `(0,0)`-fibre is one dimensional. -/
...
The section-level iterated covariant gradient of a packaged field is the
packaging of the intrinsic `iterCov` tower of that field:

`iteratedCovGrad g 0 s j (ccOfField g s A) = ccOfField g (s + j) (iterCov g s A j)`.

Both sides are `(0, s + j)`-sections, so no arity cast and no combinatorial
factor enters.  This is the strongest form of the tower bridge: `ccOfField_unit`
and `rfns_ccOfField_eq` are its unit-value and norm-level faces, and any other
functional of the jet transports through it verbatim.

Proof: the unit fields agree (`iterCovGrad_unit_eq` composed with
`ccOfField_unit`), and `cc_ext_unit` upgrades that to equality of sections. -/
...
/-- **The curvature jet is the packaged curvature derivative.**

The object-level face of `rfns_rmSection_eq`: the `a`-th section-level iterated
covariant gradient of `rmSection g` *is* the packaging of `∇^a Rm(g)`, written in
the generic currency as `iterCov g 4 (metricRm04 g) a`.  Consequently every
statement about the packaged field — not only its fibre norm — applies verbatim
to the curvature jet the `SmoothCcTensor` consumers hold. -/
