...
/-!
# Curvature forcing bounds for the H6 Jacobi recursion

This file projects the order-zero and order-one HCG curvature bounds onto the
vector-valued curvature terms in the differentiated intrinsic Jacobi equation.
-/
...
/-- A zeroth HCG curvature bound controls `curvAlong` on arbitrary fields. -/
...
/-- A first HCG curvature bound controls the actual smooth along-curve
curvature derivative appearing in the differentiated Jacobi equation. -/
...
/-- Metric length of six left-associated summands is bounded by the sum of
their metric lengths. -/
...
/-- The explicit forcing in the first launch derivative of the Jacobi
equation is controlled by the order-zero and order-one HCG curvature bounds.
The right side contains exactly two `∇R` products and four `R` products; the
third product carries the coefficient two from `jacVarForce`. -/
...
/-- The actual forcing generated by the complete intrinsic two-launch family
obeys the pointwise `C0`/`C1` curvature estimate. All regularity hypotheses of
`jacVarForce_le` are discharged by the jointly smooth intrinsic launch maps. -/
