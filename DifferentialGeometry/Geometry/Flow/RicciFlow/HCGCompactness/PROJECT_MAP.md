...
|---|---|---|---|
| `InjRadiusDecayInput` (A0) | `lbl384` (CGT) | **TRUE statement; theorem 0%, native pointwise machinery about 71–75%** | The decay formula `a·min{ρ,1}^n·e^{−C·d}` matches the book and its constants are uniform in `k`. The intrinsic framed pullback metric, local curvature/Jacobi positivity, complete compact perturbation, first-hit fenced minimizing producer `exists_fenced_min`, and core equality `intrCore_edist_eq` between the true pullback and complete-extension distances are focused-green; the incomplete carrier is no longer the distance seam. The next true producer is `intrCore_dist_germ`: a localized Whitehead short-cut/unique-minimizer theorem making the minimizing inverse branch represent true distance near every core endpoint. Only then do the existing Hessian/Jacobi consumers yield `intrCore_jensen` (theorem 0%, dedicated machinery about 78–82%), finite-center uniqueness, CGT Lemma 4.6, and the collision-to-loop/injectivity corollary. Connectedness is handled by the open-ball subtype or, for a legacy ambient capstone, restriction to `connectedComponent p`; it is not added globally. The final `HasInjRadiusAt` backend must use intrinsic tangent balls, equivalently orthonormally framed model balls. |
