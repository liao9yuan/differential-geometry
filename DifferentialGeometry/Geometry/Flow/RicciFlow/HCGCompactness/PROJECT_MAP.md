...
| Uniform low-regularity existence `(N)` | `../ShortTime/UNIF_EXISTENCE_PLAN7.md` + `../ShortTime/LowRegUnifBounds.lean` + `../ShortTime/LowRegBgBootstrap.lean` | **Class-first bounds/common low solve: 100%; same-horizon H2 representative: 100%; packet endpoint and final gauge assembly: 100%; fixed-background metricwise lift certificate `IsBgLiftAt` per `(g, gB)`: 100% (`bgLift_of_radial`, ledger №212), class-uniform: 0% (the G3 `_unif` lane, ~2/56 nodes); theorem `(N)`: 0%.**  `lowreg_solve_unif` chooses one positive horizon with fixed background `gBase` for every dimension-three order-three class metric.  `lowSolve_cross` constructs the canonical closed-slab H2 representative directly from `IsLowSolveBg`, without shrinking that horizon.  `BgSmoothPacket`, `dt_of_bg_packet`, `lowreg_dt_unif`, and the body of `ricci_flow_unif_existence` are verified.  The only direct `sorryAx` is the single same-horizon producer `bg_packet_of_solve`; its first remaining output is a fixed-background order-two maximal-regularity lift, after which the self-background all-order mass/jet assembly must be generalized.  The qinz theorem confirms one-sided smoothness to `t=0`, but has per-metric `∀g₀,∃T` quantifiers and cannot fill this fixed-background leaf.  Whole HCG closure remains approximately 3%. |
