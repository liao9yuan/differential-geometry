...
| Unconditional Thm 3.9 `metricCompactness` | `MetricCompactness.lean` | 0%; the conditional endpoint is available and the Bishop–Gromov/uniform-packing producer is now complete, but the native CGT, [H6], and connectedness producers needed to instantiate it remain external to this Chapter 4 assembly. |
...
| A0′ volume-overlap producer (unconditional 3.9) | `C4/A0PRIME_VOLUME_PLAN.md` | **COMPLETE 2026-07-27. Endpoint `volInput_of_bg`: 100%; dedicated volume machinery: 100%.** `segBall_vol_le`, `segBall_vol_fin`, and `segBall_vol_rel` are proved and exact-current. `VolumeOverlap` replays exact GREEN (3982 jobs), and `#print axioms volInput_of_bg = [propext, Classical.choice, Quot.sound]`, with no `sorryAx`. Honest inputs remain `RealizesEdist`, `SeqMetricComplete`, and per-member connectedness. This closes the A0′ producer, not unconditional Theorem 3.9, which remains theorem-level 0% pending the other native time-zero producers. |
...
|---|---|---|---|
| `InjRadiusDecayInput` (A0) | `lbl384` (CGT) | **TRUE statement; theorem 0%, native pointwise machinery about 85%** | The decay formula `a·min{ρ,1}^n·e^{−C·d}` matches the book and its constants are uniform in `k`. The localized Whitehead/Jensen lane remains closed through `intrCore_dist_germ` and `intrCore_jensen`. The paper-facing native form of CGT Lemma 4.6 is now theorem-level 100%: `intrIter_family` constructs `N+1` pairwise distinct loop-transport endpoints in the basepoint fibre with the sharp `i·L` norm bound; `intrFiber_encard_ge` converts this to a base-fibre count; and `intrFiber_count_ge` composes Lemma 4.5 to propagate the same count over the short target ball. `CGTPropeller` is focused/exact-current (`3975/3975`), and direct axiom audits of all three endpoints report only `propext`, `Classical.choice`, and `Quot.sound`, with no `sorryAx`. `intrLoop_ge_cgt` itself remains theorem 0% with about 90% dedicated machinery: its exact remaining frontier is a multiplicity-sensitive area theorem converting the pointwise inverse-fibre `encard` lower bound into a Jacobian-integral lower bound. The existing noninjective image inequality counts the image only once, while the exact formula assumes an injective source. No new geometric input, fake pullback completeness, or global `ConnectedSpace` was introduced. The sequence `InjRadiusDecayInput` and unconditional Theorem 3.9 remain theorem-level 0%; whole HCG supporting machinery is about 64%. The final `HasInjRadiusAt` backend must use intrinsic tangent balls, equivalently orthonormally framed model balls. |
...
| `NormalCoordMetricBoundInput` / `NormalRadiusProfile` | `lbl395` = [H6] Cor 4.12 + CGT compatibility | **conditional consumers checked; native theorem 0%, producer machinery about 40%** | The total intrinsic framed map, pullback metric, uniform half/two estimate, differential injectivity, nonconjugacy, and pointwise branches are exact-green. The whole-plan ruling in `C4/H6_RADIUS_CONSULT.md` retains Route A but upgrades Route C to branch-carrying `H6NormalData`: first migrate `HasInjRadiusAt` to intrinsic injectivity, then combine `hd.decay` with the uniform local-diffeomorphism ball and generic glue, migrate consumers through a branch-parametric provider, and separately prove the all-order metric-jet induction. Do not lower-bound the legacy qualitative `expRadiusGp` or replace it by an injectivity-only radius. |
