...
| Hamilton positive-Ricci main `ham3_main_hcg` | `HamiltonPositiveRicciAdapter.lean` | **Source replacement for `ham3_flow_exists_normalized` is written without a local `sorry`; axiom-clean endpoint remains 0% pending exact refresh and direct audit.** `Closed3Manifold` explicitly carries connectedness. The provider-native route bypasses legacy `ham3_cgh_limit`; `flowInj_of_vol`, `exists_ham3_vol`, and `ham3_const` are exact-green and axiom-clean. The adapter's previous artifact is exact-green `10470/10470`. Current verification is blocked by the missing imported `DeTurckRemainderLowBaseH2Pair.olean`, not by a reported error in the maximal-flow source. |
...
| Uniform low-regularity existence `(N)` | `../ShortTime/ROUTE_C_PLAN.md` + `../ShortTime/LowRegBgC1Pair.lean` + `../ShortTime/LowRegA1FixedPairBg.lean` + `../ShortTime/LowRegBgBootstrap.lean` | **Theorem `(N)`: 0%; dedicated fixed-background direct-smoothing machinery: approximately 94%; conditional metricwise adapted-to-mass-to-packet chain: 100%; `lieBgCorr_unif`: 100%; `lowC1Corr_unif`: 100%; `galA1FixPair3_le` and signed C1 split: 100%; finite spectral/diagonal Galerkin transport: 100%; exact diagonal low-base path/centered algebra: 100%; class-first principal, raw Cross, and diagonal-pair estimates: 100%; exact complete-edge peel: unstated, 0%; rest-only Rung-3 theorem: unstated, 0%.**  The proposed arbitrary-acted-field refold remains rejected: the true cross orientation `Top(U) (nabla^2 P)` cannot be replaced by `Top(P) (nabla^2 U)`.  The corrected route keeps state=acted=`T`; focused-green `b02_center_nf` and `edge_center_s_nf` isolate the exact complete-edge block without double-counting Cross.  The covariant argument corner genuinely contains `(nabla^2 Rm(g)) * T`, and the complete transparent action is generally nonzero, but this does not fire the STOP condition: the cap order is `eta -> exists delta2,R2 -> g -> exists G_g`, so a fourth jet of the fixed smooth `g` may enter the lower `G_g * H3^2` term when it carries at most one `H4` factor and at least one actual lower state norm.  The live frontier is the non-Green principal-head-isolating `edge_center_peel`, followed by sharp `P20/P11` and self-low carrier estimates.  STOP applies only if those leave a naked or metricwise coefficient multiplying `H4^2`, require a high state ball, differentiate the test to `H5`, or force a cap to depend on `g`.  `lowbase_full3_unif` remains 0%.  The existing `bg_packet_of_solve` still contributes the endpoint `sorryAx`; verified conditional consumers do not close its missing class-first producer.  Whole HCG closure remains approximately 3%. |
...
| `H6NormalData` / legacy `NormalRadiusProfile` | `lbl395` = [H6] Cor 4.12 + CGT compatibility | **branch-carrying H6 producer 100%; provider substitution 100%; legacy profile theorem 0%** | `exists_h6NormalData` closes the intrinsic injectivity, uniform whole-ball branch, half/two metric equivalence, and all-order pullback-metric derivative package from the intended sequence inputs. `exists_supp_data`, the H6 target decode, root/jet consumers, and `exists_stage_data` now keep the same provider through the complete selected stage-data chain. Do not infer or lower-bound the legacy qualitative `expRadiusGp`; `NormalRadiusProfile.le_exp_radius` remains an independent migration theorem. |
...
- Ch3: Lemma 3.11 is done under its cited `hShi` hypothesis.  The canonical
  ...
  **2026-07-26 Hamilton derivative input:** the strict buffered source,
  ...
  about 60%.
  **2026-07-27 closed Hamilton limit solution:** the canonical source now has
  exact-green finite stage/limit spatial jets, closed-window chart-Gram
  smoothness, the invariant limit metric PDE, and
  `HamiltonPositiveRicciAdapter.ham3_limit_soln`.  The last theorem is
  focused/exact-green (`10190/10190`), and its axiom replay contains only
  `propext`, `Classical.choice`, and `Quot.sound`.  Thus the closed
  `IsSolutionOn` theorem and its dedicated common-window solution machinery
  are 100%.  This does not prove `ham3_cgh_limit`, which remains theorem-level
  0%.  The next conditional producer is a closed-window `FlowUpgradeData` plus
  all-slice completeness assembly; the next unconditional producer is
  `MetricCompactBase` for the time-zero Hamilton source.  Fixed-scale Perelman
...
- **Hamilton E1 low-regularity short-time input:** the per-metric low-regularity
  ...
  census contains only the standard three axioms.  The class-first
  tame/continuity producer, literal packet `exists_lowBounds`, common envelope
  `lowreg_bounds_unif`, and unconditional common low solve
  `lowreg_solve_unif` are now also focused/exact-green and axiom-clean.  The older
  ...
  bootstrapped on the same horizon after the common low solution.  Thus
  `lowreg_bounds_unif` and `lowreg_solve_unif` are theorem-level 100%, while
  the class-uniform geometric DeTurck endpoint and
  `ricci_flow_unif_existence` remain theorem-level 0%; see the live ledger
  `../ShortTime/UNIF_EXISTENCE_PLAN7.md` (No. 212) and the producer consult
  `../ShortTime/CONSULT_UNIF_TAME_PRODUCER_FOLLOWUP.md`
  (PLAN6/PLAN5/PLAN4/PLAN3/PLAN2/PLAN frozen).
...
- **B/C global finite-stage map (updated 2026-07-16):** `stageTarget`,
  ...
  and 0%.  Rounded machinery estimates are approximately 100% / 95% / 72% for
  the selected B/C producer route / Chapter 4 / whole HCG.
- **Provider-native H6 B1/Step-D assembly (updated 2026-07-29):** the same
  `H6NormalData.chart` now flows through target decoding, root readout, the jet
  tail, intrinsic forward/reverse estimates, the exact-inverse metric carrier,
  `StepB1RawInput`, and the Step-D compactness consumer.  The canonical H6
  stage selection, intrinsic metric bridge, carrier, raw producer, and
  conditional endpoint are exact-green respectively through `4150/4150`,
...
`DimensionThree/HamiltonPositiveRicci.lean` now has one proof `sorry`:
legacy `ham3_cgh_limit`, which the provider-native route bypasses.  The source
body of `ham3_flow_exists_normalized`, the uniform-existence producer on the
`ham3_main_hcg` dependency path, has been replaced, but exact refresh and axiom
audit remain pending on a missing imported DeTurck artifact.  Therefore the
axiom-clean endpoint is not yet claimed. `Evolution/BernsteinComplete.lean`
