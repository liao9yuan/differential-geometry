...
| Uniform low-regularity existence `(N)` | `../ShortTime/ROUTE_C_PLAN.md` + `../ShortTime/LowRegBgC1Pair.lean` + `../ShortTime/LowRegA1FixedPairBg.lean` + `../ShortTime/LowRegBgBootstrap.lean` | **Theorem `(N)`: 0%; dedicated fixed-background direct-smoothing machinery: approximately 96%; conditional metricwise adapted-to-mass-to-packet chain: 100%; `lieBgCorr_unif`: 100%; `lowC1Corr_unif`: 100%; `galA1FixPair3_le` and signed C1 split: 100%; finite spectral/diagonal Galerkin transport: 100%; exact diagonal low-base path/centered algebra: 100%; class-first principal, raw Cross, and diagonal-pair estimates: 100%; exact complete-edge peel: 100%; centred top-kernel fibre caps, principal-face pairing, and nonzero curvature-defect pairing: 100%; rest-only Rung-3 theorem: unstated, 0%.**  The proposed arbitrary-acted-field refold remains rejected: the true cross orientation `Top(U) (nabla^2 P)` cannot be replaced by `Top(P) (nabla^2 U)`.  The corrected route keeps state=acted=`T`; focused-green `b02_center_nf`, `edge_center_s_nf`, and `edge_center_peel` isolate the complete block without double-counting Cross.  The covariant argument corner genuinely contains `(nabla^2 Rm(g)) * T`, but the verified `bg_pair_abs_unif` places it in the lower `G_g * H3^2` term under the target quantifier order.  The live frontier is the sharp `P20/P11` corner estimate, followed by the self-low carrier estimate and diagonal G̴rding assembly.  STOP applies only if those leave a naked or metricwise coefficient multiplying `H4^2`, require a high state ball, differentiate the test to `H5`, or force a cap to depend on `g`.  `edge_center_h4_unif`, `lowbase_full3_unif`, and the rest-only theorem remain 0%.  The existing `bg_packet_of_solve` still contributes the endpoint `sorryAx`; verified conditional consumers do not close its missing class-first producer.  Whole HCG closure remains approximately 3%. |
