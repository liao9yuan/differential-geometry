...
| Uniform low-regularity existence `(N)` | `../ShortTime/ROUTE_C_PLAN.md` + `../ShortTime/LowRegBgC1Pair.lean` + `../ShortTime/LowRegA1FixedPairBg.lean` + `../ShortTime/LowRegBgBootstrap.lean` | **Theorem `(N)`: 0%; dedicated fixed-background direct-smoothing machinery: approximately 92%; conditional metricwise adapted-to-mass-to-packet chain: 100%; Rung-3 full Lie correction `lieBgCorr_unif`: 100%; actual C1 path producer `lowC1Corr_unif`: 100%; corrected Galerkin pairing theorem `galA1FixPair3_le`: 100%; signed C1 forcing split: 100%; complementary spectral split and `finite_symm_scale`: 100%; polarized monomial Green identity: 100%; rest-only Rung-3 theorem: unstated, 0%.**  The old G3/adjacent-scale and closed-slab all-rung class-first gates are superseded because they force fourth and higher varying-metric jets into the common absorption budget.  The complete C1 correction is focused-check green, uses no varying fourth metric jet, and retains its correct `eta -> g -> G` order with `G` independent of the later capped solver radius.  The former absolute-value C2-only blocker is also superseded: the binding route recombines C0+C2 and seeks the homogeneous one-sided producer `lowbase_full3_unif`, with cap order `eta -> exists delta2, R2 -> g -> exists G`, public caps `0 < delta2 < 1/3` and `0 < R2 <= 1`, and conclusion `eta * H4^2 + G * H3^2`.  Only the later raw/rest consumer weakens the lower term to `G * (1 + E3)^2`.  `finite_pair_split` at `a = 1`, `b = 2`, `finite_symm_scale`, and the polarized monomial theorems through `edgePair_green_bi` are complete; no step divides by `theta`.  The exact remaining analytic frontier is the low-base full-slope adapter plus its C0+C2 commutator/Gårding estimate, canonically targeted for future `LowRegBgC2Pair.lean`; a later rest consumer may live in `LowRegA1RestPairBg.lean`.  The complete low-base C2 kernel is not definitionally the closed-edge `edgeTopPair`, so neither that identification nor a raw `oneMinus_appCc2_comm` estimate is treated as the binding theorem, and no reusable lower-DeTurck statement is claimed in advance.  No partial rest theorem or stronger assumption has been added.  Rungs 4+ remain metricwise post-construction work.  The existing `bg_packet_of_solve` still contributes the endpoint `sorryAx`; verified conditional consumers do not close its missing class-first producer.  Whole HCG closure remains approximately 3%. |
