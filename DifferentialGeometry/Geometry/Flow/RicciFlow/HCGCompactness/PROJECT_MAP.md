...
| Ch3 P4 producer lane (3.10 ⇐ 3.9) | `P4_CONV_PLAN.md` + `P4_PRODUCER_RULING.md` + `ConvFieldOpenAssembly.md` | fixed-window/open consumer assembly is checked through `open_upgrade_of_raw`; the grow-only `hcovTail` migration is green and the Step-D provenance branch is closed. `movingShi_of_bound`, `movingShi_complete`, and `CurvBoundInput.movingShi_open` are green wrappers, not trusted analytic producers. The direct costs, coefficient monotonicity, and arbitrary-index level-zero reaction algebra are focused-green; the first analytic frontier is the arbitrary-dimensional identity `partial_t Rm04 = roughLap Rm04 + rmBaseReact`, followed by the successor residual/direct tower assembly. Independently, complete Bernstein still needs the corrected solution-produced cutoff/Kato localization chain. `srcCovLip_of_soln` is also unproved. Thm 3.10 stays 0%; dedicated P4 consumer machinery is about 97%. |
