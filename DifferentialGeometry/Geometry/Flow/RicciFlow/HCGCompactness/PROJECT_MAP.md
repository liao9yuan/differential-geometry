...
| Native [H6] normal-coordinate producer | `C4/H6_RADIUS_CONSULT.md` + `C4/H6NormalData.md` | **100% for the branch-carrying producer.** `exists_h6NormalData` is focused/exact-green (`3983/3983`): completeness, connectedness, sequence bounded geometry, and the realized CGT injectivity profile construct one uniform whole-ball chart package with half/two metric equivalence and all-order metric-derivative bounds. No extra radius or compatibility assumption is introduced. Provider substitution into the selected support/root consumer remains open; the legacy `NormalRadiusProfile.le_exp_radius` theorem is separate and still 0%. |
...
| `H6NormalData` / legacy `NormalRadiusProfile` | `lbl395` = [H6] Cor 4.12 + CGT compatibility | **branch-carrying H6 producer 100%; legacy profile theorem 0%** | `exists_h6NormalData` now closes the intrinsic injectivity, uniform whole-ball branch, half/two metric equivalence, and all-order pullback-metric derivative package from the intended sequence inputs. The selected Step B/C route still needs provider substitution through its chart-parametric support/root chain. Do not infer or lower-bound the legacy qualitative `expRadiusGp`; `NormalRadiusProfile.le_exp_radius` remains an independent migration theorem. |
