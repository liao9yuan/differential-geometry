...
| Ch3 P4 producer lane (3.10 ⇐ 3.9) | `P4_CONV_PLAN.md` + `P4_PRODUCER_RULING.md` + `P4_CUTOFF_CONSULT.md` + `DISTANCE_CALABI_CONSULT.md` + `CALABI_BRANCH_CONSULT.md` + **`CALABI_BRANCH_PLAN.md`** + `ConvFieldCanon.md` | fixed-window/open consumer assembly is checked through `open_upgrade_of_raw`; the grow-only `hcovTail` migration is green and the Step-D provenance branch is closed. `rm04Base_of_solution_any`, `e0Residual`, the arbitrary-index fixed successor `resStarNext`, global residual capstone `rmResidual_cost`, direct tower `towerHeatSol_raw`, HCG `rmTowerCost`, constants-first `srcCovLip_of_soln`, and `open_upgrade_canon` are focused- and exact-green. Both the smooth fixed-order `BernsteinTower.estimate_cutoff_at` and family-centered `estimate_barrier_at` are exact-current; the private barrier adapter in `MovingShiOpen` is focused-green. Route B-prime is selected for complete Shi.  The 2026-07-24 ruling fixes the early Calabi split at `s₀ = 1/4`, selects one canonical fixed-first `ExpInvBranch`, requires nonconjugacy on `(0,b)` for some `b>1`, and uses a whole-tail intrinsic Bishop comparison with a separate dimension-one branch. `ExpInvBranch`, `branch_of_not_conj`, `DiagInvBranch.fixed`, connectivity-free `minExp_of_ne_top`, the migrated fixed-first radius/Hessian/Laplacian chain, and `exists_intrMean` (including `Fin 0`) are now focused-green; the proof-owning branch/IFT and first calculus modules are exact-current. `CalabiTailData` is stated and focused-green. The remaining fixed-metric theorem frontier is conjugacy reversal plus `tail_no_conj`, followed by `exists_calabiTail` and `calabiDist_support`. Its upper-support maximum principle, barrier data boundary, basepoint-free completeness, finite closed-eball compactness, and fixed-path Ricci-flow length variation are already checked. The new producer chain is tracked in `CALABI_BRANCH_PLAN.md`; the spatial evolving-distance Calabi support, solution-generated barrier cutoff, and final MovingShi switch remain open. Thm 3.10 stays 0%; dedicated P4 consumer machinery is about 98%, while selected Route B-prime producer machinery is about 45%. |
