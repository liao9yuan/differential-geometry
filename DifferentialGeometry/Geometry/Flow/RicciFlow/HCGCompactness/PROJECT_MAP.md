...
| Unconditional Thm 3.9 `metricCompactness` | `MetricCompactness.lean` | 0%; the conditional endpoint and the native CGT, Bishop–Gromov overlap/total-packing, and branch-carrying [H6] producers are complete. Per-member connectedness is now an explicit public hypothesis. The remaining frontier is H6 provider substitution plus final native `MetricCompactBase` assembly. |
...
| Native [H6] normal-coordinate producer | `C4/H6_RADIUS_CONSULT.md` + `C4/H6NormalData.md` | **100% for the branch-carrying producer.** `exists_h6NormalData` is focused/exact-green (`3983/3983`): completeness, connectedness, sequence bounded geometry, and the realized CGT injectivity profile construct one uniform whole-ball chart package with half/two metric equivalence and all-order metric-derivative bounds. No extra radius or compatibility assumption is introduced. Gate 5 provider substitution is about 45% and is now stopped at one quantitative projection from the concrete producer: retain `ratio * mu 0 <= 1/2` after capping the launch radius by one. Without it, target-stage decode requires the unproved constant inequality `5 + 9 * exp(C * (10 * lambda D 0)) < 384`; see the live `C4/STEPB_PLAN.md`. The legacy `NormalRadiusProfile.le_exp_radius` theorem is separate and still 0%. |
