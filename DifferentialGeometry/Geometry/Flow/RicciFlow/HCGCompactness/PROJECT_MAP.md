...
| Uniform low-regularity existence `(N)` | `../ShortTime/ROUTE_C_PLAN.md` + `../ShortTime/LowRegBgC1Pair.lean` + `../ShortTime/LowRegA1FixedPairBg.lean` + `../ShortTime/LowRegBgBootstrap.lean` | **Theorem `(N)`: 0%; dedicated fixed-background direct-smoothing machinery: approximately 92%; conditional metricwise adapted-to-mass-to-packet chain: 100%; Rung-3 full Lie correction `lieBgCorr_unif`: 100%; actual C1 path producer `lowC1Corr_unif`: 100%; corrected Galerkin pairing theorem `galA1FixPair3_le`: 100%; signed C1 forcing split: 100%; rest-only Rung-3 theorem: unstated, 0%.**  The old G3/adjacent-scale and closed-slab all-rung class-first gates are superseded because they force fourth and higher varying-metric jets into the common absorption budget.  The energy-pairing-first Route-(c) chain through the complete C1 correction is focused-check green and uses no varying fourth metric jet; the pairing has the required `eta -> g -> G` order with `G` independent of the later capped solver radius.  **STOP:** the remaining `C0`, self-`C1`, and seed pieces are routine, but no available structural API proves the C2-only signed Rung-3 estimate `eta * E4 + G * (1 + E3)^2` from the `C3` metric class and capped `H2` state radius.  Three routes failed honestly: direct H2 C2 smallness requires a high state jet at the action step; the H3-to-H1 top-path bound is one derivative short and naive duality requires H5; and the edge-refold route lacks the `covDivergence edgeTopPartner` structural bound.  No partial rest theorem or stronger assumption was added.  Rungs 4+ remain metricwise post-construction work.  The existing `bg_packet_of_solve` still contributes the endpoint `sorryAx`; verified conditional consumers do not close its missing class-first producer.  Whole HCG closure remains approximately 3%. |
