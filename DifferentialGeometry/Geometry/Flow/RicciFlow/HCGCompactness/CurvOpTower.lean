          ...
          exact
            (chartBasisVec_contMDiffOn (I := I) a j x hxbase).contMDiffAt
              ((trivializationAt E (TangentSpace I) a).open_baseSet.mem_nhds
                hxbase))
  simpa [Tensor0SSpace.toModel] using hEval
...
theorem curvOpN_eq_sharp
    ...
    curvOpN (I := I) g k x v =
      Tensor0SBundle.cotangentSharp_gen (I := I) g x
        (DifferentialGeometry.Integral.Connection.oneFormAtSlot0S
          (I := I) (curvCovDeriv (I := I) (M := M) g k x)
          (Fin.snoc v 0) (Fin.last (k + 3))) := by
  rw [DifferentialGeometry.Integral.DivergenceTheorem.cotangentSharp_gen_eq_metricSharp]
  unfold curvOpN
  congr 1
  ext w
  rw [Tensor0SBundle.cotangentToDual_apply_gen,
    DifferentialGeometry.Integral.Connection.oneFormAtSlot0S_apply]
...
noncomputable def curvOpNField
  ...
  exact ContMDiffSection.mk
    (fun x : M =>
      Tensor0SBundle.cotangentSharp_gen (I := I) g x
        (curvOpNForm (I := I) g k Y x))
    (DifferentialGeometry.Integral.DivergenceTheorem.cotangentSharp_gen_contMDiff_total
      (I := I) g (fun a j => by
...
theorem curvOpN_smoothAlong
  ...
  have hbase (r : Fin (k + 3) -> Fin (Module.finrank Real E)) :
    ...
    simpa only [Function.comp_apply, curvOpNField_apply, Bsec] using
      (curvOpNField (I := I) g k (fun i => Bsec (r i))).contMDiff.comp hgamma
...
private theorem curvOpNabla_eval_raw
    ...
    curvOpNablaForm (I := I) g k Y x
        (DifferentialGeometry.Integral.Connection.vec2 (I := I) (X x) U) =
  ...
  have hBval :
      curvOpNablaForm (I := I) g k Y x
        ...
        Tensor0SBundle.totalNabla0SFun (𝕜 := Real) (E := E) (H := H)
          (I := I) (M := M) 1 cov
          (curvOpNForm (I := I) g k Y) x
          (DifferentialGeometry.Integral.Connection.vec2 (I := I) (X x) U) := rfl
  have hAval :
      curvCovDeriv (I := I) (M := M) g (k + 1) x
          (Fin.cons (X x)
...
theorem curvOpN_cov_restrict
  ...
  have hleft :
    ...
    simpa only [DifferentialGeometry.Integral.Connection.LeviCivita,
      DifferentialGeometry.Integral.Connection.metricCov] using
...
private theorem curvOpNDeriv_slot
  ...
  have hFrameCurve :
    ...
    intro a
    simpa only [Function.comp_apply] using (hFrameSm a).comp hgamma
  ...
  have hCongrC :
    ...
    by_cases hji : j = i
    · subst j
      simpa only [Function.update_self] using hcExp
  ...
  have hCongrD :
    ...
    by_cases hji : j = i
    · subst j
      simpa only [Function.update_self] using hdExp
    · simpa only [Function.update_of_ne hji] using
        (Filter.EventuallyEq.rfl : Y j =ᶠ[nhds t] Y j)
  calc
    curvOpNDerivAlong (I := I) g k gamma (Function.update Y i A) t =
...
theorem curvOpN_covAlong
  ...
  have hGcurve :
    ...
    intro i
    simpa only [Function.comp_apply] using (G i).contMDiff.comp hgamma
  calc
    curvOpNDerivAlong (I := I) g k gamma Y t =
        curvOpNDerivAlong (I := I) g k gamma
  ...
  calc
    ...
    _ = curvCovDeriv (I := I) (M := M) g (k + 1) x
        (Fin.snoc (Fin.cons (X x) (fun i : Fin (k + 3) => Y i x))
          (slots 0)) := by
      rw [update_snoc_last, Fin.cons_snoc_eq_snoc_cons]
    _ = DifferentialGeometry.Integral.Connection.oneFormAtSlot0S
        (I := I) (curvCovDeriv (I := I) (M := M) g (k + 1) x)
        (Fin.snoc (Fin.cons (X x) (fun i : Fin (k + 3) => Y i x)) 0)
...
theorem curvOpN_inner
  ...
  simpa only [curvOpN, curvLastCov] using
    (DifferentialGeometry.Integral.Connection.inner_metricSharp_right
      (I := I) g x
...
private theorem snoc_vec3
    ...
    Fin.snoc
        (DifferentialGeometry.Integral.Connection.vec3 (I := I) X Y Z) W =
...
private theorem snoc_vec4
    ...
    Fin.snoc
        (DifferentialGeometry.Integral.Connection.vec4 (I := I) D X Y Z) W =
      DifferentialGeometry.Integral.Connection.vec5 (I := I) D X Y Z W := by
  funext i
  fin_cases i
...
theorem curvOpN_zero
    ...
    curvOpN (I := I) g 0 x
      ...
      DifferentialGeometry.Integral.Connection.riemannOp
        ...
        x X Y Z := by
  apply DifferentialGeometry.Integral.Connection.SmoothRiemannianMetric.eq_of_inner_eq g
  intro W
  calc
    g.inner x
        (curvOpN (I := I) g 0 x
...
theorem curvOpN_one
    ...
    curvOpN (I := I) g 1 x
        (DifferentialGeometry.Integral.Connection.vec4 (I := I) D X Y Z) =
