...
import DifferentialGeometry.Geometry.Flow.RicciFlow.HCGCompactness.C4.StepCStageInjectivityH6
...
theorem HasRadiusTailOn.geom_tail
  ...
  obtain ⟨Ninj, hinj⟩ := d.inj_tail inp P Lbase
    (Nat.cast_nonneg q) (a.phi ∘ rho) (a.phi_strict.comp hrho)
      hcomplete hconn a.V a.U a.C0 a.C1 a.aInf a.Jinf a.Jbarinf a.gInf
      hstage R0 R1 hroom hR1q
  rcases hstage with ⟨hdata, hmetric, hjets, hbase⟩
  dsimp only [HasStageBaseTail] at hbase
  obtain ⟨Nbase, hbase⟩ := eventually_atTop.mp hbase
  ...
  have hlInj : Ninj ≤ l - q :=
    (le_max_left _ _).trans
      ((le_max_right Nloc (max Ninj Nbase)).trans hlNt)
  have hkBase : Nbase ≤ k - q :=
    (le_max_right _ _).trans
      ((le_max_right Nloc (max Ninj Nbase)).trans hkNt)
  have hki : Lq.φ (k - q) = psi k := by
    exact (hindex (k - q)).trans
      (congrArg psi (Nat.add_sub_of_le hkq))
  have hli : Lq.φ (l - q) = psi l := by
