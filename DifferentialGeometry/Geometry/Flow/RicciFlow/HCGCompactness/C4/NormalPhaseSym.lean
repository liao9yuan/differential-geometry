...
theorem exists_chartBiflow
    ...
    ∃ Φ : (E × E) → Real → E × E,
      ...
      (∀ z ∈ Metric.closedBall (0 : E × E) q, ∀ t ∈ Ioo (-1) 1,
        HasDerivAt (Φ z)
          (PhaseFlow.phaseField (c.accel g) (Φ z t)) t) ∧
      (∀ z ∈ Metric.closedBall (0 : E × E) q, ∀ t ∈ Icc (-1) 1,
  ...
  have hzeroEq : EqOn (Φ 0) (fun _ : Real ↦ (0 : E × E)) (Ioo (-T) T) :=
    Analysis.ODE.Flow.orbit_unique_smooth
      ...
      ((chartPhase_contDiff (I := I) g c).of_le (by simp))
      (a := -T) (b := T) (t₀ := 0)
  ...
  have happ := chartDiag_approx (I := I) g c b hrMetric hrQuarter V
  ...
  have hslice : ContDiffOn Real ∞ (fun z ↦ Φ z 1)
    ...
    apply Analysis.ODE.Flow.flow_slice_smooth
      ...
      (F := Φ)
    · exact chartPhase_contDiff (I := I) g c
...
/-- The legacy sequence-level bilateral flow is the controlled-chart theorem
specialized to the legacy normal chart. -/
theorem exists_normal_biflow
  ...
  have hqAcc' : 3 * b.C 1 * (2 * (q : Real)) ^ 2 ≤
      (2 / 3 : Real) * (q : Real) := by
    simpa only [b, NormalCoordMetricBoundInput.metricBounds] using hqAcc
  have hflow := exists_chartBiflow (I := I) (X.obj k).metric c b
    hrMetric' hrQuarter' q hq hqPos hqAcc'
  simpa only [c, b, normalAccel_eq_chart, normalPhaseK, chartPhaseK,
    NormalCoordMetricBoundInput.metricBounds] using hflow
