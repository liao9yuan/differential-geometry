...
import DifferentialGeometry.Geometry.Flow.RicciFlow.HCGCompactness.C4.H6BranchMin
...
theorem strict_dist
  ...
  letI : MetricSpace (X.obj k).M :=
    HopfRinow.riemMetricSpace (I := I) (M := (X.obj k).M)
  dsimp only
  intro hρInner hρ hρq hr hxp hpts hcage
  ...
  have hptsCage (i : ι) : dist x (pts i) < R + 6 * r := by
    calc
      dist x (pts i) ≤ dist x p + dist p (pts i) :=
        dist_triangle _ _ _
  ...
  have hstrict_pt (pt : (X.obj k).M) (hpt : dist x pt < R + 6 * r)
    ...
    simpa only [Function.comp_apply, γ] using
      strictConvex_geo (I := I) (X.obj k).metric hSopen hsmooth
