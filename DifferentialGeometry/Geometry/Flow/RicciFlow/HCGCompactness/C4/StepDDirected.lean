...
/-- **Fiberwise speed bound from the `C⁰` tensor error** (the pointwise heart of the
hspeed supplier).  If the pullback field value at `x` is within `ε` of `g` in the
`g`-tensor norm, then the pulled-back quadratic form is bounded:
`P x (v,v) ≤ (1+ε)·g(v,v)`.  Cauchy–Schwarz at a `g`-orthonormal basis, exactly as in
`inner_le_of_c0` but for a single evaluation of a general field. -/
...
/-- **The image-ball control from `(ε,p)` data** (lbl367 in the form the D1b recursion
consumes).  A partial map with `PreApproxIsoDataOn` on the closed `r₂`-eball maps the open
`r`-eball into the closed `√(1+ε)·r`-eball, `r ≤ r₂`.  The manifold `edist`s are assumed
Riemannian (`IsRiemannianManifold`) with the bundle enorms reading off `g` resp. `h`
(`hgnorm`/`hhnorm` — satisfied by `rfl`-level readouts for member metrics built by
`ofRiemannianMetric`). -/
theorem data_image_ball
    ...
    (Φ : M → N) '' Metric.eball O (ENNReal.ofReal r) ⊆
      Metric.closedEBall ((Φ : M → N) O) (ENNReal.ofReal (Real.sqrt (1 + ε) * r)) := by
  refine image_ball_local (I := I) (Φ : M → N) (by linarith) hr O ?_
  intro y γ hγC hγ0 hγ1 hγlen
  -- the path localizes into the closed r₂-eball
  have hrange : ∀ t ∈ Set.Icc (0 : ℝ) 1, γ t ∈ Metric.closedEBall O (ENNReal.ofReal r₂) := by
    intro t ht
  ...
  -- the pushed path
  refine ⟨(Φ : M → N) ∘ γ, ?_, by simp [Function.comp, hγ0], by simp [Function.comp, hγ1], ?_⟩
  · exact (Φ.contMDiffOn_toFun.of_le (by exact_mod_cast le_top)).comp hγC
      (fun t ht => hsub (hrange t ht))
  · -- pointwise speed bound, integrated
    ...
    -- chain rule at the interior point
    ...
    have happ : mfderiv 𝓘(ℝ, ℝ) I ((Φ : M → N) ∘ γ) t 1
      ...
      rfl
    rw [happ]
    -- fiber bound through the pullback field
    ...
    have hquad : hdata.pullback (γ t) (fun _ => w) ≤ (1 + ε) * g.inner (γ t) w w :=
      speed_le_of_c0 (I := I) hdata.pullback g (hdata.c0_small (γ t) hγt) w
    have hgnn : 0 ≤ g.inner (γ t) w w := metricInner_nonneg (I := I) g (γ t) w
    calc ‖mfderiv I I (Φ : M → N) (γ t) w‖ₑ
      ...
      _ ≤ ENNReal.ofReal (Real.sqrt ((1 + ε) * g.inner (γ t) w w)) := by
          ...
          rw [hPval]
          exact hquad
      _ = ENNReal.ofReal (Real.sqrt (1 + ε)) * ENNReal.ofReal (Real.sqrt (g.inner (γ t) w w)) := by
