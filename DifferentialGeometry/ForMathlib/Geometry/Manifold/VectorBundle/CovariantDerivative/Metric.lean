...
private lemma fromTangentSpace_mfderiv_add_apply
    {f g : M → ℝ} {x : M} (hf : MDiffAt f x) (hg : MDiffAt g x)
    (X₀ : TangentSpace I x) :
    fromTangentSpace (f x + g x) (mfderiv% (f + g) x X₀) =
      fromTangentSpace (f x) (mfderiv% f x X₀) +
        fromTangentSpace (g x) (mfderiv% g x X₀) := by
  have h := congr($(extDerivFun_add (I := I) (g := f) (g' := g) (x := x) hf hg) X₀)
  simpa [extDerivFun] using h

variable [VectorBundle ℝ F V] [IsContMDiffRiemannianBundle I 1 F V] {x : M}

private lemma fromTangentSpace_mfderiv_product_add_left_apply
    {σ σ' τ : Π x : M, V x} (hσ : MDiffAt (T% σ) x)
    (hσ' : MDiffAt (T% σ') x) (hτ : MDiffAt (T% τ) x)
    (X₀ : TangentSpace I x) :
    fromTangentSpace ((⟪σ, τ⟫ + ⟪σ', τ⟫) x)
        (mfderiv% (⟪σ, τ⟫ + ⟪σ', τ⟫) x X₀) =
      fromTangentSpace (⟪σ, τ⟫ x) (mfderiv% ⟪σ, τ⟫ x X₀) +
        fromTangentSpace (⟪σ', τ⟫ x) (mfderiv% ⟪σ', τ⟫ x X₀) :=
  fromTangentSpace_mfderiv_add_apply (I := I) (hσ.inner_bundle hτ) (hσ'.inner_bundle hτ) X₀

private lemma fromTangentSpace_mfderiv_product_add_right_apply
    {σ τ τ' : Π x : M, V x} (hσ : MDiffAt (T% σ) x)
...
theorem compatibilityTensorAux_tensorial₁ (τ : Π x, V x) (hτ : MDiffAt (T% τ) x) :
  ...
  add hσ hσ' := by
    ...
    rw [fromTangentSpace_mfderiv_product_add_left_apply (I := I) hσ hσ' hτ]
    simp [compatibilityTensorAux_apply, product, cov.isCovariantDerivativeOn.add hσ hσ',
      inner_add_left]
    abel
...
theorem compatibilityTensorAux_tensorial₂ (σ : Π x, V x) (hσ : MDiffAt (T% σ) x) :
  ...
  add hτ hτ' := by
    ...
    rw [fromTangentSpace_mfderiv_product_add_right_apply (I := I) hσ hτ hτ']
    simp [compatibilityTensorAux_apply, product, cov.isCovariantDerivativeOn.add hτ hτ',
      inner_add_right]
    abel
...
def IsCompatible [FiniteDimensional ℝ F] : Prop := ∀ x, compatibilityTensor cov x = 0
...
lemma isCompatible_iff [FiniteDimensional ℝ F] :
  ...
  · have H := congrArg
      (fun T : V x →L[ℝ] V x →L[ℝ] TangentSpace I x →L[ℝ] ℝ =>
        T (σ x) (τ x) (X x)) (hcov x)
    simp [compatibilityTensor_apply _ _ hσ hτ] at H
    linear_combination H
  intro x
  ext σ₀ τ₀ X₀
  rw [compatibilityTensor_apply_eq_extend]
  have h' := h (x := x) (X := extend E X₀) (σ := extend F σ₀) (τ := extend F τ₀)
    (mdifferentiableAt_extend I E X₀) (mdifferentiableAt_extend I F σ₀)
