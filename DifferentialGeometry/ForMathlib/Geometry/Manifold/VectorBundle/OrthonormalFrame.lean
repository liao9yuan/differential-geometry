...
-/
import DifferentialGeometry.ForMathlib.Geometry.Manifold.VectorBundle.GramSchmidtOrtho
...
--   -- Need a version of `b.repr_apply_apply` for orthogonal bases.
...
lemma contMDiffWithinAt_coeff (ht : CMDiffAt[u] n (T% t) x) (hx : x ∈ u) (i : ι) :
  ...
  ((hs.toIsLocalFrameOn.contMDiffOn i x hx).inner_bundle ht).congr_of_mem
    (fun _ hy ↦ by simpa using hs.coeff_eq_inner' t hy i) hx
...
lemma contMDiffAt_coeff (hu : u ∈ 𝓝 x) (ht : CMDiffAt n (T% t) x) (i : ι) :
  ...
  (((hs.toIsLocalFrameOn.contMDiffOn i).contMDiffAt hu).inner_bundle ht).congr_of_eventuallyEq <|
    Filter.eventually_of_mem hu fun _ hx ↦ by simpa using hs.coeff_eq_inner' t hx i
...
lemma contMDiffAt_iff_coeff' [FiniteDimensional ℝ F] (hu : u ∈ 𝓝 x) :
  ...
  have (i : ι) := Filter.eventually_of_mem hu fun x hx ↦ by
    simpa using hs.coeff_eq_inner' t hx i
...
lemma contMDiffOn_iff_coeff' [FiniteDimensional ℝ F] :
  ...
  ⟨fun h i x hx ↦
      (contMDiffOn_coeff hs h i x hx).congr_of_mem
        (fun y hy ↦ by simpa using (hs.coeff_eq_inner' t hy i).symm) hx,
    fun h ↦ (contMDiffOn_iff_coeff (hs := hs) (t := t)).2 fun i x hx ↦
      (h i x hx).congr_of_mem (fun y hy ↦ by simpa using hs.coeff_eq_inner' t hy i) hx⟩
...
lemma orthonormalFrame_apply_of_notMem {i : ι} (hx : x ∉ e.baseSet) :
  ...
  simp only [RCLike.ofReal_real_eq_id, id_eq, smul_eq_zero, inv_eq_zero, norm_eq_zero, or_self]
  convert InnerProductSpace.gramSchmidt_zero ℝ i
  apply Bundle.Trivialization.localFrame_apply_of_notMem e b hx
