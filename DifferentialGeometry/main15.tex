...
\newcommand{\owedge}{\mathbin{\text{\raisebox{0.15ex}{$\scriptstyle\wedge$}}}}
\newcommand{\M}{M}
\newcommand{\leanref}[1]{\nolinkurl{#1}}
\newcommand{\leanfile}[1]{\nolinkurl{#1}}
\newcommand{\rffile}[1]{\leanfile{#1}}
\newcommand{\rfdir}[1]{\leanfile{#1/}}
\newcommand{\leanchanged}[1]{\textcolor{blue}{#1}}
\makeatletter
...
\begin{abstract}
This working paper describes a Lean formalization project for Hamilton's 1982
classification theorem for closed three-manifolds with positive Ricci
curvature.  The project focuses not only on the final theorem statement, but on
the geometric-analysis infrastructure needed for Hamilton's proof: Riemannian
tensor calculus, Ricci-flow evolution equations, maximum-principle consumers,
three-dimensional curvature algebra, Ricci pinching, and the blow-up argument.
The current Lean development is substantial but not yet a completely closed
formalization of Hamilton's theorem.  Its endpoint is theorem-shaped and
machine-checkable relative to explicitly named global analytic, compactness,
transfer, and topological inputs.
\end{abstract}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item the final global/topological handoff
    \leanref{limit_to_orig},
    \leanref{ham3_space_box}, and
    \leanref{spaceForm_const_metric}.
\end{itemize}
...
The first detailed formalization example is the Levi-Civita connection.  It is
mathematically familiar, but it exposes one of the central issues in a Lean
Ricci-flow development: textbook differential geometry often suppresses the
point, the tangent direction, the domain of definition, and the regularity data,
whereas Lean's typed APIs make these choices explicit.  The same infrastructure
is then reused in the later Ricci-flow evolution equations, where the project
must move reliably between invariant tensor notation, coordinate-frame
components, and local analytic hypotheses.

The construction builds on the differential-geometry infrastructure of
...
A representative checked example is not a low-valence special case, but the
general coordinate theorem for mixed tensor covariant derivatives:
\[
  \texttt{nablaRS\_coordFrame\_slots\_of\_smooth}.
\]
This theorem appears in
\[
  \texttt{...Coordinates/NablaComponents/TensorRS/Formula.lean}.
\]
It states the coordinate-frame component formula for the covariant derivative of
an arbitrary mixed $(r,s)$-tensor field.

Let $T$ be a mixed tensor field of type $(r,s)$, let
\[
  I : \operatorname{Fin}(r) \to \{1,\ldots,n\},
  \qquad
  J : \operatorname{Fin}(s) \to \{1,\ldots,n\}
\]
be choices of upper and lower coordinate indices, and write
\[
  T^I{}_J
\]
for the corresponding coordinate component of $T$.  The theorem proves the
formula
\[
\begin{aligned}
  (\nabla_X T)^I{}_J
  ...
  &\quad
  +
  \sum_{a\in \operatorname{Fin}(r)}
  \sum_k
  \Gamma^{I_a}(X,k)\,
  T^{I[a\mapsto k]}{}_J                                      \\
  &\quad
  -
  \sum_{b\in \operatorname{Fin}(s)}
  \sum_k
  \Gamma^k(X,J_b)\,
  T^I{}_{J[b\mapsto k]} .
\end{aligned}
\]
Here $I[a\mapsto k]$ denotes the index tuple obtained from $I$ by replacing its
$a$-th upper index by $k$, and similarly $J[b\mapsto k]$ denotes replacement in
the lower-index tuple.  The sign convention is exactly the classical one:
contravariant slots contribute positive Christoffel terms, while covariant slots
contribute negative Christoffel terms.

The proof of \texttt{nablaRS\_coordFrame\_slots\_of\_smooth} is organized as a
sequence of exchange lemmas between three representations of the same object.
The theorem itself is a smooth-input wrapper around the more explicit theorem
\texttt{nablaRS\_coordFrame\_slots}, whose additional hypothesis asserts that
the fixed-chart model derivative agrees with the scalar directional derivative
of the corresponding coordinate component.

The core proof first unfolds \texttt{nablaRSFun} into its chart-level
definition.  This replaces the invariant-looking tensor field
\[
  \nabla_XT
\]
by the model-space covariant derivative of the fixed-chart representative of
$T$, with the connection represented by
\[
  \texttt{connectionEndomorphismInChart}.
\]
It then rewrites the desired coordinate component using the bridge theorem
\[
  \texttt{tensorRSModelAt\_coordComponentRSAt},
\]
which identifies the fixed-chart model component at the base point with the
coordinate-frame component.

After this normalization, the proof invokes the model-space formula
\[
  \texttt{covariantDeriv\_tensorRSModelWithin\_apply\_basis\_slots}.
\]
This theorem supplies the algebraic mixed-tensor covariant-derivative formula:
one derivative term, one positive Christoffel correction for each upper slot,
and one negative Christoffel correction for each lower slot.  The remaining work
is to translate the three types of terms back into coordinate language.

First, the model connection coefficients are rewritten as Christoffel
coefficients in the coordinate frame using
\[
  \texttt{connCoeff\_eq\_christoffelAlong\_coord}.
\]
Second, the model derivative term is rewritten as the coordinate directional
derivative by the bridge
\[
  \texttt{modelDeriv\_eq\_coordDerivRSAt}.
\]
This is the only genuinely analytic bridge in the proof: it identifies the
Fr\'echet derivative of the fixed-chart model representative with the manifold
directional derivative of the scalar coordinate component.  Finally, the tensor
evaluations inside the two correction sums are rewritten back to coordinate
components by another use of
\[
  \texttt{tensorRSModelAt\_coordComponentRSAt}.
\]
The proof then closes by congruence of finite sums and elementary simplification
of \texttt{Function.update} on the upper and lower index tuples.
...
\begin{remark}[Lean status of the theorem statement]
\emph{File paths in this remark and in the final status proof refer to the live
\textnormal{\texttt{DifferentialGeometry/}} repository tree.  When a Ricci-flow
file is abbreviated, it is relative to
\textnormal{\texttt{DifferentialGeometry/Geometry/Flow/RicciFlow/}}.}

The Lean-facing theorem-shaped endpoint is
\leanref{ham3_main} in
\rffile{DimensionThree/HamiltonPositiveRicci.lean}, in the namespace
\leanref{DifferentialGeometry.PDE.RicciFlow.HamiltonPositiveRicci}.  Its
hypotheses are expressed through the
predicates \leanref{Closed3Manifold} and \leanref{AdmitsPosRicci}, and its
conclusion is expressed through \leanref{AdmitsConstPosSec} together with
\leanref{SphericalSpaceForm}.  The metric part of the conclusion is routed
through \leanref{ham3_const_metric}.  The spherical-space-form conclusion still
passes through the global and topological interfaces \leanref{ham3_space_box}
and \leanref{spaceForm_const_metric}.

Thus the endpoint has the shape of Hamilton's theorem and is useful as a
machine-checked assembly target, but it is not yet a closed, assumption-free
formalization of Theorem~\ref{thm:main-hamilton-3d}.  In the live endpoint the
direct proof frontiers are the short-time regularity bridge
\leanref{ham3_short_isSolution}, the normalized maximal-flow package
\leanref{ham3_flow_exists_normalized}, Perelman noncollapsing
\leanref{ham3_noncollapse}, Cheeger--Gromov--Hamilton compactness
\leanref{ham3_cgh_limit}, compact-limit transfer \leanref{limit_to_orig}, and
the two space-form handoff directions \leanref{ham3_space_box} and
\leanref{spaceForm_const_metric}.  The final assembly theorems
\leanref{ham3_const_metric} and \leanref{ham3_main} are checked consumers over
those named frontiers.

The constant-sectional-curvature predicate uses the lowered curvature
convention
\[
  \Rm_{04}(X,Y,Z,W)=\langle R(X,Y)Z,W\rangle,
\]
with sectional numerator \(\Rm_{04}(X,Y,Y,X)\).  The convention checks live in
\rffile{Curvature/Convention.lean} and
\rffile{Realized/CurvatureTensor.lean}.
\end{remark}
...
\section{Foundational geometric assumptions}
...
\section{Ricci flow and short-time existence}
...
\section{Maximal Ricci flow interval}
...
\paragraph{Lean scope note.}
The formulas in this section are displayed in standard tensor notation.  In the
current Lean development, the corresponding producers are often local-frame or
coordinate-frame component statements rather than single invariant tensor
identities.  They are attached to the derived smooth-solution view
\leanref{IsSmoothSolutionOn}.  This is a formalization-scope statement, not an
extra analytic assumption: \leanref{IsSmoothSolutionOn} extends
\leanref{IsSolutionOn}, and the upgrade theorem \leanref{smoothOfSol} supplies
this view from the metric Ricci-flow solution predicate under the standing
boundaryless coordinate hypothesis.

\subsection{Evolution of the connection}
...
The \texttt{RicciFlower} proof is the fixed-frame component theorem
\leanref{RicciFlow.evol_inverse_metric_inFrame} in
\rffile{RicciFlow/Evolution/Metric/Evolution.lean}.  The proof does not treat
\(g^{ij}\) as an independent formal symbol.  Instead, it works with a local
inverse-metric component family, called \(gInv(t,x,i,j)\) in Lean and
mathematically read as \(g^{ij}(t,x)\).  The hypotheses
\leanref{MetricFrameTimeRegularityInFrameOnLocal} provide the two-sided inverse
identities and the required time regularity.

Lean differentiates the local inverse identity
\[
  \sum_a g^{ia}g_{aj}=\delta^i_j .
\]
The Ricci-flow equation supplies
\(\partial_tg_{aj}=-2\Ric_{aj}\).  Solving the differentiated inverse identity
then gives, in the fixed local frame,
\[
  \frac d{dt}g^{ij}=2\Ric^{ij}.
\]
The formal endpoint is a \texttt{HasDerivWithinAt} statement on the time
carrier \(D.carrier\), with right-hand side
\leanref{raisedRicciCompInFrame}; the displayed formula is its standard
coordinate-frame reading.
\end{proof}

\begin{lemma}[Evolution of the Christoffel symbols]
...
For the complete proof, see \S \ref{subsec:1stVarFormulaChristoffel}.

\begin{proof}[Lean-facing calculation]
The checked route is in \rffile{RicciFlow/Evolution/Connection/Producers.lean},
and its endpoint is \leanref{evol_christoffel_inFrame}.  The formal proof uses
the connection-variation layer rather than an informal normal-coordinate
shortcut.  This is important for the Lean development: tensors and their
coordinate components are moved between APIs explicitly, rather than being
identified by unrecorded tensoriality arguments.

First, the general metric-variation formula is proved in lowered pairing form:
\[
  \left\langle
  ...
  =
  \frac12\left(
    \nabla_i\frac{\partial g_{j\ell}}{\partial t}
    +\nabla_j\frac{\partial g_{i\ell}}{\partial t}
    -\nabla_\ell\frac{\partial g_{ij}}{\partial t}
  \right).
\]
This lowered formula is represented by the pairing API together with the
Christoffel-evolution API.  Substituting the Ricci-flow equation
\[
  \frac{\partial g_{ab}}{\partial t}=-2\Ric_{ab}
\]
gives the lowered expression
\[
  -\nabla_i\Ric_{j\ell}
  -\nabla_j\Ric_{i\ell}
  +\nabla_\ell\Ric_{ij}.
\]
Lean then raises the \(\ell\)-index with the local inverse metric and rewrites
the result to the displayed coordinate right-hand side.  The component theorem
therefore verifies precisely
\[
  \partial_t\Gamma^k_{ij}
  =
  -g^{k\ell}
  ...
  \right).
\]
This agrees with what one obtains by differentiating the classical coordinate
formula
\[
  \Gamma^k_{ij}
  =
  ...
  \right),
\]
but the Lean proof factors the calculation through the lower-index variation
and the explicit raising step.
\end{proof}

\subsection{Evolution of Ricci and scalar curvature}
...
\begin{proof}[Lean-facing calculation]
The native proof follows the first-variation route, carried out componentwise
in a centered coordinate frame.  The final producer is
\leanref{coordRicciEvol} in
\rffile{RicciFlow/Evolution/Ricci/CoordinateIdentities.lean}; it proves the
within-time derivative statement for
\[
  \Ric_{ij}(t,x_0)
\]
in the coordinate frame centered at \(x_0\).  The proof is deliberately split
into three layers, so that the formal development exposes the intermediate
coordinate identities rather than compressing the calculation into a single
textbook rewrite.

First, Lean differentiates the Christoffel trace formula for the Ricci tensor.
If
\[
  A^k{}_{ij}:=\frac{\partial\Gamma^k_{ij}}{\partial t},
\]
then the first-variation formula is
\[
  \frac{\partial\Ric_{ij}}{\partial t}
  =
  \nabla_k A^k{}_{ij}-\nabla_i A^k{}_{kj}.
\]
The Ricci-variation predicates live in
\rffile{RicciFlow/Evolution/Ricci/GammaAlgebra.lean}, while the coordinate
producer differentiating the Christoffel curvature trace lives in
\rffile{RicciFlow/Evolution/Ricci/GammaCoord.lean}.

Second, the Christoffel-symbol evolution is substituted into \(A\).  The
covariant derivative of \(A\) is rewritten using \(\nabla g^{-1}=0\), so the
derivative of the raised Christoffel variation becomes a fixed-frame second
covariant derivative of Ricci.  This is the role of
\leanref{nablaGammaDtFromNabla2RicInFrame} and the core theorem
\leanref{ricciVarCore}.  At this stage the expression is still an expanded sum
of second Ricci derivatives, scalar-Hessian trace terms, and curvature
corrections.

Third, the expanded expression is reduced by the contracted commutator package.
The package \leanref{RicciContractedCommutatorsInFrame} is produced from the
differentiated contracted Bianchi identity together with the native
\((0,2)\)-tensor Ricci identity; the relevant files are
\rffile{RicciFlow/Evolution/Ricci/Bianchi.lean} and
\rffile{RicciFlow/Evolution/Ricci/Commutator.lean}.  This reduction cancels the
gauge and Hessian-trace terms, reorganizes the remaining second derivatives as
\[
  \Delta\Ric_{ij},
\]
and identifies the reaction terms as
\[
  2R_{ikj\ell}\Ric^{k\ell}-2\Ric_i{}^k\Ric_{kj}.
\]
Finally, \leanref{coordRicciEvol} assembles the coordinate inputs: the
Christoffel evolution, regularity for \(g^{-1}\), \(\nabla\Ric\), and
\(\nabla^2\Ric\), the identity \(\nabla g^{-1}=0\), curvature realization
bridges for \(Rm_{13}\) and \(Rm_{04}\), and the contracted commutator package.
The displayed equation is the mathematical projection of that checked component
statement.
\end{proof}
...
\begin{proof}[Lean-facing calculation]
The Lichnerowicz rewrite is a separate checked layer in
\rffile{Ricci/Lichnerowicz.lean} under \rfdir{RicciFlow/Evolution}.  Lean
introduces the component right-hand side
\leanref{lichnerowiczRHSInFrame} and a proposition expressing
\(\partial_t\Ric=\Delta_L\Ric\).  The assembly theorem in that file turns the
Ricci evolution equation into this Lichnerowicz form.

The algebraic specialization is that the two Ricci-action terms in
\(\Delta_Lh\) become the same quadratic term when \(h=\Ric\).  Using Ricci
symmetry and symmetry of the local inverse-metric components, Lean proves
\[
-\Ric_i{}^kh_{kj}-\Ric_j{}^kh_{ki}
=
-2\Ric_i{}^k\Ric_{kj}
\qquad (h=\Ric).
\]
Composing this specialization with the Ricci evolution producer gives
\(\partial_t\Ric=\Delta_L\Ric\) in the coordinate-frame component API.
\end{proof}
...
\begin{proof}[Lean-facing calculation]
The intrinsic producer is \leanref{scalarEvolOfSmooth} in
\rffile{Scalar/Basic.lean} under \rfdir{RicciFlow/Evolution}.  The calculation
is split into a pre-Bianchi trace identity and a contracted-Bianchi reduction.
The theorem \leanref{ScalarPreBianchiEvolutionEquationOn} records the
derivative of
\[
  R=g^{ij}\Ric_{ij},
\]
and \leanref{ScalarContractedBianchiReductionOn} identifies the contracted
Ricci-Hessian term with the scalar Laplacian.  The assembly theorem
\leanref{scalarEvolutionEquationOn_of_contractedBianchi} then produces
\leanref{ScalarEvolutionEquationOn}.

At the component level,
\[
\partial_tR=(\partial_tg^{ij})\Ric_{ij}+g^{ij}\partial_t\Ric_{ij}.
\]
The inverse-metric evolution contributes \(2\norm{\Ric}^2\).  Tracing the Ricci
evolution contributes the scalar Laplacian after the contracted-Bianchi
reduction; the remaining zero-order trace terms cancel in the standard way.
Thus the only surviving reaction term is \(2\norm{\Ric}^2\).  For a smooth
Ricci-flow solution package \leanref{IsSmoothSolutionOn},
\leanref{scalarEvolOfSmooth} packages the result as
\[
  \partial_tR=\Delta R+2\norm{\Ric}^2.
\]
\end{proof}
...
\begin{proof}[Lean-facing calculation]
The native endpoint is \leanref{ricciHeatSmooth} in \rffile{RicciNorm.lean}
under the Ricci-flow evolution folder.  It first builds
\leanref{ricciHeatDataSmooth} from the strengthened
\leanref{IsSmoothSolutionOn} fields:
\begin{itemize}[leftmargin=2em]
  \item inverse-metric evolution: \leanref{hS.invEvol};
  \item Ricci evolution: \leanref{hS.ricciEvol};
  \item inverse-metric and Ricci symmetry: \leanref{hS.invSymm},
    \leanref{hS.ricciSymm};
  \item Ricci Laplacian expansion: \leanref{hS.ricciLap}.
\end{itemize}
The algebraic consumer \leanref{ricciHeat_of_data} then combines the time
variation of the norm with the Laplacian expansion.

Mathematically, the coordinate norm is represented by the full contraction
\[
  \norm{\Ric}^2=g^{ia}g^{jb}\Ric_{ij}\Ric_{ab}.
\]
Lean differentiates this expression using the inverse-metric evolution and the
Ricci evolution.  The cubic terms produced by differentiating the two inverse
metrics cancel the quadratic-Ricci part of the Ricci evolution.  The Laplacian
expansion contributes the standard structure
\[
  \Delta\norm{\Ric}^2
  =2\ip{\Delta\Ric}{\Ric}+2\norm{\nabla\Ric}^2,
\]
which is represented by \leanref{ricciNormLap} together with the gradient term
\leanref{ricciGradSq}.  The remaining reaction is
\[
  4R_{ikj\ell}\Ric^{ij}\Ric^{k\ell}.
\]
Thus the checked proposition \leanref{RicciNormHeatEquationOn} is obtained for
the canonical functions \leanref{ricciNorm}, \leanref{ricciNormLap},
...
\section{Three-dimensional curvature algebra}
...
R_{ijkl}
=
g_{ik}\Ric_{j\ell}-g_{i\ell}\Ric_{jk}
-g_{jk}\Ric_{i\ell}+g_{j\ell}\Ric_{ik}
-\frac R2\left(g_{ik}g_{j\ell}-g_{i\ell}g_{jk}\right).
\]
If $\Ric$ is diagonal with eigenvalues $\lambda_1,\lambda_2,\lambda_3$, then
\[
K_{ij}=R_{ijij}=\frac{\lambda_i+\lambda_j-\lambda_k}{2},
\]
where $\{i,j,k\}=\{1,2,3\}$.
\end{lemma}
...
\section{Preservation of Ricci positivity and Ricci pinching}
...
\section{The improved Ricci pinching quantity}
...
\subsection{Definition and evolution of the pinching quantity}
...
\subsection{The algebraic estimate for \texorpdfstring{$Q$}{Q}}
...
\section{Auxiliary estimates and black-box interfaces for the blow-up argument}
...
=
\frac R6
\left(h_{ik}h_{j\ell}-h_{i\ell}h_{jk}\right),
\]
...
surjectivity.  The current endpoint records this global transfer through
\leanref{limit_to_orig}.  This belongs to the remaining topological and
CGH-transfer boundary, not to the local Ricci-flow calculation layers.
...
handoff.  In the current endpoint this handoff is represented by
\leanref{limit_to_orig}, \leanref{ham3_space_box}, and
\leanref{spaceForm_const_metric}.  Consequently, \leanref{ham3_const_metric} is
a theorem-shaped consumer over the remaining global analytic, compactness, transfer, and topological inputs, while
...
\begin{itemize}[leftmargin=2em]
  ...
  \item The compact-limit transfer back to the original manifold,
  \leanref{limit_to_orig}.
  \item The spherical-space-form and constant-curvature handoff interfaces
  \leanref{ham3_space_box} and \leanref{spaceForm_const_metric}.
\end{itemize}
...
\rffile{DimensionThree/HamiltonPositiveRicci.lean} &
Final checked assembly theorem over the seven direct frontiers listed in the status remark. \\
...
\rffile{DimensionThree/HamiltonPositiveRicci.lean} &
Checked limit-flow assembly; the metric conclusion on the original manifold still passes through \leanref{limit_to_orig}. \\

Compact-limit/topological handoff &
...
Transfer/topological handoff from the constant-curvature limit back to the original manifold. \\
