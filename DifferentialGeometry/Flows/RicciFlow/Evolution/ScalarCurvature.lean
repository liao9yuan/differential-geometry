...
def partial_ricci_form {Time R V : Type}
  ...
  TensorAlgebra.fromBilinear
    { toFun := fun X =>
        { toFun := fun Y => TimeDerivative.partial_t (fun s => eval02 (ricciForm (conn_fam s)) X Y) t
          map_add' := fun Y1 Y2 => by
            have h : (fun s => eval02 (ricciForm (conn_fam s)) X (Y1 + Y2)) = fun s => eval02 (ricciForm (conn_fam s)) X Y1 + eval02 (ricciForm (conn_fam s)) X Y2 := by
              funext s; exact eval02_add_right (ricciForm (conn_fam s)) X Y1 Y2
            rw [h, TimeDerivativeRules.t_add V]
          map_smul' := fun c Y => by
            have h : (fun s => eval02 (ricciForm (conn_fam s)) X (c • Y)) = fun s => c * eval02 (ricciForm (conn_fam s)) X Y := by
              funext s; exact eval02_smul_right (ricciForm (conn_fam s)) c X Y
      ...
      map_add' := fun X1 X2 => LinearMap.ext fun Y => by
        ...
        have h : (fun s => eval02 (ricciForm (conn_fam s)) (X1 + X2) Y) = fun s => eval02 (ricciForm (conn_fam s)) X1 Y + eval02 (ricciForm (conn_fam s)) X2 Y := by
          funext s; exact eval02_add_left (ricciForm (conn_fam s)) X1 X2 Y
        rw [h, TimeDerivativeRules.t_add V]
      map_smul' := fun c X => LinearMap.ext fun Y => by
        simp only [LinearMap.coe_mk, AddHom.coe_mk, LinearMap.smul_apply, RingHom.id_apply]
        have h : (fun s => eval02 (ricciForm (conn_fam s)) (c • X) Y) = fun s => c * eval02 (ricciForm (conn_fam s)) X Y := by
          funext s; exact eval02_smul_left (ricciForm (conn_fam s)) c X Y
        rw [h, TimeDerivativeRules.t_smul V c]
        rfl }
