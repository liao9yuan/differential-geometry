...
lemma ricci_raise_variation {Time : Type}
  ...
  - (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t).val ((g_fam t).raise (ricciForm (conn_fam t)) X) Y
  ...
  calc (g_fam t).g (TimeDerivative.partial_t (fun s => (g_fam s).raise (ricciForm (conn_fam s)) X) t) Y
    _ = (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t).val ((g_fam t).raise (ricciForm (conn_fam t)) X) Y + (g_fam t).g (TimeDerivative.partial_t (fun s => (g_fam s).raise (ricciForm (conn_fam s)) X) t) Y - (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t).val ((g_fam t).raise (ricciForm (conn_fam t)) X) Y := by abel
    _ = (partial_ricci_form conn_fam t).val X Y - (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t).val ((g_fam t).raise (ricciForm (conn_fam t)) X) Y := by rw [t_both]
    _ = - (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t).val ((g_fam t).raise (ricciForm (conn_fam t)) X) Y + (partial_ricci_form conn_fam t).val X Y := by abel
...
lemma scalar_curvature_evolution {Time : Type}
  ...
  (h_trace_R : ∀ s, ScalarCurvature (conn_fam s) ((g_fam s).toNonDegenerateMetric.toMetricTensor) =
  ...
  have rewrite_R : (fun s => ScalarCurvature (conn_fam s) ((g_fam s).toNonDegenerateMetric.toMetricTensor)) =
  ...
  have partial_raise_eq : ∀ X, TimeDerivative.partial_t (fun s => (g_fam s).raise (ricciForm (conn_fam s)) X) t =
                          - (g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X)
    ...
    rw [h1]
    have h2 : (g_fam t).g (- (g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X) + (g_fam t).raise (partial_ricci_form conn_fam t) X) Z =
              (g_fam t).g (- (g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X)) Z +
              (g_fam t).g ((g_fam t).raise (partial_ricci_form conn_fam t) X) Z := by
      exact (g_fam t).bilinear_add_left _ _ _
    rw [h2]
    have h3 : (g_fam t).g (- (g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X)) Z =
              - (g_fam t).g ((g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X)) Z := metric_neg_left _ _ _
    rw [h3]
    have h5 : (g_fam t).g ((g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X)) Z =
              (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t).val ((g_fam t).raise (ricciForm (conn_fam t)) X) Z := (g_fam t).g_raise _ _ _
  ...
  have trace_eq : (TraceOperator.trace (fun X => TimeDerivative.partial_t (fun s => (g_fam s).raise (ricciForm (conn_fam s)) X) t) : R) =
                  TraceOperator.trace (fun X => - (g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X) + (g_fam t).raise (partial_ricci_form conn_fam t) X) := by
    congr 1
    funext X
    exact partial_raise_eq X
  rw [trace_eq]
  have trace_add : (TraceOperator.trace (fun X => - (g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X) + (g_fam t).raise (partial_ricci_form conn_fam t) X) : R) =
                   TraceOperator.trace (fun X => - (g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X)) +
                   TraceOperator.trace (fun X => (g_fam t).raise (partial_ricci_form conn_fam t) X) := by
    have split_eq : (fun X => - (g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X) + (g_fam t).raise (partial_ricci_form conn_fam t) X) =
                    (fun X => - (g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X)) +
                    (fun X => (g_fam t).raise (partial_ricci_form conn_fam t) X) := rfl
    rw [split_eq]
    exact TraceLinearityRules.trace_add
  rw [trace_add]
  have trace_neg1 : (TraceOperator.trace (fun X => - (g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X)) : R) =
                    -(1:R) * TraceOperator.trace (fun X => (g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X)) := by
    have smul_eq : (fun X => - (g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X)) =
                   (fun X => -(1:R) • (g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X)) := by
      funext X
  ...
  have inner_prod_sub : (TraceOperator.trace (fun X => (g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X)) : R) =
                        -(2:R) * TraceOperator.trace (fun X => (g_fam t).raise (ricciForm (conn_fam t)) ((g_fam t).raise (ricciForm (conn_fam t)) X)) := by
    have eq_func : (fun X => (g_fam t).raise (metric_var_form (fun s => (g_fam s).toNonDegenerateMetric.toMetricTensor) t) ((g_fam t).raise (ricciForm (conn_fam t)) X)) =
