...
/-! ## The DeTurck vector field in a fixed chart -/
...
/-! ## Bilinear expansion of `connDiff` against finite sums

`connDiff g g' x` is a continuous bilinear map valued in `TangentSpace I x`.  We
record the standard expansion of such a map over two finite sums of scaled
vectors; this is the vector-valued analogue of the bilinear-form expansion used
in the chart-bridge files. -/
...
private lemma clm_bilinear_expand_two_sums_vector
  ...
  classical
  -- Outer linearity: `B (∑ p, a_p • u_p) = ∑ p, a_p • B (u_p)`.
  ...
  refine Finset.sum_congr rfl ?_
  intro p _
  -- `(a p • B (u p)) (∑ q, b_q • w_q) = ∑ q, (a p * b q) • B (u p) (w q)`.
...
/-! ## Change-of-basis matrix between the chart-`α` frame and the model basis

For `x` in the chart-`α` base set, the chart-`α` coordinate frame
`chartBasisVecFiber α · x` and the fixed model basis `chartModelBasis E` are two
bases of `TangentSpace I x`.  The change-of-basis matrix records the coordinates
...
private lemma deTurckCobMatrix_isUnit (α : M) {x : M}
  ...
  -- The transpose of a unit matrix is a unit; the un-transposed matrix is the
  -- transition matrix `(chartModelBasis E).toMatrix (chartBasisFamily α x)`,
  -- which is a unit since `chartBasisFamily α x` is a basis.
...
/-! ## The Gram-matrix transformation law

The chart-`α` Gram matrix at `x` is the conjugate, by the change-of-basis matrix,
of the *model* Gram matrix — the Gram matrix of `g.inner x` in the fixed model
basis.  Crucially the model Gram matrix is `chartGramMatrix g x x` (by
`chartBasisVecFiber_self`), the same for every basepoint `α`. -/
...
private lemma chartGramMatrix_eq_cob_conj (g : SmoothRiemannianMetric I M)
  ...
  -- Expand each chart-`α` basis vector in the model basis, then expand the
  -- inner product bilinearly over the two finite sums.
  have hbilinear :
  ...
  rw [hbilinear]
  -- LHS: `∑_{k,l} P_{ik} P_{jl} G^{model}_{kl}`.
  -- RHS: `(P · Gx · Pᵀ)_{ij}`.
  rw [Matrix.mul_apply]
  rw [Finset.sum_comm]
  refine Finset.sum_congr rfl ?_
...
/-! ## The model-basis metric trace

The reference, manifestly chart-independent expression for the DeTurck vector
field: the metric trace of `connDiff g g'` taken in the fixed model basis. -/
...
/-! ## The matrix identity at the heart of chart-independence

For an invertible change-of-basis matrix `P` and any model Gram matrix `Gx`, the
chart-`α` inverse Gram matrix `(P · Gx · Pᵀ)⁻¹` satisfies
`Pᵀ · (P · Gx · Pᵀ)⁻¹ · P = Gx⁻¹`. -/
...
/-! ## The chart-`α` representative equals the model-basis trace

This is the key lemma: for every `x` in the chart-`α` base set, the chart-`α`
representative `deTurckChartLocal g g' α x` equals the chart-independent
expression `deTurckModelTrace g g' x`. -/
...
private lemma deTurckChartLocal_eq_modelTrace (g g' : SmoothRiemannianMetric I M)
  ...
  -- Invertibility of the change-of-basis matrix at chart-`α` base-set points.
  have hP_unit : IsUnit P := deTurckCobMatrix_isUnit (I := I) α hx
  -- The chart-`α` inverse Gram matrix is `Gα⁻¹`, and `Gα = P · Gx · Pᵀ`.
  have hGα_eq : Gα = P * Gx * Pᵀ := chartGramMatrix_eq_cob_conj (I := I) g α x
  -- Step 1: expand each chart-`α` frame vector in the model basis.
  have hframe : ∀ j : Fin n,
      (chartBasisVecFiber (I := I) α j x : TangentSpace I x) =
        ∑ p : Fin n, P j p • e p := fun j =>
    chartBasisVecFiber_eq_sum_model (I := I) α x j
  -- Rewrite `deTurckChartLocal` and apply the vector bilinear expansion.
  ...
  -- Substitute the inverse-Gram coefficient and the expanded bilinear term.
  ...
  calc
    ...
    -- Distribute the scalar `Gα⁻¹ j k` into the double sum.
    _ = ∑ j : Fin n, ∑ k : Fin n, ∑ p : Fin n, ∑ q : Fin n,
          ...
          rw [smul_smul]
    -- Reorder the sums so that `p, q` are outermost.
    _ = ∑ p : Fin n, ∑ q : Fin n, ∑ j : Fin n, ∑ k : Fin n,
            (Gα⁻¹ j k * (P j p * P k q)) • A (e p) (e q) := by
          -- Collapse `(j, k)` and `(p, q)` into pairs and apply a single `Finset.sum_comm`.
          ...
          rw [show (∑ p : Fin n, ∑ q : Fin n, ∑ j : Fin n, ∑ k : Fin n,
            ...
            rw [← Finset.sum_product', Finset.univ_product_univ]]
          rw [Finset.sum_comm]
    -- Collect the scalar coefficient: `∑_{j,k} Gα⁻¹_{jk} P_{jp} P_{kq} = (Pᵀ Gα⁻¹ P)_{pq}`.
    _ = ∑ p : Fin n, ∑ q : Fin n,
            (Pᵀ * Gα⁻¹ * P) p q • A (e p) (e q) := by
          refine Finset.sum_congr rfl (fun p _ => Finset.sum_congr rfl (fun q _ => ?_))
          -- Pull both inner sums out of the scalar multiplication.
          ...
          rw [hpull]
          congr 1
          -- The collected coefficient is the `(p, q)` entry of `Pᵀ · Gα⁻¹ · P`.
          -- `(Pᵀ · Gα⁻¹ · P) p q = ∑_a ∑_b P_{bp} Gα⁻¹_{ba} P_{aq}`.
          rw [Matrix.mul_apply]
          rw [show (∑ a : Fin n, (Pᵀ * Gα⁻¹) p a * P a q)
                = ∑ a : Fin n, ∑ b : Fin n,
                    P b p * Gα⁻¹ b a * P a q from ?_]
          · -- Relabel `(a, b) ↦ (k, j)` and commute the sums to match
            -- `∑_j ∑_k Gα⁻¹_{jk} P_{jp} P_{kq}`.
            rw [Finset.sum_comm]
            refine Finset.sum_congr rfl (fun j _ => ?_)
    ...
    -- Apply the conjugation identity `Pᵀ · Gα⁻¹ · P = Gx⁻¹`.
...
/-! ## The DeTurck vector field as a chart-independent function -/
...
theorem deTurckChartLocal_eq_deTurckFun (g g' : SmoothRiemannianMetric I M)
  ...
  -- The chart at `x` always contains `x` in its base set.
...
/-! ## Vanishing for a metric with itself -/
