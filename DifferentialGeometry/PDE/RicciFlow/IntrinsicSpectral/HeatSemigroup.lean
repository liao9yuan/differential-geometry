...
noncomputable def tensor_heat_semigroup_predicate_free
    ...
    (h : TensorL2 r s g →L[ℝ] TensorL2 r s g) :
    TensorL2 r s g →L[ℝ] TensorL2 r s g := h
...
noncomputable def tensor_heat_semigroup_package_boundedC0
    ...
    (h : BoundedC0Semigroup (TensorL2 r s g)) :
    BoundedC0Semigroup (TensorL2 r s g) := h
...
theorem parabolic_smoothing_ensures_Cinf_on_positive_t
    ...
    {t : ℝ} (ht : 0 < t)
    (semigroup : TensorL2 r s g →L[ℝ] TensorL2 r s g)
    (h_smooth : ∃ S : SmoothCcTensor g r s,
      (S : TensorL2 r s g) =
        tensor_heat_semigroup_predicate_free g r s t semigroup u₀) :
    ∃ S : SmoothCcTensor g r s,
      (S : TensorL2 r s g) =
        tensor_heat_semigroup_predicate_free g r s t semigroup u₀ := h_smooth
