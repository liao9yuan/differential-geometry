...
theorem assemble_pou_h1_iso_intrinsic_h1
  ...
  -- already yields constants `c₁ ≤ C₁` matching the chart-frame Hilbert-
  -- Schmidt aggregation of `∇^k T` (for `k ∈ {0, 1}`) against the
  -- corresponding operator-norm chart-Sobolev aggregation.
  ...
  -- `pou_weighted_norm_equals_chart_component_norm_up_to_constant`. This
  -- contributes the `T`-component (no derivative) of the `H^1`
  -- Hilbert-Schmidt versus operator-norm chart-Sobolev equivalence, and
  -- ensures that the Gram-twist constants of `fibrewise_gram_twist_estimate`
  -- absorb into the absolute constants alongside the `k = 1` Christoffel
  -- contribution of `christoffel_Ck_bound_from_metric_Ck1`.
  obtain ⟨c₀, C₀, hc₀_pos, hc₀_le_C₀, h₀⟩ :=
