...
/-! ### The autonomisation bridge: intrinsic `hX` ⇒ jointly-`C¹` autonomised field -/
...
theorem autonomizedFlowVF_section_contMDiff
  ...
  -- Time factor: the constant-`1` section of `TangentBundle 𝓘(ℝ, ℝ) ℝ`, precomposed
  -- with `Prod.fst`.
  have hψ : ContMDiff (𝓘(ℝ, ℝ).prod I) (𝓘(ℝ, ℝ).prod 𝓘(ℝ, ℝ)) ∞
      (fun p : ℝ × M =>
        (TotalSpace.mk' ℝ p.1 ((1 : ℝ) : TangentSpace 𝓘(ℝ, ℝ) p.1) :
          TangentBundle 𝓘(ℝ, ℝ) ℝ)) := by
    -- The constant-`1` section on `ℝ` is `C∞` (pointwise, via the section criterion).
    have hone : ContMDiff 𝓘(ℝ, ℝ) (𝓘(ℝ, ℝ).prod 𝓘(ℝ, ℝ)) ∞
      ...
      -- On the model fibre `𝓘(ℝ, ℝ)` the trivialization reading is the identity:
      -- the fibre coordinate of the constant-`1` section is the constant `1`.
      simpa using (contMDiffAt_const (c := (1 : ℝ)))
    exact hone.comp contMDiff_fst
  -- Spatial factor: `p ↦ ⟨p.2, X p.1 p.2⟩` is exactly the intrinsic datum `hX`.
  have hχ : ContMDiff (𝓘(ℝ, ℝ).prod I) (I.prod 𝓘(ℝ, E)) ∞
      (fun p : ℝ × M => (TotalSpace.mk' E p.2 (X p.1 p.2) : TangentBundle I M)) := hX
  -- Pair the two factors, then apply the smooth inverse product equivalence.
...
/-! ### The global jointly-smooth flow on a closed manifold -/
...
theorem h3_global_flow_jointContMDiffOn_on_closed_manifold
  ...
  classical
  -- The joint-`C¹` autonomisation, the input to the uniqueness glue.
  have hX_auto : AutonomizedFieldJointC1 (I := I) X :=
    autonomizedFieldJointC1_of_contMDiff X hX
  -- For each base point, the local flow data from H3.
  ...
  -- Extract the local data as functions of the base point.
  choose U hU_open hU_mem Tloc hTloc_pos Φloc hΦloc_init hΦloc_smooth hΦloc_bare
    using hlocal
  -- The `U p₀` form an open cover of the compact `M`.
  ...
  -- Whether `M` is empty governs whether the cover index set `s` is nonempty.
  by_cases hMempty : IsEmpty M
  · -- Empty manifold: every base-point statement is vacuous.
    refine ⟨1, one_pos, fun p _ => p, fun p => (hMempty.false p).elim,
      ?_, fun p => (hMempty.false p).elim⟩
    -- `Ioo (t₀-1) (t₀+1) ×ˢ univ` is empty (the `univ : Set M` factor is empty).
  ...
  · -- Nonempty manifold.
    ...
    -- `s` is nonempty: it covers the nonempty `univ`.
    have hs_ne : s.Nonempty := by
      ...
      obtain ⟨i, hi, _⟩ := hx
      exact ⟨i, hi⟩
    -- Uniform horizon `T := minᵢ Tloc i > 0`.
    ...
    -- `t₀` is interior to the uniform window.
    have ht₀_mem : t₀ ∈ Set.Ioo (t₀ - T) (t₀ + T) := ⟨by linarith, by linarith⟩
    -- A covering index for each point.
    ...
    -- (Agreement) any two local flows through `x ∈ U i ∩ U j` agree on the uniform
    -- window, by bare-flow uniqueness.  This makes `Φ` independent of the index choice.
    have hagree : ∀ (i j : M) (x : M), i ∈ s → j ∈ s → x ∈ U i → x ∈ U j →
        ∀ t ∈ Set.Ioo (t₀ - T) (t₀ + T), Φloc i x t = Φloc j x t := by
      intro i j x hi hj hxi hxj t ht
      -- Restrict the window of `i` and `j` to the uniform window.
      ...
      -- Bare flow of `i`/`j` (as within-derivatives on the uniform window).
      ...
      -- The two curves agree at `t₀` (both equal `x`).
    ...
    have hbare : ∀ p, ∀ t ∈ Set.Ioo (t₀ - T) (t₀ + T),
      ...
      -- `Φ p = Φloc (αRep p) p` definitionally.
      change HasMFDerivAt 𝓘(ℝ, ℝ) I (fun u => Φloc (αRep p) p u) t _
      simpa [hΦ_def] using h
    -- (Joint smoothness) on `Ioo ×ˢ univ`.  Local: near each `(t, x)` the global flow
    -- agrees with the local flow `Φloc (αRep x)`, which is jointly `ContMDiffOn`.
    have hsmooth : ContMDiffOn (𝓘(ℝ, ℝ).prod I) I ∞ (fun q : ℝ × M => Φ q.2 q.1)
        (Set.Ioo (t₀ - T) (t₀ + T) ×ˢ (Set.univ : Set M)) := by
      -- `ContMDiffOn` from a cover by relatively-open sets on which `Φ` agrees with a
      -- jointly-`ContMDiffOn` local flow.
      intro q hq
      obtain ⟨ht, _⟩ := hq
      -- The base index for the spatial component `q.2`.
      set i : M := αRep q.2 with hi_def
      have hi_s : i ∈ s := hαRep_mem q.2
      have hq2_Ui : q.2 ∈ U i := hαRep_in q.2
      -- The relatively-open neighbourhood of `q` inside the source.
      set W : Set (ℝ × M) := Set.Ioo (t₀ - T) (t₀ + T) ×ˢ U i with hW_def
      have hW_open : IsOpen W := isOpen_Ioo.prod (hU_open i)
      have hq_W : q ∈ W := ⟨ht, hq2_Ui⟩
      -- The local flow of index `i` is jointly `ContMDiffOn` on its own (larger) window,
      -- hence on the uniform window `W`.
      have hwin_i : Set.Ioo (t₀ - T) (t₀ + T) ⊆ Set.Ioo (t₀ - Tloc i) (t₀ + Tloc i) :=
      ...
      -- On `W`, the global flow agrees with the local flow of index `i`:
      -- for `q' = (t', x') ∈ W` we have `x' ∈ U i ∩ U (αRep x')`, so by `hagree`
      -- `Φ x' t' = Φloc (αRep x') x' t' = Φloc i x' t'`.
      ...
      -- `Φ` is `ContMDiffWithinAt` at `q` within the full source: it agrees on the
      -- relatively-open `W` with the smooth local flow, and `W ∈ 𝓝[source] q`.
      ...
      -- Swap to the global flow on `W` (eventually-equal within `𝓝[W] q`).
      ...
      -- Enlarge the set from `W` to the full source.
