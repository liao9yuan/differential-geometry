...
/-! ## Pushforward smoothness and chain identities -/
...
private lemma pushforward_eval
  ...
  -- Rewrite `b = Φ (Φ.symm b)` and use the previous lemma.
...
/-! ## Pushforward smoothness for general points

We need that `Φ_* Y` is smooth as a section of `TM` when `Y` is smooth. This is the
section-level version of `pushforward_mdiffAt` from `CovDerivPullbackPointwise.lean`,
extended to global smoothness. -/
...
private lemma pushforward_contMDiff
  ...
  -- Use the function-level equality `pushforward = mpullback ∘ symm`.
  have hfun_eq := pushforward_eq_mpullback_symm_global (I := I) Φ Y
  -- Convert to smoothness via the equality at the total-space level.
  have hfun_total :
  ...
  rw [hfun_total]
  -- Now prove smoothness of mpullback of a smooth section under a smooth diffeomorphism.
...
/-! ## Pushforward of covApply equals covApply of pushforwards
...
private lemma pushforward_covApply_eq
  ...
  funext b
  -- Define `y := Φ.symm b`, so `Φ y = b`.
  let y := Φ.symm b
  have hb : Φ y = b := Φ.apply_symm_apply b
  -- Rewrite both sides via `pushforward_eval`.
...
/-! ## Section-level riemannSec naturality

For smooth global tangent vector fields `X, Y, Z` on `M` and `x : M`,
$$
  \mathrm{d}\Phi_x\bigl(\text{riemannSec}\,(LC\,(\Phi^*g))\,X\,Y\,Z\,x\bigr)
    = \text{riemannSec}\,(LC\,g)\,(\Phi_*X)\,(\Phi_*Y)\,(\Phi_*Z)\,(\Phi x).
$$
This is the section-level Riemann curvature naturality under a diffeomorphism. -/
...
private lemma riemannSec_pullback_pointwise
  ...
  -- Unfold both sides of `riemannSec`.
  ...
  -- The formula for `riemannSec`:
  --   `cov.toFun (covApply cov Y Z) x (X x) - cov.toFun (covApply cov X Z) x (Y x)
  --      - cov.toFun Z x (mlieBracket I X Y x)`
  rw [riemannSec_def, riemannSec_def]
  -- Apply linearity of `mfderiv Φ x` (which is a CLM) to distribute over `(- -)`.
  -- mfderiv Φ x is a CLM = ContinuousLinearMap, so it's linear.
  rw [ContinuousLinearMap.map_sub, ContinuousLinearMap.map_sub]
  -- Now we have three pieces to match. Use the chain rule on each.
  -- Term 1: mfderiv Φ x (cov₁.toFun (covApply cov₁ Y Z) x (X x))
  --       = cov₂.toFun (Φ_*(covApply cov₁ Y Z)) (Φ x) (mfderiv Φ x (X x))
  --       = cov₂.toFun (covApply cov₂ Y' Z') (Φ x) (X' (Φ x))
  have hcovYZ_smooth : ContMDiff I (I.prod 𝓘(ℝ, E)) ∞ (T% (covApply cov₁ Y Z)) := by
    -- Reuse the helper from `Ricci.lean` (`covApply_smooth_section`).
    -- Without exposing it, re-derive via `covApply_contMDiffOn`.
  ...
  -- Now match each term using pushforward_covApply_eq, pushforward identities,
  -- and Lie-bracket naturality.
  -- For terms 1, 2: replace `Φ_*(covApply cov₁ Y Z)` with `covApply cov₂ Y' Z'`.
  have hcovYZ_eq := pushforward_covApply_eq (I := I) g Φ hY hZ
  have hcovXZ_eq := pushforward_covApply_eq (I := I) g Φ hX hZ
  -- Replace `mfderiv Φ x (X x)` with `X' (Φ x)` and similarly for Y.
  ...
  -- The bracket-pushforward identity reads:
  --   mfderiv Φ x (mlieBracket I X Y x) = mlieBracket I X' Y' (Φ x)
...
/-! ## Bundled riemannOp naturality

Lift the section-level naturality to the bundled trilinear operator by choosing smooth
extensions of the fibre vectors. -/
...
private lemma riemannOp_pullback_pointwise
  ...
  classical
  -- Set up smooth extensions of u, v, w.
  ...
  -- Push the LHS through `riemannOp_apply_smooth`.
  ...
  rw [riemannOp_apply_smooth (cov := LeviCivita (I := I) (Diffeomorph.pullbackMetric g Φ))
        hU hV hW]
  -- Use section-level riemannSec naturality.
  rw [riemannSec_pullback_pointwise (I := I) g Φ hU hV hW]
  -- The RHS uses pushforwards `Φ_*U, Φ_*V, Φ_*W` smooth global sections. Use riemannOp_apply_smooth
  -- on the right side with these.
  ...
  -- Replace `(Φ_*U)(Φ x) = mfderiv Φ x (U x)`, etc.
  rw [pushforward_eval_at_image (I := I) Φ U x,
      pushforward_eval_at_image (I := I) Φ V x,
      pushforward_eval_at_image (I := I) Φ W x]

/-! ## Endomorphism conjugation identity

Specialise the `riemannOp` naturality to obtain the conjugation identity for the
endomorphism `ricciEndo`. -/
...
private lemma ricciEndo_conjugation
  ...
  apply LinearMap.ext
  intro Z
  -- `ricciEndo (Φ*g) x v w Z = riemannOp (LeviCivita (Φ*g)) x Z v w`.
  ...
  -- Use `mfderiv Φ.symm (Φx) ∘ mfderiv Φ x = id`, i.e., apply `mfderiv Φ x` to both sides
  -- and use `riemannOp_pullback_pointwise`.
  rw [← riemannOp_pullback_pointwise (I := I) g Φ x v w Z]
  -- Goal: `riemannOp (Φ*g) x Z v w = mfderiv Φ.symm (Φx) (mfderiv Φ x (riemannOp (Φ*g) x Z v w))`
...
theorem ricciTensor_pullback_conjugation
  ...
  classical
  -- Reduce both sides to traces of endomorphisms.
  ...
  -- Now the LHS is `trace (e.symm.toLinearMap ∘ F ∘ e.toLinearMap)` where
  -- `e := mfderivToContinuousLinearEquiv Φ ... x : T_x M ≃L T_{Φx} M`
  -- and `F := ricciEndo g (Φx) (dΦv) (dΦw)`.
  -- We need to identify this with `e.toLinearEquiv.conj F` (an endomorphism of T_{Φx} M
  -- conjugated to an endomorphism of T_x M):
  --   `e.toLinearEquiv.symm.conj F = e.symm ∘ F ∘ e`. Then `trace_conj'` says
  --   `trace (e.symm.conj F) = trace F`.
  ...
  -- `e.symm` as `CLM` is `mfderiv Φ.symm (Φ x)`.
  -- This needs an extra step: `Diffeomorph.mfderivToContinuousLinearEquiv` for `Φ.symm` at
  -- `Φ x` returns `T_{Φx} M ≃L T_{Φ.symm (Φx)} M = T_x M`. And its coe is `mfderiv Φ.symm (Φ x)`.
  have hsymm_coe : (e.symm : TangentSpace I (Φ x) →L[ℝ] TangentSpace I x)
      = mfderiv I I (⇑Φ.symm) (Φ x) := by
    -- `e.symm` corresponds to differentiating `Φ.symm` at `Φ x`. Show both CLMs apply the same
    -- way: `e.symm y` and `mfderiv Φ.symm (Φ x) y` both equal `y`'s preimage under `e = mfderiv Φ x`.
    apply ContinuousLinearMap.ext
    intro y
    -- Show that `mfderiv Φ x (e.symm y) = y` and `mfderiv Φ x (mfderiv Φ.symm (Φ x) y) = y`,
    -- then use injectivity of `mfderiv Φ x = e`.
    have h2 : mfderiv I I (⇑Φ) x (mfderiv I I (⇑Φ.symm) (Φ x) y) = y :=
      mfderiv_compose_symm (I := I) Φ x y
    -- Apply `he_coe` to convert `mfderiv Φ x` ↔ `e` in `e.right_inv`.
    have hee : (e : TangentSpace I x →L[ℝ] TangentSpace I (Φ x)) (e.symm y) = y := e.right_inv y
    have h1' : mfderiv I I (⇑Φ) x (e.symm y) = y := by
      rw [← he_coe]; exact hee
    -- Injectivity of `mfderiv Φ x` (since it equals the CLE `e`).
  ...
  -- Now rewrite the LHS in terms of `e.toLinearEquiv.symm.conj`.
  set F := ricciEndo (I := I) g (Φ x) (mfderiv I I (⇑Φ) x v) (mfderiv I I (⇑Φ) x w) with hF_def
  -- LHS of the equation we are proving:
  --   `trace ℝ (T_x M) ((mfderiv Φ.symm (Φ x)).toLinearMap.comp (F.comp (mfderiv Φ x).toLinearMap))`
  --   `= trace ℝ (T_x M) (e.symm.toLinearMap.comp (F.comp e.toLinearMap))`
  --   `= trace ℝ (T_x M) (e.toLinearEquiv.symm.conj F)`
  have hreplace_outer : ((mfderiv I I (⇑Φ.symm) (Φ x)).toLinearMap.comp
    ...
    rw [h1, h2]; rfl
  rw [hreplace_outer]
  -- Now LHS is `trace (e.symm.toLinearMap ∘ F ∘ e.toLinearMap)`.
  -- Identify with `e.toLinearEquiv.symm.conj F = e.toLinearEquiv.symm.toLinearMap ∘ F
  --                ∘ e.toLinearEquiv.symm.symm.toLinearMap`.
  -- Note `e.toLinearEquiv.symm.symm = e.toLinearEquiv`, so `e.toLinearEquiv.symm.conj F
  -- = e.toLinearEquiv.symm.toLinearMap ∘ F ∘ e.toLinearEquiv.toLinearMap`.
  -- And `e.toLinearEquiv.symm.toLinearMap = e.symm.toLinearMap`, by `toLinearEquiv_symm`.
  have hconj : (e.toLinearEquiv.symm.conj F :
      TangentSpace I x →ₗ[ℝ] TangentSpace I x) =
      ((e.symm : TangentSpace I (Φ x) →L[ℝ] TangentSpace I x).toLinearMap.comp
        (F.comp (e : TangentSpace I x →L[ℝ] TangentSpace I (Φ x)).toLinearMap)) := by
    rw [LinearEquiv.conj_apply]
    -- After unfolding `conj_apply`, the linear map presentation is
    -- `((e.toLinearEquiv.symm : T_{Φx} M →ₗ T_x M).comp F).comp
    --     (e.toLinearEquiv.symm.symm : T_x M →ₗ T_{Φx} M)`.
    -- We need:
    -- `e.symm.toLinearMap = e.toLinearEquiv.symm.toLinearMap` (where LHS is from CLM).
    -- `e.toLinearMap = e.toLinearEquiv.symm.symm.toLinearMap` (using `symm_symm`).
    rfl
  rw [← hconj]
  -- Now use `LinearMap.trace_conj'`: `trace ℝ N (e'.conj F) = trace ℝ M F` for
  -- `e' : M ≃ₗ N` and `F : M →ₗ M`.
  -- Here `e' := e.toLinearEquiv.symm : T_{Φx} M ≃ₗ T_x M`, so
  --   `trace ℝ (T_x M) (e.toLinearEquiv.symm.conj F) = trace ℝ (T_{Φx} M) F`.
