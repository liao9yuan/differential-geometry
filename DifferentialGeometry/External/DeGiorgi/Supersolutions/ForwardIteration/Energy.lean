...
private theorem sq_div_four_one_sub_nonneg {p : ℝ} (hp1 : p < 1) :
    0 ≤ p ^ 2 / (4 * (1 - p)) :=
  div_nonneg (sq_nonneg _)
    (mul_nonneg (by norm_num) (sub_nonneg.mpr hp1.le))

private theorem forward_termA_bound
  ...
  have hconst_nonneg : 0 ≤ p ^ 2 / (4 * (1 - p)) :=
    sq_div_four_one_sub_nonneg hp1
...
theorem superPowerCutoffFwd_energy_bound_reg
  ...
  have hTermA :
    ...
    exact forward_termA_bound hp1 A.1.Λ_pos hcoreA hleft_bound hbound_est
  have hTermB_int :
