...
noncomputable def ContMDiffVectorBundleEquiv.ofLinearEquivSection
  ...
  -- Construct the bundle equivalence via a helper that pattern-matches to avoid subst issues
  have mk_equiv :
      ∀ (Φ : ContMDiffVectorBundleHom ℝ I n F₁ E₁ F₂ E₂)
        (Ψ : ContMDiffVectorBundleHom ℝ I n F₂ E₂ F₁ E₁)
        (hΦ : Φ.baseMap = _root_.id) (hΨ : Ψ.baseMap = _root_.id),
        (∀ p, Ψ.toFun (Φ.toFun p) = p) → (∀ p, Φ.toFun (Ψ.toFun p) = p) →
        ContMDiffVectorBundleEquiv ℝ I n F₁ E₁ F₂ E₂
  ...
  exact mk_equiv Φ Ψ hΦ hΨ hΨΦ hΦΨ
