...
theorem exists_smooth_extension {E : Type} [NormedAddCommGroup E] [NormedSpace ℝ E]
    ...
    (hf : ∃ r : ℝ, 0 < r ∧ ContDiffOn ℝ (⊤ : ℕ∞) f (Metric.ball (0 : E) r)) :
    ∃ g : E → ℝ, ContDiff ℝ (⊤ : ℕ∞) g ∧ g =ᶠ[nhds (0 : E)] f := by
  ...
  have hgSmooth : ContDiff ℝ (⊤ : ℕ∞) g := by
    have hContDiffOn : ContDiffOn ℝ (⊤ : ℕ∞) g Set.univ := by
      rw [isOpen_univ.contDiffOn_iff]
      intro x hxuniv
      by_cases hx : x ∈ Metric.ball (0 : E) r
      · have hφat : ContDiffAt ℝ (⊤ : ℕ∞) (fun y : E => (φ : E → ℝ) y) x :=
          ((φ.contDiff (n := (⊤ : ℕ∞))) : ContDiff ℝ (⊤ : ℕ∞) (φ : E → ℝ)).contDiffAt
        have hfat : ContDiffAt ℝ (⊤ : ℕ∞) f x := by
          have hmp : ∀ ⦃a : E⦄, a ∈ Metric.ball (0 : E) r → ContDiffAt ℝ (⊤ : ℕ∞) f a :=
      ...
      · have hxr : r ≤ dist x 0 := by
        ...
        have hgzero : g =ᶠ[nhds x] (fun _ : E => (0 : ℝ)) := by
          ...
          simp [hy]
        exact (contDiffAt_const : ContDiffAt ℝ (⊤ : ℕ∞) (fun _ : E => (0 : ℝ)) x).congr_of_eventuallyEq hgzero
    exact (contDiffOn_univ (𝕜 := ℝ) (n := (⊤ : ℕ∞)) (f := g)).mp hContDiffOn
