...
noncomputable def morseRoundedModelField {m k : ℕ} (hk : k ≤ m + 1)
    (c ε₀ ε₁ R₀ R₁ : ℝ) {H : Type} [TopologicalSpace H] {M : Type} [TopologicalSpace M]
    [ChartedSpace H M] {I : ModelWithCorners ℝ (MorseModel (m + 1)) H} {f : M → ℝ}
    (data : MorseChart (m + 1) k hk c I f)
    (V₀ : (x : M) → TangentSpace I x) (y : MorseModel (m + 1)) : TangentSpace I (data.χ y) :=
...
theorem morseRoundedModelField_descent {m k : ℕ} (hk : k ≤ m + 1)
    ...
    (hV₀ : ∀ x ∈ f ⁻¹' Set.Icc (c - ε - δ) (c + r ^ 2 / 2),
      (NormedSpace.fromTangentSpace (f x)) ((mfderiv I 𝓘(ℝ, ℝ) f x) (V₀ x)) = -1)
    {y : MorseModel (m + 1)}
    (hy : data.χ y ∈ f ⁻¹' Set.Icc (c - ε - δ) (c + r ^ 2 / 2)) :
    (NormedSpace.fromTangentSpace (f (data.χ y))) (mfderiv I 𝓘(ℝ, ℝ) f (data.χ y)
      (morseRoundedModelField hk c ε₀ ε₁ R₀ R₁ data V₀ y)) = -1 := by
  let β : ℝ := Real.smoothTransition ((‖posPart hk y‖ - ε₀) / (ε₁ - ε₀)) *
    (1 - Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)))
  have hmain : (NormedSpace.fromTangentSpace (f (data.χ y))) (mfderiv I 𝓘(ℝ, ℝ) f (data.χ y)
      (β • (mfderiv 𝓘(ℝ, MorseModel (m + 1)) I data.χ y (CellAttachment.modelFlowField hk y)) +
        (1 - β) • (V₀ (data.χ y)))) = -1 := by
    by_cases hβ : β = 0
    · have hmap : (NormedSpace.fromTangentSpace (f (data.χ y))) (mfderiv I 𝓘(ℝ, ℝ) f (data.χ y)
          (β • (mfderiv 𝓘(ℝ, MorseModel (m + 1)) I data.χ y (CellAttachment.modelFlowField hk y)) +
            (1 - β) • (V₀ (data.χ y)))) =
          (NormedSpace.fromTangentSpace (f (data.χ y))) (mfderiv I 𝓘(ℝ, ℝ) f (data.χ y) (V₀ (data.χ y))) := by
        rw [hβ]
        simp
      rw [hmap]
      exact hV₀ (data.χ y) hy
    · have hβne : β ≠ 0 := hβ
      ...
      have hlin : (NormedSpace.fromTangentSpace (f (data.χ y))) (mfderiv I 𝓘(ℝ, ℝ) f (data.χ y)
        ...
        simp [smul_eq_mul]
      rw [hlin, hdescent', hV₀ (data.χ y) hy]
      ring
...
theorem isCompact_ballImage {m k : ℕ} (hk : k ≤ m + 1) (c R₁ : ℝ)
    ...
    {I : ModelWithCorners ℝ (MorseModel (m + 1)) H} {f : M → ℝ}
    (data : MorseChart (m + 1) k hk c I f) (hR₁R : R₁ ≤ data.R) :
    IsCompact (data.χ '' {y : MorseModel (m + 1) | morseNorm (m + 1) y ≤ R₁}) := by
  have hcontNorm : Continuous (fun w : MorseModel (m + 1) => morseNorm (m + 1) w) := by
    dsimp [morseNorm]
    exact continuous_norm.comp (PiLp.continuous_toLp (p := (2 : ENNReal)) (β := fun _ : Fin (m + 1) => ℝ))
  have hclosed : IsClosed {y : MorseModel (m + 1) | morseNorm (m + 1) y ≤ R₁} := by
  ...
  have hsrc' : {y : MorseModel (m + 1) | morseNorm (m + 1) y ≤ R₁} ⊆ data.χ.source := by
    intro y hy
    exact data.hχsrc y (le_trans hy hR₁R)
  exact hcompBall.image_of_continuousOn (data.χ.continuousOn.mono hsrc')

theorem isCompact_tsupport_morseRoundedDescentField {m k : ℕ} (hk : k ≤ m + 1)
    (c ε₀ ε₁ R₀ R₁ : ℝ) {H : Type} [TopologicalSpace H] {M : Type} [TopologicalSpace M]
    [ChartedSpace H M] [T2Space M]
    {I : ModelWithCorners ℝ (MorseModel (m + 1)) H} [I.Boundaryless]
    [IsManifold I (⊤ : WithTop ℕ∞) M] {f : M → ℝ}
    (data : MorseChart (m + 1) k hk c I f)
    (V₀ : (x : M) → TangentSpace I x) (hR₀R₁ : R₀ < R₁) (hR₁R : R₁ ≤ data.R)
    (hV₀supp : IsCompact (tsupport V₀)) :
    IsCompact (tsupport (morseRoundedDescentField hk c ε₀ ε₁ R₀ R₁ data V₀)) := by
  let field : (x : M) → TangentSpace I x := morseRoundedDescentField hk c ε₀ ε₁ R₀ R₁ data V₀
  have hcompImg := isCompact_ballImage (hk := hk) (c := c) (R₁ := R₁) (data := data) hR₁R
  have hsub : Function.support field ⊆ tsupport V₀ ∪ data.χ '' {y | morseNorm (m + 1) y ≤ R₁} := by
    intro x hx
    by_contra hnot
    have hxV : x ∉ tsupport V₀ := by
      intro h
      exact hnot (Or.inl h)
    have hxball : x ∉ data.χ '' {y : MorseModel (m + 1) | morseNorm (m + 1) y ≤ R₁} := by
      intro h
      exact hnot (Or.inr h)
    have hfield : field x = V₀ x := by
      dsimp [field]
      exact morseRoundedDescentField_eq_V₀_of_not_mem_ballImage hk c ε₀ ε₁ R₀ R₁ data V₀ hR₀R₁ hxball
    have hVzero : V₀ x = 0 := by
      by_contra hne
      have hmem : x ∈ Function.support V₀ := Function.mem_support.mpr hne
      exact hxV (subset_closure hmem)
    exact hx (by
      rw [hfield, hVzero]
      rfl)
  have hclosedU : IsClosed (tsupport V₀ ∪ data.χ '' {y : MorseModel (m + 1) | morseNorm (m + 1) y ≤ R₁}) := by
    exact (isClosed_tsupport (f := V₀)).union hcompImg.isClosed
  have htsub : tsupport field ⊆ tsupport V₀ ∪ data.χ '' {y : MorseModel (m + 1) | morseNorm (m + 1) y ≤ R₁} :=
    closure_minimal hsub hclosedU
  have hcompU : IsCompact (tsupport V₀ ∪ data.χ '' {y : MorseModel (m + 1) | morseNorm (m + 1) y ≤ R₁}) :=
...
theorem morseRoundedDescentField_unstretchInChart {m k : ℕ} (hk : k ≤ m + 1)
    (c ε r δ ε₀ ε₁ R₀ R₁ : ℝ) {H : Type} [TopologicalSpace H] {M : Type} [TopologicalSpace M]
    [ChartedSpace H M] [T2Space M]
    {I : ModelWithCorners ℝ (MorseModel (m + 1)) H} [I.Boundaryless]
    [IsManifold I (⊤ : WithTop ℕ∞) M] {f : M → ℝ}
    (data : MorseChart (m + 1) k hk c I f)
    ...
    curveAt (morseRoundedDescentField hk c ε₀ ε₁ R₀ R₁ data V₀)
      ...
      (data.χ y) (CellAttachment.modelAttachedUnstretchTime hk ε r δ y) =
      data.χ (CellAttachment.modelAttachedUnstretch hk ε r δ y) := by
  ...
  have hchartField : ∀ t ∈ Set.Ioo a b,
    ...
    simpa [v, z] using hf
  have hflow := morseAttachedUnstretchInChart (hk := hk) (c := c) (ε := ε) (r := r) (δ := δ)
    (data := data) (v := v) (hv := hv) (hsupp := hsupp)
    (hchartField := hchartField) (hδ0 := hδ0) (hδr := hδr)
...
noncomputable def modelRoundedTransportTime {m k : ℕ} (hk : k ≤ m + 1)
  ...
  let β : ℝ := (1 - Real.smoothTransition
...
theorem modelRoundedTransportTime_eq_unstretch {m k : ℕ} (hk : k ≤ m + 1)
  ...
  have hβ₁ : 1 - Real.smoothTransition
    ...
    have harg : (‖posPart hk y‖ ^ 2 - smoothCap ε r δ (‖negPart hk y‖ ^ 2)) / θ ≤ 0 := by
      exact div_nonpos_of_nonpos_of_nonneg (by nlinarith [hy]) (le_of_lt hθ)
    rw [Real.smoothTransition.zero_of_nonpos harg]
    norm_num
...
theorem modelRoundedTransportTime_eq_collar {m k : ℕ} (hk : k ≤ m + 1)
    ...
    (hy : smoothCap ε r δ (‖negPart hk y‖ ^ 2) + θ ≤ ‖posPart hk y‖ ^ 2) :
    modelRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ y =
      morseFarExpandTimeSmooth c ε δ ρ ρ' (morseNormalForm hk c y) := by
  dsimp [modelRoundedTransportTime]
  have hβ₁ : 1 - Real.smoothTransition
      ((‖posPart hk y‖ ^ 2 - smoothCap ε r δ (‖negPart hk y‖ ^ 2)) / θ) = 0 := by
    have harg : 1 ≤ (‖posPart hk y‖ ^ 2 - smoothCap ε r δ (‖negPart hk y‖ ^ 2)) / θ := by
      rw [one_le_div hθ]
      nlinarith [hy]
    rw [Real.smoothTransition.one_of_one_le harg]
    norm_num
...
theorem contDiff_modelRoundedTransportTime {m k : ℕ} (hk : k ≤ m + 1)
  ...
  have hβ₁ : ContDiff ℝ (⊤ : ℕ∞)
      (fun y : MorseModel (m + 1) => 1 - Real.smoothTransition
        ((‖posPart hk y‖ ^ 2 - smoothCap ε r δ (‖negPart hk y‖ ^ 2)) / θ)) := by
    ...
    simpa using (contDiff_const : ContDiff ℝ (⊤ : ℕ∞) (fun _ : MorseModel (m + 1) => (1 : ℝ))).sub hsm
...
theorem contMDiff_morseRoundedTransportMap {m k : ℕ} (hk : k ≤ m + 1)
      ...
      (fun x : M =>
        curveAt (morseRoundedDescentField hk c ε₀ ε₁ R₀ R₁ data V₀)
          (exists_globalIntegralCurve_of_compactSupport (morseRoundedDescentField hk c ε₀ ε₁ R₀ R₁ data V₀)
            ...
            (isCompact_tsupport_morseRoundedDescentField hk c ε₀ ε₁ R₀ R₁ data V₀ hR₀R₁
              (le_of_lt hR₁R) hV₀supp))
          x (morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x)) := by
  let v : (x : M) → TangentSpace I x := morseRoundedDescentField hk c ε₀ ε₁ R₀ R₁ data V₀
  ...
  have hτ : ContMDiff I 𝓘(ℝ, ℝ) (↑(⊤ : ℕ∞) : WithTop ℕ∞)
      (morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data) :=
    contMDiff_morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data hf
  ...
  have hpair : ContMDiff I (𝓘(ℝ, ℝ).prod I) (↑(⊤ : ℕ∞) : WithTop ℕ∞)
      (fun x : M => (morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x, x)) :=
  ...
  have hc : ContMDiff I I (↑(⊤ : ℕ∞) : WithTop ℕ∞)
      (fun x : M => curveAt v (exists_globalIntegralCurve_of_compactSupport v hv hsupp) x
        (morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x)) := by
    have hcomp : ContMDiff I I (min (⊤ : ℕ∞) (⊤ : ℕ∞))
        (fun x : M => curveAt v (exists_globalIntegralCurve_of_compactSupport v hv hsupp)
          (morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x, x).2
          (morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x, x).1) :=
      hjoint.comp hpair
    simpa [Function.comp_def] using hcomp
  simpa [v] using hc

noncomputable def morseRoundedTransportMap {m k : ℕ} (hk : k ≤ m + 1)
    (c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ : ℝ) {H : Type} [TopologicalSpace H] {M : Type} [TopologicalSpace M]
    [ChartedSpace H M] [T2Space M]
    {I : ModelWithCorners ℝ (MorseModel (m + 1)) H} [I.Boundaryless]
    [IsManifold I (⊤ : WithTop ℕ∞) M] {f : M → ℝ}
...
theorem morseRoundedTransportMap_eq_unstretch_of_handle {m k : ℕ} (hk : k ≤ m + 1)
  ...
  have htimeData : morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data (data.χ y) =
      modelAttachedUnstretchTime hk ε r δ y := by
    rw [morseRoundedTransportTime_eq_model hk c ε r δ ρ ρ' θ R₀ R₁ data hyball]
    rw [data.χ.left_inv hysrc]
    exact hτ
  have hflow := morseRoundedDescentField_unstretchInChart hk c ε r δ ε₀ ε₁ R₀ R₁ data V₀ hV₀sm
    hV₀supp hε₀ hε₀ε₁ hR₀R₁ hRltRp hR₁R hδ0 hδr
...
theorem morseRoundedTransportMap_mem_upper_of_handle {m k : ℕ} (hk : k ≤ m + 1)
    (c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ : ℝ) {H : Type} [TopologicalSpace H] {M : Type} [TopologicalSpace M]
    [ChartedSpace H M] [T2Space M]
    {I : ModelWithCorners ℝ (MorseModel (m + 1)) H} [I.Boundaryless]
    [IsManifold I (⊤ : WithTop ℕ∞) M] {f : M → ℝ}
...
theorem morseRoundedTransportMap_strict_of_handle {m k : ℕ} (hk : k ≤ m + 1)
    (c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ : ℝ) {H : Type} [TopologicalSpace H] {M : Type} [TopologicalSpace M]
    [ChartedSpace H M] [T2Space M]
    {I : ModelWithCorners ℝ (MorseModel (m + 1)) H} [I.Boundaryless]
    [IsManifold I (⊤ : WithTop ℕ∞) M] {f : M → ℝ}
  ...
  have hfval : f (data.χ (modelAttachedUnstretch hk ε r δ y)) =
    ...
    data.hnorm (modelAttachedUnstretch hk ε r δ y) hnormUnstretch
  rw [hfval]
  exact modelAttachedUnstretch_strict_of_roundedSublevel hk c ε r δ R₀ R₁ hε hδ0 hδr
    hR₀R₁ (le_of_lt hR₀) hbig hyg
...
theorem morseRoundedTransportMap_boundary_of_collar {m k : ℕ} (hk : k ≤ m + 1)
    (c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ : ℝ) {H : Type} [TopologicalSpace H] {M : Type} [TopologicalSpace M]
    [ChartedSpace H M] [T2Space M]
    {I : ModelWithCorners ℝ (MorseModel (m + 1)) H} [I.Boundaryless]
    [IsManifold I (⊤ : WithTop ℕ∞) M] {f : M → ℝ}
    ...
    (hε₀ : 0 < ε₀) (hε₀ε₁ : ε₀ < ε₁) (hR₀R₁ : R₀ < R₁) (hRltRp : data.R < data.R')
    (hR₁R : R₁ < data.R)
    (hδ0 : 0 < δ) (hρ : 0 < ρ) (hρ' : 0 < ρ') (hρρ' : ρ + ρ' ≤ δ)
    (hε : 0 < ε) (hr2 : r ^ 2 = 2 * ε)
    (hf : ContMDiff I 𝓘(ℝ, ℝ) (↑(⊤ : ℕ∞) : WithTop ℕ∞) f)
    (hV₀desc : ∀ x ∈ f ⁻¹' Set.Icc (c - ε - δ) (c + r ^ 2 / 2),
      (NormedSpace.fromTangentSpace (f x)) ((mfderiv I 𝓘(ℝ, ℝ) f x) (V₀ x)) = -1)
  ...
  have hfx : f x = c - ε := by
    have hle : f x + ε = c := by
      rwa [hfadd] at hxg
    nlinarith
  let z : SublevelSpace f (c - ε) := ⟨x, by
    change f x ≤ c - ε
    rw [hfx]⟩
  have hτ : morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x =
      morseFarExpandTimeSmooth c ε δ ρ ρ' (f x) :=
    morseRoundedTransportTime_eq_collar hk c ε r δ ρ ρ' θ R₀ R₁ data hx
  have hval := morseRoundedCollarMapSmooth_value c ε r δ ρ ρ' hf hε hδ0 hρ hρ' hr2
    v hv hsupp hdfOn hrate z
  dsimp [morseRoundedTransportMap, z] at hval ⊢
  rw [hτ]
  have htop := morseFarExpandTimeSmooth_top (c := c) (ε := ε) (δ := δ) (ρ := ρ) (ρ' := ρ') hρ hε hδ0 hρρ'
  have hτval : morseFarExpandTimeSmooth c ε δ ρ ρ' (f x) = -(2 * ε) := by
    rw [hfx]
    exact htop
  rw [hval]
  rw [hτval, hfx]
...
theorem morseRoundedTransportMap_strict_of_collar {m k : ℕ} (hk : k ≤ m + 1)
    (c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ : ℝ) {H : Type} [TopologicalSpace H] {M : Type} [TopologicalSpace M]
    [ChartedSpace H M] [T2Space M]
    {I : ModelWithCorners ℝ (MorseModel (m + 1)) H} [I.Boundaryless]
    [IsManifold I (⊤ : WithTop ℕ∞) M] {f : M → ℝ}
  ...
  have hmono := morseFarExpandTimeSmooth_levelMap_strictMono (c := c) (ε := ε) (δ := δ)
    (ρ := ρ) (ρ' := ρ') hρ hρ' hε hδ0 hfx
  have htop := morseFarExpandTimeSmooth_top (c := c) (ε := ε) (δ := δ) (ρ := ρ) (ρ' := ρ') hρ hε hδ0 hρρ'
  have hLtop : (c - ε) - morseFarExpandTimeSmooth c ε δ ρ ρ' (c - ε) = c + ε := by
    rw [htop]
    ring
  have hr2' : c + ε = c + r ^ 2 / 2 := by rw [hr2]; ring
  rw [← hr2']
...
private lemma modelRoundedTransportTime_eq_attached_comb {m k : ℕ} (hk : k ≤ m + 1)
    (c ε r δ ρ ρ' θ R₀ R₁ : ℝ) (hθ : 0 < θ) {y : MorseModel (m + 1)}
    (hy : y ∈ modelAttachedRegion hk ε r δ) :
    modelRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ y =
      (1 - Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀))) *
...
private lemma modelAttachedUnstretchTime_nonpos {m k : ℕ} (hk : k ≤ m + 1) (ε r δ : ℝ)
    (hε : 0 ≤ ε) (hδ : 0 < δ) (hδr : δ < r ^ 2) (y : MorseModel (m + 1)) :
    modelAttachedUnstretchTime hk ε r δ y ≤ 0 := by
  dsimp [modelAttachedUnstretchTime]
  have hsc : 0 < smoothCap ε r δ (‖negPart hk y‖ ^ 2) :=
    smoothCap_pos (ε := ε) (r := r) (δ := δ) (t := ‖negPart hk y‖ ^ 2) hδ hδr
  have hcap : smoothCap ε r δ (‖negPart hk y‖ ^ 2) ≤ ‖negPart hk y‖ ^ 2 + r ^ 2 := by
    have hle := smoothCap_le_max (ε := ε) (r := r) (δ := δ) (t := ‖negPart hk y‖ ^ 2) hε hδ
  ...
  have hratio : 1 ≤ (‖negPart hk y‖ ^ 2 + r ^ 2) / smoothCap ε r δ (‖negPart hk y‖ ^ 2) :=
    (one_le_div hsc).2 hcap
  have hnonpos : 1 - (‖negPart hk y‖ ^ 2 + r ^ 2) / smoothCap ε r δ (‖negPart hk y‖ ^ 2) ≤ 0 := by
    linarith
  have hmul : ‖posPart hk y‖ ^ 2 * (1 - (‖negPart hk y‖ ^ 2 + r ^ 2) / smoothCap ε r δ (‖negPart hk y‖ ^ 2)) ≤ 0 :=
    mul_nonpos_of_nonneg_of_nonpos (sq_nonneg ‖posPart hk y‖) hnonpos
  nlinarith

private lemma modelAttachedUnstretch_level_eq {m k : ℕ} (hk : k ≤ m + 1) (c ε r δ : ℝ)
    (hε : 0 ≤ ε) (hδ : 0 < δ) (hδr : δ < r ^ 2) (y : MorseModel (m + 1)) :
    morseNormalForm hk c (modelAttachedUnstretch hk ε r δ y) =
      morseNormalForm hk c y - modelAttachedUnstretchTime hk ε r δ y := by
  by_cases hpos : ‖posPart hk y‖ = 0
  · have ht : modelAttachedUnstretchTime hk ε r δ y = 0 := by
      dsimp [modelAttachedUnstretchTime]
      rw [hpos]
      norm_num
    rw [ht]
    have hu : modelAttachedUnstretch hk ε r δ y = y := by
  ...
  · have hp : 0 < ‖posPart hk y‖ := lt_of_le_of_ne (norm_nonneg _) (Ne.symm hpos)
    have ht : 0 ≤ -modelAttachedUnstretchTime hk ε r δ y := by
      exact neg_nonneg.mpr (modelAttachedUnstretchTime_nonpos hk ε r δ hε hδ hδr y)
    have hval := modelFlow_f_add hk c (-modelAttachedUnstretchTime hk ε r δ y) y ht hp
    have hflow : modelAttachedUnstretch hk ε r δ y =
        modelFlow hk (modelAttachedUnstretchTime hk ε r δ y) y :=
      modelAttachedUnstretch_eq_modelFlow hk ε r δ hδ hδr y
    rw [hflow]
    simpa [neg_neg, sub_eq_add_neg] using hval

private lemma attachedBoundary_level_ge {m k : ℕ} (hk : k ≤ m + 1) (c ε r δ : ℝ)
    (hδ : 0 < δ) {y : MorseModel (m + 1)}
    (hA : modelAttachedFunction hk c ε r δ y = c) :
    c - ε - δ ≤ morseNormalForm hk c y := by
  have hpos : ‖posPart hk y‖ ^ 2 = smoothCap ε r δ (‖negPart hk y‖ ^ 2) := by
    dsimp [modelAttachedFunction] at hA
    nlinarith
  have hN : morseNormalForm hk c y =
      c + (1 / 2) * (smoothCap ε r δ (‖negPart hk y‖ ^ 2) - ‖negPart hk y‖ ^ 2) := by
    rw [morseNormalForm_split hk c y, hpos]
  rw [hN]
  by_cases ht : r ^ 2 + 2 * ε + δ ≤ ‖negPart hk y‖ ^ 2
  · have hcap : smoothCap ε r δ (‖negPart hk y‖ ^ 2) = ‖negPart hk y‖ ^ 2 - 2 * ε :=
      smoothCap_upper hδ ht
    rw [hcap]
    nlinarith
  · have ht' : ‖negPart hk y‖ ^ 2 < r ^ 2 + 2 * ε + δ := lt_of_not_ge ht
    have hcap_ge : ‖negPart hk y‖ ^ 2 - 2 * ε - 2 * δ ≤
        smoothCap ε r δ (‖negPart hk y‖ ^ 2) := by
      let t : ℝ := ‖negPart hk y‖ ^ 2
      have hcap_ge_min : min (r ^ 2) (t - 2 * ε) ≤ smoothCap ε r δ t := by
        dsimp [smoothCap]
        let σ : ℝ := Real.smoothTransition ((t - (r ^ 2 + 2 * ε - δ)) / (2 * δ))
        have hσ : 0 ≤ σ := Real.smoothTransition.nonneg _
        have hσ₁ : σ ≤ 1 := Real.smoothTransition.le_one _
        by_cases ha : 0 ≤ t - 2 * ε - r ^ 2
        · have hmin : min (r ^ 2) (t - 2 * ε) = r ^ 2 := by
            rw [min_eq_left]
            nlinarith
          rw [hmin]
          nlinarith [hσ, ha]
        · have ha' : t - 2 * ε - r ^ 2 ≤ 0 := le_of_not_ge ha
          have hmin : min (r ^ 2) (t - 2 * ε) = t - 2 * ε := by
            rw [min_eq_right]
            nlinarith
          rw [hmin]
          have hfac : t - 2 * ε - r ^ 2 ≤ (t - 2 * ε - r ^ 2) * σ := by
            simpa [mul_comm] using (mul_le_mul_of_nonpos_right hσ₁ ha')
          nlinarith
      have hmin_ge : t - 2 * ε - 2 * δ ≤ min (r ^ 2) (t - 2 * ε) := by
        rw [le_min_iff]
        constructor <;> nlinarith [ht']
      exact le_trans hmin_ge hcap_ge_min
    nlinarith [hcap_ge]

private theorem morseRoundedTransportMap_rate_upper {m k : ℕ} (hk : k ≤ m + 1)
    (c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ : ℝ) {H : Type} [TopologicalSpace H] {M : Type} [TopologicalSpace M]
    [ChartedSpace H M] [T2Space M]
    {I : ModelWithCorners ℝ (MorseModel (m + 1)) H} [I.Boundaryless]
    [IsManifold I (⊤ : WithTop ℕ∞) M] {f : M → ℝ}
    ...
    (hV₀rate : ∀ x : M,
      -1 ≤ (NormedSpace.fromTangentSpace (f x)) ((mfderiv I 𝓘(ℝ, ℝ) f x) (V₀ x)) ∧
      (NormedSpace.fromTangentSpace (f x)) ((mfderiv I 𝓘(ℝ, ℝ) f x) (V₀ x)) ≤ 0)
    (x : M) (hτle : morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x ≤ 0) :
    f (morseRoundedTransportMap hk c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ data V₀ hV₀sm hV₀supp hε₀ hε₀ε₁
      hR₀R₁ hRltRp hR₁R x) ≤ f x - morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x := by
  let v : (x : M) → TangentSpace I x := morseRoundedDescentField hk c ε₀ ε₁ R₀ R₁ data V₀
  have hv : ContMDiff I (I.prod 𝓘(ℝ, MorseModel (m + 1))) (↑(⊤ : ℕ∞) : WithTop ℕ∞)
      (fun x : M => (⟨x, v x⟩ : TangentBundle I M)) := by
    simpa [v] using contMDiff_morseRoundedDescentFieldSection hk c ε₀ ε₁ R₀ R₁ data V₀
  ...
  let hcomplete : ∀ x : M, ∃ γ : ℝ → M, γ 0 = x ∧ IsMIntegralCurve γ v :=
    exists_globalIntegralCurve_of_compactSupport v hv hsupp
  have ht : 0 ≤ -morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x := by linarith
  have hrb := f_rate_bounds_of_integralCurve_back f hf v hrate
    (hγ := curveAt_integralCurve v hcomplete x)
    (t := -morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x) ht
  have hmain : f (curveAt v hcomplete x (morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x)) ≤
      f x - morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x := by
    have hb : f (curveAt v hcomplete x (morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x)) ≤
        f x + (-morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x) := by
...
theorem morseRoundedTransportMap_mem_upper_of_sublevel {m k : ℕ} (hk : k ≤ m + 1)
    ...
    (hε₀ : 0 < ε₀) (hε₀ε₁ : ε₀ < ε₁) (hR₀R₁ : R₀ < R₁) (hRltRp : data.R < data.R')
    (hR₁R : R₁ < data.R)
    (hδ0 : 0 < δ) (hδr : δ < r ^ 2) (hρ : 0 < ρ) (hρ' : 0 < ρ')
  ...
  · rcases hx with ⟨y, hy, hxy⟩
    ...
    have hτle : modelRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ y ≤ 0 := by
      ...
      have hσmem : Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) ∈
          Set.Icc (0 : ℝ) 1 :=
        ⟨Real.smoothTransition.nonneg _, Real.smoothTransition.le_one _⟩
      ...
      have h₂ : Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) *
    ...
    have hmain : f x - morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x ≤
      ...
      · have hσ₀ : Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) = 0 := by
          apply Real.smoothTransition.zero_of_nonpos
          exact div_nonpos_of_nonpos_of_nonneg (by linarith) (le_of_lt (sub_pos.mpr hR₀R₁))
        have hβ₂ : 1 - Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) = 1 := by
          rw [hσ₀]
          norm_num
      ...
      · have hygt : R₀ < morseNorm (m + 1) y := lt_of_not_ge hR₀y
        ...
        have hb₂ : morseNormalForm hk c y - morseFarExpandTimeSmooth c ε δ ρ ρ'
          ...
          have hle := morseFarExpandTimeSmooth_levelMap_le_top (c := c) (ε := ε) (δ := δ)
            (ρ := ρ) (ρ' := ρ') (t := morseNormalForm hk c y) hρ hρ' hε hδ0 hNle
          have hr2' : c + ε = c + r ^ 2 / 2 := by rw [hr2]; ring
          nlinarith
        have hσmem : Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) ∈
            Set.Icc (0 : ℝ) 1 :=
        ...
        have h₂ : Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) *
            (morseNormalForm hk c y - morseFarExpandTimeSmooth c ε δ ρ ρ' (morseNormalForm hk c y)) ≤
            Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) *
              (c + r ^ 2 / 2) := by
...
private theorem flowValueBackwardOnBand {n : ℕ} {H : Type} [TopologicalSpace H] {M : Type}
  ...
  rw [← hneg]
  rw [hmain]
  have hadd : f y + s = f y - t := by dsimp [s]; ring
  simpa [hneg] using hadd

theorem morseRoundedTransportMap_boundary_of_sublevel {m k : ℕ} (hk : k ≤ m + 1)
    (c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ : ℝ) {H : Type} [TopologicalSpace H] {M : Type} [TopologicalSpace M]
    [ChartedSpace H M] [T2Space M]
    {I : ModelWithCorners ℝ (MorseModel (m + 1)) H} [I.Boundaryless]
    [IsManifold I (⊤ : WithTop ℕ∞) M] {f : M → ℝ}
  ...
  · rcases hx with ⟨y, hy, hxy⟩
    ...
    have hsymm : data.χ.symm x = y := by
      rw [← hxy]
      exact data.χ.left_inv hsrc
    have hygc : CellAttachment.modelRoundedFunction hk c ε r δ R₀ R₁ y = c := by
      have heq := morseRoundedFunction_eq_model_of_mem_closedBall hk c ε r δ R₀ R₁ data
        ⟨y, hy, hxy⟩
      rw [heq] at hxg
      rwa [hsymm] at hxg
    have hyatt : y ∈ modelAttachedRegion hk ε r δ :=
      (CellAttachment.modelRoundedFunction_le_c_iff_mem_attachedRegion hk c ε r δ R₀ R₁
        hε hδ0 hR₀R₁ hR₀ hbig y).mp (le_of_eq hygc)
    have hAc : CellAttachment.modelAttachedFunction hk c ε r δ y = c :=
      (CellAttachment.modelRoundedFunction_eq_c_iff_attachedFunction_eq hk c ε r δ R₀ R₁
        hε hδ0 hR₀R₁ hR₀ hbig y).mp hygc
    have hτ : morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x =
        modelRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ y := by
      rw [morseRoundedTransportTime_eq_model hk c ε r δ ρ ρ' θ R₀ R₁ data ⟨y, hy, hxy⟩]
      rw [hsymm]
    have hN : f x = morseNormalForm hk c y := by
      rw [← hxy]
      exact data.hnorm y (le_trans hy hR₁₂R)
    let τ : ℝ := morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x
    have hτdef : morseRoundedTransportMap hk c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ data V₀ hV₀sm
        hV₀supp hε₀ hε₀ε₁ hR₀R₁ hRltRp hR₁R x =
        curveAt v (exists_globalIntegralCurve_of_compactSupport v hv hsupp) x τ := by
      dsimp [morseRoundedTransportMap, v, τ]
    by_cases hR₀y : morseNorm (m + 1) y ≤ R₀
    · have hσ₀ : Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) = 0 := by
        apply Real.smoothTransition.zero_of_nonpos
        exact div_nonpos_of_nonpos_of_nonneg (by linarith) (le_of_lt (sub_pos.mpr hR₀R₁))
      ...
      have hτeq : modelRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ y =
        ...
        rw [hβ₂, hσ₀]
        ring
      have hτval : τ = modelAttachedUnstretchTime hk ε r δ y := by
        dsimp [τ]
        rw [hτ, hτeq]
      have hstrip : f x ∈ Set.Icc (c - ε - δ) (c + r ^ 2 / 2) := by
        rw [hN]
        constructor
        · exact attachedBoundary_level_ge hk c ε r δ hδ0 hAc
        · exact morseNormalForm_le_upper_of_mem_attachedRegion hk c ε r δ (le_of_lt hε) hδ0 hyatt
      have ht1 : f x - (c + r ^ 2 / 2) ≤ τ := by
        rw [hτval]
        have hNub : morseNormalForm hk c (modelAttachedUnstretch hk ε r δ y) ≤ c + r ^ 2 / 2 :=
          le_of_eq (modelAttachedUnstretch_boundary_of_roundedSublevel hk c ε r δ R₀ R₁ hε hδ0 hδr
            hR₀R₁ hR₀ hbig hygc)
        have hEq : f x - modelAttachedUnstretchTime hk ε r δ y =
            morseNormalForm hk c (modelAttachedUnstretch hk ε r δ y) := by
          rw [hN]
          exact (modelAttachedUnstretch_level_eq hk c ε r δ (le_of_lt hε) hδ0 hδr y).symm
        have hle : f x - modelAttachedUnstretchTime hk ε r δ y ≤ c + r ^ 2 / 2 := by
          rw [hEq]
          exact hNub
        linarith
      have hτle : τ ≤ 0 := by
      ...
      have hmain : f (morseRoundedTransportMap hk c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ data V₀ hV₀sm
        ...
        rw [hτdef]
        simpa [v] using hflowVal
      rw [hmain]
      have hfinal : f x - τ = c + r ^ 2 / 2 := by
        rw [hτval, hN]
        have hEq : morseNormalForm hk c y - modelAttachedUnstretchTime hk ε r δ y =
            morseNormalForm hk c (modelAttachedUnstretch hk ε r δ y) :=
          (modelAttachedUnstretch_level_eq hk c ε r δ (le_of_lt hε) hδ0 hδr y).symm
        rw [hEq]
        exact modelAttachedUnstretch_boundary_of_roundedSublevel hk c ε r δ R₀ R₁ hε hδ0 hδr
    ...
    · have hygt : R₀ < morseNorm (m + 1) y := lt_of_not_ge hR₀y
      have hnegl : r ^ 2 + 2 * ε + δ ≤ ‖negPart hk y‖ ^ 2 := by
        ...
        have hlt : ‖negPart hk y‖ ^ 2 < r ^ 2 + 2 * ε + δ := lt_of_not_ge hnot
        have hnormle : morseNorm (m + 1) y ^ 2 < R₀ ^ 2 := by
          have hmain := modelAttached_norm_sq_lt_of_negPart_lt hk ε r δ (le_of_lt hε) hδ0
            hyatt hlt
        ...
        have hle : morseNorm (m + 1) y ≤ R₀ := by
          ...
          rw [h1, h2] at habs
          exact le_of_lt habs
        exact (not_lt_of_ge hle) hygt
      have hgN : CellAttachment.modelRoundedFunction hk c ε r δ R₀ R₁ y =
          morseNormalForm hk c y + ε :=
        CellAttachment.modelRoundedFunction_eq_morse_add_eps_of_negPart_large hk c ε r δ R₀ R₁
          hδ0 hnegl
      have hNε : morseNormalForm hk c y = c - ε := by
        have hc' : morseNormalForm hk c y + ε = c := by
          rw [← hgN]
          exact hygc
        linarith
      have ht_u : modelAttachedUnstretchTime hk ε r δ y = -(r ^ 2 + 2 * ε) / 2 := by
        have hp : ‖posPart hk y‖ ^ 2 = ‖negPart hk y‖ ^ 2 - 2 * ε := by
          have hsplit := morseNormalForm_split hk c y
          rw [hNε] at hsplit
          nlinarith
        exact modelAttachedUnstretchTime_eq_boundary hk ε r δ hδ0 hnegl hp
      have ht_f : morseFarExpandTimeSmooth c ε δ ρ ρ' (morseNormalForm hk c y) = -(2 * ε) := by
        rw [hNε]
        exact morseFarExpandTimeSmooth_top (c := c) (ε := ε) (δ := δ) (ρ := ρ) (ρ' := ρ')
          hρ hε hδ0 hρρ'
      have hτeq : modelRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ y =
          -(2 * ε) := by
        rw [modelRoundedTransportTime_eq_attached_comb hk c ε r δ ρ ρ' θ R₀ R₁ hθ hyatt]
        rw [ht_u, ht_f]
        have hr2' : -(r ^ 2 + 2 * ε) / 2 = -(2 * ε) := by rw [hr2]; ring
        rw [hr2']
        ring
      have hτval : τ = -(2 * ε) := by
        dsimp [τ]
      ...
      have hfinal : f x - τ = c + r ^ 2 / 2 := by
        ...
        ring
      exact hfinal
  · exact morseRoundedTransportMap_boundary_of_collar hk c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ data V₀
      hV₀sm hV₀supp hε₀ hε₀ε₁ hR₀R₁ hRltRp hR₁R hδ0 hρ hρ' hρρ' hε hr2 hf hV₀desc hV₀rate
      x hx hxg

theorem morseRoundedTransportMap_strict_of_sublevel {m k : ℕ} (hk : k ≤ m + 1)
    (c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ : ℝ) {H : Type} [TopologicalSpace H] {M : Type} [TopologicalSpace M]
    [ChartedSpace H M] [T2Space M]
    {I : ModelWithCorners ℝ (MorseModel (m + 1)) H} [I.Boundaryless]
    [IsManifold I (⊤ : WithTop ℕ∞) M] {f : M → ℝ}
  ...
  · rcases hx with ⟨y, hy, hxy⟩
    ...
    have hyatt : y ∈ modelAttachedRegion hk ε r δ :=
      (CellAttachment.modelRoundedFunction_le_c_iff_mem_attachedRegion hk c ε r δ R₀ R₁
        hε hδ0 hR₀R₁ hR₀ hbig y).mp (le_of_lt hyg)
    have hτ : morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x =
        modelRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ y := by
      rw [morseRoundedTransportTime_eq_model hk c ε r δ ρ ρ' θ R₀ R₁ data ⟨y, hy, hxy⟩]
      rw [hsymm]
    ...
    have hmain : f x - morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x <
      ...
      · have hygt : R₀ < morseNorm (m + 1) y := lt_of_not_ge hR₀y
        ...
        have hgN : CellAttachment.modelRoundedFunction hk c ε r δ R₀ R₁ y =
          ...
          CellAttachment.modelRoundedFunction_eq_morse_add_eps_of_negPart_large hk c ε r δ R₀ R₁
            hδ0 hnegl
        have hNlt : morseNormalForm hk c y < c - ε := by
          have hc' : morseNormalForm hk c y + ε < c := by
            rw [← hgN]
            exact hyg
          linarith
        ...
        rw [hval]
        have hb₁ : morseNormalForm hk c (modelAttachedUnstretch hk ε r δ y) < c + r ^ 2 / 2 :=
          modelAttachedUnstretch_strict_of_roundedSublevel hk c ε r δ R₀ R₁ hε hδ0 hδr
            hR₀R₁ hR₀ hbig hyg
        have hb₂ : morseNormalForm hk c y - morseFarExpandTimeSmooth c ε δ ρ ρ'
            (morseNormalForm hk c y) < c + r ^ 2 / 2 := by
          have hlt := morseFarExpandTimeSmooth_levelMap_strictMono (c := c) (ε := ε) (δ := δ)
            (ρ := ρ) (ρ' := ρ') hρ hρ' hε hδ0 hNlt
          have hle := morseFarExpandTimeSmooth_levelMap_le_top (c := c) (ε := ε) (δ := δ)
            (ρ := ρ) (ρ' := ρ') (t := c - ε) hρ hρ' hε hδ0 le_rfl
          have hLtop : (c - ε) - morseFarExpandTimeSmooth c ε δ ρ ρ' (c - ε) ≤ c + r ^ 2 / 2 := by
            have hr2' : c + ε = c + r ^ 2 / 2 := by rw [hr2]; ring
            rw [← hr2']
            exact hle
          exact lt_of_lt_of_le hlt hLtop
        have hσmem : Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) ∈
            Set.Icc (0 : ℝ) 1 :=
          ⟨Real.smoothTransition.nonneg _, Real.smoothTransition.le_one _⟩
        have h₁ : (1 - Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀))) *
            ...
            (1 - Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀))) *
              (c + r ^ 2 / 2) := by
          exact mul_le_mul_of_nonneg_left (le_of_lt hb₁) (sub_nonneg.mpr hσmem.2)
        have h₂ : Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) *
            (morseNormalForm hk c y - morseFarExpandTimeSmooth c ε δ ρ ρ' (morseNormalForm hk c y)) ≤
            Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) *
              (c + r ^ 2 / 2) := by
          exact mul_le_mul_of_nonneg_left (le_of_lt hb₂) hσmem.1
        have hstrict : (1 - Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀))) *
            morseNormalForm hk c (modelAttachedUnstretch hk ε r δ y) +
              Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) *
                (morseNormalForm hk c y -
                  morseFarExpandTimeSmooth c ε δ ρ ρ' (morseNormalForm hk c y)) <
            c + r ^ 2 / 2 := by
          by_cases hσ0 : Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) = 0
          · rw [hσ0]
            simpa using hb₁
          · have hσ0' : 0 < Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) :=
              lt_of_le_of_ne hσmem.1 (Ne.symm hσ0)
            by_cases hσ1 : Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) = 1
            · rw [hσ1]
              simpa using hb₂
            · have hσ1' : Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) < 1 :=
                lt_of_le_of_ne hσmem.2 hσ1
              have h₁' : (1 - Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀))) *
                  morseNormalForm hk c (modelAttachedUnstretch hk ε r δ y) <
                  (1 - Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀))) *
                    (c + r ^ 2 / 2) := by
                exact mul_lt_mul_of_pos_left hb₁ (sub_pos.mpr hσ1')
              have h₂' : Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) *
                  (morseNormalForm hk c y -
                    morseFarExpandTimeSmooth c ε δ ρ ρ' (morseNormalForm hk c y)) ≤
                  Real.smoothTransition ((morseNorm (m + 1) y - R₀) / (R₁ - R₀)) *
              ...
              nlinarith
        exact hstrict
    exact lt_of_le_of_lt hupper hmain
  · exact morseRoundedTransportMap_strict_of_collar hk c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ data V₀
      hV₀sm hV₀supp hε₀ hε₀ε₁ hR₀R₁ hRltRp hR₁R hδ0 hρ hρ' hρρ' hε hr2 hf hV₀desc hV₀rate
      x hx hxg
...
private theorem morseFarExpandLevelMap_eq_of_le_upper {c ε δ ρ ρ' : ℝ} (hρ : 0 < ρ)
  ...
  by_cases hdeep : t ≤ c - ε - δ
  · refine ⟨t, ?_, ?_⟩
    · nlinarith
    · have hτ : morseFarExpandTimeSmooth c ε δ ρ ρ' t = 0 :=
        morseFarExpandTimeSmooth_zero hρ (by linarith)
      rw [hτ]
      ring
  · have hnotdeep : c - ε - δ < t := lt_of_not_ge hdeep
    let L : ℝ → ℝ := fun s => s - morseFarExpandTimeSmooth c ε δ ρ ρ' s
    have hLcont : Continuous L := by
      dsimp [L]
      exact continuous_id.sub (contDiff_morseFarExpandTimeSmooth hρ hρ' hδ).continuous
    have hmem : t ∈ Set.Icc (L (c - ε - δ)) (L (c - ε)) := by
      have hτ₁ : morseFarExpandTimeSmooth c ε δ ρ ρ' (c - ε - δ) = 0 := by
        exact morseFarExpandTimeSmooth_zero hρ (by linarith)
      have hτ₂ : morseFarExpandTimeSmooth c ε δ ρ ρ' (c - ε) = -(2 * ε) :=
        morseFarExpandTimeSmooth_top (c := c) (ε := ε) (δ := δ) (ρ := ρ) (ρ' := ρ') hρ hε hδ hρρ'
      dsimp [L]
      constructor
      · rw [hτ₁]
        nlinarith
      · rw [hτ₂]
        nlinarith [ht]
    have hivt := intermediate_value_Icc (a := c - ε - δ) (b := c - ε) (by linarith)
      (f := L) (by
        exact hLcont.continuousOn)
    have hmem' : t ∈ L '' Set.Icc (c - ε - δ) (c - ε) := hivt hmem
    rcases hmem' with ⟨s, hs, hsL⟩
    refine ⟨s, hs.2, hsL⟩

private theorem morseFarExpandLevelMap_eq_unique {c ε δ ρ ρ' : ℝ} (hρ : 0 < ρ)
    (hρ' : 0 < ρ') (hε : 0 < ε) (hδ : 0 < δ) {s₁ s₂ t : ℝ}
    (h₁ : s₁ - morseFarExpandTimeSmooth c ε δ ρ ρ' s₁ = t)
    (h₂ : s₂ - morseFarExpandTimeSmooth c ε δ ρ ρ' s₂ = t) :
    s₁ = s₂ := by
  by_contra hne
...
private theorem morseRoundedTransportMap_preimage_of_outside {m k : ℕ} (hk : k ≤ m + 1)
    (c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ : ℝ) {H : Type} [TopologicalSpace H] {M : Type} [TopologicalSpace M]
    [ChartedSpace H M] [T2Space M]
    {I : ModelWithCorners ℝ (MorseModel (m + 1)) H} [I.Boundaryless]
    [IsManifold I (⊤ : WithTop ℕ∞) M] {f : M → ℝ}
    ...
    (hV₀rate : ∀ x : M,
      -1 ≤ (NormedSpace.fromTangentSpace (f x)) ((mfderiv I 𝓘(ℝ, ℝ) f x) (V₀ x)) ∧
      (NormedSpace.fromTangentSpace (f x)) ((mfderiv I 𝓘(ℝ, ℝ) f x) (V₀ x)) ≤ 0)
    {y : M} (hy : y ∉ data.χ '' {y : MorseModel (m + 1) | morseNorm (m + 1) y ≤ R₁})
    (hfy : f y ≤ c + r ^ 2 / 2)
    (hroom : ∀ t : ℝ, 0 ≤ t → t ≤ 2 * ε →
      curveAt (morseRoundedDescentField hk c ε₀ ε₁ R₀ R₁ data V₀)
        (exists_globalIntegralCurve_of_compactSupport (morseRoundedDescentField hk c ε₀ ε₁ R₀ R₁ data V₀)
          ...
          (isCompact_tsupport_morseRoundedDescentField hk c ε₀ ε₁ R₀ R₁ data V₀ hR₀R₁
            (le_of_lt hR₁R) hV₀supp))
        y t ∉
      data.χ '' {y : MorseModel (m + 1) | morseNorm (m + 1) y ≤ R₁}) :
    ∃ x : M, morseRoundedFunction hk c ε r δ R₀ R₁ data x ≤ c ∧
      morseRoundedTransportMap hk c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ data V₀ hV₀sm hV₀supp hε₀ hε₀ε₁
  ...
  let hcomplete : ∀ x : M, ∃ γ : ℝ → M, γ 0 = x ∧ IsMIntegralCurve γ v :=
    exists_globalIntegralCurve_of_compactSupport v hv hsupp
  by_cases hdeep : f y ≤ c - ε - δ + ρ'
  · refine ⟨y, ?_, ?_⟩
    · have hfy' : f y ≤ c - ε := by nlinarith
      rw [morseRoundedFunction_eq_f_add_eps_of_not_mem_closedBall hk c ε r δ R₀ R₁ data hy]
      nlinarith
    · exact morseRoundedTransportMap_deep hk c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ data V₀ hV₀sm hV₀supp
        hε₀ hε₀ε₁ hR₀R₁ hRltRp hR₁R hρ y hy hdeep
  · have hnotdeep : c - ε - δ + ρ' < f y := lt_of_not_ge hdeep
    have hfy_le : f y ≤ c + ε := by
      have hr2' : c + r ^ 2 / 2 = c + ε := by rw [hr2]; ring
      rwa [hr2'] at hfy
    rcases morseFarExpandLevelMap_eq_of_le_upper (c := c) (ε := ε) (δ := δ) (ρ := ρ) (ρ' := ρ')
      hρ hρ' hε hδ0 hρρ' (t := f y) hfy_le with ⟨s, hs_le, hsL⟩
    have hs_gt : c - ε - δ < s := by
      by_contra hnot
      have hsle : s ≤ c - ε - δ := le_of_not_gt hnot
      have hτ : morseFarExpandTimeSmooth c ε δ ρ ρ' s = 0 :=
        morseFarExpandTimeSmooth_zero hρ (by linarith)
      have hL : s - morseFarExpandTimeSmooth c ε δ ρ ρ' s = f y := hsL
      rw [hτ] at hL
      have hsle' : s ≤ c - ε - δ + ρ' := by linarith
      linarith
    let τs : ℝ := morseFarExpandTimeSmooth c ε δ ρ ρ' s
    have hτs_ge : -(2 * ε) ≤ τs := by
      have hle := morseFarExpandTime_ge (c := c) (ε := ε) (δ := δ) hδ0 hε (t := s) hs_le
      have hσ₁ : Real.smoothTransition ((s - (c - ε - δ + ρ')) / ρ) ≤ 1 :=
        Real.smoothTransition.le_one _
      have hmle : morseFarExpandTime c ε δ s ≤ 0 :=
        morseFarExpandTime_le_zero (c := c) (ε := ε) (δ := δ) hδ0 hε
      have hfac : Real.smoothTransition ((s - (c - ε - δ + ρ')) / ρ) *
          morseFarExpandTime c ε δ s ≥ morseFarExpandTime c ε δ s := by
        simpa [mul_comm] using (mul_le_mul_of_nonpos_right hσ₁ hmle)
      dsimp [τs, morseFarExpandTimeSmooth]
      nlinarith
    have hτs_le : τs ≤ 0 := by
      have hle := morseFarExpandTime_le_zero (c := c) (ε := ε) (δ := δ) hδ0 hε (t := s)
      have hσ : 0 ≤ Real.smoothTransition ((s - (c - ε - δ + ρ')) / ρ) :=
        Real.smoothTransition.nonneg _
      dsimp [τs, morseFarExpandTimeSmooth]
      exact mul_nonpos_of_nonneg_of_nonpos hσ hle
    let t : ℝ := -τs
    have ht0 : 0 ≤ t := by dsimp [t]; linarith
    have ht2e : t ≤ 2 * ε := by dsimp [t]; linarith
    let x : M := curveAt v hcomplete y t
    have hxout : x ∉ data.χ '' {y : MorseModel (m + 1) | morseNorm (m + 1) y ≤ R₁} := by
      dsimp [x]
      simpa [v] using hroom t ht0 ht2e
    have hfx : f x = s := by
      have hstrip : f y ∈ Set.Icc (c - ε - δ) (c + r ^ 2 / 2) := by
        constructor
        · exact le_of_lt (by linarith)
        · exact hfy
      have ht1 : f y - (c + r ^ 2 / 2) ≤ t := by
        dsimp [t]
        have hr2' : c + ε = c + r ^ 2 / 2 := by rw [hr2]; ring
        have hL : s - morseFarExpandTimeSmooth c ε δ ρ ρ' s = f y := hsL
        nlinarith [hs_le, hr2']
      have ht2 : t ≤ f y - (c - ε - δ) := by
        dsimp [t]
        have hs_ge : c - ε - δ ≤ s := le_of_lt hs_gt
        have hL : s - morseFarExpandTimeSmooth c ε δ ρ ρ' s = f y := hsL
        nlinarith
      have hval := morseCollarFlow_valueOnStrip (I := I) f c ε r δ hf v hv hsupp hdfOn hrate
        (hy := hstrip) (t := t) ht1 ht2
      dsimp [x]
      have hmain : f (curveAt v hcomplete y t) = f y - t := hval
      rw [hmain]
      dsimp [t]
      have hL : s - morseFarExpandTimeSmooth c ε δ ρ ρ' s = f y := hsL
      nlinarith
    have hτx : morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data x =
        morseFarExpandTimeSmooth c ε δ ρ ρ' s := by
      rw [morseRoundedTransportTime_eq_collar hk c ε r δ ρ ρ' θ R₀ R₁ data hxout]
      rw [hfx]
    have hgx : morseRoundedFunction hk c ε r δ R₀ R₁ data x ≤ c := by
      rw [morseRoundedFunction_eq_f_add_eps_of_not_mem_closedBall hk c ε r δ R₀ R₁ data hxout]
      have : f x + ε ≤ c := by
        have hs : s ≤ c - ε := hs_le
        rw [hfx]
        linarith
    ...
    have hΦ : morseRoundedTransportMap hk c ε r δ ρ ρ' θ R₀ R₁ ε₀ ε₁ data V₀ hV₀sm hV₀supp
      ...
      have hmain : curveAt v hcomplete (curveAt v hcomplete y t)
          (morseRoundedTransportTime hk c ε r δ ρ ρ' θ R₀ R₁ data (curveAt v hcomplete y t)) = y := by
        rw [hτx]
        have hv1 : ContMDiff I (I.prod 𝓘(ℝ, MorseModel (m + 1))) (1 : WithTop ℕ∞)
            (fun x : M => (⟨x, v x⟩ : TangentBundle I M)) :=
          hv.of_le (by norm_num : (1 : WithTop ℕ∞) ≤ ∞)
        have hadd := curveAt_add v hv1 hcomplete y t (morseFarExpandTimeSmooth c ε δ ρ ρ' s)
        have hsum : t + morseFarExpandTimeSmooth c ε δ ρ ρ' s = 0 := by
          dsimp [t]
          ring
        calc
          curveAt v hcomplete (curveAt v hcomplete y t)
              (morseFarExpandTimeSmooth c ε δ ρ ρ' s) =
              curveAt v hcomplete y (t + morseFarExpandTimeSmooth c ε δ ρ ρ' s) := hadd.symm
          _ = curveAt v hcomplete y 0 := by rw [hsum]
          _ = y := curveAt_zero v hcomplete y
      simpa [v] using hmain
    exact ⟨x, hgx, hΦ⟩

private theorem morseRoundedHandleOrbitStayInner {m k : ℕ} (hk : k ≤ m + 1)
    (ε₁ ε r δ R₀ R' : ℝ)
    (hε₁ : 0 < ε₁) (hε : 0 ≤ ε) (hδ : 0 ≤ δ) (hR₀ : 0 ≤ R₀) (hR' : 0 ≤ R')
    {y : MorseModel (m + 1)} {a b : ℝ}
    (hynorm : morseNorm (m + 1) y ^ 2 + 2 * ε + r ^ 2 + δ + 1 ≤ R₀ ^ 2)
    (hposBig : ε₁ ^ 2 ≤ ‖posPart hk y‖ ^ 2 / 2)
    (hb : b ≤ ‖posPart hk y‖ ^ 2 / 4)
    (habs : ∀ t ∈ Set.Ioo a b, |t| ≤ (2 * ε + r ^ 2 + δ) / 2 + 1 / 2)
    (hRroom : R₀ ^ 2 + 2 * ε + r ^ 2 + δ + 1 < R' ^ 2) :
    ∀ t ∈ Set.Ioo a b,
      (ε₁ ≤ ‖posPart hk (modelFlow hk t y)‖ ∧ morseNorm (m + 1) (modelFlow hk t y) ≤ R₀) ∧
      ‖modelFlow hk t y‖ < R' := by
  intro t ht
  have hpos : 0 < ‖posPart hk y‖ := by
    have hε₁sq : 0 < ε₁ ^ 2 := sq_pos_of_pos hε₁
    have hsqpos : 0 < ‖posPart hk y‖ ^ 2 := by
  ...
  have hposge : ε₁ ≤ ‖posPart hk (modelFlow hk t y)‖ :=
    modelFlow_posPart_ge_eps_of_interval hk ε₁ a b y hpos hb hposBig t ht
  let ε' : ℝ := (2 * ε + r ^ 2 + δ) / 4 + 1 / 4
  have hε'0 : 0 ≤ ε' := by
    dsimp [ε']
    nlinarith [hε, sq_nonneg r, hδ]
  have habs' : |t| ≤ 2 * ε' := by
    dsimp [ε']
    nlinarith [habs t ht]
  have hsq := modelFlow_norm_sq_le_add hk ε' hε'0 habs' y
  have h4 : 4 * ε' = 2 * ε + r ^ 2 + δ + 1 := by
    dsimp [ε']
    ring
  have hnormle : morseNorm (m + 1) (modelFlow hk t y) ^ 2 ≤ R₀ ^ 2 := by
    nlinarith [hsq, hynorm, h4]
  have hnorm : morseNorm (m + 1) (modelFlow hk t y) ≤ R₀ := by
    have hnon : 0 ≤ morseNorm (m + 1) (modelFlow hk t y) := by
      dsimp [morseNorm]
      exact norm_nonneg _
    have habs' := sq_le_sq.mp hnormle
    have h1 : |morseNorm (m + 1) (modelFlow hk t y)| = morseNorm (m + 1) (modelFlow hk t y) :=
      abs_of_nonneg hnon
    have h2 : |R₀| = R₀ := abs_of_nonneg hR₀
    rwa [h1, h2] at habs'
  have hnormR' : ‖modelFlow hk t y‖ < R' := by
    have hR'0 : 0 ≤ R' := hR'
    have hsq' : morseNorm (m + 1) (modelFlow hk t y) ^ 2 < R' ^ 2 := by
      have hy' : morseNorm (m + 1) y ^ 2 ≤ R₀ ^ 2 := by nlinarith [hynorm]
      have hle : morseNorm (m + 1) (modelFlow hk t y) ^ 2 ≤ R₀ ^ 2 + 2 * ε + r ^ 2 + δ + 1 := by
        nlinarith [hsq, hy', h4]
      nlinarith [hle, hRroom]
    have hnorm : morseNorm (m + 1) (modelFlow hk t y) < R' := by
      have hnon : 0 ≤ morseNorm (m + 1) (modelFlow hk t y) := by
        dsimp [morseNorm]
        exact norm_nonneg _
      ...
      have h1 : |morseNorm (m + 1) (modelFlow hk t y)| = morseNorm (m + 1) (modelFlow hk t y) :=
        abs_of_nonneg hnon
      have h2 : |R'| = R' := abs_of_nonneg hR'0
      rwa [h1, h2] at habs'
