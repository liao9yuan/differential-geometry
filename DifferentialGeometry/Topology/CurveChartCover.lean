...
/-- Uniformly convergent maps eventually preserve the image of a compact set
inside any open set containing the limiting image. -/
...
/-- A continuous curve on a compact real interval admits an eventually finite monotone
subdivision whose closed pieces each lie in the source of one preferred chart. -/
theorem exists_chart_split
  ...
  have hcopen : ∀ p, IsOpen (c p) := fun p ↦
    (chartAt H p).open_source.preimage hγ.restrict
...
/-- A continuous curve on a compact real interval admits an eventually finite
monotone subdivision whose closed pieces lie in the interiors of compact
subsets of preferred chart sources. -/
