...
variable {R V : Type*} {A : Type*} {Time : Type*}
...
lemma metric_var_form_eval
    ...
    td.dt_apply (fun s => (g_fam s).g X Y) t :=
  rfl
...
lemma metric_var_form_symm
    ...
    (h_met : ∀ vs αs, td.isSmoothFam (fun τ => (g_fam τ).g_tensor vs αs))
    (t : Time) (A' B : V) :
    metric_var_form td g_fam h_met t ![A', B] ![] =
    metric_var_form td g_fam h_met t ![B, A'] ![] := by
  change td.dt_apply (fun s => (g_fam s).g A' B) t = td.dt_apply (fun s => (g_fam s).g B A') t
...
variable {k R V : Type*} {A : Type*} {Time : Type*}
...
noncomputable def h_cov_deriv
    ...
    (conn : V → V → V)
    (t : Time) (X Y Z : V) : R :=
  action emb X (metric_var_form td g_fam h_met t ![Y, Z] ![])
  - metric_var_form td g_fam h_met t ![conn X Y, Z] ![]
  - metric_var_form td g_fam h_met t ![Y, conn X Z] ![]
...
private lemma mvf_sub_right
    ...
    metric_var_form td g_fam h_met t ![A', B] ![] -
      metric_var_form td g_fam h_met t ![A', C] ![] := by
  change td.dt_apply (fun s => (g_fam s).g A' (B - C)) t =
       td.dt_apply (fun s => (g_fam s).g A' B) t - td.dt_apply (fun s => (g_fam s).g A' C) t
  ...
  rw [h_eq]
  exact td.dt_apply_sub _ _ t (h_met ![A', B] ![]) (h_met ![A', C] ![])
...
theorem connection_variation
    ...
    h_cov_deriv emb td g_fam h_met (conn_fam t) t X Y Z +
    h_cov_deriv emb td g_fam h_met (conn_fam t) t Y X Z -
    h_cov_deriv emb td g_fam h_met (conn_fam t) t Z X Y +
    2 * metric_var_form td g_fam h_met t ![conn_fam t X Y, Z] ![] := by
  -- Abbreviations
  let h (A' B : V) := metric_var_form td g_fam h_met t ![A', B] ![]
  let n := conn_fam t
  ...
  have h_base :
      td.dt_apply (fun s => 2 * (g_fam s).g (conn_fam s X Y) Z) t =
      td.dt_apply (fun s =>
        (emb.embed X) ((g_fam s).g Y Z) + (emb.embed Y) ((g_fam s).g Z X)
        - (emb.embed Z) ((g_fam s).g X Y)
        - (g_fam s).g X (bracket emb Y Z)
  ...
  have hLHS_smul :
      td.dt_apply (fun s => 2 * (g_fam s).g (conn_fam s X Y) Z) t =
      2 * td.dt_apply (fun s => (g_fam s).g (conn_fam s X Y) Z) t :=
    td.dt_apply_const_mul 2 _ t h_conn_smooth
  ...
  have pt_action : ∀ (W A' B : V),
      td.dt_apply (fun s => (emb.embed W) ((g_fam s).g A' B)) t =
      (emb.embed W) (h A' B) := by
    intro W A' B
    -- Smoothness of `fun s => (g_fam s).g A' B` comes from `h_met`.
    have h_gAB : td.isSmoothFam (fun s => (g_fam s).g A' B) := h_met ![A', B] ![]
    exact h_st W (fun s => (g_fam s).g A' B) t h_gAB

  -- ∂_t(g(s)(A, constant)) for constant A, constant B = h(A, B) by definition
  have h_eval : ∀ (A' B : V),
      td.dt_apply (fun s => (g_fam s).g A' B) t = h A' B := fun A' B => rfl

  -- Smoothness for the six Koszul-term scalar slices.
  have h_gf_smooth : ∀ (U W : V), td.isSmoothFam (fun s => (g_fam s).g U W) :=
    fun U W => h_met ![U, W] ![]
  ...
  have hRHS :
    ...
    -- Smooth families for each piece
    have s1 : td.isSmoothFam (fun s => (emb.embed X) ((g_fam s).g Y Z)) :=
      h_emb_met X Y Z
    have s2 : td.isSmoothFam (fun s => (emb.embed Y) ((g_fam s).g Z X)) :=
      h_emb_met Y Z X
    have s3 : td.isSmoothFam (fun s => (emb.embed Z) ((g_fam s).g X Y)) :=
      h_emb_met Z X Y
    have s4 : td.isSmoothFam (fun s => (g_fam s).g X (bracket emb Y Z)) :=
      h_gf_smooth X (bracket emb Y Z)
    have s5 : td.isSmoothFam (fun s => (g_fam s).g Y (bracket emb Z X)) :=
      h_gf_smooth Y (bracket emb Z X)
    have s6 : td.isSmoothFam (fun s => (g_fam s).g Z (bracket emb X Y)) :=
      h_gf_smooth Z (bracket emb X Y)
    have s12 := td.isSmoothFam_add _ _ s1 s2
    have s12_3 := td.isSmoothFam_sub _ _ s12 s3
    have s12_3_4 := td.isSmoothFam_sub _ _ s12_3 s4
    have s12_3_4_5 := td.isSmoothFam_add _ _ s12_3_4 s5
    rw [h_fun,
        td.dt_apply_add _ _ _ s12_3_4_5 s6,
        td.dt_apply_add _ _ _ s12_3_4 s5,
        td.dt_apply_sub _ _ _ s12_3 s4,
        td.dt_apply_sub _ _ _ s12 s3,
        td.dt_apply_add _ _ _ s1 s2]
  ...
  have b1 : h X (bracket emb Y Z) = h X (n Y Z) - h X (n Z Y) := by
    rw [← torsion_free Y Z]; exact mvf_sub_right td g_fam h_met t X (n Y Z) (n Z Y)
  have b2 : h Y (bracket emb Z X) = h Y (n Z X) - h Y (n X Z) := by
    rw [← torsion_free Z X]; exact mvf_sub_right td g_fam h_met t Y (n Z X) (n X Z)
  have b3 : h Z (bracket emb X Y) = h Z (n X Y) - h Z (n Y X) := by
    rw [← torsion_free X Y]; exact mvf_sub_right td g_fam h_met t Z (n X Y) (n Y X)

  -- h_cov_deriv sum expands
  have eq_cov :
      h_cov_deriv emb td g_fam h_met n t X Y Z +
      h_cov_deriv emb td g_fam h_met n t Y X Z -
      h_cov_deriv emb td g_fam h_met n t Z X Y =
  ...
  -- Symmetry
  have a1 : h (n X Y) Z = h Z (n X Y) := metric_var_form_symm td g_fam h_met t _ _
  have a2 : h (n Y X) Z = h Z (n Y X) := metric_var_form_symm td g_fam h_met t _ _
  have a3 : h (n Z X) Y = h Y (n Z X) := metric_var_form_symm td g_fam h_met t _ _

  -- h symmetry to align Koszul argument order with eq_cov
  have hZX_eq : h Z X = h X Z := metric_var_form_symm td g_fam h_met t Z X
  ...
  have step_rhs :
      ...
      h_cov_deriv emb td g_fam h_met n t X Y Z +
      h_cov_deriv emb td g_fam h_met n t Y X Z -
      h_cov_deriv emb td g_fam h_met n t Z X Y + 2 * h Z (n X Y) := by
    rw [eq_cov', b1, b2, b3, a1, a2, a3]; ring

  rw [step_rhs]
  rw [show h Z (n X Y) = metric_var_form td g_fam h_met t ![conn_fam t X Y, Z] ![] from
    metric_var_form_symm td g_fam h_met t Z (conn_fam t X Y)]
...
theorem raise_variation_const
  ...
  rw [h_const]; exact td.dt_apply_const (T ![X, Y] ![]) t
...
theorem raise_variation
    ...
    (h_met : ∀ vs αs, td.isSmoothFam (fun τ => (g_fam τ).g_tensor vs αs))
    (T : TensorData R V 0 2) (X Y : V) (t : Time)
    (h_decomp : td.dt_apply (fun s => (g_fam s).g ((g_fam s).sharp (flat_covector T X)) Y) t =
      metric_var_form td g_fam h_met t ![((g_fam t).sharp (flat_covector T X)), Y] ![] +
      td.dt_apply (fun s => (g_fam t).flat Y ((g_fam s).sharp (flat_covector T X))) t) :
    td.dt_apply (fun s => (g_fam t).flat Y ((g_fam s).sharp (flat_covector T X))) t =
    - metric_var_form td g_fam h_met t ![((g_fam t).sharp (flat_covector T X)), Y] ![] := by
...
theorem tr_g_variation
    ...
    (idx₁ : Fin 2) (idx₂ : Fin 1)
    (t : Time)
    (h_smooth : ∀ vs αs, td.isSmoothFam
      (fun τ => metric_trace (g_fam τ) atr idx₁ idx₂ T vs αs)) :
    td.dt_apply (fun s =>
      (metric_trace (g_fam s) atr idx₁ idx₂ T) ![] ![]) t =
    dt_tensor td t (fun s =>
      metric_trace (g_fam s) atr idx₁ idx₂ T) h_smooth ![] ![] := by
  rfl
