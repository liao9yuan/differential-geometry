...
theorem jacobi_identity_proof {R V : Type} [CommRing R] [AddCommGroup V] [Module R V] [DerivationAction R V] [LieBracket V] [VectorFieldNonDegenerate R V] [LieDerivationRules R V] (X Y Z : V) : bracket X (bracket Y Z) + bracket Y (bracket Z X) + bracket Z (bracket X Y) = 0 := by
