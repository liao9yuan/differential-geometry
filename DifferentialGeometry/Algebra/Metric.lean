...
lemma metric_neg_left {R V} [CommRing R] [AddCommGroup V] [Module R V] [ScalarMul R V] (metric : MetricTensor R V) (X Y : V) : metric.g (-X) Y = - metric.g X Y := by
...
lemma metric_sub_left {R V} [CommRing R] [AddCommGroup V] [Module R V] [ScalarMul R V] (metric : MetricTensor R V) (X Y Z : V) : metric.g (X - Y) Z = metric.g X Z - metric.g Y Z := by
