...
Input: (V, V)
Output: R -/
structure MetricTensor (R V : Type) [CommRing R] [AddCommGroup V] [Module R V] where
  g : V → V → R
  symm : ∀ X Y : V, g X Y = g Y X
  bilinear_add_left : ∀ X Y Z : V, g (X + Y) Z = g X Z + g Y Z
  bilinear_smul_left : ∀ (f : R) (X Y : V), g (f • X) Y = f * (g X Y)

lemma metric_neg_left {R V} [CommRing R] [AddCommGroup V] [Module R V] (metric : MetricTensor R V) (X Y : V) : metric.g (-X) Y = - metric.g X Y := by
...
lemma metric_sub_left {R V} [CommRing R] [AddCommGroup V] [Module R V] (metric : MetricTensor R V) (X Y Z : V) : metric.g (X - Y) Z = metric.g X Z - metric.g Y Z := by
...
Input: (V → V → R)
Output: R -/
class MetricTraceOperator (R V : Type) [CommRing R] [AddCommGroup V] [Module R V] (metric : MetricTensor R V) where
