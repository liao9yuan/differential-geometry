...
tensor Sobolev bound `h_uniform`, this file upgrades the right-limit
...
`s ↦ tensorHeatSemigroup g r s h_uniform (t - s) (F s)` is continuous
...
`‖tensorHeatSemigroup g r s h_uniform t T − tensorHeatSemigroup g r s h_uniform t₀ T‖
  ≤ ‖tensorHeatSemigroup g r s h_uniform |t − t₀| T − T‖`. -/
private lemma norm_tensorHeatSemigroup_sub_le_tensorHeatSemigroup_diff
    ...
    ‖tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t T -
        tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t₀ T‖ ≤
      ‖tensorHeatSemigroup (I := I) (M := M) g r s h_uniform |t - t₀| T - T‖ := by
  ...
  · -- Case t ≥ t₀: use t = t₀ + (t - t₀).
    ...
    have h_law :
        tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t =
          (tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t₀).comp
            (tensorHeatSemigroup (I := I) (M := M) g r s h_uniform (t - t₀)) := by
      have h_add :=
        tensorHeatSemigroup_add (I := I) (M := M) h_uniform ht₀ h_diff_nn
    ...
    have h_apply :
        tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t T =
          tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t₀
            (tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform (t - t₀) T) := by
      rw [h_law]; rfl
    rw [h_apply]
    have h_sub_eq :
        tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t₀
            (tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform (t - t₀) T) -
          tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t₀ T =
        tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t₀
          (tensorHeatSemigroup
            (I := I) (M := M) g r s h_uniform (t - t₀) T - T) := by
      rw [← (tensorHeatSemigroup
        (I := I) (M := M) g r s h_uniform t₀).map_sub]
    rw [h_sub_eq]
    have h_norm_le :
        ‖tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t₀
            (tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform (t - t₀) T - T)‖ ≤
          ‖tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t₀‖ *
            ‖tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform (t - t₀) T - T‖ :=
      ContinuousLinearMap.le_opNorm _ _
    have h_op_le_one :
        ‖tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t₀‖ ≤ 1 :=
      tensorHeatSemigroup_opNorm_le_one (I := I) (M := M) (h_uniform := h_uniform) t₀
    have h_norm_nn :
        0 ≤ ‖tensorHeatSemigroup
            (I := I) (M := M) g r s h_uniform (t - t₀) T - T‖ := norm_nonneg _
    calc ‖tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t₀
              (tensorHeatSemigroup
                (I := I) (M := M) g r s h_uniform (t - t₀) T - T)‖
        ≤ ‖tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t₀‖ *
            ‖tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform (t - t₀) T - T‖ := h_norm_le
      _ ≤ 1 * ‖tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform (t - t₀) T - T‖ := by
            exact mul_le_mul_of_nonneg_right h_op_le_one h_norm_nn
      _ = ‖tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform (t - t₀) T - T‖ := one_mul _
      _ = ‖tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform |t - t₀| T - T‖ := by rw [h_abs]
  · -- Case t < t₀: symmetric, use t₀ = t + (t₀ - t).
    ...
    have h_law :
        tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t₀ =
          (tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t).comp
            (tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform (t₀ - t)) := by
      have h_add :=
        tensorHeatSemigroup_add (I := I) (M := M) h_uniform ht h_diff_nn
      have h_eq : t + (t₀ - t) = t₀ := by ring
      rw [h_eq] at h_add
    ...
    have h_norm_nn :
        0 ≤ ‖T - tensorHeatSemigroup
            (I := I) (M := M) g r s h_uniform (t₀ - t) T‖ := norm_nonneg _
    have h_norm_swap :
        ‖T - tensorHeatSemigroup
            (I := I) (M := M) g r s h_uniform (t₀ - t) T‖ =
          ‖tensorHeatSemigroup
    ...
    calc ‖tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t
        ...
        ≤ ‖tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t‖ *
            ‖T - tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform (t₀ - t) T‖ := h_norm_le
...
theorem tensorHeatSemigroup_continuous_at_pos
    ...
    ContinuousAt
      (fun u : ℝ => tensorHeatSemigroup
        (I := I) (M := M) g r s h_uniform u T) t := by
  -- Unfold `ContinuousAt` to `Tendsto`.
  change Filter.Tendsto
      (fun u : ℝ => tensorHeatSemigroup
        (I := I) (M := M) g r s h_uniform u T)
      (𝓝 t)
      (𝓝 (tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t T))
  -- It suffices to show the difference tends to 0.
  rw [show (𝓝 (tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t T)) =
      𝓝 (0 +
        tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t T) by rw [zero_add]]
  have h_diff_to_zero :
      Tendsto (fun u : ℝ =>
          tensorHeatSemigroup (I := I) (M := M) g r s h_uniform u T -
            tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform t T) (𝓝 t) (𝓝 0) := by
    -- Bound: norm ≤ ‖tensorHeatSemigroup g r s h_uniform |u - t| T - T‖.
    have h_pos_nhds : Set.Ioi (0 : ℝ) ∈ 𝓝 t := Ioi_mem_nhds ht
    have h_bound_event : ∀ᶠ u : ℝ in 𝓝 t,
        ‖tensorHeatSemigroup (I := I) (M := M) g r s h_uniform u T -
            tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform t T‖ ≤
          ‖tensorHeatSemigroup
            (I := I) (M := M) g r s h_uniform |u - t| T - T‖ := by
      filter_upwards [h_pos_nhds] with u hu_pos
      exact norm_tensorHeatSemigroup_sub_le_tensorHeatSemigroup_diff
        (I := I) (M := M) h_uniform (le_of_lt hu_pos) ht.le T
    ...
    have h_strong :=
      tensorHeatSemigroup_continuous_at_zero (I := I) (M := M) h_uniform T
    have h_compose :
        Tendsto (fun u : ℝ =>
            tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform |u - t| T) (𝓝 t) (𝓝 T) :=
      h_strong.comp h_abs_to_zero_within
    have h_diff_to_zero' :
        Tendsto (fun u : ℝ =>
            tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform |u - t| T - T)
            (𝓝 t) (𝓝 0) := by
  ...
  -- Add the constant `tensorHeatSemigroup g r s h_uniform t T` back.
  have h_added := h_diff_to_zero.add (tendsto_const_nhds
    (x := tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t T))
  simpa using h_added
...
theorem tensorHeatSemigroup_continuous_on_nonneg
  ...
  · -- Interior point: use the two-sided result.
    have h := tensorHeatSemigroup_continuous_at_pos
      (I := I) (M := M) h_uniform ht_pos T
    exact h.continuousWithinAt
  · -- Boundary point t = 0: use right-continuity.
    subst ht_eq
    have h_zero :
        tensorHeatSemigroup (I := I) (M := M) g r s h_uniform 0 T = T := by
...
`s ↦ tensorHeatSemigroup g r s h_uniform (t - s) (F s)` is continuous on
...
`s ↦ tensorHeatSemigroup g r s h_uniform (t - s) (F s)` on `[0, t]`,
...
theorem tensorHeatSemigroup_apply_F_continuous
    ...
    ContinuousOn
      (fun u : ℝ =>
        tensorHeatSemigroup (I := I) (M := M) g r s h_uniform (t - u) (F u))
      (Set.Icc 0 t) := by
  intro s₀ hs₀
  ...
  rw [show (𝓝 (tensorHeatSemigroup
        (I := I) (M := M) g r s h_uniform (t - s₀) (F s₀))) =
      𝓝 (0 + tensorHeatSemigroup
        (I := I) (M := M) g r s h_uniform (t - s₀) (F s₀)) by rw [zero_add]]
  have h_diff_to_zero :
      Tendsto
        (fun u : ℝ =>
          tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform (t - u) (F u) -
            tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform (t - s₀) (F s₀))
        ...
        Tendsto
          (fun u : ℝ =>
            tensorHeatSemigroup
                (I := I) (M := M) g r s h_uniform (t - u) (F u) -
              tensorHeatSemigroup
                (I := I) (M := M) g r s h_uniform (t - u) (F s₀))
          (𝓝[Set.Icc 0 t] s₀) (𝓝 0) := by
      have h_diff_eq : ∀ u,
          tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform (t - u) (F u) -
            tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform (t - u) (F s₀) =
          tensorHeatSemigroup
            (I := I) (M := M) g r s h_uniform (t - u) (F u - F s₀) := by
        intro u
        rw [← (tensorHeatSemigroup
          (I := I) (M := M) g r s h_uniform (t - u)).map_sub]
    ...
    have h_termB :
          ...
          ∀ᶠ u in 𝓝[Set.Icc 0 t] s₀,
            ‖tensorHeatSemigroup
              ...
              tensorHeatSemigroup
                (I := I) (M := M) g r s h_uniform (t - s₀) (F s₀)‖ ≤
            ‖tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform |s₀ - u| (F s₀) - F s₀‖ := by
        rw [Filter.eventually_iff_exists_mem]
        refine ⟨Set.Icc 0 t, self_mem_nhdsWithin, ?_⟩
        intro u hu
        have h_t_minus_u_nn : 0 ≤ t - u := sub_nonneg.mpr hu.2
        have h := norm_tensorHeatSemigroup_sub_le_tensorHeatSemigroup_diff
          (I := I) (M := M) h_uniform h_t_minus_u_nn h_diff_nn (F s₀)
      ...
      have h_strong :=
        tensorHeatSemigroup_continuous_at_zero
          (I := I) (M := M) h_uniform (F s₀)
      have h_compose :
          Tendsto (fun u : ℝ =>
              tensorHeatSemigroup
                (I := I) (M := M) g r s h_uniform |s₀ - u| (F s₀))
            (𝓝 s₀) (𝓝 (F s₀)) :=
        h_strong.comp h_abs_to_zero_within
      have h_compose_diff :
          Tendsto (fun u : ℝ =>
              tensorHeatSemigroup
                (I := I) (M := M) g r s h_uniform |s₀ - u| (F s₀) - F s₀)
            (𝓝 s₀) (𝓝 0) := by
    ...
    have h_total_eq : ∀ u,
        ...
        (tensorHeatSemigroup
            (I := I) (M := M) g r s h_uniform (t - u) (F s₀) -
          tensorHeatSemigroup
            (I := I) (M := M) g r s h_uniform (t - s₀) (F s₀)) := by
      intro u; abel
    have h_eq_fun :
        (fun u : ℝ =>
          tensorHeatSemigroup
