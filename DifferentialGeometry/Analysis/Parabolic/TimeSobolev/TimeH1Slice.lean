...
/-- The translate of an `L²` time function restricted to a later subinterval. -/
...
/-- The subinterval `L²` representative is the translated original representative almost everywhere. -/
...
/-- Restrict a time-`H¹` path to `[a,b]` and translate its parameter interval to `[0,b-a]`. -/
def slice (u : timeH1 X T) (a b : ℝ) (ha : 0 ≤ a) (hbT : b ≤ T) :
    timeH1 X (b - a) :=
  mk (u.toFun a) (timeL2.slice u.deriv a b ha hbT)

/-- The weak derivative of a time-`H¹` slice is the translated original weak derivative. -/
theorem slice_deriv (u : timeH1 X T) (a b : ℝ) (ha : 0 ≤ a) (hbT : b ≤ T) :
  ...
  simpa only [deriv_mk, add_comm] using ht

/-- The continuous representative of a time-`H¹` slice is the translated original path. -/
