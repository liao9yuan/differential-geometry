...
/-!
# Euler identity for nonlinear time actions

This file combines the curve-dependent quadratic coefficient variation with a
nonlinear potential and derives the weak Euler identity from a genuine
fixed-endpoint local minimizer.
-/
...
/-- A curve-dependent quadratic action plus a nonlinear potential. -/
def timeCoeffPot
...
/-- The spatial force of a differentiated quadratic coefficient has quadratic
...
private theorem coeffQuad_at
  ...
  have hBc : HasDerivAt (fun z ↦ B (xc z)) ((DB (xc c)) w) c := by
    simpa only [xc] using hB.comp_hasDerivAt c hxc
  ...
  have hsym : inner ℝ (B (xc c) q) (pc c) =
    ...
    calc
      ...
      _ = inner ℝ (B (xc c) (pc c)) q := by
        simpa only [xc] using (hself.isSymmetric (pc c) q).symm
  simpa only [xc, pc, inner_add_left, hsym, two_mul, add_assoc, add_comm,
    add_left_comm] using hinner

omit [FiniteDimensional ℝ X] in
/-- Differentiation of a curve-dependent quadratic coefficient along an
affine time-`H¹` line, assuming differentiability and self-adjointness only on
the affine tube actually traversed by that line. -/
...
-- The quantified domination data make this assembly unusually costly to elaborate.
omit [FiniteDimensional ℝ X] in
/-- A fixed-endpoint local minimizer of a curve-dependent quadratic action
plus a nonlinear potential satisfies the weak Euler identity in the exact
`L¹`-force form consumed by `mom_rep_cont_l1`.

For each variation, the coefficient-line hypotheses are required only on a
positive rescaling of its affine tube.  The resulting scaled identity is
divided by that positive factor.  The derivative formula itself is supplied by
`timeCoeff_line_on`, not assumed here. -/
theorem timeCoeff_euler
  ...
  have hFp : Integrable Fp (timeMeasure T) := by
    simpa only [timeMeasure] using hFpCont.integrableOn_Icc
  have hF : Integrable F (timeMeasure T) := hFc.add hFp
  refine ⟨by simpa only [F, Fc, Fp, timeMeasure] using hF, ?_⟩
  intro v hv0 hvT
  let r : ℝ := scale v
  ...
  have henergy : HasDerivAt (timeCoeffPot B V ∘ line)
    ...
    simpa only [Function.comp_apply, timeCoeffPot, line, Fp] using hcoeff.add hpot
  ...
  have hmomPair : IntervalIntegrable
    ...
    simpa only [timeMeasure] using hmomTime
