...
This module approximates the two affine pieces of a time-`H¹` tent separately,
then glues their flat endpoint germs at the prescribed interior node.
-/
...
/-- A global `C¹` representative of a time-`H¹` curve has the curve's weak
derivative almost everywhere whenever it agrees with the continuous
representative on the whole time interval. -/
theorem deriv_ae_of_eqOn {T : ℝ} (hT : 0 < T) (u : timeH1 X T)
...
/-- A time-`H¹` tent is a strong limit of closed-interval `C¹` curves with
exact zero outer endpoints and the exact prescribed value at its node. -/
theorem exists_tent_c1 {T c : ℝ} (hc : 0 < c) (hcT : c < T) (z : X) :
  ...
  have hRsq : ∀ n, ‖(wR n).deriv - uR.deriv‖ ^ 2 =
    ...
    have hpoint : ∀ᵐ t ∂timeMeasure (T - c),
      ...
      rw [hw, hu]
    simpa only [timeMeasure, uIoc_of_le (sub_nonneg.mpr hcT.le),
      restrict_Ioc_eq_restrict_Icc] using hpoint
