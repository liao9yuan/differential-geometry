...
/-!
# Affine endpoint ramps in time H1

This file realizes the two affine endpoint ramps as reusable `timeH1` curves.
-/
...
/-- The affine time-`H¹` ramp from `0` to `z` on a nonnegative interval. -/
...
/-- The affine time-`H¹` ramp from `z` to `0` on a nonnegative interval. -/
...
/-- The continuous representative of the upward ramp is its affine formula. -/
...
/-- The continuous representative of the downward ramp is its affine formula. -/
...
/-- The downward ramp ends at zero. -/
@[simp]
...
/-- The weak derivative of the upward ramp is the constant slope. -/
...
/-- The weak derivative of the downward ramp is the negative constant slope. -/
theorem rampDown_deriv {T : ℝ} (hT : 0 < T) (z : X) :
  ...
  rw [ht]
  simpa only [zero_sub, neg_div, one_div] using
    ((((hasDerivAt_const t T).sub (hasDerivAt_id t)).div_const T).smul_const z).deriv

/-- The upward ramp is linear in its endpoint vector. -/
...
/-- The downward ramp is linear in its endpoint vector. -/
