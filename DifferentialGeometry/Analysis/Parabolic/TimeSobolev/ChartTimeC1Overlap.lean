...
/-- The derivative of a shifted time representative agrees with the derivative
of the original model-space curve on the translated closed interval. -/
...
/-- At one point of a `C¹` manifold curve, chart-coordinate derivatives are
related by the tangent coordinate change.  Only the first chart must contain
the whole represented set; the second chart is needed only at the point. -/
...
/-- The within-derivatives of two `C¹` time-coordinate representatives of the same
manifold curve are related pointwise by the tangent coordinate change. -/
...
/-- On a positive initial subinterval, the derivative of a longer coordinate
representative is the tangent coordinate change of the shorter representative's
derivative. -/
