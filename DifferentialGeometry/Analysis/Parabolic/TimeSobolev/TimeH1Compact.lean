...
/-- The `L¹` norm on a subinterval is controlled by its square-root length
times the ambient time-`L²` norm. -/
...
/-- The continuous representative of a time-`H¹` map has the standard
one-dimensional square-root modulus of continuity. -/
...
private def contRep (u : timeH1 X T) : C(Icc (0 : ℝ) T, X) :=
  ⟨fun t => u.toFun t, u.continuousOn_toFun.restrict⟩
...
/-- A norm-bounded sequence of finite-dimensional-valued time-`H¹` maps has
a common subsequence which converges weakly in time-`H¹` and uniformly in time. -/
theorem compact_subseq (u : ℕ → timeH1 Y T) {C : ℝ} (hu : ∀ n, ‖u n‖ ≤ C) :
  ...
  have hequi : Equicontinuous (fun n => (f n : Icc (0 : ℝ) T → Y)) := by
    ...
    calc
      ...
      _ = C * Real.sqrt (dist a b) := by
        rw [mul_comm]
        congr 2
        simpa [Real.dist_eq, abs_sub_comm] using congrArg Real.sqrt (Subtype.dist_eq a b)
  ...
  have hwmem : ∀ n, w n ∈ WeakDual.toStrongDual ⁻¹'
    ...
    exact hu (phi₁ n)
  letI : SecondCountableTopology Y := inferInstance
  letI : TopologicalSpace.SeparableSpace Y := inferInstance
  letI : IsSeparable (timeMeasure T) := inferInstance
  letI : Fact (1 ≤ (2 : ℝ≥0∞)) := ⟨by norm_num⟩
  letI : Fact ((2 : ℝ≥0∞) ≠ ∞) := ⟨by norm_num⟩
  letI : SecondCountableTopology (timeL2 Y T) := Lp.SecondCountableTopology
  letI : SecondCountableTopology (timeH1 Y T) := by
    unfold timeH1
    exact WithLp.secondCountableTopology 2 Y (timeL2 Y T)
  letI : TopologicalSpace.SeparableSpace (timeH1 Y T) :=
  ...
  have hweak : ∀ z, Tendsto (fun n => inner ℝ (u (phi₁ (phi₂ n))) z) atTop
    ...
    simpa only [Function.comp_apply, w, StrongDual.toWeakDual_apply,
      innerSL_apply_apply, uLim, InnerProductSpace.toDual_symm_apply] using heval
  have hgunif' : TendstoUniformly (fun n => f (phi₁ (phi₂ n))) g atTop := by
    intro V hV
    simpa only [Function.comp_apply] using
      hphi₂.tendsto_atTop.eventually (hgunif V hV)
