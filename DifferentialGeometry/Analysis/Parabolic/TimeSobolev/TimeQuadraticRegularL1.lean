...
/-!
# Momentum regularity with an integrable force

The position derivative of a nonlinear time-quadratic energy naturally has
only time-`L¹` regularity.  This file converts its genuine weak Euler identity
into the corresponding absolutely continuous momentum representative.
-/
...
/-- A weak Euler identity with an integrable force identifies the momentum with
an interval primitive of that force almost everywhere. -/
theorem mom_primitive_l1
    (hT : 0 < T)
    (A : ℝ → X →L[ℝ] X)
    ...
    (u : timeH1 X T) (F : ℝ → X)
    (hF : IntegrableOn F (Icc (0 : ℝ) T) volume)
  ...
  have hFIoo : IntegrableOn F (Ioo (0 : ℝ) T) volume :=
    hF.congr_set_ae Ioo_ae_eq_Icc
  apply weakDeriv_primitive hT hp hFIoo
  ...
  have hscalar :
    ...
    have hpot :
        (∫ t in Icc (0 : ℝ) T, inner ℝ (F t) (v.toFun t)) =
          ∫ t in Ioo (0 : ℝ) T, φ t * inner ℝ (F t) x := by
      rw [show (∫ t in Icc (0 : ℝ) T, inner ℝ (F t) (v.toFun t)) =
          ∫ t in Ioo (0 : ℝ) T, inner ℝ (F t) (v.toFun t) by
        rw [← Measure.restrict_congr_set Ioo_ae_eq_Icc]]
      apply MeasureTheory.integral_congr_ae
      filter_upwards [ae_restrict_mem measurableSet_Ioo] with t ht
      rw [timeH1.toFun_ofContDiffOn hT.le fx hfx.contDiffOn
        ⟨le_of_lt ht.1, le_of_lt ht.2⟩]
      simp only [fx, real_inner_smul_right]
    change 2 * (∫ t in Icc (0 : ℝ) T, inner ℝ (L t) (v.deriv t)) +
      (∫ t in Icc (0 : ℝ) T, inner ℝ (F t) (v.toFun t)) = 0 at he
    rw [hkin, hpot] at he
    calc
      (∫ t in Ioo (0 : ℝ) T,
          inner ℝ ((deriv φ t) • ((2 : ℝ) • A t (u.deriv t))) x)
  ...
  have hright : inner ℝ (-∫ t in Ioo (0 : ℝ) T, φ t • F t) x =
    ...
    have hFφ : Integrable (fun t ↦ φ t • F t)
        (volume.restrict (Ioo (0 : ℝ) T)) :=
      (show Integrable F (volume.restrict (Ioo (0 : ℝ) T)) from hFIoo)
...
/-- The primitive from `mom_primitive_l1` is continuous on the closed time interval. -/
theorem mom_rep_cont_l1
    (hT : 0 < T)
    (A : ℝ → X →L[ℝ] X)
    (hA : AEStronglyMeasurable A (timeMeasure T))
    (C : NNReal)
    ...
    ∃ c : X,
      ...
      ContinuousOn (fun t ↦ c + ∫ r in (0 : ℝ)..t, F r)
        (Icc (0 : ℝ) T) := by
  obtain ⟨c, hc⟩ := mom_primitive_l1 hT A hA C hC u F hF hEuler
