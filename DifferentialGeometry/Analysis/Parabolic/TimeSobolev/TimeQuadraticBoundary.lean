...
/-!
# Endpoint ramps for time-quadratic momentum identities

The product rule and the fundamental theorem of calculus evaluate the weak
Euler pairing against the two affine ramps attached to an interval endpoint.
-/
...
/-- The affine ramp from `0` to `z` evaluates the momentum boundary term at
...
/-- The affine ramp from `z` to `0` evaluates the negative momentum boundary
term at the initial endpoint. -/
