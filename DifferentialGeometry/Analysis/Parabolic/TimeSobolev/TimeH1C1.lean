...
/-!
# Continuous derivative representatives for time H1 curves

A time-`H¹` curve whose weak derivative has a continuous representative is
`C¹` on the closed time interval, with the expected one-sided derivative at
the endpoints.
-/
...
/-- A time-`H¹` curve with a continuous weak-derivative representative is
`C¹` on the closed time interval, and its derivative within that interval is
