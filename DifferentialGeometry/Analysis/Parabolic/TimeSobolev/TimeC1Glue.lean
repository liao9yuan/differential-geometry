...
/-!
# Gluing C1 time curves

This module supplies the one-dimensional closed-interval gluing lemma used
after matching the one-sided derivatives of adjacent time curves.
-/
...
/-- A function that is `C¹` on two positive adjacent closed intervals is `C¹`
on their union when its two one-sided derivatives agree at the common node. -/
