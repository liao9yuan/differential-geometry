...
/-- Extend a curve on `[a, b]` by its endpoint values. -/
def flatExtend {X : Type*} (f : ℝ → X) (a b : ℝ) : ℝ → X :=
  Set.piecewise (Iic a) (fun _ ↦ f a)
    (Set.piecewise (Iic b) f (fun _ ↦ f b))

/-- On its defining interval, `flatExtend` agrees with the original curve. -/
...
/-- A flat extension over a singleton interval is the constant endpoint curve. -/
...
/-- The flat extension maps into any set containing the original interval image. -/
...
/-- Flat extension preserves continuity. -/
theorem flatExtend_cont {X : Type*} [TopologicalSpace X]
  ...
  have htail : Continuous tail := by
    ...
    intro s hs
    have hs' : s = b := by simpa only [frontier_Iic] using hs
    subst s
    rfl
  ...
  intro s hs
  have hs' : s = a := by simpa only [frontier_Iic] using hs
  subst s
  exact htaila.symm

/-- If a smooth curve is locally constant at both endpoints, its flat extension is smooth. -/
...
/-- The flat extension is locally constant at its left endpoint. -/
...
/-- The flat extension is locally constant at its right endpoint. -/
...
/-- Successively glue curves at the prescribed real nodes, starting from a constant curve. -/
def flatJoin {X : Type*} (x : X) (t : ℕ → ℝ) (f : ℕ → ℝ → X) : ℕ → ℝ → X
  | 0 => fun _ ↦ x
  | n + 1 => Set.piecewise (Iic (t n)) (flatJoin x t f n) (f n)

/-- On the right-open part of an inserted segment, the finite join is that segment. -/
...
/-- A finite succession of smooth curves is smooth when consecutive curves have equal germs
at their nondecreasing gluing nodes. -/
...
/-- Lift a coordinate curve through one chart after extending it flat outside its segment. -/
noncomputable def chartFlatLift
    (I : ModelWithCorners ℝ E H) [ChartedSpace H M]
    (p : M) (f : ℝ → E) (a b : ℝ) : ℝ → M :=
  (extChartAt I p).symm ∘ flatExtend f a b

/-- A continuous coordinate curve staying in the chart target has a continuous flat chart lift. -/
...
/-- A `C¹` coordinate curve locally constant at its endpoints has a global `C¹` chart lift. -/
...
/-- Chart coordinates recover the original coordinate curve on the lifted segment. -/
...
/-- A flat chart lift is locally constant at its left endpoint. -/
...
/-- A flat chart lift is locally constant at its right endpoint. -/
