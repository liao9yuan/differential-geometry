...
/-!
# Gluing finitely many C1 manifold-curve pieces

This module iterates `curve_c1_join` across a strictly increasing finite
subdivision.  Repeated nodes are deliberately excluded from this layer.
-/
...
/-- A manifold curve that is `C¹` on every piece of a positive strict finite
subdivision is `C¹` on the full interval when adjacent one-sided derivatives
agree in the chart centered at every internal node. -/
