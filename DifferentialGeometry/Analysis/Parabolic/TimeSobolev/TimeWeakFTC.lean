...
/-!
# A weak fundamental theorem in one time dimension

This file identifies an integrable finite-dimensional curve from its distributional time
derivative.  The statement is independent of the `timeH1` packaging and can therefore be used
when a weak derivative is obtained directly from a variational or density argument.
-/
...
/-- An integrable finite-dimensional curve with distributional derivative `q` agrees almost
everywhere on `(a, b)` with a constant plus the interval primitive of `q`. -/
