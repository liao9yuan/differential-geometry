...
/-!
# Momentum regularity for time-quadratic energies

This file converts the weak Euler identity for a time-quadratic energy into a
distributional derivative statement for its momentum.  In finite dimensions,
the one-dimensional weak fundamental theorem then supplies an absolutely
continuous primitive representative.
-/
...
/-- The momentum in a time-quadratic weak Euler identity agrees almost everywhere with an
interval primitive of the position force. -/
...
/-- The momentum primitive supplied by `mom_primitive` is continuous on the closed time
interval. -/
