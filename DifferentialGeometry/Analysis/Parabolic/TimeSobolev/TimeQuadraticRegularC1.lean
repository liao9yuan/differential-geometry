...
/-!
# C1 momentum representatives for time-quadratic energies

A continuous force upgrades the interval primitive supplied by the weak Euler
identity from an absolutely continuous representative to a closed-interval
`C¹` representative, including the natural one-sided endpoint derivatives.
-/
...
/-- The constant-plus-primitive of a continuous force is `C¹` on the closed
time interval, with derivative within the interval equal to that force. -/
...
/-- A time-quadratic weak Euler identity with continuous force has a `C¹`
momentum representative whose derivative within the closed interval is the
force. -/
theorem mom_rep_c1
    (hT : 0 < T)
    (A : ℝ → X →L[ℝ] X)
