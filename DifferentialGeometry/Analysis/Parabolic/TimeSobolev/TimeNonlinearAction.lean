...
This file differentiates a genuine curve-dependent potential density along
affine time-`H¹` variations.  It then combines that calculation with the
time-quadratic kinetic term to obtain an `L¹` weak Euler identity from an
actual fixed-endpoint local minimizer.
-/
...
/-- The integral of a curve-dependent potential density. -/
def timeNlinPot (V : ℝ → X → ℝ) (u : timeH1 X T) : ℝ :=
  ∫ t in (0 : ℝ)..T, V t (u.toFun t)

/-- A fixed time-quadratic kinetic term plus a nonlinear integral potential. -/
def timeNlinAction
    (A : ℝ → X →L[ℝ] X)
...
/-- The Riesz representative of the spatial derivative of a quadratic
coefficient at a fixed velocity. -/
...
/-- Evaluation of the spatial force represented by `coeffForce`. -/
...
/-- The curve-dependent quadratic action associated to `B`. -/
def timeCoeffAction
    (B : ℝ → X → X →L[ℝ] X) (u : timeH1 X T) : ℝ :=
  ∫ t in (0 : ℝ)..T,
    inner ℝ (B t (u.toFun t) (u.deriv t)) (u.deriv t)

/-- The integrand of `timeCoeffAction` along an affine variation. -/
def coeffLineLag
...
/-- The actual line derivative of `coeffLineLag`: its first term is the
spatial force pairing and its second term is the momentum pairing. -/
def coeffLineDeriv
    (B : ℝ → X → X →L[ℝ] X)
...
/-- Pointwise first variation of a genuinely position-dependent quadratic
coefficient along simultaneous affine position and velocity lines.  The first
term is the spatial coefficient derivative; the second is the symmetric
velocity derivative. -/
...
/-- Differentiation under the time integral for a curve-dependent quadratic
coefficient.  The analytic hypotheses are precisely measurability and a common
integrable domination; the derivative formula itself is proved from `DB` and
self-adjointness rather than assumed. -/
...
/-- Differentiation of the nonlinear potential along an affine time-`H¹`
line.  `G t x` is the pointwise gradient of `V t` at `x`; joint continuity
supplies the domination needed to differentiate under the integral. -/
theorem timeNlinPot_line
  ...
  have hslice (c t : ℝ) : HasDerivAt (fun z : ℝ ↦ lag z t) (dLag c t) c := by
    ...
    simpa only [lag, dLag, innerSL_apply_apply] using
  ...
  convert hparam.2.congr_of_eventuallyEq
    (Eventually.of_forall fun c ↦ hlineEq c) using 1
  simp only [dLag, zero_smul, add_zero]

/-- A fixed-endpoint local minimizer of the nonlinear integral action satisfies
the weak Euler identity with its actual pointwise `L¹` force. -/
theorem timeNlin_euler
  ...
  have henergy : HasDerivAt (timeNlinAction A hA C hC V ∘ line)
      (2 * inner ℝ (timeOp A hA C hC u.deriv) v.deriv +
        ∫ t in (0 : ℝ)..T,
          inner ℝ (G t (u.toFun t)) (v.toFun t)) 0 := by
    simpa only [Function.comp_apply, timeNlinAction, line] using hkin.add hpot
