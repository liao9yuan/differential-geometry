...
/-!
# Gluing C1 manifold curves

This module lifts the closed-interval `C¹` gluing lemma from model spaces to
curves in a manifold.  Only a node-centered chart is used, so the two full
pieces need not lie in a common chart.
-/
...
/-- Two `C¹` pieces of the same manifold curve glue across a node when their
one-sided derivatives agree in the chart centered at that node. -/
