...
/-- A manifold curve represented by the continuous representative of a fixed-chart
time-`H¹` path is continuous on the represented subinterval. -/
...
/-- A manifold curve with a shifted fixed-chart time-`H¹` representative is
continuous on the original subinterval. -/
...
/-- A curve represented by a fixed-chart time-`H¹` path is manifold differentiable
almost everywhere in the interior of its represented interval. -/
...
/-- Weak derivatives of two time-`H¹` coordinate representatives of the same manifold
curve are related by the tangent coordinate change almost everywhere. -/
...
/-- For a `C¹` curve contained in two charts, the weak derivatives of its two canonical
time-`H¹` realizations satisfy the tangent coordinate-change formula almost everywhere. -/
