...
private theorem timeOp_sub
  ...
  exact calc
    ...
    _ = (A t - B t) (f t) :=
      (ContinuousLinearMap.sub_apply (A t) (B t) (f t)).symm
    _ = (timeOp (fun t ↦ A t - B t) (hA.sub hB) C hC f : ℝ → Y) t := hDf.symm

/-- Multiplication by essentially bounded, uniformly convergent operator-valued
coefficients preserves weak convergence in time `L²`. -/
theorem timeOp_weak_lim
  ...
  simpa only [Ln, L, ContinuousLinearMap.sub_apply, inner_sub_left,
    sub_add_cancel, zero_add] using
    herr_inner.add hfixed

/-- Multiplication by essentially bounded operator families preserves weak convergence when
the coefficients converge uniformly in essential operator norm, without a chosen error rate. -/
        ...
        ((hA n).sub hA_lim) δnn hn by
          simpa only [Ln, L] using
            timeOp_sub (A n) A_lim (hA n) hA_lim (C n) C_lim δnn
              (hC n) hC_lim hn]
      simpa only [δnn, NNReal.coe_mk] using timeOp_norm_le _ _ δnn hn
    have hmul : δ * D < e := by
      dsimp only [δ]
