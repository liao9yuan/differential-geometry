...
theorem heatSolution_log_evolution
    ...
    deriv (fun s => Real.log (u s x)) t =
      Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) x +
      g.inner x
  ...
  calc
    ...
    _ = (u t x)⁻¹ * Δ_g (I := I) g ut.smooth x := by
      rw [hpde.deriv]
      rfl
...
theorem liYauQuantity_eq_neg_laplacian_log
    ...
    g.inner x
      ...
      -Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) x := by
...
theorem liYauQuantity_evolution_identity
  ...
  have hqid : ∀ (τ : ℝ) (hτ : τ ∈ D.regular) (y : M),
      q τ y = -Δ_g (I := I) g (hlogslice τ (D.regular_subset hτ)) y :=
  ...
  have hq_chart_eq : (fun s : ℝ => q s x) =ᶠ[𝓝 t]
    ...
    have hvw : Δ_g (I := I) g (hlogslice s (D.regular_subset hs)) x =
  ...
  have hdq : deriv (fun s : ℝ => q s x) t =
    ...
    hdq_has.deriv
  have hdq' : deriv (fun s : ℝ => q s x) t = -Δ_g (I := I) g hftslice x := by
    have hchart_eq : chartVossWeylLaplacian (I := I) g α
        (fun w : M => deriv (fun s : ℝ => f s w) t) x = Δ_g (I := I) g hftslice x := by
      exact (voss_weyl_laplacian_formula_pointwise (I := I) g α hftslice hxsrc).symm
    exact hdq.trans (congrArg Neg.neg hchart_eq)
  have hdq_laplacian : Δ_g (I := I) g (hqslice t ht) x =
      -Δ_g (I := I) g (Δ_g_contMDiff (I := I) g (hlogslice t (D.regular_subset ht))) x := by
    have heq : (fun y : M => liYauQuantity g f t y) =ᶠ[𝓝 x]
        (fun y : M => -Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) y) := by
      rw [Filter.eventuallyEq_iff_exists_mem]
      refine ⟨Set.univ, Filter.univ_mem, ?_⟩
      intro y hy
      exact hqid t ht y
    have hneg : ContMDiff I 𝓘(ℝ, ℝ) ∞
        (fun y : M => -Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) y) :=
      ContMDiff.neg (Δ_g_contMDiff (I := I) g (hlogslice t (D.regular_subset ht)))
    have hcongr := Δ_g_congr_of_eventuallyEq (I := I) g
      (hqslice t ht) hneg heq
    rw [hcongr]
    exact Δ_g_neg (I := I) g (Δ_g_contMDiff (I := I) g (hlogslice t (D.regular_subset ht))) (x := x)
  have hheq : ∀ y, deriv (fun s : ℝ => f s y) t -
      Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) y =
  ...
  have hmain : deriv (fun s : ℝ => q s x) t -
    ...
    have hsub_fun : (fun y : M => deriv (fun s : ℝ => f s y) t -
          Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) y) =ᶠ[𝓝 x]
    ...
    have hΔsub : Δ_g (I := I) g
        (hftslice.sub (Δ_g_contMDiff (I := I) g (hlogslice t (D.regular_subset ht)))) x =
        Δ_g (I := I) g hftslice x -
          Δ_g (I := I) g (Δ_g_contMDiff (I := I) g (hlogslice t (D.regular_subset ht))) x := by
      have h1 := Δ_g_add (I := I) g hftslice
        (ContMDiff.neg (Δ_g_contMDiff (I := I) g (hlogslice t (D.regular_subset ht)))) x
      have h2 := Δ_g_neg (I := I) g (Δ_g_contMDiff (I := I) g (hlogslice t (D.regular_subset ht))) (x := x)
      have hsub_eq : (fun y : M => deriv (fun s : ℝ => f s y) t -
            Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) y) =ᶠ[𝓝 x]
          (fun y : M => deriv (fun s : ℝ => f s y) t +
            -(Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) y)) := by
        rw [Filter.eventuallyEq_iff_exists_mem]
        refine ⟨Set.univ, Filter.univ_mem, ?_⟩
        intro y hy
        ring
      have hbridge := Δ_g_congr_of_eventuallyEq (I := I) g
        (hftslice.sub (Δ_g_contMDiff (I := I) g (hlogslice t (D.regular_subset ht))))
        (hftslice.add (ContMDiff.neg (Δ_g_contMDiff (I := I) g (hlogslice t (D.regular_subset ht)))))
        hsub_eq
      rw [hbridge, h1, h2]
      ring
    ...
    have hstep : Δ_g (I := I) g hftslice x -
        Δ_g (I := I) g (Δ_g_contMDiff (I := I) g (hlogslice t (D.regular_subset ht))) x =
        Δ_g (I := I) g (normGradSqFun_contMDiff (I := I) g
          (hlogslice t (D.regular_subset ht))) x :=
  ...
  have hgrad : 2 * g.inner x
      ...
      2 * g.inner x (gradFun (I := I) g (f t) x)
         (gradFun (I := I) g (fun y => -Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) y) x) := by
    simp [f, q, hqid t ht]
  have hmain' : deriv (fun s : ℝ => q s x) t -
      Δ_g (I := I) g (hqslice t ht) x =
      -2 * chartHessFrobeniusSq (I := I) g (fun y : M => Real.log (u t y)) x -
        ...
        2 * g.inner x (gradFun (I := I) g (fun y : M => Real.log (u t y)) x)
          (gradFun (I := I) g (Δ_g (I := I) g (hlogslice t (D.regular_subset ht))) x) := by
    rw [hmain, hbochner]
    ring
  ...
  have hinner_eq : 2 * g.inner x (gradFun (I := I) g (f t) x)
        (gradFun (I := I) g (fun y : M => -Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) y) x) =
      -2 * g.inner x (gradFun (I := I) g (fun y : M => Real.log (u t y)) x)
        (gradFun (I := I) g (Δ_g (I := I) g (hlogslice t (D.regular_subset ht))) x) := by
    ...
    have hgradneg : gradFun (I := I) g
        (fun y : M => -Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) y) x =
        -gradFun (I := I) g (Δ_g (I := I) g (hlogslice t (D.regular_subset ht))) x := by
      have hneg : gradientFun (I := I) g
          (fun y : M => -Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) y) x =
          -gradientFun (I := I) g (Δ_g (I := I) g (hlogslice t (D.regular_subset ht))) x :=
        gradientFun_neg g
          ((Δ_g_contMDiff (I := I) g (hlogslice t (D.regular_subset ht))).mdifferentiableAt (by simp))
      calc
        gradFun (I := I) g
            (fun y : M => -Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) y) x
            = gradientFun (I := I) g
                (fun y : M => -Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) y) x :=
              (hvecg (fun y : M => -Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) y)).symm
        _ = -gradientFun (I := I) g (Δ_g (I := I) g (hlogslice t (D.regular_subset ht))) x := hneg
        _ = -gradFun (I := I) g (Δ_g (I := I) g (hlogslice t (D.regular_subset ht))) x := by
              rw [hvecg (Δ_g (I := I) g (hlogslice t (D.regular_subset ht)))]
    rw [hfun, hgradneg]
    rw [map_neg]
    ring
  rw [hinner_eq]
...
theorem liYauQuantity_evolution_inequality
  ...
  have htrace' : -2 * chartHessFrobeniusSq (I := I) g logut x ≤
    ...
    have hstep : -2 * chartHessFrobeniusSq (I := I) g logut x ≤
        -2 * ((Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) x)^2 /
          (Module.finrank ℝ E : ℝ)) := by
      nlinarith [hdiv]
    have hring : -2 * ((Δ_g (I := I) g (hlogslice t (D.regular_subset ht)) x)^2 /
...
theorem liYau_estimate_of_nonnegative_ricci_on
  ...
  have hevol : ∀ (τ : ℝ) (hτ : τ ∈ D.regular) (y : M),
      deriv (fun s => q s y) τ - Δ_g (I := I) g (hqslice τ hτ) y ≤
  ...
  have hH_nonpos : ∀ eps : ℝ, 0 < eps → H eps t x ≤ 0 := by
    ...
    have hdq_Δq : deriv (fun τ' : ℝ => q τ' x₀) s - laplacianAt (I := I) G s (q s) x₀ ≤
        -(2 / n) * (q s x₀)^2 := by
      have hlapeq : laplacianAt (I := I) G s (q s) x₀ = Δ_g (I := I) g (hqslice s hsreg) x₀ := by
      ...
      have hcanc : 2 * g.inner x₀ (gradientFun (I := I) g (f s) x₀) (gradientFun (I := I) g (q s) x₀) = 0 := by
        rw [hgradq']
        ring
      have hev0' : deriv (fun τ' : ℝ => q τ' x₀) s - Δ_g (I := I) g (hqslice s hsreg) x₀ ≤
          -(2 / n) * (q s x₀)^2 := by
...
theorem liYau_estimate_of_nonnegative_ricci
  ...
  have hqcd : ContMDiffOn (𝓘(ℝ, ℝ).prod I) 𝓘(ℝ, ℝ) ∞
    ...
    change ContMDiffOn (𝓘(ℝ, ℝ).prod I) 𝓘(ℝ, ℝ) ∞
      (fun p : ℝ × M => liYauQuantity g (fun τ y => Real.log (u τ y)) p.1 p.2)
      (D.regular ×ˢ univ)
    simpa [D] using liYauQuantity_contMDiff (I := I) (M := M) (D := D) g u hu.contMDiffOn
  ...
  simpa [D] using liYau_estimate_of_nonnegative_ricci_on (I := I) (M := M) g hRic D G
    hGmetric hGconn u huOn (fun τ hτ x => hpos τ x) (t := t)
    (by change t ∈ (Set.univ : Set ℝ); trivial)
    ht (by intro τ hτ; trivial) (by intro τ hτ; trivial) hqCont x
...
theorem heat_solution_differential_harnack_of_nonnegative_ricci_on
  ...
  classical
  have hly := liYau_estimate_of_nonnegative_ricci_on (I := I) (M := M) g hRic D G
    hGmetric hGconn u hu hpos ht ht0 hslabCarrier hslabRegular hqCont x
