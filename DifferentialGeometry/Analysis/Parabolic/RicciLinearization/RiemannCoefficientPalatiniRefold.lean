...
set_option linter.unusedVariables false in
set_option linter.unusedSectionVars false in
private lemma bdToModel_cons_sum_smul (x : M) {n : ℕ}
...
set_option linter.unusedVariables false in
set_option linter.unusedSectionVars false in
private lemma bdToModel_cons_cons_sum_smul (x : M) {n : ℕ}
...
set_option linter.unusedSectionVars false in
private lemma bdOrthoFrame_center_repr (g : SmoothRiemannianMetric I M) (x : M)
    (v : TangentSpace I x) :
...
private lemma bdPureDT_eq_trace_fullRaised (g₀ g₁ : SmoothRiemannianMetric I M)
  ...
  have hLHS : Tensor0SSpace.toModel
      ((show Tensor0SSpace (s + 2) I x →L[ℝ] Tensor0SSpace s I x from
        (bdPureDT (I := I) (M := M) g₀ g₁ s).toSection x) Z) mm =
    ...
    rw [show ((show Tensor0SSpace (s + 2) I x →L[ℝ] Tensor0SSpace s I x from
        (bdPureDT (I := I) (M := M) g₀ g₁ s).toSection x) Z) =
...
private def bdPairTraceOp (g₀ g₁ : SmoothRiemannianMetric I M) : SmoothCcTensor g₀ 6 2 :=
...
set_option linter.unusedSectionVars false in
set_option maxHeartbeats 12800000 in
private lemma bdPairTraceOp_apply_toModel (g₀ gm : SmoothRiemannianMetric I M)
    (X : SmoothCcTensor g₀ 0 4) (x : M) (D : Tensor0SSpace 2 I x) (v : Fin 2 → E) :
    Tensor0SSpace.toModel
        ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
          (appCcRS (I := I) (M := M) g₀ 2 6 2 (bdPairTraceOp (I := I) (M := M) g₀ gm)
...
private lemma bdFullRaised_diff_split (g₀ g₁ : SmoothRiemannianMetric I M) :
  ...
  rw [show (gInvDiffRaisedEndoField (I := I) g₀ g₁ x) =
      DifferentialGeometry.Analysis.Sobolev.TensorHilbert.gInvDiffRaisedEndo (I := I) g₀ g₁ x
    from rfl]
...
set_option linter.unusedVariables false in
set_option linter.unusedSectionVars false in
private theorem bdSharpFlat_tgrid (g₀ : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ S : ℕ → ℝ, (∀ l, 0 ≤ S l) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
        (htie : ∀ (y : M) (v w : TangentSpace I y),
...
set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
private lemma bdSlotInsertZero_fullRaisedRev_eq_omRecover
...
set_option linter.unusedSectionVars false in
private lemma bdRfns_symmS_zero_le (g₀ : SmoothRiemannianMetric I M)
...
set_option linter.unusedVariables false in
set_option linter.unusedSectionVars false in
private theorem bdOmRecover_gridWindow (g₀ : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ C : ℕ → ℝ, (∀ l, 0 ≤ C l) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
        (htie : ∀ (y : M) (v w : TangentSpace I y),
  ...
  have hB : riemannianFiberNormSq (I := I) (M := M) g₀ 1 (1 + l) x
      ...
      (Module.finrank ℝ E : ℝ) ^ 2 * (max δ₀ 0) ^ 2 +
        Combinatorics.antidiagonalTupleGridWindow b (l + 1) := by
    rw [rfns_iteratedCovGrad_cometricRaiseSlot0Field_eq (I := I) (M := M) g₀ 0
      (symmS (I := I) (M := M) g₀ T) l x]
    match l with
    | 0 =>
        have h1 := bdRfns_symmS_zero_le (I := I) (M := M) g₀ T hδ0 hbound x
...
private lemma bdConnDiffSection_eq_armSlotEndoCc_zero (g₀ g₁ : SmoothRiemannianMetric I M) :
  ...
  rw [show ((show Tensor0SSpace 1 I x →L[ℝ] Tensor0SSpace 2 I x from
        (armSlotEndoCc (I := I) (M := M) g₀ 0
          (bdConnPair (I := I) (M := M) g₀ g₁)).toSection x) om) =
      armSlotFib (I := I) (M := M) 0 x (bdConnPair (I := I) (M := M) g₀ g₁ x) om
      from rfl]
  rw [armSlotFib_apply_eval (I := I) (M := M) 0 x
    (bdConnPair (I := I) (M := M) g₀ g₁ x) om v]
  rw [slotInsertEndoFib_apply_eval]
  rw [show (Function.update (Matrix.vecTail (fun k : Fin 2 => (v k : E))) 0
        (bdConnPair (I := I) (M := M) g₀ g₁ x (v 0)
...
private def bdTau4 : Equiv.Perm (Fin 4) :=
...
private lemma bdArmSlotEndoCc_two_eq_reindex_slotExtend (g₀ : SmoothRiemannianMetric I M)
    ...
    armSlotEndoCc (I := I) (M := M) g₀ 2 Arm =
      reindexCoeffGen (I := I) (M := M) g₀ 3 4
        (rsDomDomCongrSection (I := I) (M := M) g₀ 3 4 bdTau4
  ...
  apply ContinuousMultilinearMap.ext
  intro w
  have hτ0 : bdTau4 (0 : Fin 4) = (2 : Fin 4) := by decide
  have hτ1 : bdTau4 (1 : Fin 4) = (0 : Fin 4) := by decide
  have hτ2 : bdTau4 (2 : Fin 4) = (1 : Fin 4) := by decide
  have hτ3 : bdTau4 (3 : Fin 4) = (3 : Fin 4) := by decide
  ...
  have hLHS : Tensor0SSpace.toModel
    ...
    rw [show ((show Tensor0SSpace 3 I x →L[ℝ] Tensor0SSpace 4 I x from
        ...
        armSlotFib (I := I) (M := M) 2 x (Arm x) D from rfl]
      ...
      Tensor0SSpace.toModel
        ((show Tensor0SSpace 3 I x →L[ℝ] Tensor0SSpace 4 I x from
          (slotExtend (I := I) (M := M) g₀ 2 3
            (armSlotEndoCc (I := I) (M := M) g₀ 1 Arm)).toSection x) D')
        (fun i => w (bdTau4 i)) := by
    rw [toModel_rsDomDomCongr_apply (I := I) (M := M) bdTau4
      ((slotExtend (I := I) (M := M) g₀ 2 3
        (armSlotEndoCc (I := I) (M := M) g₀ 1 Arm)).toSection x) D']
    rw [ContinuousMultilinearMap.domDomCongr_apply]
  have e3 : Tensor0SSpace.toModel
      ...
      Tensor0SSpace.toModel
        (armSlotFib (I := I) (M := M) 1 x (Arm x)
          (tensor0S_curry (I := I) (M := M) (𝕜 := ℝ) 2 x D' (w (bdTau4 0))))
        (Matrix.vecTail (fun i => w (bdTau4 i))) := by
    rw [show ((show Tensor0SSpace 3 I x →L[ℝ] Tensor0SSpace 4 I x from
          (slotExtend (I := I) (M := M) g₀ 2 3
            (armSlotEndoCc (I := I) (M := M) g₀ 1 Arm)).toSection x) D') =
    ...
    rw [show (fun i => w (bdTau4 i)) =
        Fin.cons (w (bdTau4 0))
          (Matrix.vecTail (fun i => w (bdTau4 i))) from by
      funext k
      refine Fin.cases rfl (fun j => rfl) k]
    have hkey := TensorMultilinear.tensor0S_curry_apply_eval (I := I) (M := M) (n := 3)
      (T := (tensor0S_curry (I := I) (M := M) (𝕜 := ℝ) 3 x).symm
        ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 3 I x from
          ...
          (tensor0S_curry (I := I) (M := M) (𝕜 := ℝ) 2 x D')))
      (v0 := w (bdTau4 0))
      (vs := Matrix.vecTail (fun i => w (bdTau4 i)))
    rw [ContinuousLinearEquiv.apply_symm_apply] at hkey
    rw [← hkey]
    rfl
  have e4 : Tensor0SSpace.toModel
      (armSlotFib (I := I) (M := M) 1 x (Arm x)
        (tensor0S_curry (I := I) (M := M) (𝕜 := ℝ) 2 x D' (w (bdTau4 0))))
      (Matrix.vecTail (fun i => w (bdTau4 i))) =
      Tensor0SSpace.toModel
        (tensor0S_curry (I := I) (M := M) (𝕜 := ℝ) 2 x D' (w (bdTau4 0)))
        (Function.update (Matrix.vecTail (Matrix.vecTail (fun i => w (bdTau4 i)))) 0
          (Arm x (w (bdTau4 1)) (w (bdTau4 2)))) := by
  ...
  have e5 : Tensor0SSpace.toModel
    ...
    TensorMultilinear.tensor0S_curry_apply_eval (I := I) (M := M) (n := 2)
      (T := D') (v0 := w (bdTau4 0))
      (vs := Function.update (Matrix.vecTail (Matrix.vecTail (fun i => w (bdTau4 i)))) 0
        (Arm x (w (bdTau4 1)) (w (bdTau4 2))))
  have e6 : Tensor0SSpace.toModel D'
      (Fin.cons (w (bdTau4 0))
        (Function.update (Matrix.vecTail (Matrix.vecTail (fun i => w (bdTau4 i)))) 0
    ...
    funext k
    fin_cases k
    · show ((Arm x) (w (bdTau4 1))) (w (bdTau4 2)) =
...
set_option linter.unusedSectionVars false in
private lemma bdArmSlot2_rfns_le (g₀ g₁ : SmoothRiemannianMetric I M) (j : ℕ) (x : M) :
    riemannianFiberNormSq (I := I) (M := M) g₀ 3 (4 + j) x
        ((iteratedCovGrad (I := I) g₀ 3 4 j
          (armSlotEndoCc (I := I) (M := M) g₀ 2
            (bdConnPair (I := I) (M := M) g₀ g₁))).toSection x) ≤
  ...
  rw [rfns_iteratedCovGrad_rsDomDomCongr_both_eq (I := I) (M := M) g₀ 3 4
    ...
    (slotExtend (I := I) (M := M) g₀ 2 3
      (armSlotEndoCc (I := I) (M := M) g₀ 1 (bdConnPair (I := I) (M := M) g₀ g₁))) j x]
  refine le_trans (rfns_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 2 3
    (armSlotEndoCc (I := I) (M := M) g₀ 1 (bdConnPair (I := I) (M := M) g₀ g₁)) j x) ?_
  ...
  rw [rfns_iteratedCovGrad_rsDomDomCongr_both_eq (I := I) (M := M) g₀ 2 3
    ...
    (slotExtend (I := I) (M := M) g₀ 1 2
      (armSlotEndoCc (I := I) (M := M) g₀ 0 (bdConnPair (I := I) (M := M) g₀ g₁))) j x]
...
private noncomputable def bdKernelCLM (gA gB : SmoothRiemannianMetric I M) (x : M) :
  ...
  haveI : FiniteDimensional ℝ (TangentSpace I x) := inferInstanceAs (FiniteDimensional ℝ E)
  LinearMap.toContinuousLinearMap
    { toFun := fun v0 => dLaCovKernel (I := I) gA gB x v0
...
set_option linter.unusedSectionVars false in
private lemma bdKernelCLM_apply (gA gB : SmoothRiemannianMetric I M) (x : M)
    (v0 p q : TangentSpace I x) :
    bdKernelCLM (I := I) (M := M) gA gB x v0 p q =
      dLaCovKernel (I := I) gA gB x v0 p q := by
...
set_option linter.unusedSectionVars false in
private lemma bdFixLoweredCovec_apply (g₀ gA gB : SmoothRiemannianMetric I M) (x : M)
    (m : Fin 4 → TangentSpace I x) :
    bdFixLoweredCovec (I := I) g₀ gA gB x m =
      g₀.inner x (dLaCovKernel (I := I) gA gB x (m 1) (m 2) (m 3)) (m 0) := by
  change g₀.inner x (bdKernelCLM (I := I) (M := M) gA gB x (m 1) (m 2) (m 3)) (m 0) = _
  rw [bdKernelCLM_apply]

set_option linter.unusedSectionVars false in
private lemma bdFixLoweredScalar_global (g₀ gA gB : SmoothRiemannianMetric I M)
    ...
    ContMDiff I 𝓘(ℝ, ℝ) ∞
      (fun x : M => g₀.inner x
        (dLaCovKernel (I := I) gA gB x (V0 x) (p x) (q x)) (W x)) := by
  classical
  have hAsec := deTurckLieCovDerivA_section_contMDiff (I := I) gA gB V0 p q hV0 hp hq
  have hcongr : (fun x : M => g₀.inner x
        (dLaCovKernel (I := I) gA gB x (V0 x) (p x) (q x)) (W x)) =
      (fun x : M => g₀.inner x (deTurckLieCovDerivA (I := I) gA gB V0 p q x) (W x)) := by
    funext x
  ...
  exact contMDiff_g_inner_of_smooth_sections (I := I) g₀
    ⟨fun b => deTurckLieCovDerivA (I := I) gA gB V0 p q b, hAsec⟩ ⟨fun b => W b, hW⟩

set_option linter.unusedSectionVars false in
private lemma bdFixLoweredScalar_contMDiffAt (g₀ gA gB : SmoothRiemannianMetric I M)
    (V0 V1 V2 V3 : Cₛ^∞⟮I; E, (TangentSpace I : M → Type _)⟯) (x₀ : M) :
    ContMDiffAt I 𝓘(ℝ, ℝ) ∞
      (fun x : M =>
        g₀.inner x (dLaCovKernel (I := I) gA gB x (V1 x) (V2 x) (V3 x)) (V0 x)) x₀ := by
...
set_option backward.isDefEq.respectTransparency false in
private theorem bdFixLoweredCovec_section_contMDiff (g₀ gA gB : SmoothRiemannianMetric I M) :
  ...
  have hscalar : ContMDiffAt I 𝓘(ℝ, ℝ) ∞
      (fun x : M => g₀.inner x
        (dLaCovKernel (I := I) gA gB x (Y (σ 1) x) (Y (σ 2) x) (Y (σ 3) x))
        (Y (σ 0) x)) x₀ :=
...
private def bdFixLoweredField (g₀ gA gB : SmoothRiemannianMetric I M) :
...
private def bdFixLoweredCc (g₀ gA gB : SmoothRiemannianMetric I M) :
...
private def bdQuadLowCc (g₀ gArm gOut : SmoothRiemannianMetric I M) :
    SmoothCcTensor g₀ 0 4 :=
  appCcRS (I := I) (M := M) g₀ 0 3 4
    (armSlotEndoCc (I := I) (M := M) g₀ 2 (bdConnPair (I := I) (M := M) g₀ gArm))
...
private lemma bdQuadLowCc_unitModel_apply (g₀ gArm gOut : SmoothRiemannianMetric I M)
    ...
    unitModel (I := I) (M := M) g₀ 4 (bdQuadLowCc (I := I) (M := M) g₀ gArm gOut) x m =
      g₀.inner x
        (PDE.DeTurck.connDiff (I := I) gOut g₀ x
          (PDE.DeTurck.connDiff (I := I) gArm g₀ x (m 0) (m 1)) (m 2)) (m 3) := by
  rw [unitModel]
  rw [show (bdQuadLowCc (I := I) (M := M) g₀ gArm gOut).toSection x
        (unitTensor (I := I) (M := M) x) =
      armSlotFib (I := I) (M := M) 2 x (bdConnPair (I := I) (M := M) g₀ gArm x)
        ((show Tensor0SSpace 0 I x →L[ℝ] Tensor0SSpace 3 I x from
          (connDiffLoweredCc (I := I) g₀ gOut).toSection x)
          (unitTensor (I := I) (M := M) x)) from by
    rw [bdQuadLowCc, appCcRS_toSection]
    rfl]
  rw [armSlotFib_apply_eval (I := I) (M := M) 2 x
    (bdConnPair (I := I) (M := M) g₀ gArm x)
    ((show Tensor0SSpace 0 I x →L[ℝ] Tensor0SSpace 3 I x from
      (connDiffLoweredCc (I := I) g₀ gOut).toSection x)
  ...
  rw [show (Function.update (Matrix.vecTail m) 0
        (bdConnPair (I := I) (M := M) g₀ gArm x (m 0) (Matrix.vecTail m 0))) =
      (![(show TangentSpace I x from
          PDE.DeTurck.connDiff (I := I) gArm g₀ x (m 0) (m 1)), m 2, m 3] :
        Fin 3 → TangentSpace I x) from by
    funext k
    fin_cases k
    · show Function.update (Matrix.vecTail m) 0
          (bdConnPair (I := I) (M := M) g₀ gArm x (m 0) (Matrix.vecTail m 0)) (0 : Fin 3) =
        (show TangentSpace I x from PDE.DeTurck.connDiff (I := I) gArm g₀ x (m 0) (m 1))
      rw [Function.update_self]
      rfl
    · show Function.update (Matrix.vecTail m) 0
          (bdConnPair (I := I) (M := M) g₀ gArm x (m 0) (Matrix.vecTail m 0)) (1 : Fin 3) =
        m 2
      rw [Function.update_of_ne (by decide : (1 : Fin 3) ≠ 0)]
      rfl
    · show Function.update (Matrix.vecTail m) 0
          (bdConnPair (I := I) (M := M) g₀ gArm x (m 0) (Matrix.vecTail m 0)) (2 : Fin 3) =
        m 3
...
set_option linter.unusedSectionVars false in
private lemma bdConnDiff_self_apply (g₀ : SmoothRiemannianMetric I M) (x : M)
...
set_option linter.unusedSectionVars false in
private lemma bdKd_vector (g₀ g₁ g_bg : SmoothRiemannianMetric I M) (x : M)
    (v0 p q : TangentSpace I x) :
    dLaCovKernel (I := I) g₁ g_bg x v0 p q - dLaCovKernel (I := I) g₁ g₀ x v0 p q =
      -dLaCovKernel (I := I) g_bg g₀ x v0 p q
...
private def bdLow0 (g₀ g₁ g_bg : SmoothRiemannianMetric I M) : SmoothCcTensor g₀ 0 4 :=
  (-1 : ℝ) • bdFixLoweredCc (I := I) (M := M) g₀ g_bg g₀
    ...
    - domDomCongrSection (I := I) g₀ bdSigma3 (bdQuadLowCc (I := I) (M := M) g₀ g_bg g_bg)
    - domDomCongrSection (I := I) g₀ bdSigma4 (bdQuadLowCc (I := I) (M := M) g₀ g_bg g_bg)
...
private lemma bdLow0_unitModel_apply (g₀ g₁ g_bg : SmoothRiemannianMetric I M)
    (x : M) (m : Fin 4 → TangentSpace I x) :
    unitModel (I := I) (M := M) g₀ 4 (bdLow0 (I := I) (M := M) g₀ g₁ g_bg) x m =
      g₀.inner x
        (dLaCovKernel (I := I) g₁ g_bg x (m 1) (m 2) (m 3) -
          dLaCovKernel (I := I) g₁ g₀ x (m 1) (m 2) (m 3)) (m 0) := by
  classical
  rw [bdLow0]
  ...
  rw [bdKd_vector (I := I) (M := M) g₀ g₁ g_bg x (m 1) (m 2) (m 3)]
...
private def bdXdHalf (g₀ g₁ g_bg : SmoothRiemannianMetric I M) : SmoothCcTensor g₀ 0 4 :=
  appCcRS (I := I) (M := M) g₀ 0 4 4
    (slotInsertEndoCc (I := I) (M := M) g₀ 3 (fullRaisedEndoField (I := I) (M := M) g₁ g₀))
    (bdLow0 (I := I) (M := M) g₀ g₁ g_bg)

private def bdXd (g₀ g₁ g_bg : SmoothRiemannianMetric I M) : SmoothCcTensor g₀ 0 4 :=
...
open DifferentialGeometry.Analysis.Sobolev.TensorHilbert (gInvRaisedEndo gInvRaisedEndo_apply
  inverseMetricSharpFib_g0FlatCLM cotangentToDual_g0FlatCLM g0FlatCLM) in
set_option linter.unusedSectionVars false in
private lemma bdG0_inner_lambda (g₀ g₁ : SmoothRiemannianMetric I M) (x : M)
    (u v : TangentSpace I x) :
    g₀.inner x u (gInvRaisedEndo (I := I) g₁ g₀ x v) = g₁.inner x u v := by
  rw [gInvRaisedEndo_apply]
...
private lemma bdXdHalf_unitModel_apply (g₀ g₁ g_bg : SmoothRiemannianMetric I M)
    (x : M) (m : Fin 4 → TangentSpace I x) :
    unitModel (I := I) (M := M) g₀ 4 (bdXdHalf (I := I) (M := M) g₀ g₁ g_bg) x m =
      g₁.inner x
        (dLaCovKernel (I := I) g₁ g_bg x (m 1) (m 2) (m 3) -
          dLaCovKernel (I := I) g₁ g₀ x (m 1) (m 2) (m 3)) (m 0) := by
  rw [unitModel]
  rw [show (bdXdHalf (I := I) (M := M) g₀ g₁ g_bg).toSection x
        (unitTensor (I := I) (M := M) x) =
      slotInsertEndoFib (I := I) (M := M) 4 0 x
        (fullRaisedEndoField (I := I) (M := M) g₁ g₀ x)
        ((show Tensor0SSpace 0 I x →L[ℝ] Tensor0SSpace 4 I x from
          (bdLow0 (I := I) (M := M) g₀ g₁ g_bg).toSection x)
          (unitTensor (I := I) (M := M) x)) from by
    rw [bdXdHalf, appCcRS_toSection]
    rfl]
  rw [slotInsertEndoFib_apply_eval]
  rw [show Tensor0SSpace.toModel
        ((show Tensor0SSpace 0 I x →L[ℝ] Tensor0SSpace 4 I x from
          (bdLow0 (I := I) (M := M) g₀ g₁ g_bg).toSection x)
          (unitTensor (I := I) (M := M) x)) =
      unitModel (I := I) (M := M) g₀ 4 (bdLow0 (I := I) (M := M) g₀ g₁ g_bg) x from rfl]
  ...
  exact bdG0_inner_lambda (I := I) (M := M) g₀ g₁ x
    (dLaCovKernel (I := I) g₁ g_bg x (m 1) (m 2) (m 3) -
      dLaCovKernel (I := I) g₁ g₀ x (m 1) (m 2) (m 3)) (m 0)

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
private lemma bdXd_unitModel_apply (g₀ g₁ g_bg : SmoothRiemannianMetric I M)
    (x : M) (m : Fin 4 → TangentSpace I x) :
    unitModel (I := I) (M := M) g₀ 4 (bdXd (I := I) (M := M) g₀ g₁ g_bg) x m =
      g₁.inner x
          (dLaCovKernel (I := I) g₁ g_bg x (m 0) (m 2) (m 3) -
            dLaCovKernel (I := I) g₁ g₀ x (m 0) (m 2) (m 3)) (m 1) +
        g₁.inner x
          (dLaCovKernel (I := I) g₁ g_bg x (m 1) (m 2) (m 3) -
            dLaCovKernel (I := I) g₁ g₀ x (m 1) (m 2) (m 3)) (m 0) := by
  rw [bdXd, bdUnitModel_add, ContinuousMultilinearMap.add_apply]
  rw [domDomCongrSection_unitModel, ContinuousMultilinearMap.domDomCongr_apply]
...
private theorem bdCovDerivArmDiff_eq_pairTrace
    ...
    deTurckLieDLaCoeffField (I := I) (M := M) g₀ g₁ g_bg -
      ...
      (-1 : ℝ) • appCcRS (I := I) (M := M) g₀ 2 6 2 (bdPairTraceOp (I := I) (M := M) g₀ g₁)
        (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
  ...
  rw [bdPairTraceOp_apply_toModel (I := I) (M := M) g₀ g₁
    (bdXd (I := I) (M := M) g₀ g₁ g_bg) x D v]
  rw [show ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
      ((deTurckLieDLaCoeffField (I := I) (M := M) g₀ g₁ g_bg -
        deTurckLieDLaCoeffField (I := I) (M := M) g₀ g₁ g₀).toSection x)) D) =
      (show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
          (deTurckLieDLaCoeffField (I := I) (M := M) g₀ g₁ g_bg).toSection x) D -
        (show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
          (deTurckLieDLaCoeffField (I := I) (M := M) g₀ g₁ g₀).toSection x) D from by
    rw [show ((deTurckLieDLaCoeffField (I := I) (M := M) g₀ g₁ g_bg -
        deTurckLieDLaCoeffField (I := I) (M := M) g₀ g₁ g₀).toSection x) =
        (deTurckLieDLaCoeffField (I := I) (M := M) g₀ g₁ g_bg).toSection x -
          (deTurckLieDLaCoeffField (I := I) (M := M) g₀ g₁ g₀).toSection x from by
      rw [SmoothCcTensor.toSection_sub]; rfl]
    rfl]
  rw [Tensor0SSpace.toModel_sub, ContinuousMultilinearMap.sub_apply]
  rw [show (show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
  ...
  rw [show (show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
      (deTurckLieDLaCoeffField (I := I) (M := M) g₀ g₁ g₀).toSection x) D =
      dLaBiContrFib (I := I) g₁ g₀ x D from rfl]
  rw [show (dLaBiContrFib (I := I) g₁ g_bg x : Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x) =
      dLaBiContrFibFixedFrame (I := I) g₁ g_bg (smoothOrthoFrame (I := I) g₁ x) x from rfl]
  rw [show (dLaBiContrFib (I := I) g₁ g₀ x : Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x) =
      dLaBiContrFibFixedFrame (I := I) g₁ g₀ (smoothOrthoFrame (I := I) g₁ x) x from rfl]
  ...
  have hXval : ∀ a b : Fin (Module.finrank ℝ E),
      unitModel (I := I) (M := M) g₀ 4 (bdXd (I := I) (M := M) g₀ g₁ g_bg) x
      ...
      (g₁.inner x (dLaCovKernel (I := I) g₁ g₀ x (v 0)
          (smoothOrthoFrame (I := I) g₁ x a x) (smoothOrthoFrame (I := I) g₁ x b x)) (v 1) +
        g₁.inner x (dLaCovKernel (I := I) g₁ g₀ x (v 1)
...
set_option linter.unusedSectionVars false in
private lemma bdRfns_iCG_sub_le (g : SmoothRiemannianMetric I M) (r s : ℕ) (j : ℕ)
    (A B : SmoothCcTensor g r s) (x : M) :
    riemannianFiberNormSq (I := I) (M := M) g r (s + j) x
        ((iteratedCovGrad (I := I) g r s j (A - B)).toSection x) ≤
      2 * riemannianFiberNormSq (I := I) (M := M) g r (s + j) x
...
set_option linter.unusedVariables false in
set_option linter.unusedSectionVars false in
private theorem bdPureDT_tgrid (g₀ : SmoothRiemannianMetric I M) (s : ℕ)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ C : ℕ → ℝ, (∀ j, 0 ≤ C j) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
        (htie : ∀ (y : M) (v w : TangentSpace I y),
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ (s + 2) (s + j) x
            ((iteratedCovGrad (I := I) g₀ (s + 2) s j
              (bdPureDT (I := I) (M := M) g₀ g₁ s)).toSection x) ≤
          C j * Combinatorics.antidiagonalTupleGridWindow
            (fun l => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l) x
              ((iteratedCovGrad (I := I) g₀ 0 2 l T).toSection x)) (j + 1) := by
  classical
  obtain ⟨S, hS_nn, hS⟩ := bdSharpFlat_tgrid (I := I) (M := M) g₀ hδ₀
...
set_option linter.unusedVariables false in
set_option linter.unusedSectionVars false in
private theorem bdPairTraceOp_tgrid (g₀ : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ C : ℕ → ℝ, (∀ j, 0 ≤ C j) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (T : SmoothCcTensor g₀ 0 2)
        (htie : ∀ (y : M) (v w : TangentSpace I y),
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 6 (2 + j) x
            ((iteratedCovGrad (I := I) g₀ 6 2 j
              (bdPairTraceOp (I := I) (M := M) g₀ g₁)).toSection x) ≤
          C j * Combinatorics.antidiagonalTupleGridWindow
            (fun l => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l) x
              ((iteratedCovGrad (I := I) g₀ 0 2 l T).toSection x)) (j + 1) := by
  classical
...
private theorem bdQuadLow_movingOuter_gridWindow (g₀ g_bg : SmoothRiemannianMetric I M)
      ...
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
            ((iteratedCovGrad (I := I) g₀ 0 4 l
              (bdQuadLowCc (I := I) (M := M) g₀ g_bg g₁)).toSection x) ≤
          C l * Combinatorics.antidiagonalTupleGridWindow
            (fun l' => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l') x
  ...
  obtain ⟨cAS, hcAS_nn, hcAS⟩ := bdExists_fixedField_rfns_jet (I := I) (M := M) g₀ 3 4
    (armSlotEndoCc (I := I) (M := M) g₀ 2 (bdConnPair (I := I) (M := M) g₀ g_bg))
  refine ⟨fun l => appCcGdiag (E := E) l *
  ...
  have hcell : ∀ i' ∈ Finset.range (l + 1),
      riemannianFiberNormSq (I := I) (M := M) g₀ 3 (4 + i') x
          ((iteratedCovGrad (I := I) g₀ 3 4 i'
            (armSlotEndoCc (I := I) (M := M) g₀ 2
              (bdConnPair (I := I) (M := M) g₀ g_bg))).toSection x) *
        ∑ l' ∈ Finset.range (l + 1 - i'),
          riemannianFiberNormSq (I := I) (M := M) g₀ 0 (3 + l') x
            ((iteratedCovGrad (I := I) g₀ 0 3 l'
              (connDiffLoweredCc (I := I) g₀ g₁)).toSection x) ≤
    ...
    have hA1 : riemannianFiberNormSq (I := I) (M := M) g₀ 3 (4 + i') x
        ((iteratedCovGrad (I := I) g₀ 3 4 i'
          (armSlotEndoCc (I := I) (M := M) g₀ 2
            (bdConnPair (I := I) (M := M) g₀ g_bg))).toSection x) ≤ cAS i' := hcAS i' x
    have hA2 : (∑ l' ∈ Finset.range (l + 1 - i'),
        riemannianFiberNormSq (I := I) (M := M) g₀ 0 (3 + l') x
          ((iteratedCovGrad (I := I) g₀ 0 3 l'
            (connDiffLoweredCc (I := I) g₀ g₁)).toSection x)) ≤
...
private theorem bdQuadLow_movingArm_gridWindow (g₀ g_bg : SmoothRiemannianMetric I M)
    ...
    ∃ C : ℕ → ℝ, (∀ l, 0 ≤ C l) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
            ((iteratedCovGrad (I := I) g₀ 0 4 l
              (bdQuadLowCc (I := I) (M := M) g₀ g₁ g_bg)).toSection x) ≤
          C l * Combinatorics.antidiagonalTupleGridWindow
            (fun l' => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l') x
              ((iteratedCovGrad (I := I) g₀ 0 2 l' P).toSection x)) (l + 2) := by
  classical
  ...
  refine le_trans (rfns_iteratedCovGrad_appCcRS_diagonalProductGrid_rankLeft_le
    (I := I) (M := M) g₀ l 0 3 4
    (armSlotEndoCc (I := I) (M := M) g₀ 2 (bdConnPair (I := I) (M := M) g₀ g₁))
...
private theorem bdLow0_gridWindow (g₀ g_bg : SmoothRiemannianMetric I M)
  ...
  obtain ⟨cQ, hcQ_nn, hcQ⟩ := bdExists_fixedField_rfns_jet (I := I) (M := M) g₀ 0 4
    (bdQuadLowCc (I := I) (M := M) g₀ g_bg g_bg)
  ...
  set B := domDomCongrSection (I := I) g₀ bdSigma2
    (bdQuadLowCc (I := I) (M := M) g₀ g_bg g_bg) with hB_def
  set Cc := domDomCongrSection (I := I) g₀ bdSigma3
    (bdQuadLowCc (I := I) (M := M) g₀ g_bg g_bg) with hC_def
  set Dd := domDomCongrSection (I := I) g₀ bdSigma4
    (bdQuadLowCc (I := I) (M := M) g₀ g_bg g_bg) with hD_def
  set Ee := domDomCongrSection (I := I) g₀ bdSigma2
    (bdQuadLowCc (I := I) (M := M) g₀ g_bg g₁) with hE_def
  set Ff := domDomCongrSection (I := I) g₀ bdSigma3
    (bdQuadLowCc (I := I) (M := M) g₀ g₁ g_bg) with hF_def
  set Gg := domDomCongrSection (I := I) g₀ bdSigma4
    (bdQuadLowCc (I := I) (M := M) g₀ g₁ g_bg) with hG_def
  have hlow : bdLow0 (I := I) (M := M) g₀ g₁ g_bg = ((((A + B) - Cc) - Dd) - Ee) + Ff + Gg := by
    rw [bdLow0, hA_def, hB_def, hC_def, hD_def, hE_def, hF_def, hG_def]
  ...
  have hQfix : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
      ((iteratedCovGrad (I := I) g₀ 0 4 l
        (bdQuadLowCc (I := I) (M := M) g₀ g_bg g_bg)).toSection x) ≤ cQ l * W := by
    have h := hcQ l x
...
private theorem bdXd_gridWindow (g₀ g_bg : SmoothRiemannianMetric I M)
  ...
  have hhalf : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
      ((iteratedCovGrad (I := I) g₀ 0 4 l
        (bdXdHalf (I := I) (M := M) g₀ g₁ g_bg)).toSection x) ≤
      (appCcGdiag (E := E) l *
        ∑ i' ∈ Finset.range (l + 1), CΛ i' * ∑ l' ∈ Finset.range (l + 1 - i'),
  ...
  have hswap : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
      ...
      riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
        ((iteratedCovGrad (I := I) g₀ 0 4 l
          (bdXdHalf (I := I) (M := M) g₀ g₁ g_bg)).toSection x) :=
    riemannianFiberNormSq_iteratedCovGrad_domDomCongrSection (I := I) (M := M) g₀
      (Equiv.swap (0 : Fin 4) 1) (bdXdHalf (I := I) (M := M) g₀ g₁ g_bg) l x
  rw [hswap]
  linarith [hhalf]
  ...
  have hWtower : ∀ l, l ≤ i →
    ...
    have hperm : riemannianFiberNormSq (I := I) (M := M) g₀ 2 (6 + l) x
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 2 (6 + l) x
          ((iteratedCovGrad (I := I) g₀ 2 6 l
            (slotExtendIter (I := I) (M := M) g₀ 0 4 2
              (bdXd (I := I) (M := M) g₀ g₁ g_bg))).toSection x) :=
      rfns_iteratedCovGrad_rs_eq_of_section_domDomCongr (I := I) (M := M) g₀ 2 6 bdSigmaE0
        (slotExtendIter (I := I) (M := M) g₀ 0 4 2
          (bdXd (I := I) (M := M) g₀ g₁ g_bg))
        (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
          (slotExtendIter (I := I) (M := M) g₀ 0 4 2
            (bdXd (I := I) (M := M) g₀ g₁ g_bg)))
        (fun y d => by
    ...
    calc riemannianFiberNormSq (I := I) (M := M) g₀ 2 (6 + l) x
        ...
        ≤ fr * riemannianFiberNormSq (I := I) (M := M) g₀ 1 (5 + l) x
            ((iteratedCovGrad (I := I) g₀ 1 5 l
              (slotExtendIter (I := I) (M := M) g₀ 0 4 1
                (bdXd (I := I) (M := M) g₀ g₁ g_bg))).toSection x) := h1
...
theorem exists_deTurckLieCovDerivArm_backgroundDifference_perOrder_l2_tameEnvelope_generic
  ...
  have hsubj : deTurckLieCovDerivArmField (I := I) (M := M) g₀ g₁ g_bg -
      ...
      deTurckLieDLaCoeffField (I := I) (M := M) g₀ g₁ g_bg -
        deTurckLieDLaCoeffField (I := I) (M := M) g₀ g₁ g₀ := by
...
private def bdMonoPerm (σ : Equiv.Perm (Fin 4)) : Equiv.Perm (Fin 6) :=
  ((finSumFinEquiv (m := 4) (n := 2)).permCongr
    (Equiv.sumCongr σ (Equiv.refl (Fin 2)))).trans bdSigmaE0

private lemma bdMonoPerm_castAdd (σ : Equiv.Perm (Fin 4)) (j : Fin 4) :
    bdMonoPerm σ (Fin.castAdd 2 j) = (![1, 3, 4, 5] : Fin 4 → Fin 6) (σ j) := by
  have hpad : (finSumFinEquiv (m := 4) (n := 2)).permCongr
    ...
    rfl
  rw [bdMonoPerm, Equiv.trans_apply, hpad]
  exact (by decide : ∀ k : Fin 4,
    bdSigmaE0 (Fin.castAdd 2 k) = (![1, 3, 4, 5] : Fin 4 → Fin 6) k) (σ j)

private lemma bdMonoPerm_natAdd (σ : Equiv.Perm (Fin 4)) (k : Fin 2) :
    bdMonoPerm σ (Fin.natAdd 4 k) = (![0, 2] : Fin 2 → Fin 6) k := by
  have hpad : (finSumFinEquiv (m := 4) (n := 2)).permCongr
  ...
  exact (by decide : ∀ k' : Fin 2,
    bdSigmaE0 (Fin.natAdd 4 k') = (![0, 2] : Fin 2 → Fin 6) k') k

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
set_option maxHeartbeats 12800000 in
private lemma bdSlotExtendIterFour_toModel (g₀ : SmoothRiemannianMetric I M)
...
set_option linter.unusedSectionVars false in
set_option maxHeartbeats 12800000 in
private theorem bdMonoRefold_eq_pairTrace (g₀ g₁ : SmoothRiemannianMetric I M)
    (S : SmoothCcTensor g₀ 0 2) (σ : Equiv.Perm (Fin 4)) :
    curvatureRefoldMonomialCoeffField (I := I) (M := M) g₀ g₁
        (ccTensorUnitValueSection (I := I) (M := M) g₀ S)
        (ccTensorUnitValueSection_contMDiff (I := I) (M := M) g₀ S) σ =
      appCcRS (I := I) (M := M) g₀ 4 6 2 (bdPairTraceOp (I := I) (M := M) g₀ g₁)
        (rsDomDomCongrSection (I := I) (M := M) g₀ 4 6 (bdMonoPerm σ)
  ...
  have hYval : ∀ w : Fin 6 → TangentSpace I x,
    ...
    rw [show ((show Tensor0SSpace 4 I x →L[ℝ] Tensor0SSpace 6 I x from
        (rsDomDomCongrSection (I := I) (M := M) g₀ 4 6 (bdMonoPerm σ)
          (slotExtendIter (I := I) (M := M) g₀ 0 2 4 S)).toSection x) G) =
        ((show Tensor0SSpace 4 I x →L[ℝ] Tensor0SSpace 6 I x from
          rsDomDomCongr (bdMonoPerm σ)
            ((slotExtendIter (I := I) (M := M) g₀ 0 2 4 S).toSection x)) G) from by
      rw [rsDomDomCongrSection_toSection]]
    rw [toModel_rsDomDomCongr_apply (I := I) (M := M) (bdMonoPerm σ)
      ((slotExtendIter (I := I) (M := M) g₀ 0 2 4 S).toSection x) G]
    rw [ContinuousMultilinearMap.domDomCongr_apply]
    rw [bdSlotExtendIterFour_toModel (I := I) (M := M) g₀ S x G
      (fun i => w (bdMonoPerm σ i))]
    refine congrArg₂ (· * ·) ?_ ?_
    · refine congrArg _ ?_
      funext j
      fin_cases j
      · exact congrArg w (bdMonoPerm_castAdd σ 0)
      · exact congrArg w (bdMonoPerm_castAdd σ 1)
      · exact congrArg w (bdMonoPerm_castAdd σ 2)
      · exact congrArg w (bdMonoPerm_castAdd σ 3)
    · refine congrArg _ ?_
      funext k
      fin_cases k
      · exact congrArg w (bdMonoPerm_natAdd σ 0)
      · exact congrArg w (bdMonoPerm_natAdd σ 1)
  have hLHS : Tensor0SSpace.toModel
    ...
    rw [show ((show Tensor0SSpace 4 I x →L[ℝ] Tensor0SSpace 2 I x from
        ...
        curvatureRefoldMonomialFibFixedFrame (I := I) (M := M)
...
set_option linter.unusedSectionVars false in
private lemma bdGridWindow_mono_of_le (b b' : ℕ → ℝ) (hb : ∀ j, 0 ≤ b j)
    (hbb : ∀ j, b j ≤ b' j) (w : ℕ) :
...
set_option linter.unusedSectionVars false in
private lemma bdSingle_b_le_grid (b : ℕ → ℝ) (hb : ∀ j, 0 ≤ b j) (q : ℕ) (hq : 1 ≤ q) :
...
set_option linter.unusedVariables false in
set_option linter.unusedSectionVars false in
private theorem bdMonoRefold_pointwise_gridWindow (g₀ : SmoothRiemannianMetric I M)
    ...
    ∃ C : ℕ → ℝ, (∀ i, 0 ≤ C i) ∧
      ∀ (g₁ : SmoothRiemannianMetric I M) (P S : SmoothCcTensor g₀ 0 2)
        (htie : ∀ (y : M) (v w : TangentSpace I y),
          g₁.inner y v w = g₀.inner y v w + ccTensorBilinSymm (I := I) g₀ P y v w)
        {δ : ℝ} (hδ_le : δ ≤ δ₀) (hδ0 : 0 ≤ δ)
        (hboundP : gFibreOpBound (I := I) (M := M) g₀ (ccTensorBilinSymm (I := I) g₀ P) δ)
        (hboundS : gFibreOpBound (I := I) (M := M) g₀ (ccTensorBilinSymm (I := I) g₀ S) δ)
        (hPS : ∀ (l : ℕ) (x : M),
            ...
            ((iteratedCovGrad (I := I) g₀ 4 2 i
              (curvatureRefoldMonomialCoeffField (I := I) (M := M) g₀ g₁
                (ccTensorUnitValueSection (I := I) (M := M) g₀
                  (symmS (I := I) (M := M) g₀ S))
                (ccTensorUnitValueSection_contMDiff (I := I) (M := M) g₀
                  (symmS (I := I) (M := M) g₀ S)) σ)).toSection x) ≤
          C i * Combinatorics.antidiagonalTupleGridWindow
            (fun l' => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l') x
  ...
  have hWtower : ∀ l : ℕ,
    ...
    have hperm : riemannianFiberNormSq (I := I) (M := M) g₀ 4 (6 + l) x
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 4 (6 + l) x
          ((iteratedCovGrad (I := I) g₀ 4 6 l
            (slotExtendIter (I := I) (M := M) g₀ 0 2 4
              (symmS (I := I) (M := M) g₀ S))).toSection x) :=
      rfns_iteratedCovGrad_rs_eq_of_section_domDomCongr (I := I) (M := M) g₀ 4 6
        (bdMonoPerm σ)
        (slotExtendIter (I := I) (M := M) g₀ 0 2 4 (symmS (I := I) (M := M) g₀ S))
        (rsDomDomCongrSection (I := I) (M := M) g₀ 4 6 (bdMonoPerm σ)
          (slotExtendIter (I := I) (M := M) g₀ 0 2 4 (symmS (I := I) (M := M) g₀ S)))
        (fun y d => by
          rw [rsDomDomCongrSection_toSection, toModel_rsDomDomCongr_apply]) l x
    ...
    have h4 : riemannianFiberNormSq (I := I) (M := M) g₀ 4 (6 + l) x
      ...
      rfns_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 3 5
        (slotExtendIter (I := I) (M := M) g₀ 0 2 3 (symmS (I := I) (M := M) g₀ S)) l x
    have h3 : riemannianFiberNormSq (I := I) (M := M) g₀ 3 (5 + l) x
      ...
      rfns_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 2 4
        (slotExtendIter (I := I) (M := M) g₀ 0 2 2 (symmS (I := I) (M := M) g₀ S)) l x
    have h2 : riemannianFiberNormSq (I := I) (M := M) g₀ 2 (4 + l) x
      ...
      rfns_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 1 3
        (slotExtendIter (I := I) (M := M) g₀ 0 2 1 (symmS (I := I) (M := M) g₀ S)) l x
    ...
    have hbase : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l) x
      ...
      · subst hl0
        have hzero := bdRfns_symmS_zero_le (I := I) (M := M) g₀ S hδ0 hboundS x
        ...
        have hstep : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + 0) x
            ((iteratedCovGrad (I := I) g₀ 0 2 0
              (symmS (I := I) (M := M) g₀ S)).toSection x) ≤ fr ^ 2 * δ₀ ^ 2 := by
          rw [iteratedCovGrad_zero]
    ...
    calc riemannianFiberNormSq (I := I) (M := M) g₀ 4 (6 + l) x
        ...
        ≤ fr * riemannianFiberNormSq (I := I) (M := M) g₀ 3 (5 + l) x
            ((iteratedCovGrad (I := I) g₀ 3 5 l
              (slotExtendIter (I := I) (M := M) g₀ 0 2 3
                (symmS (I := I) (M := M) g₀ S))).toSection x) := h4
      ...
      _ ≤ fr * (fr * (fr * riemannianFiberNormSq (I := I) (M := M) g₀ 1 (3 + l) x
            ((iteratedCovGrad (I := I) g₀ 1 3 l
              (slotExtendIter (I := I) (M := M) g₀ 0 2 1
                (symmS (I := I) (M := M) g₀ S))).toSection x))) := by
        ...
        ((iteratedCovGrad (I := I) g₀ 4 2 i
          (appCcRS (I := I) (M := M) g₀ 4 6 2 (bdPairTraceOp (I := I) (M := M) g₀ g₁)
            (rsDomDomCongrSection (I := I) (M := M) g₀ 4 6 (bdMonoPerm σ)
              (slotExtendIter (I := I) (M := M) g₀ 0 2 4
                (symmS (I := I) (M := M) g₀ S))))).toSection x) := by
    rw [bdMonoRefold_eq_pairTrace (I := I) (M := M) g₀ g₁
      (symmS (I := I) (M := M) g₀ S) σ]
  rw [hlift]
  refine le_trans (rfns_iteratedCovGrad_appCcRS_diagonalProductGrid_rankLeft_le
    (I := I) (M := M) g₀ i 4 6 2
...
set_option linter.unusedVariables false in
set_option linter.unusedSectionVars false in
private theorem bdL2_tameEnvelope_of_gridWindow42 (g₀ : SmoothRiemannianMetric I M) (a : ℕ)
    (ha_super : 2 * Module.finrank ℝ E + 10 ≤ a) {R : ℝ} (hR : 0 ≤ R) :
    ∃ Kg : ℕ → ℝ, (∀ k, 0 ≤ Kg k) ∧
      ∀ (P : SmoothCcTensor g₀ 0 2),
        (∀ j : ℕ, j ≤ a + 2 → ‖iteratedCovGrad (I := I) g₀ 0 2 j P‖ ≤ R) →
...
theorem exists_curvatureRefoldMonomialCoeffField_symmS_realizedFam_l2JetWindow
  ...
  set δ₁ : ℝ := max δ₀ 0 with hδ₁_def
  have hδ₁_lt : δ₁ < 1 := max_lt hδ₀ one_pos
  obtain ⟨C, hC_nn, hpt⟩ := bdMonoRefold_pointwise_gridWindow (I := I) (M := M) g₀ hδ₁_lt σ
  obtain ⟨Kg, hKg_nn, hKg⟩ :=
    bdL2_tameEnvelope_of_gridWindow42 (I := I) (M := M) g₀ a ha_super hR
  refine ⟨fun i => C i * ∑ k ∈ Finset.range (i + 2), Kg k,
    fun i => mul_nonneg (hC_nn i) (Finset.sum_nonneg fun k _ => hKg_nn k), ?_⟩
  intro T δ hδ_le hδ hδZ hball i s hs
  have hwin_nn : (0 : ℝ) ≤ 1 + ∑ j ∈ Finset.range (i + 2),
...
set_option linter.unusedSectionVars false in
private lemma ccTensorBilin_smul_local (g₀ : SmoothRiemannianMetric I M)
...
set_option linter.unusedSectionVars false in
private theorem threeArmHjoint_const_smul_local (g₀ : SmoothRiemannianMetric I M) {r : ℕ}
...
set_option linter.unusedSectionVars false in
private lemma norm_add_sq_le_local (g : SmoothRiemannianMetric I M) {r s : ℕ}
...
private def bdBgRArmWeight (g₀ : SmoothRiemannianMetric I M) : SmoothCcTensor g₀ 2 4 :=
...
private lemma bdBgRArmWeight_toModel (g₀ : SmoothRiemannianMetric I M) (x : M)
    ...
    Tensor0SSpace.toModel
        ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 4 I x from
          (bdBgRArmWeight (I := I) (M := M) g₀).toSection x) D) m =
  ...
  have hYval : ∀ w : Fin 6 → TangentSpace I x,
    ...
    · refine congrArg _ ?_
      ...
      fin_cases k <;> simp [Equiv.swap_apply_def]
  rw [show ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 4 I x from
      (bdBgRArmWeight (I := I) (M := M) g₀).toSection x) D) =
      cometricDoubleTraceFib (I := I) g₀ 4 x Y from by
    rw [hY_def, bdBgRArmWeight]
    rw [appCcRS_toSection]
    rfl]
  rw [cometricDoubleTraceFib_toModel (I := I) g₀ 4 x Y]
  rw [modelDoubleTrace_apply (E := E) 4 (cometricLmodel (I := I) g₀ x)]
...
private theorem bdBgRComm_eq_refold (g₀ g : SmoothRiemannianMetric I M) :
    ricciArmOrder0BgRCommCoeffField (I := I) (M := M) g₀ g =
      appCcRS (I := I) (M := M) g₀ 2 4 2 (bdPureDT (I := I) (M := M) g₀ g 2)
        (bdBgRArmWeight (I := I) (M := M) g₀) := by
  classical
  apply SmoothCcTensor.ext
  apply ContMDiffSection.ext
  intro x
  ...
  rw [show ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
      ...
      cometricDoubleTraceFib (I := I) g 2 x
        ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 4 I x from
          (bdBgRArmWeight (I := I) (M := M) g₀).toSection x) D) from by
    rw [appCcRS_toSection]
    rfl]
  beta_reduce
  rw [cometricDoubleTraceFib_toModel (I := I) g 2 x]
...
private def bdRicciFoldWeightA (g₀ : SmoothRiemannianMetric I M)
...
private def bdRicciFoldWeightB (g₀ : SmoothRiemannianMetric I M)
    (S : SmoothCcTensor g₀ 0 2) : SmoothCcTensor g₀ 0 4 :=
  appCcRS (I := I) (M := M) g₀ 0 6 4 (cometricDoubleTraceField (I := I) g₀ 4)
    (rsDomDomCongrSection (I := I) (M := M) g₀ 0 6 bdTauK3b
      (appCcRS (I := I) (M := M) g₀ 0 2 6
        (slotExtendIter (I := I) (M := M) g₀ 0 4 2
          (riemannLoweredCc (I := I) (M := M) g₀ g₀ g₀)) S))
...
private lemma bdRicciFoldWeights_unitModel_eq_kernel (g₀ : SmoothRiemannianMetric I M)
    ...
    unitModel (I := I) (M := M) g₀ 4
        (bdRicciFoldWeightA (I := I) (M := M) g₀ S +
          bdRicciFoldWeightB (I := I) (M := M) g₀ S) x
        ![(v0 : E), (v1 : E), (p : E), (q : E)] =
  ...
  rw [bdUnitModel_add (I := I) (M := M) g₀ 4
    (bdRicciFoldWeightA (I := I) (M := M) g₀ S) (bdRicciFoldWeightB (I := I) (M := M) g₀ S) x,
    ContinuousMultilinearMap.add_apply]
  have hA : unitModel (I := I) (M := M) g₀ 4 (bdRicciFoldWeightA (I := I) (M := M) g₀ S) x
      ![(v0 : E), (v1 : E), (p : E), (q : E)] =
      ccTensorBilin (I := I) g₀ S x (riemannOp (LeviCivita (I := I) g₀) x v0 p q) v1 := by
    rw [show bdRicciFoldWeightA (I := I) (M := M) g₀ S =
        appCcRS (I := I) (M := M) g₀ 0 6 4 (cometricDoubleTraceField (I := I) g₀ 4)
          (rsDomDomCongrSection (I := I) (M := M) g₀ 0 6 (Equiv.swap (1 : Fin 6) 3)
            (appCcRS (I := I) (M := M) g₀ 0 2 6
              (slotExtendIter (I := I) (M := M) g₀ 0 4 2
    ...
    ring
  have hB : unitModel (I := I) (M := M) g₀ 4 (bdRicciFoldWeightB (I := I) (M := M) g₀ S) x
      ![(v0 : E), (v1 : E), (p : E), (q : E)] =
      ccTensorBilin (I := I) g₀ S x q (riemannOp (LeviCivita (I := I) g₀) x v0 p v1) := by
    rw [show bdRicciFoldWeightB (I := I) (M := M) g₀ S =
        appCcRS (I := I) (M := M) g₀ 0 6 4 (cometricDoubleTraceField (I := I) g₀ 4)
          (rsDomDomCongrSection (I := I) (M := M) g₀ 0 6 bdTauK3b
            (appCcRS (I := I) (M := M) g₀ 0 2 6
              (slotExtendIter (I := I) (M := M) g₀ 0 4 2
                (riemannLoweredCc (I := I) (M := M) g₀ g₀ g₀)) S)) from rfl]
    rw [bdRicciFoldWeight_unitModel_gen (I := I) (M := M) g₀ bdTauK3b S x
      ![(v0 : E), (v1 : E), (p : E), (q : E)]]
...
set_option linter.unusedSectionVars false in
set_option maxHeartbeats 12800000 in
private lemma bdRicciFold_eq_refold (g₀ g₁ : SmoothRiemannianMetric I M)
    (S : SmoothCcTensor g₀ 0 2) :
      ...
      (-(1 / 2) : ℝ) •
        appCcRS (I := I) (M := M) g₀ 2 6 2 (bdPairTraceOp (I := I) (M := M) g₀ g₁)
          (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
            (slotExtendIter (I := I) (M := M) g₀ 0 4 2
              (bdRicciFoldWeightA (I := I) (M := M) g₀ S +
                bdRicciFoldWeightB (I := I) (M := M) g₀ S))) := by
  classical
  apply SmoothCcTensor.ext
  apply ContMDiffSection.ext
  intro x
      ...
      (((-(1 / 2) : ℝ) •
        appCcRS (I := I) (M := M) g₀ 2 6 2 (bdPairTraceOp (I := I) (M := M) g₀ g₁)
          (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
            (slotExtendIter (I := I) (M := M) g₀ 0 4 2
              (bdRicciFoldWeightA (I := I) (M := M) g₀ S +
                bdRicciFoldWeightB (I := I) (M := M) g₀ S)))).toSection x)) D) =
      (-(1 / 2) : ℝ) • ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
        (appCcRS (I := I) (M := M) g₀ 2 6 2 (bdPairTraceOp (I := I) (M := M) g₀ g₁)
          (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
            (slotExtendIter (I := I) (M := M) g₀ 0 4 2
              (bdRicciFoldWeightA (I := I) (M := M) g₀ S +
                bdRicciFoldWeightB (I := I) (M := M) g₀ S)))).toSection x) D) := by
    rw [show ((((-(1 / 2) : ℝ) •
        appCcRS (I := I) (M := M) g₀ 2 6 2 (bdPairTraceOp (I := I) (M := M) g₀ g₁)
          (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
            (slotExtendIter (I := I) (M := M) g₀ 0 4 2
              (bdRicciFoldWeightA (I := I) (M := M) g₀ S +
                bdRicciFoldWeightB (I := I) (M := M) g₀ S)))).toSection x)) =
        (-(1 / 2) : ℝ) •
          ((appCcRS (I := I) (M := M) g₀ 2 6 2 (bdPairTraceOp (I := I) (M := M) g₀ g₁)
            (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
              (slotExtendIter (I := I) (M := M) g₀ 0 4 2
                (bdRicciFoldWeightA (I := I) (M := M) g₀ S +
                  bdRicciFoldWeightB (I := I) (M := M) g₀ S)))).toSection x) from by
      rw [SmoothCcTensor.toSection_smul]; rfl]
    rfl
  rw [hRHSsmul, Tensor0SSpace.toModel_smul, ContinuousMultilinearMap.smul_apply, smul_eq_mul]
  rw [bdPairTraceOp_apply_toModel (I := I) (M := M) g₀ g₁
    (bdRicciFoldWeightA (I := I) (M := M) g₀ S + bdRicciFoldWeightB (I := I) (M := M) g₀ S)
    x D v]
  rw [show ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 2 I x from
...
private lemma bdRicciFoldWeights_pair_smul (g₀ : SmoothRiemannianMetric I M)
    ...
    bdRicciFoldWeightA (I := I) (M := M) g₀ (c • T) +
      bdRicciFoldWeightB (I := I) (M := M) g₀ (c • T) =
      c • (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
        bdRicciFoldWeightB (I := I) (M := M) g₀ T) := by
  classical
  apply SmoothCcTensor.ext
  apply ContMDiffSection.ext
  intro x
  ...
  show unitModel (I := I) (M := M) g₀ 4
      (bdRicciFoldWeightA (I := I) (M := M) g₀ (c • T) +
        bdRicciFoldWeightB (I := I) (M := M) g₀ (c • T)) x m =
    unitModel (I := I) (M := M) g₀ 4
      (c • (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
        bdRicciFoldWeightB (I := I) (M := M) g₀ T)) x m
  rw [bdUnitModel_smul (I := I) (M := M) g₀ 4 c
    (bdRicciFoldWeightA (I := I) (M := M) g₀ T + bdRicciFoldWeightB (I := I) (M := M) g₀ T) x]
  ...
  rw [show unitModel (I := I) (M := M) g₀ 4
      (bdRicciFoldWeightA (I := I) (M := M) g₀ (c • T) +
        bdRicciFoldWeightB (I := I) (M := M) g₀ (c • T)) x ![m 0, m 1, m 2, m 3] =
      ccTensorBilin (I := I) g₀ (c • T) x
          (riemannOp (LeviCivita (I := I) g₀) x (m 0) (m 2) (m 3)) (m 1) +
        ccTensorBilin (I := I) g₀ (c • T) x (m 3)
  ...
  rw [show unitModel (I := I) (M := M) g₀ 4
      (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
        bdRicciFoldWeightB (I := I) (M := M) g₀ T) x ![m 0, m 1, m 2, m 3] =
      ccTensorBilin (I := I) g₀ T x
          (riemannOp (LeviCivita (I := I) g₀) x (m 0) (m 2) (m 3)) (m 1) +
        ccTensorBilin (I := I) g₀ T x (m 3)
          (riemannOp (LeviCivita (I := I) g₀) x (m 0) (m 2) (m 1)) from
...
set_option linter.unusedSectionVars false in
set_option maxHeartbeats 12800000 in
private lemma bdRicciFoldXi_smul (g₀ : SmoothRiemannianMetric I M)
    (T : SmoothCcTensor g₀ 0 2) (c : ℝ) :
    rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
      (slotExtendIter (I := I) (M := M) g₀ 0 4 2
        (bdRicciFoldWeightA (I := I) (M := M) g₀ (c • T) +
          bdRicciFoldWeightB (I := I) (M := M) g₀ (c • T))) =
    c • rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
      (slotExtendIter (I := I) (M := M) g₀ 0 4 2
        (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
          bdRicciFoldWeightB (I := I) (M := M) g₀ T)) := by
  classical
  ...
  rw [show ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 6 I x from
      ((c • rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
        (slotExtendIter (I := I) (M := M) g₀ 0 4 2
          (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
            bdRicciFoldWeightB (I := I) (M := M) g₀ T))).toSection x)) D) =
      c • ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 6 I x from
        (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
          (slotExtendIter (I := I) (M := M) g₀ 0 4 2
            (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
              bdRicciFoldWeightB (I := I) (M := M) g₀ T))).toSection x) D) from by
    rw [show (((c • rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
        (slotExtendIter (I := I) (M := M) g₀ 0 4 2
          (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
            bdRicciFoldWeightB (I := I) (M := M) g₀ T))).toSection x)) =
        c • ((rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
          (slotExtendIter (I := I) (M := M) g₀ 0 4 2
            (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
              bdRicciFoldWeightB (I := I) (M := M) g₀ T))).toSection x) from by
      rw [SmoothCcTensor.toSection_smul]; rfl]
    rfl]
  beta_reduce
  rw [Tensor0SSpace.toModel_smul, ContinuousMultilinearMap.smul_apply, smul_eq_mul]
  have hchain : ∀ (X : SmoothCcTensor g₀ 0 4),
    ...
    exact bdSlotExtendIter_two_toModel (I := I) (M := M) g₀ X x D
      (fun i => w (bdSigmaE0 i))
  rw [hchain (c • (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
      bdRicciFoldWeightB (I := I) (M := M) g₀ T)),
    hchain (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
      bdRicciFoldWeightB (I := I) (M := M) g₀ T)]
  rw [bdUnitModel_smul (I := I) (M := M) g₀ 4 c
    (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
      bdRicciFoldWeightB (I := I) (M := M) g₀ T) x]
  rw [ContinuousMultilinearMap.smul_apply, smul_eq_mul]
  ring

set_option linter.unusedSectionVars false in
private theorem bdJointTotalSpace0S_smulFun_local {d : ℕ} {S : Set ℝ}
    {f : ℝ → ℝ} (hf : ContDiff ℝ ∞ f)
...
set_option linter.unusedSectionVars false in
private lemma bdAACommMonomial_chartBasis_eq (g₀ g₁ : SmoothRiemannianMetric I M)
...
set_option linter.unusedSectionVars false in
private lemma bdGenJointGram_const_g0
...
private lemma bdChartChristoffel_g0_jointContMDiffOn
  ...
  classical
  have hG := bdGenJointGram_const_g0 (I := I) g₀ α (S := S)
...
set_option linter.unusedSectionVars false in
private lemma bdAACommKernScalar_realizedFam_jointContMDiffOn
    (g₀ : SmoothRiemannianMetric I M) (T T' : SmoothCcTensor g₀ 0 2)
    {δ : ℝ} (hδ : gFibreOpBound (I := I) (M := M) g₀ (ccTensorBilinSymm (I := I) g₀ T) δ)
    {δ' : ℝ} (hδ' : gFibreOpBound (I := I) (M := M) g₀ (ccTensorBilinSymm (I := I) g₀ T') δ')
    (α : M) (m k u v : Fin (Module.finrank ℝ E)) :
    ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) 𝓘(ℝ) ∞
      (fun p : M × ℝ => connDiffAACommKernelBilin (I := I) g₀
  ...
  rw [connDiffAACommKernelBilin_apply,
    bdAACommMonomial_chartBasis_eq (I := I) (M := M) g₀
...
set_option linter.unusedSectionVars false in
private lemma bdAACommBiContrFibAppY_chartCoord_jointContMDiffOn
    (g₀ : SmoothRiemannianMetric I M) (T T' : SmoothCcTensor g₀ 0 2)
    {δ : ℝ} (hδ : gFibreOpBound (I := I) (M := M) g₀ (ccTensorBilinSymm (I := I) g₀ T) δ)
    {δ' : ℝ} (hδ' : gFibreOpBound (I := I) (M := M) g₀ (ccTensorBilinSymm (I := I) g₀ T') δ')
  ...
  have hcomb : ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) 𝓘(ℝ) ∞
      (fun p : M × ℝ => ∑ m, ∑ n, chartInvGramMatrix (I := I)
        ...
        (∑ k, ∑ l, chartInvGramMatrix (I := I)
          ...
          (connDiffAACommKernelBilin (I := I) g₀
              (realizedFam (I := I) g₀ T T' hδ hδ' p.2) p.1
              (chartBasisVecFiber (I := I) α m p.1) (chartBasisVecFiber (I := I) α k p.1)
              ((![(chartBasisVecFiber (I := I) α (σc 0) p.1 : E),
    ...
    refine (realizedFam_chartInvGramMatrix_jointContMDiffOn_free (I := I) g₀ T T'
      hδ hδ' α k l).mul ?_
    exact (bdAACommKernScalar_realizedFam_jointContMDiffOn (I := I) g₀ T T' hδ hδ'
...
private lemma bdAACommBiContrFibAppY_realizedFam_jointContMDiffOn
  ...
  have hcoordEach : ∀ σc : Fin 2 → Fin (Module.finrank ℝ E),
      ContMDiffWithinAt (I.prod 𝓘(ℝ, ℝ)) 𝓘(ℝ) ∞
        ...
        ((Set.univ : Set M) ×ˢ S) p₀ := by
    intro σc
    have hscal := bdAACommBiContrFibAppY_chartCoord_jointContMDiffOn
...
theorem ricciArmOrder0BgRCommCoeffField_realizedFam_threeArmHjoint
    ...
    (hδZ : gFibreOpBound (I := I) (M := M) g₀
      (ccTensorBilinSymm (I := I) g₀ (0 : SmoothCcTensor g₀ 0 2)) δ) :
    linearizedRicciThreeArmHjoint (I := I) (M := M) g₀ 2
      (fun s => ricciArmOrder0BgRCommCoeffField (I := I) (M := M) g₀
        (realizedFam (I := I) g₀ T 0 hδ hδZ s)) (δ := δ) (δ' := δ) := by
  classical
  have hperY : ∀ (Y : Cₛ^∞⟮I; Tensor0SModel 2 ℝ E, fun x : M => Tensor0SSpace 2 I x⟯),
      ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) (I.prod 𝓘(ℝ, Tensor0SModel 2 ℝ E)) ∞
        (fun p : M × ℝ => TotalSpace.mk' (Tensor0SModel 2 ℝ E)
          (E := fun z : M => Tensor0SSpace 2 I z) p.1
          (bgRBiContrFib (I := I) g₀ (realizedFam (I := I) g₀ T 0 hδ hδZ p.2) p.1 (Y p.1)))
        ((Set.univ : Set M) ×ˢ realizedSmallSet (δ := δ) (δ' := δ)) := by
    intro Y
    have hWapp : ContMDiff I (I.prod 𝓘(ℝ, Tensor0SModel 4 ℝ E)) ∞
        (fun x : M => TotalSpace.mk' (Tensor0SModel 4 ℝ E)
          (E := fun z : M => Tensor0SSpace 4 I z) x
          ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 4 I x from
            (bdBgRArmWeight (I := I) (M := M) g₀).toSection x) (Y x))) :=
      ContMDiff.clm_bundle_apply (b := id)
        (bdBgRArmWeight (I := I) (M := M) g₀).toSection.contMDiff Y.contMDiff
    have hWjoint : ContMDiffOn (I.prod 𝓘(ℝ, ℝ)) (I.prod 𝓘(ℝ, Tensor0SModel 4 ℝ E)) ∞
        (fun q : M × ℝ => TotalSpace.mk' (Tensor0SModel 4 ℝ E)
          ...
          ((show Tensor0SSpace 2 I q.1 →L[ℝ] Tensor0SSpace 4 I q.1 from
            (bdBgRArmWeight (I := I) (M := M) g₀).toSection q.1) (Y q.1)))
        ((Set.univ : Set M) ×ˢ realizedSmallSet (δ := δ) (δ' := δ)) :=
    ...
    calc bgRBiContrFib (I := I) g₀ (realizedFam (I := I) g₀ T 0 hδ hδZ q.2) q.1 (Y q.1)
      ...
      _ = (show Tensor0SSpace 2 I q.1 →L[ℝ] Tensor0SSpace 2 I q.1 from
            (appCcRS (I := I) (M := M) g₀ 2 4 2
              ...
              (bdBgRArmWeight (I := I) (M := M) g₀)).toSection q.1) (Y q.1) := by
  ...
  have hCLM := contMDiffOn_clm_section_of_pointwise_jointMR (I := I) (M := M)
    (F₁ := Tensor0SModel 2 ℝ E) (V₁ := fun x : M => Tensor0SSpace 2 I x)
    (F₂ := Tensor0SModel 2 ℝ E) (V₂ := fun x : M => Tensor0SSpace 2 I x)
    (φ := fun p : M × ℝ => bgRBiContrFib (I := I) g₀
      (realizedFam (I := I) g₀ T 0 hδ hδZ p.2) p.1)
...
set_option linter.unusedSectionVars false in
private def bdKRaw (g₀ : SmoothRiemannianMetric I M) (S : SmoothCcTensor g₀ 0 2) :
...
private def bdSGKMvWeight (g₀ g₁ : SmoothRiemannianMetric I M) (σ : Equiv.Perm (Fin 6))
...
set_option linter.unusedSectionVars false in
set_option maxHeartbeats 12800000 in
private lemma bdSGK_eq_refold (g₀ g₁ : SmoothRiemannianMetric I M)
    (P S : SmoothCcTensor g₀ 0 2)
    (htie : ∀ (y : M) (v w : TangentSpace I y),
      g₁.inner y v w = g₀.inner y v w + ccTensorBilinSymm (I := I) g₀ P y v w) :
...
set_option linter.unusedSectionVars false in
set_option maxHeartbeats 12800000 in
private lemma bdSGKXi_smul (g₀ g₁ : SmoothRiemannianMetric I M)
    (T : SmoothCcTensor g₀ 0 2) (c : ℝ) :
    rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
      (slotExtendIter (I := I) (M := M) g₀ 0 4 2
        ((bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau1 (c • T) (c • T) +
...
set_option linter.unusedSectionVars false in
private lemma bdRfns_iCG_koszulCovecCc_le (g₀ : SmoothRiemannianMetric I M)
...
set_option linter.unusedSectionVars false in
private lemma bdRfns_iCG_bdKRaw_le (g₀ : SmoothRiemannianMetric I M)
    (T : SmoothCcTensor g₀ 0 2) (i : ℕ) (x : M) :
    riemannianFiberNormSq (I := I) (M := M) g₀ 0 (3 + i) x
        ((iteratedCovGrad (I := I) g₀ 0 3 i (bdKRaw (I := I) (M := M) g₀ T)).toSection x) ≤
      10 * riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + (i + 1)) x
        ((iteratedCovGrad (I := I) g₀ 0 2 (i + 1) T).toSection x) := by
  classical
  ...
  have hpermW : ∀ σ : Equiv.Perm (Fin 3),
    ...
    rw [hcomm]
  have hkos : bdKRaw (I := I) (M := M) g₀ T = (1 / 2 : ℝ) • (DA + DB - DC) := by
    rw [bdKRaw, hDA, hDB, hDC, hW]
  have hsub : iteratedCovGrad (I := I) g₀ 0 3 i (DA + DB - DC) =
      iteratedCovGrad (I := I) g₀ 0 3 i DA + iteratedCovGrad (I := I) g₀ 0 3 i DB -
        iteratedCovGrad (I := I) g₀ 0 3 i DC := by
    rw [sub_eq_add_neg, sub_eq_add_neg, iteratedCovGrad_add, iteratedCovGrad_add,
  ...
  have htoSec : ((iteratedCovGrad (I := I) g₀ 0 3 i
        (bdKRaw (I := I) (M := M) g₀ T)).toSection x :
        TensorRSSpace 0 (3 + i) I x) =
      (1 / 2 : ℝ) • ((iteratedCovGrad (I := I) g₀ 0 3 i DA).toSection x +
        (iteratedCovGrad (I := I) g₀ 0 3 i DB).toSection x -
        (iteratedCovGrad (I := I) g₀ 0 3 i DC).toSection x) := by
...
attribute [-instance] Tensor0SBundle.tensorRSSpace_normedAddCommGroup
  Tensor0SBundle.tensorRSSpace_normedSpace in
private theorem bdExists_rfnsT_one_jet_cap (g₀ : SmoothRiemannianMetric I M) (a : ℕ)
...
theorem exists_ricciArmSharpGradKoszulResidualField_realizedFam_rfns_ballUniform
  ...
  have hcomp : riemannianFiberNormSq (I := I) (M := M) g₀ 2 2 x
      ...
      riemannianFiberNormSq (I := I) (M := M) g₀ 6 2 x
          ((bdPairTraceOp (I := I) (M := M) g₀ (realizedFam (I := I) g₀ T 0 hδ hδZ s)).toSection x) *
  ...
  have hPTO : riemannianFiberNormSq (I := I) (M := M) g₀ 6 2 x
      ((bdPairTraceOp (I := I) (M := M) g₀ (realizedFam (I := I) g₀ T 0 hδ hδZ s)).toSection x) ≤ CP 0 := by
  ...
  have hDT4cap : riemannianFiberNormSq (I := I) (M := M) g₀ 6 4 x
      ((bdPureDT (I := I) (M := M) g₀ (realizedFam (I := I) g₀ T 0 hδ hδZ s) 4).toSection x) ≤ C4 0 := by
  ...
  have hKRawcap : riemannianFiberNormSq (I := I) (M := M) g₀ 0 3 x
      ((bdKRaw (I := I) (M := M) g₀ T).toSection x) ≤
      10 * (Csob1 * R) ^ 2 := by
    have h := bdRfns_iCG_bdKRaw_le (I := I) (M := M) g₀ T 0 x
    rw [iteratedCovGrad_zero] at h
    refine le_trans h ?_
  ...
  have hZcap : ∀ τ : Equiv.Perm (Fin 6),
    ...
    have hperm : riemannianFiberNormSq (I := I) (M := M) g₀ 0 6 x
      ...
      have h := rfns_iteratedCovGrad_rs_eq_of_section_domDomCongr (I := I) (M := M)
        g₀ 0 6 τ
        (appCcRS (I := I) (M := M) g₀ 0 3 6
          (slotExtendIter (I := I) (M := M) g₀ 0 3 3 (bdKRaw (I := I) (M := M) g₀ T))
    ...
    have hcw : riemannianFiberNormSq (I := I) (M := M) g₀ 0 4 x
      ...
      (realizedFam (I := I) g₀ T 0 hδ hδZ s) bdSGKTau1 T T).toSection x) ≤
        riemannianFiberNormSq (I := I) (M := M) g₀ 6 4 x
            ((bdPureDT (I := I) (M := M) g₀ (realizedFam (I := I) g₀ T 0 hδ hδZ s) 4).toSection x) *
          riemannianFiberNormSq (I := I) (M := M) g₀ 0 6 x
            ((rsDomDomCongrSection (I := I) (M := M) g₀ 0 6 bdSGKTau1
        (appCcRS (I := I) (M := M) g₀ 0 3 6
          (slotExtendIter (I := I) (M := M) g₀ 0 3 3 (bdKRaw (I := I) (M := M) g₀ T))
      ...
      (realizedFam (I := I) g₀ T 0 hδ hδZ s) bdSGKTau1 T T =
          appCcRS (I := I) (M := M) g₀ 0 6 4
            (bdPureDT (I := I) (M := M) g₀ (realizedFam (I := I) g₀ T 0 hδ hδZ s) 4)
            (rsDomDomCongrSection (I := I) (M := M) g₀ 0 6 bdSGKTau1
        (appCcRS (I := I) (M := M) g₀ 0 3 6
          (slotExtendIter (I := I) (M := M) g₀ 0 3 3 (bdKRaw (I := I) (M := M) g₀ T))
          (koszulCovecCc (I := I) g₀ T))) from rfl]
  ...
  have hW4 : riemannianFiberNormSq (I := I) (M := M) g₀ 0 4 x
    ...
    refine le_trans (mul_le_mul_of_nonneg_right hDT4cap hznn) ?_
    exact mul_le_mul_of_nonneg_left hz (hC4_nn 0)
  have hXsplit : ((((bdSGKMvWeight (I := I) (M := M) g₀
      (realizedFam (I := I) g₀ T 0 hδ hδZ s) bdSGKTau1 T T +
      bdSGKMvWeight (I := I) (M := M) g₀
      (realizedFam (I := I) g₀ T 0 hδ hδZ s) bdSGKTau2 T T) -
      (bdSGKMvWeight (I := I) (M := M) g₀
...
attribute [-instance] Tensor0SBundle.tensorRSSpace_normedAddCommGroup
  Tensor0SBundle.tensorRSSpace_normedSpace in
private theorem bdExists_rfnsT_zero_jet_cap (g₀ : SmoothRiemannianMetric I M) (a : ℕ)
    (ha_super : 2 * Module.finrank ℝ E + 10 ≤ a) :
    ∃ Csob : ℝ, 0 ≤ Csob ∧
      ∀ (T : SmoothCcTensor g₀ 0 2) {R : ℝ} (_hR : 0 ≤ R),
        (∀ j : ℕ, j ≤ a + 2 → ‖iteratedCovGrad (I := I) g₀ 0 2 j T‖ ≤ R) →
...
theorem exists_ricciArmRicciFoldRemainderField_realizedFam_rfns_ballUniform
  ...
  obtain ⟨Csob, hCsob_nn, hTcapAll⟩ :=
    bdExists_rfnsT_zero_jet_cap (I := I) (M := M) g₀ a ha_super
        ...
        (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
          (slotExtendIter (I := I) (M := M) g₀ 0 4 2
            (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
              bdRicciFoldWeightB (I := I) (M := M) g₀ T)))).toSection x) ≤
      riemannianFiberNormSq (I := I) (M := M) g₀ 6 2 x
          ((bdPairTraceOp (I := I) (M := M) g₀
            (realizedFam (I := I) g₀ T 0 hδ hδZ s)).toSection x) *
        riemannianFiberNormSq (I := I) (M := M) g₀ 2 6 x
          ((rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
            (slotExtendIter (I := I) (M := M) g₀ 0 4 2
              (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
                bdRicciFoldWeightB (I := I) (M := M) g₀ T))).toSection x) := by
    rw [appCcRS_toSection]
  ...
  have hXi : riemannianFiberNormSq (I := I) (M := M) g₀ 2 6 x
    ...
    have hslot2 : riemannianFiberNormSq (I := I) (M := M) g₀ 2 6 x
      ...
      have h := rfns_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 1 5
        (slotExtendIter (I := I) (M := M) g₀ 0 4 1
          (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
            bdRicciFoldWeightB (I := I) (M := M) g₀ T)) 0 x
      rwa [iteratedCovGrad_zero, iteratedCovGrad_zero] at h
    ...
    have hW4 : riemannianFiberNormSq (I := I) (M := M) g₀ 0 4 x
      ...
      have hsplit : ((bdRicciFoldWeightA (I := I) (M := M) g₀ T +
          bdRicciFoldWeightB (I := I) (M := M) g₀ T).toSection x) =
          (bdRicciFoldWeightA (I := I) (M := M) g₀ T).toSection x +
            (bdRicciFoldWeightB (I := I) (M := M) g₀ T).toSection x := by
    ...
    calc riemannianFiberNormSq (I := I) (M := M) g₀ 2 6 x
        ...
        = riemannianFiberNormSq (I := I) (M := M) g₀ 2 6 x
            ((slotExtendIter (I := I) (M := M) g₀ 0 4 2
              (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
                bdRicciFoldWeightB (I := I) (M := M) g₀ T)).toSection x) := hperm6
      _ ≤ fr * riemannianFiberNormSq (I := I) (M := M) g₀ 1 5 x
            ((slotExtendIter (I := I) (M := M) g₀ 0 4 1
              (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
                bdRicciFoldWeightB (I := I) (M := M) g₀ T)).toSection x) := hslot2
    ...
    _ = riemannianFiberNormSq (I := I) (M := M) g₀ 2 2 x
          ((appCcRS (I := I) (M := M) g₀ 2 6 2
            ...
            (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
              (slotExtendIter (I := I) (M := M) g₀ 0 4 2
                (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
                  bdRicciFoldWeightB (I := I) (M := M) g₀ T)))).toSection x) := one_mul _
    _ ≤ riemannianFiberNormSq (I := I) (M := M) g₀ 6 2 x
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 2 6 x
          ((rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
            (slotExtendIter (I := I) (M := M) g₀ 0 4 2
              (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
                bdRicciFoldWeightB (I := I) (M := M) g₀ T))).toSection x) := hcomp
...
theorem exists_ricciArmOrder0BgRCommCoeffField_realizedFam_backgroundDifference_l2JetWindow
      ...
      ∀ (T : SmoothCcTensor g₀ 0 2)
        ...
        ∀ i : ℕ, ∀ s ∈ Set.Icc (0 : ℝ) 1,
          ‖iteratedCovGrad (I := I) g₀ 2 2 i
            (ricciArmOrder0BgRCommCoeffField (I := I) (M := M) g₀
                (realizedFam (I := I) g₀ T 0 hδ hδZ s)
              - ricciArmOrder0BgRCommCoeffField (I := I) (M := M) g₀ g₀)‖ ^ 2 ≤
            K i * (1 + ∑ j ∈ Finset.range (i + 2),
              ‖iteratedCovGrad (I := I) g₀ 0 2 j T‖ ^ 2) := by
  classical
  set δ₁ : ℝ := max δ₀ 0 with hδ₁_def
  ...
  · haveI hM' : IsEmpty M := not_nonempty_iff.mp hM
    have hz : ‖iteratedCovGrad (I := I) g₀ 2 2 i
        (ricciArmOrder0BgRCommCoeffField (I := I) (M := M) g₀
            (realizedFam (I := I) g₀ T 0 hδ hδZ s)
          - ricciArmOrder0BgRCommCoeffField (I := I) (M := M) g₀ g₀)‖ = 0 :=
      bdNorm_zero_of_isEmpty (I := I) (M := M) g₀ 2 (2 + i) _
    rw [hz]
    have hK_nn : 0 ≤ C i * Kflat i := mul_nonneg (hC_nn i) (hKflat_nn i)
    nlinarith [hwin_nn, hK_nn]
...
set_option linter.unusedSectionVars false in
private lemma bdSGKZ_window (g₀ : SmoothRiemannianMetric I M) :
    ∃ CZ : ℕ → ℝ, (∀ w, 0 ≤ CZ w) ∧
  ...
  have hperm : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (6 + w) x
      ...
      riemannianFiberNormSq (I := I) (M := M) g₀ 0 (6 + w) x
        ...
        (slotExtendIter (I := I) (M := M) g₀ 0 3 3 (bdKRaw (I := I) (M := M) g₀ T))
        (koszulCovecCc (I := I) g₀ T))).toSection x) :=
    rfns_iteratedCovGrad_rs_eq_of_section_domDomCongr (I := I) (M := M)
      g₀ 0 6 τ
      (appCcRS (I := I) (M := M) g₀ 0 3 6
        (slotExtendIter (I := I) (M := M) g₀ 0 3 3 (bdKRaw (I := I) (M := M) g₀ T))
        (koszulCovecCc (I := I) g₀ T))
  ...
  have hΦ : ∀ w₁ : ℕ,
      riemannianFiberNormSq (I := I) (M := M) g₀ 3 (6 + w₁) x
        ((iteratedCovGrad (I := I) g₀ 3 6 w₁
          (slotExtendIter (I := I) (M := M) g₀ 0 3 3 (bdKRaw (I := I) (M := M) g₀ T))).toSection x) ≤
      fr * (fr * (fr * (10 * b (w₁ + 1)))) := by
    intro w₁
    have h3 := rfns_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 2 5
      (slotExtendIter (I := I) (M := M) g₀ 0 3 2 (bdKRaw (I := I) (M := M) g₀ T)) w₁ x
    have h2 := rfns_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 1 4
      (slotExtendIter (I := I) (M := M) g₀ 0 3 1 (bdKRaw (I := I) (M := M) g₀ T)) w₁ x
    have h1 := rfns_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 0 3
      (bdKRaw (I := I) (M := M) g₀ T) w₁ x
    have h0 := bdRfns_iCG_bdKRaw_le (I := I) (M := M) g₀ T w₁ x
    refine le_trans h3 ?_
    refine mul_le_mul_of_nonneg_left (le_trans h2 ?_) hfr_nn
    refine mul_le_mul_of_nonneg_left (le_trans h1 ?_) hfr_nn
...
set_option linter.unusedSectionVars false in
set_option linter.unusedVariables false in
private lemma bdSGKW_window (g₀ : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ CW : ℕ → ℝ, (∀ l, 0 ≤ CW l) ∧
  ...
  rw [show bdSGKMvWeight (I := I) (M := M) g₀ g₁ τ T T =
      appCcRS (I := I) (M := M) g₀ 0 6 4 (bdPureDT (I := I) (M := M) g₀ g₁ 4)
        (rsDomDomCongrSection (I := I) (M := M) g₀ 0 6 τ
        (appCcRS (I := I) (M := M) g₀ 0 3 6
          (slotExtendIter (I := I) (M := M) g₀ 0 3 3 (bdKRaw (I := I) (M := M) g₀ T))
...
set_option linter.unusedSectionVars false in
set_option linter.unusedVariables false in
private lemma bdSGKComposite_pointwise_boundedWindow (g₀ : SmoothRiemannianMetric I M)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ C : ℕ → ℝ, (∀ i, 0 ≤ C i) ∧
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 2 (2 + i) x
            ((iteratedCovGrad (I := I) g₀ 2 2 i
              (appCcRS (I := I) (M := M) g₀ 2 6 2
                ...
                (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
        (slotExtendIter (I := I) (M := M) g₀ 0 4 2
          ((bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau1 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau2 T T) -
          (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau3 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau4 T T)))))).toSection x) ≤
          C i * Combinatorics.boundedFactorGridWindow
            (fun l => riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + l) x
              ((iteratedCovGrad (I := I) g₀ 0 2 l T).toSection x)) (i + 1) (i + 3) := by
  classical
  obtain ⟨CP, hCP_nn, hCP⟩ := bdPairTraceOp_tgrid (I := I) (M := M) g₀ hδ₀
  obtain ⟨CW, hCW_nn, hCW⟩ := bdSGKW_window (I := I) (M := M) g₀ hδ₀
  set fr : ℝ := (Module.finrank ℝ E : ℝ) with hfr_def
  have hfr_nn : 0 ≤ fr := Nat.cast_nonneg _
  refine ⟨fun i => appCcGdiag (E := E) i *
  ...
  refine le_trans (rfns_iteratedCovGrad_appCcRS_diagonalProductGrid_rankLeft_le
    ...
    (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
        (slotExtendIter (I := I) (M := M) g₀ 0 4 2
          ((bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau1 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau2 T T) -
          (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau3 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau4 T T)))) x) ?_
  have hXiJet : ∀ l : ℕ, l ≤ i →
      riemannianFiberNormSq (I := I) (M := M) g₀ 2 (6 + l) x
        ((iteratedCovGrad (I := I) g₀ 2 6 l
          (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
    ...
    have hperm : riemannianFiberNormSq (I := I) (M := M) g₀ 2 (6 + l) x
      ...
      rfns_iteratedCovGrad_rs_eq_of_section_domDomCongr (I := I) (M := M)
        ...
        (slotExtendIter (I := I) (M := M) g₀ 0 4 2
          ((bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau1 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau2 T T) -
          (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau3 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau4 T T)))
        (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
        (slotExtendIter (I := I) (M := M) g₀ 0 4 2
          ((bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau1 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau2 T T) -
          (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau3 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau4 T T))))
    ...
    have h2 := rfns_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 1 5
      (slotExtendIter (I := I) (M := M) g₀ 0 4 1
        ((bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau1 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau2 T T) -
          (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau3 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau4 T T))) l x
    have h1 := rfns_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 0 4
      ((bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau1 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau2 T T) -
          (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau3 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau4 T T)) l x
    have hsub := bdRfns_iCG_sub_le (I := I) (M := M) g₀ 0 4 l
      (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau1 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau2 T T)
      (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau3 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau4 T T) x
    have hadd12 := bdRfns_iCG_add_le (I := I) (M := M) g₀ 0 4 l
      (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau1 T T) (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau2 T T) x
    ...
    have hXi4 : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
      ...
      nlinarith [hsub, hadd12, hadd34, hW1, hW2, hW3, hW4, hWnn, hCW_nn l,
        riemannianFiberNormSq_nonneg (I := I) (M := M) g₀ 0 (4 + l) x
          ((iteratedCovGrad (I := I) g₀ 0 4 l
            (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau1 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau2 T T)).toSection x),
        riemannianFiberNormSq_nonneg (I := I) (M := M) g₀ 0 (4 + l) x
          ((iteratedCovGrad (I := I) g₀ 0 4 l
            (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau3 T T + bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau4 T T)).toSection x),
        riemannianFiberNormSq_nonneg (I := I) (M := M) g₀ 0 (4 + l) x
          ((iteratedCovGrad (I := I) g₀ 0 4 l (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau1 T T)).toSection x),
        riemannianFiberNormSq_nonneg (I := I) (M := M) g₀ 0 (4 + l) x
          ((iteratedCovGrad (I := I) g₀ 0 4 l (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau2 T T)).toSection x),
        riemannianFiberNormSq_nonneg (I := I) (M := M) g₀ 0 (4 + l) x
          ((iteratedCovGrad (I := I) g₀ 0 4 l (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau3 T T)).toSection x),
        riemannianFiberNormSq_nonneg (I := I) (M := M) g₀ 0 (4 + l) x
          ((iteratedCovGrad (I := I) g₀ 0 4 l (bdSGKMvWeight (I := I) (M := M) g₀ g₁ bdSGKTau4 T T)).toSection x)]
    refine le_trans h2 ?_
...
theorem exists_ricciArmSharpGradKoszulResidualField_realizedFam_l2JetWindow
  ...
  have hδ₁_lt : δ₁ < 1 := max_lt hδ₀ one_pos
  obtain ⟨C, hC_nn, hpt⟩ :=
    bdSGKComposite_pointwise_boundedWindow (I := I) (M := M) g₀ hδ₁_lt
  obtain ⟨Kflat, hKflat_nn, hKflat⟩ :=
    boundedFactorGridWindow_integral_ballUniform_flat_allOrders (I := I) (M := M) g₀ a
      ha_super hR
...
set_option linter.unusedVariables false in
set_option linter.unusedSectionVars false in
private theorem bdRicciFoldComposite_pointwise_gridWindow (g₀ : SmoothRiemannianMetric I M)
  ...
  set fr : ℝ := (Module.finrank ℝ E : ℝ) with hfr_def
  have hfr_nn : 0 ≤ fr := Nat.cast_nonneg _
  set C6 : ℕ → ℝ := fun m => appCcGdiag (E := E) m *
    ∑ m' ∈ Finset.range (m + 1),
  ...
  set C7 : ℕ → ℝ := fun l => appCcGdiag (E := E) l *
    ∑ l' ∈ Finset.range (l + 1),
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 0 (6 + m) x
          ((iteratedCovGrad (I := I) g₀ 0 6 m
            (appCcRS (I := I) (M := M) g₀ 0 2 6
              (slotExtendIter (I := I) (M := M) g₀ 0 4 2
                (riemannLoweredCc (I := I) (M := M) g₀ g₀ g₀)) T)).toSection x) :=
      fun m => rfns_iteratedCovGrad_rs_eq_of_section_domDomCongr (I := I) (M := M)
        g₀ 0 6 σw
        (appCcRS (I := I) (M := M) g₀ 0 2 6
          (slotExtendIter (I := I) (M := M) g₀ 0 4 2
            (riemannLoweredCc (I := I) (M := M) g₀ g₀ g₀)) T)
  ...
  have hXitower : ∀ l : ℕ,
    ...
    intro l
    have hperm := rfns_iteratedCovGrad_rs_eq_of_section_domDomCongr (I := I) (M := M)
      g₀ 2 6 bdSigmaE0
      (slotExtendIter (I := I) (M := M) g₀ 0 4 2
        (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
          bdRicciFoldWeightB (I := I) (M := M) g₀ T))
    ...
    have h2 := rfns_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 1 5
      (slotExtendIter (I := I) (M := M) g₀ 0 4 1
        (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
          bdRicciFoldWeightB (I := I) (M := M) g₀ T)) l x
    have h1 := rfns_iteratedCovGrad_slotExtend_le (I := I) (M := M) g₀ 0 4
      (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
        bdRicciFoldWeightB (I := I) (M := M) g₀ T) l x
    ...
    calc riemannianFiberNormSq (I := I) (M := M) g₀ 2 (6 + l) x
        ...
        ≤ fr * riemannianFiberNormSq (I := I) (M := M) g₀ 1 (5 + l) x
            ((iteratedCovGrad (I := I) g₀ 1 5 l
              (slotExtendIter (I := I) (M := M) g₀ 0 4 1
                (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
                  bdRicciFoldWeightB (I := I) (M := M) g₀ T))).toSection x) := h2
      _ ≤ fr * (fr * riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + l) x
            ((iteratedCovGrad (I := I) g₀ 0 4 l
              (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
                bdRicciFoldWeightB (I := I) (M := M) g₀ T)).toSection x)) :=
  ...
  refine le_trans (rfns_iteratedCovGrad_appCcRS_diagonalProductGrid_rankLeft_le
    ...
    (rsDomDomCongrSection (I := I) (M := M) g₀ 2 6 bdSigmaE0
      (slotExtendIter (I := I) (M := M) g₀ 0 4 2
        (bdRicciFoldWeightA (I := I) (M := M) g₀ T +
          bdRicciFoldWeightB (I := I) (M := M) g₀ T))) x) ?_
  have hcell : ∀ i' ∈ Finset.range (i + 1),
      riemannianFiberNormSq (I := I) (M := M) g₀ 6 (2 + i') x
          ((iteratedCovGrad (I := I) g₀ 6 2 i'
            (bdPairTraceOp (I := I) (M := M) g₀ g₁)).toSection x) *
...
theorem exists_ricciArmRicciFoldRemainderField_realizedFam_l2JetWindow
  ...
  obtain ⟨Csob, hCsob_nn, hTcapAll⟩ :=
    bdExists_rfnsT_zero_jet_cap (I := I) (M := M) g₀ a ha_super
  obtain ⟨C, hC_nn, hpt⟩ := bdRicciFoldComposite_pointwise_gridWindow (I := I) (M := M)
