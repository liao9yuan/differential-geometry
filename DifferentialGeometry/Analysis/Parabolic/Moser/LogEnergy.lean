...
theorem log_energy_differential_of_supersolution
  ...
  have hcross_cont : Continuous (fun x : M =>
    ...
    have hinner := contMDiff_g_inner_of_smooth_sections (I := I) (M := M) g
      (grad_g (I := I) g cutoff.smooth) (grad_g (I := I) g w.smooth)
    exact cutoff.smooth.continuous.mul (by simpa only [grad_g_apply] using hinner.continuous)
  have hlap_cont : Continuous (fun x : M => Δ_g (I := I) g w.smooth x) :=
    (Δ_g_contMDiff (I := I) g w.smooth).continuous
  ...
  have hlap_int : Integrable (fun x : M =>
      cutoff.toFun x ^ 2 * Δ_g (I := I) g w.smooth x) μ :=
  ...
  have htime_int : Integrable (fun x : M =>
    ...
    ((cutoff.smooth.continuous.pow 2).mul htime_cont).integrable_of_hasCompactSupport
      (HasCompactSupport.of_compactSpace _)
  have hpointwise : ∀ x : M,
      cutoff.toFun x ^ 2 * Δ_g (I := I) g w.smooth x +
          cutoff.toFun x ^ 2 *
            g.inner x
              (gradFun (I := I) g w.toFun x)
              (gradFun (I := I) g w.toFun x) ≤
    ...
    change cutoff.toFun x ^ 2 *
        (Δ_g (I := I) g w.smooth x +
          g.inner x
            (gradientFun (I := I) g (fun y => Real.log (u t y)) x)
            (gradientFun (I := I) g (fun y => Real.log (u t y)) x)) ≤
      cutoff.toFun x ^ 2 * deriv (fun s => Real.log (u s x)) t at hmul
    change cutoff.toFun x ^ 2 * Δ_g (I := I) g w.smooth x +
        cutoff.toFun x ^ 2 *
          g.inner x
            (gradientFun (I := I) g (fun y => Real.log (u t y)) x)
            (gradientFun (I := I) g (fun y => Real.log (u t y)) x) ≤
    ...
    all_goals ring
  have htime_le :
      (∫ x, cutoff.toFun x ^ 2 * Δ_g (I := I) g w.smooth x ∂μ) +
          ∫ x, cutoff.toFun x ^ 2 *
            g.inner x
              (gradFun (I := I) g w.toFun x)
              (gradFun (I := I) g w.toFun x) ∂μ ≤
    ...
    have hgreen' :
        2 * ∫ x, cutoff.toFun x *
            g.inner x
              (gradFun (I := I) g cutoff.toFun x)
              (gradFun (I := I) g w.toFun x) ∂μ =
          -∫ x, cutoff.toFun x ^ 2 * Δ_g (I := I) g w.smooth x ∂μ := by
      calc
        2 * ∫ x, cutoff.toFun x *
              g.inner x
                (gradFun (I := I) g cutoff.toFun x)
        ...
        _ = -∫ x, cutoff.toFun x ^ 2 * Δ_g (I := I) g w.smooth x ∂μ := by
              simpa only [μ, test, smoothScalarSlice_toFun] using hgreen
    linarith
  have hcross :
      2 * ∫ x, cutoff.toFun x *
          g.inner x
