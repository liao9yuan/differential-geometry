...
The proof of `nlDuhamel_dist_le` proceeds by combining the two Duhamel
integrals into a single integral, pushing the difference through the
linear operator `S (t - τ)`, and bounding the integrand pointwise by the
constant `L * C` via the contraction bound of the semigroup and the
Lipschitz bound of `N`. -/
...
private lemma continuousOn_semigroup_apply (S : BoundedC0Semigroup X)
  ...
  -- The clipped map `τ ↦ S (max 0 (t - τ)) (w τ)` is continuous on `ℝ`.
  ...
  -- On `uIcc 0 t` the clipped map agrees with `τ ↦ S (t - τ) (w τ)`.
...
theorem nlDuhamel_dist_le (S : BoundedC0Semigroup X) (u₀ : X)
  ...
  -- The two forcing terms `N ∘ u` and `N ∘ v` are continuous.
  have hNu : Continuous (fun τ => N (u τ)) := hN.continuous.comp hu
  have hNv : Continuous (fun τ => N (v τ)) := hN.continuous.comp hv
  -- Reduce the difference of Duhamel maps to a difference of integrals.
  have h_diff_eq :
    ...
    abel
  -- Interval-integrability of the two integrands.
  ...
  -- Push the difference through the linear operator `S (t - τ)`.
  have h_integrand_eq :
      (∫ τ in (0 : ℝ)..t,
          (S (t - τ) (N (u τ)) - S (t - τ) (N (v τ)))) =
    ...
    change S (t - τ) (N (u τ)) - S (t - τ) (N (v τ)) =
      S (t - τ) (N (u τ) - N (v τ))
    rw [(S (t - τ)).map_sub]
  -- The combined integrand `τ ↦ N (u τ) - N (v τ)` is continuous.
  have hw_cont : Continuous (fun τ => N (u τ) - N (v τ)) :=
    hNu.sub hNv
  -- Norm bound for the integral.
  have h_norm_le :
      ‖∫ τ in (0 : ℝ)..t, S (t - τ) (N (u τ) - N (v τ))‖ ≤
        ∫ τ in (0 : ℝ)..t, ‖S (t - τ) (N (u τ) - N (v τ))‖ :=
    intervalIntegral.norm_integral_le_integral_norm ht
  -- Interval-integrability of the norm integrand: continuity on `uIcc`.
  ...
  -- Evaluate the integral of the constant.
  have h_const_eval :
      (∫ _ in (0 : ℝ)..t, (L : ℝ) * C) = (L : ℝ) * C * t := by
    rw [intervalIntegral.integral_const]
    rw [sub_zero, smul_eq_mul, mul_comm]
  -- Assemble the chain of estimates.
