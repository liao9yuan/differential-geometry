...
with its `L²` time derivative `∂_t u ∈ L²([0,T]; Hᵃ)`.  We therefore take
  ...
  `MaxRegSolutionSpace h_uniform a T := H¹([0,T]; Hᵃ)`,
...
* `MaxRegSolutionSpace h_uniform a T` — the solution space `E_T`, `= H¹([0,T];
  Hᵃ)`.
* `maxRegHomogeneousSolField h_uniform a u₀` — the `H^{a+2}`-valued field
  `t ↦ e^{tΔ_∇} u₀`, an element of `L²([0,T]; H^{a+2})`.
* `maxRegHomogeneousDerivField h_uniform a u₀` — its time derivative
  `t ↦ Δ_∇ e^{tΔ_∇} u₀`, an element of `L²([0,T]; Hᵃ)`.
* `maxRegHomogeneous h_uniform a u₀` — the homogeneous part `t ↦ e^{tΔ_∇} u₀` as
  an element of `E_T`.
* `maxRegDuhamelSolField h_uniform a hT hT1 u₀ g` — the `H^{a+2}`-valued field of
  the Duhamel image.
* `maxRegDuhamelMap h_uniform a hT hT1 u₀ g` — the affine Duhamel map
...
abbrev MaxRegSolutionSpace (h_uniform : uniformTensorChartSobolevBound g r s)
    (a : ℝ) (T : ℝ) : Type _ :=
...
def homModeCoeff (h_uniform : uniformTensorChartSobolevBound g r s) {a : ℝ}
    {T : ℝ}
...
def homDerivModeCoeff (h_uniform : uniformTensorChartSobolevBound g r s) {a : ℝ}
    {T : ℝ}
    (u₀ : tensorHs (I := I) (M := M) g r s h_uniform (a + 2))
    (i : TensorEigenIdx (I := I) (M := M) g r s) : timeL2 ℝ T :=
  TimeSobolev.ofContinuousOn (X := ℝ) (T := T)
...
theorem norm_homModeCoeff_sq_le (hT : 0 ≤ T)
    (u₀ : tensorHs (I := I) (M := M) g r s h_uniform (a + 2))
    (i : TensorEigenIdx (I := I) (M := M) g r s) :
    ‖homModeCoeff (I := I) (M := M) h_uniform (a := a) (T := T) u₀ i‖ ^ 2 ≤
      T * (u₀.coeff i) ^ 2 := by
  rw [show homModeCoeff (I := I) (M := M) h_uniform (a := a) (T := T) u₀ i =
        TimeSobolev.ofContinuousOn (X := ℝ) (T := T)
          (f := fun t => Real.exp (-(TensorEigenIdx.lambda (I := I) (M := M) i) * t) *
            u₀.coeff i)
...
theorem weighted_homModeCoeff_le (hT : 0 ≤ T)
  ...
  have hbase := norm_homModeCoeff_sq_le (I := I) (M := M) (a := a) (T := T) hT u₀ i
...
theorem summable_homModeCoeff (hT : 0 ≤ T)
    ...
    Summable (fun i => tensorSobolevWeight (I := I) (M := M) i (a + 2) *
      ‖homModeCoeff (I := I) (M := M) h_uniform (a := a) (T := T) u₀ i‖ ^ 2) := by
...
theorem weighted_homDerivModeCoeff_le (hT : 0 ≤ T)
    ...
    tensorSobolevWeight (I := I) (M := M) i a *
        ‖homDerivModeCoeff (I := I) (M := M) h_uniform (a := a) (T := T) u₀ i‖ ^ 2 ≤
      T * (tensorSobolevWeight (I := I) (M := M) i (a + 2) * (u₀.coeff i) ^ 2) := by
  ...
  have hderiv_eq := homDerivModeCoeff_eq_smul (I := I) (M := M) (a := a) (T := T) u₀ i
  have hnorm : ‖homDerivModeCoeff (I := I) (M := M) h_uniform (a := a) (T := T) u₀ i‖ ^ 2 =
      lam ^ 2 *
        ‖homModeCoeff (I := I) (M := M) h_uniform (a := a) (T := T) u₀ i‖ ^ 2 := by
    rw [hderiv_eq, norm_smul, mul_pow, Real.norm_eq_abs, sq_abs, neg_pow,
      neg_one_sq, one_mul]
  -- The homogeneous-flow coordinate bound, scaled by `(1 + λᵢ)ᵃ · λᵢ²`.
  have hmode := norm_homModeCoeff_sq_le (I := I) (M := M) (a := a) (T := T) hT u₀ i
  ...
  calc tensorSobolevWeight (I := I) (M := M) i a *
    ...
    _ ≤ tensorSobolevWeight (I := I) (M := M) i a *
        ...
        have hmode_nn : 0 ≤ ‖homModeCoeff (I := I) (M := M) h_uniform
...
theorem summable_homDerivModeCoeff (hT : 0 ≤ T)
    ...
    Summable (fun i => tensorSobolevWeight (I := I) (M := M) i a *
      ‖homDerivModeCoeff (I := I) (M := M) h_uniform (a := a) (T := T) u₀ i‖ ^ 2) := by
  have hdom : Summable (fun i => T *
      (tensorSobolevWeight (I := I) (M := M) i (a + 2) * (u₀.coeff i) ^ 2)) :=
    u₀.weighted_summable.mul_left T
  refine Summable.of_nonneg_of_le (fun i => ?_) (fun i => ?_) hdom
...
def maxRegHomogeneousSolField (h_uniform : uniformTensorChartSobolevBound g r s)
    (a : ℝ) (T : ℝ)
    (u₀ : tensorHs (I := I) (M := M) g r s h_uniform (a + 2)) :
    timeL2 (tensorHs (I := I) (M := M) g r s h_uniform (a + 2)) T :=
  timeL2OfModes (I := I) (M := M) (σ := a + 2) h_uniform
    (fun i => homModeCoeff (I := I) (M := M) h_uniform (a := a) (T := T) u₀ i)
...
theorem maxRegHomogeneousSolField_timeModeCoeff (hT : 0 ≤ T)
    ...
    timeModeCoeff (I := I) (M := M)
        (maxRegHomogeneousSolField (I := I) (M := M) h_uniform a T u₀) i =
      homModeCoeff (I := I) (M := M) h_uniform (a := a) (T := T) u₀ i :=
  timeL2OfModes_timeModeCoeff (I := I) (M := M) (σ := a + 2)
    (h_uniform := h_uniform) _
...
e^{tΔ_∇} u₀`. -/
def maxRegHomogeneousDerivField (h_uniform : uniformTensorChartSobolevBound g r s)
    (a : ℝ) (T : ℝ)
    (u₀ : tensorHs (I := I) (M := M) g r s h_uniform (a + 2)) :
    timeL2 (tensorHs (I := I) (M := M) g r s h_uniform a) T :=
  timeL2OfModes (I := I) (M := M) (σ := a) h_uniform
    (fun i => homDerivModeCoeff (I := I) (M := M) h_uniform (a := a) (T := T) u₀ i)
...
theorem maxRegHomogeneousDerivField_timeModeCoeff (hT : 0 ≤ T)
    ...
    (i : TensorEigenIdx (I := I) (M := M) g r s) :
    timeModeCoeff (I := I) (M := M)
        (maxRegHomogeneousDerivField (I := I) (M := M) h_uniform a T u₀) i =
      homDerivModeCoeff (I := I) (M := M) h_uniform (a := a) (T := T) u₀ i :=
...
def maxRegHomogeneous (h_uniform : uniformTensorChartSobolevBound g r s)
    (a : ℝ) (T : ℝ)
    (u₀ : tensorHs (I := I) (M := M) g r s h_uniform (a + 2)) :
    MaxRegSolutionSpace (I := I) (M := M) h_uniform a T :=
  TimeSobolev.timeH1.mk
    (tensorHsInclusion (I := I) (M := M) h_uniform
      (show a ≤ a + 2 by linarith) u₀)
    (maxRegHomogeneousDerivField (I := I) (M := M) h_uniform a T u₀)

/-- The initial value of the homogeneous part is `u₀`, included into `Hᵃ` via the
spectral inclusion `H^{a+2} ↪ Hᵃ`. -/
@[simp] theorem maxRegHomogeneous_init
    (u₀ : tensorHs (I := I) (M := M) g r s h_uniform (a + 2)) :
    (maxRegHomogeneous (I := I) (M := M) h_uniform a T u₀).init =
      tensorHsInclusion (I := I) (M := M) h_uniform
        (show a ≤ a + 2 by linarith) u₀ :=
  rfl
...
theorem maxRegHomogeneous_trace0
    ...
    TimeSobolev.timeH1.trace0 _ T
        (maxRegHomogeneous (I := I) (M := M) h_uniform a T u₀) =
      tensorHsInclusion (I := I) (M := M) h_uniform
        (show a ≤ a + 2 by linarith) u₀ :=
  rfl
...
@[simp] theorem maxRegHomogeneous_deriv
    (u₀ : tensorHs (I := I) (M := M) g r s h_uniform (a + 2)) :
    (maxRegHomogeneous (I := I) (M := M) h_uniform a T u₀).deriv =
      maxRegHomogeneousDerivField (I := I) (M := M) h_uniform a T u₀ :=
  rfl
...
theorem maxRegHomogeneousDerivField_eq_scaleLaplacian (hT : 0 ≤ T)
    ...
    maxRegHomogeneousDerivField (I := I) (M := M) h_uniform a T u₀ =
      timeScaleLaplacian (I := I) (M := M) h_uniform a
        (maxRegHomogeneousSolField (I := I) (M := M) h_uniform a T u₀) := by
...
theorem maximalRegularityDerivField_add (hT : 0 ≤ T)
    ...
    maximalRegularityDerivField (I := I) (M := M) h_uniform a hT (f + f') =
      maximalRegularityDerivField (I := I) (M := M) h_uniform a hT f +
        maximalRegularityDerivField (I := I) (M := M) h_uniform a hT f' := by
  refine timeModeCoeff_injective (I := I) (M := M) (fun i => ?_)
  rw [maximalRegularityDerivField_timeModeCoeff (I := I) (M := M) (a := a)
      hT (f + f') i,
    timeModeCoeff_add (I := I) (M := M),
    maximalRegularityDerivField_timeModeCoeff (I := I) (M := M) (a := a)
      hT f i,
    maximalRegularityDerivField_timeModeCoeff (I := I) (M := M) (a := a)
      hT f' i]
...
theorem maximalRegularityDerivField_smul (hT : 0 ≤ T) (c : ℝ)
    ...
    maximalRegularityDerivField (I := I) (M := M) h_uniform a hT (c • f) =
      c • maximalRegularityDerivField (I := I) (M := M) h_uniform a hT f := by
  refine timeModeCoeff_injective (I := I) (M := M) (fun i => ?_)
  rw [maximalRegularityDerivField_timeModeCoeff (I := I) (M := M) (a := a)
      hT (c • f) i,
...
theorem maximalRegularityOp_add (hT : 0 < T) (hT1 : T ≤ 1)
    (f f' : timeL2 (tensorHs (I := I) (M := M) g r s h_uniform a) T) :
    maximalRegularityOp (I := I) (M := M) h_uniform a hT hT1 (f + f') =
      maximalRegularityOp (I := I) (M := M) h_uniform a hT hT1 f +
        maximalRegularityOp (I := I) (M := M) h_uniform a hT hT1 f' := by
  ...
  · rw [TimeSobolev.timeH1.init_add]
    change (0 : tensorHs (I := I) (M := M) g r s h_uniform a) =
      (0 : tensorHs (I := I) (M := M) g r s h_uniform a) +
        (0 : tensorHs (I := I) (M := M) g r s h_uniform a)
    rw [add_zero]
...
theorem maximalRegularityOp_smul (hT : 0 < T) (hT1 : T ≤ 1) (c : ℝ)
    (f : timeL2 (tensorHs (I := I) (M := M) g r s h_uniform a) T) :
    maximalRegularityOp (I := I) (M := M) h_uniform a hT hT1 (c • f) =
      c • maximalRegularityOp (I := I) (M := M) h_uniform a hT hT1 f := by
  refine TimeSobolev.timeH1.ext ?_ ?_
  · rw [TimeSobolev.timeH1.init_smul]
    change (0 : tensorHs (I := I) (M := M) g r s h_uniform a) =
      c • (0 : tensorHs (I := I) (M := M) g r s h_uniform a)
    rw [smul_zero]
...
theorem maximalRegularityOp_sub (hT : 0 < T) (hT1 : T ≤ 1)
    ...
    maximalRegularityOp (I := I) (M := M) h_uniform a hT hT1 (f - f') =
      maximalRegularityOp (I := I) (M := M) h_uniform a hT hT1 f -
        maximalRegularityOp (I := I) (M := M) h_uniform a hT hT1 f' := by
  have hadd := maximalRegularityOp_add (I := I) (M := M) (h_uniform := h_uniform)
    (a := a) hT hT1 (f - f') f'
...
def maxRegDuhamelSolField (h_uniform : uniformTensorChartSobolevBound g r s)
    (a : ℝ) {T : ℝ} (hT : 0 < T) (_hT1 : T ≤ 1)
    (u₀ : tensorHs (I := I) (M := M) g r s h_uniform (a + 2))
    (gforce : timeL2 (tensorHs (I := I) (M := M) g r s h_uniform a) T) :
    timeL2 (tensorHs (I := I) (M := M) g r s h_uniform (a + 2)) T :=
  maxRegHomogeneousSolField (I := I) (M := M) h_uniform a T u₀ +
    maximalRegularitySolField (I := I) (M := M) h_uniform a hT.le gforce
...
(N ∘ u)`. -/
def maxRegDuhamelMap (h_uniform : uniformTensorChartSobolevBound g r s)
    (a : ℝ) {T : ℝ} (hT : 0 < T) (hT1 : T ≤ 1)
    (u₀ : tensorHs (I := I) (M := M) g r s h_uniform (a + 2))
    (gforce : timeL2 (tensorHs (I := I) (M := M) g r s h_uniform a) T) :
    MaxRegSolutionSpace (I := I) (M := M) h_uniform a T :=
  maxRegHomogeneous (I := I) (M := M) h_uniform a T u₀ +
    maximalRegularityOp (I := I) (M := M) h_uniform a hT hT1 gforce
...
@[simp] theorem maxRegDuhamelMap_init (hT : 0 < T) (hT1 : T ≤ 1)
    (u₀ : tensorHs (I := I) (M := M) g r s h_uniform (a + 2))
    (gforce : timeL2 (tensorHs (I := I) (M := M) g r s h_uniform a) T) :
    (maxRegDuhamelMap (I := I) (M := M) h_uniform a hT hT1 u₀ gforce).init =
      tensorHsInclusion (I := I) (M := M) h_uniform
  ...
  rw [show (maximalRegularityOp (I := I) (M := M) h_uniform a hT hT1 gforce).init =
        (0 : tensorHs (I := I) (M := M) g r s h_uniform a) from rfl, add_zero]
...
@[simp] theorem maxRegDuhamelMap_deriv (hT : 0 < T) (hT1 : T ≤ 1)
    (u₀ : tensorHs (I := I) (M := M) g r s h_uniform (a + 2))
    (gforce : timeL2 (tensorHs (I := I) (M := M) g r s h_uniform a) T) :
    (maxRegDuhamelMap (I := I) (M := M) h_uniform a hT hT1 u₀ gforce).deriv =
      maxRegHomogeneousDerivField (I := I) (M := M) h_uniform a T u₀ +
        maximalRegularityDerivField (I := I) (M := M) h_uniform a hT.le gforce := by
...
theorem maxRegDuhamelMap_timeDeriv_eq (hT : 0 < T) (hT1 : T ≤ 1)
  ...
  rw [maxRegHomogeneousDerivField_eq_scaleLaplacian (I := I) (M := M) (a := a)
    (T := T) hT.le u₀]
  -- The Duhamel summand: `∂_t (Duhamel g) = Δ_∇ (sol field) + g`.
  have hsolve := maximalRegularityOp_solves (I := I) (M := M) (h_uniform := h_uniform)
    (a := a) hT hT1 gforce
  rw [maximalRegularityOp_timeDeriv (I := I) (M := M) (h_uniform := h_uniform)
...
theorem maxRegDuhamelMap_dist_le (hT : 0 < T) (hT1 : T ≤ 1)
  ...
  have hdiff : maxRegDuhamelMap (I := I) (M := M) h_uniform a hT hT1 u₀ gforce -
        maxRegDuhamelMap (I := I) (M := M) h_uniform a hT hT1 u₀ gforce' =
      maximalRegularityOp (I := I) (M := M) h_uniform a hT hT1
        (gforce - gforce') := by
    rw [maximalRegularityOp_sub (I := I) (M := M) (h_uniform := h_uniform)
      (a := a) hT hT1 gforce gforce']
    change (maxRegHomogeneous (I := I) (M := M) h_uniform a T u₀ +
          maximalRegularityOp (I := I) (M := M) h_uniform a hT hT1 gforce) -
        (maxRegHomogeneous (I := I) (M := M) h_uniform a T u₀ +
          maximalRegularityOp (I := I) (M := M) h_uniform a hT hT1 gforce') =
      maximalRegularityOp (I := I) (M := M) h_uniform a hT hT1 gforce -
    ...
    abel
  rw [hdiff]
  exact maximalRegularityOp_norm_le (I := I) (M := M) (h_uniform := h_uniform)
    (a := a) hT hT1 (gforce - gforce')
...
theorem maxRegDuhamelMap_lipschitzWith (hT : 0 < T) (hT1 : T ≤ 1)
  ...
  have h := maxRegDuhamelMap_dist_le (I := I) (M := M) (h_uniform := h_uniform)
