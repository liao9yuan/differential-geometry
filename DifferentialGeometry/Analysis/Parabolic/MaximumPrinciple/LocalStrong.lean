...
private theorem exists_abs_laplacian_le_on_chartClosedAnnulus
    ...
    ∃ B : Real, 0 ≤ B ∧ ∀ x ∈ chartClosedAnnulus (I := I) c r R,
      |Δ_g (I := I) g (compactCoordRadiusSq_contMDiff (I := I) b) x| ≤ B := by
  let f : M → Real := compactCoordRadiusSq (I := I) b
  let q : M → Real := fun x => |Δ_g (I := I) g
    (compactCoordRadiusSq_contMDiff (I := I) b) x|
  have hK := chartClosedAnnulus_isCompact (I := I) b (r := r) (R := R)
    (hR.le.trans b.rIn_lt_rOut.le)
  have hqcont : Continuous q := by
    exact (continuous_abs.comp
      (Δ_g_contMDiff (I := I) g
        (compactCoordRadiusSq_contMDiff (I := I) b)).continuous)
...
private theorem delta_exp_neg_mul
    ...
    (hf : ContMDiff I 𝓘(Real, Real) ∞ f)
    (alpha : Real) (x : M) :
    Δ_g (I := I) g (exp_neg_mul_contMDiff (I := I) hf alpha) x =
      Real.exp (-alpha * f x) *
        (-alpha * Δ_g (I := I) g hf x + alpha ^ 2 *
  ...
  have hfLap : laplacianAt (I := I) G 0 f x = Δ_g (I := I) g hf x := by
  ...
  have hchain' :
      Δ_g (I := I) g hcompSmooth x =
        deriv phi (f x) * Δ_g (I := I) g hf x +
          deriv (deriv phi) (f x) *
            g.inner x (gradientFun (I := I) g f x)
              (gradientFun (I := I) g f x) := by
  ...
  change Δ_g (I := I) g hcompSmooth x = _
...
private theorem barrierPhase_parabolicOperator
    ...
    parabolicOperatorWithDrift (I := I) (fixedMetricFamily (I := I) g) T
      ...
      2 * kappa * (t - tau) -
        Δ_g (I := I) g (barrierRadius_contMDiff (I := I) b R) x := by
  ...
  have hadd := Δ_g_add (I := I) g
    (barrierRadius_contMDiff (I := I) b R)
    (contMDiff_const : ContMDiff I 𝓘(Real, Real) ∞
      (fun _ : M => kappa * (t - tau) ^ 2)) x
  have hconst := Δ_g_const (I := I) g (kappa * (t - tau) ^ 2) x
  have hadd' : Δ_g (I := I) g hslice x =
      Δ_g (I := I) g (barrierRadius_contMDiff (I := I) b R) x := by
    calc
      Δ_g (I := I) g hslice x =
          Δ_g (I := I) g (barrierRadius_contMDiff (I := I) b R) x +
            Δ_g (I := I) g
              (contMDiff_const : ContMDiff I 𝓘(Real, Real) ∞
                (fun _ : M => kappa * (t - tau) ^ 2)) x := by
                  simpa using hadd
      _ = Δ_g (I := I) g (barrierRadius_contMDiff (I := I) b R) x := by
            rw [hconst, add_zero]
  rw [hadd']
  simp
...
private theorem fixed_metric_local_positivity
    ...
    (hlap_bound : ∀ x ∈ chartClosedAnnulus (I := I) c r Rdist,
      |Δ_g (I := I) g (compactCoordRadiusSq_contMDiff (I := I) b) x| ≤ B)
