...
theorem parallelProperCone_mem_innerDualZeroFace_of_terminal_eq_zero
  ...
  have hfixed_face := properCone_mem_innerDualZeroFace_of_terminal_eq_zero
    (I := I) G hT (C x₀) (transportedSectionFamily F P x₀ u) y hy V
      hsol htau hfixed_mem hgrad_cont hlaplacian_cont L hV hzero
  intro t ht x
  have htransported :=
    (hC.transport_mem_innerDualZeroFace_iff F x₀ x y
