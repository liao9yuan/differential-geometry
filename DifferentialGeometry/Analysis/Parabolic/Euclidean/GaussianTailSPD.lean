...
/-!
# Uniform tails for positive-definite Gaussians

This module shows that normalized centered Gaussians defined by arbitrary
positive-definite quadratic forms have one common radial tail bound when the
radius is measured in the same quadratic form.
-/
...
private theorem stdGauss_tail :
  ...
  have ht' : Tendsto (fun R ↦ ν (s R)) atTop (nhds 0) := by
    simpa only [Function.comp_apply, hinter, measure_empty] using ht
  have heq :
      (fun R ↦ ν (s R)) =
        (fun R : Real ↦
          ∫⁻ y : Eucl (n := n) in {y | R < ‖y‖},
...
omit [Nonempty n] in
/-- A normalized positive-definite Gaussian has the same intrinsic radial tail
as the standard centered Gaussian. -/
theorem gaussSPDTail_eq
  ...
  let d : Real := LinearMap.det
    ((L : Eucl (n := n) →L[Real] Eucl (n := n)) :
      Eucl (n := n) →ₗ[Real] Eucl (n := n))
  ...
  have hdetpos : 0 < LinearMap.det
      ((L : Eucl (n := n) →L[Real] Eucl (n := n)) :
        Eucl (n := n) →ₗ[Real] Eucl (n := n)) := by
    simpa only [d] using hdpos
  have hdetinvpos : 0 < (LinearMap.det
      ((L : Eucl (n := n) →L[Real] Eucl (n := n)) :
        Eucl (n := n) →ₗ[Real] Eucl (n := n)))⁻¹ :=
    inv_pos.mpr hdetpos
  have hmap : Measure.map
    ...
    simpa only [d, abs_of_pos hdetinvpos] using
  ...
  have hset :
      {x : Eucl (n := n) | R < Real.sqrt
        ...
        L ⁻¹' s := by
    ext x
    simp only [mem_setOf_eq, mem_preimage, s]
...
/-- For every positive error threshold, one intrinsic radius controls the
tail of every normalized positive-definite Gaussian in the fixed dimension. -/
