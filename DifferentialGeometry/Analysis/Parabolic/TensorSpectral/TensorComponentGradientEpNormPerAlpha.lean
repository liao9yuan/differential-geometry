...
/-! ## Unconditional Riemannian-norm twin of the G5 headline

The locality-conditioned headline
`exists_eLpNorm_sqrt_g_inner_gradFun_tensorChartComponentScalar_le_const_mul_h1Norm`
bounds the `L²` seminorm of `√ g.inner b (∇u_α b) (∇u_α b)` (a quantity that does
not reference the tensor-bundle fibre norm). The locality predicate
`HasLocallyConstantChartAt` enters that proof only through the two model-norm
slot-correction operator-norm bounds and the trivialisation bridge inequality
used to control the chart-Christoffel correction atom; the raw-component atom and
the model-norm covariant-derivative atom are already locality-free.

This twin reproves the **same conclusion** with the locality predicate removed.
...
eLpNorm
  ...
  ENNReal.ofReal C * ‖S‖₊,
```
where `u_α := tensorChartComponentScalar g r s S.toCcTensor α Idx Jdx`. This is
the locality-free counterpart of
`exists_eLpNorm_sqrt_g_inner_gradFun_tensorChartComponentScalar_le_const_mul_h1Norm`:
the `HasLocallyConstantChartAt` predicate is dropped and the Christoffel
slot-correction atom is routed through its intrinsic Riemannian-fibre-norm
operator bounds. -/
theorem exists_eLpNorm_sqrt_g_inner_gradFun_tensorChartComponentScalar_le_const_mul_h1Norm_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ) (α : M) :
    letI : Bundle.RiemannianBundle (fun b : M => TensorRSSpace r s I b) :=
      Tensor0SBundle.tensorRS_riemannianBundle (I := I) (M := M) g r s
    ∃ C : ℝ, 0 ≤ C ∧
