...
`tensorHs g r s h_uniform σ` is a one-field structure carrying the
...
* `tensorHs g r s h_uniform σ` — the spectral `Hˢ` Sobolev space, a
  real Hilbert space.
...
def tensorL2Coeff {g : SmoothRiemannianMetric I M} {r s : ℕ}
    ...
    ℝ :=
  (tensorResolventHilbertEigenbasisSigma (I := I) (M := M) h_uniform).repr T i

/-- The eigenbasis coordinate equals the inner product `⟪bᵢ, T⟫`. -/
lemma tensorL2Coeff_eq_inner {g : SmoothRiemannianMetric I M} {r s : ℕ}
    (h_uniform : uniformTensorChartSobolevBound g r s)
    (T : TensorL2 r s g) (i : TensorEigenIdx (I := I) (M := M) g r s) :
    tensorL2Coeff (I := I) (M := M) h_uniform T i =
      ⟪tensorResolventHilbertEigenbasisSigma (I := I) (M := M) h_uniform i,
        T⟫_ℝ := by
...
lemma tensorL2Coeff_summable_sq {g : SmoothRiemannianMetric I M} {r s : ℕ}
    ...
    Summable (fun i : TensorEigenIdx (I := I) (M := M) g r s =>
      (tensorL2Coeff (I := I) (M := M) h_uniform T i) ^ 2) := by
  have h := tensorSummable_basis_coeff_sq (I := I) (M := M) h_uniform T
  have h_eq : (fun i : TensorEigenIdx (I := I) (M := M) g r s =>
        ‖⟪tensorResolventHilbertEigenbasisSigma
            (I := I) (M := M) h_uniform i, T⟫_ℝ‖ ^ 2) =
      (fun i => (tensorL2Coeff (I := I) (M := M) h_uniform T i) ^ 2) := by
    funext i
...
lemma tensorL2Coeff_add {g : SmoothRiemannianMetric I M} {r s : ℕ}
    ...
    tensorL2Coeff (I := I) (M := M) h_uniform (S + T) i =
      tensorL2Coeff (I := I) (M := M) h_uniform S i +
        tensorL2Coeff (I := I) (M := M) h_uniform T i := by
  unfold tensorL2Coeff
  rw [map_add]
  rfl
...
lemma tensorL2Coeff_smul {g : SmoothRiemannianMetric I M} {r s : ℕ}
    ...
    (i : TensorEigenIdx (I := I) (M := M) g r s) :
    tensorL2Coeff (I := I) (M := M) h_uniform (c • T) i =
      c * tensorL2Coeff (I := I) (M := M) h_uniform T i := by
  unfold tensorL2Coeff
  rw [map_smul]
  rfl
...
structure tensorHs (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (_h_uniform : uniformTensorChartSobolevBound g r s) (σ : ℝ) where
...
@[ext] lemma ext {S T : tensorHs (I := I) (M := M) g r s h_uniform σ}
    (h : S.coeff = T.coeff) : S = T := by
  cases S; cases T; cases h; rfl

/-! ### Additive-group and module structure -/

instance : Zero (tensorHs (I := I) (M := M) g r s h_uniform σ) where
  zero := ⟨fun _ => 0, by simp⟩

@[simp] lemma zero_coeff :
    (0 : tensorHs (I := I) (M := M) g r s h_uniform σ).coeff =
      (fun _ => 0) := rfl

instance : Add (tensorHs (I := I) (M := M) g r s h_uniform σ) where
  add S T :=
...
instance : Neg (tensorHs (I := I) (M := M) g r s h_uniform σ) where
  neg S :=
...
@[simp] lemma neg_coeff (S : tensorHs (I := I) (M := M) g r s h_uniform σ) :
    (-S).coeff = (fun i => -S.coeff i) := rfl

instance : SMul ℝ (tensorHs (I := I) (M := M) g r s h_uniform σ) where
  smul c S :=
...
@[simp] lemma smul_coeff (c : ℝ)
    (S : tensorHs (I := I) (M := M) g r s h_uniform σ) :
    (c • S).coeff = (fun i => c * S.coeff i) := rfl

instance : AddCommGroup (tensorHs (I := I) (M := M) g r s h_uniform σ) where
...
instance : Module ℝ (tensorHs (I := I) (M := M) g r s h_uniform σ) where
...
def innerFun (S T : tensorHs (I := I) (M := M) g r s h_uniform σ) : ℝ :=
  ∑' i, tensorSobolevWeight (I := I) (M := M) i σ *
    (S.coeff i * T.coeff i)

/-- `innerFun T T` is the weighted sum of squares
`∑ᵢ (1 + λᵢ)^σ (coeff i T)²`. -/
lemma innerFun_self (T : tensorHs (I := I) (M := M) g r s h_uniform σ) :
...
@[reducible] def innerCore :
    InnerProductSpace.Core ℝ
      (tensorHs (I := I) (M := M) g r s h_uniform σ) where
...
lemma inner_self_eq (T : tensorHs (I := I) (M := M) g r s h_uniform σ) :
    (inner ℝ T T : ℝ) =
      ∑' i, tensorSobolevWeight (I := I) (M := M) i σ *
        (T.coeff i) ^ 2 :=
...
/-! ## The rescaling isometry `Hˢ ≃ₗᵢ ℓ²(ι, ℝ)`

The diagonal rescaling `cᵢ ↦ √(1+λᵢ)^σ · cᵢ` carries `Hˢ` bijectively
onto `ℓ²(ι, ℝ)`: weighted square-summability of `cᵢ` is exactly
...
lemma rescale_memℓp (T : tensorHs (I := I) (M := M) g r s h_uniform σ) :
...
`T ↦ (i ↦ √(1+λᵢ)^σ · coeff i)`. -/
def rescaleToL2 (T : tensorHs (I := I) (M := M) g r s h_uniform σ) :
...
@[simp] lemma rescaleFromL2_coeff
    (f : lp (fun _ : TensorEigenIdx (I := I) (M := M) g r s => ℝ) 2)
    (i : TensorEigenIdx (I := I) (M := M) g r s) :
    (rescaleFromL2 (I := I) (M := M) (h_uniform := h_uniform) (σ := σ)
        f).coeff i =
      (Real.sqrt (tensorSobolevWeight (I := I) (M := M) i σ))⁻¹ *
        (f : _ → ℝ) i := rfl
...
identity. -/
def rescaleEquivL2 :
    tensorHs (I := I) (M := M) g r s h_uniform σ ≃ₗᵢ[ℝ]
...
@[simp] lemma rescaleEquivL2_apply
    ...
    (rescaleEquivL2 (I := I) (M := M)
      (h_uniform := h_uniform) (σ := σ) T : _ → ℝ) =
      (fun i => Real.sqrt (tensorSobolevWeight (I := I) (M := M) i σ) *
        T.coeff i) := rfl

/-! ### Completeness of `Hˢ`

Transported from completeness of `ℓ²(ι, ℝ)` along the linear isometric
equivalence `rescaleEquivL2`. -/

instance instCompleteSpace :
    CompleteSpace (tensorHs (I := I) (M := M) g r s h_uniform σ) :=
  (rescaleEquivL2 (I := I) (M := M)
    (g := g) (r := r) (s := s)
    (h_uniform := h_uniform) (σ := σ)).toIsometryEquiv.completeSpace

end tensorHs

/-! ## The inclusion `Hˢ →L[ℝ] TensorL2`

For `σ ≥ 0` the weight `(1 + λᵢ)^σ ≥ 1`, so a weighted-square-summable
coordinate family is in particular square-summable: it is the
coordinate family of an honest `L²` tensor. The resulting map is the
inclusion of `Hˢ` into `TensorL2 r s g`. -/
...
lemma norm_toL2Fun_le (hσ : 0 ≤ σ)
  ...
  have h_l2_sq : ‖toL2Fun (I := I) (M := M) hσ T‖ ^ 2 =
      ∑' i, (T.coeff i) ^ 2 := by
    have h_par := tensorParseval_norm_sq (I := I) (M := M) h_uniform
...
@[simp] lemma tensorHsToL2_apply {g : SmoothRiemannianMetric I M}
    {r s : ℕ} {h_uniform : uniformTensorChartSobolevBound g r s} {σ : ℝ}
    (hσ : 0 ≤ σ) (T : tensorHs (I := I) (M := M) g r s h_uniform σ) :
    tensorHsToL2 (I := I) (M := M) h_uniform hσ T =
...
theorem tensorHsToL2_opNorm_le_one {g : SmoothRiemannianMetric I M}
    {r s : ℕ} {h_uniform : uniformTensorChartSobolevBound g r s} {σ : ℝ}
    (hσ : 0 ≤ σ) :
    ‖tensorHsToL2 (I := I) (M := M) h_uniform hσ‖ ≤ 1 :=
...
theorem tensorHsToL2_injective {g : SmoothRiemannianMetric I M}
    ...
    (hσ : 0 ≤ σ) :
    Function.Injective
      (tensorHsToL2 (I := I) (M := M) h_uniform hσ) := by
  intro S T hST
  ext i
...
At `σ = 0` the weight is `1`, so the rescaling isometry
`H⁰ ≃ₗᵢ ℓ²(ι, ℝ)` is the bare coordinate map, and composing with the
...
def tensorHsZeroEquivL2 {g : SmoothRiemannianMetric I M} {r s : ℕ}
    (h_uniform : uniformTensorChartSobolevBound g r s) :
    tensorHs (I := I) (M := M) g r s h_uniform 0 ≃ₗᵢ[ℝ]
      TensorL2 r s g :=
  (tensorHs.rescaleEquivL2 (I := I) (M := M)
    (g := g) (r := r) (s := s) (h_uniform := h_uniform) (σ := 0)).trans
    (tensorResolventHilbertEigenbasisSigma
      (I := I) (M := M) h_uniform).repr.symm
...
theorem tensorHsZeroEquivL2_symm_coeff {g : SmoothRiemannianMetric I M}
    ...
    (U : TensorL2 r s g)
    (i : TensorEigenIdx (I := I) (M := M) g r s) :
    ((tensorHsZeroEquivL2 (I := I) (M := M) h_uniform).symm U).coeff i =
      tensorL2Coeff (I := I) (M := M) h_uniform U i := by
  have h := tensorHsZeroEquivL2_tensorL2Coeff (I := I) (M := M) h_uniform
    ((tensorHsZeroEquivL2 (I := I) (M := M) h_uniform).symm U) i
  rw [LinearIsometryEquiv.apply_symm_apply] at h
  exact h.symm

/-! ## Membership of finitely-supported coordinate families

A coordinate family with finite support is weighted-square-summable for
every exponent `σ`, hence defines an element of every `Hˢ`. In
particular every single eigenvector `bᵢ` lies in every `Hˢ`. -/
...
@[simp] lemma tensorHsOfFiniteSupport_coeff {g : SmoothRiemannianMetric I M}
    ...
    (f : TensorEigenIdx (I := I) (M := M) g r s → ℝ)
    (hf : (Function.support f).Finite) :
    (tensorHsOfFiniteSupport (I := I) (M := M) h_uniform σ f hf).coeff =
      f := rfl
...
def tensorHsBasisVec {g : SmoothRiemannianMetric I M} {r s : ℕ}
    ...
    tensorHs (I := I) (M := M) g r s h_uniform σ :=
  tensorHsOfFiniteSupport (I := I) (M := M) h_uniform σ
...
@[simp] lemma tensorHsBasisVec_coeff {g : SmoothRiemannianMetric I M}
    ...
    (tensorHsBasisVec (I := I) (M := M) h_uniform σ j).coeff i =
      (if i = j then (1 : ℝ) else 0) := rfl

/-- For `σ ≥ 0`, the inclusion `Hˢ → TensorL2` carries the spectral
basis vector `tensorHsBasisVec h_uniform σ j` to the actual eigenbasis
vector `b j`. -/
theorem tensorHsToL2_tensorHsBasisVec {g : SmoothRiemannianMetric I M}
    {r s : ℕ} {h_uniform : uniformTensorChartSobolevBound g r s} {σ : ℝ}
    (hσ : 0 ≤ σ) (j : TensorEigenIdx (I := I) (M := M) g r s) :
    tensorHsToL2 (I := I) (M := M) h_uniform hσ
        (tensorHsBasisVec (I := I) (M := M) h_uniform σ j) =
      tensorResolventHilbertEigenbasisSigma
        (I := I) (M := M) h_uniform j := by
  classical
  ...
  set b := tensorResolventHilbertEigenbasisSigma
    (I := I) (M := M) h_uniform with hb
  apply b.repr.injective
  ext i
  have h_lhs : (b.repr (tensorHsToL2 (I := I) (M := M) h_uniform hσ
        (tensorHsBasisVec (I := I) (M := M) h_uniform σ j))) i =
      (if i = j then (1 : ℝ) else 0) := by
    have h_coeff : tensorL2Coeff (I := I) (M := M) h_uniform
        (tensorHsToL2 (I := I) (M := M) h_uniform hσ
          (tensorHsBasisVec (I := I) (M := M) h_uniform σ j)) i =
        (if i = j then (1 : ℝ) else 0) := by
