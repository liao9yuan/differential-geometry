...
`eigenvectorChartWeakPartial`, built from `eigenvectorChartPartialLp_unconditional`. -/
def eigenvectorChartWeakPartial_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) : EuclN → ℝ :=
...
/-- Chart-locality-free twin of `eigenvectorChartPartialLp_approx_coeFn`. -/
lemma eigenvectorChartPartialLp_approx_coeFn_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) (n : ℕ) :
...
`eigenvectorChartWeakPartial_approx_hasWeakPartialDeriv`. -/
private lemma eigenvectorChartWeakPartial_approx_hasWeakPartialDeriv_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) (n : ℕ) :
  ...
  refine hasWeakPartialDeriv_congr_ae ?_ ?_ h_weak_smul
  · exact (eigenvectorChartComponentL2_approx_coeFn_unconditional (I := I) (M := M)
      g r s i α P₀ n).symm
  · exact (eigenvectorChartPartialLp_approx_coeFn_unconditional (I := I) (M := M)
      g r s i α P₀ k n).symm

open DifferentialGeometry.PDE.RicciFlow.IntrinsicSpectral in
/-- Chart-locality-free twin of `eigenvectorChartComponent_eLpNorm_tendsto`. -/
private lemma eigenvectorChartComponent_eLpNorm_tendsto_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) :
...
/-- Chart-locality-free twin of `eigenvectorChartPartial_eLpNorm_tendsto`. -/
private lemma eigenvectorChartPartial_eLpNorm_tendsto_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) :
...
theorem eigenvectorChartWeakPartial_hasWeakPartialDeriv_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) :
  ...
  have h_weak : ∀ n,
    ...
    intro n
    simp only [hgApprox_def, huApprox_def]
    exact eigenvectorChartWeakPartial_approx_hasWeakPartialDeriv_unconditional
      ...
      Filter.Tendsto
        (fun n => eLpNorm (fun x => uApprox n x - uLim x) 2
          ((volume : Measure EuclN).restrict
            (chartTargetEuclid (I := I) (M := M) α)))
        atTop (𝓝 0) := by
    have h := eigenvectorChartComponent_eLpNorm_tendsto_unconditional
      (I := I) (M := M) g r s i α P₀
    simp only [huApprox_def, huLim_def]
    exact h
  -- `eLpNorm`-convergence of the chart partials.
      ...
      Filter.Tendsto
        (fun n => eLpNorm (fun x => gApprox n x - gLim x) 2
          ((volume : Measure EuclN).restrict
            (chartTargetEuclid (I := I) (M := M) α)))
        atTop (𝓝 0) := by
    have h := eigenvectorChartPartial_eLpNorm_tendsto_unconditional
...
theorem eigenvectorChartWeakPartial_locally_memLp_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E))
  ...
  let _ := hK
  -- The coercion-to-function is in `MemLp 2` of the chart's `L²` measure.
  have h_memLp : MemLp (eigenvectorChartWeakPartial_unconditional (I := I) (M := M)
      g r s i α P₀ k) 2
      (chartL2Measure (I := I) (M := M) α) := by
