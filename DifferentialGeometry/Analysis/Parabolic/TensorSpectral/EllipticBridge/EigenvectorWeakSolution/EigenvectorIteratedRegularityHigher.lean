...
(tensorResolventL2_isCompactOperator_intrinsic g r s) i` (via
...
`eigenvectorChartIteratedPartial_unconditional g r s i α P₀ (m + 1)
...
`chosenWeakPartial' 2 a (eigenvectorChartIteratedPartial_unconditional … m dirs)`
of the `m`-fold mixed weak partial along `dirs`. -/
theorem eigenvectorChartIteratedPartial_cons_eq_chosenWeakPartial_ae_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) :
    ∀ (m : ℕ) (dirs : Fin m → Fin (Module.finrank ℝ E))
  ...
  | succ m ih =>
      ...
      have h_lhs_unfold :
        ...
        rw [eigenvectorChartIteratedPartial_unconditional_succ, h_last, h_init]
      have h_dirs_unfold :
          eigenvectorChartIteratedPartial_unconditional (I := I) (M := M)
      ...
      have h_final :
        ...
        rw [← h_dirs_unfold]
      calc eigenvectorChartIteratedPartial_unconditional (I := I) (M := M)
              g r s i α P₀ (m + 2) (Fin.cons a dirs)
          = chosenWeakPartial' (d := Module.finrank ℝ E) 2 (dirs (Fin.last m))
              (eigenvectorChartIteratedPartial_unconditional (I := I) (M := M)
                g r s i α P₀ (m + 1) (Fin.cons a (Fin.init dirs))) Ω :=
...
omit [CompleteSpace E] in
/-- Polymorphic per-direction-count engine for the order assembly. For any
`m, j : ℕ` and any `j`-direction multi-index `dirs`, if every chosen
`(j + m)`-mixed partial of the chart component lies in chart-`H^{m+1}` and every
chosen `j`-mixed partial lies in chart-`H¹`, then the `j`-fold mixed weak
partial in directions `dirs` lies in chart-`H^{m+2}`. The headline assembly
...
(tensorResolventL2_isCompactOperator_intrinsic g r s) i` (via
`eigenvectorChartIteratedPartial_unconditional`). For any `m, j : ℕ` and any
...
chart-`H^{m+2}`. -/
private theorem eigenvectorIteratedPartial_memWkp_of_chartH_at_all_indices_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) :
    ∀ (m : ℕ) (j : ℕ) (dirs : Fin j → Fin (Module.finrank ℝ E)),
  ...
  · -- `MemW1p 2` of the `j`-fold mixed partial.
    ...
    exact h
  · -- `chosenWeakPartial' 2 a (j-fold partial in dirs)`
    -- `= eigenvectorChartIteratedPartial_unconditional … (j+1) (Fin.snoc dirs a)`.
    have h_succ_eq :
        eigenvectorChartIteratedPartial_unconditional (I := I) (M := M)
...
`m : ℕ`, the eigenvector chart component lies in chart-`H^{m+2}` on the chart
target, provided:
...
This is the eigenvector/tensor mirror of the scalar campaign's
`chartPushed_memWkp_m_plus_two_of_mthMixed_chart_H_two`. It is the structural
step of the arbitrary-order interior-regularity bootstrap that raises the
...
* chart-`H²` of every chosen `m`-fold mixed weak partial — the per-direction
  chart-`H²` regularity that the order-2 interior engine delivers. -/
theorem eigenvectorChartComponent_memWkp_m_plus_two_of_iterated_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (h_intermediate_w1p :
  ...
  have h_general : ∀ (gap : ℕ) (j : ℕ), j + gap = m →
    ...
    | succ gap ih =>
        ...
        -- Apply the per-direction-count engine at `m_eng := gap + 1`, `j`.
        have h_engine :=
          eigenvectorIteratedPartial_memWkp_of_chartH_at_all_indices_unconditional
  ...
  -- component.
  have h_final := h_general m 0 (by omega) Fin.elim0
  rw [eigenvectorChartIteratedPartial_unconditional_zero] at h_final
  exact h_final
...
mixed partial as a chosen weak partial. -/
