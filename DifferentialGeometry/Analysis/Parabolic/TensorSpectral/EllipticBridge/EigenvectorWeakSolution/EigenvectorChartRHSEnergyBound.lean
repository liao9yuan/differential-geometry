...
The unconditional eigenbasis vector is a unit vector of the orthonormal
eigenbasis of the resolvent eigenspace at `μ := i.fst.val`; since it is non-zero,
`μ` lies in the resolvent's unit interval `(0, 1]`, hence is strictly positive and
bounded above by `1`. -/
...
private lemma crossRightLimitComponent_eLpNorm_le_energy
  ...
  -- The eigen-equation: `TensorH1ComplToTensorL2 (eigenvectorResolvent
  -- …) = μ • φ`.
...
private lemma componentLpLimit_eLpNorm_le_energy
  ...
  have hμ_nn : 0 ≤ i.fst.val :=
    le_of_lt (eigenvalue_mem_Ioc (I := I) (M := M) g r s i).1
  -- `componentLpLimit = μ • tensorL2ChartComponent (φ) α P`; the
