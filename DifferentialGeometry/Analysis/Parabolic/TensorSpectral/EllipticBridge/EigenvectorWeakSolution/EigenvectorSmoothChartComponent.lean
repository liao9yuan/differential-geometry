...
The per-chart component formula presents the chart-`β` component of a per-chart
smooth section as the chart-`β` pushforward of the partition-of-unity weight
times the chart-`β` pushforward of an `if`-gated component sum. This subsection
moves the chart pushforward through the inner finite sum, so the single-term
transport reconciliation applies per component multi-index. -/
...
The following results prove the chart-component equality, re-keyed onto the
intrinsic compact-operator eigenbasis `tensorResolventEigenbasisVec …`.
