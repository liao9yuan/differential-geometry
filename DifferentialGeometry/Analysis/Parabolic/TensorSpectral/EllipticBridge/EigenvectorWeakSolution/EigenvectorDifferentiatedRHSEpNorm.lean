...
numerator `eigenvectorChartRHSDiffNumerator_unconditional`, the chart-locality-
free iterated weak partial `eigenvectorChartIteratedPartial_unconditional`, and
...
def diffNumeratorAggregate_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
...
private lemma eLpNorm_iteratedPartial_succ_le_unconditional
    (a : Fin (Module.finrank ℝ E)) :
    eLpNorm (eigenvectorChartIteratedPartial_unconditional (I := I) (M := M)
...
private lemma eLpNorm_iteratedPartial_le_unconditional :
    eLpNorm (eigenvectorChartIteratedPartial_unconditional (I := I) (M := M)
...
private lemma eLpNorm_chosenWeakPartial_iteratedPartial_succ_le_unconditional
    (a b : Fin (Module.finrank ℝ E)) :
    eLpNorm (chosenWeakPartial' (d := Module.finrank ℝ E) 2 b
        (eigenvectorChartIteratedPartial_unconditional (I := I) (M := M)
          g r s i α P₀ (m + 1) (Fin.cons a (Fin.init l)))
...
private lemma eigenvectorChartRHSDiffNumerator_layerA_eLpNorm_le_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
    ...
    ∃ C : ℝ, 0 ≤ C ∧
      eLpNorm (fun y => ∑ a : Fin (Module.finrank ℝ E),
        ...
        ((volume : Measure EuclN).restrict
          (chartPouKernel (I := I) (M := M) α))
        ≤ ENNReal.ofReal C *
          diffNumeratorAggregate_unconditional (I := I) (M := M)
            g r s i α P₀ m l fChartEffPrev := by
  classical
  set μ : Measure EuclN := (volume : Measure EuclN).restrict
    (chartPouKernel (I := I) (M := M) α) with hμ_def
  set A := diffNumeratorAggregate_unconditional (I := I) (M := M)
  ...
  have h_atom_le : ∀ a : Fin (Module.finrank ℝ E),
      wkpNorm (d := Module.finrank ℝ E) 2 2
          ...
          (chartTargetEuclid (I := I) (M := M) α) ≤ A := by
    intro a
    rw [hA_def, diffNumeratorAggregate_unconditional]
    refine le_trans (Finset.single_le_sum (f := fun a =>
      wkpNorm (d := Module.finrank ℝ E) 2 2
        (eigenvectorChartIteratedPartial_unconditional (I := I) (M := M)
          g r s i α P₀ (m + 1) (Fin.cons a (Fin.init l)))
  ...
  · intro a
    ...
    · intro b
      ...
      refine ⟨C₀, hC₀_nn, le_trans hC₀ ?_⟩
      gcongr
      exact le_trans (eLpNorm_iteratedPartial_succ_le_unconditional
        (I := I) (M := M) g r s i α P₀ m l a) (h_atom_le a)

/-- Chart-locality-free twin of
`eigenvectorChartRHSDiffNumerator_layerB_eLpNorm_le`. -/
private lemma eigenvectorChartRHSDiffNumerator_layerB_eLpNorm_le_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
  ...
  have h_atom_le : ∀ a : Fin (Module.finrank ℝ E),
    ...
    intro a
    rw [hA_def, diffNumeratorAggregate_unconditional, ← hΩ_def]
    refine le_trans (Finset.single_le_sum (f := fun a =>
      wkpNorm (d := Module.finrank ℝ E) 2 2
        (eigenvectorChartIteratedPartial_unconditional (I := I) (M := M)
  ...
  · intro a
    ...
    · intro b
      ...
      refine ⟨C₀, hC₀_nn, le_trans hC₀ ?_⟩
      gcongr
      exact le_trans (eLpNorm_chosenWeakPartial_iteratedPartial_succ_le_unconditional
        (I := I) (M := M) g r s i α P₀ m l a b) (h_atom_le a)

/-- Chart-locality-free twin of
`eigenvectorChartRHSDiffNumerator_layerC_eLpNorm_le`. -/
private lemma eigenvectorChartRHSDiffNumerator_layerC_eLpNorm_le_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
  ...
  refine ⟨C₀, hC₀_nn, le_trans hC₀ ?_⟩
  gcongr
  exact le_trans (eLpNorm_iteratedPartial_le_unconditional
    (I := I) (M := M) g r s i α P₀ m l) h_atom_le

/-- Chart-locality-free twin of
`eigenvectorChartRHSDiffNumerator_layerD_eLpNorm_le`. -/
private lemma eigenvectorChartRHSDiffNumerator_layerD_eLpNorm_le_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
  ...
  have hK_in : chartPouKernel (I := I) (M := M) α ⊆
    ...
    chartPouKernel_subset_chartTargetEuclid (I := I) (M := M) α
  have h_atom_le : eLpNorm fChartEffPrev 2 μ ≤ A := by
    rw [hA_def, diffNumeratorAggregate_unconditional, ← hμ_def]
    exact le_add_self
  obtain ⟨C₀, hC₀_nn, hC₀⟩ := eLpNorm_volume_restrict_contDiffOn_mul_le
    (I := I) (M := M) α
    (densityDerivOnEuclid_contDiffOn (I := I) g α (l (Fin.last m)))
...
private lemma eigenvectorChartRHSDiffNumerator_layerE_eLpNorm_le_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
...
theorem eigenvectorChartRHSDiffNumerator_eLpNorm_le_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
    ...
    (h_prev_zero : ∀ᵐ y ∂((volume : Measure EuclN).restrict
      ...
      y ∉ chartPouKernel (I := I) (M := M) α → fChartEffPrev y = 0) :
    ∃ C : ℝ, 0 ≤ C ∧
      eLpNorm (fun y => eigenvectorChartRHSDiffNumerator_unconditional (I := I) (M := M)
  ...
  have hA_mem : MemLp layerA 2 μ := by
    ...
    have h_coeff : ContDiffOn ℝ ∞
      ...
      have h_eval : ContDiff ℝ ∞
        ...
        (ContinuousLinearMap.apply ℝ ℝ (EuclideanSpace.single b (1 : ℝ))).contDiff
      exact h_eval.contDiffOn.comp h_fderiv (mapsTo_univ _ _)
    have h_atom_mem : MemLp (eigenvectorChartIteratedPartial_unconditional (I := I) (M := M)
        g r s i α P₀ (m + 1) (Fin.cons a (Fin.init l))) 2 μ := by
      have h0 := (h_iter (m + 1) (Fin.cons a (Fin.init l))).le_of_le
        (Nat.zero_le 2)
      rw [MemWkp_zero] at h0
  ...
  have hE_mem : MemLp layerE 2 μ := by
    ...
    exact memLp_volume_compact_contDiffOn_mul (I := I) (M := M) α
      ...
      hK_compact hK_meas hK_in h_chosen_mem
  obtain ⟨CA, hCA_nn, hCA⟩ :=
    eigenvectorChartRHSDiffNumerator_layerA_eLpNorm_le_unconditional
    (I := I) (M := M) g r s i α P₀ m l fChartEffPrev
    (fun a => h_iter (m + 1) (Fin.cons a (Fin.init l)))
  obtain ⟨CB, hCB_nn, hCB⟩ :=
    eigenvectorChartRHSDiffNumerator_layerB_eLpNorm_le_unconditional
    (I := I) (M := M) g r s i α P₀ m l fChartEffPrev
    (fun a => h_iter (m + 1) (Fin.cons a (Fin.init l)))
  obtain ⟨CC, hCC_nn, hCC⟩ :=
    eigenvectorChartRHSDiffNumerator_layerC_eLpNorm_le_unconditional
    (I := I) (M := M) g r s i α P₀ m l fChartEffPrev
  obtain ⟨CD, hCD_nn, hCD⟩ :=
    eigenvectorChartRHSDiffNumerator_layerD_eLpNorm_le_unconditional
    (I := I) (M := M) g r s i α P₀ m l fChartEffPrev
  obtain ⟨CE, hCE_nn, hCE⟩ :=
    eigenvectorChartRHSDiffNumerator_layerE_eLpNorm_le_unconditional
    (I := I) (M := M) g r s i α P₀ m l fChartEffPrev
  rw [← hμ_def, ← hA_def] at hCA hCB hCC hCD hCE
  refine ⟨CA + CB + CC + CD + CE, by positivity, ?_⟩
  rw [h_num_eq]
...
/-- Chart-locality-free twin of
`eigenvectorChartRHSDiffNumerator_layerA_eLpNorm_le_uniform`. -/
private lemma eigenvectorChartRHSDiffNumerator_layerA_eLpNorm_le_uniform_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
    (fChartEffPrev : TensorEigenIdx (I := I) (M := M) g r s → EuclN → ℝ)
  ...
  have h_atom_le : ∀ (i : TensorEigenIdx (I := I) (M := M) g r s)
      ...
      wkpNorm (d := Module.finrank ℝ E) 2 2
        ...
        diffNumeratorAggregate_unconditional (I := I) (M := M)
          g r s i α P₀ m l (fChartEffPrev i) := by
    intro i a
    rw [diffNumeratorAggregate_unconditional]
    refine le_trans (Finset.single_le_sum (f := fun a =>
      wkpNorm (d := Module.finrank ℝ E) 2 2
        (eigenvectorChartIteratedPartial_unconditional (I := I) (M := M)
          g r s i α P₀ (m + 1) (Fin.cons a (Fin.init l)))
  ...
  refine eLpNorm_sum_le_const_mul_aggregate_uniform
    ...
    (fun i => diffNumeratorAggregate_unconditional (I := I) (M := M)
...
/-- Chart-locality-free twin of
`eigenvectorChartRHSDiffNumerator_layerB_eLpNorm_le_uniform`. -/
private lemma eigenvectorChartRHSDiffNumerator_layerB_eLpNorm_le_uniform_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
    (fChartEffPrev : TensorEigenIdx (I := I) (M := M) g r s → EuclN → ℝ)
  ...
  have h_atom_le : ∀ (i : TensorEigenIdx (I := I) (M := M) g r s)
      ...
      wkpNorm (d := Module.finrank ℝ E) 2 2
        ...
        diffNumeratorAggregate_unconditional (I := I) (M := M)
          g r s i α P₀ m l (fChartEffPrev i) := by
    intro i a
    rw [diffNumeratorAggregate_unconditional, ← hΩ_def]
    refine le_trans (Finset.single_le_sum (f := fun a =>
      wkpNorm (d := Module.finrank ℝ E) 2 2
        (eigenvectorChartIteratedPartial_unconditional (I := I) (M := M)
          g r s i α P₀ (m + 1) (Fin.cons a (Fin.init l))) Ω)
...
/-- Chart-locality-free twin of
`eigenvectorChartRHSDiffNumerator_layerC_eLpNorm_le_uniform`. -/
private lemma eigenvectorChartRHSDiffNumerator_layerC_eLpNorm_le_uniform_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
    (fChartEffPrev : TensorEigenIdx (I := I) (M := M) g r s → EuclN → ℝ) :
...
/-- Chart-locality-free twin of
`eigenvectorChartRHSDiffNumerator_layerD_eLpNorm_le_uniform`. -/
private lemma eigenvectorChartRHSDiffNumerator_layerD_eLpNorm_le_uniform_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
    (fChartEffPrev : TensorEigenIdx (I := I) (M := M) g r s → EuclN → ℝ) :
  ...
  have h_atom_le : ∀ i : TensorEigenIdx (I := I) (M := M) g r s,
      eLpNorm (fChartEffPrev i) 2 μ ≤
        diffNumeratorAggregate_unconditional (I := I) (M := M)
          g r s i α P₀ m l (fChartEffPrev i) := by
    intro i
    rw [diffNumeratorAggregate_unconditional, ← hμ_def]
    exact le_add_self
  obtain ⟨C₀, hC₀_nn, hC₀⟩ := eLpNorm_volume_restrict_contDiffOn_mul_le_uniform
    (I := I) (M := M) α
    (densityDerivOnEuclid_contDiffOn (I := I) g α (l (Fin.last m)))
...
/-- Chart-locality-free twin of
`eigenvectorChartRHSDiffNumerator_layerE_eLpNorm_le_uniform`. -/
private lemma eigenvectorChartRHSDiffNumerator_layerE_eLpNorm_le_uniform_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
    (fChartEffPrev : TensorEigenIdx (I := I) (M := M) g r s → EuclN → ℝ) :
...
theorem eigenvectorChartRHSDiffNumerator_eLpNorm_le_uniform_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
    (fChartEffPrev : TensorEigenIdx (I := I) (M := M) g r s → EuclN → ℝ)
  ...
  classical
  obtain ⟨CA, hCA_nn, hCA⟩ :=
    eigenvectorChartRHSDiffNumerator_layerA_eLpNorm_le_uniform_unconditional
      (I := I) (M := M) g r s α P₀ m l fChartEffPrev
      (fun i a => h_iter i (m + 1) (Fin.cons a (Fin.init l)))
  obtain ⟨CB, hCB_nn, hCB⟩ :=
    eigenvectorChartRHSDiffNumerator_layerB_eLpNorm_le_uniform_unconditional
      (I := I) (M := M) g r s α P₀ m l fChartEffPrev
      (fun i a => h_iter i (m + 1) (Fin.cons a (Fin.init l)))
  obtain ⟨CC, hCC_nn, hCC⟩ :=
    eigenvectorChartRHSDiffNumerator_layerC_eLpNorm_le_uniform_unconditional
      (I := I) (M := M) g r s α P₀ m l fChartEffPrev
  obtain ⟨CD, hCD_nn, hCD⟩ :=
    eigenvectorChartRHSDiffNumerator_layerD_eLpNorm_le_uniform_unconditional
      (I := I) (M := M) g r s α P₀ m l fChartEffPrev
  obtain ⟨CE, hCE_nn, hCE⟩ :=
    eigenvectorChartRHSDiffNumerator_layerE_eLpNorm_le_uniform_unconditional
      (I := I) (M := M) g r s α P₀ m l fChartEffPrev
  refine ⟨CA + CB + CC + CD + CE, by positivity, fun i => ?_⟩
  set μ : Measure EuclN := (volume : Measure EuclN).restrict
    (chartPouKernel (I := I) (M := M) α) with hμ_def
...
private lemma iter_memLp_volume_restrict_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (m : ℕ) (l : Fin m → Fin (Module.finrank ℝ E))
...
/-- Chart-locality-free twin of
`eigenvectorChartRHSDiffNumerator_layerA_eLpNorm_le_chartcpt`. -/
private lemma eigenvectorChartRHSDiffNumerator_layerA_eLpNorm_le_chartcpt_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
    (CatomA : ℝ) (eAtomA : ℕ) (_hCatomA_nn : 0 ≤ CatomA)
  ...
  have h_main : ∃ C : ℝ, 0 ≤ C ∧
    ...
    · intro a i
      refine memLp_finset_sum _ (fun b _ => ?_)
      have h_atom_mem := iter_memLp_volume_restrict_unconditional
...
/-- Chart-locality-free twin of
`eigenvectorChartRHSDiffNumerator_layerB_eLpNorm_le_chartcpt`. -/
private lemma eigenvectorChartRHSDiffNumerator_layerB_eLpNorm_le_chartcpt_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
    (CatomB : ℝ) (eAtomB : ℕ) (_hCatomB_nn : 0 ≤ CatomB)
...
/-- Chart-locality-free twin of
`eigenvectorChartRHSDiffNumerator_layerC_eLpNorm_le_chartcpt`. -/
private lemma eigenvectorChartRHSDiffNumerator_layerC_eLpNorm_le_chartcpt_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
    (CatomC : ℝ) (eAtomC : ℕ) (_hCatomC_nn : 0 ≤ CatomC)
  ...
  refine le_trans (hC₀ _) ?_
  calc
    ENNReal.ofReal C₀ * eLpNorm (eigenvectorChartIteratedPartial_unconditional (I := I) (M := M)
...
/-- Chart-locality-free twin of
`eigenvectorChartRHSDiffNumerator_layerD_eLpNorm_le_chartcpt`. -/
private lemma eigenvectorChartRHSDiffNumerator_layerD_eLpNorm_le_chartcpt_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (_P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
    (fChartEffPrev : TensorEigenIdx (I := I) (M := M) g r s → EuclN → ℝ)
...
/-- Chart-locality-free twin of
`eigenvectorChartRHSDiffNumerator_layerE_eLpNorm_le_chartcpt`. -/
private lemma eigenvectorChartRHSDiffNumerator_layerE_eLpNorm_le_chartcpt_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (_P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
    (fChartEffPrev : TensorEigenIdx (I := I) (M := M) g r s → EuclN → ℝ)
...
private lemma eigen_inv_one_le_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s) :
    1 ≤ (i.fst.val)⁻¹ := by
  have h_norm :
...
private lemma pow_eigen_inv_le_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    {k e : ℕ} (hke : k ≤ e) :
    (i.fst.val)⁻¹ ^ k ≤ (i.fst.val)⁻¹ ^ e := by
  exact pow_le_pow_right₀
    (eigen_inv_one_le_unconditional (I := I) (M := M) g r s i) hke

/-- Chart-locality-free twin of `ofReal_const_pow_eigen_inv_le`. -/
private lemma ofReal_const_pow_eigen_inv_le_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    {C : ℝ} (hC_nn : 0 ≤ C) {k e : ℕ} (hke : k ≤ e) :
    ENNReal.ofReal (C * (i.fst.val)⁻¹ ^ k) ≤
  ...
  refine ENNReal.ofReal_le_ofReal ?_
  exact mul_le_mul_of_nonneg_left
    (pow_eigen_inv_le_unconditional (I := I) (M := M) g r s i hke) hC_nn

/-- Chart-locality-free twin of
`eigenvectorChartRHSDiffNumerator_eLpNorm_le_chartcpt`. -/
theorem eigenvectorChartRHSDiffNumerator_eLpNorm_le_chartcpt_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (m : ℕ)
    (l : Fin (m + 1) → Fin (Module.finrank ℝ E))
    (fChartEffPrev : TensorEigenIdx (I := I) (M := M) g r s → EuclN → ℝ)
  ...
  classical
  obtain ⟨CA, hCA_nn, hCA⟩ :=
    eigenvectorChartRHSDiffNumerator_layerA_eLpNorm_le_chartcpt_unconditional
      (I := I) (M := M) g r s α P₀ m l CatomA eAtomA hCatomA_nn hAtomA_bd
  obtain ⟨CB, hCB_nn, hCB⟩ :=
    eigenvectorChartRHSDiffNumerator_layerB_eLpNorm_le_chartcpt_unconditional
      (I := I) (M := M) g r s α P₀ m l CatomB eAtomB hCatomB_nn hAtomB_bd
  obtain ⟨CC, hCC_nn, hCC⟩ :=
    eigenvectorChartRHSDiffNumerator_layerC_eLpNorm_le_chartcpt_unconditional
      (I := I) (M := M) g r s α P₀ m l CatomC eAtomC hCatomC_nn hAtomC_bd
  obtain ⟨CD, hCD_nn, hCD⟩ :=
    eigenvectorChartRHSDiffNumerator_layerD_eLpNorm_le_chartcpt_unconditional
      (I := I) (M := M) g r s α P₀ m l fChartEffPrev CatomD eAtomD
      hCatomD_nn hAtomD_bd
  obtain ⟨CE, hCE_nn, hCE⟩ :=
    eigenvectorChartRHSDiffNumerator_layerE_eLpNorm_le_chartcpt_unconditional
  ...
  have hCA_e : eLpNorm layerA 2 μ ≤
      ENNReal.ofReal (CA * CatomA * (i.fst.val)⁻¹ ^ e) * Rhs := by
    refine le_trans hCA_i (mul_le_mul' ?_ (le_refl _))
    exact ofReal_const_pow_eigen_inv_le_unconditional (I := I) (M := M) g r s i
      hCA_prod_nn heA
  have hCB_e : eLpNorm layerB 2 μ ≤
      ENNReal.ofReal (CB * CatomB * (i.fst.val)⁻¹ ^ e) * Rhs := by
    refine le_trans hCB_i (mul_le_mul' ?_ (le_refl _))
  ...
  have hμi_nn : 0 ≤ μi := by
    rw [hμi_def]
    have h1 : 1 ≤ (i.fst.val)⁻¹ :=
      eigen_inv_one_le_unconditional (I := I) (M := M) g r s i
    have : 0 ≤ (i.fst.val)⁻¹ := le_trans zero_le_one h1
    exact pow_nonneg this _
  have h_pull :
      ENNReal.ofReal (CA * CatomA * (i.fst.val)⁻¹ ^ e) * Rhs +
