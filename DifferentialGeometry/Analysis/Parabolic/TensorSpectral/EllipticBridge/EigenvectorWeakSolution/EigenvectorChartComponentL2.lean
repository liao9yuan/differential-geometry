...
converge to the eigenvector resolvent `eigenvectorResolvent g r s h_uniform i`.
...
* `eigenvectorSmoothApprox g r s h_uniform i` — the canonical smooth
...
`TensorH1Compl g r s`, to `eigenvectorResolvent g r s h_uniform i`.
...
noncomputable def eigenvectorSmoothApprox
  ...
  Classical.choose (exists_smoothApprox (I := I) (M := M) g r s h_uniform i)

/-- **Spec of the canonical smooth `H¹`-approximating sequence.** The
`H¹`-completion embeddings of `eigenvectorSmoothApprox g r s h_uniform i`
...
theorem eigenvectorSmoothApprox_tendsto
  ...
  Classical.choose_spec (exists_smoothApprox (I := I) (M := M) g r s h_uniform i)

/-! ## The `L²`-coercion of the eigenvector resolvent equals `μ • φ`

The eigenvector `φ := tensorResolventEigenbasisVec h_uniform i` satisfies, by
...
`φ := tensorResolventEigenbasisVec h_uniform i` is the eigenvector. This is the
...
`w n := eigenvectorSmoothApprox g r s h_uniform i n`. Composing the spec lemma
...
`μ := i.fst.val` and `φ := tensorResolventEigenbasisVec h_uniform i`. -/
...
eigenvector `φ := tensorResolventEigenbasisVec h_uniform i`. -/
private lemma smoothApprox_smul_coe_tendsto
  ...
  have h_smul :=
    (smoothApprox_coe_tendsto (I := I) (M := M) g r s h_uniform i).const_smul
      (i.fst.val)⁻¹
  -- `μ⁻¹ • (μ • φ) = φ`.
  have hμ_ne : i.fst.val ≠ 0 := i.fst.val_ne_zero
...
`(eigenvectorSmoothApprox g r s h_uniform i n).toCcTensor` equals `μ⁻¹` times
    ...
    tensorL2ChartComponent (I := I) (M := M) g r s
      ...
      (i.fst.val)⁻¹ •
        (tensorChartComponent_memLp (I := I) (M := M) g r s
          (eigenvectorSmoothApprox (I := I) (M := M)
            g r s h_uniform i n).toCcTensor α P₀.1 P₀.2).toLp
...
`eigenvectorSmoothApprox g r s h_uniform i n` converge, as `n → ∞` and in
...
`tensorChartComponent g r s (eigenvectorSmoothApprox … n).toCcTensor α P₀.1 P₀.2`
of the smooth section `(eigenvectorSmoothApprox g r s h_uniform i n).toCcTensor`. -/
theorem eigenvectorChartComponentL2_tendsto
  ...
  have h_clm :=
    ((tensorL2ChartComponentCLM (I := I) (M := M) g r s α P₀).continuous.tendsto
      _).comp
      (smoothApprox_smul_coe_tendsto (I := I) (M := M) g r s h_uniform i)
...
example (g : SmoothRiemannianMetric I M) (r s : ℕ)
    ...
    (i : TensorEigenIdx (I := I) (M := M) g r s) :
    ℕ → SmoothCcTensorH1 g r s :=
  eigenvectorSmoothApprox (I := I) (M := M) g r s h_uniform i
...
example (g : SmoothRiemannianMetric I M) (r s : ℕ)
  ...
  eigenvectorChartComponentL2_tendsto (I := I) (M := M) g r s h_uniform i α P₀
