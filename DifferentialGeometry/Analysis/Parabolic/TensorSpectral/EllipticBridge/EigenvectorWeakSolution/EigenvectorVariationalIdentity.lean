...
Chart-locality-free `_unconditional` twins of the helper lemmas and the headline
variational identity, re-keyed onto the chart-locality-free smooth approximant
`eigenvectorSmoothApprox_unconditional` / partition-of-unity-weighted approximant
`eigenvectorPouApprox_unconditional`, the chart-locality-free candidate weak
chart partial / right-hand side / limit objects, and the chart-locality-free
eigenbasis vector `tensorResolventEigenbasisVec_ofCompact` at the unconditional
resolvent-compactness witness `tensorResolventL2_isCompactOperator_intrinsic`. -/

/-- Chart-locality-free twin of
`eigenvectorPouApprox_component_tsupport_subset`. -/
private lemma eigenvectorPouApprox_component_tsupport_subset_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (n : ℕ) :
    tsupport (tensorComponentEuclid (I := I) (M := M) g r s
        (eigenvectorPouApprox_unconditional (I := I) (M := M) g r s i α n) α P₀) ⊆
      chartPouKernel (I := I) (M := M) α := by
  rw [eigenvectorPouApprox_unconditional,
    ← tensorChartComponent_eq_tensorComponentEuclid_pouSmul]
  exact tensorChartComponent_tsupport_subset_chartPouKernel (I := I) (M := M)
    g r s _ α P₀.1 P₀.2

/-- Chart-locality-free twin of `eigenvectorPouApprox_tsupport_subset_source`. -/
private lemma eigenvectorPouApprox_tsupport_subset_source_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (n : ℕ) :
    tsupport (eigenvectorPouApprox_unconditional (I := I) (M := M)
        g r s i α n).toFun ⊆ (chartAt H α).source :=
  pouSmul_tsupport_subset_chartSource (I := I) (M := M) g r s α _

/-- Chart-locality-free twin of `eigenvectorPouApprox_component_contDiff`. -/
private lemma eigenvectorPouApprox_component_contDiff_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (n : ℕ) :
    ContDiff ℝ ∞ (tensorComponentEuclid (I := I) (M := M) g r s
  ...
  tensorComponentEuclid_contDiff (I := I) (M := M) g r s
    (eigenvectorPouApprox_unconditional (I := I) (M := M) g r s i α n) α P₀
    (eigenvectorPouApprox_tsupport_subset_source_unconditional (I := I) (M := M)
      g r s i α n)

/-- Chart-locality-free twin of
`euclidPartial_eigenvectorPouApprox_component_contDiff`. -/
private lemma euclidPartial_eigenvectorPouApprox_component_contDiff_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) (n : ℕ) :
    ContDiff ℝ ∞ (euclidPartial (E := E) k
      (tensorComponentEuclid (I := I) (M := M) g r s
        (eigenvectorPouApprox_unconditional (I := I) (M := M) g r s i α n) α P₀)) :=
  euclidPartial_contDiff (E := E)
    (eigenvectorPouApprox_component_contDiff_unconditional (I := I) (M := M)
      g r s i α P₀ n) k

/-- Chart-locality-free twin of
`euclidPartial_eigenvectorPouApprox_component_hasCompactSupport`. -/
private lemma euclidPartial_eigenvectorPouApprox_component_hasCompactSupport_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) (n : ℕ) :
  ...
  have hu_evt : (tensorComponentEuclid (I := I) (M := M) g r s
    ...
    Filter.eventually_of_mem (hopen_c.mem_nhds hyK)
      (fun z hz => image_eq_zero_of_notMem_tsupport
        (fun h => hz (eigenvectorPouApprox_component_tsupport_subset_unconditional
...
private lemma euclidPartial_eigenvectorPouApprox_component_memLp_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) (n : ℕ) :
    MemLp (euclidPartial (E := E) k
      ...
      (chartL2Measure (I := I) (M := M) α) := by
  rw [chartL2Measure]
  exact ((euclidPartial_eigenvectorPouApprox_component_contDiff_unconditional
    (I := I) (M := M) g r s i α P₀ k n).continuous.memLp_of_hasCompactSupport
    (euclidPartial_eigenvectorPouApprox_component_hasCompactSupport_unconditional
      (I := I) (M := M) g r s i α P₀ k n)).restrict _

open DifferentialGeometry.PDE.RicciFlow.IntrinsicSpectral in
/-- Chart-locality-free twin of
`eigenvectorChartPartialCLM_smoothApprox_coeFn_eq`. -/
private lemma eigenvectorChartPartialCLM_smoothApprox_coeFn_eq_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) (n : ℕ) :
  ...
  classical
  have hμ : i.fst.val ≠ 0 := i.fst.val_ne_zero
  have h1 := eigenvectorChartPartialLp_approx_coeFn_unconditional (I := I) (M := M)
    g r s i α P₀ k n
  have h2 := chosenWeakPartial'_tensorChartComponent_ae_eq (I := I) (M := M)
...
private lemma euclidPartial_eigenvectorPouApprox_toLp_eq_clm_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) (n : ℕ) :
    (euclidPartial_eigenvectorPouApprox_component_memLp_unconditional (I := I) (M := M)
  ...
  refine Lp.ext ?_
  refine (MemLp.coeFn_toLp _).trans ?_
  exact (eigenvectorChartPartialCLM_smoothApprox_coeFn_eq_unconditional (I := I) (M := M)
    g r s i α P₀ k n).symm

/-- Chart-locality-free twin of
`euclidPartial_eigenvectorPouApprox_toLp_tendsto`. -/
private lemma euclidPartial_eigenvectorPouApprox_toLp_tendsto_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) :
    Filter.Tendsto
      (fun n => (euclidPartial_eigenvectorPouApprox_component_memLp_unconditional
  ...
  classical
  have hμ : i.fst.val ≠ 0 := i.fst.val_ne_zero
  have h_base := eigenvectorChartPartialLp_tendsto_unconditional (I := I) (M := M)
    g r s i α P₀ k
  have h_scaled := h_base.const_smul i.fst.val
  have h_eq : ∀ n : ℕ, i.fst.val • ((i.fst.val)⁻¹ •
        eigenvectorChartPartialCLM (I := I) (M := M) g r s α P₀ k
          (smoothToTensorH1Compl (I := I) (M := M) g r s
            (eigenvectorSmoothApprox_unconditional (I := I) (M := M)
              g r s i n))) =
      (euclidPartial_eigenvectorPouApprox_component_memLp_unconditional
        (I := I) (M := M) g r s i α P₀ k n).toLp _ := by
    intro n
    rw [smul_smul, mul_inv_cancel₀ hμ, one_smul,
      euclidPartial_eigenvectorPouApprox_toLp_eq_clm_unconditional (I := I) (M := M)
        g r s i α P₀ k n]
  exact h_scaled.congr h_eq

/-- Chart-locality-free twin of
`principalIntegrand_eigenvectorPouApprox_eqOn`. -/
private lemma principalIntegrand_eigenvectorPouApprox_eqOn_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    {ψ : EuclN → ℝ} (n : ℕ) :
...
private lemma bilin_eigenvectorPouApprox_eq_sum_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    {ψ : EuclN → ℝ} (hψ : ContDiff ℝ ∞ ψ) (_hψ_cs : HasCompactSupport ψ)
  ...
  have huₙ_tsupport_compact : IsCompact (tsupport uₙ) :=
    IsCompact.of_isClosed_subset (chartPouKernel_isCompact (I := I) (M := M) α)
      (isClosed_tsupport _)
      (eigenvectorPouApprox_component_tsupport_subset_unconditional (I := I) (M := M)
        g r s i α P₀ n)
  have hcs_principal : HasCompactSupport (Bform.principalIntegrand uₙ ψ) := by
    refine HasCompactSupport.of_support_subset_isCompact
      huₙ_tsupport_compact ?_
          ...
          (∑ k : Fin (Module.finrank ℝ E),
            ∑ l : Fin (Module.finrank ℝ E),
              weightedInvGramOnEuclid (I := I) g α k l y *
                ...
                euclidPartial (E := E) l ψ y) ∂(volume : Measure EuclN) := by
    refine MeasureTheory.setIntegral_congr_fun hcTE_meas ?_
    have := principalIntegrand_eigenvectorPouApprox_eqOn_unconditional (I := I) (M := M)
      g r s i α P₀ (ψ := ψ) n
    rw [hBform_def, huₙ_def]
    exact this
  have hreorg :
    ...
    have huₙ_partial_cd : ∀ k : Fin (Module.finrank ℝ E),
        ContDiff ℝ ∞ (euclidPartial (E := E) k uₙ) :=
      fun k => euclidPartial_eigenvectorPouApprox_component_contDiff_unconditional
        (I := I) (M := M) g r s i α P₀ k n
    have huₙ_partial_cs : ∀ k : Fin (Module.finrank ℝ E),
        HasCompactSupport (euclidPartial (E := E) k uₙ) :=
      fun k => euclidPartial_eigenvectorPouApprox_component_hasCompactSupport_unconditional
        (I := I) (M := M) g r s i α P₀ k n
    have huₙ_partial_supp : ∀ k : Fin (Module.finrank ℝ E),
        tsupport (euclidPartial (E := E) k uₙ) ⊆
          chartTargetEuclid (I := I) (M := M) α := by
...
private lemma bilin_eigenvectorPouApprox_tendsto_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    {ψ : EuclN → ℝ} (hψ : ContDiff ℝ ∞ ψ) (hψ_cs : HasCompactSupport ψ)
  ...
  have h_dir : ∀ i' : Fin (Module.finrank ℝ E),
    ...
    exact tendsto_lp_inner_integral (μ := chartL2Measure (I := I) (M := M) α)
      (mtest i')
      (euclidPartial_eigenvectorPouApprox_toLp_tendsto_unconditional (I := I) (M := M)
        g r s i α P₀ i')
  have h_int_n : ∀ (i' : Fin (Module.finrank ℝ E)) (n : ℕ),
      ∫ y, (mtest i' : EuclN → ℝ) y *
        ((euclidPartial_eigenvectorPouApprox_component_memLp_unconditional
    ...
    filter_upwards [(by rw [hmtest_def]; exact MemLp.coeFn_toLp _ :
        (mtest i' : EuclN → ℝ) =ᵐ[chartL2Measure (I := I) (M := M) α]
          principalSymbolTest (I := I) (M := M) g α ψ i'),
      MemLp.coeFn_toLp (euclidPartial_eigenvectorPouApprox_component_memLp_unconditional
  ...
  have h_bilin_n : ∀ n : ℕ,
      ...
      ∑ i' : Fin (Module.finrank ℝ E),
        ∫ y, (mtest i' : EuclN → ℝ) y *
          ...
          ∂(chartL2Measure (I := I) (M := M) α) := by
    intro n
    rw [bilin_eigenvectorPouApprox_eq_sum_unconditional (I := I) (M := M)
    ...
    have h_inner_integrable : ∀ i' : Fin (Module.finrank ℝ E),
        Integrable (fun y => ∑ j' : Fin (Module.finrank ℝ E),
          ...
          ((volume : Measure EuclN).restrict
            (chartTargetEuclid (I := I) (M := M) α)) := by
      intro i'
      have hwp_memLp : MemLp (eigenvectorChartWeakPartial_unconditional (I := I) (M := M)
          g r s i α P₀ i') 2
          ((volume : Measure EuclN).restrict
            (chartTargetEuclid (I := I) (M := M) α)) := by
        have h : MemLp (fun y => ((eigenvectorChartPartialLp_unconditional
...
convergence `smoothApprox_dirichlet_tendsto_unconditional`, the chart-locality-free
eigenvector weak equation `eigenWeakEquation_unconditional` and the resolvent
rescaling `eigenvector_eq_resolvent_smul_unconditional`, so that no
`mainDir_tendsto_unconditional` companion lemma is needed. -/
private lemma eigenvectorMainDir_tendsto_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    {ψ : EuclN → ℝ} (hψ : ContDiff ℝ ∞ ψ) (hψ_cs : HasCompactSupport ψ)
  ...
  set φ : TensorL2 r s g :=
    ...
    with hφ_def
  -- The fixed test section as a smooth compactly-supported `H¹` section.
  ...
  -- The chart-locality-free approximating sequence and its `H¹`-convergence.
  have hw_tendsto :
      Filter.Tendsto
  ...
  -- The Dirichlet-convergence of the canonical approximating sequence.
  have h_dir :=
    smoothApprox_dirichlet_tendsto_unconditional (I := I) (M := M) g r s i Sh1
      hw_tendsto
  ...
  have h_weak :
    ...
    eigenWeakEquation_unconditional (I := I) (M := M) g r s i Sh1
  -- `TensorH1ComplToTensorL2 R = μ • φ`.
  have h_resolvent :
      TensorH1ComplToTensorL2 (I := I) (M := M) g r s
          (eigenvectorResolvent_unconditional (I := I) (M := M) g r s i) =
        i.fst.val • φ := by
    have h_eq := eigenvector_eq_resolvent_smul_unconditional (I := I) (M := M)
      g r s i
  ...
  have h_l2part :
    ...
    rw [h_resolvent, inner_smul_left, starRingEnd_apply, star_trivial,
      real_inner_comm]
  -- Assemble the main-Dirichlet closed-form limit.
  have h_md_eq :
      ⟪eigenvectorResolvent_unconditional (I := I) (M := M) g r s i,
            smoothToTensorH1Compl (I := I) (M := M) g r s Sh1⟫_ℝ -
          ⟪TensorH1ComplToTensorL2 (I := I) (M := M) g r s
...
private lemma crossLeftPairing_integrable_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (P Q : TensorCompIdx (E := E) r (s + 1))
...
private lemma crossLeftLimitPairing_integrable_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (P Q : TensorCompIdx (E := E) r (s + 1))
...
private lemma eigenvectorCrossLeft_tendsto_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    {ψ : EuclN → ℝ} (hψ : ContDiff ℝ ∞ ψ) (hψ_cs : HasCompactSupport ψ)
  ...
  have h_dir : ∀ (P Q : TensorCompIdx (E := E) r (s + 1)),
    ...
    fun P Q => tendsto_lp_inner_integral
      (μ := chartL2Measure (I := I) (M := M) α) (mtest P Q)
      (crossLeftComponent_tendsto_unconditional (I := I) (M := M) g r s i α P)
  have h_sum_tendsto :
      Filter.Tendsto
        (fun n => ∑ P : TensorCompIdx (E := E) r (s + 1),
          ∑ Q : TensorCompIdx (E := E) r (s + 1),
  ...
  have h_cross_n : ∀ n : ℕ,
    ...
    rw [MeasureTheory.integral_finset_sum _ (fun P _ =>
        MeasureTheory.integrable_finset_sum _ (fun Q _ =>
          crossLeftPairing_integrable_unconditional (I := I) (M := M) g r s i
            α P₀ P Q hψ hψ_cs hψ_supp n))]
    refine Finset.sum_congr rfl (fun P _ => ?_)
    rw [MeasureTheory.integral_finset_sum _ (fun Q _ =>
        crossLeftPairing_integrable_unconditional (I := I) (M := M) g r s i
  ...
  have h_limit_eq :
    ...
    rw [Finset.sum_congr rfl (fun P _ =>
      (MeasureTheory.integral_finset_sum _ (fun Q _ =>
        crossLeftLimitPairing_integrable_unconditional (I := I) (M := M)
...
private lemma crossRightValuePairing_integrable_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (P Q : TensorCompIdx (E := E) r s)
...
private lemma crossRightValueLimitPairing_integrable_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (P Q : TensorCompIdx (E := E) r s)
...
private lemma eigenvectorCrossRight_tendsto_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    {ψ : EuclN → ℝ} (hψ : ContDiff ℝ ∞ ψ) (hψ_cs : HasCompactSupport ψ)
  ...
  have h_value_n : ∀ n : ℕ,
    ...
    rw [MeasureTheory.integral_finset_sum _ (fun P _ =>
        MeasureTheory.integrable_finset_sum _ (fun Q _ =>
          crossRightValuePairing_integrable_unconditional (I := I) (M := M) g r s i
            α P₀ P Q hψ hψ_cs hψ_supp n))]
    refine Finset.sum_congr rfl (fun P _ => ?_)
    rw [MeasureTheory.integral_finset_sum _ (fun Q _ =>
        crossRightValuePairing_integrable_unconditional (I := I) (M := M) g r s i
  ...
  have h_limit_eq :
    ...
    rw [Finset.sum_congr rfl (fun P _ =>
      (MeasureTheory.integral_finset_sum _ (fun Q _ =>
        crossRightValueLimitPairing_integrable_unconditional (I := I) (M := M)
          g r s i α P₀ P Q hψ hψ_cs hψ_supp)).symm)]
    rw [← MeasureTheory.integral_finset_sum _ (fun P _ =>
      MeasureTheory.integrable_finset_sum _ (fun Q _ =>
        crossRightValueLimitPairing_integrable_unconditional (I := I) (M := M)
...
private lemma eigenvectorCrossRightGrad_tendsto_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    {ψ : EuclN → ℝ} (hψ : ContDiff ℝ ∞ ψ) (hψ_cs : HasCompactSupport ψ)
  ...
  have h_int_lim :
    ...
    have h_ae_prod : (fun y => (m : EuclN → ℝ) y * (glim : EuclN → ℝ) y)
        =ᵐ[chartL2Measure (I := I) (M := M) α]
        fun y => crossRightGradCoeffDivLimit_unconditional (I := I) (M := M)
          g r s i α P₀ y * ψ y := by
      filter_upwards [h_m_ae, h_g_ae] with y hy_m hy_g
      rw [hy_m, hy_g]; ring
    exact MeasureTheory.integral_congr_ae h_ae_prod
  have h_tendsto_lp : Filter.Tendsto gseq atTop (𝓝 glim) := by
    rw [hgseq_def, hglim_def]
    exact crossRightGradCoeffDivSum_tendsto_unconditional (I := I) (M := M)
      g r s i α P₀
  have h_main := tendsto_lp_inner_integral
    (μ := chartL2Measure (I := I) (M := M) α) m h_tendsto_lp
  rw [h_int_lim] at h_main
...
private lemma eigenvectorSource_integral_split_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    {ψ : EuclN → ℝ} (hψ : ContDiff ℝ ∞ ψ) (hψ_cs : HasCompactSupport ψ)
  ...
  have h_pointwise : ∀ x,
      tensorCovDerivPointwiseInner (I := I) (M := M)
        g r s (eigenvectorPouApprox_unconditional (I := I) (M := M) g r s i α n)
        vRot x =
      Amain x - Bleft x + Cright x := by
    intro x
    rw [eigenvectorPouApprox_unconditional, pouSmul_eq_scalarSmul (I := I) (M := M)
      g r s α wₙ, hAmain_def, hBleft_def, hCright_def]
    exact tensorCovDerivPointwiseInner_scalarSmul_left (I := I) (M := M)
      g r s (chartAtlasPOU I M α) wₙ vRot x
  calc ∫ x, tensorCovDerivPointwiseInner (I := I) (M := M) g r s
...
private lemma density_crossRightTestGradTerm_partialTest_integrable_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (l : Fin (Module.finrank ℝ E))
...
private lemma density_crossRightValueSum_test_integrable_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    {ψ : EuclN → ℝ} (hψ : ContDiff ℝ ∞ ψ) (hψ_cs : HasCompactSupport ψ)
...
private lemma tensorL2ChartComponentCutoff_smoothApprox_ae_all_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (n : ℕ) :
    ∀ᵐ y ∂(chartL2Measure (I := I) (M := M) α),
...
private lemma eigenvectorCrossRight_integral_eq_value_plus_grad_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    {ψ : EuclN → ℝ} (hψ : ContDiff ℝ ∞ ψ) (hψ_cs : HasCompactSupport ψ)
  ...
  have h_integrand_ae :
    ...
    filter_upwards
      [MeasureTheory.ae_restrict_mem
        (chartTargetEuclid_measurableSet (I := I) (M := M) α),
      (tensorL2ChartComponentCutoff_smoothApprox_ae_all_unconditional
        (I := I) (M := M) g r s i α n :
        ∀ᵐ y ∂(chartL2Measure (I := I) (M := M) α), _)]
      with y hy hcut
    have hdec : ∀ Q : CompIdx E r s,
  ...
  rw [MeasureTheory.integral_congr_ae h_integrand_ae]
  rw [MeasureTheory.integral_add
    (hvalueIntegrand_def ▸ density_crossRightValueSum_test_integrable_unconditional
      (I := I) (M := M) g r s i α P₀ hψ hψ_cs hψ_supp n)
    (hgradIntegrand_def ▸ MeasureTheory.integrable_finset_sum _
      (fun l _ => density_crossRightTestGradTerm_partialTest_integrable_unconditional
...
theorem eigenvectorChartVariationalIdentity_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    {ψ : EuclN → ℝ} (hψ : ContDiff ℝ ∞ ψ) (hψ_cs : HasCompactSupport ψ)
    ...
    ∫ y in chartTargetEuclid (I := I) (M := M) α,
      densityOnEuclid (I := I) g α y *
        eigenvectorChartRHS_unconditional (I := I) (M := M) g r s i α P₀ y * ψ y
      ∂(volume : Measure EuclN) := by
  classical
  set μ : ℝ := i.fst.val with hμ_def
  have hμ_ne : μ ≠ 0 := i.fst.val_ne_zero
  ...
  have h_per_n : ∀ n : ℕ,
    ...
    intro n
    rw [tensorComponent_chartBilinIdentity_of_dirichlet (I := I) (M := M) g r s
      (eigenvectorPouApprox_unconditional (I := I) (M := M) g r s i α n) α
      (chartPouKernel_isCompact (I := I) (M := M) α)
      (chartPouKernel_subset_chartTargetEuclid (I := I) (M := M) α) P₀
      (eigenvectorPouApprox_tsupport_subset_source_unconditional (I := I) (M := M)
        g r s i α n)
    ...
    rw [show (∫ x, tensorCovDerivPointwiseInner (I := I) (M := M) g r s
        ...
        ∫ x, tensorCovDerivPointwiseInner (I := I) (M := M) g r s
            ...
            (eigenvectorRotatedTestSection (I := I) (M := M) g r s α P₀
              hψ hψ_cs hψ_supp) x
          ∂(riemannianVolumeMeasure (I := I) (M := M) g) from rfl]
    rw [eigenvectorSource_integral_split_unconditional (I := I) (M := M) g r s i
      α P₀ hψ hψ_cs hψ_supp n,
      eigenvectorCrossRight_integral_eq_value_plus_grad_unconditional (I := I) (M := M)
        g r s i α P₀ hψ hψ_cs hψ_supp n]
  set L : ℝ := ((1 - μ) * U - CL + (CRV + -CRGdiv) - PRC - LOV + GD) with hL_def
  have h_rhs_tendsto : Filter.Tendsto
      (fun n => ((∫ x, tensorCovDerivPointwiseInner (I := I) (M := M) g r s
      ...
      atTop (𝓝 L) := by
    rw [hL_def]
    have h_md := eigenvectorMainDir_tendsto_unconditional (I := I) (M := M) g r s i
      α P₀ hψ hψ_cs hψ_supp
    refine (((((h_md.sub
      (eigenvectorCrossLeft_tendsto_unconditional (I := I) (M := M) g r s i
        α P₀ hψ hψ_cs hψ_supp)).add
      ((eigenvectorCrossRight_tendsto_unconditional (I := I) (M := M) g r s i
        α P₀ hψ hψ_cs hψ_supp).add
        (eigenvectorCrossRightGrad_tendsto_unconditional (I := I) (M := M) g r s i
          α P₀ hψ hψ_cs hψ_supp))).sub
      (covPrincipalRotationCoeff_source_tendsto_unconditional (I := I) (M := M) g r s
        i α P₀ hψ hψ_cs hψ_supp)).sub
      (covLowerOrderRotationValueCoeff_source_tendsto_unconditional (I := I) (M := M)
        g r s i α P₀ hψ hψ_cs hψ_supp)).add
      (weightedGradCoeffDivSum_source_tendsto_unconditional (I := I) (M := M) g r s
  ...
  have h_lhs_tendsto : Filter.Tendsto
      ...
      atTop (𝓝 (μ * P)) := by
    rw [hμ_def, hP_def]
    exact bilin_eigenvectorPouApprox_tendsto_unconditional (I := I) (M := M) g r s i
      α P₀ hψ hψ_cs hψ_supp
  have h_mu_P : μ * P = L := by
    refine tendsto_nhds_unique (h_lhs_tendsto.congr ?_) h_rhs_tendsto
    intro n
  ...
  have h_rhs_integral :
    ...
    have hint_CLsum : Integrable (fun y => densityOnEuclid (I := I) g α y *
      ...
      have h_sum : Integrable (fun y => ∑ P' : TensorCompIdx (E := E) r (s + 1),
        ...
        MeasureTheory.integrable_finset_sum _ (fun P' _ =>
          MeasureTheory.integrable_finset_sum _ (fun Q _ =>
            crossLeftLimitPairing_integrable_unconditional (I := I) (M := M) g r s i
              α P₀ P' Q hψ hψ_cs hψ_supp))
      refine h_sum.congr (Filter.Eventually.of_forall (fun y => ?_))
      simp only [Finset.mul_sum, Finset.sum_mul]
    have hint_CRVsum : Integrable (fun y => densityOnEuclid (I := I) g α y *
    ...
    have hint_GD : Integrable (fun y => densityOnEuclid (I := I) g α y *
      ...
      density_coeff_memLp2_test_integrable (I := I) (M := M) g α
        ...
        (memLp_finset_sum (μ := (volume : Measure EuclN).restrict
            (chartTargetEuclid (I := I) (M := M) α))
          (Finset.univ : Finset (Fin (Module.finrank ℝ E)))
          (fun l _ => weightedGradCoeffDivLimit_memLp_unconditional (I := I) (M := M)
            g r s i α P₀ l))
        hψ hψ_cs hψ_supp
    have hint_CRGD : Integrable (fun y => densityOnEuclid (I := I) g α y *
        ((1 / densityOnEuclid (I := I) g α y) *
    ...
    have h_integrand : Set.EqOn
        ...
        (chartTargetEuclid (I := I) (M := M) α) := by
      intro y _hy
      simp only [eigenvectorChartRHS_unconditional, hφ_def]
      ring
    rw [MeasureTheory.setIntegral_congr_fun hcTE_meas h_integrand]
    rw [MeasureTheory.integral_const_mul]
    congr 1
    ...
    have hD7 : (∫ y in chartTargetEuclid (I := I) (M := M) α,
      ...
      rw [mul_one_div, div_self (densityOnEuclid_pos (I := I) g α hy).ne',
        one_mul]
    -- The seven bracket integrand pieces, named as functions.
    ...
    -- Peel the bracket integral one term at a time by Bochner additivity. The
    -- integrability facts and the goal are now all stated through `fU … fCRGD`
    -- (the `set` tactic folded them); each step pins the flat bracket on its
    -- left so the additivity rewrites match.
    have e1 : (∫ y in chartTargetEuclid (I := I) (M := M) α,
