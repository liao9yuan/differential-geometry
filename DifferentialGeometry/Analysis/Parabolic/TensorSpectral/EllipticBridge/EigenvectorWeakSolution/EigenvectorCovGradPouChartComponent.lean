...
`φ := tensorResolventEigenbasisVec h_uniform i` be the eigenvector and
`R := eigenvectorResolvent g r s h_uniform i` its `H¹`-completion resolvent. The
...
The approximating sequence `wₙ := eigenvectorSmoothApprox g r s h_uniform i n`
...
* `covGradPouLeibnizCrossLimit g r s h_uniform i β P₀ k` — the explicit
  ...
  fun y =>
    euclidPartial (E := E) k
      ...
      (tensorL2ChartComponentCutoff (I := I) (M := M) g r s
        (tensorResolventEigenbasisVec (I := I) (M := M) h_uniform i) β P₀ :
        EuclN → ℝ) y
...
theorem covGradPouLeibnizCrossLimit_memLp
  ...
  refine (memLp_bdd_mul (I := I) (M := M) β hC0 hC
    ...
    (Lp.memLp (tensorL2ChartComponentCutoff (I := I) (M := M) g r s
      (tensorResolventEigenbasisVec (I := I) (M := M) h_uniform i) β P₀))).ae_eq ?_
  refine Filter.Eventually.of_forall (fun y => ?_)
  change crossMultiplier (I := I) (M := M) β k y *
      (tensorL2ChartComponentCutoff (I := I) (M := M) g r s
        (tensorResolventEigenbasisVec (I := I) (M := M) h_uniform i) β P₀ :
        EuclN → ℝ) y =
    covGradPouLeibnizCrossLimit (I := I) (M := M) g r s h_uniform i β P₀ k y
...
private lemma cutoffComponent_smoothApprox_tendsto
    ...
    Filter.Tendsto
      (fun n => tensorL2ChartComponentCutoff (I := I) (M := M) g r s
        (((eigenvectorSmoothApprox (I := I) (M := M)
          g r s h_uniform i n).toCcTensor) : TensorL2 r s g) β P₀)
      atTop
      (𝓝 (i.fst.val •
        tensorL2ChartComponentCutoff (I := I) (M := M) g r s
          (tensorResolventEigenbasisVec (I := I) (M := M) h_uniform i) β P₀)) := by
  classical
  ...
  have h_term : ∀ n : ℕ,
      tensorL2ChartComponentCutoffCLM (I := I) (M := M) g r s β P₀
        ...
        tensorL2ChartComponentCutoff (I := I) (M := M) g r s
          (((eigenvectorSmoothApprox (I := I) (M := M)
            g r s h_uniform i n).toCcTensor) : TensorL2 r s g) β P₀ := by
    intro n
    rw [TensorH1ComplToTensorL2_smoothToTensorH1Compl_eq_coe (I := I) (M := M)
      g r s (eigenvectorSmoothApprox (I := I) (M := M) g r s h_uniform i n),
  ...
  have h_lim :
    ...
    have h_shadow :
      ...
      have h_eq :=
        eigenvector_eq_resolvent_smul (I := I) (M := M) g r s h_uniform i
    ...
    euclidPartial (E := E) k
      ...
      chartPushedRaw I β
        (tensorChartComponentRaw (I := I) (M := M) g r s
          (eigenvectorSmoothApprox (I := I) (M := M)
            g r s h_uniform i n).toCcTensor β P₀.1 P₀.2) y
...
private lemma crossTermCutoff_memLp
    ...
    MemLp (crossTermCutoff (I := I) (M := M) g r s h_uniform i β P₀ k n) 2
      (chartL2Measure (I := I) (M := M) β) := by
  classical
  obtain ⟨C, hC0, hC⟩ := exists_bound_crossMultiplier (I := I) (M := M) β k
...
private lemma crossTermApprox_ae_eq_crossTermCutoff
    ...
    (β : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) (n : ℕ) :
    crossTermApprox (I := I) (M := M) g r s h_uniform i β P₀ k n
      =ᵐ[chartL2Measure (I := I) (M := M) β]
      crossTermCutoff (I := I) (M := M) g r s h_uniform i β P₀ k n := by
  classical
  ...
  have h_coeFn :=
    tensorL2ChartComponentCutoff_smoothToTensorL2_coeFn (I := I) (M := M)
      g r s (eigenvectorSmoothApprox (I := I) (M := M)
        g r s h_uniform i n).toCcTensor β P₀
...
private lemma crossTermApprox_memLp
    ...
    (β : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) (n : ℕ) :
    MemLp (crossTermApprox (I := I) (M := M) g r s h_uniform i β P₀ k n) 2
      (chartL2Measure (I := I) (M := M) β) :=
  (crossTermCutoff_memLp (I := I) (M := M) g r s h_uniform i β P₀ k n).ae_eq
    (crossTermApprox_ae_eq_crossTermCutoff (I := I) (M := M)
      g r s h_uniform i β P₀ k n).symm
...
approximant `eigenvectorSmoothApprox g r s h_uniform i n` — converge, as
`n → ∞` and in `Lp ℝ 2 (chartL2Measure β)`, to the `L²` class of the explicit
limit function `covGradPouLeibnizCrossLimit g r s h_uniform i β P₀ k`.
  ...
  have h_smul :=
    (tendsto_bdd_mul (I := I) (M := M) β hC0 hC
      ...
      (cutoffComponent_smoothApprox_tendsto (I := I) (M := M)
        g r s h_uniform i β P₀)).const_smul (i.fst.val)⁻¹
  ...
  have h_lim :
    ...
    filter_upwards [MemLp.coeFn_toLp (memLp_bdd_mul (I := I) (M := M) β hC0 hC
      ...
      Lp.coeFn_smul (i.fst.val)
        (tensorL2ChartComponentCutoff (I := I) (M := M) g r s
          (tensorResolventEigenbasisVec (I := I) (M := M)
            h_uniform i) β P₀)] with y hy_toLp hy_smul
...
private lemma covGrad_chartComponent_ae_decompose
    ...
    tensorChartComponent (I := I) (M := M) g r (s + 1)
        (covGrad (I := I) (M := M) g r s
          (eigenvectorSmoothApprox (I := I) (M := M)
            g r s h_uniform i n).toCcTensor) β Q'.1 Q'.2
      =ᵐ[chartL2Measure (I := I) (M := M) β]
      fun y =>
        euclidPartial (E := E) (Q'.2 0)
            (tensorChartComponent (I := I) (M := M) g r s
              (eigenvectorSmoothApprox (I := I) (M := M)
                g r s h_uniform i n).toCcTensor β Q'.1
              (Matrix.vecTail Q'.2)) y
          - crossTermApprox (I := I) (M := M) g r s h_uniform i β
              (Q'.1, Matrix.vecTail Q'.2) (Q'.2 0) n y
          + covDerivLowerOrderTerm (I := I) (M := M) g r s
              (pouSmul (I := I) (M := M) g r s β
                (eigenvectorSmoothApprox (I := I) (M := M)
                  g r s h_uniform i n).toCcTensor) β (Q'.2 0) Q'.1
...
private lemma principalTerm_tendsto
    ...
    Filter.Tendsto
      ...
      (𝓝 (eigenvectorChartPartialLp (I := I) (M := M)
        g r s h_uniform i β (Q'.1, Matrix.vecTail Q'.2) (Q'.2 0))) := by
  classical
...
private lemma christoffelTerm_tendsto
    ...
    Filter.Tendsto
      ...
      (𝓝 ((covGradChristoffelLimit_memLp (I := I) (M := M)
        g r s h_uniform i β (Q'.1, Matrix.vecTail Q'.2) (Q'.2 0)).toLp
      ...
      (𝓝 ((i.fst.val)⁻¹ •
        tensorL2ChartComponent (I := I) (M := M) g r (s + 1)
          (tensorCovGradL2Compl (I := I) (M := M) g r s
            (eigenvectorResolvent (I := I) (M := M) g r s h_uniform i)) β Q')) := by
  classical
...
resolvent `eigenvectorResolvent g r s h_uniform i` equals, almost everywhere on
...
`eigenvectorResolvent g r s h_uniform i` has `L²`-shadow `μ • φ`
...
theorem eigenvectorCovGrad_pou_chartComponent_ae_eq
    ...
    (((i.fst.val)⁻¹ •
      ...
      =ᵐ[chartL2Measure (I := I) (M := M) β]
      (fun y =>
        eigenvectorChartWeakPartial (I := I) (M := M) g r s h_uniform i β
            (Q'.1, Matrix.vecTail Q'.2) (Q'.2 0) y
          - covGradPouLeibnizCrossLimit (I := I) (M := M) g r s h_uniform i β
              (Q'.1, Matrix.vecTail Q'.2) (Q'.2 0) y
          + covGradChristoffelLimit (I := I) (M := M) g r s h_uniform i β
  ...
  have hf2 : ∀ n : ℕ, MemLp
      ...
      (chartL2Measure (I := I) (M := M) β) := fun n =>
    crossTermApprox_memLp (I := I) (M := M) g r s h_uniform i β
  ...
  have hf3 : ∀ n : ℕ, MemLp
      (covDerivLowerOrderTerm (I := I) (M := M) g r s
        ...
        (Matrix.vecTail Q'.2)) 2
      (chartL2Measure (I := I) (M := M) β) := fun n =>
    covDerivLowerOrderTerm_pouSmul_memLp (I := I) (M := M) g r s h_uniform i β
      (Q'.1, Matrix.vecTail Q'.2) (Q'.2 0) n
  ...
  have hfA : ∀ n : ℕ, MemLp
      (tensorChartComponent (I := I) (M := M) g r (s + 1)
        (covGrad (I := I) (M := M) g r s
          (eigenvectorSmoothApprox (I := I) (M := M)
            g r s h_uniform i n).toCcTensor) β Q'.1 Q'.2) 2
      (chartL2Measure (I := I) (M := M) β) := fun n =>
  ...
  have h_tendsto_A :=
    covGrad_chartComponent_tendsto (I := I) (M := M) g r s h_uniform i β Q'
        ...
        (((i.fst.val)⁻¹ •
          ...
          fun y => (i.fst.val)⁻¹ •
            tensorChartComponent (I := I) (M := M) g r (s + 1)
              (covGrad (I := I) (M := M) g r s
                (eigenvectorSmoothApprox (I := I) (M := M)
                  g r s h_uniform i n).toCcTensor) β Q'.1 Q'.2 y := by
      filter_upwards [Lp.coeFn_smul (i.fst.val)⁻¹
        ...
        tensorL2ChartComponent_smoothToTensorL2_coeFn (I := I) (M := M)
          g r (s + 1) (covGrad (I := I) (M := M) g r s
            (eigenvectorSmoothApprox (I := I) (M := M)
              g r s h_uniform i n).toCcTensor) β Q'] with y hy_smul hy_comp
  ...
  have h_lim1 := principalTerm_tendsto (I := I) (M := M)
    g r s h_uniform i β Q' hf1
  have h_lim2 := covGradPouLeibnizCrossLimit_tendsto (I := I) (M := M)
    g r s h_uniform i β (Q'.1, Matrix.vecTail Q'.2) (Q'.2 0)
  have h_lim3 := christoffelTerm_tendsto (I := I) (M := M)
    g r s h_uniform i β Q' hf3
      ...
      Filter.Tendsto
        ...
        (𝓝 ((eigenvectorChartPartialLp (I := I) (M := M)
            ...
            (covGradPouLeibnizCrossLimit_memLp (I := I) (M := M)
              g r s h_uniform i β (Q'.1, Matrix.vecTail Q'.2) (Q'.2 0)).toLp
              (covGradPouLeibnizCrossLimit (I := I) (M := M)
                g r s h_uniform i β (Q'.1, Matrix.vecTail Q'.2)
                (Q'.2 0))) +
  ...
  filter_upwards [Lp.coeFn_add
      (eigenvectorChartPartialLp (I := I) (M := M) g r s h_uniform i β
        (Q'.1, Matrix.vecTail Q'.2) (Q'.2 0) -
      (covGradPouLeibnizCrossLimit_memLp (I := I) (M := M)
        g r s h_uniform i β (Q'.1, Matrix.vecTail Q'.2) (Q'.2 0)).toLp _)
      ((covGradChristoffelLimit_memLp (I := I) (M := M)
        g r s h_uniform i β (Q'.1, Matrix.vecTail Q'.2) (Q'.2 0)).toLp _),
    Lp.coeFn_sub
      (eigenvectorChartPartialLp (I := I) (M := M) g r s h_uniform i β
...
example (g : SmoothRiemannianMetric I M) (r s : ℕ)
    ...
    (k : Fin (Module.finrank ℝ E)) :
    EuclN → ℝ :=
  covGradPouLeibnizCrossLimit (I := I) (M := M) g r s h_uniform i β P₀ k
...
example (g : SmoothRiemannianMetric I M) (r s : ℕ)
    ...
    MemLp (covGradPouLeibnizCrossLimit (I := I) (M := M)
        g r s h_uniform i β P₀ k) 2
      (chartL2Measure (I := I) (M := M) β) :=
  covGradPouLeibnizCrossLimit_memLp (I := I) (M := M) g r s h_uniform i β P₀ k
