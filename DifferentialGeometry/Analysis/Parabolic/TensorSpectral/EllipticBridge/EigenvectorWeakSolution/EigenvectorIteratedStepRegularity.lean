...
* `eigenvectorChartIteratedStep_memWkp_K_two` — the headline regularity
  propagator at every `K ≥ 0`.
* `eigenvectorChartIteratedStep_memW1p_two` — the `K = 1` corollary, restated as
...
/-! ## Headline theorem: `MemWkp K 2` of `eigenvectorChartIteratedStep`

The standalone inductive step is the indicator, of the compact partition-of-unity
kernel `chartPouKernel α`, of the chart-density-divided differentiated numerator.
We reduce its `MemWkp K 2` regularity to that of the numerator quotient (the
substep-2a campaign lemma) by stripping the indicator through an ae-equality on
...
the level-`(m+1)` standalone inductive step `eigenvectorChartIteratedStep` lies
in `MemWkp K 2` on `chartTargetEuclid α`.

This discharges the per-step regularity propagator at every `K ≥ 0`, upgrading
the existing weighted-`L²` regularity to arbitrary chart-Sobolev regularity. It
...
At `K = 1` the headline regularity propagator gives `MemWkp 1 2
(eigenvectorChartIteratedStep)`, which `MemWkp.one_iff_memW1p` restates as
`DeGiorgi.MemW1p 2`. This is the form consumed by the iterated chart-bilinear
data: the propagator is now unconditional, given `MemWkp (m + 3) 2` regularity of
the eigenvector chart component and `MemWkp 2 2` (= chart-`H²`) regularity of the
previous-level source, plus its ae-vanishing off the partition-of-unity kernel. -/

/-- **`K = 1` corollary.** At `K = 1`, the headline regularity propagator gives
`MemWkp 1 2 (eigenvectorChartIteratedStep) = MemW1p 2 (eigenvectorChartIteratedStep)`.
