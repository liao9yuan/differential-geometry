...
* `eigenvectorCutoffChartPartialLp_approx_eq` — the `n`-th approximant cutoff
...
/-! ## The candidate weak cutoff chart partial of the eigenvector chart component
...
Applying the continuous linear map `eigenvectorCutoffChartPartialCLM g r s α P₀
k` to the `H¹`-convergence `smoothToTensorH1Compl (eigenvectorSmoothApprox … n) →
eigenvectorResolvent …` of `eigenvectorSmoothApprox_tendsto`, then rescaling by
the continuous map `μ⁻¹ • ·`, produces the headline. -/

/-- **The cutoff chart partials of an eigenvector chart component as an
`L²`-limit of smooth cutoff chart partials.** For a closed Riemannian manifold
`(M, g)`, ranks `(r, s)`, an eigenbasis index `i` with nonzero resolvent
