...
* `eigenvector_chartComponent_memWkp` — for the eigenvector chart-bilinear data
  parameters `(g, r, s, h_atlas, i, α, P₀)`, an interior subdomain `Ω''`
...
component.** For the eigenvector chart-bilinear data
`D := eigenvectorTensorChartBilinearData g r s h_atlas i α P₀`, an interior
...
nonnegative bound `M_bound i k` such that, at the difference-quotient sub-radius
`R₀ / 16`, for every `0 < |h| ≤ R₀ / 16` and every pair `(i, k)`:
...
— the analytical Caccioppoli / energy-estimate ingredients are discharged
inside that scalar lemma. The Nirenberg cutoff `η`, the precompact `Ω'`, and
the geometric scales are constructed internally from `Ω''` and `R₀`; the
sub-radius `R₀ / 16` leaves room for the cutoff support to sit strictly inside
          ...
          eLpNorm
            (DifferentialGeometry.Analysis.Sobolev.diffQuot
              ...
              ((eigenvectorTensorChartBilinearData (I := I) (M := M)
                g r s h_atlas i α P₀).weak_partial j)) 2
...
`eigenvectorTensorChartBilinearData_unconditional g r s i α P₀` (built over the
intrinsic eigenbasis vector `tensorResolventEigenbasisVec_ofCompact
(tensorResolventL2_isCompactOperator_intrinsic g r s) i`). For an interior
subdomain `Ω''` (open, relatively compact closure) and a room radius `R₀ > 0`
with `Metric.cthickening R₀ (closure Ω'') ⊆ chartTargetEuclid α`, there is a
...
private lemma eigenvectorTensorChartBilinear_uniform_diffQuot_bound_unconditional
    {g : SmoothRiemannianMetric I M} {r s : ℕ}
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    {Ω'' : Set EuclN} (hΩ''_open : IsOpen Ω'')
  ...
  -- The underlying scalar chart-bilinear divergence-form data.
  set D : TensorChartBilinearH1ComplData (I := I) (M := M) g r s α P₀ :=
    eigenvectorTensorChartBilinearData_unconditional (I := I) (M := M) g r s i α P₀
    with hD_def
  -- The interior compact set and its enclosing open chart target.
  set K_α : Set EuclN := closure Ω'' with hK_α_def
  have hK_α_compact : IsCompact K_α := hΩ''_compact_closure
...
/-- **Interior `H²`/`W^{2,2}_loc` regularity of the eigenvector chart
component.**
...
`W^{2,2}` regularity on `Ω''`. Equivalently, the explicit weak chart partials
`eigenvectorChartWeakPartial g r s h_atlas i α P₀` of the chart component
themselves admit interior weak partial derivatives in `L²(Ω'')`.

The uniform difference-quotient bound consumed by the interior-regularity
engine is **discharged** (it is proved, not assumed): via the thin wrapper
`TensorChartBilinearH1ComplData.toChartData`, the scalar unconditional uniform
bound `chartBilinearH1Compl_uniform_diffQuot_bound_of_data` delivers it, with
the Caccioppoli / energy-estimate ingredients handled inside that scalar lemma.
The interior `H²` extraction is then the tensor non-smooth Nirenberg engine
...
theorem eigenvector_chartComponent_memWkp
  ...
  classical
  -- The eigenvector chart-bilinear divergence-form data.
  set D : TensorChartBilinearH1ComplData (I := I) (M := M) g r s α P₀ :=
    eigenvectorTensorChartBilinearData (I := I) (M := M) g r s h_atlas i α P₀
    with hD_def
  ...
  obtain ⟨M_bound, hM_nn, h_diffQuot_bd⟩ :=
    eigenvectorTensorChartBilinear_uniform_diffQuot_bound
      (I := I) (M := M) (g := g) (r := r) (s := s) h_atlas i α P₀
...
the intrinsic eigenbasis vector `tensorResolventEigenbasisVec_ofCompact
(tensorResolventL2_isCompactOperator_intrinsic g r s) i` and the chart-locality-
free eigenvector chart-bilinear data `eigenvectorTensorChartBilinearData_unconditional`.
...
`tensor_h2_chart_loc_of_uniform_bound`. -/
theorem eigenvector_chartComponent_memWkp_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    {Ω'' : Set EuclN} (hΩ''_open : IsOpen Ω'')
  ...
  -- Step 1: discharge the uniform-in-`h` difference-quotient bound.
  obtain ⟨M_bound, hM_nn, h_diffQuot_bd⟩ :=
    eigenvectorTensorChartBilinear_uniform_diffQuot_bound_unconditional
...
/-- The headline produces interior `MemWkp 2 2` regularity of the eigenvector
chart component on any interior subdomain with a difference-quotient room. -/
