...
/-! ## The bounded-`K'` hypothesis structure

A bounded analogue of `sharpDiffPerK`: each `K'`-quantified bound carries
an additional precondition `K' ≤ N`, where `N : ℕ` is a parameter of the
bundle. This allows the bundle to be inhabited even when no uniform
finite bound on the chart components exists for *every* order; it
suffices that the bounds hold up to a specified ceiling. The recursion
at outer `(m, K)` queries only `K' ≤ K + m + 1`, so a bundle with
`N = K + m + 1` is enough. -/

/-- The per-`K'`-family hypothesis bundle for the level-`0` chart-RHS atoms,
*restricted* to orders `K' ≤ N`. Identical in shape to
`sharpDiffPerK`, but each `K'`-quantified bound carries a precondition
`K' ≤ N`. -/
structure sharpDiffPerKBdd
  ...
  needed to apply `eigenvectorChartRHSDiff_memWkp` at every order `K' ≤ N`. -/
...
/-! ## Derived `MemWkp` from the bounded bundle, at orders within range -/

omit [CompleteSpace E] in
/-- The level-`m` differentiated chart RHS is `MemWkp K' 2` for any chain `K'`
with `m + 1 + K' ≤ N`, derived via `eigenvectorChartRHSDiff_memWkp` from the
structural resolvent chart-component regularity input in the bounded bundle. -/
...
/-! ## Bounded-`K'` analogue of the per-`K`-family energy bound for the
level-`0` source aggregate

Mirrors `rhsZeroAggregate_le_energy_perK`, but consumes the bounded
hypothesis bundle `sharpDiffPerKBdd ... N` with the precondition
`K + 1 ≤ N`. The proof mirrors the universally-quantified version,
discharging the `K' ≤ N` preconditions of each `H.hC*_bd` use site by
`Nat.le_trans` from `K ≤ N` (or `K + 1 ≤ N` for `H.hCresH_bd`). -/

set_option maxHeartbeats 1600000 in
set_option synthInstance.maxHeartbeats 1000000 in
/-- Bounded-`K'` analogue of `rhsZeroAggregate_le_energy_perK`. Given a
bounded hypothesis bundle `H : sharpDiffPerKBdd … N` with `K + 1 ≤ N`,
the level-`0` chart-RHS source aggregate at order `K` is bounded by
`ofReal(C · μ⁻¹^e) · ‖tensorResolventEigenbasisVec h_atlas i‖`. The
proof is the per-`K`-family proof of
`rhsZeroAggregate_le_energy_perK`, adapted to consume the bounded
bundle. -/
lemma rhsZeroAggregate_le_energy_perK_bdd
  ...
  classical
  -- The bounded hypothesis bundle yields each `H.hC*_bd i ... K' (hK'_le_N)`
  -- for `K' ≤ N`. We have `K + 1 ≤ N`, hence `K ≤ N`. Bind specialised
  -- predicates at order `K` (or `K + 1` for `hCresH_bd`), then feed them to
  -- the per-`K`-family aggregate bound `rhsZeroAggregate_le_energy_perK`.
  have hK_le_N : K ≤ N := by omega
  -- Specialised hypothesis predicates, packaged in the universally-quantified
  -- shape consumed by `rhsZeroAggregate_le_energy_perK`. The bound predicates
  -- are universally quantified over `K' : ℕ`; we package the bounded data by
  -- *specialising at the single value `K' = K` (and `K' = K + 1` for the
  -- high-order resolvent atom)*. The other `K'` values of the universal
  -- binding are inhabited by the same bounded hypothesis applied at `K`, after
  -- a rewriting that effectively swaps the family parameter `K'` for the
  -- fixed `K`. To stay within the universally-bound shape, we provide
  -- **monotone-dominating** families: `Ceig' K' := H.Ceig K`, `eEig' K' := H.eEig K`,
  -- so the family-bound `wkpNorm K' (chart cpt) ≤ ofReal(Ceig' K' · μ⁻¹^(eEig' K'))
  -- · ‖vec‖` must hold for *every* K' — which is impossible without further data.
  --
  -- We bypass this by INLINING the seven-summand proof of
  -- `rhsZeroAggregate_le_energy_perK` directly, using the bounded predicates
  -- at `K` (and `K + 1` for `hCresH_bd`).
  -- Set per-summand cardinal collapse constants at the fixed `K`, mirroring
  -- the per-K proof body.
  ...
  set Cagg : ℝ := H.Ceig K + Cmid_α + Clow_α + Cpar' + Ccom' + CcR' + Ccut'
    with hCagg_def
  -- Common exponent: maximum over the eight per-`K`-family exponents
  -- consumed at orders `K` and `K + 1`.
  set e_max : ℕ :=
  ...
  -- Eigenvalue facts.
  have hvec_norm : ‖tensorResolventEigenbasisVec (I := I) (M := M) h_atlas i‖ = 1 :=
...
/-! ## The bounded-`K'` analogue of the level-`0` sharp `wkpNorm K` bound -/

omit [CompleteSpace E] in
/-- **Bounded-`K'` analogue of `sharpDiff_level_zero_wkpNorm`.**

The chart RHS at level `0` is `μ⁻¹ • (a seven-term bracket)`. The seven
bounded-`K'`-family atom bounds collapse the bracket via
`rhsZeroAggregate_le_energy_perK_bdd`; combining with the scaling factor
`μ⁻¹` (a single power of `μ⁻¹` from the chart-RHS smul identity) produces
a single `wkpNorm K`-bound for the chart RHS at level `0`. Requires
`K + 1 ≤ N` so that the source atoms at orders `K` and `K + 1` are
available from the bounded bundle. -/
private lemma sharpDiffBdd_level_zero_wkpNorm
  ...
  obtain ⟨Cagg, eAgg, hCagg_nn, hCagg_bd⟩ :=
    rhsZeroAggregate_le_energy_perK_bdd (I := I) (M := M) g r s h_atlas α P₀ K N
      hKN H
  -- The `μ⁻¹`-prefactor bound by the seven-summand source aggregate, derived
  -- from the resolvent chart-cpt regularity input in the bounded bundle.
...
/-! ## The main bounded-`K'` chain-length-aware sharp `wkpNorm K` recursion -/

set_option maxHeartbeats 32000000 in
omit [CompleteSpace E] in
/-- **Bounded-`K'` chain-length-aware sharp `wkpNorm K` recursion for the
differentiated chart RHS.**

Given the bounded hypothesis bundle `H : sharpDiffPerKBdd … N` with
`N := K + m + 1` (the maximum order queried during the recursion), for
every level `m` and chain `K` (with `K + m + 1 ≤ N`), there is a single
nonnegative constant `C` and an exponent `e` such that, for every
...
recursion invokes the inductive hypothesis at chains `K` and `K + 1`
(both `≤ N` because the outer `K + m + 1 ≤ N` allows growth by one). -/
private lemma sharpDiffBdd_recursion
  ...
  | zero =>
      ...
      -- We need `K + 1 ≤ N`. From `K + 0 + 1 ≤ N` we get `K + 1 ≤ N`.
      have hK1_le_N : K + 1 ≤ N := by simpa using hKN
      obtain ⟨C, e, hC_nn, hC_bd⟩ :=
        sharpDiffBdd_level_zero_wkpNorm (I := I) (M := M) g r s h_atlas α P₀ K N
          hK1_le_N H
  ...
  | succ m ih =>
      ...
      -- `Fin.init l`. The chain `K + 1` is needed for Layer E. Both must
      -- satisfy `chain + m + 1 ≤ N`. For chain `K`: `K + m + 1 ≤ N` since
      -- `K + (m + 1) + 1 = K + m + 2 ≤ N`. For chain `K + 1`:
      -- `(K + 1) + m + 1 = K + m + 2 ≤ N`. Both follow from the outer
      -- hypothesis `K + (m + 1) + 1 ≤ N`.
      ...
      -- `f_chart_m` is in `MemWkp (K+1) 2` (via `sharpDiffBdd_diff_memWkp`),
      -- and `f_chart_m` ae-vanishes off `chartPouKernel α`.
      -- For the diff memWkp at level `m` chain `K+1`, we need
      -- `m + 1 + (K + 1) ≤ N`, i.e. `K + m + 2 ≤ N`. This follows from
      -- `K + (m + 1) + 1 ≤ N` (= the outer `hKN`).
      ...
      have h_prev_mem_succ : ∀ i : TensorEigenIdx (I := I) (M := M) g r s,
        ...
        sharpDiffBdd_diff_memWkp (I := I) (M := M) g r s h_atlas α P₀ N H i
      ...
      -- The chart-cpt bound is at order `K + m + 1 ≤ N`.
      ...
      -- The chart-cpt bound is at order `K + m + 2 ≤ N` since
      -- `(K + 1) + (m + 1) = K + m + 2` and outer `K + (m + 1) + 1 ≤ N` gives
      -- `K + m + 2 ≤ N`.
      ...
      -- Now apply the sharp numerator bound.
...
/-! ## The public bounded-`K'` headline -/

/-- **Bounded-`K'` sharp chain-length-aware order-`K` `wkpNorm` bound on the
differentiated chart RHS.**

For a closed Riemannian manifold `(M, g)`, ranks `(r, s)`, an eigenbasis
chart center `α : M`, a component multi-index `P₀`, a level `m`, a
regularity order `K`, a direction multi-index `l : Fin m → Fin n`, and a
bounded *per-`K'`-family* hypothesis bundle
`H : sharpDiffPerKBdd g r s h_atlas α P₀ (K + m + 1)`, there is a
nonnegative constant `C : ℝ` and an exponent `e : ℕ` such that, for
*every* eigenbasis index `i`, the order-`K` `wkpNorm` of the level-`m`
differentiated chart RHS is bounded by `ofReal(C · μ⁻¹^e) · ‖vec‖`.

The bundle parameter `N := K + m + 1` is the precise upper bound on the
order `K'` queried during the recursion. -/
      ...
      ∀ i : TensorEigenIdx (I := I) (M := M) g r s,
        wkpNorm (d := Module.finrank ℝ E) K 2
          ...
          ≤ ENNReal.ofReal (C * (i.fst.val)⁻¹ ^ e) *
            ENNReal.ofReal
              ‖tensorResolventEigenbasisVec (I := I) (M := M) h_atlas i‖ :=
  sharpDiffBdd_recursion (I := I) (M := M) g r s h_atlas α P₀ (K + m + 1) H m K
    (le_refl _) l
