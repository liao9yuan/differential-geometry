...
this campaign hand us the cutoff chart component and the cutoff chart partial of
an approximant in two presentations: the concrete `C^∞` chart-coordinate scalar
...
continuous linear map `TensorH1ComplToTensorL2`, rewriting the `n`-th term by
`TensorH1ComplToTensorL2_smoothToTensorH1Compl_eq_coe`, and rescaling by the
continuous map `μ⁻¹ • ·` shows that the `μ⁻¹`-rescaled `L²`-coercions of the
underlying smooth `L²` sections `(eigenvectorSmoothApprox … n).toCcTensor`
converge, in `TensorL2 r s g`, to the eigenvector `φ`. -/
...
/-! ## The almost-everywhere descriptions of the approximant cutoff chart data

For each `n`, the `n`-th approximant cutoff chart component and the `n`-th
approximant cutoff chart partial are `Lp` classes; their coercions-to-functions
agree almost everywhere with the concrete `μ⁻¹`-rescaled cutoff Euclidean chart
component, respectively the `μ⁻¹`-rescaled concrete chosen weak `k`-th cutoff
chart partial of that cutoff chart component. These descriptions feed both the
per-approximant genuine-weak-partial fact and the `eLpNorm`-convergence
bridges. -/

/-- The coercion-to-function of the cutoff chart component of the `μ⁻¹`-rescaled
`L²`-coercion of the smooth section `(eigenvectorSmoothApprox … n).toCcTensor`
agrees, almost everywhere with respect to `chartL2Measure α`, with `μ⁻¹` times
the concrete cutoff Euclidean chart component `cutoffComponentEuclid g r s
...
agrees, almost everywhere with respect to `chartL2Measure α`, with `μ⁻¹` times
the concrete chosen weak `k`-th cutoff chart partial of the cutoff Euclidean
...
/-! ## Per-approximant genuine weak cutoff chart partial

The cutoff Euclidean chart component of the smooth section
...
(`cutoffComponentEuclid_memW1p`). Its `chosenWeakPartial'` is therefore a genuine
weak `k`-th cutoff chart partial (`chosenWeakPartial'_isWeakPartial_of_mem`).
Scaling by `μ⁻¹` and rewriting both arguments by their almost-everywhere `L²`
descriptions transports this to the `Lp`-class presentation of the approximant
cutoff chart data. -/

/-- For each `n`, the coercion-to-function of the `n`-th approximant cutoff chart
...
private lemma eigenvectorCutoffChartWeakPartial_approx_hasWeakPartialDeriv
  ...
  classical
  -- The cutoff chart component of the smooth approximant lies in `W^{1,2}`.
  have h_w1p :
...
coercion of a continuous linear map. Applying that continuous linear map to the
`μ⁻¹`-rescaled `L²`-convergence `cutoff_smoothApprox_smul_coe_tendsto` produces
the `L²`-convergence of the cutoff chart components of the `μ⁻¹`-rescaled smooth
approximants to the cutoff chart component of the eigenvector. -/
...
The cutoff-chart-component convergence `eigenvectorCutoffChartComponentL2_tendsto`
and the companion-file headline `eigenvectorCutoffChartPartialLp_tendsto` are
convergences in the `Lp ℝ 2 (chartL2Measure α)` norm. By
`Lp.tendsto_Lp_iff_tendsto_eLpNorm'`, convergence of `Lp` classes is exactly
convergence of the `eLpNorm` of the coercion-to-function difference to `0` — the
form required by the `L²`-closure theorem
`hasWeakPartialDeriv_of_tendsto_eLpNorm`. -/

/-- The coercions-to-functions of the approximant cutoff chart components
converge to the coercion-to-function of the eigenvector cutoff chart component,
...
* the per-approximant genuine weak cutoff chart partials
  (`eigenvectorCutoffChartWeakPartial_approx_hasWeakPartialDeriv`);
* the `eLpNorm`-convergence of the cutoff chart components
  (`eigenvectorCutoffChartComponent_eLpNorm_tendsto`);
* the `eLpNorm`-convergence of the cutoff chart partials
...
/-- **The eigenvector cutoff chart partial is a genuine weak cutoff chart
...
headline `eigenvectorChartWeakPartial_hasWeakPartialDeriv`: both `eigenvector`
...
theorem eigenvectorCutoffChartPartialLp_hasWeakPartialDeriv
  ...
  -- Abbreviations for the approximant cutoff chart component / partial classes.
  ...
  -- `gLim` and `uLim` are, by definition, the headline arguments.
  rw [hgLim_def, huLim_def] at h_closure
  exact h_closure

/-! ## Local `L²`-integrability of the weak cutoff chart partial

`eigenvectorCutoffChartWeakPartial` is the coercion-to-function of an `Lp ℝ 2
(chartL2Measure α)` class, hence in `MemLp 2` of `chartL2Measure α =
volume.restrict (chartTargetEuclid α)`. Restricting the reference measure to any
compact subset of the chart target is a smaller measure, so membership is
inherited. -/
...
`h_atlas` enters only through the eigenvector and its canonical smooth
approximating sequence; both have chart-locality-free twins in the companion
files (`SmoothApprox.lean`, `EigenvectorChartComponentL2.lean`,
`EigenvectorCutoffChartPartialL2.lean`). The eigenvector itself is re-keyed onto
...
(`hasWeakPartialDeriv_congr_ae`, `hasWeakPartialDeriv_const_smul`) carry no
`h_atlas`, so they are reused verbatim. -/

/-- **The weak `k`-th cutoff chart partial of an eigenvector cutoff chart
component (chart-locality-free).** Chart-locality-free twin of
`eigenvectorCutoffChartWeakPartial`. -/
def eigenvectorCutoffChartWeakPartial_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) : EuclN → ℝ :=
...
private lemma cutoff_smoothApprox_smul_coe_tendsto_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s) :
        ...
        i.fst.val •
          tensorResolventEigenbasisVec_ofCompact (I := I) (M := M)
            (tensorResolventL2_isCompactOperator_intrinsic (I := I) (M := M)
              g r s) i := by
    have h_eq := eigenvector_eq_resolvent_smul_unconditional
      (I := I) (M := M) g r s i
    rw [h_eq, smul_smul, mul_inv_cancel₀ hμ_ne, one_smul]
  rw [h_term, h_shadow] at h_l2
  -- Scalar multiplication by `μ⁻¹` is continuous; `μ⁻¹ • (μ • φ) = φ`.
...
lemma eigenvectorCutoffChartComponentL2_approx_coeFn_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) (n : ℕ) :
    ((tensorL2ChartComponentCutoff (I := I) (M := M) g r s
...
lemma eigenvectorCutoffChartPartialLp_approx_coeFn_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) (n : ℕ) :
...
`eigenvectorCutoffChartWeakPartial_approx_hasWeakPartialDeriv`. -/
private lemma eigenvectorCutoffChartWeakPartial_approx_hasWeakPartialDeriv_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) (n : ℕ) :
  ...
  refine hasWeakPartialDeriv_congr_ae ?_ ?_ h_weak_smul
  · exact (eigenvectorCutoffChartComponentL2_approx_coeFn_unconditional
      (I := I) (M := M) g r s i α P₀ n).symm
  · exact (eigenvectorCutoffChartPartialLp_approx_coeFn_unconditional
      (I := I) (M := M) g r s i α P₀ k n).symm

open DifferentialGeometry.PDE.RicciFlow.IntrinsicSpectral in
/-- Chart-locality-free twin of `eigenvectorCutoffChartComponentL2_tendsto`. -/
private lemma eigenvectorCutoffChartComponentL2_tendsto_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) :
    letI : CompleteSpace E := FiniteDimensional.complete ℝ E
  ...
  have h_clm :=
    ((tensorL2ChartComponentCutoffCLM (I := I) (M := M)
      ...
      (cutoff_smoothApprox_smul_coe_tendsto_unconditional (I := I) (M := M) g r s i)
  simp only [Function.comp_def, tensorL2ChartComponentCutoffCLM_apply] at h_clm
  exact h_clm
...
/-- Chart-locality-free twin of
`eigenvectorCutoffChartComponent_eLpNorm_tendsto`. -/
private lemma eigenvectorCutoffChartComponent_eLpNorm_tendsto_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s) :
    letI : CompleteSpace E := FiniteDimensional.complete ℝ E
  ...
  exact (MeasureTheory.Lp.tendsto_Lp_iff_tendsto_eLpNorm'
    ...
    (tensorL2ChartComponentCutoff (I := I) (M := M) g r s
      ...
      α P₀)).mp
    (eigenvectorCutoffChartComponentL2_tendsto_unconditional (I := I) (M := M)
      g r s i α P₀)

/-- Chart-locality-free twin of `eigenvectorCutoffChartPartial_eLpNorm_tendsto`. -/
private lemma eigenvectorCutoffChartPartial_eLpNorm_tendsto_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) :
...
theorem eigenvectorCutoffChartPartialLp_hasWeakPartialDeriv_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E)) :
  ...
  have h_weak : ∀ n,
    ...
    intro n
    simp only [hgApprox_def, huApprox_def]
    exact eigenvectorCutoffChartWeakPartial_approx_hasWeakPartialDeriv_unconditional
      (I := I) (M := M) g r s i α P₀ k n
  -- `eLpNorm`-convergence of the cutoff chart components.
  have h_u_tendsto :
      Filter.Tendsto
        (fun n => eLpNorm (fun x => uApprox n x - uLim x) 2
          ((volume : Measure EuclN).restrict
            (chartTargetEuclid (I := I) (M := M) α)))
        atTop (𝓝 0) := by
    have h := eigenvectorCutoffChartComponent_eLpNorm_tendsto_unconditional
      (I := I) (M := M) g r s i α P₀
    simp only [huApprox_def, huLim_def]
    exact h
  -- `eLpNorm`-convergence of the cutoff chart partials.
      ...
      Filter.Tendsto
        (fun n => eLpNorm (fun x => gApprox n x - gLim x) 2
          ((volume : Measure EuclN).restrict
            (chartTargetEuclid (I := I) (M := M) α)))
        atTop (𝓝 0) := by
    have h := eigenvectorCutoffChartPartial_eLpNorm_tendsto_unconditional
...
(chart-locality-free).** Chart-locality-free twin of
`eigenvectorCutoffChartWeakPartial_locally_memLp`. -/
theorem eigenvectorCutoffChartWeakPartial_locally_memLp_unconditional
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (i : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
    (k : Fin (Module.finrank ℝ E))
    {K : Set EuclN} (hK : IsCompact K)
    (hK_in : K ⊆ chartTargetEuclid (I := I) (M := M) α) :
    MemLp (eigenvectorCutoffChartWeakPartial_unconditional (I := I) (M := M)
        g r s i α P₀ k) 2
      ((volume : Measure EuclN).restrict K) := by
  classical
  let _ := hK
  -- The coercion-to-function is in `MemLp 2` of the chart's `L²` measure.
  have h_memLp : MemLp (eigenvectorCutoffChartWeakPartial_unconditional
      (I := I) (M := M) g r s i α P₀ k) 2
      (chartL2Measure (I := I) (M := M) α) := by
    rw [eigenvectorCutoffChartWeakPartial_unconditional]
    exact Lp.memLp _
  -- Restricting to a subset of the chart target is a smaller measure.
  have h_le : (volume : Measure EuclN).restrict K ≤
      chartL2Measure (I := I) (M := M) α := by
