...
/-! ## Real-arithmetic helper for the geometric-energy bound

The quantitative interior order-2 engine `tensor_h2_chart_loc_of_data_quantitative`
bounds each extracted weak second partial by `C_geom i k · √(DATA D)`, where the
energy `DATA D` collects the squared `L²(closure Ω')`-norms of the weak partials,
of `u_chart`, and of `f_chart`. The lemma below converts a `√(DATA)` bound into a
linear bound: if every weak-partial / `u_chart` term is `≤ A` and the `f_chart`
term is `≤ K_f · B`, then `√(DATA) ≤ √(n + 1 + K_f²) · (A + B)`. -/
...
private lemma sqrt_geometricEnergy_le
  ...
  -- The energy is bounded by `(√(n + 1 + K_f²) · (A + B))²`.
  ...
  have h_f_le : a_f ^ 2 ≤ K_f ^ 2 * B ^ 2 := by
    ...
    calc a_f ^ 2 ≤ (K_f * B) ^ 2 := h1
      _ = K_f ^ 2 * B ^ 2 := by ring
  -- Step 4: combine into `DATA ≤ (n + 1 + K_f²) · (A + B)²`.
  have h_energy_le :
...
/-! ## Density-comparison helper for the `f_chart` energy term

The geometric-energy term involving `f_chart` is measured in the plain volume
`L²` on the compact set `closure Ω'`, whereas the headline bound exposes the
`f_chart` `L²`-norm in the chart-pulled weighted measure on the full chart
target. The lemma below relates the two via the dominating-measure bound
`volume_restrict_compact_le_chartPulledWeightedMeasure`. -/
...
private lemma eLpNorm_volume_restrict_compact_le_weighted
  ...
  classical
  -- The dominating-measure bound: `volume.restrict K ≤ ofReal c • μ_w.restrict`.
  obtain ⟨c, _hc_pos, h_le⟩ :=
    volume_restrict_compact_le_chartPulledWeightedMeasure (I := I) (M := M)
      (g := g) α hK_compact hK_meas hK_in
  -- The `(1/2)`-power factor produced by `eLpNorm` measure-scaling.
  set c_d_enn : ℝ≥0∞ :=
  ...
  -- `eLpNorm f 2 (volume.restrict K) ≤ eLpNorm f 2 (ofReal c • μ_w.restrict)`.
  have h_mono :
      eLpNorm f 2 ((volume : Measure EuclN).restrict K) ≤
  ...
  -- `eLpNorm f 2 (ofReal c • ν) = c_d_enn • eLpNorm f 2 ν`.
  have h_smul :
      eLpNorm f 2 (ENNReal.ofReal c •
          ((chartPulledWeightedMeasure (I := I) g α).restrict
            (chartTargetEuclid (I := I) (M := M) α))) =
    ...
    exact eLpNorm_smul_measure_of_ne_top (by norm_num : (2 : ℝ≥0∞) ≠ ⊤) f
      (ENNReal.ofReal c)
  -- `c_d_enn • x = ENNReal.ofReal c_d_enn.toReal * x`.
  have h_smul_eq :
      c_d_enn •
        eLpNorm f 2 ((chartPulledWeightedMeasure (I := I) g α).restrict
          (chartTargetEuclid (I := I) (M := M) α)) =
...
private lemma eLpNorm_volume_restrict_compact_le_weighted_uniform
  ...
  classical
  -- The dominating-measure bound: `volume.restrict K ≤ ofReal c • μ_w.restrict`.
  -- The scalar `c` is produced from `g`, `α`, `K` only — integrand-free.
  obtain ⟨c, _hc_pos, h_le⟩ :=
    volume_restrict_compact_le_chartPulledWeightedMeasure (I := I) (M := M)
      (g := g) α hK_compact hK_meas hK_in
  -- The `(1/2)`-power factor produced by `eLpNorm` measure-scaling.
  ...
  have hc_d_enn_ne_top : c_d_enn ≠ ⊤ := by
    rw [hc_d_enn_def]
    exact ENNReal.rpow_ne_top_of_nonneg ENNReal.toReal_nonneg ENNReal.ofReal_ne_top
  -- Fix the integrand-free scalar, then introduce the integrand.
...
/-! ## Headline: quantitative weakened Nirenberg interior `W^{2,2}` of the
iterated mixed weak partial

The headline keeps the region setup of the qualitative
`eigenvectorChartIteratedPartial_memWkp_two_two`, constructs the smooth
Nirenberg cutoff `η` that the quantitative interior order-2 engine requires,
runs that engine for an explicit per-`(i, k)` weak-second-partial bound,
assembles the interior `W^{2,2}`-norm via the `wkpNorm` order-`(k+1)`
decomposition, and promotes the interior norm to a global `wkpNorm`-bound on the
chart target via the quantitative support-aware promotion. -/

set_option maxHeartbeats 1600000 in
/-- **Quantitative weakened Nirenberg interior `W^{2,2}` of the iterated mixed
weak partial.**
...
* lies in `MemWkp 2 2 … (chartTargetEuclid α)` — global `W^{2,2}` regularity;
* has an explicit `W^{2,2}`-norm bound: there is a constant `C > 0` with
  `wkpNorm 2 2 … (chartTargetEuclid α) ≤ ENNReal.ofReal C · (wkpNorm 1 2 …
  (chartTargetEuclid α) + eLpNorm f_chart 2 …)`, where `f_chart` is the
  carrier's effective `L²` source measured in the chart-pulled weighted measure.

The first conjunct is exactly the qualitative headline
`eigenvectorChartIteratedPartial_memWkp_two_two`.

The second conjunct keeps the region setup of the qualitative proof — the
compact partition-of-unity kernel `K`, the precompact interior subdomain
`Ω'' := thickening (R_α/2) K`, the difference-quotient room radius
`R₀ := R_α/4` — and replaces the qualitative order-2 elliptic engine with the
quantitative interior engine `tensor_h2_chart_loc_of_data_quantitative`. The
smooth Nirenberg cutoff `η` and the precompact target `Ω'` that the quantitative
engine consumes are constructed internally, mirroring the cutoff construction of
`eigenvectorTensorChartBilinear_uniform_diffQuot_bound`. The interior
`W^{2,2}`-norm is assembled from the order-`(k+1)` `wkpNorm` decomposition
`wkpNorm_succ_eq_eLpNorm_add_sum_partial`, each weak-second-partial term being
controlled by the quantitative engine's explicit constant, and the quantitative
support-aware promotion `wkpNorm_le_of_memWkp_precompact_of_ae_zero_off_compact`
raises the interior bound to the global chart-target `wkpNorm`-bound — the
`m`-fold mixed weak partial vanishes almost everywhere off the compact kernel.

This is the explicit-norm-bound analogue of
`eigenvectorChartIteratedPartial_memWkp_two_two`. -/
theorem eigenvectorChartIteratedPartial_wkpNorm_two_two_le
  ...
  -- The first conjunct: the qualitative global `W^{2,2}` headline.
  have h_first :
      DifferentialGeometry.Analysis.Sobolev.Euclidean.MemWkp
        (d := Module.finrank ℝ E) 2 2
        (eigenvectorChartIteratedPartial (I := I) (M := M)
  ...
  -- The principal factor: the `m`-fold mixed weak partial along `directions`.
  -- Naming it `P` folds it in `h_first` and in the remaining goal.
  set P : EuclN → ℝ :=
    eigenvectorChartIteratedPartial (I := I) (M := M)
      g r s h_atlas i α P₀ m directions with hP_def
  -- `D.u_chart = P` after rewriting `D_m.directions` to `directions`.
  have hD_u_chart_P : D.u_chart = P := by rw [hD_u_chart, hP_def, h_dir]
  -- `D.weak_partial j` is the canonical chosen weak `j`-partial of `P` on the
  -- chart target.
  have hD_weak_partial : ∀ j,
    ...
    rw [h_dir]
  -- `D.f_chart = D_m.fChartEff` definitionally.
  ...
  -- The qualitative order-2 interior elliptic engine: `D.u_chart` and every
  -- `D.weak_partial j` lie in `W^{1,2}(Ω'')`.
  ...
  -- The interior `W^{2,2}` of `D.u_chart` on `Ω''` (assembled below as in the
  -- qualitative proof, supplying the finiteness input for the promotion).
  ...
  /- ## The smooth Nirenberg cutoff for the quantitative interior engine

  Mirroring the cutoff construction of
  `eigenvectorTensorChartBilinear_uniform_diffQuot_bound`: the geometric scales
  are sixteenths of the room radius `R₀`; the precompact target is
  `Ω' := thickening (8ε) (closure Ω'')`; the cutoff `η` is `1` on a
  neighbourhood of `cthickening (3ε) (closure Ω'')` and is supported inside
  `thickening (5ε) (closure Ω'')`. -/
  ...
  -- The precompact target `Ω' := thickening (8ε) K_Ω` inside the chart target.
  ...
  -- The smooth Nirenberg cutoff `η`: `≡ 1` on a neighbourhood of
  -- `cthickening (3ε) K_Ω`, supported inside `thickening (5ε) K_Ω`.
  ...
  -- `tsupport η ⊆ Ω'` (cutoff support inside the precompact target).
  ...
  -- The difference-quotient room: `cthickening |h| (tsupport η) ⊆ Ω'` for
  -- `|h| ≤ ε`.
  ...
  -- The diff-quot radius `ε`'s room hypothesis for the quantitative engine.
  have h_room_ε : Metric.cthickening ε (closure Ω'') ⊆
      chartTargetEuclid (I := I) (M := M) α :=
    (Metric.cthickening_mono (by rw [hε_def]; linarith) (closure Ω'')).trans h_room
  /- ## The quantitative interior order-2 elliptic engine

  Q3 produces a chart-geometric constant `C_geom i k`, uniform over the bilinear
  data, and for every pair `(i, k)` a weak `k`-partial `g_{i,k} ∈ L²(Ω'')` of
  `D.weak_partial i` with `‖g_{i,k}‖_{L²(Ω'')} ≤ C_geom i k · √(DATA D)`. -/
  ...
  -- All region/principal-factor construction is complete; opacify the `set`
  -- variables so downstream theorem applications do not unfold their
  -- definitions during strict-implicit-argument unification.
  clear_value P K Ω'' Ω'
  /- ## Assembly of the interior `W^{2,2}`-norm bound

  Abbreviation: `RHS_inner` is the headline's inner expression (the chart-target
  `W^{1,2}`-norm of `P` plus the weighted `L²`-norm of `f_chart`). -/
  set RHS_inner : ℝ≥0∞ :=
    DifferentialGeometry.Analysis.Sobolev.Euclidean.wkpNorm
  ...
  -- `P` lies in `W^{1,2}` of the chart target (from the qualitative `W^{2,2}`).
  have h_P_W1 :
      DifferentialGeometry.Analysis.Sobolev.Euclidean.MemWkp
  ...
  -- The chart-target `W^{1,2}`-norm of `P` is finite.
  have h_wkpNorm1_finite :
    ...
    DifferentialGeometry.Analysis.Sobolev.Euclidean.wkpNorm_lt_top_of_memWkp
      h_P_W1
  -- The carrier's effective source is weighted-`MemLp 2`.
  have h_fchart_memLp :
      MemLp D.f_chart 2
        ((chartPulledWeightedMeasure (I := I) g α).restrict
          (chartTargetEuclid (I := I) (M := M) α)) := by
  ...
  -- The real scalars `A`, `B` for the energy comparison.
  set A : ℝ :=
    (DifferentialGeometry.Analysis.Sobolev.Euclidean.wkpNorm
      (d := Module.finrank ℝ E) 1 2 P
  ...
  -- `ENNReal.ofReal A ≤ wkpNorm 1 2 P chartTarget` and likewise for `B`.
  have h_ofReal_A_le :
      ENNReal.ofReal A ≤
        DifferentialGeometry.Analysis.Sobolev.Euclidean.wkpNorm
          (d := Module.finrank ℝ E) 1 2 P
  ...
  -- `ENNReal.ofReal A + ENNReal.ofReal B ≤ RHS_inner`.
  have h_ofReal_AB_le_RHS :
      ENNReal.ofReal A + ENNReal.ofReal B ≤ RHS_inner := by
    rw [hRHS_inner_def]
    exact add_le_add h_ofReal_A_le h_ofReal_B_le
  -- The density-comparison constant for the `f_chart` energy term, on the
  -- compact set `closure Ω'`.
  ...
  /- ### Energy bounds: every weak-partial / `u_chart` term is `≤ A`, the
  `f_chart` term is `≤ c_d · B`. -/
  -- Monotonicity: `volume.restrict (closure Ω') ≤ volume.restrict chartTarget`.
  ...
  -- Each `eLpNorm (D.weak_partial l) 2 (restrict closure Ω')` is bounded by the
  -- chart-target `W^{1,2}`-norm of `P`.
  ...
  -- `eLpNorm D.u_chart 2 (restrict closure Ω')` is bounded by the chart-target
  -- `W^{1,2}`-norm of `P`.
  ...
  /- ### The geometric energy `√`-bound

  The energy `DATA D` collects the squared `L²(closure Ω')`-norms; by
  `sqrt_geometricEnergy_le` its square root is bounded by `C₂ · (A + B)` with
  `C₂ := √(n + 1 + c_d²)`. -/
  set C₂ : ℝ :=
  ...
  `C_geom_max` is the (nonnegative) sum of all chart-geometric constants — an
  upper bound for every `C_geom i k`. The final per-`(i, k)` constant is
  `C₁ := C_geom_max · C₂`. -/
  set C_geom_max : ℝ :=
  ...
  /- ### Per-`(i, k)`: the extracted weak second partial is `L²`-bounded by
  `ofReal C₁ · RHS_inner`. -/
  have h_gik_bound : ∀ i' k' : Fin (Module.finrank ℝ E),
    ...
    -- Transport to `ℝ≥0∞`: `ofReal (C_geom i' k' · √(DATA D)) ≤ ofReal C₁ ·
    -- RHS_inner`.
  ...
  /- ### Per-`i`: the order-`1` `wkpNorm` of the chosen weak `i`-partial of
  `D.u_chart` on `Ω''` is bounded by `RHS_inner + ∑ₖ ofReal C₁ · RHS_inner`. -/
  -- The chosen weak `i`-partial of `D.u_chart` on `Ω''` agrees a.e. with the
  -- engine's weak partial `D.weak_partial i`.
  ...
  have h_per_i_bound : ∀ i' : Fin (Module.finrank ℝ E),
    ...
    intro i'
    -- Rewrite the order-`1` norm of the chosen partial as that of
    -- `D.weak_partial i'` (a.e.-equal on `Ω''`).
    rw [DifferentialGeometry.Analysis.Sobolev.Euclidean.wkpNorm_congr_ae
      (by norm_num : (1 : ℝ≥0∞) ≤ 2) hΩ''_open (h_chosen_ae_wp i')]
    -- Decompose the order-`1` norm: `eLpNorm (D.weak_partial i') + ∑ₖ
    -- eLpNorm (chosenWeakPartial' 2 k (D.weak_partial i') Ω'')`.
    ...
    · -- `eLpNorm (D.weak_partial i') 2 (restrict Ω'') ≤ RHS_inner`.
      have h_mono :
    ...
    · -- `wkpNorm 0 2 (chosenWeakPartial' 2 k' (D.weak_partial i') Ω'') Ω''
      -- = eLpNorm (chosenWeakPartial' 2 k' (D.weak_partial i') Ω'')`, which is
      -- a.e.-equal to the engine's `g_{i',k'}` and hence `≤ ofReal C₁ ·
      -- RHS_inner`.
      ...
      -- `chosenWeakPartial' 2 k' (D.weak_partial i') Ω''` and `g_{i',k'}` are
      -- both weak `k'`-partials of `D.weak_partial i'` on `Ω''`, hence a.e.
      -- equal.
      have h_chosen_partial : DeGiorgi.HasWeakPartialDeriv
      ...
      exact hg_ik_bd
  /- ### Assembling the interior `W^{2,2}`-norm on `Ω''`

  Decompose `wkpNorm 2 2 D.u_chart Ω''` via the order-`(k+1)` recursion and
  bound each summand. -/
  set C' : ℝ :=
  ...
  have h_interior_bound :
    ...
    -- Bound the sum: each summand `≤ RHS_inner + ∑ₖ ofReal C₁ · RHS_inner`.
    have h_sum_le :
        (∑ i' : Fin (Module.finrank ℝ E),
          DifferentialGeometry.Analysis.Sobolev.Euclidean.wkpNorm
            (d := Module.finrank ℝ E) 1 2
    ...
    -- Collapse the constant sums and identify with `ofReal C' · RHS_inner`.
    refine (add_le_add h_uChart_le_RHS h_sum_le).trans ?_
    -- `RHS_inner + ∑ᵢ (RHS_inner + ∑ₖ ofReal C₁ · RHS_inner)
    --   = ofReal C' · RHS_inner`.
    ...
    rw [h_ofReal_C']
    -- Pure `ℝ≥0∞` algebra: convert the `ℕ`-smuls to multiplications and
    -- expand. The `ℕ`-cast `↑(Module.finrank ℝ E)` is treated as an atom.
    simp only [nsmul_eq_mul]
    exact le_of_eq (by ring)
  /- ### Quantitative support-aware promotion to a global chart-target bound

  The `m`-fold mixed weak partial `P` vanishes a.e. off the compact kernel `K`;
  the quantitative support-aware promotion raises the interior `W^{2,2}`-norm
  on `Ω''` to a global `W^{2,2}`-norm bound on the chart target. -/
  -- `P` is `MemLp 2` of the plain volume restricted to the chart target.
  have h_global_Lp : MemLp P 2
      ((volume : Measure EuclN).restrict
        (chartTargetEuclid (I := I) (M := M) α)) := by
    rw [hP_def]
    exact eigenvectorChartIteratedPartial_memLp_volume
      (I := I) (M := M) g r s h_atlas i α P₀ m directions
  -- `P` is a.e. zero off the compact kernel `K`.
  have h_ae_zero :
    ...
    exact eigenvectorChartIteratedPartial_ae_zero_off_chartPouKernel
      (I := I) (M := M) g r s h_atlas i α P₀ m directions
  -- The quantitative support-aware promotion.
  ...
  -- Chain the promotion with the interior bound.
...
set_option maxHeartbeats 1600000 in
/-- **Eigenbasis-uniform quantitative weakened Nirenberg interior `W^{2,2}` of
the iterated mixed weak partial.**

For a closed Riemannian manifold `(M, g)`, ranks `(r, s)`, the uniform-Sobolev
hypothesis `h_atlas`, a chart center `α : M`, a component multi-index `P₀`, and
a level `m : ℕ`, there is a single constant `C > 0` — depending only on `g`,
`α`, `P₀`, and the manifold dimension, never on the eigenbasis index — such
that, for *every* eigenbasis index `i`, every direction multi-index
`directions`, every iterated carrier `D_m` whose direction multi-index is
`directions`, and chart-`H^{m+1}` regularity `h_parent` of the eigenvector
chart component, the `m`-fold mixed weak partial:

* lies in `MemWkp 2 2 … (chartTargetEuclid α)`;
* has its `W^{2,2}`-norm bounded by `ENNReal.ofReal C` times the sum of its
  `W^{1,2}`-norm and the weighted `L²`-norm of the carrier's effective source.

This is the eigenbasis-uniform companion of
`eigenvectorChartIteratedPartial_wkpNorm_two_two_le`. The per-`i` statement
carries an existential constant *after* the index `i` and the carrier `D_m`,
hence a potentially `i`-dependent constant. Here the geometric constant is
hoisted in front of the index quantifier. The witness is `K_prom · C'`, where:

* `C'` is assembled from the order-2 interior engine's chart-geometric constant
  `C_geom` — produced by `tensor_h2_chart_loc_of_data_quantitative` *before* it
  quantifies over the bilinear datum, hence uniform over the carrier — together
  with the integrand-free density scalar `c_d` of
  `eLpNorm_volume_restrict_compact_le_weighted_uniform`;
* `K_prom` is the smooth-cutoff Leibniz promotion constant supplied by
  `wkpNorm_le_of_memWkp_precompact_of_ae_zero_off_compact_uniform`, which is
  uniform over the promoted function.

The proof builds the region data, the engine constant, the density scalar, and
the promotion constant — all eigenbasis-free — fixes the witness `K_prom · C'`,
and only then introduces the eigenbasis index together with its carrier and
regularity hypotheses; the remaining interior-norm assembly and support-aware
promotion are identical to the per-`i` proof. -/
theorem eigenvectorChartIteratedPartial_wkpNorm_two_two_le_uniform
          ...
          ≤ ENNReal.ofReal C *
              ...
              + eLpNorm
                  ...
                  2 ((chartPulledWeightedMeasure (I := I) g α).restrict
                      (chartTargetEuclid (I := I) (M := M) α))) := by
  classical
  /- ## Eigenbasis-free region setup

  The compact partition-of-unity kernel `K`, the precompact interior subdomain
  `Ω''`, the difference-quotient room radius `R₀`, the precompact engine target
  `Ω'`, and the smooth Nirenberg cutoff `η` are all built from `g`, `α` only —
  never from the eigenbasis index. -/
  set K : Set EuclN := chartPouKernel (I := I) (M := M) α with hK_def
  have hK_compact : IsCompact K := chartPouKernel_isCompact (I := I) (M := M) α
  have hK_meas : MeasurableSet K :=
    chartPouKernel_measurableSet (I := I) (M := M) α
  ...
  have h_room : Metric.cthickening R₀ (closure Ω'') ⊆
    ...
    have h3 : Metric.cthickening (R₀ + R_α / 2) K ⊆
      ...
      exact Metric.cthickening_mono hle K
    exact ((h1.trans h2).trans h3).trans hR_α_subset
  /- ## The eigenbasis-free smooth Nirenberg cutoff -/
  set K_Ω : Set EuclN := closure Ω'' with hK_Ω_def
  have hK_Ω_compact : IsCompact K_Ω := hΩ''_compact_closure
  set ε : ℝ := R₀ / 16 with hε_def
  have hε_pos : 0 < ε := by positivity
  ...
  have h_room_ε : Metric.cthickening ε (closure Ω'') ⊆
      chartTargetEuclid (I := I) (M := M) α :=
    (Metric.cthickening_mono (by rw [hε_def]; linarith) (closure Ω'')).trans h_room
  /- ## The eigenbasis-free order-2 interior engine constant -/
  obtain ⟨C_geom, hC_geom_nn, hC_geom⟩ :=
    tensor_h2_chart_loc_of_data_quantitative (I := I) (M := M) (g := g)
      (α := α) (r := r) (s := s) (P₀ := P₀)
      hη_smooth hη_supp hη_range hN_nn h_fderiv_eta hΩ'_open h_closureΩ'_in_chart
      hΩ'_compact_closure hη_in_Ω' hε_pos hh_supp_in_Ω' hη_one_on_Ω''
      hΩ''_open hΩ''_compact_closure h_room_ε
  -- Opacify the eigenbasis-free regions before downstream theorem applications.
  clear_value K Ω'' Ω'
  /- ## The eigenbasis-free density scalar and promotion constant -/
  -- The integrand-free density scalar for the `f_chart` energy term.
  obtain ⟨c_d, hc_d_nn, h_fchart_density_uniform⟩ :=
    eLpNorm_volume_restrict_compact_le_weighted_uniform (I := I) (M := M) g α
      hΩ'_compact_closure hΩ'_compact_closure.isClosed.measurableSet
      h_closureΩ'_in_chart
  -- The function-uniform smooth-cutoff Leibniz promotion constant.
  obtain ⟨K_prom, hK_prom_pos, hK_prom_bound_uniform⟩ :=
    DifferentialGeometry.Analysis.Sobolev.Euclidean.wkpNorm_le_of_memWkp_precompact_of_ae_zero_off_compact_uniform
      ...
      (by norm_num : (1 : ℝ≥0∞) ≤ 2) (by norm_num : (2 : ℝ≥0∞) ≠ ⊤)
      h_chart_open hΩ''_open hK_compact hK_in_Ω'' h_closureΩ''_in_chart
  /- ## The eigenbasis-free geometric constant `C := K_prom · C'`

  `C_geom_max` upper-bounds every `C_geom i' k'`; `C₂ := √(n + 1 + c_d²)`;
  `C₁ := C_geom_max · C₂`; `C' := 1 + n + n² · C₁`. -/
  set C_geom_max : ℝ :=
    ∑ i' : Fin (Module.finrank ℝ E), ∑ k' : Fin (Module.finrank ℝ E),
      C_geom i' k' with hC_geom_max_def
  have hC_geom_max_nn : 0 ≤ C_geom_max := by
  ...
  have hC'_pos : 0 < C' := by
    ...
    have h2 : (0 : ℝ) ≤ ((Module.finrank ℝ E : ℕ) : ℝ) ^ 2 * C₁ :=
      mul_nonneg (by positivity) hC₁_nn
    linarith
  -- Fix the eigenbasis-free witness `K_prom · C'`, then introduce the index.
  refine ⟨K_prom * C', mul_pos hK_prom_pos hC'_pos, ?_⟩
  ...
  obtain ⟨h_uChart_memW1p, h_wp_memW1p⟩ :=
    tensorChartBilinear_chartComponent_regularity_of_data
      (g := g) (r := r) (s := s) (α := α) (P₀ := P₀) D
      hΩ''_open hΩ''_compact_closure hR₀_pos h_room
  -- The interior `W^{2,2}` of `D.u_chart` on `Ω''`.
  have h_uChart_memWkp_two_Ω'' :
      DifferentialGeometry.Analysis.Sobolev.Euclidean.MemWkp
        (d := Module.finrank ℝ E) 2 2 D.u_chart Ω'' := by
    refine ⟨h_uChart_memW1p, fun j => ?_⟩
  ...
  have h_ofReal_AB_le_RHS :
    ...
    rw [hRHS_inner_def]
    exact add_le_add h_ofReal_A_le h_ofReal_B_le
  -- Specialize the integrand-uniform density bound to the carrier's source.
  ...
  have h_gik_bound : ∀ i' k' : Fin (Module.finrank ℝ E),
    ...
    calc ENNReal.ofReal (C_geom i' k' * Real.sqrt _)
      ...
      _ ≤ ENNReal.ofReal C₁ * RHS_inner :=
          mul_le_mul_right h_ofReal_AB_le_RHS _
  /- ### Per-`i`: the order-`1` `wkpNorm` of the chosen weak `i`-partial. -/
  have h_chosen_ae_wp : ∀ i' : Fin (Module.finrank ℝ E),
      DifferentialGeometry.Analysis.Sobolev.Euclidean.chosenWeakPartial'
          2 i' D.u_chart Ω''
        =ᵐ[(volume : Measure EuclN).restrict Ω''] D.weak_partial i' := by
  ...
  have h_ae_zero :
    ...
    exact eigenvectorChartIteratedPartial_ae_zero_off_chartPouKernel
      (I := I) (M := M) g r s h_atlas i α P₀ m directions
  -- Specialize the function-uniform promotion to the principal factor `P`.
  have hK_prom_bound :
      DifferentialGeometry.Analysis.Sobolev.Euclidean.wkpNorm
          (d := Module.finrank ℝ E) 2 2 P
          (chartTargetEuclid (I := I) (M := M) α)
...
/-- The quantitative headline produces global `W^{2,2}` of the `m`-fold mixed
weak partial together with an explicit `W^{2,2}`-norm bound. -/
