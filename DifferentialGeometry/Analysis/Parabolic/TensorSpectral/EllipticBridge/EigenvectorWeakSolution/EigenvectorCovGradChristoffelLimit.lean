      ...
      (covDerivLowerOrderTerm (I := I) (M := M) g r s
        (pouSmul (I := I) (M := M) g r s α
          (eigenvectorSmoothApprox (I := I) (M := M)
            g r s h_atlas i n).toCcTensor) α k P₀.1 P₀.2) 2
      (chartL2Measure (I := I) (M := M) α) := by
  classical
  refine (memLp_finset_sum (μ := chartL2Measure (I := I) (M := M) α)
    (Finset.univ : Finset (TensorCompIdx (E := E) r s))
    (fun p _ =>
      memLp_factor_mul_componentAtom (I := I) (M := M) g r s h_atlas i α p n
...
private theorem covDerivLowerOrderTerm_pouSmul_tendsto
    ...
    Filter.Tendsto
      ...
      (𝓝 ((covGradChristoffelUnscaledLimit_memLp (I := I) (M := M)
        g r s h_atlas i α P₀ k).toLp _)) := by
  classical
  ...
  have h_tendsto : ∀ p : TensorCompIdx (E := E) r s,
    ...
    tendsto_componentSummand (I := I) (M := M) g r s h_atlas i α p
      (covDerivLowerOrderCoeff_contDiffOn (I := I) (M := M)
        g r s α k P₀.1 p.1 P₀.2 p.2)
  ...
  have hFn_eq : ∀ n : ℕ,
      covDerivLowerOrderTerm (I := I) (M := M) g r s
          (pouSmul (I := I) (M := M) g r s α
            (eigenvectorSmoothApprox (I := I) (M := M)
              g r s h_atlas i n).toCcTensor) α k P₀.1 P₀.2
...
private lemma smul_componentLpLimit_coeFn_ae
  ...
  refine (Lp.coeFn_smul (i.fst.val)⁻¹
    (componentLpLimit (I := I) (M := M) g r s h_atlas i α p)).symm.trans ?_
...
private lemma smul_unscaledLimit_toLp_eq
  ...
  refine (Lp.coeFn_smul (i.fst.val)⁻¹
    ((covGradChristoffelUnscaledLimit_memLp (I := I) (M := M)
      g r s h_atlas i α P₀ k).toLp _)).trans ?_
  refine Filter.EventuallyEq.trans ?_
    (MemLp.coeFn_toLp (covGradChristoffelLimit_memLp (I := I) (M := M)
      g r s h_atlas i α P₀ k)).symm
  ...
  have h_all : ∀ᵐ y ∂(chartL2Measure (I := I) (M := M) α),
    ...
    (ae_all_iff).mpr (fun p =>
      smul_componentLpLimit_coeFn_ae (I := I) (M := M) g r s h_atlas i α p)
  filter_upwards [MemLp.coeFn_toLp (covGradChristoffelUnscaledLimit_memLp
    (I := I) (M := M) g r s h_atlas i α P₀ k), h_all] with y hy hy_all
  rw [Pi.smul_apply, hy]
...
h_atlas i n).toCcTensor` converge, as `n → ∞` and in `Lp ℝ 2 (chartL2Measure
α)`, to the `L²` class of the explicit limit function
`covGradChristoffelLimit g r s h_atlas i α P₀ k`.
...
example (g : SmoothRiemannianMetric I M) (r s : ℕ)
    ...
    (k : Fin (Module.finrank ℝ E)) :
    EuclN → ℝ :=
  covGradChristoffelLimit (I := I) (M := M) g r s h_atlas i α P₀ k
...
example (g : SmoothRiemannianMetric I M) (r s : ℕ)
  ...
  covGradChristoffelLimit_memLp (I := I) (M := M) g r s h_atlas i α P₀ k
