...
`eigenvectorTensorChartBilinearData_unconditional g r s ι α P₀`, which is built
from the chart-locality-free eigenbasis vector
`tensorResolventEigenbasisVec_ofCompact (tensorResolventL2_isCompactOperator_intrinsic g r s) ι`.
...
by the chart-locality-free data `eigenvectorTensorChartBilinearData_unconditional
...
constant `C_geom i k ≥ 0` such that for every pair `(i, k)` the `i`-th weak
partial of the eigenvector chart-bilinear data
`(eigenvectorTensorChartBilinearData_unconditional g r s ι α P₀).weak_partial i`
admits a weak `k`-partial derivative `g_{i,k} ∈ L²(Ω'')` with
...
theorem eigenvector_chartComponent_h2_quantitative_unconditional
    [I.Boundaryless] [T2Space M] [SigmaCompactSpace M] [CompactSpace M]
    (g : SmoothRiemannianMetric I M) (r s : ℕ)
    (ι : TensorEigenIdx (I := I) (M := M) g r s)
    (α : M) (P₀ : TensorCompIdx (E := E) r s)
      ...
      ∀ (i k : Fin (Module.finrank ℝ E)),
        ∃ g_ik : EuclN → ℝ,
          ...
          eLpNorm g_ik 2 ((volume : Measure EuclN).restrict Ω'') ≤
            ENNReal.ofReal (C_geom i k * Real.sqrt (
              (∑ l : Fin (Module.finrank ℝ E),
                (eLpNorm ((eigenvectorTensorChartBilinearData_unconditional (I := I) (M := M)
