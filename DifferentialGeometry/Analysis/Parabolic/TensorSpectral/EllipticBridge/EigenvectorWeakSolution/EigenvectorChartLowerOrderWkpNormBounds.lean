...
omit [CompleteSpace E] in
/-- The chart-partial atom `partialLpLimit g r s h_atlas i α P k` — `μ` times the
weak `k`-th chart partial of the canonical eigenvector chart component — is
...
(`eigenvectorVec_pou_memWkp` at order `K + 1`), and the eigenvector weak chart
partial is a genuine weak partial of it
(`eigenvectorChartWeakPartial_hasWeakPartialDeriv`); a weak partial of a
