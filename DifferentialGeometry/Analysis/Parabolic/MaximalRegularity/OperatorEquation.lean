...
* `tensorScaleLaplacian h_uniform τ` — the diagonal operator `Δ_∇ : H^{τ+2}
  →L[ℝ] Hᵗ`, multiplying the `i`-th coordinate by `-λᵢ`.
* `timeScaleLaplacian h_uniform τ` — its action on time-`L²` tensor fields,
...
private theorem weightLambdaMulSqLe
    (v : tensorHs (I := I) (M := M) g r s h_uniform (τ + 2))
    (i : TensorEigenIdx (I := I) (M := M) g r s) :
    tensorSobolevWeight (I := I) (M := M) i τ *
...
`Hᵗ`. -/
def scaleLaplacianFun (v : tensorHs (I := I) (M := M) g r s h_uniform (τ + 2)) :
...
theorem scaleLaplacianFun_add
    (v w : tensorHs (I := I) (M := M) g r s h_uniform (τ + 2)) :
...
def tensorScaleLaplacian (h_uniform : uniformTensorChartSobolevBound g r s)
    (τ : ℝ) :
    tensorHs (I := I) (M := M) g r s h_uniform (τ + 2) →L[ℝ]
...
@[simp] theorem tensorScaleLaplacian_apply (τ : ℝ)
    (v : tensorHs (I := I) (M := M) g r s h_uniform (τ + 2)) :
    tensorScaleLaplacian (I := I) (M := M) h_uniform τ v =
...
@[simp] theorem tensorScaleLaplacian_coeff (τ : ℝ)
    (v : tensorHs (I := I) (M := M) g r s h_uniform (τ + 2))
    (i : TensorEigenIdx (I := I) (M := M) g r s) :
    (tensorScaleLaplacian (I := I) (M := M) h_uniform τ v).coeff i =
      -(TensorEigenIdx.lambda (I := I) (M := M) i) * v.coeff i := rfl
...
pointwise in time. -/
def timeScaleLaplacian (h_uniform : uniformTensorChartSobolevBound g r s)
    (τ : ℝ) {T : ℝ} :
    timeL2 (tensorHs (I := I) (M := M) g r s h_uniform (τ + 2)) T →L[ℝ]
      timeL2 (tensorHs (I := I) (M := M) g r s h_uniform τ) T :=
  (tensorScaleLaplacian (I := I) (M := M) h_uniform τ).compLpL 2 (timeMeasure T)

/-- `timeScaleLaplacian τ v` is represented a.e. by the pointwise rough
Laplacian `t ↦ Δ_∇ (v t)`. -/
theorem timeScaleLaplacian_coeFn
    (v : timeL2 (tensorHs (I := I) (M := M) g r s h_uniform (τ + 2)) T) :
    timeScaleLaplacian (I := I) (M := M) h_uniform τ v =ᵐ[timeMeasure T]
      fun t => tensorScaleLaplacian (I := I) (M := M) h_uniform τ (v t) :=
  (tensorScaleLaplacian (I := I) (M := M) h_uniform τ).coeFn_compLpL
...
theorem timeModeCoeff_timeScaleLaplacian
    ...
    timeModeCoeff (I := I) (M := M)
        (timeScaleLaplacian (I := I) (M := M) h_uniform τ v) i =
      (-(TensorEigenIdx.lambda (I := I) (M := M) i)) •
        timeModeCoeff (I := I) (M := M) v i := by
  refine Lp.ext ?_
  have hlhs := timeModeCoeff_coeFn (I := I) (M := M)
    (timeScaleLaplacian (I := I) (M := M) h_uniform τ v) i
  have hΔ := timeScaleLaplacian_coeFn (I := I) (M := M) (h_uniform := h_uniform)
    (τ := τ) v
...
theorem maximalRegularityOp_solves_perMode {a : ℝ} (hT : 0 ≤ T)
  ...
  rw [maximalRegularityDerivField_timeModeCoeff (I := I) (M := M) (a := a) hT f i,
...
theorem maximalRegularityOp_solves {a : ℝ} (hT : 0 < T) (hT1 : T ≤ 1)
  ...
  rw [timeModeCoeff_add (I := I) (M := M),
    timeModeCoeff_timeScaleLaplacian (I := I) (M := M) (τ := a)
      (maximalRegularitySolField (I := I) (M := M) h_uniform a hT.le f) i]
  -- Reduce to the per-mode evolution identity.
  exact maximalRegularityOp_solves_perMode (I := I) (M := M) (a := a) hT.le f i
