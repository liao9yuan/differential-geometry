...
tensor Sobolev bound `h_uniform`, given an initial datum
...
`∂_t T = Δ_∇ T + F`, `T(0) = T_0`, is
  ...
  `T(t) := tensorHeatSemigroup g r s h_uniform t T_0
           + ∫_0^t tensorHeatSemigroup g r s h_uniform (t - τ) (F τ) dτ`,
...
* `tensorMildSolution g r s h_uniform T_0 F t` — the Duhamel formula.
...
* `tensorIntegrable_heatSemigroup_apply_F` — the Duhamel integrand
  `τ ↦ tensorHeatSemigroup g r s h_uniform (t - τ) (F τ)` is
...
noncomputable def tensorMildSolution
    ...
    (t : ℝ) :
    TensorL2 r s g :=
  tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t T_0 +
    ∫ τ in (0 : ℝ)..t,
      tensorHeatSemigroup (I := I) (M := M) g r s h_uniform (t - τ) (F τ)
...
theorem tensorMildSolution_zero
    ...
    tensorMildSolution (I := I) (M := M) g r s h_uniform T_0 F 0 = T_0 := by
  unfold tensorMildSolution
  rw [intervalIntegral.integral_same, add_zero]
  rw [tensorHeatSemigroup_zero (I := I) (M := M) h_uniform]
  rfl

/-! ## Integrability of the integrand on `[0, t]` -/

/-- The Duhamel integrand `τ ↦ tensorHeatSemigroup g r s h_uniform (t - τ) (F τ)`
...
`tensorMildSolution g r s h_uniform T_0 F t =
  tensorMildSolution g r s h_uniform T_0 0 t +
  tensorMildSolution g r s h_uniform 0 F t`.
...
homogeneous part `tensorMildSolution g r s h_uniform V_0 0`. -/
theorem tensorMildSolution_add_initial
    {g : SmoothRiemannianMetric I M} {r s : ℕ}
    (h_uniform : uniformTensorChartSobolevBound g r s)
    (T_0 V_0 : TensorL2 r s g)
    (F : ℝ → TensorL2 r s g) (t : ℝ) :
    tensorMildSolution (I := I) (M := M) g r s h_uniform (T_0 + V_0) F t =
      tensorMildSolution (I := I) (M := M) g r s h_uniform T_0 F t +
        tensorMildSolution (I := I) (M := M) g r s h_uniform V_0
          (fun _ => 0) t := by
  unfold tensorMildSolution
  -- The Duhamel integral with zero forcing vanishes.
  have h_zero_int :
      ∫ τ in (0 : ℝ)..t,
          tensorHeatSemigroup (I := I) (M := M) g r s h_uniform (t - τ)
            (0 : TensorL2 r s g) =
        (0 : TensorL2 r s g) := by
    have h_eq : (fun τ : ℝ =>
        tensorHeatSemigroup (I := I) (M := M) g r s h_uniform (t - τ)
      ...
      rw [(tensorHeatSemigroup
        (I := I) (M := M) g r s h_uniform (t - τ)).map_zero]
  ...
  have h_add : tensorHeatSemigroup
      (I := I) (M := M) g r s h_uniform t (T_0 + V_0) =
      tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t T_0 +
        tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t V_0 := by
    rw [(tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t).map_add]
  rw [h_add]
  abel
...
theorem tensorMildSolution_smul_initial
    ...
    (F : ℝ → TensorL2 r s g) (t : ℝ) :
    tensorMildSolution (I := I) (M := M) g r s h_uniform (c • T_0) F t =
      c • tensorMildSolution (I := I) (M := M) g r s h_uniform T_0
            (fun _ => 0) t +
        tensorMildSolution (I := I) (M := M) g r s h_uniform 0 F t := by
  unfold tensorMildSolution
  -- The Duhamel integral with zero forcing vanishes.
  have h_zero_int :
      ∫ τ in (0 : ℝ)..t,
  ...
  have h_heat_zero :
    ...
    rw [(tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t).map_zero]
...
theorem tensorMildSolution_add_forcing
    ...
    {t : ℝ} (ht : 0 ≤ t) :
    tensorMildSolution (I := I) (M := M) g r s h_uniform T_0
        (fun τ => F τ + G τ) t =
      tensorMildSolution (I := I) (M := M) g r s h_uniform T_0 F t +
        tensorMildSolution (I := I) (M := M) g r s h_uniform 0 G t := by
  unfold tensorMildSolution
  ...
  have h_split : ∀ τ,
    ...
    intro τ
    rw [(tensorHeatSemigroup
      (I := I) (M := M) g r s h_uniform (t - τ)).map_add]
  have h_eq_fun :
      (fun τ : ℝ =>
          tensorHeatSemigroup (I := I) (M := M) g r s h_uniform (t - τ)
            (F τ + G τ)) =
  ...
  rw [intervalIntegral.integral_add
    (tensorIntegrable_heatSemigroup_apply_F
      (I := I) (M := M) h_uniform hF ht)
    (tensorIntegrable_heatSemigroup_apply_F
      (I := I) (M := M) h_uniform hG ht)]
  abel

/-! ## Continuity in `t` on `[0, ∞)`

The continuity proof uses a clipped representation of the integrand.
The integrand `(t, τ) ↦ tensorHeatSemigroup g r s h_uniform (t - τ) (F τ)`
...
clipped variant
  ...
  `tensorClippedKernel (t, τ) :=
      tensorHeatSemigroup g r s h_uniform (max 0 (t - τ)) (F τ)`
...
`tensorHeatSemigroup g r s h_uniform (max 0 (t - τ)) (F τ)`. Globally
...
private noncomputable def tensorClippedKernel
  ...
  tensorHeatSemigroup (I := I) (M := M) g r s h_uniform (max 0 (t - τ)) (F τ)
...
private lemma continuous_tensorClippedKernel
    ...
    Continuous
      (Function.uncurry
        (tensorClippedKernel (I := I) (M := M) g r s h_uniform F)) := by
  ...
  set K : ℝ × ℝ → TensorL2 r s g :=
    Function.uncurry (tensorClippedKernel (I := I) (M := M) g r s h_uniform F)
    with hK_def
  -- Bound for `(t, τ)`:
  --   `‖K (t,τ) − K (t₀,τ₀)‖
  --      ≤ ‖F τ − F τ₀‖
  ...
  have h_bound : ∀ p : ℝ × ℝ,
      ‖K p - K (t₀, τ₀)‖ ≤
        ‖F p.2 - F τ₀‖ +
        ‖tensorHeatSemigroup (I := I) (M := M) g r s h_uniform
              (max 0 (p.1 - p.2)) (F τ₀) -
            tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform σ₀ (F τ₀)‖ := by
    rintro ⟨t, τ⟩
    have hσ_nn : 0 ≤ max 0 (t - τ) := le_max_left _ _
    have h_decomp : K (t, τ) - K (t₀, τ₀) =
        tensorHeatSemigroup
            (I := I) (M := M) g r s h_uniform (max 0 (t - τ))
            (F τ - F τ₀) +
          (tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform (max 0 (t - τ)) (F τ₀) -
            tensorHeatSemigroup
              (I := I) (M := M) g r s h_uniform σ₀ (F τ₀)) := by
      change tensorHeatSemigroup
            (I := I) (M := M) g r s h_uniform (max 0 (t - τ)) (F τ) -
          tensorHeatSemigroup
            (I := I) (M := M) g r s h_uniform σ₀ (F τ₀) = _
      rw [(tensorHeatSemigroup
        (I := I) (M := M) g r s h_uniform (max 0 (t - τ))).map_sub]
      abel
    rw [h_decomp]
    have h_norm_sum :=
      norm_add_le
        (tensorHeatSemigroup
          (I := I) (M := M) g r s h_uniform (max 0 (t - τ)) (F τ - F τ₀))
        (tensorHeatSemigroup
            (I := I) (M := M) g r s h_uniform (max 0 (t - τ)) (F τ₀) -
          tensorHeatSemigroup
            (I := I) (M := M) g r s h_uniform σ₀ (F τ₀))
    have h_first_bound :
        ‖tensorHeatSemigroup
            (I := I) (M := M) g r s h_uniform (max 0 (t - τ))
            (F τ - F τ₀)‖ ≤ ‖F τ - F τ₀‖ := by
      have h_op :=
        tensorHeatSemigroup_opNorm_le_one
          (I := I) (M := M) (h_uniform := h_uniform) (max 0 (t - τ))
...
theorem tensorMildSolution_continuous
    ...
    ContinuousOn
      (fun t : ℝ =>
        tensorMildSolution (I := I) (M := M) g r s h_uniform T_0 F t)
      (Set.Ici (0 : ℝ)) := by
  -- The first term `tensorHeatSemigroup g r s h_uniform t T_0` is
  -- `ContinuousOn [0, ∞)`.
  have h_first_cont :=
    tensorHeatSemigroup_continuous_on_nonneg
      (I := I) (M := M) h_uniform T_0
  ...
  have h_clip_eq : ∀ t : ℝ, 0 ≤ t →
      tensorMildSolution (I := I) (M := M) g r s h_uniform T_0 F t =
        tensorHeatSemigroup (I := I) (M := M) g r s h_uniform t T_0 +
          ∫ τ in (0 : ℝ)..t,
            tensorClippedKernel
              (I := I) (M := M) g r s h_uniform F t τ := by
    intro t ht
    unfold tensorMildSolution
    congr 1
    ...
    change tensorHeatSemigroup
        (I := I) (M := M) g r s h_uniform (t - τ) (F τ) =
      tensorClippedKernel (I := I) (M := M) g r s h_uniform F t τ
...
theorem tensor_linear_parabolic_existence
    ...
    ∃ S : ℝ → TensorL2 r s g,
      ...
      ∀ t : ℝ, S t = tensorHeatSemigroup g r s h_uniform t T_0 +
        ∫ τ in (0 : ℝ)..t,
          tensorHeatSemigroup g r s h_uniform (t - τ) (F τ) := by
  refine ⟨tensorMildSolution (I := I) (M := M) g r s h_uniform T_0 F,
    ?_, ?_, ?_⟩
  · -- Continuity on `[0, ∞)`.
    exact tensorMildSolution_continuous
      (I := I) (M := M) h_uniform T_0 hF
  · -- Initial condition `S 0 = T_0`.
    exact tensorMildSolution_zero
      (I := I) (M := M) h_uniform T_0 F
