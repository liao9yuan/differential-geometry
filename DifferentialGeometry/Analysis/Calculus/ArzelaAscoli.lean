...
/-- An equicontinuous sequence of continuous maps whose values lie in one
compact set has a uniformly convergent subsequence. -/
theorem arzela_subseq_cpt
  ...
  have hunif : TendstoUniformly (fun n => fb (phi n)) g atTop :=
    BoundedContinuousFunction.tendsto_iff_tendstoUniformly.mp
      (by simpa only [Function.comp_apply] using htendsto)
