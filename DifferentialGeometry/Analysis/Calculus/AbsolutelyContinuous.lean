...
/-- Absolutely continuous curves on adjacent intervals can be pasted when they
agree at the common endpoint. -/
...
/-- Absolute continuity is preserved by changing a function on neither point
of its parameter interval. -/
theorem congr_of_eqOn {f g : ℝ → X} {a b : ℝ}
    (hf : AbsolutelyContinuousOnInterval f a b)
...
/-- Postcomposition by a Lipschitz map preserves absolute continuity when the
curve stays in the set on which that map is Lipschitz. -/
theorem comp_lipschitzOn
  ...
  calc
    ∑ i ∈ Finset.range p.1,
      ...
      simpa only [Function.comp_apply, ENNReal.toReal_mul,
...
/-- Precomposition by a monotone Lipschitz time change preserves absolute
continuity when the time change stays in the original parameter interval. -/
theorem comp_mono_lip
  ...
  calc
    ∑ i ∈ Finset.range n, dist (q i).1 (q i).2 ≤
      ...
      apply ENNReal.toReal_mono
        (ENNReal.mul_ne_top ENNReal.coe_ne_top (edist_ne_top _ _)) at hle
      simpa only [q, ENNReal.toReal_mul, ← dist_edist] using hle
...
/-- Precomposition by the square map preserves absolute continuity on a
nonnegative ordered interval. -/
theorem comp_sq
  ...
  apply comp_mono_lip hf (by simpa only [uIcc_of_le hab] using hK)
  · intro x hx y hy hxy
...
/-- The square root is absolutely continuous between two nonnegative square
times. -/
theorem sqrt_ac_sq {a b : ℝ} (ha : 0 ≤ a) (hab : a ≤ b) :
    AbsolutelyContinuousOnInterval Real.sqrt (a ^ 2) (b ^ 2) := by
  have habSq : a ^ 2 ≤ b ^ 2 :=
    (sq_le_sq₀ ha (ha.trans hab)).2 hab
  let q : ℝ := -(1 / 2 : ℝ)
  let v : ℝ → ℝ := fun t ↦ (1 / 2 : ℝ) * t ^ q
  have hvInt : IntervalIntegrable v volume (a ^ 2) (b ^ 2) := by
    exact (intervalIntegral.intervalIntegrable_rpow'
      (a := a ^ 2) (b := b ^ 2) (by norm_num [q])).const_mul (1 / 2 : ℝ)
  have hprim : AbsolutelyContinuousOnInterval
      (fun x ↦ ∫ t in a ^ 2..x, v t) (a ^ 2) (b ^ 2) :=
    hvInt.absolutelyContinuousOnInterval_intervalIntegral
      (by simpa only [uIcc_of_le habSq] using left_mem_Icc.2 habSq)
  have hconst : AbsolutelyContinuousOnInterval
      (fun _ : ℝ ↦ Real.sqrt (a ^ 2)) (a ^ 2) (b ^ 2) :=
    (LipschitzWith.const (Real.sqrt (a ^ 2))).lipschitzOnWith.absolutelyContinuousOnInterval
  apply (hconst.add hprim).congr_of_eqOn
  intro x hx
  have hx0 : 0 ≤ x := by
    rw [uIcc_of_le habSq] at hx
    exact (sq_nonneg a).trans hx.1
  simp only [Pi.add_apply, v, q]
  rw [intervalIntegral.integral_const_mul,
    integral_rpow (Or.inl (by norm_num : (-1 : ℝ) < -(1 / 2 : ℝ)))]
  rw [show (-(1 / 2 : ℝ) + 1) = 1 / 2 by norm_num]
  simp only [← Real.sqrt_eq_rpow, Real.sqrt_sq ha]
  ring
...
/-- An upper right Dini bound integrates along an absolutely continuous real
function.  The filter hypothesis is the finite-valued form of
`D⁺ f t ≤ g t`. -/
theorem sub_le_integral_dini (hab : a ≤ b)
    (hf : AbsolutelyContinuousOnInterval f a b)
...
/-- The local-at-positive-times form of upper-Dini integration.  Continuity at
the left endpoint and integrability of the upper bound close the limit there. -/
theorem sub_le_int_loc_dini (hab : a ≤ b)
  ...
  have hclosure : closure (Ioc a b) ⊆ s := by
    ...
    refine ⟨sub_le_integral_dini (f := f) (g := g) hcb
