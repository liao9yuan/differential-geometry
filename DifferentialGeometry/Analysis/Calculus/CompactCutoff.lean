...
/-- A compact set in a finite-dimensional Hausdorff manifold has a smooth
compactly supported cutoff which is one on a neighborhood of the set. -/
theorem exists_bump_nhds
  ...
  · apply contMDiff_const.sub
    exact contMDiff_finset_prod fun x _ =>
