...
private local instance tensorRSChartFiberFromModelRiemannianNormedAddCommGroup
...
private lemma tensorRSChartFiberFromModel_local_bound
  ...
  have h_norm_cc_v :
      ‖(eα.coordChangeL ℝ ey₀ b
        : TensorRSModel r s ℝ E →L[ℝ] TensorRSModel r s ℝ E) v‖ ≤ C₂ * ‖v‖ :=
    (eα.coordChangeL ℝ ey₀ b :
      TensorRSModel r s ℝ E →L[ℝ] TensorRSModel r s ℝ E).le_opNorm v |>.trans
  ...
  calc
    ...
    _ ≤ C₁ * ‖(eα.coordChangeL ℝ ey₀ b :
          TensorRSModel r s ℝ E →L[ℝ] TensorRSModel r s ℝ E) v‖ :=
      mul_le_mul_of_nonneg_right h_symmL_norm_le (norm_nonneg _)
    _ ≤ C₁ * (C₂ * ‖v‖) :=
      mul_le_mul_of_nonneg_left h_norm_cc_v hC₁_nn
    _ = C₁ * C₂ * ‖v‖ := by ring
