...
/-! ## Smoothness of the perturbed source for the chart component

`perturbedSource B u f l` involves at most `∂_l f`, the coefficients of `B`, and
at most second derivatives of `u`. With `u` the chart component
`tensorComponentEuclid g r s T α P₀` — which is `C^∞`
(`tensorComponentEuclid_contDiff`) — and `f` the explicit right-hand side
`tensorComponentWeakRHS g r s T F α hK hK_target P₀` — which is `C^∞`
(`tensorComponentWeakRHS_contDiff`) — and the coefficients of
`tensorPrincipalForm` smooth by construction, the perturbed source is `C^∞`.

The smoothness of `tensorComponentWeakRHS` is the only place the chart-source
support hypotheses on `T` and `F` enter. -/
...
theorem tensorComponent_perturbedSource_contDiff
    ...
    ContDiff ℝ ∞
      (perturbedSource (d := Module.finrank ℝ E)
        ...
        l) := by
  classical
  -- The chart component is `C^∞`.
  have hu_cd : ContDiff ℝ ∞
      (tensorComponentEuclid (I := I) (M := M) g r s T α P₀) :=
    tensorComponentEuclid_contDiff (I := I) (M := M) g r s T α P₀ hT_supp
  -- The explicit right-hand side is `C^∞`.
  have hf_cd : ContDiff ℝ ∞
      (tensorComponentWeakRHS (I := I) (M := M) g r s T F α hK hK_target P₀) :=
    tensorComponentWeakRHS_contDiff (I := I) (M := M)
      g r s T F α hK hK_target P₀ hT_supp hF_supp
  -- `perturbedSource` is smooth whenever `u` and `f` are smooth: this is the
  -- content of the engine-side lemma `partial_smooth_weak_solution`, whose
  -- output source is smooth. We re-derive the smoothness term by term, mirroring
  -- the Euclidean construction, so that no private lemma is needed.
  ...
  refine ContDiff.sum ?_
  intro j _
  -- `(∂_l a^{ij})(∂_i u)` is smooth, hence its `∂_j` is smooth.
...
theorem tensorComponent_partial_isSmoothWeakSolution
  ...
  have hf_cd : ContDiff ℝ (⊤ : ℕ∞)
    ...
    tensorComponentWeakRHS_contDiff (I := I) (M := M)
      g r s T F α hK hK_target P₀ hT_supp hF_supp
  -- Apply the differentiated weak-solution identity. The domain of the
  -- principal-part form is `Set.univ`, which is open.
...
/-! ## Generic `W^{2,2}` adapter for the interior-`H²` engine

The interior `H²` regularity engine `h2_loc_smooth_solution` delivers, for a
smooth weak solution `u` with `C^∞` source `f` and any precompact open
subdomain `Ω''`, a weak second partial derivative of every classical first
partial `∂_i u`, in `L²(Ω'')`, with a quantitative `∫ g²`-bound. The adapter
below repackages this output for a **smooth, compactly supported** weak
solution into iterated-Sobolev (`MemWkp`) form.

Two facts make the repackaging clean:

* a smooth compactly supported function lies in `W^{k,p}` of *every* open set —
  no support-containment hypothesis is needed, because compact support gives
  global `L^p` membership and the classical partials are weak partials
  (`MemWkp_of_smooth_compactSupport_anyOpen` below);
* the engine's weak second partial of `∂_i u` agrees almost everywhere on
  `Ω''` with the classical mixed partial `∂_k ∂_i u`, by uniqueness of weak
  partials, so the engine's `∫ g²`-bound is a bound on `∫ (∂_k ∂_i u)²`.

The `W^{2,2}` norm `wkpNorm 2 2 u Ω''` collects the `L²`-norms (`eLpNorm`) of
all iterated weak partials of order `≤ 2`. Each of those iterated weak
partials agrees a.e. on `Ω''` with the corresponding classical iterated
partial; the order-`0` and order-`1` terms are bounded directly by the
`H¹`-and-`L²` data, the order-`2` terms by the engine's quantitative
constant. Globalising the engine's slightly-enlarged neighbourhood `Ω'` to
`Set.univ` (legitimate because every integrand is a nonnegative integrable
function) yields a single constant. -/
...
The vendored `MemWkp_of_smooth_compactSupport` carries the hypothesis
`tsupport ψ ⊆ Ω`. For the bootstrap adapter the solution `u` is smooth and
compactly supported but its support need not sit inside the precompact
subdomain `Ω''`. The two private lemmas below remove the support hypothesis:
compact support already gives global `L^p` membership and `of_contDiff` gives
the classical partials as weak partials on any open set. -/
...
private theorem memWkp_of_smooth_compactSupport_anyOpen
  ...
  | succ k ih =>
      ...
      refine ⟨hψ_W1p, ?_⟩
      intro i
      -- `chosenWeakPartial' p i ψ Ω` agrees a.e. on `Ω` with the classical
      -- partial `∂_i ψ`, which is smooth and compactly supported.
...
/-! ### Iterated weak partials of a smooth compactly supported function

The iterated weak partial of order `j` along a multi-index agrees almost
everywhere on every open set with the iterated classical partial, for smooth
compactly supported input. -/
...
private theorem iterWeakPartial_smooth_ae_eq_iterClassical_anyOpen
  ...
  | succ j ih =>
      ...
      have hψ_W1p : DeGiorgi.MemW1p p ψ Ω :=
        memW1p_of_smooth_compactSupport_anyOpen (d := d) hΩ_open hψ_smooth hψ_cpt
      -- `chosenWeakPartial' p (β 0) ψ Ω =ᵐ ∂_{β 0} ψ` on `Ω`.
      have h_ae := chosenWeakPartial_smooth_ae_eq (d := d) hp hΩ_open hψ_smooth
        hψ_W1p (β 0)
      have h_classical_smooth : ContDiff ℝ (⊤ : ℕ∞)
      ...
      -- Recurse on the lower-order tail with the smooth classical first partial.
      have h_ih := ih (fun i : Fin j => β i.succ) h_classical_smooth h_classical_cpt
      -- Transfer the a.e. equality of the inner argument through `iterWeakPartial`.
      have h_iter_congr := iterWeakPartial_ae_congr (d := d) hp hΩ_open j
        (fun i : Fin j => β i.succ) h_ae
      exact h_iter_congr.trans h_ih

/-! ### Real-analytic helpers for the `eLpNorm` repackaging -/
...
private lemma eLpNorm_two_sq_eq_ofReal_integral_sq
  ...
  have h_sq_lintegral :
    ...
    norm_num
  rw [h_sq_lintegral]
  -- `∫⁻ ‖h‖ₑ ^ 2 = ∫⁻ ENNReal.ofReal (h ^ 2) = ENNReal.ofReal (∫ h ^ 2)`.
...
theorem smooth_cc_h2_loc_memWkp_two
  ...
  have h_engine_choice : ∀ i k : Fin d, ∃ C : ℝ, 0 ≤ C ∧
    ...
    -- Transfer the engine's `∫ g²`-bound to `∫ (∂_k ∂_i u)²`.
    ...
    -- The engine bound, with the slightly-enlarged neighbourhood globalised.
    have h_data_le :
        (∫ x in Ω',
      ...
      -- Globalise each term: `∫_{Ω'} h ≤ ∫_{univ} h` for `h ≥ 0` integrable.
  ...
  have h_iter_classical_bound : ∀ (j : ℕ), j ≤ 2 → ∀ β : Fin j → Fin d,
    ...
    intro j hj β
    -- `∫_{Ω''} h² ≤ ∫_{univ} h²` requires the global integrability of `h²`.
    ...
    interval_cases j
    · -- Order `0`: `iterClassicalPartial 0 β u = u`.
    ...
    · -- Order `1`: `iterClassicalPartial 1 β u = ∂_{β 0} u`.
      ...
      calc ∫ x in Ω'', ((fderiv ℝ u x) (EuclideanSpace.single (β 0) 1)) ^ 2
        ...
        _ = 1 * D := (one_mul D).symm
        _ ≤ C₀ * D := mul_le_mul_of_nonneg_right h_one_le_C₀ hD_nn
    · -- Order `2`: `iterClassicalPartial 2 β u = ∂_{β 1} ∂_{β 0} u`.
  ...
  have hΩ''_open := hΩ''
  -- Each iterated weak partial agrees a.e. with the iterated classical partial.
  ...
  -- For each term in the `wkpNorm` sum, `eLpNorm ≤ ENNReal.ofReal (√(C₀ · D))`.
  have h_term_bound : ∀ (j : ℕ), j ∈ Finset.range 3 → ∀ β : Fin j → Fin d,
    ...
    -- The iterated weak partial agrees a.e. with the classical one.
    have h_ae := h_iterWeak_ae j β
    rw [eLpNorm_congr_ae h_ae]
    -- The classical iterated partial is smooth and compactly supported.
    have h_iter_smooth : ContDiff ℝ (⊤ : ℕ∞)
        (iterClassicalPartial (d := d) j β u) :=
      contDiff_iterClassicalPartial (d := d) j β hu_cd
    have h_iter_cpt : HasCompactSupport (iterClassicalPartial (d := d) j β u) :=
  ...
  -- Sum the per-term bounds. The number of terms is `∑_{j ∈ range 3} d^j`.
  have h_wkp_le :
      wkpNorm (d := d) 2 2 u Ω'' ≤
