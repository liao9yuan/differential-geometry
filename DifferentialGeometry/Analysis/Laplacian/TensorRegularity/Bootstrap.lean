...
/-! ## The generic-Euclidean assembly and iterated-source arithmetic

The assembly identity and the iterated-source order arithmetic are independent
of the manifold and of the tensor structure: they work with `d : ℕ`,
`[NeZero d]`, and `E := EuclideanSpace ℝ (Fin d)`. -/
...
/-! ### Assembly: `W^{m+2,2}` from the iterated `W^{2,2}` partials

For a smooth compactly supported `u`, the `W^{m+2,2}` norm is bounded by the sum
over multi-indices `idx : Fin m → Fin d` of the `W^{2,2}` norms of the iterated
classical partials `iterClassicalPartial m idx u`, plus a count constant times
the `W^{m+1,2}` norm of `u`. The constant is uniform in `u` (it depends only on
`m` and `d`). The proof is an induction on `m`: each step unfolds the top order
via `wkpNorm_succ_eq_eLpNorm_add_sum_partial`, rewrites the chosen weak partial
as the classical one (`u` is smooth), reindexes the multi-index sum via
`Fin.consEquiv`, and folds the head into `iterClassicalPartial_succ`. -/
...
private theorem wkpNorm_assembly_le
  ...
  | zero =>
      ...
      -- The multi-index sum over `Fin 0 → Fin d` is a singleton equal to
      -- `wkpNorm 2 2 u Ω`.
  ...
  | succ m ih =>
      ...
      -- Each smooth classical partial `∂_i u`.
      ...
      -- `wkpNorm (m+2) 2 (chosenWeakPartial' 2 i u Ω) Ω = wkpNorm (m+2) 2 (∂_i u) Ω`.
      ...
      -- The inductive bound for each `∂_i u`.
      ...
      -- `iterClassicalPartial m idx (∂_i u) = iterClassicalPartial (m+1) (cons i idx) u`.
      have h_iter_cons : ∀ (i : Fin d) (idx : Fin m → Fin d),
        ...
        have h_head : (Fin.cons i idx : Fin (m + 1) → Fin d) 0 = i := by
          simp [Fin.cons_zero]
        rw [h_tail, h_head]
      -- `∂_i u ∈ W^{m+1,2}(Ω)`, with norm controlled by `wkpNorm (m+2) 2 u Ω`.
      ...
      -- Reindex `∑_i ∑_{idx}` into `∑_{β : Fin (m+1) → Fin d}` via `Fin.consEquiv`.
      have h_reindex :
          ∑ β : Fin (m + 1) → Fin d,
      ...
      -- The `eLpNorm u` term is bounded by `wkpNorm (m+1) 2 u Ω`.
      ...
      have h_main :
        ...
        -- Rewrite the chosen-partial norms as classical-partial norms.
        have h_sum_eq :
            ∑ i : Fin d, wkpNorm (d := d) (m + 2) 2
                (chosenWeakPartial' (d := d) 2 i u Ω) Ω =
              ∑ i : Fin d, wkpNorm (d := d) (m + 2) 2
        ...
        rw [h_sum_eq]
        -- Apply the per-direction inductive bound.
        have h_step1 :
            ∑ i : Fin d, wkpNorm (d := d) (m + 2) 2
                (fun x : EE => (fderiv ℝ u x)
                  (EuclideanSpace.single i 1)) Ω ≤
        ...
        -- Split the sum and reindex.
        rw [Finset.sum_add_distrib] at h_step1
        rw [← h_reindex] at h_step1
        -- Bound `∑_i N · wkpNorm (m+1) 2 (∂_i u) Ω` by `(d · N) · wkpNorm (m+2) 2 u Ω`.
        have h_lower :
...
/-! ### Support of the iterated perturbed source

The interior-`H²` engine `smooth_cc_h2_loc_memWkp_two` produces its quantitative
bound in terms of the **global** `L²` data of the source. To convert that global
data into the `Ω''`-restricted `wkpNorm` of the headline right-hand side, the
source must be supported inside the precompact subdomain. The order-`m` iterated
perturbed source of `(u, f)` is supported in any closed set containing the
supports of `u` and `f`: the single-step perturbed source `perturbedSource B u f
l` involves only `∂_l f` (supported in `tsupport f`), `(∂_l c) · u` (supported
in `tsupport u`), and `∂_j[(∂_l a^{ij})(∂_i u)]` (supported in `tsupport u`); and
each iteration step replaces `(u, f)` with `(∂ u, perturbedSource B u f ·)`,
neither of which leaves the union of the original supports. -/
...
private theorem tsupport_perturbedSource_subset
  ...
  classical
  -- Name the three coefficient groups of `perturbedSource` as atomic functions.
  set termA : EE → ℝ :=
  ...
  -- The inner row sum `∑_j ∂_j[(∂_l a^{ij})(∂_i u)]` is supported in `S`.
  ...
  -- The full divergence term `termC` is supported in `S`.
  have h_div : tsupport termC ⊆ S := by
    rw [hC_def]
    exact tsupport_finsetSum_subset_of_forall Finset.univ _ hS_closed
      (fun i _ => h_row i)
  -- `perturbedSource = (termA − termB) + termC`; each group vanishes at any
  -- point outside `S`.
...
/-! ### Iterated-source order arithmetic

The order-`m` iterated perturbed source loses exactly `m` derivatives relative to
`f` and `m+1` relative to `u`: if `f ∈ W^{m,2}(Ω)` and `u ∈ W^{m+1,2}(Ω)`, then
the iterated perturbed source lies in `L²(Ω)` with `L²`-norm controlled by the
`W^{m,2}` norm of `f` and the `W^{m+1,2}` norm of `u`, with a uniform constant.
The estimate is **non-circular**: the order-`m` iterated source carries
`u`-derivatives only to order `m+1`, strictly below the `m+2` solution order
that the single per-step boost produces. The induction folds
`iteratedPerturbedSource_succ` against the single-step Sobolev-order arithmetic
`perturbedSource_memWkp_of_source_memWkp` — which drops one order from `f` and
two from `u` — together with `wkpNorm_classicalPartial_le` to absorb the
...
private theorem wkpNorm_iteratedPerturbedSource_zero_le
  ...
  induction m with
  | zero =>
      -- Order `0`: the iterated source is `f` itself.
      ...
      -- `wkpNorm 0 2 f Ω ≤ wkpNorm 0 2 f Ω + wkpNorm 1 2 u Ω`.
      exact le_self_add
  | succ m ih =>
      obtain ⟨C, hC_nn, hC⟩ := ih
      -- The single-step Sobolev-order arithmetic constant, uniform in `u, f`,
      -- for each head direction `l`.
      ...
      -- The aggregate constant for order `m+1`: `C · (max + 1)`, where `max` is
      -- the largest single-step constant over the finitely many head directions.
      ...
      rw [iteratedPerturbedSource_succ]
      -- The differentiated solution `∂_{idx 0} u`, smooth and compactly supported.
      set u' : EE → ℝ :=
      ...
      -- The once-perturbed source `perturbedSource B u f (idx 0)`.
      ...
      -- The single-step Sobolev-order arithmetic for `f' = perturbedSource B u f (idx 0)`.
      have hf'_norm :=
        (hK_step_bound (idx 0) hu_cd hu_cpt hf_cd hu_mem_succ2 hf_mem_succ).2
      -- Bound `wkpNorm m 2 f' Ω` by the `W^{m+1,2}` data of `f` and the
      -- `W^{m+2,2}` data of `u`, with the location-uniform `Kmax`.
      have hf'_le :
          wkpNorm (d := d) m 2 f' Ω ≤
            ENNReal.ofReal Kmax *
              (wkpNorm (d := d) (m + 1) 2 f Ω +
      ...
      -- `wkpNorm (m+1) 2 u' Ω ≤ wkpNorm (m+2) 2 u Ω`.
      have hu'_le : wkpNorm (d := d) (m + 1) 2 u' Ω ≤
          wkpNorm (d := d) (m + 2) 2 u Ω :=
        wkpNorm_classicalPartial_le (d := d) hΩ_open hu_cd hu_mem_succ2 (idx 0)
      -- Assemble: `wkpNorm 0 2 (iterated source) Ω ≤ ofReal C · (wkpNorm m 2 f' Ω
      --   + wkpNorm (m+1) 2 u' Ω) ≤ ofReal (C · (Kmax+1)) · (data)`.
      refine h_ih.trans ?_
      rw [ENNReal.ofReal_mul hC_nn, mul_assoc]
      refine mul_le_mul_of_nonneg_left ?_ (zero_le _)
      -- `wkpNorm m 2 f' Ω + wkpNorm (m+1) 2 u' Ω ≤
      --   ofReal (Kmax+1) · (wkpNorm (m+1) 2 f Ω + wkpNorm (m+2) 2 u Ω)`.
...
/-! ### Support and norm comparison for iterated classical partials

The iterated classical partial of a function shrinks support and, in `W^{k,2}`
norm, is controlled by the `W^{m+k,2}` norm of the parent (each differentiation
step drops one order). -/
...
private theorem wkpNorm_iterClassicalPartial_le
  ...
  | succ m ih =>
      ...
      rw [iterClassicalPartial_succ]
      -- `∂_{idx 0} u`, smooth and compactly supported.
      set u' : EE → ℝ :=
        fun x => (fderiv ℝ u x) (EuclideanSpace.single (idx 0) 1) with hu'_def
      have hu'_cd : ContDiff ℝ (⊤ : ℕ∞) u' := by
        have h_fderiv : ContDiff ℝ (⊤ : ℕ∞) (fderiv ℝ u) :=
      ...
      have hu'_cpt : HasCompactSupport u' :=
        hu_cpt.fderiv_apply (𝕜 := ℝ) (EuclideanSpace.single (idx 0) 1)
      -- Apply the inductive bound to `∂_{idx 0} u`, then drop one order.
...
/-! ### Real-analytic helpers for the `W^{2,2}` wrapper

The interior-`H²` engine `smooth_cc_h2_loc_memWkp_two` delivers a bound in terms
of `Real.sqrt` of the global `H¹`-and-`L²` data; the helpers below convert that
real `sqrt` into the `ENNReal`-valued `wkpNorm`. -/
...
private lemma eLpNorm_two_sq_eq_ofReal_integral_sq_univ
  ...
  -- `(eLpNorm h 2 (vol.restrict Ω))² = ofReal (∫_Ω h²)`.
  ...
  -- `∫_Ω h² = ∫_univ h²`, since `h²` vanishes off `Ω`.
  rw [h_sq]
  congr 1
...
/-! ### The generic `W^{2,2}` wrapper for the interior-`H²` engine

Repackaging the interior-`H²` engine output for a smooth compactly supported
weak solution `(w, s)` whose support — and the support of its source — lies
inside the precompact subdomain `Ω''`: the engine's global `H¹`-and-`L²` data
becomes the `Ω''`-restricted `W^{1,2}` norm of `w` and the `L²(Ω'')` norm of
`s`, because every integrand vanishes off `Ω''`. -/
...
private theorem smooth_cc_wkp2_wkpNorm_le
  ...
  classical
  -- The interior-`H²` engine constant, uniform in the solution data.
  obtain ⟨C, hC_nn, hC⟩ :=
  ...
  -- The engine output for `(w, s)`.
  obtain ⟨_h_memWkp, h_engine_le⟩ := hC h_weak hw_cpt hs_cd hs_cpt
  -- The smooth classical first partials of `w`.
  ...
  -- `wkpNorm 1 2 w Ω''` decomposes via `chosenWeakPartial'`.
  have hw_W1 : DeGiorgi.MemW1p (d := d) 2 w Ω'' :=
    (memWkp_of_smooth_compactSupport_anyOpen (d := d) hΩ'' hw_cd hw_cpt
      (by norm_num : (1 : ℝ≥0∞) ≤ 2) 1).memW1p
  -- `eLpNorm w` and `eLpNorm (∂_j w)` as squared `ofReal` integrals.
  ...
  -- `eLpNorm (chosenWeakPartial' 2 j w Ω'')² = ofReal (∫ (∂_j w)²)`.
  ...
  -- The squared `W^{1,2}` norm of `w` dominates `ofReal` of the gradient-and-`L²`
  -- data.
  have hw_W1_sq : ENNReal.ofReal
    ...
    rw [h_int_swap]
    -- `ofReal (∑_j ∫(∂_j w)² + ∫ w²) = (∑_j (eLpNorm ∂_j w)²) + (eLpNorm w)²`.
    ...
    rw [h_ofReal_eq]
    -- `(∑ aⱼ²) + b² ≤ (∑ aⱼ + b)²`, where the RHS is `(wkpNorm 1 2 w Ω'')²`.
...
/-! ## The tensor-component interior elliptic a-priori estimate

Specialising the generic Euclidean machinery to the chart component of a tensor
weak solution. The chart component `tensorComponentEuclid g r s T α P₀` of a
smooth compactly supported `(r, s)`-tensor section is a smooth weak solution of
the principal-part elliptic bilinear form `tensorPrincipalForm`, against the
test-function-independent right-hand side `tensorComponentWeakRHS`. Each `m`-fold
mixed partial of that chart component is, by `tensorComponent_iterated_partial_…`,
again a smooth weak solution of the **same** principal form; feeding these into
the generic `W^{2,2}` wrapper and assembling via the order-`m` decomposition
yields the per-step regularity boost. -/
...
/-! ### The per-step regularity boost for a single chart component

For a fixed component multi-index `P₀`, the `W^{m+2,2}` norm of the chart
component over a precompact open subdomain `Ω''` containing the compact support
barrier `K` is bounded by the `W^{m,2}` norm of the right-hand side and the
`W^{m+1,2}` norm of the chart component, with a constant uniform in the tensor
sections. -/
...
private theorem tensorComponent_aPriori_succ
  ...
  -- The generic assembly count constant for order `m`.
  ...
  -- `u` is smooth and compactly supported, with `tsupport u ⊆ K ⊆ Ω''`.
  ...
  -- The right-hand side is supported inside `tsupport u ⊆ K ⊆ Ω''`.
  ...
  -- Step 1: the order-`m` assembly of the `W^{m+2,2}` norm.
  have h_assembly := hN hu_cd hu_cpt
  -- Step 2: each `m`-fold mixed partial is a smooth weak solution of `B`,
  -- against the iterated perturbed source.
  have h_partial_le : ∀ idx : Fin m → Fin dimE,
    ...
    intro idx
    -- The iterated chart partial is a smooth weak solution of `B`.
    have h_weak_sol :
    ...
    -- The iterated solution `∂_{idx} u`, smooth and compactly supported, with
    -- support inside `Ω''`.
    ...
    -- The iterated perturbed source, smooth and compactly supported, with
    -- support inside `Ω''`.
    ...
    -- The generic `W^{2,2}` wrapper bound.
    have h_wrap := hCw h_weak_sol h_w_cpt h_s_cd h_s_cpt h_w_Ω'' h_s_Ω''
    -- `wkpNorm 1 2 (∂_{idx} u) Ω'' ≤ wkpNorm (m+1) 2 u Ω''`.
    ...
    -- `wkpNorm 0 2 (iterated source) Ω'' ≤ ofReal Cs · (data)`.
    ...
    -- Combine the wrapper with the two component bounds.
  ...
  -- Sum the per-`idx` bounds over the multi-index set.
  have h_sum_partial :
      ∑ idx : Fin m → Fin dimE,
  ...
  -- Bound the multi-index sum and the count term.
  set DR : ℝ≥0∞ :=
  ...
  -- `∑_idx wkpNorm 2 2 (∂_idx u) ≤ ↑(dimE^m) · Cw · (Cs+1) · DR`.
  have h_sum_le :
      ∑ idx : Fin m → Fin dimE,
        wkpNorm (d := dimE) 2 2 (iterClassicalPartial (d := dimE) m idx u) Ω'' ≤
      (dimE ^ m : ℕ) *
    ...
    rw [h_card_sum]
  -- `wkpNorm (m+2) 2 u ≤ (∑_idx …) + N · wkpNorm (m+1) 2 u ≤ ofReal C · DR`.
  refine (add_le_add h_sum_le (le_refl _)).trans ?_
  -- `↑(dimE^m) · (ofReal Cw · ((ofReal Cs + 1) · DR)) =
  --   ofReal ((dimE:ℝ)^m · Cw · (Cs+1)) · DR`.
  have h_first_eq :
      (dimE ^ m : ℕ) *
        (ENNReal.ofReal Cw * ((ENNReal.ofReal Cs + 1) * DR)) =
    ...
    ring
  -- `wkpNorm (m+1) 2 u Ω'' ≤ DR`.
  have h_u_le_DR : wkpNorm (d := dimE) (m + 1) 2 u Ω'' ≤ DR := by
    rw [hDR_def]; exact le_add_self
  -- Assemble the two pieces under the aggregate `ofReal` constant.
  calc (dimE ^ m : ℕ) *
        (ENNReal.ofReal Cw * ((ENNReal.ofReal Cs + 1) * DR)) +
...
/-! ### The per-step boost on the full component tuple

Summing the per-step single-component boost over all component multi-indices,
and feeding the right-hand-side norm through the quantitative source bound
`tensorComponentWeakRHS_wkpNorm_le`, yields the per-step boost on the full
component tuple: the order-`m+2` Sobolev norms of the solution's chart
components are controlled by the order-`m` norms of the source's chart
components and the order-`m+1` norms of the solution's chart components. -/
...
theorem tensorComponent_aPriori_succ_sum
          ...
          ((∑ Q : CompIdx E r s,
            ...
            ∑ P : CompIdx E r s,
              wkpNorm (d := dimE) (m + 1) 2
                (tensorComponentEuclid (I := I) (M := M) g r s T α P) Ω'') := by
  classical
  -- The per-component regularity boost, with the constant uniform in `T, F`.
  ...
  -- The per-component source bound, with the constant uniform in `T, F`.
  ...
  -- The aggregate constant: the sum of the per-component products.
  refine ⟨∑ P₀ : CompIdx E r s, (Cstep P₀) * (Ksrc P₀ + 1),
    Finset.sum_nonneg (fun P₀ _ =>
      mul_nonneg (hCstep_nn P₀) (by linarith [hKsrc_nn P₀])), ?_⟩
  intro T F hT_supp hF_supp hT_K hF_K hweak
  -- Abbreviations for the two component sums on the right-hand side.
  ...
  -- The per-component bound, lifted to the aggregate `(SF + ST)` shape.
  have h_per : ∀ P₀ : CompIdx E r s,
      wkpNorm (d := dimE) (m + 2) 2
          (tensorComponentEuclid (I := I) (M := M) g r s T α P₀) Ω'' ≤
        ENNReal.ofReal ((Cstep P₀) * (Ksrc P₀ + 1)) * (SF + ST) := by
    intro P₀
    -- The per-component step bound and the per-component source bound.
    ...
    -- Chain: `wkpNorm (m+2) (T-comp P₀) ≤ ofReal(Cstep·(Ksrc+1))·(SF+ST)`.
    refine h_step.trans ?_
    rw [ENNReal.ofReal_mul (hCstep_nn P₀), mul_assoc]
    refine mul_le_mul_of_nonneg_left ?_ (zero_le _)
    -- `wkpNorm m RHS + wkpNorm (m+1) (T-comp P₀) ≤ ofReal(Ksrc+1)·(SF+ST)`.
