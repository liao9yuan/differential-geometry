...
`v_n → u_h`; the chain via density (M5/M6/M7/M8 in
`DiffChartBilinearH1ComplFinal`) supplies the smooth approximators for
which the residual MemW1p holds unconditionally. The general
