...
```
fHLeibnizResidualLp = preimage⟨smoothMulHC ρα u_h⟩
...
* `laplacianDomain_preimage_smoothMulHC` — the Phase 3 preimage identity
  `preimage⟨smoothMulHC ρα u_h⟩ = fHLeibnizGeneral g ρα u_h hu_dom`.
...
* `fHLeibnizResidualLp_eq_preimageDiff` — for `u_h ∈ laplacianDomain g`:
  ...
  fHLeibnizResidualLp g α u_h
    = preimage⟨smoothMulHC ρα u_h, smoothMulHC_mem_laplacianDomain⟩
...
```
fChartResidual =ᵃᵉ
    chartPushedRawLpFromLp(preimage⟨smoothMulHC ρα u_h⟩).coeFn
...
The first summand `chartPushedRawLpFromLp(preimage⟨smoothMulHC ρα u_h⟩)`
would require `preimage⟨smoothMulHC ρα u_h⟩.coeFn ∈ MemWkpChart g 1 2`, which
in turn requires `smoothMulHC ρα u_h ∈ laplacianDomainPow g 2` (iterated
...
private lemma fHLeibnizGeneralResidualCLM_eq_fHLeibnizResidualLp
...
`(1-Δ_g)`-preimages. -/
theorem preimage_smoothMulHC_eq_smoothMulLp_preimage_add_residual
    (g : SmoothRiemannianMetric I M) (α : M)
    {u_h : H1Compl (I := I) (M := M) g}
  ...
  classical
  -- Step 1: `preimage⟨smoothMulHC ρα u_h⟩ = fHLeibnizGeneral g ρα u_h hu_dom`.
  have h_preimage_smoothMulHC :=
    laplacianDomain_preimage_smoothMulHC (I := I) (M := M) g
      (chartAtlasPOU I M α : C^∞⟮I, M; ℝ⟯) hu_dom
  -- Step 2: Unfold `fHLeibnizGeneral`.
  unfold fHLeibnizGeneral at h_preimage_smoothMulHC
  ...
  have h_diff_eq :
    ...
    abel
  rw [h_diff_eq] at h_preimage_smoothMulHC
  exact h_preimage_smoothMulHC
    ...
    fHLeibnizGeneralResidualCLM (I := I) (M := M) g
      ...
      laplacianDomain.preimage (I := I) (M := M) g
        ...
        smoothMulLp (I := I) (M := M) g
          ...
          (laplacianDomain.preimage (I := I) (M := M) g ⟨u_h, hu_dom⟩) := by
  classical
  have h := preimage_smoothMulHC_eq_smoothMulLp_preimage_add_residual
    (I := I) (M := M) g α hu_dom
  -- h: preimage⟨smoothMulHC ρα u_h⟩ = smoothMulLp ρα (preimage⟨u_h⟩) + fHLeibnizGeneralResidualCLM g ρα u_h.
