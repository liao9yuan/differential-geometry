...
/-! ## Step 1: identification of `chartPushedPartial` with the partial of `chartSmoothExt`

On the chart-target `chartTargetEuclid α`, the chart-pushed function
`chartPushed POU α v.toFun` agrees pointwise with `chartSmoothExt α (ρα · v.toFun)`.
Since `chartTargetEuclid α` is open, the Fréchet derivatives of the two
functions agree on this set, and hence so do their partials in the
direction `EuclideanSpace.single j 1`. -/
...
private lemma chartPushedPartial_eq_fderiv_chartSmoothExt
  ...
  congr 1
  -- Goal: fderiv ℝ chartPushed = fderiv ℝ chartSmoothExt at y.
  -- They agree on the open set chartTargetEuclid α.
  have hOpen : IsOpen (chartTargetEuclid (I := I) (M := M) α) :=
    Sobolev.Chart.chartTargetEuclid_isOpen (I := I) (M := M) α
  apply Filter.EventuallyEq.fderiv_eq
  filter_upwards [hOpen.mem_nhds hy] with z hz
  classical
  -- chartPushed POU α v.toFun z = chartSmoothExt α (ρα · v.toFun) z for z ∈ chartTargetEuclid α.
  rcases hz with ⟨w, hw_target, hwz⟩
  have h_eq : (toEuclidean (E := E)).symm z = w := by
    rw [← hwz]; exact (toEuclidean (E := E)).symm_apply_apply w
  -- chartPushed POU α v.toFun z = ρα(symm z) · v.toFun(symm z)
  have h_chartPushed : chartPushed (I := I) (M := M) (chartAtlasPOU I M) α v.toFun z =
    ...
    rfl
  rw [h_chartPushed]
  -- chartSmoothExt α (ρα · v.toFun) z = (ρα · v.toFun)(symm z) on chartTargetEuclid α.
...
/-! ## Step 2: density bound conversion

The chart-pulled weighted measure `chartPulledWeightedMeasure g α` is
`vol.withDensity (ofReal density)`. On the compact set `kPouCompact α`
the density is uniformly bounded above. Hence for any function supported
in `kPouCompact α`, the L² norm against the chart-pulled weighted measure
is bounded by `√M_d` times the L² norm against `vol`. -/

/-! ## Step 2 helper: density-bound conversion of L² norms via measure smul

The chart-pulled weighted measure restricted to the compact `kPouCompact α`
is dominated by `M_d • vol.restrict kPouCompact α`. -/
...
private lemma chartPulledWeightedMeasure_le_smul_volume_on_kPouCompact
  ...
  -- LHS: chartPulledWeightedMeasure (A ∩ K).
  unfold chartPulledWeightedMeasure
  rw [withDensity_apply _ (hA.inter hK_meas)]
  -- Goal: ∫⁻ y in A ∩ K, ofReal density(y) dvol ≤ ofReal M_d * vol(A ∩ K).
...
private lemma eLpNorm_chartPulledWeighted_le_eLpNorm_volume_of_support_in_kPou
  ...
  -- chartPulledWeightedConst = √M_d_unsqrt by definition.
  change eLpNorm f 2
        ((chartPulledWeightedMeasure (I := I) g α).restrict
            (chartTargetEuclid (I := I) (M := M) α)) ≤
  ...
  -- Step 1: f.support ⊆ kPouCompact ⊆ ChTE, so the restricted-eLpNorm equals the unrestricted one.
  ...
  -- Now: eLpNorm f 2 (chartPulledWeighted.restrict K) ≤ ENNReal.ofReal √M_d_unsqrt · eLpNorm f 2 (vol.restrict K)
  ...
  -- eLpNorm f 2 (c • μ) = c^{1/2} • eLpNorm f 2 μ for c ≠ ⊤.
  rw [eLpNorm_smul_measure_of_ne_top (by norm_num : (2 : ℝ≥0∞) ≠ ⊤)]
  -- Goal: ENNReal.ofReal M_d_unsqrt ^ ((1/2).toReal) • eLpNorm f 2 (vol.restrict K)
  --       ≤ ENNReal.ofReal √M_d_unsqrt * eLpNorm f 2 (vol.restrict K)
  -- (1/2 : ℝ≥0∞).toReal = 1/2 (as ℝ).
  have h_toReal : ((1 / 2 : ℝ≥0∞).toReal : ℝ) = (1 : ℝ) / 2 := by
    rw [show (1 / 2 : ℝ≥0∞) = (1 : ℝ≥0∞) / 2 from rfl]
    simp
  rw [h_toReal]
  -- Goal: ENNReal.ofReal M_d_unsqrt ^ (1/2) • eLpNorm = ENNReal.ofReal √M_d_unsqrt * eLpNorm
...
/-! ## Step 4: relating L² norms of `v.toFun` and `√g(grad v, grad v)` to `‖v‖`

The H¹ pre-norm `‖v‖` on `SmoothScalar g` equals `√(inner v v)`, where the
inner product unfolds to `∫_M v² dμ_g + ∫_M ‖grad v‖²_g dμ_g`. Both
contributions are individually bounded by `‖v‖²`. -/
...
private lemma lintegral_enorm_v_toFun_sq_eq
  ...
  · refine MeasureTheory.lintegral_congr ?_
    intro x
    -- Goal: ‖v.toFun x‖ₑ ^ (2 : ℝ) = ENNReal.ofReal (v.toFun x * v.toFun x)
    have h1 : ‖v.toFun x‖ₑ ^ (2 : ℝ) = (‖v.toFun x‖ₑ)^(2 : ℕ) := by
      rw [show (2 : ℝ) = ((2 : ℕ) : ℝ) from by norm_num]
      rw [ENNReal.rpow_natCast]
    rw [h1]
    -- Goal: ‖v.toFun x‖ₑ^2 = ENNReal.ofReal (v.toFun x * v.toFun x)
    rw [Real.enorm_eq_ofReal_abs]
    -- Goal: (ENNReal.ofReal |v.toFun x|)^2 = ENNReal.ofReal (v.toFun x * v.toFun x)
...
private lemma eLpNorm_v_toFun_le_norm
    ...
    eLpNorm v.toFun 2 (riemannianVolumeMeasure (I := I) (M := M) g) ≤
      ENNReal.ofReal ‖v‖ := by
  classical
  -- ‖v‖² = inner v v = ∫ v² dμ_g + ∫ ‖grad v‖² dμ_g ≥ ∫ v² dμ_g = (eLpNorm v 2 μ_g)².
  -- So eLpNorm v 2 μ_g ≤ ‖v‖.
  ...
  have h_norm_sq : ‖v‖^2 = smoothScalarH1Inner (I := I) (M := M) v v := by
    ...
    rw [h_inner_self]
  -- Compute (eLpNorm v 2 μ_g)² in terms of the lintegral.
  ...
  -- Goal: ENNReal.ofReal (∫ v²) ^ (1/2) ≤ ENNReal.ofReal ‖v‖
  ...
  -- Goal: ENNReal.ofReal (sqrt (∫ v²)) ≤ ENNReal.ofReal ‖v‖
  -- It suffices to show: sqrt (∫ v²) ≤ ‖v‖.
  refine ENNReal.ofReal_le_ofReal ?_
  -- Show sqrt(∫ v²) ≤ ‖v‖ using sq_le_sq.
...
private lemma lintegral_enorm_sqrt_grad_v_sq_eq
  ...
  -- Pointwise: ‖√g(grad v, grad v)‖ₑ² = ENNReal.ofReal (g(grad v, grad v))
  -- Note: grad_g v applied is gradFun.
  rw [MeasureTheory.integral_eq_lintegral_of_nonneg_ae
  ...
  · refine MeasureTheory.lintegral_congr ?_
    ...
    have h_grad_eq : ((grad_g (I := I) g v.smooth :
      ...
      rw [grad_g_apply]
    rw [h_grad_eq]
    -- Goal: ‖√g(grad v, grad v)‖ₑ ^ (2 : ℝ) = ENNReal.ofReal (g(grad v, grad v))
...
/-! ## Headline H¹-Lipschitz bound

We combine the chart-side identification (Step 1), the density-bound L² norm
conversion (Step 3), the per-α gradient L² bound (existing infrastructure
`eLpNorm_fderiv_chartSmoothExt_apply_le_const_mul`), and the H¹-pre-norm
bounds on the L² components (Step 4) to obtain the headline bound. -/
...
theorem chartPushedPartial_h1_lipschitz
  ...
  classical
  -- Get the per-α gradient L² bound from EquivalenceReverse.
  ...
  -- The constant is C := chartPulledWeightedConst * 2 * C_grad.
  ...
  refine ⟨C, hC_nn, ?_⟩
  intro v
  -- Step A: chartPushedPartial =ᵐ smoothChartExtPartial against chartPulledWeighted.restrict ChTE.
  have h_aeEq := chartPushedPartial_aeEq_fderiv_chartSmoothExt
    (I := I) (M := M) g α j v
  rw [eLpNorm_congr_ae h_aeEq]
  -- Step B: support of fderiv ℝ chartSmoothExt α (...) (·) (e_j) ⊆ kPouCompact.
  have h_supp : Function.support (fun y : EuclN => (fderiv ℝ
    ...
    -- We re-use the support of smoothChartExtPartial in kPouCompact.
    -- chartSmoothExt and smoothChartExt agree pointwise (definitions).
    intro y hy
    have hy_ne : (fderiv ℝ
        (chartSmoothExt (I := I) (M := M) α
          (fun x : M => ((chartAtlasPOU I M α : C^∞⟮I, M; ℝ⟯) : M → ℝ) x * v.toFun x)) y)
        (EuclideanSpace.single j 1) ≠ 0 := hy
    -- chartSmoothExt α (ρα * v.toFun) = smoothChartExt g α v pointwise.
    ...
    rw [h_funeq] at hy_ne
    -- This is the support of `smoothChartExtPartial g α j v`, which is ⊆ kPouCompact.
  ...
  -- Step C: density-bound conversion: eLpNorm fderiv 2 (chartPulledWeighted.restrict ChTE)
  --       ≤ ofReal M_sqrt * eLpNorm fderiv 2 (vol.restrict ChTE).
  ...
  -- Step D: per-α gradient L² bound.
  have h_grad_bd := hC_grad_bd v.smooth j
  -- h_grad_bd : eLpNorm ... 2 (vol.restrict ChTE) ≤ ENNReal.ofReal C_grad * (eLpNorm v + eLpNorm √g)
  -- Step E: bound (eLpNorm v + eLpNorm √g) by 2 * ‖v‖.
  ...
  -- Combine: ENNReal.ofReal M_sqrt * (ENNReal.ofReal C_grad * (eLpNorm + eLpNorm))
  --        ≤ ENNReal.ofReal M_sqrt * (ENNReal.ofReal C_grad * (2 * ENNReal.ofReal ‖v‖))
  --        = ENNReal.ofReal C * ENNReal.ofReal ‖v‖
  --        = ENNReal.ofReal C * (‖v‖₊ : ℝ≥0∞)  (since ‖v‖ ≥ 0)
