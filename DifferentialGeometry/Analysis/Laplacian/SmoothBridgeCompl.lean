...
This file connects the variational Laplacian via Lax-Milgram (`resolvent`) to
the classical Laplace-Beltrami operator (`Δ_g`) on smooth functions, on a
...
* `smoothScalar_bilin_eq_lpFunctional_smooth`: the variational identity for
  smooth test functions.
* `smoothScalar_resolvent_one_sub_lap`: the central bridge,
  `smoothToH1Compl u = resolvent (L²-class of (u - Δ_g u))`.
-/
