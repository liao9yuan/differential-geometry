...
theorem appD2_pair_h3
  ...
  have hgradL :
    ...
    have hodd' : y ^ 2 = ‖L‖ ^ 2 +
      ...
      simpa only [y, L, Nat.reduceMul, Nat.reduceAdd, Nat.cast_ofNat,
        norm_ccHs_eq_smoothHs, oneMinusConnLapSmoothIter_succ,
        oneMinusConnLapSmoothIter_zero, SmoothCcTensor.norm_toL2] using hodd
    nlinarith [hodd', norm_nonneg (covGrad (I := I) (M := M) g 0 2 L),
      norm_nonneg L]
  have hD₂ : ‖D₂‖ ≤ C₂ * x := by
    have hpick : ‖D₂‖ ≤
        ∑ j ∈ Finset.range 3,
  ...
  have hD₃ :
    ...
    have hcomp :
      ...
      dsimp only [D₂]
      exact icg_comp_norm (I := I) (M := M) g 2 2 1 W
    rw [hcomp]
    exact (Finset.single_le_sum (s := Finset.range 4)
      (f := fun j => ‖iteratedCovGrad (I := I) g 0 2 j W‖)
      (fun j _ => norm_nonneg _) (show 3 ∈ Finset.range 4 by norm_num)).trans (by
  ...
  have hslot : ∀ p : M,
      riemannianFiberNormSq (I := I) (M := M) g 5 3 p
        ...
        (sd * B₀) ^ 2 := by
    intro p
    rw [rfns_slotExtend_eq (I := I) (M := M) g 4 2 Φ p]
    calc
