...
private noncomputable def appD1CcLin
...
theorem appD1Hs_norm
    ...
    ∃ C : ℝ, 0 ≤ C ∧
      ∀ (Φ : SmoothCcTensor g (s + 1) c) (A : ℝ), 0 ≤ A →
        ...
        ‖appD1Hs (I := I) (M := M) g s c Φ‖ ≤ C * A := by
  obtain ⟨C, hC, happ⟩ :=
    appCc_h2_h3_h2 (I := I) (M := M) hDim g s c
  refine ⟨C, hC, ?_⟩
  intro Φ A hA hΦ
