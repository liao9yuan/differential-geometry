...
/-!
# Top-separated residual integrator and radius-free windows
...
theorem antidiagonalTupleGrid_integral_radiusFree
  ...
  intro P hsup i
  have hone_le : (1 : ℝ) ≤ 1 + ‖iteratedCovGrad (I := I) g₀ 0 2 i P‖ ^ 2 := by
    linarith [sq_nonneg (‖iteratedCovGrad (I := I) g₀ 0 2 i P‖)]
  by_cases hi0 : i = 0
  · subst hi0
    ...
    · rw [hgrid0, MeasureTheory.integral_const, smul_eq_mul, mul_one,
      ...
      calc vol ≤ ((∑ n ∈ Finset.range (0 + 1),
        ...
        _ ≤ ((∑ n ∈ Finset.range (0 + 1),
            ...
            (1 + ‖iteratedCovGrad (I := I) g₀ 0 2 0 P‖ ^ 2) :=
  ...
  · have hi1 : 1 ≤ i := Nat.one_le_iff_ne_zero.mpr hi0
    ...
    have hPT : ∀ n ∈ Finset.range (i + 1), ∀ e ∈ Finset.Nat.antidiagonalTuple n i,
      ...
      have hres := grid_prod_int_le (I := I) (M := M) g₀ P
        (norm_nonneg (iteratedCovGrad (I := I) g₀ 0 2 i P)) i hi1 hΛ₀0 hsup
    ...
    calc (∑ n ∈ Finset.range (i + 1), ((Finset.Nat.antidiagonalTuple n i).card : ℝ)) *
            (Gfun i * ‖iteratedCovGrad (I := I) g₀ 0 2 i P‖ ^ 2)
        = ((∑ n ∈ Finset.range (i + 1), ((Finset.Nat.antidiagonalTuple n i).card : ℝ)) *
            Gfun i) * ‖iteratedCovGrad (I := I) g₀ 0 2 i P‖ ^ 2 := by ring
      _ ≤ ((∑ n ∈ Finset.range (i + 1), ((Finset.Nat.antidiagonalTuple n i).card : ℝ)) *
            Gfun i) * (1 + ‖iteratedCovGrad (I := I) g₀ 0 2 i P‖ ^ 2) := by
          refine mul_le_mul_of_nonneg_left ?_ (mul_nonneg hcard_nn (hGfun_nn i))
          linarith [sq_nonneg (‖iteratedCovGrad (I := I) g₀ 0 2 i P‖)]
      _ ≤ ((∑ n ∈ Finset.range (i + 1), ((Finset.Nat.antidiagonalTuple n i).card : ℝ)) *
            Gfun i + vol) * (1 + ‖iteratedCovGrad (I := I) g₀ 0 2 i P‖ ^ 2) := by
          refine mul_le_mul_of_nonneg_right ?_
            (by linarith [sq_nonneg (‖iteratedCovGrad (I := I) g₀ 0 2 i P‖)])
          linarith
...
theorem boundedFactorGridWindow_integral_radiusFree_topSeparated
          ...
          MeasureTheory.Integrable
            ...
            (∫ x, Combinatorics.boundedFactorGridWindow
              ...
              Klow i * (1 + ∑ j ∈ Finset.range (i + 2),
                ‖iteratedCovGrad (I := I) g₀ 0 2 j P‖ ^ 2) +
                Ktop i * ‖iteratedCovGrad (I := I) g₀ 0 2 (i + 2) P‖ ^ 2 := by
  classical
  letI : MeasurableSpace E := borel E
  haveI : BorelSpace E := ⟨rfl⟩
  letI : MeasurableSpace M := borel M
  ...
  haveI : IsFiniteMeasure (riemannianVolumeMeasure (I := I) (M := M) g₀) :=
    riemannianVolumeMeasure_isFiniteMeasure_of_compactSpace g₀
  obtain ⟨Kt, hKt_nn, hKt⟩ := antidiagonalTupleGrid_integral_radiusFree (I := I) (M := M) g₀ hΛ₀0
  ...
  have hint2_k : ∀ k : ℕ,
      (∫ x, Combinatorics.antidiagonalTupleGrid (b x) k
          ∂(riemannianVolumeMeasure (I := I) (M := M) g₀)) ≤
        Kt k * (1 + ‖iteratedCovGrad (I := I) g₀ 0 2 k P‖ ^ 2) :=
  ...
  have hstep1 : (∑ k ∈ Finset.range (i + 3),
        ∫ x, Combinatorics.antidiagonalTupleGrid (b x) k
          ∂(riemannianVolumeMeasure (I := I) (M := M) g₀)) ≤
      ∑ k ∈ Finset.range (i + 3), Kt k * (1 + ‖iteratedCovGrad (I := I) g₀ 0 2 k P‖ ^ 2) :=
  ...
  show (∑ k ∈ Finset.range (i + 3),
      ...
      (∑ k ∈ Finset.range (i + 3), Kt k) *
        ...
        Kt (i + 2) * ‖iteratedCovGrad (I := I) g₀ 0 2 (i + 2) P‖ ^ 2
  ...
  have hLHS : (∑ k ∈ Finset.range (i + 3),
      ...
      (∑ k ∈ Finset.range (i + 3), Kt k) +
        ∑ k ∈ Finset.range (i + 3),
          Kt k * ‖iteratedCovGrad (I := I) g₀ 0 2 k P‖ ^ 2 := by
    rw [← Finset.sum_add_distrib]
    apply Finset.sum_congr rfl; intro k _; ring
  rw [hLHS]
  have hjsplit : (∑ k ∈ Finset.range (i + 3),
        Kt k * ‖iteratedCovGrad (I := I) g₀ 0 2 k P‖ ^ 2) =
      (∑ k ∈ Finset.range (i + 2),
          Kt k * ‖iteratedCovGrad (I := I) g₀ 0 2 k P‖ ^ 2) +
        Kt (i + 2) * ‖iteratedCovGrad (I := I) g₀ 0 2 (i + 2) P‖ ^ 2 := by
