...
private noncomputable def radialHighCore
    (g : SmoothRiemannianMetric I M) (ρ : ℝ) :
    highCore (I := I) (M := M) g → metricH3 (I := I) (M := M) g :=
  fun x =>
    ccToHsLin (I := I) (M := M) g 2 (3 : ℝ)
      (lowRadial (I := I) (M := M) g ρ
        (highRep (I := I) (M := M) g x))
...
private theorem radialHigh_balls
    ...
    ∀ R : ℝ, ∃ K : NNReal,
      LipschitzOnWith K (radialHighCore (I := I) (M := M) g ρ)
        {x : highCore (I := I) (M := M) g |
          dist (x : metricH3 (I := I) (M := M) g) 0 ≤ R} := by
  intro R
  let q : ℝ := max R 0
  let K0 : ℝ := 1 + (1 / ρ) * q
  have hq : 0 ≤ q := by
  ...
  have hx3 :
      ccTensorToHs (I := I) (M := M) g 2 (3 : ℝ)
          (highRep (I := I) (M := M) g x) =
        (x : metricH3 (I := I) (M := M) g) := by
    simpa only [ccToHsLin_apply] using
      highRep_spec (I := I) (M := M) g x
  have hy3 :
      ccTensorToHs (I := I) (M := M) g 2 (3 : ℝ)
          (highRep (I := I) (M := M) g y) =
        (y : metricH3 (I := I) (M := M) g) := by
    simpa only [ccToHsLin_apply] using
  ...
  have hxq :
      ‖(x : metricH3 (I := I) (M := M) g)‖ ≤ q := by
    change dist
      (x : metricH3 (I := I) (M := M) g) 0 ≤ R at hx
    rw [dist_zero_right] at hx
    exact hx.trans (le_max_left _ _)
  have hyq :
      ‖(y : metricH3 (I := I) (M := M) g)‖ ≤ q := by
    change dist
      (y : metricH3 (I := I) (M := M) g) 0 ≤ R at hy
    rw [dist_zero_right] at hy
    exact hy.trans (le_max_left _ _)
  have hmax :
      max
          ‖(x : metricH3 (I := I) (M := M) g)‖
          ‖(y : metricH3 (I := I) (M := M) g)‖ ≤ q :=
    max_le hxq hyq
  have hincl :
      ‖incl32 (I := I) (M := M) g
            (x : metricH3 (I := I) (M := M) g) -
          incl32 (I := I) (M := M) g
            (y : metricH3 (I := I) (M := M) g)‖ ≤
        ‖(x : metricH3 (I := I) (M := M) g) -
          (y : metricH3 (I := I) (M := M) g)‖ := by
    rw [← map_sub]
  ...
  calc
    ...
    _ ≤ ‖(x : metricH3 (I := I) (M := M) g) -
          (y : metricH3 (I := I) (M := M) g)‖ +
        (1 / ρ) * q *
          ‖(x : metricH3 (I := I) (M := M) g) -
            (y : metricH3 (I := I) (M := M) g)‖ :=
      add_le_add le_rfl hprod
    _ = K0 * ‖(x : metricH3 (I := I) (M := M) g) -
          (y : metricH3 (I := I) (M := M) g)‖ := by
      simp only [K0]
      ring
...
private theorem radialHigh_cont
  ...
  intro x
  let R : ℝ :=
    dist (x : metricH3 (I := I) (M := M) g) 0 + 1
  obtain ⟨K, hK⟩ :=
    radialHigh_balls (I := I) (M := M) g hρ R
  have hxball :
      (x : metricH3 (I := I) (M := M) g) ∈
        Metric.ball (0 : metricH3 (I := I) (M := M) g) R := by
    rw [Metric.mem_ball]
    dsimp only [R]
    linarith
  have hxclosed :
      x ∈ {y : highCore (I := I) (M := M) g |
        dist (y : metricH3 (I := I) (M := M) g) 0 ≤ R} := by
    change dist (x : metricH3 (I := I) (M := M) g) 0 ≤ R
    dsimp only [R]
    linarith
  have hclosed :
      Metric.closedBall
          (0 : metricH3 (I := I) (M := M) g) R ∈
        𝓝 (x : metricH3 (I := I) (M := M) g) :=
    Metric.closedBall_mem_nhds_of_mem hxball
  have hpre :
      ((↑) : highCore (I := I) (M := M) g →
          metricH3 (I := I) (M := M) g) ⁻¹'
          Metric.closedBall
            (0 : metricH3 (I := I) (M := M) g) R ∈ 𝓝 x :=
...
theorem lowRadialH3_le
  ...
  let D : Set (metricH3 (I := I) (M := M) g) :=
    Set.range (ccToHsLin (I := I) (M := M) g 2 (3 : ℝ))
  let C : Set (metricH3 (I := I) (M := M) g) :=
...
theorem lowRadialHs_eq
  ...
  · exact (lipschitzWith_ballRetraction hρ).continuous.comp
      (symmHs (I := I) (M := M) g (by norm_num)).continuous
  · intro T
...
private abbrev lowA2LoOp (g : SmoothRiemannianMetric I M) :=
  tensorHs (I := I) (M := M) g 0 2 (3 : ℝ) →L[ℝ]
    tensorHs (I := I) (M := M) g 0 2 (1 : ℝ)

private noncomputable def lowA2HiCore
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
        gFibreOpBound (I := I) (M := M) g
          (ccTensorBilinSymm (I := I) g S) δ) :
    lowCore (I := I) (M := M) g → lowA2HiOp (I := I) (M := M) g :=
  fun x =>
    (lowCoreData (I := I) (M := M) g hρ hδ0 hδ_le hreal
      (lowRep (I := I) (M := M) g x)).a2Hi (I := I) (M := M)

private noncomputable def lowA2LoCore
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
        gFibreOpBound (I := I) (M := M) g
          (ccTensorBilinSymm (I := I) g S) δ) :
    lowCore (I := I) (M := M) g → lowA2LoOp (I := I) (M := M) g :=
  fun x =>
    (lowCoreData (I := I) (M := M) g hρ hδ0 hδ_le hreal
      (lowRep (I := I) (M := M) g x)).a2Lo (I := I) (M := M)

private theorem lowA2HiCore_value
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
    ...
    lowA2HiCore (I := I) (M := M) g hρ hδ0 hδ_le hreal
        ⟨ccToHsLin (I := I) (M := M) g 2 (2 : ℝ) T, ⟨T, rfl⟩⟩ =
      (lowCoreData (I := I) (M := M) g hρ hδ0 hδ_le hreal T).a2Hi
        (I := I) (M := M) := by
  have hrep :
      lowRep (I := I) (M := M) g
    ...
    simpa only [ccToHsLin_apply] using
      lowRep_spec (I := I) (M := M) g
        ⟨ccToHsLin (I := I) (M := M) g 2 (2 : ℝ) T, ⟨T, rfl⟩⟩
  simp only [lowA2HiCore, hrep]

private theorem lowA2LoCore_value
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
    ...
    lowA2LoCore (I := I) (M := M) g hρ hδ0 hδ_le hreal
        ⟨ccToHsLin (I := I) (M := M) g 2 (2 : ℝ) T, ⟨T, rfl⟩⟩ =
      (lowCoreData (I := I) (M := M) g hρ hδ0 hδ_le hreal T).a2Lo
        (I := I) (M := M) := by
  have hrep :
      lowRep (I := I) (M := M) g
          ⟨ccToHsLin (I := I) (M := M) g 2 (2 : ℝ) T, ⟨T, rfl⟩⟩ = T := by
...
abbrev A2HiOp (g : SmoothRiemannianMetric I M) :=
  lowA2HiOp (I := I) (M := M) g

abbrev A2LoOp (g : SmoothRiemannianMetric I M) :=
  lowA2LoOp (I := I) (M := M) g

noncomputable def a2HiCore
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
        gFibreOpBound (I := I) (M := M) g
          (ccTensorBilinSymm (I := I) g S) δ) :
    LowCore (I := I) (M := M) g → A2HiOp (I := I) (M := M) g :=
  lowA2HiCore (I := I) (M := M) g hρ hδ0 hδ_le hreal

noncomputable def a2LoCore
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
        gFibreOpBound (I := I) (M := M) g
          (ccTensorBilinSymm (I := I) g S) δ) :
    LowCore (I := I) (M := M) g → A2LoOp (I := I) (M := M) g :=
  lowA2LoCore (I := I) (M := M) g hρ hδ0 hδ_le hreal

theorem a2HiCore_value
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
        gFibreOpBound (I := I) (M := M) g
          (ccTensorBilinSymm (I := I) g S) δ)
    (T : SmoothCcTensor g 0 2) :
    a2HiCore (I := I) (M := M) g hρ hδ0 hδ_le hreal
        ⟨ccToHsLin (I := I) (M := M) g 2 (2 : ℝ) T, ⟨T, rfl⟩⟩ =
      (lowCoreData (I := I) (M := M) g hρ hδ0 hδ_le hreal T).a2Hi
        (I := I) (M := M) :=
  lowA2HiCore_value (I := I) (M := M) g hρ hδ0 hδ_le hreal T

theorem a2LoCore_value
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
        gFibreOpBound (I := I) (M := M) g
          (ccTensorBilinSymm (I := I) g S) δ)
    (T : SmoothCcTensor g 0 2) :
    a2LoCore (I := I) (M := M) g hρ hδ0 hδ_le hreal
        ⟨ccToHsLin (I := I) (M := M) g 2 (2 : ℝ) T, ⟨T, rfl⟩⟩ =
      (lowCoreData (I := I) (M := M) g hρ hδ0 hδ_le hreal T).a2Lo
        (I := I) (M := M) :=
  lowA2LoCore_value (I := I) (M := M) g hρ hδ0 hδ_le hreal T
...
noncomputable def lowA2Lo
    ...
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
        gFibreOpBound (I := I) (M := M) g
          (ccTensorBilinSymm (I := I) g S) δ) :
    metricH2 (I := I) (M := M) g → lowA2LoOp (I := I) (M := M) g :=
  Dense.extend
    (ccToHsLin_dense (I := I) (M := M) g 2 (by positivity))
    (lowA2LoCore (I := I) (M := M) g hρ hδ0 hδ_le hreal)
...
private abbrev lowA1LoOp (g : SmoothRiemannianMetric I M) :=
  tensorHs (I := I) (M := M) g 0 2 (2 : ℝ) →L[ℝ]
    tensorHs (I := I) (M := M) g 0 2 (1 : ℝ)

private noncomputable def lowA1HiCore
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
        gFibreOpBound (I := I) (M := M) g
          (ccTensorBilinSymm (I := I) g S) δ) :
    highCore (I := I) (M := M) g → lowA1HiOp (I := I) (M := M) g :=
  fun x =>
    (lowCoreData (I := I) (M := M) g hρ hδ0 hδ_le hreal
      (highRep (I := I) (M := M) g x)).a1Hi (I := I) (M := M)

private noncomputable def lowA1LoCore
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
        gFibreOpBound (I := I) (M := M) g
          (ccTensorBilinSymm (I := I) g S) δ) :
    highCore (I := I) (M := M) g → lowA1LoOp (I := I) (M := M) g :=
  fun x =>
    (lowCoreData (I := I) (M := M) g hρ hδ0 hδ_le hreal
      (highRep (I := I) (M := M) g x)).a1Lo (I := I) (M := M)

private theorem lowA1HiCore_value
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
    ...
    lowA1HiCore (I := I) (M := M) g hρ hδ0 hδ_le hreal
        ⟨ccToHsLin (I := I) (M := M) g 2 (3 : ℝ) T, ⟨T, rfl⟩⟩ =
      (lowCoreData (I := I) (M := M) g hρ hδ0 hδ_le hreal T).a1Hi
        (I := I) (M := M) := by
  have hrep :
      highRep (I := I) (M := M) g
    ...
    simpa only [ccToHsLin_apply] using
      highRep_spec (I := I) (M := M) g
        ⟨ccToHsLin (I := I) (M := M) g 2 (3 : ℝ) T, ⟨T, rfl⟩⟩
  simp only [lowA1HiCore, hrep]

private theorem lowA1LoCore_value
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
    ...
    lowA1LoCore (I := I) (M := M) g hρ hδ0 hδ_le hreal
        ⟨ccToHsLin (I := I) (M := M) g 2 (3 : ℝ) T, ⟨T, rfl⟩⟩ =
      (lowCoreData (I := I) (M := M) g hρ hδ0 hδ_le hreal T).a1Lo
        (I := I) (M := M) := by
  have hrep :
      highRep (I := I) (M := M) g
          ⟨ccToHsLin (I := I) (M := M) g 2 (3 : ℝ) T, ⟨T, rfl⟩⟩ = T := by
...
abbrev A1HiOp (g : SmoothRiemannianMetric I M) :=
  lowA1HiOp (I := I) (M := M) g

abbrev A1LoOp (g : SmoothRiemannianMetric I M) :=
  lowA1LoOp (I := I) (M := M) g

noncomputable def a1HiCore
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
        gFibreOpBound (I := I) (M := M) g
          (ccTensorBilinSymm (I := I) g S) δ) :
    HighCore (I := I) (M := M) g → A1HiOp (I := I) (M := M) g :=
  lowA1HiCore (I := I) (M := M) g hρ hδ0 hδ_le hreal

noncomputable def a1LoCore
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
        gFibreOpBound (I := I) (M := M) g
          (ccTensorBilinSymm (I := I) g S) δ) :
    HighCore (I := I) (M := M) g → A1LoOp (I := I) (M := M) g :=
  lowA1LoCore (I := I) (M := M) g hρ hδ0 hδ_le hreal

theorem a1HiCore_value
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
        gFibreOpBound (I := I) (M := M) g
          (ccTensorBilinSymm (I := I) g S) δ)
    (T : SmoothCcTensor g 0 2) :
    a1HiCore (I := I) (M := M) g hρ hδ0 hδ_le hreal
        ⟨ccToHsLin (I := I) (M := M) g 2 (3 : ℝ) T, ⟨T, rfl⟩⟩ =
      (lowCoreData (I := I) (M := M) g hρ hδ0 hδ_le hreal T).a1Hi
        (I := I) (M := M) :=
  lowA1HiCore_value (I := I) (M := M) g hρ hδ0 hδ_le hreal T

theorem a1LoCore_value
    (g : SmoothRiemannianMetric I M)
    {ρ δ : ℝ} (hρ : 0 ≤ ρ) (hδ0 : 0 ≤ δ) (hδ_le : δ ≤ 1 / 3)
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
        gFibreOpBound (I := I) (M := M) g
          (ccTensorBilinSymm (I := I) g S) δ)
    (T : SmoothCcTensor g 0 2) :
    a1LoCore (I := I) (M := M) g hρ hδ0 hδ_le hreal
        ⟨ccToHsLin (I := I) (M := M) g 2 (3 : ℝ) T, ⟨T, rfl⟩⟩ =
      (lowCoreData (I := I) (M := M) g hρ hδ0 hδ_le hreal T).a1Lo
        (I := I) (M := M) :=
  lowA1LoCore_value (I := I) (M := M) g hρ hδ0 hδ_le hreal T
...
noncomputable def lowA1Hi
    ...
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
        gFibreOpBound (I := I) (M := M) g
          (ccTensorBilinSymm (I := I) g S) δ) :
    metricH3 (I := I) (M := M) g → lowA1HiOp (I := I) (M := M) g :=
  Dense.extend
    (ccToHsLin_dense (I := I) (M := M) g 2 (by positivity))
    (lowA1HiCore (I := I) (M := M) g hρ hδ0 hδ_le hreal)
...
noncomputable def lowA1Lo
    ...
    (hreal : ∀ S : SmoothCcTensor g 0 2,
      ‖ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S‖ ≤ ρ →
        gFibreOpBound (I := I) (M := M) g
          (ccTensorBilinSymm (I := I) g S) δ) :
    metricH3 (I := I) (M := M) g → lowA1LoOp (I := I) (M := M) g :=
  Dense.extend
    (ccToHsLin_dense (I := I) (M := M) g 2 (by positivity))
    (lowA1LoCore (I := I) (M := M) g hρ hδ0 hδ_le hreal)
...
theorem lowCore_split
  ...
  let S := lowRadial (I := I) (M := M) g ρ T
  let R := Real.sqrt (lowJetSq (I := I) (M := M) g 2 S)
  let B := Real.sqrt (lowJetSq (I := I) (M := M) g 3 S)
  have hR : 0 ≤ R := Real.sqrt_nonneg _
  have hB : 0 ≤ B := Real.sqrt_nonneg _
  have hR2 :
      lowJetSq (I := I) (M := M) g 2 S ≤ R ^ 2 := by
    rw [show R ^ 2 = lowJetSq (I := I) (M := M) g 2 S by
      exact Real.sq_sqrt
        (Finset.sum_nonneg fun _ _ => sq_nonneg _)]
  have hB2 :
      lowJetSq (I := I) (M := M) g 3 S ≤ B ^ 2 := by
    rw [show B ^ 2 = lowJetSq (I := I) (M := M) g 3 S by
      exact Real.sq_sqrt
        (Finset.sum_nonneg fun _ _ => sq_nonneg _)]
  have hs := hdiag S
    ...
    (zeroBound (I := I) (M := M) g hδ0)
    R B hR hB hR2 hB2
  simpa only [S, lowCoreData] using hs.1
