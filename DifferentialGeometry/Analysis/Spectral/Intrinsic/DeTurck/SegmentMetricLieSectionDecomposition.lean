...
/-- **(POSIT — the `Cf`-construction bundled with the connection-level LINEAR arm: the genuine
...
part `diff − Cf g₁ g₂` of the `g₀`-retagged Lie-summand difference `diff := lieDerivRetagG0 g₀ g_bg g₁ −
lieDerivRetagG0 g₀ g_bg g₂` has its per-order covariant gradient dominated by the **connection-level**
...
This is the irreducible genuinely-deep content of the Lie half — the gauge analogue of the curvature
half's concrete `crossSection` construction **together with** its connection-level linear-arm reduction
`ricciLinearSection_covGrad_traceReductionConn_rfns_le`, **bundled into one existence** because — unlike
the curvature half, which builds the concrete `crossSection` from the concrete operator-trace value
`ricciNeg2SectionDiffCrossEval` (determining all jets) and proves the linear-arm reduction on the
fully-determined complement — the intrinsic-vector linear/quadratic split of the *Lie deformation* is
...
concrete cross *value* to determine the section by, and the section's higher jets — on which the bound
depends — are pinned only by the linear-arm bound itself, making a split-then-bound staging impossible.
...
**Non-vacuity (the value split and the linear-arm bound are coupled, rejecting `Cf ≡ 0`).**  With
`Cf ≡ 0` the linear complement is the full `diff`, whose connection-level bound is FALSE for
`j ∈ (a, 2a]` — the top coefficient jet content of the full Lie difference is genuinely
`(∑ fixed-pair) · C⁰`-order (`L²` mass of order `j + 2 ∈ (a + 2, 2a + 2]`, which an `H^{a+2}` ball cannot
bound, only the fixed-pair *cross* arm can carry it) — so `Cf ≡ 0` is rejected and the genuine quadratic
top-jet content **must** ride on `Cf`.  A zero `Cd` is rejected (the difference arm carries the
connection-level high derivative `∇^{j+1} R`).  At `g₁ = g₂` (so `T₁ = T₂` realized) the Lie-summand
difference vanishes (`lieDerivRetagG0_sub_toModel_eq`), `R = 0` and `D = 0`, so the bound forces the linear
complement to vanish there.  NO value-bounded `Φ.op 0 2 w` shape, NO pointwise-`C^{>2}`-jet claim, NO
spectral-nonlinearity, NO Weyl dependence.  Its body is `sorry`: the genuine deep covariant-gauge value-level
content of the Lie half (the intrinsic-vector linear/quadratic split absent on disk together with the
linear arm's connection-level covariant-Leibniz `rfns` grid). -/
...
/-- **(POSIT — the connection-level CROSS arm on the `lieDerivDiff_connLevel_crossWitness` cross
...
NO Weyl dependence.  Its body is `sorry`: the genuine deep covariant-gauge-jet content of the quadratic
Cross arm at the connection level. -/
...
theorem lieDerivDiff_connLevel_sectionData (g₀ g_bg : SmoothRiemannianMetric I M) :
  ...
  -- witness the consumers extract via `.choose`); the linear arm is its `.choose_spec`, the cross arm is
  -- the companion posit stated on this very `Cf`.
