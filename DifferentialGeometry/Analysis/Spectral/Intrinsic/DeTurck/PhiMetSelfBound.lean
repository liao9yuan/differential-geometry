...
private theorem doubleTrace_grid
  ...
  | 0 =>
      ...
      simpa only [selfTraceC, if_pos] using
        (cometricTrace_rfns (I := I) (M := M) g x)
  | (i' + 1) =>
...
private theorem pureSelf_eq (g : SmoothRiemannianMetric I M) :
  ...
  apply ContMDiffSection.ext
  intro x
  rw [ricciArmPrincipalCoeffPure_toSection, cometricDoubleTraceField_toSection]
...
private theorem traceSelf_grid
  ...
  rw [DifferentialGeometry.Analysis.Spectral.rfns_iteratedCovGrad_reindexCoeffGen_eq
    (I := I) (M := M) g 4 2
...
private theorem ricciSelf_rfns_le
    (g : SmoothRiemannianMetric I M) (i : ℕ) (x : M) :
    riemannianFiberNormSq (I := I) (M := M) g 4 (2 + i) x
        ((iteratedCovGrad (I := I) g 4 2 i
  ...
  set A := ricciArmPrincipalCoeff (I := I) (M := M) g g with hA
  ...
  have hbA :
    ...
    rw [hPA, hPC, hR1]
    exact DifferentialGeometry.Analysis.Spectral.rfns_iteratedCovGrad_reindexCoeffGen_eq
  ...
  have hnegC :
      riemannianFiberNormSq (I := I) (M := M) g 4 (2 + i) x (-PC) =
        riemannianFiberNormSq (I := I) (M := M) g 4 (2 + i) x PC := by
    rw [← neg_one_smul ℝ PC, rfns_smul (I := I) (M := M)]
    norm_num
  have hsum :
      riemannianFiberNormSq (I := I) (M := M) g 4 (2 + i) x
          (PA + PB - PC) ≤
...
private theorem ricciSelf_grid
  ...
  | 0 =>
      ...
      have hricci := riemannianFiberNormSq_ricciArmPrincipalCoeffFib_le
        (I := I) (M := M) g g z htie
          (show (0 : ℝ) < 1 by norm_num) (show (0 : ℝ) ≤ 0 by norm_num) hz x
      rw [iteratedCovGrad_zero, ricciArmPrincipalCoeff_toSection]
      refine hricci.trans (le_of_eq ?_)
      simp only [selfTraceC, if_pos, sub_zero, div_one, mul_one]
      ring
  | (i' + 1) =>
      have hmain := ricciSelf_rfns_le (I := I) (M := M) g (i' + 1) x
...
theorem phiSelf_grid
  ...
  set R := (iteratedCovGrad (I := I) g 4 2 i
    (ricciArmPrincipalCoeff (I := I) (M := M) g g)).toSection x with hR
  set P := (iteratedCovGrad (I := I) g 4 2 i
    (ricciArmPrincipalCoeffPure (I := I) (M := M) g g)).toSection x with hP
  have hsec :
      (iteratedCovGrad (I := I) g 4 2 i
        (deTurckPhiMetTotal (I := I) (M := M) g g_bg g -
          ricciArmPrincipalCoeffPure (I := I) (M := M) g g)).toSection x =
  ...
  have hAbound :
      riemannianFiberNormSq (I := I) (M := M) g 4 (2 + i) x A ≤
        selfTraceC (E := E) i := by
    rw [hA, DifferentialGeometry.Analysis.Spectral.rfns_iteratedCovGrad_reindexCoeffGen_eq
      (I := I) (M := M) g 4 2
  ...
  have hBbound :
      riemannianFiberNormSq (I := I) (M := M) g 4 (2 + i) x B ≤
        selfTraceC (E := E) i := by
    rw [hB, DifferentialGeometry.Analysis.Spectral.rfns_iteratedCovGrad_reindexCoeffGen_eq
      (I := I) (M := M) g 4 2
