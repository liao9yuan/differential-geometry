import DifferentialGeometry.Analysis.Spectral.Intrinsic.DeTurck.DeTurckRemainderTameLipschitzLieCorrectionMixedInnerFibreIdentity
...
lemma lc0b_amix_middle_fiber (g₀ g₁ g_bg : SmoothRiemannianMetric I M)
    (x : M) (D : Tensor0SSpace 2 I x) :
    (show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 6 I x from
      (lc0AMixLiftedField (I := I) (M := M) g₀ g₁ g_bg).toSection x) D =
  ...
  change (show Tensor0SSpace 3 I x →L[ℝ] Tensor0SSpace 6 I x from
      ...
      ((show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 3 I x from
        (lc0AMixInnerField (I := I) (M := M) g₀ g₁).toSection x) D) = _
  rw [lc0b_amix_inner_fiber (I := I) (M := M) g₀ g₁ x D]
  rw [lc0b_KLift_fiber_33 (I := I) (M := M) g₀
    (lc0Kappa (I := I) (M := M) g₀ g₁ g_bg) x _]
