...
theorem radialA1Lo_pair
    ...
    ∃ ρ₀ : ℝ, 0 < ρ₀ ∧
      ...
      ∀ r : ℝ, ∃ K : ℝ, ∀ T U : SmoothCcTensor g 0 2,
        ...
        ‖(lowCoreData (I := I) (M := M) g
              hρ.le hδ0 hδ_le hreal T).a1Lo
                (I := I) (M := M) -
            (lowCoreData (I := I) (M := M) g
              hρ.le hδ0 hδ_le hreal U).a1Lo
                (I := I) (M := M)‖ ≤
          K * ‖ccToHsLin (I := I) (M := M) g 2 (3 : ℝ) T -
            ccToHsLin (I := I) (M := M) g 2 (3 : ℝ) U‖ := by
  obtain ⟨ρ₀, K₀, hρ₀, hK₀, hpair⟩ :=
    a1Lo_pair_lip (I := I) (M := M) hDim g
  ...
  have hcore :
      ‖(lowCoreData (I := I) (M := M) g
            hρ.le hδ0 hδ_le hreal T).a1Lo (I := I) (M := M) -
          (lowCoreData (I := I) (M := M) g
            hρ.le hδ0 hδ_le hreal U).a1Lo (I := I) (M := M)‖ ≤
        K₀ R₂ * P ^ 2 * (D₃ + D₂ + D) := by
    simpa only [lowCoreData, T₀, U₀, P, A₃] using hout
  calc
    ‖(lowCoreData (I := I) (M := M) g
          hρ.le hδ0 hδ_le hreal T).a1Lo (I := I) (M := M) -
        (lowCoreData (I := I) (M := M) g
