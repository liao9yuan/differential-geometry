...
private theorem mvPairTrace_joint
  ...
  have hCLM := contMDiffOn_clm_section_of_pointwise_jointMR (I := I) (M := M)
    ...
    (fun Y => by
      ...
      have h4 := cometricDoubleTraceFib_realizedFam_jointContMDiffOn
        (I := I) (p := 4) g T 0 hdelta hdeltaZ
        (fun p : M × Real => Y p.1) hY
      have h2 := cometricDoubleTraceFib_realizedFam_jointContMDiffOn
        (I := I) (p := 2) g T 0 hdelta hdeltaZ
        (fun p : M × Real =>
          cometricDoubleTraceFib (I := I)
            (realizedFam (I := I) g T 0 hdelta hdeltaZ p.2) 4 p.1
            (Y p.1)) h4
...
theorem edgeLiePair_joint
    ...
    linearizedRicciThreeArmHjoint (I := I) (M := M) g 2
      (edgeLiePairFam (I := I) (M := M) g T hdelta hdeltaZ q epsilon)
      (δ := delta) (δ' := delta) := by
...
private theorem ricciHalf_joint
  ...
  simpa only [edgeRicciHalf, linearizedRicciConnDiffOrder0Coeff] using hsum

private theorem ricciRefold0_joint
    (g : SmoothRiemannianMetric I M) (T : SmoothCcTensor g 0 2)
    {delta : Real}
    (hdelta : gFibreOpBound (I := I) (M := M) g
      (ccTensorBilinSymm (I := I) g T) delta)
    ...
    linearizedRicciThreeArmHjoint (I := I) (M := M) g 2
      (fun t => ricciRefold0 (I := I) (M := M) g
        (realizedFam (I := I) g T 0 hdelta hdeltaZ t) (t • T))
      (δ := delta) (δ' := delta) := by
...
private theorem covDeriv_joint
    ...
    (hdeltaZ : gFibreOpBound (I := I) (M := M) g
      (ccTensorBilinSymm (I := I) g (0 : SmoothCcTensor g 0 2)) delta) :
    linearizedRicciThreeArmHjoint (I := I) (M := M) g 2
      (fun t => deTurckLieCovDerivArmField (I := I) (M := M) g
        (realizedFam (I := I) g T 0 hdelta hdeltaZ t) g_bg)
      (δ := delta) (δ' := delta) := by
  rw [linearizedRicciThreeArmHjoint]
  exact (dLaBiContrFib_realizedFam_jointContMDiffOn
...
private theorem endo_joint
    ...
    linearizedRicciThreeArmHjoint (I := I) (M := M) g 2
      (fun t => deTurckLieEndoArmField (I := I) (M := M) g
        (realizedFam (I := I) g T 0 hdelta hdeltaZ t) g_bg)
      (δ := delta) (δ' := delta) := by
  have hall := deTurckLieCoeffField_realizedFam_jointSmooth
...
theorem rhsRefold0_joint
  ...
  have hEndo := endo_joint (I := I) (M := M)
    g g_bg T hdelta hdeltaZ
  have hCorr := lieCorr0_path_joint (I := I) (M := M)
...
def rhsRefoldTopInt
  ...
  pathIntegralCoeffField (I := I) (M := M) g 4 2
    (rhsRefoldTop (I := I) (M := M) g g_bg T hdelta hdeltaZ)
    (realizedSmallSet (δ := delta) (δ' := delta)) realizedSmallSet_isOpen
    (by
      rw [Set.uIcc_of_le zero_le_one]
      exact Icc_subset_realizedSmallSet hdelta_lt hdelta_lt)
...
theorem rhs_sub_zero_refold
    ...
    (hdeltaZ : gFibreOpBound (I := I) (M := M) g
      (ccTensorBilinSymm (I := I) g (0 : SmoothCcTensor g 0 2)) delta) :
    realizedRHSArm (I := I) g g_bg T hdelta_lt hdelta -
        realizedRHSArm (I := I) g g_bg 0 hdelta_lt hdeltaZ =
      appCc (I := I) (M := M) g 2 2
  ...
  have hc1 : ∀ x : M, ContinuousOn (fun t : Real =>
    ...
    jointContMDiff_toModel_continuous_slice (I := I) g 3 2 Psi1
      (realizedSmallSet (δ := delta) (δ' := delta)) hj1 x
  have hc2 : ∀ x : M, ContinuousOn (fun t : Real =>
    ...
    jointContMDiff_toModel_continuous_slice (I := I) g 4 2 Psi2
      (realizedSmallSet (δ := delta) (δ' := delta)) hj2 x
  have hPi0 : rhsRefold0Int (I := I) (M := M) g g_bg T
      hdelta_lt hdelta hdeltaZ =
      pathIntegralCoeffField (I := I) (M := M) g 2 2 Psi0
        (realizedSmallSet (δ := delta) (δ' := delta))
        hSopen hSI hj0 := rfl
  have hPi1 : rhsLow1PathIntegral (I := I) (M := M) g g_bg T 0
      hdelta_lt hdelta hdelta_lt hdeltaZ =
      pathIntegralCoeffField (I := I) (M := M) g 3 2 Psi1
        (realizedSmallSet (δ := delta) (δ' := delta))
        hSopen hSI hj1 := rfl
  have hPi2 : rhsRefoldTopInt (I := I) (M := M) g g_bg T
      hdelta_lt hdelta hdeltaZ =
  ...
  have hintegrand : ∀ᵐ s ∂volume, s ∈ Set.uIoc (0 : Real) 1 →
    ...
    simpa only [hPsi0def, hPsi1def, hPsi2def, unitModel_add_app] using
      rhsSlope_refold (I := I) (M := M) g g_bg T hTsymm
  ...
  rw [pathIntegralCoeffField_appCc_eq (I := I) (M := M) g 2 2 Psi0 T
      (realizedSmallSet (δ := delta) (δ' := delta)) hSopen hSI hj0 hc0,
    pathIntegralCoeffField_appCc_eq (I := I) (M := M) g 3 2 Psi1
      (iteratedCovGrad (I := I) g 0 2 1 T)
      (realizedSmallSet (δ := delta) (δ' := delta)) hSopen hSI hj1 hc1,
    pathIntegralCoeffField_appCc_eq (I := I) (M := M) g 4 2 Psi2
      (iteratedCovGrad (I := I) g 0 2 2 T)
      (realizedSmallSet (δ := delta) (δ' := delta)) hSopen hSI hj2 hc2]
