...
private theorem perturbCc_bound
  ...
  have hper : ∀ j,
    ...
    have hslot := insert3_icg_le (I := I) (M := M) hDim g j Λ
    have hbase :
        ...
        _ = ‖iteratedCovGrad (I := I) g 0 2 j
            (symmS (I := I) (M := M) g T)‖ := by
              exact perm_icg_norm (I := I) (M := M) g
                (Equiv.swap (0 : Fin 2) 1)
                (symmS (I := I) (M := M) g T) j
        _ ≤ _ := symm_icg_norm (I := I) (M := M) g T j
    refine le_trans ?_ (mul_le_mul_of_nonneg_left hbase (by norm_num))
    simpa only [perturbCoeff4, Λ] using hslot
  have hsum :
      ∑ j ∈ Finset.range 3,
          ‖iteratedCovGrad (I := I) g 4 4 j
            (perturbCoeff4 (I := I) (M := M) g T)‖ ≤
  ...
  have hsq :
      (∑ j ∈ Finset.range 3,
        ‖iteratedCovGrad (I := I) g 4 4 j
          (perturbCoeff4 (I := I) (M := M) g T)‖ ^ 2) ≤ A ^ 2 := by
    calc
      _ ≤ (∑ j ∈ Finset.range 3,
          ‖iteratedCovGrad (I := I) g 4 4 j
            (perturbCoeff4 (I := I) (M := M) g T)‖) ^ 2 :=
        Finset.sum_sq_le_sq_sum_of_nonneg
          (fun j _ => norm_nonneg _)
      _ ≤ A ^ 2 := pow_le_pow_left₀
        (Finset.sum_nonneg (fun j _ => norm_nonneg _)) hsum 2
  have hop := happ
    (perturbCoeff4 (I := I) (M := M) g T) A hA hsq
  calc
    _ ≤ Capp * A := hop
    _ = C * N := by
      dsimp only [A, C]
...
theorem perturbH2_norm
  ...
  have hdense : DenseRange
      (ccToHsLin (I := I) (M := M) g 2 (2 : ℝ)) :=
    ccToHsLin_dense (I := I) (M := M) g 2 (by positivity)
  have hop : ‖perturbH2 (I := I) (M := M) g‖ ≤ C := by
    unfold perturbH2
    apply LinearMap.opNorm_extendOfNorm_le hdense hC
    exact hbound
  calc
    _ ≤ ‖perturbH2 (I := I) (M := M) g‖ * ‖T‖ :=
      (perturbH2 (I := I) (M := M) g).le_opNorm T
    _ ≤ C * ‖T‖ :=
