...
/-- **(BRIDGE — the intrinsic covariant top/rest expansion of the sealed Lie-summand difference.)**

The chart-free, Cartan-route discharge of the sealed Lie difference's covariant top/rest split: the
leaf-shaped Top/Rest split of the `g₀`-retagged Lie-summand difference
`diff := lieDerivRetagG0 g₀ g_bg g₁ − lieDerivRetagG0 g₀ g_bg g₂`, with the **0-jet-inclusive** `w`-jet
difference arm `w := realizeSymmCcTensor g₀ (T₁ − T₂)` and the fixed-pair `(1/8)`-cross arm.
...
applied to each metric), then apply the deep gauge top/rest split
...
**Non-vacuity.**  Inherited from P1b (the `Top` part carries the high derivative `∇^{j+2} w` and the
order-`0` value jet, the `Rest` part both fixed-pair endpoints).  At `g₁ = g₂` the difference vanishes
and the split is `0 = 0 + 0`.  NO value-bounded `Φ.op 0 2 w` shape, NO pointwise-`C^{>2}`-jet claim, NO
...
theorem lieDerivDiff_intrinsic_covFdB_section_expansion (g₀ g_bg : SmoothRiemannianMetric I M)
  ...
  refine ⟨Cd, hCd0, fun T₁ T₂ g₁ g₂ hr1 hr2 hfib1 hfib2 hball1 hball2 j x => ?_⟩
  -- Rewrite the sealed Lie difference into the symmetrised-lowered DeTurck-field difference (P1a
  -- applied to each metric), then apply the deep gauge top/rest split P1b.
  rw [lieDerivRetagG0_sub_eq_symLoweredRetagG0_sub (I := I) g₀ g_bg g₁ g₂]
  exact hsplit T₁ T₂ g₁ g₂ hr1 hr2 hfib1 hfib2 hball1 hball2 j x
