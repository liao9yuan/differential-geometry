...
private noncomputable def hessianH1
    (g : SmoothRiemannianMetric I M) :
...
private noncomputable def traceH1
    (g : SmoothRiemannianMetric I M) :
    rank4H1 (I := I) (M := M) g →L[ℝ]
      rank2H1 (I := I) (M := M) g :=
...
private theorem hessianH1_core
  ...
  rw [hessianH1, ContinuousLinearMap.comp_apply, hcast]
...
private theorem insert3_icg_le
...
private theorem pcoeff_icg_le
  ...
  have hslot := insert3_icg_le (I := I) (M := M) hDim g i Λ
  have hbase :
      ‖iteratedCovGrad (I := I) g 1 1 i
          (slotInsertEndoCc (I := I) (M := M) g 0 Λ)‖ ≤
      ...
      _ = ‖iteratedCovGrad (I := I) g 0 2 i
          (domDomCongrSection (I := I) g
            ...
            simpa only [Nat.zero_add, Nat.reduceAdd] using
              raise_icg_norm (I := I) (M := M) g 0 i
                (domDomCongrSection (I := I) g
                  (Equiv.swap (0 : Fin 2) 1)
                  (symmS (I := I) (M := M) g T))
      _ = ‖iteratedCovGrad (I := I) g 0 2 i
          (symmS (I := I) (M := M) g T)‖ :=
        perm_icg_norm (I := I) (M := M) g
          (Equiv.swap (0 : Fin 2) 1)
          (symmS (I := I) (M := M) g T) i
      _ ≤ _ := symm_icg_norm (I := I) (M := M) g T i
  exact hslot.trans
...
private theorem pcoeff1_bound
  ...
  obtain ⟨Cmul, hCmul, hmul⟩ :=
    appRS_h2_h1_h1 (I := I) (M := M) hDim g 0 4 4
  ...
  let A : ℝ := 27 * CinT * NT
  let B : ℝ := CinW * NW
  let Φ := perturbCoeff4 (I := I) (M := M) g T
  let Y : SmoothCcTensor g 0 4 :=
    appCc (I := I) (M := M) g 4 4 Φ W
    ...
    calc
      _ ≤ ∑ j ∈ Finset.range 3,
        ...
        Finset.sum_le_sum fun j _ =>
          pcoeff_icg_le (I := I) (M := M) hDim g T j
...
private theorem principalLo_norm
    ...
    ∃ ρ C : ℝ, 0 < ρ ∧ 0 ≤ C ∧
      ∀ T : metricH2 (I := I) (M := M) g, ‖T‖ ≤ ρ →
        ‖lowRegPrincipalLo (I := I) (M := M) g T‖ ≤ C * ‖T‖ := by
  obtain ⟨ρ, Cinv, hρ, hCinv, hinv⟩ :=
    invPerturbH1_norm (I := I) (M := M) hDim g
  let A := traceH1 (I := I) (M := M) g
  let D := hessianH1 (I := I) (M := M) g
  let C : ℝ := ‖A‖ * Cinv * ‖D‖
  have hC : 0 ≤ C := by
    dsimp only [C]
    positivity
  ...
  calc
    ‖lowRegPrincipalLo (I := I) (M := M) g T‖ ≤
        ‖A‖ * ‖(invPerturbH1 (I := I) (M := M) g T).comp D‖ := by
      simpa only [lowRegPrincipalLo, A, D] using
        (A.opNorm_comp_le
...
private theorem hessian_comm
  ...
  have hfun := hdense.equalizer
    ((inc421 (I := I) (M := M) g).comp
      (hessianH2 (I := I) (M := M) g)).continuous
    ((hessianH1 (I := I) (M := M) g).comp
    ...
    have hfun := hdenseT.equalizer L.continuous R.continuous (by
      ...
      have hV := hdenseV.equalizer
        ...
        (R (ccTensorToHs (I := I) (M := M) g 2 (2 : ℝ) S₀)).continuous (by
          ...
          rw [perturbH2_core (I := I) (M := M) hDim g S₀,
...
private theorem inverse_comm
  ...
  simp only [invPerturbH2, invPerturbH1,
...
private theorem principal_comm_small
    ...
    (inc21 (I := I) (M := M) g).comp
      ...
      (lowRegPrincipalLo (I := I) (M := M) g T).comp
        (inc43 (I := I) (M := M) g) := by
  apply ContinuousLinearMap.ext
  intro U
  let X := hessianH2 (I := I) (M := M) g U
  have htrace := congrArg (fun Q => Q
      (invPerturbH2 (I := I) (M := M) g T X))
  ...
  simp only [lowRegPrincipal, lowRegPrincipalLo,
    ContinuousLinearMap.comp_apply] at htrace hinv hhess ⊢
  calc
    _ = traceH1 (I := I) (M := M) g
        (inc421 (I := I) (M := M) g
          (invPerturbH2 (I := I) (M := M) g T X)) := htrace
    _ = traceH1 (I := I) (M := M) g
        (invPerturbH1 (I := I) (M := M) g T
...
theorem principal_comm
  ...
  have h2 : ‖perturbH2 (I := I) (M := M) g T‖ < 1 :=
...
theorem principal_pair_norm
    ...
    ∃ ρ C₄₂ C₃₁ : ℝ,
      0 < ρ ∧ 0 ≤ C₄₂ ∧ 0 ≤ C₃₁ ∧
      ∀ T : metricH2 (I := I) (M := M) g, ‖T‖ ≤ ρ →
        ‖lowRegPrincipal (I := I) (M := M) g T‖ ≤ C₄₂ * ‖T‖ ∧
          ‖lowRegPrincipalLo (I := I) (M := M) g T‖ ≤ C₃₁ * ‖T‖ := by
