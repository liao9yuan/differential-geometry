...
/-- **The tensor-uniform `EuclN` bare chart-jet-content Stage-4 bound.**

The `D`-uniform hoist of `DeTurckCoefficients.bareChartJetContent_le_sqrt_fiberNormSq_sum`: a single
constant `C` (outside the `∀ D`) such that for **every** smooth compactly-supported `(0,2)`-tensor `D`,
on the POU kernel, `bareChartJetContent g 0 2 D α N y ≤ C · ∑_{i ≤ N} √rfns(∇^i D)`.  The underlying
reverse order-peeling (`iteratedFDeriv_rawPullR_le_zeroContent_sum`) and reverse fibre bound
(`exists_zeroContentR_le_fiberNorm_on_pouKernel`) already quantify the tensor inside the `∃ C`, so the
constant is genuinely `D`-uniform; this lemma merely re-states it with the `D`-quantifier inside. -/
...
/-- **(The ball-uniform bare chart-jet content domination of the Ricci–DeTurck RHS-arm difference —
PROVED from the ball-uniform chart-jet Nemytskii base towers.)**

The ball-uniform hoist of `bareChartJetContent_deTurckRHSArm_le_sqrt_fiberNormSq_sum`: there is a single
nonnegative constant `K` — uniform over the fibre-small radius-`R` covariant ball, **outside** the
`∀ T T'` quantifier — such that for any two `g₀`-fibre-small perturbations `T, T'` whose covariant-`L²`
jets up to order `a + 2` lie in the radius-`R` ball, on the closed support of the chart-`α` POU weight,
the order-`a` bare chart-jet content of the RHS-arm residual is dominated by `K · ∑_{q ≤ a+2}
√rfns(∇^q (T − T'))(b)`.

The only `(T, T')`-dependent factor of the per-pair source is the chart-Nemytskii Lipschitz modulus,
which is hoisted to a single ball-uniform `CNemMax` via
`chartDeTurckRicciRHS_realize_seminorm_le_bareChartJetContentOnE_ballUniform` (supremised over the `n²`
chart index pairs); the forward Euclidean-coordinate bridge factor `eFac` and the `g₀`-anchored Stage-4
content constant `Cstage` are `(T, T')`-independent.  Consumers transitively depend on the posited
ball-uniform base towers' `sorryAx`. -/
...
private theorem deTurckRHSArmDiff_iteratedCovGrad_rawComponent_domination_on_pouTsupport_ballUniform
  ...
  obtain ⟨K, hK_nn, hK⟩ :=
    bareChartJetContent_deTurckRHSArm_le_sqrt_fiberNormSq_sum_ballUniform
      (I := I) (M := M) g₀ g_bg a (R := R) α
  refine ⟨Cpeel * K, by positivity, ?_⟩
  intro T T' δ hδ_lt hδ δ' hδ'_lt hδ' hTball hT'ball b hb Jdx
  set RHSarm : SmoothCcTensor g₀ 0 2 :=
    (deTurckSmoothRemainder (I := I) g₀ g_bg T hδ_lt hδ -
        deTurckSmoothRemainder (I := I) g₀ g_bg T' hδ'_lt hδ') +
      rawTensorConnLapSmooth (I := I) g₀ 0 2 (T - T') with hRHSarm_def
  ...
  have hcontent : bareChartJetContent (I := I) (M := M) g₀ 0 2 RHSarm α a yb ≤ K * Ssqrt := by
    ...
    exact hK T T' hδ_lt hδ hδ'_lt hδ' hTball hT'ball hb
  -- The chart component value at `b` is the order-`0` `rawPullR` jet at the kernel point.
  have hval : DifferentialGeometry.Analysis.Parabolic.TensorSpectral.tensorChartComponentRaw
        (I := I) (M := M) g₀ 0 (2 + a)
        (iteratedCovGrad (I := I) g₀ 0 2 a RHSarm) α (![] : Fin 0 → Fin n) Jdx b =
...
/-- **The ball-uniform value-level chart-jet Nemytskii bound of the realized Ricci–DeTurck carrier
difference, with the embedding window DECOUPLED from the output order.**

This is the order-`0` (output-value) analog of
`chartDeTurckRicciRHS_realize_seminorm_le_bareChartJetContentOnE_ballUniform`, but with the underlying
ball-uniformity window `jmax := a + 2` carried as a SEPARATE parameter rather than pinned to
`(output order) + 2`.  Fix `g₀`, the DeTurck background `g_bg`, a chart base point `α`, a compact
chart-target neighbourhood `K`, an index pair `i k`, and a ball order `a`.  There is **one** nonnegative
constant `C` — outside `∀ T T'` — such that for any two `g₀`-fibre-small perturbations `T, T'` whose
covariant-`L²` jets up to order `a + 2` lie in the radius-`R` ball, the chart Fréchet **value** (order
`0`) of the chart Ricci–DeTurck carrier difference of the realized metrics is dominated by the order-`2`
`E`-coordinate bare chart-jet content of the perturbation difference `T − T'`:
```
‖chartDeTurckRicciRHS (g₀+T) g_bg α i k y − chartDeTurckRicciRHS (g₀+T') g_bg α i k y‖
  ≤ C · bareChartJetContentOnE g₀ (T − T') α 2 y .
```

It is assembled exactly like the named wrapper — `HasChartJetLipBall.seminorm_le 0` of the proved
`hasChartJetLipBall_chartDeTurckRicciRHS` (which is FREE in the ball window, so here it is run with
`jmax := a + 2`) composed with the structural chart-Gram realize-difference bridge
`chartGramJetDiffSeminormSum_realize_le_bareChartJetContentOnE` at order `0 + 2 = 2` — but the ball
membership is established with the radius-`R` ball at window `a + 2` (so the value-level read at a
SUPERCRITICAL `jmax := a + 2 ≥ 2·finrank E + 12` keeps the `H^{a+2} ↪ C²` embedding of the sorry'd base
towers in its valid regime, unlike the value-local order-`0` instantiation of the order-`a` sibling
which would pin `jmax := 2`).  Consumers transitively depend on the posited base towers' `sorryAx`. -/
private theorem chartDeTurckRicciRHS_realize_value_le_bareChartJetContentOnE_ballUniform_window
    (g₀ g_bg : SmoothRiemannianMetric I M) {R : ℝ} (α : M)
    {K : Set E} (hK : IsCompact K)
    (hKsub : K ⊆ interior (extChartAt I α).target)
    (i k : Fin (Module.finrank ℝ E)) (a : ℕ) :
    ∃ C : ℝ, 0 ≤ C ∧ ∀ (T T' : SmoothCcTensor g₀ 0 2)
        ...
        ∀ y ∈ K,
        ‖DeTurckCoefficients.chartDeTurckRicciRHS (I := I)
            (tensorSectionRealizeMetric (I := I) g₀ T hδ_lt hδ) g_bg α i k y -
          DeTurckCoefficients.chartDeTurckRicciRHS (I := I)
            (tensorSectionRealizeMetric (I := I) g₀ T' hδ'_lt hδ') g_bg α i k y‖ ≤
          C * DeTurckCoefficients.bareChartJetContentOnE (I := I) (M := M) g₀ (T - T') α 2 y := by
  classical
  -- The order-`0` ball-uniform chart-jet Lipschitz seminorm bound, run with the SUPERCRITICAL
  ...
  have hg₁mem : tensorSectionRealizeMetric (I := I) g₀ T hδ_lt hδ ∈
      DeTurckCoefficients.realizedFibreSmallBall (I := I) (M := M) g₀ R (a + 2) :=
    DeTurckCoefficients.tensorSectionRealizeMetric_mem_realizedFibreSmallBall
      (I := I) (M := M) g₀ T hδ_lt hδ hTball
  have hg₂mem : tensorSectionRealizeMetric (I := I) g₀ T' hδ'_lt hδ' ∈
      DeTurckCoefficients.realizedFibreSmallBall (I := I) (M := M) g₀ R (a + 2) :=
    DeTurckCoefficients.tensorSectionRealizeMetric_mem_realizedFibreSmallBall
      (I := I) (M := M) g₀ T' hδ'_lt hδ' hT'ball
  -- The order-`0` seminorm at `y` is the value norm.
  have hseminorm := hC _ hg₁mem _ hg₂mem y hy
  have hval_eq :
      DeTurckCoefficients.iteratedFDerivSeminorm 0
          (fun z => DeTurckCoefficients.chartDeTurckRicciRHS (I := I)
        ...
        ‖DeTurckCoefficients.chartDeTurckRicciRHS (I := I)
            (tensorSectionRealizeMetric (I := I) g₀ T hδ_lt hδ) g_bg α i k y -
          DeTurckCoefficients.chartDeTurckRicciRHS (I := I)
            (tensorSectionRealizeMetric (I := I) g₀ T' hδ'_lt hδ') g_bg α i k y‖ := by
    rw [DeTurckCoefficients.iteratedFDerivSeminorm, Finset.sum_range_one,
      norm_iteratedFDerivWithin_zero]
  rw [hval_eq] at hseminorm
  -- The structural chart-Gram realize-difference bridge at order `0 + 2 = 2`.
  have hgram := DeTurckCoefficients.chartGramJetDiffSeminormSum_realize_le_bareChartJetContentOnE
    (I := I) (M := M) g₀ T T' hδ_lt hδ hδ'_lt hδ' α (0 + 2) hyint
  refine hseminorm.trans ?_
  calc C * DeTurckCoefficients.chartGramJetDiffSeminormSum (I := I) (M := M) (0 + 2)
        (tensorSectionRealizeMetric (I := I) g₀ T hδ_lt hδ)
        (tensorSectionRealizeMetric (I := I) g₀ T' hδ'_lt hδ') α
        (interior (extChartAt I α).target) y
      ≤ C * (((Module.finrank ℝ E) : ℝ) *
          DeTurckCoefficients.bareChartJetContentOnE (I := I) (M := M) g₀ (T - T') α (0 + 2) y) :=
        mul_le_mul_of_nonneg_left hgram hC_pos.le
    _ = (C * ((Module.finrank ℝ E) : ℝ)) *
          DeTurckCoefficients.bareChartJetContentOnE (I := I) (M := M) g₀ (T - T') α 2 y := by
        norm_num; ring

set_option linter.unusedVariables false in
/-- **(The order-`0` ball-uniform Faà-di-Bruno pointwise domination of the nonlinear
Ricci–DeTurck RHS-arm difference by the order-`≤ 2` covariant-jet column of `T − T'`, PROVED from the
supercritical-window chart-jet Nemytskii bound.)**
...
**Assembly.**  Per chart `α` of the finite atlas: the reverse fibre-norm/raw-component bridge
`riemannianFiberNormSq_le_raw_components_on_pouTsupport` (at valence `(0,2)`) reduces `rfns(N)(x)` to the
squared order-`0` raw chart components of `N`; each such component equals the chart Ricci–DeTurck carrier
difference `chartDeTurckRicciRHS (g₀+T) − chartDeTurckRicciRHS (g₀+T')` (the Δ-arms cancel, identity
`tensorChartComponentRaw_deTurckRHSArm_eq_chartDeTurckRicciRHS_diff`); the value-level supercritical-window
ball-uniform chart-jet Nemytskii bound
`chartDeTurckRicciRHS_realize_value_le_bareChartJetContentOnE_ballUniform_window` (run with the embedding
window `jmax := a + 2` SUPERCRITICAL under `ha_super`, decoupled from the output order `0`) bounds that
value by `C · bareChartJetContentOnE g₀ (T − T') α 2`; the content-to-fibre-norm conversion
`bareChartJetContentOnE_le_sqrt_fiberNormSq_sum_uniform` (at order `2`) turns it into
`∑_{q ≤ 2} √rfns(∇^q (T − T'))`, and a Cauchy–Schwarz square (`3` terms) yields the squared column
`∑_{q ≤ 2} rfns(∇^q (T − T'))`.  The atlas-maximum of the finitely many per-chart constants is the single
ball-uniform `Λp`.  Consumers transitively depend on the posited ball-uniform base towers' `sorryAx`.

`ha_super` is the analytic-validity gate: it is what makes the supercritical embedding window
`jmax := a + 2 ≥ 2·finrank E + 12 > finrank E / 2 + 2` (the `H^{a+2} ↪ C²` section embedding) genuine,
so the sorry'd ball-uniform chart-Gram / inverse-Gram base towers are in their valid regime when run at
window `a + 2`.  The Lean assembly routes the supercriticality THROUGH those `jmax`-free base towers, so
`ha_super` is not mechanically consumed by a proof step (the `unusedVariables` linter is locally disabled
for this declaration); it remains a genuine, load-bearing hypothesis of the statement. -/
private theorem deTurckRHSArmDiff_order0_pointwise_domination_ballUniform
  ...
  classical
  set n : ℕ := Module.finrank ℝ E with hn_def
  -- Per chart `α` of the finite atlas, a single ball-uniform constant `Cα` such that
  -- `rfns(N)(x) ≤ Cα · ∑_{q ≤ 2} rfns(∇^q(T−T'))(x)` on the closed POU support of `α`.
  have hperChart : ∀ α : M, ∃ Cα : ℝ, 0 ≤ Cα ∧
      ∀ (T T' : SmoothCcTensor g₀ 0 2)
        {δ : ℝ} (hδ_lt : δ < 1)
        (hδ : gFibreOpBound (I := I) (M := M) g₀ (ccTensorBilinSymm (I := I) g₀ T) δ)
        {δ' : ℝ} (hδ'_lt : δ' < 1)
        ...
        ∀ {b : M},
          b ∈ tsupport (fun x : M =>
              ((chartAtlasPOU I M α : C^∞⟮I, M; ℝ⟯) : M → ℝ) x) →
          riemannianFiberNormSq (I := I) (M := M) g₀ 0 2 b
              ((deTurckRHSArmG0 (I := I) g₀ g_bg T hδ_lt hδ -
                  deTurckRHSArmG0 (I := I) g₀ g_bg T' hδ'_lt hδ').toSection b) ≤
            Cα * ∑ q ∈ Finset.range 3,
              riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + q) b
                ((iteratedCovGrad (I := I) g₀ 0 2 q (T - T')).toSection b) := by
    intro α
    -- The fixed compact chart-target neighbourhood (the chart image of the closed POU support).
    set Kc : Set E := (extChartAt I α) '' (tsupport (fun x : M =>
        ((chartAtlasPOU I M α : C^∞⟮I, M; ℝ⟯) : M → ℝ) x)) with hKc_def
    have hKc_compact : IsCompact Kc :=
      DifferentialGeometry.Analysis.Parabolic.TensorSpectral.chartImage_pouTsupport_isCompact
    ...
    have hKc_sub : Kc ⊆ interior ((extChartAt I α).target : Set E) :=
      DifferentialGeometry.Analysis.Parabolic.TensorSpectral.chartImage_pouTsupport_subset_interior_target
        (I := I) (M := M) α
    -- The reverse fibre-norm/raw-component bridge at valence `(0,2)` (uniform over all sections).
    obtain ⟨Cbr, hCbr_nn, hCbr⟩ :=
      riemannianFiberNormSq_le_raw_components_on_pouTsupport (I := I) (M := M) g₀ 0 2 α
    -- The value-level supercritical-window ball-uniform chart-jet Nemytskii bounds, one per index pair.
    have hNem : ∀ ik : Fin n × Fin n, ∃ C : ℝ, 0 ≤ C ∧
        ∀ (T T' : SmoothCcTensor g₀ 0 2)
          {δ : ℝ} (hδ_lt : δ < 1)
          (hδ : gFibreOpBound (I := I) (M := M) g₀ (ccTensorBilinSymm (I := I) g₀ T) δ)
          ...
          ∀ y ∈ Kc,
          ‖DeTurckCoefficients.chartDeTurckRicciRHS (I := I)
              (tensorSectionRealizeMetric (I := I) g₀ T hδ_lt hδ) g_bg α ik.1 ik.2 y -
            DeTurckCoefficients.chartDeTurckRicciRHS (I := I)
              (tensorSectionRealizeMetric (I := I) g₀ T' hδ'_lt hδ') g_bg α ik.1 ik.2 y‖ ≤
            C * DeTurckCoefficients.bareChartJetContentOnE (I := I) (M := M) g₀ (T - T') α 2 y := by
      intro ik
      obtain ⟨C, hC_nn, hC⟩ :=
        chartDeTurckRicciRHS_realize_value_le_bareChartJetContentOnE_ballUniform_window
          (I := I) (M := M) g₀ g_bg (R := R) α hKc_compact hKc_sub ik.1 ik.2 a
      exact ⟨C, hC_nn, fun T T' δ hδ_lt hδ δ' hδ'_lt hδ' hTball hT'ball y hy =>
        hC T T' hδ_lt hδ hδ'_lt hδ' hTball hT'ball y hy⟩
    choose CNem hCNem_nn hCNem using hNem
    ...
    have hCNemMax_nn : 0 ≤ CNemMax :=
      le_trans (hCNem_nn (Classical.arbitrary _)) (Finset.le_sup' CNem (Finset.mem_univ _))
    -- The `(T,T')`-uniform content-to-fibre-norm conversion at order `2`.
    obtain ⟨Cstage, hCstage_nn, hCstage⟩ :=
      bareChartJetContentOnE_le_sqrt_fiberNormSq_sum_uniform (I := I) (M := M) g₀ α 2
    -- The single per-chart constant: a product of the bridge, Nemytskii, content, multi-index and
    -- Cauchy–Schwarz factors.
    refine ⟨Cbr * ((n : ℝ) ^ 2 * (CNemMax * Cstage) ^ 2 * 3), by positivity, ?_⟩
    intro T T' δ hδ_lt hδ δ' hδ'_lt hδ' hTball hT'ball b hb
    set N : SmoothCcTensor g₀ 0 2 :=
      deTurckRHSArmG0 (I := I) g₀ g_bg T hδ_lt hδ -
        deTurckRHSArmG0 (I := I) g₀ g_bg T' hδ'_lt hδ' with hN_def
    set Scol : ℝ := ∑ q ∈ Finset.range 3,
      riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + q) b
        ((iteratedCovGrad (I := I) g₀ 0 2 q (T - T')).toSection b) with hScol_def
    have hScol_nn : 0 ≤ Scol :=
      Finset.sum_nonneg fun q _ => riemannianFiberNormSq_nonneg (I := I) (M := M) g₀ 0 (2 + q) b _
    -- Set up the chart points.
    have hb_src : b ∈ (extChartAt I α).source := by
      rw [extChartAt_source]
      exact DifferentialGeometry.Integral.Measure.chartAtlasPOU_isSubordinate I M α hb
    set yb : E := extChartAt I α b with hyb_def
    ...
    have hb_good : b ∈ DifferentialGeometry.Integral.Connection.chartLeviCivitaGoodSet (I := I) α := by
      rw [DifferentialGeometry.Analysis.Parabolic.TensorSpectral.chartLeviCivitaGoodSet_eq_extChartAt_source
        (I := I) α]
      exact hb_src
    -- `N.toSection = RHSarm.toSection`, so the raw chart components agree, hence read off the textbook
    -- chart Ricci–DeTurck carrier difference.
    have hNsec : ∀ Jdx : Fin 2 → Fin n,
        DifferentialGeometry.Analysis.Parabolic.TensorSpectral.tensorChartComponentRaw
            (I := I) (M := M) g₀ 0 2 N α (![] : Fin 0 → Fin n) Jdx b =
          DeTurckCoefficients.chartDeTurckRicciRHS (I := I)
              (tensorSectionRealizeMetric (I := I) g₀ T hδ_lt hδ) g_bg α (Jdx 0) (Jdx 1) yb -
            DeTurckCoefficients.chartDeTurckRicciRHS (I := I)
              (tensorSectionRealizeMetric (I := I) g₀ T' hδ'_lt hδ') g_bg α (Jdx 0) (Jdx 1) yb := by
      intro Jdx
      set RHSarm : SmoothCcTensor g₀ 0 2 :=
        (deTurckSmoothRemainder (I := I) g₀ g_bg T hδ_lt hδ -
            deTurckSmoothRemainder (I := I) g₀ g_bg T' hδ'_lt hδ') +
          rawTensorConnLapSmooth (I := I) g₀ 0 2 (T - T') with hRHSarm_def
      have hsec_eq : N.toSection b = RHSarm.toSection b := by
        rw [hRHSarm_def, deTurckRHSArm_toSection_eq (I := I) g₀ g_bg T T' hδ_lt hδ hδ'_lt hδ', hN_def]
        rfl
      rw [tensorChartComponentRaw_toSection_congr (I := I) (M := M) g₀ g₀ 0 2 N RHSarm α
        (![] : Fin 0 → Fin n) Jdx b hsec_eq]
      rw [hyb_def]
      exact tensorChartComponentRaw_deTurckRHSArm_eq_chartDeTurckRicciRHS_diff
        (I := I) g₀ g_bg T T' hδ_lt hδ hδ'_lt hδ' α hb_good Jdx
    -- The content-to-fibre-norm conversion at `b` (order `2`).
    have hcontent : DeTurckCoefficients.bareChartJetContentOnE (I := I) (M := M) g₀ (T - T') α 2 yb ≤
        Cstage * ∑ q ∈ Finset.range 3,
          Real.sqrt (riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + q) b
            ((iteratedCovGrad (I := I) g₀ 0 2 q (T - T')).toSection b)) := by
      rw [hyb_def]
      simpa using hCstage (T - T') hb
    set Ssqrt : ℝ := ∑ q ∈ Finset.range 3,
      Real.sqrt (riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + q) b
        ((iteratedCovGrad (I := I) g₀ 0 2 q (T - T')).toSection b)) with hSsqrt_def
    have hSsqrt_nn : 0 ≤ Ssqrt := Finset.sum_nonneg fun q _ => Real.sqrt_nonneg _
    -- Per `Jdx`: the squared chart component is `≤ (CNemMax · Cstage)² · Ssqrt²`.
    have hperJdxSq : ∀ Jdx : Fin 2 → Fin n,
        (DifferentialGeometry.Analysis.Parabolic.TensorSpectral.tensorChartComponentRaw
            (I := I) (M := M) g₀ 0 2 N α (![] : Fin 0 → Fin n) Jdx b) ^ 2 ≤
          (CNemMax * Cstage) ^ 2 * Ssqrt ^ 2 := by
      intro Jdx
      have hval := hNsec Jdx
      have hNem' := hCNem (Jdx 0, Jdx 1) T T' hδ_lt hδ hδ'_lt hδ' hTball hT'ball yb hyb_Kc
      -- `|component| ≤ CNemMax · Cstage · Ssqrt`.
      have habs : |DifferentialGeometry.Analysis.Parabolic.TensorSpectral.tensorChartComponentRaw
            (I := I) (M := M) g₀ 0 2 N α (![] : Fin 0 → Fin n) Jdx b| ≤
          (CNemMax * Cstage) * Ssqrt := by
        rw [hval, ← Real.norm_eq_abs]
        refine hNem'.trans ?_
        have hCNem_le : CNem (Jdx 0, Jdx 1) ≤ CNemMax :=
          Finset.le_sup' CNem (Finset.mem_univ _)
        calc CNem (Jdx 0, Jdx 1) *
              DeTurckCoefficients.bareChartJetContentOnE (I := I) (M := M) g₀ (T - T') α 2 yb
            ≤ CNem (Jdx 0, Jdx 1) * (Cstage * Ssqrt) := by
              refine mul_le_mul_of_nonneg_left ?_ (hCNem_nn _)
              rw [hSsqrt_def]; exact hcontent
          _ ≤ CNemMax * (Cstage * Ssqrt) := by
              refine mul_le_mul_of_nonneg_right hCNem_le ?_
              exact mul_nonneg hCstage_nn hSsqrt_nn
          _ = (CNemMax * Cstage) * Ssqrt := by ring
      calc (DifferentialGeometry.Analysis.Parabolic.TensorSpectral.tensorChartComponentRaw
              (I := I) (M := M) g₀ 0 2 N α (![] : Fin 0 → Fin n) Jdx b) ^ 2
          = |DifferentialGeometry.Analysis.Parabolic.TensorSpectral.tensorChartComponentRaw
              (I := I) (M := M) g₀ 0 2 N α (![] : Fin 0 → Fin n) Jdx b| ^ 2 := (sq_abs _).symm
        _ ≤ ((CNemMax * Cstage) * Ssqrt) ^ 2 :=
            pow_le_pow_left₀ (abs_nonneg _) habs 2
        _ = (CNemMax * Cstage) ^ 2 * Ssqrt ^ 2 := by ring
    -- Cauchy–Schwarz: `Ssqrt² ≤ 3 · Scol`.
    have hSsqrt_sq : Ssqrt ^ 2 ≤ 3 * Scol := by
      rw [hSsqrt_def, hScol_def]
      have hcheb := sq_sum_le_card_mul_sum_sq (s := Finset.range 3)
        (f := fun q => Real.sqrt (riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + q) b
          ((iteratedCovGrad (I := I) g₀ 0 2 q (T - T')).toSection b)))
      rw [Finset.card_range] at hcheb
      refine hcheb.trans (le_of_eq ?_)
      have hsumeq : (∑ i ∈ Finset.range 3,
            Real.sqrt (riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + i) b
              ((iteratedCovGrad (I := I) g₀ 0 2 i (T - T')).toSection b)) ^ 2) =
          ∑ q ∈ Finset.range 3,
            riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + q) b
              ((iteratedCovGrad (I := I) g₀ 0 2 q (T - T')).toSection b) :=
        Finset.sum_congr rfl (fun q _ =>
          Real.sq_sqrt (riemannianFiberNormSq_nonneg (I := I) (M := M) g₀ 0 (2 + q) b _))
      rw [hsumeq]; norm_num
    -- The reverse fibre-norm bridge at `b`, applied to `N`.
    have hbridge := hCbr N hb
    -- Assemble: bound the raw-component double sum by the per-`Jdx` squared bound, then majorise.
    calc riemannianFiberNormSq (I := I) (M := M) g₀ 0 2 b (N.toSection b)
        ≤ Cbr * (∑ Idx : Fin 0 → Fin n, ∑ Jdx : Fin 2 → Fin n,
            (DifferentialGeometry.Analysis.Parabolic.TensorSpectral.tensorChartComponentRaw
              (I := I) (M := M) g₀ 0 2 N α Idx Jdx b) ^ 2) := hbridge
      _ ≤ Cbr * (((n : ℝ) ^ 2) * ((CNemMax * Cstage) ^ 2 * Ssqrt ^ 2)) := by
          refine mul_le_mul_of_nonneg_left ?_ hCbr_nn
          calc (∑ Idx : Fin 0 → Fin n, ∑ Jdx : Fin 2 → Fin n,
                (DifferentialGeometry.Analysis.Parabolic.TensorSpectral.tensorChartComponentRaw
                  (I := I) (M := M) g₀ 0 2 N α Idx Jdx b) ^ 2)
              ≤ ∑ _Idx : Fin 0 → Fin n, ∑ _Jdx : Fin 2 → Fin n,
                  ((CNemMax * Cstage) ^ 2 * Ssqrt ^ 2) := by
                refine Finset.sum_le_sum (fun Idx _ => Finset.sum_le_sum (fun Jdx _ => ?_))
                rw [Subsingleton.elim Idx (![] : Fin 0 → Fin n)]
                exact hperJdxSq Jdx
            _ = ((n : ℝ) ^ 2) * ((CNemMax * Cstage) ^ 2 * Ssqrt ^ 2) := by
                rw [Finset.sum_const, Finset.sum_const, Finset.card_univ, Finset.card_univ,
                  nsmul_eq_mul, nsmul_eq_mul, ← mul_assoc]
                have hcard : ((Fintype.card (Fin 0 → Fin n) : ℝ) *
                    (Fintype.card (Fin 2 → Fin n) : ℝ)) = (n : ℝ) ^ 2 := by
                  simp only [Fintype.card_fun, Fintype.card_fin, pow_zero]
                  push_cast; ring
                rw [hcard]
      _ ≤ Cbr * (((n : ℝ) ^ 2) * ((CNemMax * Cstage) ^ 2 * (3 * Scol))) := by
          refine mul_le_mul_of_nonneg_left ?_ hCbr_nn
          refine mul_le_mul_of_nonneg_left ?_ (by positivity)
          exact mul_le_mul_of_nonneg_left hSsqrt_sq (by positivity)
      _ = (Cbr * ((n : ℝ) ^ 2 * (CNemMax * Cstage) ^ 2 * 3)) * Scol := by ring
  -- Glue over the finite chart atlas: the maximum of the per-chart constants is one global constant.
  choose! Cα hCα_nn hCα using hperChart
  set Ksum : ℝ := ∑ α ∈ chartAtlasPOU_finset (I := I) (M := M), Cα α with hKsum_def
  have hKsum_nn : 0 ≤ Ksum := Finset.sum_nonneg (fun α _ => hCα_nn α)
...
/-- **Front-commuting one covariant gradient through the iterated gradient, in the intrinsic squared
fibre norm.**  The `g`-Riemannian squared fibre norm of `∇^m (∇X)` at `x` equals that of `∇^{m+1}X`.
Derived from the public order-composition identity `rfns_iteratedCovGrad_comp` (with `∇ = ∇^1`, which is
`iteratedCovGrad … 1` definitionally), reindexing the order `1 + m` to `m + 1`. -/
private lemma rfns_iteratedCovGrad_covGrad_comm_loc {g : SmoothRiemannianMetric I M} (r s m : ℕ)
    (X : SmoothCcTensor g r s) (x : M) :
    riemannianFiberNormSq (I := I) (M := M) g r ((s + 1) + m) x
        ((iteratedCovGrad (I := I) g r (s + 1) m (covGrad g r s X)).toSection x) =
      riemannianFiberNormSq (I := I) (M := M) g r (s + (m + 1)) x
        ((iteratedCovGrad (I := I) g r s (m + 1) X).toSection x) := by
  have hcomp := rfns_iteratedCovGrad_comp (I := I) (M := M) g r s 1 m X x
  have he : (1 + m : ℕ) = m + 1 := by omega
  exact he ▸ hcomp

set_option linter.unusedSectionVars false in
/-- **The iterated-gradient intrinsic squared fibre norm is invariant under the rank-cast
`castRankCc`.**  Proved by `subst` on the rank equality, which collapses the cast to the identity. -/
private lemma rfns_iteratedCovGrad_castRankCc_loc {g : SmoothRiemannianMetric I M} (r : ℕ)
    {p q : ℕ} (h : p = q) (Y : SmoothCcTensor g r p) (j : ℕ) (x : M) :
    riemannianFiberNormSq (I := I) (M := M) g r (q + j) x
        ((iteratedCovGrad (I := I) g r q j
          (DifferentialGeometry.PDE.RicciFlow.castRankCc g r h Y)).toSection x) =
      riemannianFiberNormSq (I := I) (M := M) g r (p + j) x
        ((iteratedCovGrad (I := I) g r p j Y).toSection x) := by
  subst h; rfl

set_option linter.unusedSectionVars false in
/-- Nonnegativity of a single diagonal covariant-jet `rfns` grid summand `rfns(∇^p S) · rfns(∇^l T)`. -/
private lemma diagGrid_rfns_summand_nonneg {g : SmoothRiemannianMetric I M} {r₁ s₁ r₂ s₂ : ℕ}
    (S : SmoothCcTensor g r₁ s₁) (T : SmoothCcTensor g r₂ s₂) (x : M) (p l : ℕ) :
    0 ≤ riemannianFiberNormSq (I := I) (M := M) g r₁ (s₁ + p) x
  ...
  mul_nonneg (riemannianFiberNormSq_nonneg (I := I) (M := M) g r₁ (s₁ + p) x _)
    (riemannianFiberNormSq_nonneg (I := I) (M := M) g r₂ (s₂ + l) x _)

set_option backward.isDefEq.respectTransparency false in
/-- **Diagonal left shift of the covariant-jet `rfns` grid.** The `rfns` analogue of
`diagGrid_shift_left_le`: differentiating the left factor once and grading over the order-conserving
diagonal window `p + l ≤ j` is dominated by the diagonal window `p + l ≤ j + 1` of the undifferentiated
factors, via the `rfns` front-commutation `∇^p(∇S) ↦ ∇^{p+1}S`. -/
private lemma diagGrid_rfns_shift_left_le {g : SmoothRiemannianMetric I M} {r₁ s₁ r₂ s₂ : ℕ} {a b : ℕ}
    (S : SmoothCcTensor g r₁ (s₁ + a)) (T : SmoothCcTensor g r₂ (s₂ + b)) (x : M) (j : ℕ) :
    (∑ p ∈ Finset.range (j + 1), ∑ l ∈ Finset.range (j + 1 - p),
        riemannianFiberNormSq (I := I) (M := M) g r₁ ((s₁ + a + 1) + p) x
          ((iteratedCovGrad (I := I) g r₁ (s₁ + a + 1) p (covGrad g r₁ (s₁ + a) S)).toSection x) *
          riemannianFiberNormSq (I := I) (M := M) g r₂ ((s₂ + b) + l) x
            ((iteratedCovGrad (I := I) g r₂ (s₂ + b) l T).toSection x)) ≤
      ∑ p ∈ Finset.range (j + 2), ∑ l ∈ Finset.range (j + 2 - p),
        riemannianFiberNormSq (I := I) (M := M) g r₁ ((s₁ + a) + p) x
  ...
  refine le_add_of_le_of_nonneg ?_
    (Finset.sum_nonneg fun l _ => diagGrid_rfns_summand_nonneg S T x 0 l)
  refine Finset.sum_le_sum fun p _ => ?_
  rw [show j + 2 - (p + 1) = j + 1 - p from by omega]

set_option backward.isDefEq.respectTransparency false in
/-- **Diagonal right shift of the covariant-jet `rfns` grid.** The `rfns` analogue of
`diagGrid_shift_right_le`. -/
private lemma diagGrid_rfns_shift_right_le {g : SmoothRiemannianMetric I M} {r₁ s₁ r₂ s₂ : ℕ} {a b : ℕ}
    (S : SmoothCcTensor g r₁ (s₁ + a)) (T : SmoothCcTensor g r₂ (s₂ + b)) (x : M) (j : ℕ) :
    (∑ p ∈ Finset.range (j + 1), ∑ l ∈ Finset.range (j + 1 - p),
        riemannianFiberNormSq (I := I) (M := M) g r₁ ((s₁ + a) + p) x
          ((iteratedCovGrad (I := I) g r₁ (s₁ + a) p S).toSection x) *
          riemannianFiberNormSq (I := I) (M := M) g r₂ ((s₂ + b + 1) + l) x
  ...
  have hcomm : (∑ p ∈ Finset.range (j + 1), ∑ l ∈ Finset.range (j + 1 - p),
      ...
      ∑ p ∈ Finset.range (j + 1), ∑ l ∈ Finset.range (j + 1 - p),
        riemannianFiberNormSq (I := I) (M := M) g r₁ ((s₁ + a) + p) x
          ((iteratedCovGrad (I := I) g r₁ (s₁ + a) p S).toSection x) *
          riemannianFiberNormSq (I := I) (M := M) g r₂ ((s₂ + b) + (l + 1)) x
            ((iteratedCovGrad (I := I) g r₂ (s₂ + b) (l + 1) T).toSection x) := by
    refine Finset.sum_congr rfl fun p _ => Finset.sum_congr rfl fun l _ => ?_
    rw [rfns_iteratedCovGrad_covGrad_comm_loc (g := g) r₂ (s₂ + b) l T x]
  rw [hcomm]
  refine le_trans ?_ (Finset.sum_le_sum_of_subset_of_nonneg
    (Finset.range_subset_range.2 (by omega : j + 1 ≤ j + 2))
    (fun p _ _ => Finset.sum_nonneg fun l _ => diagGrid_rfns_summand_nonneg S T x p l))
  refine Finset.sum_le_sum fun p hp => ?_
  rw [show j + 2 - p = (j + 1 - p) + 1 from by have := Finset.mem_range.mp hp; omega,
    Finset.sum_range_succ' (n := j + 1 - p)
    (f := fun l => riemannianFiberNormSq (I := I) (M := M) g r₁ ((s₁ + a) + p) x
        ((iteratedCovGrad (I := I) g r₁ (s₁ + a) p S).toSection x) *
        riemannianFiberNormSq (I := I) (M := M) g r₂ ((s₂ + b) + l) x
          ((iteratedCovGrad (I := I) g r₂ (s₂ + b) l T).toSection x))]
  exact le_add_of_le_of_nonneg (le_refl _) (diagGrid_rfns_summand_nonneg S T x p 0)

set_option backward.isDefEq.respectTransparency false in
/-- **The DIAGONAL bilinear covariant Leibniz `rfns` grid for a parallel tensor product, in the
intrinsic `g`-Riemannian squared fibre norm, from a `g`-NATIVE order-0 fibrewise operator bound.**

For an intrinsic `ParallelTensorProduct` `Φ`, two smooth compactly-supported factors `S, U`, and a
**`g`-native** fibrewise squared-fibre-norm operator bound `gOp ≥ 0` (the structure's own `g`-Riemannian
field `Φ.rfns_prod_le`, valid at every extra-slot order `(a, b)`)
```
rfns₀(Φ.prod S' U')(x) ≤ gOp · rfns(S')(x) · rfns(U')(x),
```
the order-`j` covariant gradient of `Φ.prod S U` is dominated, at the squared fibre-norm level, by the
order-conserving DIAGONAL product grid
```
rfns(∇^j (Φ.prod S U))(x) ≤ gOp · 4^j · ∑_{p + l ≤ j} rfns(∇^p S)(x) · rfns(∇^l U)(x).
```

**Why `g`-native and chart-jet-free.**  The currency throughout is `riemannianFiberNormSq` (`rfns`) —
**never** the model fibre norm `‖·‖` (whose operator constant would be chartJ-UNBOUNDED relative to the
intrinsic `g`-norm on multi-chart manifolds).  The recursion proceeds by induction on `j`: the exact
covariant Leibniz `covGrad_prod` distributes the single gradient onto one factor at each step, the
front-commutation `rfns_iteratedCovGrad_covGrad_comm_loc` and the rank-cast invariance
`rfns_iteratedCovGrad_castRankCc_loc` realign the orders, and the factor-2 subadditivity
`riemannianFiberNormSq_add_le` (in squared-norm space) replaces the exact triangle inequality
— so the binomial blow-up is `4^j = (2·2)^j` rather than the model's `2^j`, matching the on-disk
`DiffBilinOp.rfns_iteratedCovGrad_grid` engine.  The `g`-native `gOp` hypothesis is genuine and bounded
(the `g`-fibre operator norm of a fixed smooth parallel contraction is the finite supremum of a
continuous coefficient over the compact base — unlike the chart-referenced model `opNorm`). -/
private theorem diagGrid_rfns_jet {g : SmoothRiemannianMetric I M} {r₁ s₁ r₂ s₂ r₀ s₀ : ℕ}
    (Φ : DifferentialGeometry.PDE.RicciFlow.ParallelTensorProduct g r₁ s₁ r₂ s₂ r₀ s₀)
    (gOp : ℝ) (hgOp : 0 ≤ gOp)
    (hop : ∀ {a b : ℕ} (S : SmoothCcTensor g r₁ (s₁ + a)) (U : SmoothCcTensor g r₂ (s₂ + b)) (x : M),
      riemannianFiberNormSq (I := I) (M := M) g r₀ (s₀ + a + b) x
          ((Φ.prod S U).toSection x) ≤
        gOp * riemannianFiberNormSq (I := I) (M := M) g r₁ (s₁ + a) x (S.toSection x) *
          riemannianFiberNormSq (I := I) (M := M) g r₂ (s₂ + b) x (U.toSection x))
    (j : ℕ) :
    ∀ {a b : ℕ} (S : SmoothCcTensor g r₁ (s₁ + a)) (U : SmoothCcTensor g r₂ (s₂ + b)) (x : M),
      riemannianFiberNormSq (I := I) (M := M) g r₀ ((s₀ + a + b) + j) x
          ((iteratedCovGrad (I := I) g r₀ (s₀ + a + b) j (Φ.prod S U)).toSection x) ≤
        gOp * (4 : ℝ) ^ j * ∑ p ∈ Finset.range (j + 1), ∑ l ∈ Finset.range (j + 1 - p),
          riemannianFiberNormSq (I := I) (M := M) g r₁ ((s₁ + a) + p) x
            ((iteratedCovGrad (I := I) g r₁ (s₁ + a) p S).toSection x) *
            riemannianFiberNormSq (I := I) (M := M) g r₂ ((s₂ + b) + l) x
              ((iteratedCovGrad (I := I) g r₂ (s₂ + b) l U).toSection x) := by
  induction j with
  | zero =>
      intro a b S U x
      ...
      have hsum : (∑ p ∈ Finset.range 1, ∑ l ∈ Finset.range (1 - p),
          riemannianFiberNormSq (I := I) (M := M) g r₁ ((s₁ + a) + p) x
            ((iteratedCovGrad (I := I) g r₁ (s₁ + a) p S).toSection x) *
            riemannianFiberNormSq (I := I) (M := M) g r₂ ((s₂ + b) + l) x
              ((iteratedCovGrad (I := I) g r₂ (s₂ + b) l U).toSection x)) =
          riemannianFiberNormSq (I := I) (M := M) g r₁ (s₁ + a) x (S.toSection x) *
            riemannianFiberNormSq (I := I) (M := M) g r₂ (s₂ + b) x (U.toSection x) := by
        simp
      rw [hsum, pow_zero, mul_one, ← mul_assoc]
      exact hop S U x
  | succ j ih =>
      intro a b S U x
      rw [show riemannianFiberNormSq (I := I) (M := M) g r₀ ((s₀ + a + b) + (j + 1)) x
            ((iteratedCovGrad (I := I) g r₀ (s₀ + a + b) (j + 1) (Φ.prod S U)).toSection x) =
          riemannianFiberNormSq (I := I) (M := M) g r₀ (((s₀ + a + b) + 1) + j) x
            ((iteratedCovGrad (I := I) g r₀ ((s₀ + a + b) + 1) j
              (covGrad g r₀ (s₀ + a + b) (Φ.prod S U))).toSection x) from
        (rfns_iteratedCovGrad_covGrad_comm_loc (g := g) r₀ (s₀ + a + b) j (Φ.prod S U) x).symm]
      rw [Φ.covGrad_prod S U, iteratedCovGrad_add]
      refine (riemannianFiberNormSq_add_le (I := I) (M := M) g r₀ (((s₀ + a + b) + 1) + j) x
          ((iteratedCovGrad (I := I) g r₀ ((s₀ + a + b) + 1) j
            (DifferentialGeometry.PDE.RicciFlow.castRankCc g r₀
              (by omega : s₀ + (a + 1) + b = s₀ + a + b + 1)
              (Φ.prod (a := a + 1) (b := b) (covGrad g r₁ (s₁ + a) S) U))).toSection x)
          ((iteratedCovGrad (I := I) g r₀ ((s₀ + a + b) + 1) j
            (Φ.prod (a := a) (b := b + 1) S (covGrad g r₂ (s₂ + b) U))).toSection x)).trans ?_
      rw [rfns_iteratedCovGrad_castRankCc_loc (g := g) r₀
        (by omega : s₀ + (a + 1) + b = s₀ + a + b + 1)
        (Φ.prod (a := a + 1) (b := b) (covGrad g r₁ (s₁ + a) S) U) j x]
      have hL := ih (a := a + 1) (b := b) (covGrad g r₁ (s₁ + a) S) U x
      have hR := ih (a := a) (b := b + 1) S (covGrad g r₂ (s₂ + b) U) x
      have hgridL := diagGrid_rfns_shift_left_le (g := g) S U x j
      have hgridR := diagGrid_rfns_shift_right_le (g := g) S U x j
      have hpowNN : (0 : ℝ) ≤ (4 : ℝ) ^ j := by positivity
      have hcoefNN : 0 ≤ gOp * (4 : ℝ) ^ j := mul_nonneg hgOp hpowNN
      set G : ℝ := ∑ p ∈ Finset.range (j + 2), ∑ l ∈ Finset.range (j + 2 - p),
          riemannianFiberNormSq (I := I) (M := M) g r₁ ((s₁ + a) + p) x
            ((iteratedCovGrad (I := I) g r₁ (s₁ + a) p S).toSection x) *
            riemannianFiberNormSq (I := I) (M := M) g r₂ ((s₂ + b) + l) x
              ((iteratedCovGrad (I := I) g r₂ (s₂ + b) l U).toSection x) with hG_def
      calc 2 * riemannianFiberNormSq (I := I) (M := M) g r₀ ((s₀ + (a + 1) + b) + j) x
              ((iteratedCovGrad (I := I) g r₀ (s₀ + (a + 1) + b) j
                (Φ.prod (a := a + 1) (b := b) (covGrad g r₁ (s₁ + a) S) U)).toSection x) +
            2 * riemannianFiberNormSq (I := I) (M := M) g r₀ ((s₀ + a + (b + 1)) + j) x
              ((iteratedCovGrad (I := I) g r₀ (s₀ + a + (b + 1)) j
                (Φ.prod (a := a) (b := b + 1) S (covGrad g r₂ (s₂ + b) U))).toSection x)
          ≤ 2 * (gOp * (4 : ℝ) ^ j * G) + 2 * (gOp * (4 : ℝ) ^ j * G) := by
            refine add_le_add (mul_le_mul_of_nonneg_left ?_ (by norm_num))
              (mul_le_mul_of_nonneg_left ?_ (by norm_num))
            · exact hL.trans (mul_le_mul_of_nonneg_left hgridL hcoefNN)
            · exact hR.trans (mul_le_mul_of_nonneg_left hgridR hcoefNN)
        _ = gOp * (4 : ℝ) ^ (j + 1) *
              ∑ p ∈ Finset.range (j + 1 + 1), ∑ l ∈ Finset.range (j + 1 + 1 - p),
                riemannianFiberNormSq (I := I) (M := M) g r₁ ((s₁ + a) + p) x
                  ((iteratedCovGrad (I := I) g r₁ (s₁ + a) p S).toSection x) *
                  riemannianFiberNormSq (I := I) (M := M) g r₂ ((s₂ + b) + l) x
                    ((iteratedCovGrad (I := I) g r₂ (s₂ + b) l U).toSection x) := by
...
`R ≥ 0`.  Outside the `∀ T T'` quantifier there are a realized coefficient rank `s`, a nonnegative
ball-uniform **`g`-native fibrewise operator envelope** `gOp`, and a nonnegative ball-uniform coefficient
`C⁰` envelope `Ccoef`; for any two `g₀`-fibre-small smooth perturbations `T, T'` whose covariant-`L²`
jets up to order `a + 2` lie in the radius-`R` ball there are three parallel fibrewise-bilinear bundle
maps `Φ₀, Φ₁, Φ₂` (the intrinsic metric-contraction symbols of the mean-value path `dF`) and a single
intrinsic `g₀`-built coefficient field `coeff : SmoothCcTensor g₀ 0 s` (the path-`dF`-symbol: the
curvature / Christoffel / inverse-Gram — Neumann-series, det-`≠ 0` by `δ, δ' < 1` — combination of the
realized metric path `g₀ + T' + r·(T − T')`, integrated `∫₀¹ … dr`) such that, with
`N := deTurckRHSArmG0 g₀ g_bg T − deTurckRHSArmG0 g₀ g_bg T'`, **every** covariant order `q ≤ a` of `N`
realizes pointwise as the order-`q` covariant gradient of the three-term contraction of `coeff` against
`∇^m (T − T')` (`m = 0, 1, 2`),
```
(∇^q N)(x) = ∇^q (Φ₀.prod coeff (T−T') + Φ₁.prod coeff (∇(T−T')) + Φ₂.prod coeff (∇²(T−T')))(x),
```
each `Φ_m` carries the **`g`-native** fibrewise squared-fibre-norm operator bound
`rfns₀(Φ_m.prod S U)(x) ≤ gOp · rfns(S)(x) · rfns(U)(x)` (the chart-independent currency — NOT the model
...
genuinely second-order, non-flat RHS difference; a `gOp = 0` envelope is rejected by a nonvanishing
`Φ_m.prod` on tensors of positive fibre norm. -/
...
/-- **(The DIAGONAL-window covariant Leibniz / Faà-di-Bruno grid for a parallel bilinear tensor
product, in the intrinsic `g`-Riemannian squared fibre-norm `riemannianFiberNormSq`.)**
...
**Why `g`-native (no `sorry` of its own).**  The currency throughout is the intrinsic `g`-Riemannian
squared fibre norm `riemannianFiberNormSq` (`rfns`) — **never** the model fibre norm `‖·‖`, whose
operator constant would be chartJ-UNBOUNDED relative to the intrinsic `g`-norm on multi-chart manifolds
(so a uniform model↔`g` fibre-norm equivalence does **not** exist).  The caller supplies the structure's
own genuine **`g`-native** fibrewise squared-fibre-norm operator bound `gOp` (the field `Φ.rfns_prod_le`),
which is bounded for a fixed smooth parallel contraction on the compact base (the finite supremum of a
continuous `g`-fibre coefficient — no chart reference).  The diagonal grid is then the `rfns`
covariant-Leibniz induction `diagGrid_rfns_jet` (factor `4^j` from the factor-2 subadditivity of `rfns`).
Expressed entirely in `iteratedCovGrad` / `riemannianFiberNormSq` — never a chart-jet ball Lipschitz
chain, never a model operator norm.

The per-order constant is `gOp · 4^j` (the `g`-native operator bound times the binomial blow-up of the
exact covariant Leibniz expansion read in squared-norm space). -/
private theorem ParallelTensorProduct_riemannianFiberNormSq_iteratedCovGrad_prod_diagonal_le
    ...
    (Φ : DifferentialGeometry.PDE.RicciFlow.ParallelTensorProduct g₀ r₁ s₁ r₂ s₂ r₀ s₀)
    (gOp : ℝ) (hgOp : 0 ≤ gOp)
    (hop : ∀ {a b : ℕ} (S : SmoothCcTensor g₀ r₁ (s₁ + a)) (U : SmoothCcTensor g₀ r₂ (s₂ + b))
        (x : M),
      riemannianFiberNormSq (I := I) (M := M) g₀ r₀ (s₀ + a + b) x ((Φ.prod S U).toSection x) ≤
        gOp * riemannianFiberNormSq (I := I) (M := M) g₀ r₁ (s₁ + a) x (S.toSection x) *
          riemannianFiberNormSq (I := I) (M := M) g₀ r₂ (s₂ + b) x (U.toSection x))
  ...
  diagGrid_rfns_jet Φ gOp hgOp hop j (a := 0) (b := 0) S U x
...
grid (the `g`-native `ParallelTensorProduct` covariant-Faà-di-Bruno engine
`ParallelTensorProduct_riemannianFiberNormSq_iteratedCovGrad_prod_diagonal_le`) leaves the top covariant
orders on the `T − T'` factor (order `≤ q + 2`) and on the `coeff` factor (order `≤ q + 2 − i`), giving
the displayed diagonal product grid.  The coefficient `C⁰` envelope is the supercritical `H^{a+2} ↪ C²` section
...
private theorem deTurckRHSArmG0_sub_eq_parallelTensorProduct_realization
        ...
        ∃ coeff : SmoothCcTensor g₀ 0 s,
          ...
          (∀ l : ℕ, l ≤ a + 2 → ∀ x : M,
            riemannianFiberNormSq (I := I) (M := M) g₀ 0 (s + l) x
              ((iteratedCovGrad (I := I) g₀ 0 s l coeff).toSection x) ≤ Ccoef) := by
  classical
  obtain ⟨s, gOp, Ccoef, hgOp_nn, hCcoef_nn, hreal⟩ :=
    deTurckRHSArmG0_diff_eq_threeTerm_parallelTensorProduct_realization
      (I := I) g₀ g_bg a ha_super hR
  -- The single ball-uniform grid constant `Cmid`: `3 · 4 · (gOp · 4^a)` (the three-term subadditivity
  -- factor `4`, the three terms, and the `g`-native per-order grid constant `gOp · 4^q ≤ gOp · 4^a`).
  refine ⟨s, 3 * (4 * (gOp * (4 : ℝ) ^ a)), Ccoef, by positivity, hCcoef_nn, ?_⟩
  intro T T' δ hδ_lt hδ δ' hδ'_lt hδ' hTball hT'ball
  obtain ⟨Φ₀, Φ₁, Φ₂, coeff, hop₀, hop₁, hop₂, hid, hC0⟩ :=
  ...
  -- The per-term `g`-native diagonal grids from the `rfns` engine `Q`, fed each `Φ_m`'s own `g`-native
  -- operator bound; each second-factor column folds to `∑_{p,l} fC p · hT (l + m)` via order conservation.
  have hQ₀' := ParallelTensorProduct_riemannianFiberNormSq_iteratedCovGrad_prod_diagonal_le
    (I := I) g₀ Φ₀ gOp hgOp_nn hop₀ coeff (T - T') x q
  have hQ₁' := ParallelTensorProduct_riemannianFiberNormSq_iteratedCovGrad_prod_diagonal_le
    (I := I) g₀ Φ₁ gOp hgOp_nn hop₁ coeff (iteratedCovGrad (I := I) g₀ 0 2 1 (T - T')) x q
  have hQ₂' := ParallelTensorProduct_riemannianFiberNormSq_iteratedCovGrad_prod_diagonal_le
    (I := I) g₀ Φ₂ gOp hgOp_nn hop₂ coeff (iteratedCovGrad (I := I) g₀ 0 2 2 (T - T')) x q
  set Cterm : ℝ := gOp * (4 : ℝ) ^ a with hCterm_def
  ...
  -- Constant domination shared by all three terms: `gOp · 4^q ≤ gOp · 4^a = Cterm`.
  have hpowq : (4 : ℝ) ^ q ≤ (4 : ℝ) ^ a := pow_le_pow_right₀ (by norm_num) hq
  have hCleg : gOp * (4 : ℝ) ^ q ≤ Cterm := by
    rw [hCterm_def]
    exact mul_le_mul_of_nonneg_left hpowq hgOp_nn
  ...
  have hterm0 : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + q) x
    ...
    rw [hcoleq]
    exact mul_le_mul_of_nonneg_right hCleg (hgrid_nn 0)
  ...
  have hterm1 : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + q) x
    ...
    rw [hcoleq]
    exact mul_le_mul_of_nonneg_right hCleg (hgrid_nn 1)
  ...
  have hterm2 : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + q) x
    ...
    rw [hcoleq]
    exact mul_le_mul_of_nonneg_right hCleg (hgrid_nn 2)
...
for `l ≤ a + 2` (built on the `g`-native diagonal covariant-Leibniz engine
`ParallelTensorProduct_riemannianFiberNormSq_iteratedCovGrad_prod_diagonal_le`).  Each coefficient column
...
private theorem deTurckRHSArmDiff_iteratedCovGrad_rfns_perOrder_intrinsic_ballUniform
  ...
  set Wi : ℕ → ℝ := fun i =>
    riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + i) x
      ((iteratedCovGrad (I := I) g₀ 0 2 i (T - T')).toSection x) with hWi_def
  have hWi_nn : ∀ i, 0 ≤ Wi i := fun i =>
    riemannianFiberNormSq_nonneg (I := I) (M := M) g₀ 0 (2 + i) x _
  -- The coefficient column at gradient order `i`: `∑_{l ≤ q+2−i} rfns(∇^l coeff)(x)`.
  set Ccol : ℕ → ℝ := fun i =>
    ∑ l ∈ Finset.range (q + 2 + 1 - i),
      riemannianFiberNormSq (I := I) (M := M) g₀ 0 (s + l) x
        ((iteratedCovGrad (I := I) g₀ 0 s l coeff).toSection x) with hCcol_def
  have hCcol_nn : ∀ i, 0 ≤ Ccol i := fun i =>
    Finset.sum_nonneg fun l _ => riemannianFiberNormSq_nonneg (I := I) (M := M) g₀ 0 (s + l) x _
  -- Each diagonal term `Cmid · Wi i · Ccol i` (window `i ≤ q + 2`) is dominated by `Λcoef · Wi i`
  -- using the ball-uniform column bound `Cmid · Ccol i ≤ Λcoef`.
  calc riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + q) x
          ((iteratedCovGrad (I := I) g₀ 0 2 q
            (deTurckRHSArmG0 (I := I) g₀ g_bg T hδ_lt hδ -
              deTurckRHSArmG0 (I := I) g₀ g_bg T' hδ'_lt hδ')).toSection x)
      ≤ Cmid * ∑ i ∈ Finset.range (q + 2 + 1), Wi i * Ccol i := hgrid q hq x
    _ = ∑ i ∈ Finset.range (q + 2 + 1), (Cmid * Ccol i) * Wi i := by
          rw [Finset.mul_sum]; exact Finset.sum_congr rfl fun i _ => by ring
    _ ≤ ∑ i ∈ Finset.range (q + 2 + 1), Λcoef * Wi i := by
          refine Finset.sum_le_sum fun i hi => ?_
          exact mul_le_mul_of_nonneg_right (hcol i hi x) (hWi_nn i)
    _ = Λcoef * ∑ i ∈ Finset.range (q + 2 + 1), Wi i := by rw [Finset.mul_sum]
...
Fix `g₀`, the DeTurck background `g_bg`, a supercritical order `a` (`2·finrank E + 10 ≤ a`), and a
covariant-`L²` ball radius `R ≥ 0`.  There is **one** nonnegative constant `Λg` — uniform over the
fibre-small radius-`R` ball, **outside** the `∀ T T'` quantifier — such that for any two
`g₀`-fibre-small smooth perturbations `T, T'` whose covariant-`L²` jets up to order `a + 2` lie in the
radius-`R` ball, **every** covariant order `q ≤ a` of the nonlinear RHS-arm difference
`N := deTurckRHSArmG0 g₀ g_bg T − deTurckRHSArmG0 g₀ g_bg T'` is dominated, at the pointwise squared
fibre-norm level, by the order-`(q+2)` covariant-jet column of the perturbation difference `T − T'`:
```
...
[T − T'] ds`, the value `N(x)` is a finite sum of intrinsic covariant-bilinear contractions of the
order-`≤ 2` jets of `T − T'` against a fixed-`g₀` coefficient field (the `dF` symbol along the path —
the curvature/Christoffel/inverse-Gram combination, rational and det-`≠ 0` by `δ < 1`).  The
coefficient enters only through its order-`0` `C⁰` sup, controlled ball-uniformly by the supercritical
`H^{a+2} ↪ C²` section embedding of the radius-`R` ball (`ha_super`: a concentrating common part of
`T, T'` keeps `T − T'` fixed while blowing up `∂²(g₀+T)`, so fibre-smallness `δ < 1` ALONE is
insufficient and the `R`-ball is genuinely load-bearing); the high covariant orders all land on the
`T − T'` factor and the order-`0` value reads them only through their `L²`-jet scale (Cauchy–Schwarz
over the closed manifold).  This is the genuine intrinsic curvature/Lie/inverse-Gram value-level
Nemytskii content, expressed entirely in `iteratedCovGrad` / `riemannianFiberNormSq` / `tensorL2Norm`
— **never** a `chartGramOnE` / `chartInvGramOnE` / `HasChartJetLip` chart-jet ball Lipschitz chain
(whose `∃B`-over-`∀g∈ball` hoist is unsatisfiable at the top jet).
...
difference.  This is a posited deficit-free analytic leaf; consumers transitively depend on its
`sorryAx`. -/
private theorem deTurckRHSArmDiff_order0_rfns_intrinsic_ballUniform
  ...
  classical
  -- The shared intrinsic covariant-Leibniz grid leaf (at covariant order `q = 0`, window `≤ 2`):
  -- `rfns(N)(x) ≤ Λg² · ∑_{i ≤ 2} rfns(∇^i (T − T'))(x)`.
  obtain ⟨Λg, hΛg_nn, hgrid⟩ :=
    deTurckRHSArmDiff_iteratedCovGrad_rfns_grid_intrinsic_ballUniform (I := I) g₀ g_bg a ha_super hR
  -- The supercritical `H^{a+2} ↪ C²` section embedding of the radius-`R` ball, applied to `T − T'`:
  -- `∑_{i ≤ 2} rfns(∇^i (T − T'))(x) ≤ Cemb² · ∑_{i ≤ a+2} ‖∇^i (T − T')‖²`.
  obtain ⟨Cemb, hCemb_nn, hemb⟩ :=
    deTurckArmDiff_supercritical_pointwise_jet_le (I := I) g₀ a ha_super
  refine ⟨Λg * Cemb, by positivity, ?_⟩
  intro T T' δ hδ_lt hδ δ' hδ'_lt hδ' hTball hT'ball x
  set S : ℝ := ∑ i ∈ Finset.range (a + 2 + 1),
    ‖iteratedCovGrad (I := I) g₀ 0 2 i (T - T')‖ ^ 2 with hS_def
  have hS_nn : 0 ≤ S := Finset.sum_nonneg fun i _ => sq_nonneg _
  -- The grid leaf at `q = 0`: `rfns(∇^0 N)(x) = rfns(N)(x)` (the order-`0` jet is the tensor itself).
  have hg0 := hgrid T T' hδ_lt hδ hδ'_lt hδ' hTball hT'ball 0 (Nat.zero_le a) x
  -- The embedding column for `T − T'`.
  have hemb' := hemb (T - T') x
  -- Chain: `rfns(N)(x) ≤ Λg² · (∑_{i ≤ 2} rfns(∇^i (T − T'))(x)) ≤ Λg² · Cemb² · S = (Λg·Cemb)² · S`.
  calc riemannianFiberNormSq (I := I) (M := M) g₀ 0 2 x
          ((deTurckRHSArmG0 (I := I) g₀ g_bg T hδ_lt hδ -
              deTurckRHSArmG0 (I := I) g₀ g_bg T' hδ'_lt hδ').toSection x)
      ≤ Λg ^ 2 * ∑ i ∈ Finset.range (0 + 2 + 1),
            riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + i) x
              ((iteratedCovGrad (I := I) g₀ 0 2 i (T - T')).toSection x) := hg0
    _ ≤ Λg ^ 2 * (Cemb ^ 2 * S) := by
        refine mul_le_mul_of_nonneg_left ?_ (by positivity)
        rw [hS_def]; exact hemb'
    _ = (Λg * Cemb) ^ 2 * S := by ring
...
**Why deficit-free, and why INTRINSIC.**  This is the integrated **classical covariant Leibniz /
Faà-di-Bruno** expansion of the second-order Nemytskii difference `N`.  In the covariant Leibniz
expansion of `∇^a N` (the binomial jet grid of the bilinear contractions of `dF` against `T − T'`,
the intrinsic `DiffBilinOp.rfns_iteratedCovGrad_grid` shape), the top covariant order always lands on
the `T − T'` factor (order `≤ a + 2`) **in `L²`**, while the order-`≤ 2` coefficient field enters only
through its order-`0` `C⁰` sup, supplied ball-uniformly by the supercritical `H^{a+2} ↪ C²` section
embedding of the radius-`R` ball (`ha_super`).  Feeding this through the intrinsic Moser tame product /
Gagliardo–Nirenberg engines (`Analysis.Sobolev.Tensor.exists_moserTameProduct_iteratedCovGrad_l2Norm_le`,
`exists_gagliardoNirenberg_iteratedCovGrad_l2Norm_le`) yields the top-order `L²` endpoint with a
constant ball-uniform in `R` — **never** an order-`(a + 2)` pointwise `C⁰` sup, **never** a
`chartGramOnE` / `chartInvGramOnE` / `HasChartJetLip` chart-jet ball Lipschitz chain.
...
private theorem deTurckRHSArmDiff_topOrder_l2_intrinsic_ballUniform
      ...
      ∀ (T T' : SmoothCcTensor g₀ 0 2)
        ...
        ‖iteratedCovGrad (I := I) g₀ 0 2 a
          ...
          Λc * Real.sqrt (∑ i ∈ Finset.range (a + 2 + 1),
            ‖iteratedCovGrad (I := I) g₀ 0 2 i (T - T')‖ ^ 2) := by
  classical
  -- The shared intrinsic covariant-Leibniz grid leaf (at covariant order `q = a`, window `≤ a + 2`):
  -- `∀ x, rfns(∇^a N)(x) ≤ Λg² · ∑_{i ≤ a+2} rfns(∇^i (T − T'))(x)`.
  obtain ⟨Λg, hΛg_nn, hgrid⟩ :=
    deTurckRHSArmDiff_iteratedCovGrad_rfns_grid_intrinsic_ballUniform (I := I) g₀ g_bg a ha_super hR
  refine ⟨Λg, hΛg_nn, ?_⟩
  intro T T' δ hδ_lt hδ δ' hδ'_lt hδ' hTball hT'ball
  set N : SmoothCcTensor g₀ 0 2 :=
    deTurckRHSArmG0 (I := I) g₀ g_bg T hδ_lt hδ -
      deTurckRHSArmG0 (I := I) g₀ g_bg T' hδ'_lt hδ' with hN_def
  -- The grid leaf at `q = a` is exactly the pointwise covariant-jet hypothesis of
  -- `l2RootSum_of_pointwise_iteratedCovGrad_jet` (P := N, W := T − T', q := a, window N := a + 2,
  -- constant C := Λg²): integrate the pointwise grid over the closed manifold to the `L²` root-sum.
  have hpt : ∀ x : M,
      riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + a) x
          ((iteratedCovGrad (I := I) g₀ 0 2 a N).toSection x) ≤
        Λg ^ 2 * ∑ i ∈ Finset.range ((a + 2) + 1),
          riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + i) x
            ((iteratedCovGrad (I := I) g₀ 0 2 i (T - T')).toSection x) := by
    intro x
    rw [hN_def]
    exact hgrid T T' hδ_lt hδ hδ'_lt hδ' hTball hT'ball a le_rfl x
  have hl2 :=
    l2RootSum_of_pointwise_iteratedCovGrad_jet (I := I) g₀ a (a + 2) N (T - T')
      (Λg ^ 2) (by positivity) hpt
  -- `√(Λg²) = Λg` (since `Λg ≥ 0`); the goal already reads `N` (via `set`).
  rwa [Real.sqrt_sq hΛg_nn] at hl2

/-- **(The top-order integrated `L²` tame bound on the nonlinear Ricci–DeTurck RHS-arm difference,
re-proved DEFICIT-FREE from the intrinsic covariant-Leibniz top-jet leaf.)**
...
**Assembly.**  The conclusion coincides verbatim with the deficit-free INTRINSIC top-jet leaf
`deTurckRHSArmDiff_topOrder_l2_intrinsic_ballUniform` (the classical covariant Leibniz / Faà-di-Bruno
expansion of `∇^a N` fed through the intrinsic Moser / Gagliardo–Nirenberg engines), so this re-proof
is a direct citation.  It routes **only** through `iteratedCovGrad` / `tensorL2Norm` — **never** the
chart-jet `chartGramOnE` / `chartInvGramOnE` / `HasChartJetLip` ball Lipschitz chain that the prior
proof transited.  Consumers transitively depend on the intrinsic leaf's `sorryAx`. -/
      ...
      ∀ (T T' : SmoothCcTensor g₀ 0 2)
        ...
        ‖iteratedCovGrad (I := I) g₀ 0 2 a
          ...
          Λt * Real.sqrt (∑ i ∈ Finset.range (a + 2 + 1),
            ‖iteratedCovGrad (I := I) g₀ 0 2 i (T - T')‖ ^ 2) := by
  -- The deficit-free INTRINSIC top-order-`a` integrated covariant-`L²` Moser–Nemytskii tame leaf:
  -- the classical covariant Leibniz / Faà-di-Bruno expansion of the second-order Nemytskii difference
  -- `N := deTurckRHSArmG0 g₀ g_bg T − deTurckRHSArmG0 g₀ g_bg T'`, expressed entirely in
  -- `iteratedCovGrad` / `tensorL2Norm` — never the chart-jet `chartGramOnE` / `chartInvGramOnE` /
  -- `HasChartJetLip` ball Lipschitz chain.  The conclusion is identical to the intrinsic child's.
  exact deTurckRHSArmDiff_topOrder_l2_intrinsic_ballUniform (I := I) g₀ g_bg a ha_super hR
...
private theorem deTurckRHSArmDiff_endpoints_l2_tame_ballUniform
  ...
  obtain ⟨Λc, hΛc_nn, hΛc⟩ :=
    deTurckRHSArmDiff_topOrder_l2_intrinsic_ballUniform (I := I) g₀ g_bg a ha_super hR
  ...
  have hpt : ∀ x : M,
    ...
    intro x
    rw [hN_def, hS_def]
    exact hΛa T T' hδ_lt hδ hδ'_lt hδ' hTball hT'ball x
  have hC0 : ∀ x : M,
...
private theorem deTurckRHSArmDiff_iteratedCovGrad_l2_tame_ballUniform
  ...
  · obtain ⟨Cgn, hCgn_nn, hGN⟩ :=
    ...
    have hS_nn : 0 ≤ S := Finset.sum_nonneg fun i _ => sq_nonneg _
    have hsqrtS_nn : 0 ≤ Real.sqrt S := Real.sqrt_nonneg _
    obtain ⟨hC0, hL0, hLa⟩ := hEnd T T' hδ_lt hδ hδ'_lt hδ' hTball hT'ball
