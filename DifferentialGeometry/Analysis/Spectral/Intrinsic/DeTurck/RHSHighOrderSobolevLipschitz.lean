...
`S`, structurally distinct from the `toHs`-norm hypothesis on the right; no packaging.  It is
**proven by composition** of the unconditional all-order Gårding bootstrap
`allOrder_covGrad_l2Norm_le_lapIter_sum_unconditional` (the full covariant-jet `L²` sum over
`j ≤ 2k` controlled by the iterated rough-Laplacian `L²` sum over `i ≤ k`) and the "easy"
iterated-Laplacian-`L²`-by-chart bound `exists_rawConnLapIter_l2Norm_le_toHs` (each
`‖(Δ_∇^i S).toL2‖ ≤ C · ‖S.toHs k‖` for `i ≤ k`), the single covariant-gradient summand being
dominated by the full nonnegative jet sum and the per-iteration constants made uniform by their
finite sum.  Consumers transitively depend on `sorryAx` only through these two children (and their
own atomic Sobolev primitives), with no spectral, perturbation-indexed, or Weyl dependence. -/
...
The mechanism is the **intrinsic covariant Moser-tame product**
`exists_moserTameProduct_iteratedCovGrad_l2Norm_le` (a proven, sorry-free `L²`-Leibniz product
bound): in each covariant-Leibniz term `(∇^{(j+2)−i} of a segment-jet coefficient) ⋆ ∇^i(g₁ − g₂)`
of the chain-rule expansion, the **intrinsic order** of the geometric nonlinearity `Ric + Lie` is
capped at `2` (it is `g⁻¹ · ∂g · ∂g` or `g⁻¹ · ∂²g`; the `g⁻¹` Neumann factors carry intrinsic
order `0`), so the coefficient's genuinely-needed pointwise sup is only its `≤2`-jet, supplied by
the proven order-`≤2` segment-metric jet sup `exists_segmentMetric_realizeSymm_iteratedCovGradJet2_sup_le`
(`iteratedCovGradJetSum`, orders `0,1,2`, uniform over the `H^{a+2}`-bounded `B`-family by the
supercritical `H^{2(a+2)} ↪ C²` embedding `ha` buys) — **never** a pointwise sup of any
higher-order jet (the `C^{2a+2}`-jet of the metric is unavailable on `finrank ≥ 4`).  The single
high derivative is therefore carried by the metric-difference factor `∇^i(g₁ − g₂)` (order
`i ≤ j + 2 ≤ 2a + 2`) **in `L²`** (`exists_moserTameProduct_iteratedCovGrad_l2Norm_le`'s `Λ · ∑
‖∇^i w‖_{L²}` arm, with `w` the metric difference), while the redistributed *top* coefficient
derivative is the `Λ₀`-arm folded into the order-`a` chart-Sobolev redistribution term `‖(T₁ −
T₂).toHs a‖` (the `H^a ↪ C⁰` embedding controlling the perturbation's `L^∞`/`C⁰` factor).  Since
the fibrewise `inner`-difference makes `(g₁ − g₂).inner = ccTensorBilinSymm g₀ (T₁ − T₂)` the
realized bilinear form of `T₁ − T₂`, each `∇^i(g₁ − g₂)` is `L²`-controlled (the realization gains
no derivatives) by the `≤ i`-order covariant gradients of `T₁ − T₂`; folding the (finitely many)
Moser-tame term constants into `C` yields the stated bound.

Its body is `sorry`: it is the genuine atomic `L²`-covariant-Faà-di-Bruno (Moser-tame) expansion
of the geometric nonlinearity `Ric + Lie` along the segment metric — the deep analytic content of
the order-`a` chart-RHS tower (the linear `Δ_∇` summand is handled separately, the
metric-realization map already factored out by the consumer).  It carries **no** pointwise-sup claim
on any jet of order `> 2`, and **no** spectral-nonlinearity, perturbation-indexed-remainder, or Weyl
dependence.  Consumers transitively depend on `sorryAx` only through this single atomic `L²`
covariant-Moser-tame segment-metric primitive. -/
