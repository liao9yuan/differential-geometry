...
theorem b02_center_pair_eq_intervalIntegral_and_intervalIntegrable
    ...
    let Js := fun s : ℝ =>
      ...
      let R0s := rhsRefold0 (I := I) (M := M) g g_bg T hdelta hdeltaZ s
      ...
      let Cs := deTurckPhiMetTotal (I := I) (M := M) g g_bg gs - Φ0
      oneMinusConnLapSmooth (I := I) g 0 2
  ...
  have hA0 := appCc_fixed_jointContMDiffOn (I := I) (M := M)
    g (fun s => R s + K0) T S hRK
  have hQ : linearizedRicciThreeArmHjoint (I := I) (M := M) g 2 Q
      (δ := delta) (δ' := delta) := by
    simpa only [Q] using edgeTopPair_joint (I := I) (M := M)
      g T T hdelta hdeltaZ ricciRefoldQA ricciRefoldQB lieRefoldQ lieRefoldEps
  have hZ0 := appCc_fixed_jointContMDiffOn (I := I) (M := M) g Q T S hQ
  ...
  have hCHT := appCc_fixed_jointContMDiffOn (I := I) (M := M)
    g (fun s => Φ s - Φ0) HT S hC
  have hCHLT := appCc_fixed_jointContMDiffOn (I := I) (M := M)
    g (fun s => Φ s - Φ0) HLT S hC
  have hRKInt : rhsRefold0Int (I := I) (M := M) g g_bg T
        hdelta_lt hdelta hdeltaZ + K0 =
      pathIntegralCoeffField (I := I) (M := M) g 2 2
        (fun s => R s + K0) S realizedSmallSet_isOpen hSI hRK := by
    simpa only [R, K0, S] using pathIntegralCoeffField_add_const
      (I := I) (M := M) g R K0 S realizedSmallSet_isOpen hSI hR hRK
  have hA0Int : appCc (I := I) (M := M) g 2 2
        (rhsRefold0Int (I := I) (M := M) g g_bg T
          hdelta_lt hdelta hdeltaZ + K0) T =
      pathIntegralCoeffField (I := I) (M := M) g 0 2 A0 S
        realizedSmallSet_isOpen hSI (by
          simpa only [A0, appCcRS_zero_eq_appCc] using hA0) := by
    rw [hRKInt]
    simpa only [A0, appCcRS_zero_eq_appCc] using
      appCc_pathIntegralCoeffField (I := I) (M := M) g
        (fun s => R s + K0) T S realizedSmallSet_isOpen hSI hRK
  have hZ0Int : appCc (I := I) (M := M) g 2 2
        (edgeTopPairInt (I := I) (M := M) g T T hdelta_lt hdelta hdeltaZ
          ricciRefoldQA ricciRefoldQB lieRefoldQ lieRefoldEps) T =
      pathIntegralCoeffField (I := I) (M := M) g 0 2 Z0 S
        realizedSmallSet_isOpen hSI (by
          simpa only [Z0, appCcRS_zero_eq_appCc] using hZ0) := by
    simpa only [Z0, Q, S, appCcRS_zero_eq_appCc] using
      edgeTopPairInt_apply (I := I) (M := M) g T T T hdelta_lt hdelta hdeltaZ
  ...
  have hCInt : rhsTopPathIntegral (I := I) (M := M) g g_bg T 0
      ...
      pathIntegralCoeffField (I := I) (M := M) g 4 2
        (fun s => Φ s - Φ0) S realizedSmallSet_isOpen hSI hC := by
    have hΦ := rhsTop_path_joint (I := I) (M := M)
      g g_bg T (0 : SmoothCcTensor g 0 2) hdelta hdeltaZ
    ...
    simpa only [Φ, Φ0, S, sub_eq_add_neg] using
      pathIntegralCoeffField_add_const (I := I) (M := M)
        g Φ (-Φ0) S realizedSmallSet_isOpen hSI
          (by simpa only [Φ] using hΦ) hΦneg
  have hCHTInt : appCc (I := I) (M := M) g 4 2
        (rhsTopPathIntegral (I := I) (M := M) g g_bg T 0
          hdelta_lt hdelta hdelta_lt hdeltaZ - Φ0) HT =
      pathIntegralCoeffField (I := I) (M := M) g 0 2 CHT S
        realizedSmallSet_isOpen hSI (by
          simpa only [CHT, appCcRS_zero_eq_appCc] using hCHT) := by
    rw [hCInt]
    simpa only [CHT, appCcRS_zero_eq_appCc] using
      appCc_pathIntegralCoeffField (I := I) (M := M) g
        (fun s => Φ s - Φ0) HT S realizedSmallSet_isOpen hSI hC
  have hCHLTInt : appCc (I := I) (M := M) g 4 2
        (rhsTopPathIntegral (I := I) (M := M) g g_bg T 0
          hdelta_lt hdelta hdelta_lt hdeltaZ - Φ0) HLT =
      pathIntegralCoeffField (I := I) (M := M) g 0 2 CHLT S
        realizedSmallSet_isOpen hSI (by
          simpa only [CHLT, appCcRS_zero_eq_appCc] using hCHLT) := by
    rw [hCInt]
    simpa only [CHLT, appCcRS_zero_eq_appCc] using
      appCc_pathIntegralCoeffField (I := I) (M := M) g
        (fun s => Φ s - Φ0) HLT S realizedSmallSet_isOpen hSI hC
  have hpath := inner_centered_pathIntegral_data (I := I) (M := M)
    g V A0 Z0 CHT CHLT S realizedSmallSet_isOpen hSI
      (by simpa only [A0, appCcRS_zero_eq_appCc] using hA0)
      (by simpa only [Z0, appCcRS_zero_eq_appCc] using hZ0)
      (by simpa only [CHT, appCcRS_zero_eq_appCc] using hCHT)
      (by simpa only [CHLT, appCcRS_zero_eq_appCc] using hCHLT)
  rw [← hA0Int, ← hZ0Int, ← hCHTInt, ← hCHLTInt] at hpath
  have hcenter := b02_center_nf (I := I) (M := M)
