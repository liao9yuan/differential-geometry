...
/-!
# Ball-uniform graded three-arm coefficient bounds and the mixed grid
...
private theorem lieArm_threeArm_coeffFields_perOrder_data
  ...
  by
    ...
    · intro s hs x
      refine ⟨?_, ?_, ?_⟩
      · refine le_trans (symmAbsorbedCoeff_rfns_le (I := I) (M := M) g₀ 0 _ σ'₀ x) ?_
        ...
        rw [hC_def]
        linarith
      · refine le_trans (symmAbsorbedCoeff_rfns_le (I := I) (M := M) g₀ 1 _ σ'₁ x) ?_
        have h1 := hΛ1 T T' hδ_le hδ hδ'_le hδ' hTball hT'ball s hs x
        rw [hC_def]
        linarith
      · refine le_trans (symmAbsorbedCoeff_rfns_le (I := I) (M := M) g₀ 2 _ σ'₂ x) ?_
...
set_option synthInstance.maxHeartbeats 1600000 in
set_option backward.isDefEq.respectTransparency false in
theorem armField_pathIntegral_jetL2_tower_le
...
set_option maxHeartbeats 1000000 in
theorem deTurckRHSArmDiff_threeArm_coeffC0_jetL2_ballUniform
    (g₀ g_bg : SmoothRiemannianMetric I M) (a : ℕ)
    (ha_super : 2 * Module.finrank ℝ E + 10 ≤ a) {R : ℝ} (hR : 0 ≤ R)
    {δ₀ : ℝ} (hδ₀ : δ₀ < 1) :
    ∃ ΛC Γ : ℝ, 0 ≤ ΛC ∧ 0 ≤ Γ ∧
