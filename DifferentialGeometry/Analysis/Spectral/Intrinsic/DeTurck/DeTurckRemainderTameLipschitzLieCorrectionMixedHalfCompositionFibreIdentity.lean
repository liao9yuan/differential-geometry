...
lemma lc0AMixHalfField_toSection (g₀ g₁ g_bg : SmoothRiemannianMetric I M)
    (σlast : Equiv.Perm (Fin 4)) (x : M) :
    (lc0AMixHalfField (I := I) (M := M) g₀ g₁ g_bg σlast).toSection x =
      (show Tensor0SSpace 4 I x →L[ℝ] Tensor0SSpace 2 I x from
        (lc0Tr (I := I) (M := M) g₀ g₁ 2 σlast).toSection x).comp
        (show Tensor0SSpace 2 I x →L[ℝ] Tensor0SSpace 4 I x from
          (lc0AMixOuterField (I := I) (M := M) g₀ g₁ g_bg).toSection x) :=
  appCcRS_toSection (I := I) (M := M) g₀ 2 4 2
    (lc0Tr (I := I) (M := M) g₀ g₁ 2 σlast)
    (lc0AMixOuterField (I := I) (M := M) g₀ g₁ g_bg) x
