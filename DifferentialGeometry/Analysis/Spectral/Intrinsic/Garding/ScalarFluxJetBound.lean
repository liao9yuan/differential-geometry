...
private lemma metricDiff_apply (q h : SmoothRiemannianMetric I M)
  ...
  change tensor0SSpace_evalScalar x c • metricCcTensorFib (I := I) h x -
      tensor0SSpace_evalScalar x c • metricCcTensorFib (I := I) q x = _
  rw [smul_sub]
...
private theorem metricInner_joint
    {D : RealTimeInterval}
  ...
  apply contMDiffOn_clm_section_of_pointwise_jointMR (I := I) (M := M)
    ...
    (φ := fun p : M × ℝ => (G.metric p.2).inner p.1)
  intro Y
  apply contMDiffOn_clm_section_of_pointwise_jointMR (I := I) (M := M)
    (F₁ := E) (V₁ := fun x : M => TangentSpace I x)
    (F₂ := ℝ) (V₂ := fun _ : M => ℝ)
    (φ := fun p : M × ℝ => (G.metric p.2).inner p.1 (Y p.1))
  intro Z p hp
  rw [Bundle.contMDiffWithinAt_totalSpace]
  refine ⟨contMDiffWithinAt_fst, ?_⟩
  have hpair := hG.pairSmoothAt
    (D.regular_isOpen.mem_nhds hp.2) (![Y, Z])
  have hswap : ContMDiffAt (I.prod 𝓘(ℝ, ℝ))
      (𝓘(ℝ, ℝ).prod I) ∞ (fun q : M × ℝ => (q.2, q.1)) p :=
    contMDiffAt_snd.prodMk contMDiffAt_fst
  have hscalar : ContMDiffAt (I.prod 𝓘(ℝ, ℝ)) 𝓘(ℝ, ℝ) ∞
      (fun q : M × ℝ =>
        (G.metric q.2).inner q.1 (Y q.1) (Z q.1)) p :=
    hpair.comp p hswap
...
private theorem metricDiff_joint
    ...
    ContMDiffOn (I.prod 𝓘(ℝ, ℝ))
      ...
      ((Set.univ : Set M) ×ˢ D.regular) := by
  have hraw := joint_to02 (I := I) (M := M)
    (fun p : M × ℝ => (G.metric p.2).inner p.1)
    (metricInner_joint (I := I) G hG)
  have hmove : ContMDiffOn (I.prod 𝓘(ℝ, ℝ))
    ...
    intro m
    change (G.metric p.2).inner p.1 (m 0) (Fin.tail m 0) =
      (G.metric p.2).inner p.1 (m 0) (m 1)
    simp only [Fin.tail, Fin.succ_zero_eq_one]
  have hfix : ContMDiffOn (I.prod 𝓘(ℝ, ℝ))
      (I.prod 𝓘(ℝ, Tensor0SModel 2 ℝ E)) ∞
      (fun p : M × ℝ => TotalSpace.mk' (Tensor0SModel 2 ℝ E)
        (E := fun x : M => Tensor0SSpace 2 I x) p.1
        (metricCcTensorFib (I := I) q p.1))
      ((Set.univ : Set M) ×ˢ D.regular) :=
    (metricCcTensorFib_section_contMDiff (I := I) q).comp_contMDiffOn
      contMDiffOn_fst
  have hdiff := hmove.sub_section hfix
  apply contMDiffOn_clm_section_of_pointwise_jointMR (I := I) (M := M)
  ...
  have hscalar' := hscalar.comp_contMDiffOn contMDiffOn_fst
  have hsmul := hscalar'.smul_section hdiff
