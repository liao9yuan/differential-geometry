...
private theorem realizedFamily_flowDeriv
  ...
  have hRepr : ∀ (S : SmoothCcTensor g₀ 0 2) {δ' : ℝ} (hδ'_lt : δ' < 1)
      ...
      (x : M) (v w : TangentSpace I x),
      ccTensorBilinSymm (I := I) g₀
          (deTurckSmoothRemainder (I := I) (M := M) g₀ g_bg S hδ'_lt hδ' +
            rawTensorConnLapSmooth (I := I) g₀ 0 2 S) x v w =
        deTurckRicciRHS (I := I) g_bg
          (tensorSectionRealizeMetric (I := I) g₀ S hδ'_lt hδ') x v w := by
    intro S δ' hδ'_lt hδ' x v w
    set gDT := tensorSectionRealizeMetric (I := I) g₀ S hδ'_lt hδ' with hgDT_def
    set R : SmoothCcTensor g₀ 0 2 :=
      { toSection := (deTurckRHSSection (I := I) g_bg gDT).toSection
        hasCompactSupport := (deTurckRHSSection (I := I) g_bg gDT).hasCompactSupport }
      with hR_def
    have hsum_eq : deTurckSmoothRemainder (I := I) (M := M) g₀ g_bg S hδ'_lt hδ' +
        rawTensorConnLapSmooth (I := I) g₀ 0 2 S = R := by
  ...
  have hForceRepr := realizedForcingCoord_eq_smoothN (I := I) (M := M) g₀ g_bg a ha_super
  ...
  exact realizedFamily_flowDeriv_of_repr (I := I) (M := M) g₀ a ha_super
    (deTurckRicciRHS (I := I) g_bg) (deTurckSmoothRemainder (I := I) (M := M) g₀ g_bg)
    hRepr hT hT1 hT₁_pos hT₁_le hd₂F_pos hd₂F_le hT₁_le_d2F u F hδ_lt hδ h_pin f hf_smooth
    hf_mass hf_id hForceRepr
...
theorem deTurckRicci_forcingBootstrap
  ...
  · intro T₁ hT₁_pos hT₁_le hT₁_le_d2F Ffam δ hδ_lt hδ h_pin hball
    exact realizedForcingCoord_eq_smoothN (I := I) (M := M) g₀ g_bg a (by omega)
      hT hT1 hTT₀ hT₁_pos hT₁_le hd₂F_pos hd₂F_le hT₁_le_d2F u gforce hduh hforce htrace
...
theorem maxreg_solution_jointly_smooth_representative
  ...
  have hinit : u.init = 0 := by have := htrace; rwa [timeH1.trace0_apply] at this
  obtain ⟨d₂F, hd₂F_pos, hd₂F_le, f, hf_smooth, hf_mass, hf_id, hforce_coord⟩ :=
    forcingSmoothCoordsRealize (I := I) (M := M) g₀ g_bg a (by omega) hT hT1 hTT₀ u gforce
      hduh hforce hgforce htrace
  set R₀ : ℝ := (Classical.choose
    (deTurckSobolevNHa2_exists_of_super (I := I) (M := M) g₀ a (by omega))).1 with hR₀_def
  have hR₀_pos : 0 < R₀ :=
      ...
      ∀ t ∈ Set.Icc (0 : ℝ) d₂, ∀ S : SmoothCcTensor g₀ 0 2,
        SmoothCcTensor.toL2 (g := g₀) (r := 0) (s := 2) S =
          tensorHsToL2 (I := I) (M := M) (g := g₀) (r := 0) (s := 2)
            ...
            (Nat.cast_nonneg a) (timeH1.toFun u t) →
          ‖smoothCcToTensorHs (I := I) (M := M) g₀ ((a : ℝ) + 2) S‖ ≤ R₀ :=
    realizedSol_solField_smallnessHorizon_Ha2 (I := I) (M := M) g₀ g_bg a (by omega) hT hT1
      hTT₀ u gforce hduh hforce hgforce htrace hR₀_pos
  have hRepr : ∀ (S : SmoothCcTensor g₀ 0 2) {δ' : ℝ} (hδ'_lt : δ' < 1)
      (hδ' : gFibreOpBound (I := I) (M := M) g₀
        (ccTensorBilinSymm (I := I) g₀ S) δ')
    ...
    rw [hsum_eq,
      ccTensorBilinSymm_toSection_congr R (deTurckRHSSectionBg (I := I) g_bg gDT)
        (by rw [hR_def, deTurckRHSSectionBg_toSection]) x v w]
    exact deTurckRHSSection_ccTensorBilinSymm_eq_deTurckRicciRHS (I := I) g_bg gDT x v w
  have hForceRepr_fam : ∀ {T₁ : ℝ} (hT₁_pos : 0 < T₁) (hT₁_le : T₁ ≤ T)
      (hT₁_le_d2F : T₁ ≤ d₂F)
      (F : ℝ → SmoothCcTensor g₀ 0 2) {δ : ℝ} (hδ_lt : δ < 1)
      (hδ : ∀ t : ℝ, gFibreOpBound (I := I) (M := M) g₀
        (ccTensorBilinSymm (I := I) g₀ (F t)) δ)
      ...
      ∀ t ∈ Set.Ico (0 : ℝ) T₁, ∀ i,
        f i t = tensorL2Coeff (I := I) (M := M)
            ...
            (SmoothCcTensor.toL2 (g := g₀) (r := 0) (s := 2)
              (deTurckSmoothRemainder (I := I) (M := M) g₀ g_bg (F t) hδ_lt (hδ t))) i := by
    intro T₁ hT₁_pos hT₁_le hT₁_le_d2F F δ hδ_lt hδ h_pin hball
    exact realizedForcingCoord_eq_smoothN (I := I) (M := M) g₀ g_bg a ha_super
      hT hT1 hTT₀ hT₁_pos hT₁_le hd₂F_pos hd₂F_le hT₁_le_d2F u gforce hduh hforce htrace
      F hδ_lt hδ f hf_id hf_smooth hf_mass hforce_coord h_pin hball
  exact maxreg_solution_jointly_smooth_representative_of_nemytskii (I := I) (M := M) g₀ a
    ha_super ha_eq (deTurckSobolevNHa2 (I := I) (M := M) g₀ g_bg a)
    (deTurckRicciRHS (I := I) g_bg) (deTurckSmoothRemainder (I := I) (M := M) g₀ g_bg)
    hRepr hT hT1 u htrace hd₂F_pos hd₂F_le f hf_smooth hf_mass hf_id hR₀_pos hHorizon
    hForceRepr_fam
