...
theorem deTurckSobolevNHa2_diff_sobolevSplit_perScale
  ...
  have hδ_fibre : ∀ (T₀ : SmoothCcTensor g₀ 0 2),
      ...
      DifferentialGeometry.PDE.RicciFlow.IntrinsicSpectral.MetricRealization.gFibreOpBound
        (I := I) (M := M) g₀
        (DifferentialGeometry.PDE.RicciFlow.IntrinsicSpectral.MetricRealization.ccTensorBilinSymm
          (I := I) g₀ T₀) (Classical.choose hex).2 :=
  ...
  set S := eigenIdxFinset (I := I) (M := M) g₀ N with hS
  set σ : ℝ := (a : ℝ) + (k : ℝ) with hσ
  set Tb : SmoothCcTensor g₀ 0 2 := finiteEigenCombo (I := I) (M := M) g₀ S (U N t) with hTb_def
  set T₀ : SmoothCcTensor g₀ 0 2 := radialScaleSmooth (I := I) (M := M) g₀ a R₀ Tb with hT₀_def
  have hball : ‖smoothCcToTensorHs (I := I) (M := M) g₀ ((a : ℝ) + 2) T₀‖ ≤ R₀ :=
    norm_smoothCcToTensorHs_radialScaleSmooth_le (I := I) (M := M) g₀ a hR₀.le Tb
  set c : ℝ := min 1 (R₀ / ‖smoothCcToTensorHs (I := I) (M := M) g₀ ((a : ℝ) + 2) Tb‖) with hc_def
  have hc_nn : 0 ≤ c := le_min zero_le_one (div_nonneg hR₀.le (norm_nonneg _))
  have hc_le_one : c ≤ 1 := min_le_left _ _
  have hsmul : T₀ = c • Tb := by rw [hT₀_def, hc_def]; rfl
  have hbridge : ∀ (τ : ℝ),
      finiteEigenComboHs (I := I) (M := M) g₀ S (U N t) τ =
        smoothCcToTensorHs (I := I) (M := M) g₀ τ Tb :=
    fun τ => by rw [hTb_def]; exact finiteEigenComboHs_eq_smoothCcToTensorHs g₀ S (U N t) τ
  ...
  have hsupp : ∀ i, i ∉ S →
    ...
    rw [hTb_coeff_off i hi, mul_zero]
  have hzero : (0 : SmoothCcTensor g₀ 0 2) = (0 : ℝ) • (0 : SmoothCcTensor g₀ 0 2) :=
    (zero_smul _ _).symm
  have hLHS_eq : ∀ i,
      (deTurckSobolevNHa2 (I := I) (M := M) g₀ g_bg a
            (finiteEigenComboHs (I := I) (M := M) g₀ S (U N t) ((a : ℝ) + 2))).coeff i -
          (deTurckSobolevNHa2 (I := I) (M := M) g₀ g_bg a
            (0 : tensorHs (I := I) (M := M) g₀ 0 2 ((a : ℝ) + 2))).coeff i =
      ...
      have hRemEq : ∀ (hp : DifferentialGeometry.PDE.RicciFlow.IntrinsicSpectral.MetricRealization.gFibreOpBound
              ...
              (DifferentialGeometry.PDE.RicciFlow.IntrinsicSpectral.MetricRealization.ccTensorBilinSymm
                (I := I) g₀ (radialScaleSmooth (I := I) (M := M) g₀ a R₀
                  (0 : SmoothCcTensor g₀ 0 2))) (Classical.choose hex).2)
            (hp0 : DifferentialGeometry.PDE.RicciFlow.IntrinsicSpectral.MetricRealization.gFibreOpBound
              (I := I) (M := M) g₀
              (DifferentialGeometry.PDE.RicciFlow.IntrinsicSpectral.MetricRealization.ccTensorBilinSymm
                (I := I) g₀ (0 : SmoothCcTensor g₀ 0 2)) (Classical.choose hex).2),
            deTurckSmoothRemainder (I := I) g₀ g_bg
  ...
  have hRHS_scale : ∀ (τ : ℝ) (i : TensorEigenIdx (I := I) (M := M) g₀ 0 2),
      (smoothCcToTensorHs (I := I) (M := M) g₀ τ T₀).coeff i =
        c * (finiteEigenComboHs (I := I) (M := M) g₀ S (U N t) τ).coeff i := by
    intro τ i
    rw [hsmul, smoothCcToTensorHs_smul]
    simp only [tensorHs.smul_coeff]
    rw [hbridge τ]
  have hkey := hsplit S k T₀ hball hsupp
  rw [← hσ] at hkey
  have hRHS1_le :
      Real.sqrt (∑ i ∈ S,
        tensorSobolevWeight (I := I) (M := M) i (σ + 1) *
          ((smoothCcToTensorHs (I := I) (M := M) g₀ (σ + 1) T₀).coeff i) ^ 2) ≤
      Real.sqrt (galerkinEnergy (I := I) (M := M) S (U N) (σ + 1) t) := by
    rw [galerkinEnergy]
    refine Real.sqrt_le_sqrt (Finset.sum_le_sum (fun i hi => ?_))
    rw [hRHS_scale (σ + 1) i, finiteEigenComboHs_coeff, if_pos hi, mul_pow]
    have hc2 : c ^ 2 ≤ 1 := by nlinarith [hc_nn, hc_le_one]
    have hwnn := tensorSobolevWeight_nonneg (I := I) (M := M) i (σ + 1)
    have hsqnn : 0 ≤ (U N t i) ^ 2 := sq_nonneg _
    nlinarith [mul_nonneg hwnn hsqnn, hc2, hwnn, hsqnn,
      mul_le_mul_of_nonneg_right hc2 (mul_nonneg hwnn hsqnn)]
  have hRHS2_le :
      Real.sqrt (∑ i ∈ S,
        tensorSobolevWeight (I := I) (M := M) i σ *
          ((smoothCcToTensorHs (I := I) (M := M) g₀ σ T₀).coeff i) ^ 2) ≤
      Real.sqrt (galerkinEnergy (I := I) (M := M) S (U N) σ t) := by
    rw [galerkinEnergy]
    refine Real.sqrt_le_sqrt (Finset.sum_le_sum (fun i hi => ?_))
    rw [hRHS_scale σ i, finiteEigenComboHs_coeff, if_pos hi, mul_pow]
    have hc2 : c ^ 2 ≤ 1 := by nlinarith [hc_nn, hc_le_one]
    have hwnn := tensorSobolevWeight_nonneg (I := I) (M := M) i σ
    have hsqnn : 0 ≤ (U N t i) ^ 2 := sq_nonneg _
    nlinarith [mul_nonneg hwnn hsqnn, hc2, hwnn, hsqnn,
      mul_le_mul_of_nonneg_right hc2 (mul_nonneg hwnn hsqnn)]
  have hLHS_rw :
  ...
  exact add_le_add (mul_le_mul_of_nonneg_left hRHS1_le hCδ₀_nn)
    (mul_le_mul_of_nonneg_left hRHS2_le (hCrem_nn k))
...
theorem deTurckGalerkinForcing_tame_diff_mass_perScale
  ...
  classical
  obtain ⟨Cδ₀, Crem, hCδ₀_nn, hCδ₀_lt, hCrem_nn, hsplit⟩ :=
    deTurckSobolevNHa2_diff_sobolevSplit_perScale (I := I) (M := M) g₀ g_bg a ha_super U
  refine ⟨Real.sqrt ((1 + Cδ₀ ^ 2) / 2),
    fun k => Real.sqrt (Crem k ^ 2 * (1 + Cδ₀ ^ 2) / (1 - Cδ₀ ^ 2)),
    Real.sqrt_nonneg _, ?_, fun k => Real.sqrt_nonneg _, ?_⟩
  · have hc2_lt : Cδ₀ ^ 2 < 1 := by nlinarith [hCδ₀_nn, hCδ₀_lt]
    have hlt : (1 + Cδ₀ ^ 2) / 2 < 1 := by linarith [hc2_lt]
    calc Real.sqrt ((1 + Cδ₀ ^ 2) / 2) < Real.sqrt 1 :=
          Real.sqrt_lt_sqrt (by positivity) hlt
      _ = 1 := Real.sqrt_one
  · intro N k t ht
    set A := ∑ i ∈ eigenIdxFinset (I := I) (M := M) g₀ N,
        tensorSobolevWeight (I := I) (M := M) i ((a : ℝ) + (k : ℝ) - 1) *
          ((deTurckSobolevNHa2 (I := I) (M := M) g₀ g_bg a
              (finiteEigenComboHs (I := I) (M := M) g₀
                (eigenIdxFinset (I := I) (M := M) g₀ N) (U N t) ((a : ℝ) + 2))).coeff i -
            (deTurckSobolevNHa2 (I := I) (M := M) g₀ g_bg a
              (0 : tensorHs (I := I) (M := M) g₀ 0 2 ((a : ℝ) + 2))).coeff i) ^ 2 with hA_def
    set B := galerkinEnergy (I := I) (M := M)
      (eigenIdxFinset (I := I) (M := M) g₀ N) (U N) ((a : ℝ) + (k : ℝ) + 1) t with hB_def
    set C := galerkinEnergy (I := I) (M := M)
      (eigenIdxFinset (I := I) (M := M) g₀ N) (U N) ((a : ℝ) + (k : ℝ)) t with hC_def
    have hA_nn : 0 ≤ A := by
      refine Finset.sum_nonneg (fun i _ => ?_)
      exact mul_nonneg (tensorSobolevWeight_nonneg (I := I) (M := M) i _) (sq_nonneg _)
    have hB_nn : 0 ≤ B := galerkinEnergy_nonneg (I := I) (M := M) _ _ _ _
    have hC_nn : 0 ≤ C := galerkinEnergy_nonneg (I := I) (M := M) _ _ _ _
    have hsqrt_split : Real.sqrt A ≤ Cδ₀ * Real.sqrt B + Crem k * Real.sqrt C :=
      hsplit N k t
    have hmass := mass_le_of_sqrt_split hA_nn hB_nn hC_nn hCδ₀_nn hCδ₀_lt (hCrem_nn k)
      hsqrt_split
    have hc2_lt : Cδ₀ ^ 2 < 1 := by nlinarith [hCδ₀_nn, hCδ₀_lt]
    have hden_pos : 0 < 1 - Cδ₀ ^ 2 := by linarith [hc2_lt]
    have hsqB : Real.sqrt ((1 + Cδ₀ ^ 2) / 2) ^ 2 = (1 + Cδ₀ ^ 2) / 2 :=
      Real.sq_sqrt (by positivity)
    have hsqC : Real.sqrt (Crem k ^ 2 * (1 + Cδ₀ ^ 2) / (1 - Cδ₀ ^ 2)) ^ 2 =
        Crem k ^ 2 * (1 + Cδ₀ ^ 2) / (1 - Cδ₀ ^ 2) := by
      refine Real.sq_sqrt ?_
      have hnum_nn : 0 ≤ Crem k ^ 2 * (1 + Cδ₀ ^ 2) := by positivity
      exact div_nonneg hnum_nn hden_pos.le
    rw [hsqB, hsqC]
    exact hmass
    ...
    ∃ (Cδ : ℝ) (Cmid seed B0 : ℕ → ℝ), Cδ < 2 ∧ (∀ k, 0 ≤ Cmid k) ∧
      ...
      (∀ (N : ℕ) (k : ℕ),
        galerkinEnergy (I := I) (M := M) (eigenIdxFinset (I := I) (M := M) g₀ N)
          (U N) ((a : ℝ) + (k : ℝ)) 0 ≤ B0 k) := by
  obtain ⟨Cδ, Cmid, seed, hCδ, hCmid, hdiss⟩ :=
    deTurckGalerkin_forcing_dissipation_perScale (I := I) (M := M) g₀ g_bg a ha_super
      (T := T) U
  refine ⟨Cδ, Cmid, seed, fun _ => 0, hCδ, hCmid, hdiss, ?_⟩
  intro N k
  have hzero : galerkinEnergy (I := I) (M := M) (eigenIdxFinset (I := I) (M := M) g₀ N)
...
theorem deTurckGalerkin_energy_uniform_bound
  ...
  have hinit_zero : ∀ N, ∀ i ∈ eigenIdxFinset (I := I) (M := M) g₀ N, U N 0 i = 0 := by
    intro N i hi
    rw [hinit_coeff N i hi]; rfl
  obtain ⟨Cδ, Cmid, seed, B0, hCδ, hCmid, hclosure, hinit⟩ :=
    deTurckGalerkin_forcing_closure_perScale (I := I) (M := M) g₀ g_bg a ha_super
      (T := T) U hinit_zero
  refine ⟨U, hinit_zero, ?_⟩
  have hkey := galerkin_energy_uniform_bound_perScale (I := I) (M := M) (g := g₀)
    (U := U)
    (Fseq := deTurckGalerkinForcing (I := I) (M := M) g₀ g_bg a U)
    (sseq := eigenIdxFinset (I := I) (M := M) g₀)
    (T := T) (σ₀ := (a : ℝ)) (Cδ := Cδ) (Cmid := Cmid) (seed := seed) (B0 := B0)
    hCδ hCmid hcont hderiv hclosure hinit
  exact hkey
