...
by fibre over `M`: chart by chart in the Euclidean reading
(`chartE_euclideanReading_contDiffOn` → `borel_interval_extend_param`), lifted back to a local
tangent-bundle section over each chart (`lift_extended_reading_contMDiffOn`), and patched globally
by a smooth partition of unity (`partitionOfUnity_assembled_section_contMDiff`).  The file is
`sorry`-free.
-/
