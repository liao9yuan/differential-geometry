...
theorem fromZero_bare_flow_eqOn_of_weakDatum [CompactSpace M]
  ...
  classical
  set α : M := γ₁ 0 with hα
  obtain ⟨a, δ₀, K, ha_pos, hδ₀_pos, _hδ₀_le, hdata⟩ :=
    ...
    (hgrad0 : ∀ α : M,
      ContinuousOn
        (fun q : ℝ × M =>
          fderiv ℝ (chartRawRepr (I := I) α (X q.1)) (extChartAt I α q.2))
        (Set.Icc (0 : ℝ) T ×ˢ Set.univ))
    (Φ Φ' : ℝ → M → M) (x x' : M) {δ : ℝ} (hδ : 0 < δ)
    (hstart : Φ 0 x = Φ' 0 x')
    (hflow : ∀ t ∈ Set.Ico (0:ℝ) δ,
      HasMFDerivWithinAt 𝓘(ℝ, ℝ) I (fun u : ℝ => Φ u x) (Set.Ici 0) t
        ((1 : ℝ →L[ℝ] ℝ).smulRight (X t (Φ t x))))
    (hflow' : ∀ t ∈ Set.Ico (0:ℝ) δ,
      HasMFDerivWithinAt 𝓘(ℝ, ℝ) I (fun u : ℝ => Φ' u x') (Set.Ici 0) t
        ((1 : ℝ →L[ℝ] ℝ).smulRight (X t (Φ' t x')))) :
    ∃ δ' : ℝ, 0 < δ' ∧ ∀ t ∈ Set.Icc (0:ℝ) δ', Φ t x = Φ' t x' := by
  obtain ⟨δ', hδ'_pos, heqOn⟩ :=
    fromZero_bare_flow_eqOn_of_weakDatum (I := I) X T hT hint hcont0 hgrad0
      (fun u => Φ u x) (fun u => Φ' u x') hδ hstart hflow hflow'
  exact ⟨δ', hδ'_pos, fun t ht => heqOn ht⟩
