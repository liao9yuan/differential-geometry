...
manifold), with `Φ0 0 = id`, the bare manifold velocity on `(0, σ)`, and the **trivialised**
chart integral identity
`extChartAt α (Φ0 s x) = extChartAt α x + ∫₀ˢ chartTrivRepr α (X r) (extChartAt α (Φ0 r x))`
on each orbit's right-half neighbourhood of `0`.  Every conclusion is stated through the
corrected `chartTrivRepr` field or the intrinsic bare velocity — never the raw chart value.
-/
...
flow `Φ0` on a uniform horizon `σ`, with `Φ0 0 = id`, bare velocity on `(0, σ)`, and the
trivialised chart integral identity on each orbit's right-half neighbourhood of `0`. -/
    ...
    ∃ (σ : ℝ) (_ : 0 < σ) (Φ0 : ℝ → M → M),
      ...
      (∀ x : M, ∃ α : M, ∃ δ : ℝ, 0 < δ ∧ x ∈ (chartAt H α).source ∧
        ∀ s ∈ Set.Ico (0 : ℝ) (min δ σ), Φ0 s x ∈ (chartAt H α).source ∧
          extChartAt I α (Φ0 s x) = extChartAt I α x + ∫ r in (0 : ℝ)..s, chartTrivRepr (I := I) α (X r) (extChartAt I α (Φ0 r x))) := sorry
