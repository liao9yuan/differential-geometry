...
and proves the initial-condition and ODE clauses on a closed sub-interval
around `h₀` (short Picard interval, the elementary building block for the
global theory).

## Main definitions

* `HasLinearODESolution A h₀ Z₀ x` — per-parameter existence predicate:
  there is a curve `Z : ℝ → G` satisfying `Z h₀ = Z₀ x` and the ODE on
  all of `ℝ`.
* `linearODESolution A h₀ Z₀ : F → ℝ → G` — the parametric solution.  Defined
  unconditionally via `Classical.choose`: returns *the* global solution when
...
* `linearODESolution_init` — unconditional: `linearODESolution A h₀ Z₀ x h₀ = Z₀ x`.
* `linearODESolution_hasDerivAt_of_hasSolution` — ODE clause under the
  `HasLinearODESolution` hypothesis.

## Status (this substep)

The two foundational building blocks `exists_linearODE_solution_of_short`
(local Picard) and `linearODE_unique_on_Ioo` (Grönwall uniqueness on `Ioo`)
are unconditional.  The solution operator `linearODESolution` is defined
unconditionally and the init clause is unconditional.

The ODE clause is proved conditional on the per-parameter global-existence
predicate `HasLinearODESolution`.  Discharging this predicate via patching
of finitely many short Picard steps on each closed sub-interval of `Ioo a b`
is the content of the follow-up substep `PARAM.LIN.def.global`.
...
The parametric solution `linearODESolution A h₀ Z₀ : F → ℝ → G` is defined
unconditionally via `Classical.choose`.  For each `x : F`, the per-parameter
existence statement `HasLinearODESolution A h₀ Z₀ x` provides a curve when it
holds; otherwise the definition falls back to the constant function
`fun _ => Z₀ x`.
...
Discharging `HasLinearODESolution` from open-interval continuity hypotheses
(`hab_lt`, `h₀_mem`, `hA_cont` jointly on `U ×ˢ Ioo a b`) is the content of
the follow-up substep `PARAM.LIN.def.global`, which assembles
`exists_linearODE_solution_of_short` over a finite cover of each closed
sub-interval `Icc α β ⊂ Ioo a b`.
-/
...
/--
**Per-parameter existence predicate.**  `HasLinearODESolution A h₀ Z₀ x` asserts
that for the fixed parameter `x : F`, there exists a curve `Z : ℝ → G` with
`Z h₀ = Z₀ x` and `HasDerivAt Z (A x t (Z t)) t` for all `t : ℝ`.

This predicate is used internally by `linearODESolution` to decide whether to
return the chosen global solution or the constant fallback.
-/
def HasLinearODESolution
    (A : F → ℝ → (G →L[ℝ] G)) (h₀ : ℝ) (Z₀ : F → G) (x : F) : Prop :=
  ∃ Z : ℝ → G, Z h₀ = Z₀ x ∧ ∀ t : ℝ, HasDerivAt Z (A x t (Z t)) t
...
* If a global solution exists for the parameter `x` (predicate
  `HasLinearODESolution`), `linearODESolution A h₀ Z₀ x` is *the* global
  solution, chosen via `Classical.choose`.
...
noncomputable def linearODESolution
  ...
  classical
  exact fun x =>
    if h : HasLinearODESolution A h₀ Z₀ x then
      Classical.choose h
    else
      fun _ => Z₀ x
...
theorem linearODESolution_init
    (A : F → ℝ → (G →L[ℝ] G)) (h₀ : ℝ) (Z₀ : F → G) (x : F) :
    linearODESolution A h₀ Z₀ x h₀ = Z₀ x := by
  unfold linearODESolution
  by_cases h : HasLinearODESolution A h₀ Z₀ x
  · simp only [dif_pos h]
    exact (Classical.choose_spec h).1
  · simp only [dif_neg h]

/--
**ODE clause** for `linearODESolution` under the per-parameter
global-existence hypothesis.

When `HasLinearODESolution A h₀ Z₀ x` holds, the parametric solution at `x`
satisfies the linear ODE pointwise on all of `ℝ`.
-/
theorem linearODESolution_hasDerivAt_of_hasSolution
    (A : F → ℝ → (G →L[ℝ] G)) (h₀ : ℝ) (Z₀ : F → G)
    {x : F} (hx : HasLinearODESolution A h₀ Z₀ x) (t : ℝ) :
    HasDerivAt (linearODESolution A h₀ Z₀ x ·)
      (A x t (linearODESolution A h₀ Z₀ x t)) t := by
  have hZ_eq : linearODESolution A h₀ Z₀ x = Classical.choose hx := by
    unfold linearODESolution
    simp only [dif_pos hx]
  rw [hZ_eq]
  exact (Classical.choose_spec hx).2 t
