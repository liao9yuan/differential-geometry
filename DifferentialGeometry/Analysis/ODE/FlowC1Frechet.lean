...
/-! ## Auxiliary: Grönwall bound estimate `gronwallBound 0 K ε x ≤ ε · x · exp (K · x)` -/
...
/-! ## Uniform continuity of the partial derivative near the central orbit

The set `Icc (t₀ - T) (t₀ + T) ×ˢ ((fun s => Φ ⟨x₀, s⟩) '' Icc (t₀ - T) (t₀ + T))` is compact.
By `IsCompact.uniformContinuousAt_of_continuousAt`, the partial Fréchet derivative
`(τ, z) ↦ fderiv ℝ (f τ) z` is uniformly continuous at this compact set in `ℝ × E`. -/
...
lemma exists_uniform_partial_fderiv_of_contDiffOn_univ
  ...
  -- The compact "graph": `K_graph := { (τ, α τ) | τ ∈ Icc a b }`.
  ...
  -- F := fun p => fderiv (f p.1) p.2, continuous on univ.
  ...
  -- The uniformity entourage from `ε`.
  ...
  refine ⟨δ, hδ_pos, ?_⟩
  intro τ hτ z hz
  -- Apply hδ at p₁ := (τ, α τ) ∈ K_graph and p₂ := (τ, z).
  set p₁ : ℝ × E := (τ, α τ)
  set p₂ : ℝ × E := (τ, z)
  ...
  -- `F p₁ = fderiv ℝ (f τ) (α τ)`, `F p₂ = fderiv ℝ (f τ) z`.
...
/-! ## Main theorem: Fréchet derivative of the flow -/
...
theorem hasFDerivAt_flow_at_initial_of_isLocalFlow
  ...
  -- Uniform bound on `‖fderiv (f τ) z‖` for `z` near the orbit.  Apply the helper
  -- with `ε := 1` to get a `δ₁ > 0` such that for `z` within `δ₁` of the orbit
  -- (uniformly in `τ ∈ Icc (t₀-T) (t₀+T)`), `‖fderiv (f τ) z‖ ≤ M + 1`.
  obtain ⟨δ_K, hδ_K_pos, hδ_K⟩ :=
    exists_uniform_partial_fderiv_of_contDiffOn_univ hf_C1
      (α := fun s => Φ ⟨x₀, s⟩) horbit_cont' 1 one_pos
  -- We choose `δ_K_strict := δ_K / 2` to get a `≤`-bound.
  ...
  -- Gronwall factor on [t₀, t₀+T]: `T · exp(K_lip · T)`.  Also on the left half by symmetry.
  set GfactorR : ℝ := T * exp (K_lip * T) with hGfactorR_def
  have hGfactorR_pos : 0 < GfactorR := by
    have : 0 < exp (K_lip * T) := exp_pos _
    have hT_pos : 0 < T := hT
    positivity
  -- We pick `ε_target` so that the final bound is `< c · ‖h‖`.
  ...
  -- Choose final `δ`.  Constraints:
  --   (a) `‖h‖ ≤ r` (so `x₀ + h ∈ closedBall x₀ r`)
  --   (b) `L · ‖h‖ < δ_K_strict` (so α_h-orbit stays in K_lip-Lipschitz ball)
  --   (c) `CE · ‖h‖ < δ_K_strict` (so β_h-orbit stays in K_lip-Lipschitz ball)
  --   (d) `CE · ‖h‖ < δ_uc` (so segment is within δ_uc, gives ε_target bound)
  ...
  -- Generic helper: from ‖h‖ < target/scale and coef ≤ scale (with 0 < scale, 0 ≤ coef),
  -- conclude coef * ‖h‖ < target.
  ...
  We use a *small* convex set `closedBall (Φ⟨x₀,s⟩) (CE · ‖h‖)` containing both
  `Φ⟨x₀,s⟩` and `β_h s = Φ⟨x₀,s⟩ + y_h s`.  Points in it are within `CE · ‖h‖ < δ_uc`
  of the central orbit, so the variation of the partial derivative is `< ε_target`. -/
  have h_residual_bound : ∀ s ∈ Icc (t₀ - T) (t₀ + T),
      ‖f s (Φ ⟨x₀, s⟩) + A s (y_h s) - f s (β_h s)‖ ≤ ε_target * ‖y_h s‖ := by
    intro s hs
    -- Use the small ball `closedBall (Φ⟨x₀,s⟩) (CE * ‖h‖)`.
    ...
    -- Now rewrite the LHS.  Recall `A s = fderiv ℝ (f s) (Φ⟨x₀,s⟩)` and `β_h s = Φ⟨x₀,s⟩ + y_h s`.
  ...
  -- For the Gronwall application, we use `dist_le_of_approx_trajectories_ODE_of_mem` with
  -- `v t := f t`, `s t := closedBall (Φ⟨x₀, t⟩) δ_K_strict`, initial time `a := t₀`, terminal `b := t₀+T`.
  ...
  have h_diff_right : ∀ τ ∈ Icc t₀ (t₀ + T),
    ...
    intro τ hτ
    -- Strategy: apply `dist_le_of_approx_trajectories_ODE_of_mem`.
    ...
    -- HasDerivWithinAt for α_h on (Ici τ) for τ ∈ Ico t₀ (t₀+T).
    ...
    -- HasDerivWithinAt for β_h on (Ici τ).
    ...
    -- `gronwallBound 0 K_lip (0 + εg) (τ - t₀) ≤ εg · T · exp(K_lip · T) = GfactorR · εg`.
    rw [dist_eq_norm] at hG
    have h_zero_add : (0 : ℝ) + εg = εg := zero_add _
    rw [h_zero_add] at hG
    -- `gronwallBound 0 K_lip εg (τ - t₀) ≤ εg * (τ - t₀) * exp(K_lip * (τ - t₀))`.
  ...
  Reflect time: `ψ(s) := 2 t₀ - s` maps `[t₀, t₀+T]` to `[t₀-T, t₀]`.  Define
  `αR(s) := α_h (2 t₀ - s)`, `βR(s) := β_h (2 t₀ - s)`.  Their derivatives become negated,
  so they satisfy the reflected ODE with vector field `vR τ x := -f (2 t₀ - τ) x`. -/
  have h_diff_left : ∀ τ ∈ Icc (t₀ - T) t₀,
    ...
    -- The reflection ψ has derivative -1.
    have hψ_deriv : ∀ s, HasDerivAt ψ (-1 : ℝ) s := fun s => by
      ...
      exact h1
    -- vR τ is K_lip-Lipschitz on sR τ for τ ∈ Ico t₀ (t₀+T).
    ...
    -- Continuity of αR and βR on Icc t₀ (t₀+T).
    ...
    -- HasDerivWithinAt for αR on (Ici τ').
    have hαR_deriv_R : ∀ τ' ∈ Ico t₀ (t₀ + T),
      ...
      have hτ'_Ioc_full : 2 * t₀ - τ' ∈ Ioc tmin tmax := by
        refine ⟨?_, hτ'_full.2⟩
        -- We need (tmin < 2 t₀ - τ').  Since `τ' < t₀+T`, `2t₀-τ' > t₀-T`.  And `tmin ≤ t₀-T`
        -- from the assumed inclusion `Icc (t₀-T) (t₀+T) ⊆ Icc tmin tmax`.
      ...
      -- h_d : HasDerivWithinAt α_h (f (2t₀-τ') (α_h (2t₀-τ'))) (Icc tmin tmax) (2 t₀ - τ')
      have h_d_left :
      ...
      -- `h_comp : HasDerivWithinAt (α_h ∘ ψ) (-1 • f (2*t₀-τ') (α_h (2*t₀-τ'))) (Ici τ') τ'`.
      ...
      exact h_comp
    -- HasDerivWithinAt for βR on (Ici τ').
    have hβR_deriv_R : ∀ τ' ∈ Ico t₀ (t₀ + T),
      ...
      have h_comp := HasDerivWithinAt.scomp (g₁ := β_h) (h := ψ)
        τ' h_d_left hψ_dwa hψ_maps'
      -- h_comp : HasDerivWithinAt (β_h ∘ ψ) (-1 • (f .. + A .. y_h ..)) (Ici τ') τ'
      have h_simplify :
    ...
    -- Time to apply.  We want to bound `dist (αR s) (βR s)` for `s := 2 t₀ - τ`, with τ ∈ Icc (t₀-T) t₀.
    ...
    have h_zero_add : (0 : ℝ) + εg = εg := zero_add _
    rw [h_zero_add] at hG
    -- αR s_eval = α_h (2 t₀ - s_eval) = α_h τ.  Same for βR.
  ...
  -- On Icc (t₀-T) (t₀+T), ‖α_h - β_h‖ ≤ GfactorR · ε_target · CE · ‖h‖.
