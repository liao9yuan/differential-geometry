...
/-- The chosen global flow of a complete smooth vector field is jointly smooth
in time and its initial point. -/
theorem curveAt_contMDiff [FiniteDimensional ℝ E] [I.Boundaryless]
...
/-- Evaluation at a fixed time of the chosen global flow, as a smooth
diffeomorphism whose inverse is evaluation at the opposite time. -/
noncomputable def curveAtDiffeo [FiniteDimensional ℝ E] [I.Boundaryless]
    [IsManifold I ∞ M] [T2Space M]
    (v : (x : M) → TangentSpace I x)
    (hv : ContMDiff I I.tangent ∞
      (fun x : M ↦ (⟨x, v x⟩ : TangentBundle I M)))
  ...
  have hsmooth (s : ℝ) :
    ...
    simpa [Function.comp_def] using
      (curveAt_contMDiff v hv hcomplete).comp hpair
  exact
    { toEquiv :=
