...
class AffineTensorCalculus (conn : AbstractAffineConnection R V) where
  ...
  nabla_scalar : ∀ (X : V) (f : R),
    nabla_tensor X (TensorAlgebra.fromScalar f) = TensorAlgebra.fromScalar (action X f)

  /-- Axiom 2: Degenerates to the affine connection on vector fields (1-0 tensors) -/
  nabla_vector : ∀ (X Y : V),
    nabla_tensor X (TensorAlgebra.fromVector Y) = TensorAlgebra.fromVector (conn.nabla X Y)
