...
private lemma mfderiv_basis_eq
    (f : E → M) {x : E}
    (hf : MDifferentiableAt (modelWithCornersSelf ℝ E) I f x)
  ...
  have hchart : MDifferentiableAt I (modelWithCornersSelf ℝ E)
      (extChartAt I y₀) (f x) :=
...
private lemma mapGram_det_eq
  ...
  have hmul :
    ...
    have hsum :
      ...
      rw [mfderiv_basis_eq (I := I) f hf y₀ hy i,
        mfderiv_basis_eq (I := I) f hf y₀ hy j]
      have hL :
          g.inner (f x)
...
omit [NeZero (Module.finrank ℝ E)] [T2Space M] [SigmaCompactSpace M] in
private lemma mapJac_eq_chart_eq
    (g : SmoothRiemannianMetric I M)
    (f : E → M) {x : E}
    (hf : MDifferentiableAt (modelWithCornersSelf ℝ E) I f x)
    (y₀ : M) (hy : f x ∈ (trivializationAt E (TangentSpace I) y₀).baseSet) :
    mapJacDensity (I := I) g f x =
      chartDensity g y₀ (f x) *
        |(fderiv ℝ (fun z : E => extChartAt I y₀ (f z)) x).det| := by
  rw [mapJacDensity, mapGram_det_eq (I := I) g f hf y₀ hy]
  unfold chartDensity
...
private lemma mapJac_contOn_eq
  ...
  have hVopen : IsOpen V := by
    dsimp only [V]
    exact hf.continuousOn.isOpen_inter_preimage hU
      (chartAt H y₀).open_source
  have hVU : V ⊆ U := by
    dsimp only [V]
  ...
  have hVchart : ∀ z ∈ V,
    ...
    intro z hz
    simpa only [V, trivializationAt_baseSet_eq_chartAt_source (I := I)]
      using hz.2
  have hxV : (x : E) ∈ V := by
    exact ⟨x.2, by simpa only [y₀] using mem_chart_source H (f x)⟩
  have hcoord : ContDiffOn ℝ 1
      (fun z : E => extChartAt I y₀ (f z)) V :=
    mapChart_contDiffOn_eq (I := I) hf hVU y₀ (fun z hz => by
      simpa only [trivializationAt_baseSet_eq_chartAt_source (I := I)]
        using hVchart z hz)
  have hderiv : ContinuousOn
      (fun z : E => fderiv ℝ (fun w : E => extChartAt I y₀ (f w)) z) V :=
    hcoord.continuousOn_fderiv_of_isOpen hVopen le_rfl
  have hdet : ContinuousOn
      (fun z : E =>
        |(fderiv ℝ (fun w : E => extChartAt I y₀ (f w)) z).det|) V :=
    (ContinuousLinearMap.continuous_det.comp_continuousOn hderiv).abs
  have hfdens : ContinuousOn (fun z => chartDensity g y₀ (f z)) V :=
    (chartDensity_continuousOn (I := I) g y₀).comp
      ((hf.mono hVU).continuousOn) hVchart
  have heq : ∀ z ∈ V,
      mapJacDensity (I := I) g f z =
        chartDensity g y₀ (f z) *
          |(fderiv ℝ (fun w : E => extChartAt I y₀ (f w)) z).det| := by
    intro z hz
    have hmdiff : MDifferentiableAt (modelWithCornersSelf ℝ E) I f z :=
      (((hf.mono hVU) z hz).contMDiffAt
        (hVopen.mem_nhds hz)).mdifferentiableAt (by norm_num)
    exact mapJac_eq_chart_eq (I := I) g f hmdiff y₀ (hVchart z hz)
  have hcontV : ContinuousOn (mapJacDensity (I := I) g f) V :=
    (hfdens.mul hdet).congr heq
...
private lemma pou_term_map_eq
  ...
  have hcoord : ContDiffOn ℝ 1 (fun x : E => extChartAt I a (f x)) V :=
    mapChart_contDiffOn_eq (I := I) hf hVU a hVchart
  ...
  calc
    ...
    _ = ∫⁻ x in Ka,
      ...
      have hbase :
        ...
        exact hfxS
      have hjac := mapJac_eq_chart_eq (I := I) g f hmdiff a hbase
      have hcdnn : 0 ≤ chartDensity g a (f x) := Real.sqrt_nonneg _
      rw [hWat,
        ← ENNReal.ofReal_mul
...
omit [NeZero (Module.finrank ℝ E)] in
/-- For a map that is `C¹` on an open neighborhood of a measurable set and
injective on that set, the Riemannian volume of its image is the integral of
its Gram-Jacobian density. -/
theorem riemVol_image_eq
    (g : SmoothRiemannianMetric I M) {f : E → M} {U K : Set E}
    (hU : IsOpen U) (hK : MeasurableSet K) (hKU : K ⊆ U)
    (hf : ContMDiffOn (modelWithCornersSelf ℝ E) I 1 f U)
    (hinj : Set.InjOn f K) :
  ...
  have himg : MeasurableSet (f '' K) :=
    hK.image_of_continuousOn_injOn hfK hinj
  have hJacK : ContinuousOn (mapJacDensity (I := I) g f) K :=
    (mapJac_contOn_eq (I := I) g hU hf).mono hKU
  have hsm : ∀ a : M, AEMeasurable (fun x : E =>
    ...
    have hpou : ContinuousOn
        (fun x : E => chartAtlasPOU I M a (f x)) K :=
  ...
  let T : Set M :=
    {a : M | (Function.support (chartAtlasPOU I M a)).Nonempty}
  have hT_count : T.Countable :=
    countable_nonempty_support_of_pou (I := I) (chartAtlasPOU I M)
  ...
  have hsupp : Function.support (fun a : M => ∫⁻ x in K,
    ...
    have hpou_zero : ∀ x : E, chartAtlasPOU I M a (f x) = 0 := by
      intro x
      by_contra hne
      have hx : f x ∈ Function.support (chartAtlasPOU I M a) := hne
      rw [hzero] at hx
      exact hx
  ...
  calc
    ∑' a : M,
      ...
      ∑' a : M, ∫⁻ x in K,
        ...
        apply tsum_congr
        intro a
        exact pou_term_map_eq (I := I) g hU hK hKU hf hinj himg a
    _ = ∑' a : T, ∫⁻ x in K,
        ENNReal.ofReal (chartAtlasPOU I M a.val (f x)) *
          ENNReal.ofReal (mapJacDensity (I := I) g f x)
          ∂(modelHaar (E := E)) :=
    ...
    _ = ∫⁻ x in K, ∑' a : T,
        ENNReal.ofReal (chartAtlasPOU I M a.val (f x)) *
          ENNReal.ofReal (mapJacDensity (I := I) g f x)
          ∂(modelHaar (E := E)) := by
      rw [lintegral_tsum (fun a : T => hsm a.val)]
    _ = ∫⁻ x in K,
        ENNReal.ofReal (mapJacDensity (I := I) g f x)
          ∂(modelHaar (E := E)) := by
