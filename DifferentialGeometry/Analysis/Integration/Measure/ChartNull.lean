...
/-- A compact charted space is covered by finitely many canonical chart sources. -/
...
/-- A set covered by finitely many chart pieces is null when every such piece is null. -/
...
/-- A Euclidean null set in one chart pulls back to a Riemannian-volume null set
...
/-- A model-coordinate null set in one chart pulls back to a Riemannian-volume
null set on the chart source. -/
