...
/-- A locally Lipschitz map on a finite-dimensional set is differentiable
within that set almost everywhere. -/
...
/-- In one manifold chart, a chart-Lipschitz real-valued function is
...
theorem chart_nondiff_null
  ...
  have hset :
    ...
    ext x
    simp only [Set.mem_inter_iff, Set.mem_setOf_eq, N]
...
/-- In one manifold chart, a chart-locally-Lipschitz real-valued function is
manifold-differentiable outside a Riemannian-volume null set. -/
...
/-- On a compact manifold, a real-valued function that is locally Lipschitz in
every canonical chart is manifold-differentiable outside a Riemannian-volume
null set. -/
