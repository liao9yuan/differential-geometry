...
/-- Integrability against `volumeIoiPow d` is weighted Lebesgue integrability
...
/-- Integration against `volumeIoiPow d` is weighted Lebesgue integration on
...
/-- A radial indicator against `volumeIoiPow d` is weighted Lebesgue
integration over its measurable positive-radius support. -/
theorem integral_ioiPow_set (d : ℕ) (S : Set ℝ) (hS : MeasurableSet S)
  ...
  calc
    (∫ r in Ioi (0 : ℝ), r ^ d • S.indicator F r) =
      ...
      apply setIntegral_eq_of_subset_of_forall_diff_eq_zero measurableSet_Ioi hS0
      intro r hr
...
/-- Polar-coordinate evaluation of a signed or vector-valued integral, before
applying Fubini. -/
...
/-- An integrable function remains integrable after transport to polar
coordinates. -/
...
/-- Fubini form of polar integration for an integrable signed or
...
/-- Polar-coordinate evaluation of a signed or vector-valued integral
...
/-- Polar-coordinate evaluation of a nonnegative integral restricted to a
measurable set. -/
