...
/-- Weighted integration by parts with an upper bound for the weight
derivative and a favorable oriented boundary term. -/
theorem neg_mul_deriv_le
...
/-- Weighted integration by parts on an initial open interval when the test is
supported strictly inside a compact subinterval. -/
theorem neg_mul_deriv_le_on_initial
    {a c b : ℝ} (ha : 0 < a) (hac : a ≤ c) (hc : c < b)
    {J ψ h : ℝ → ℝ}
    (hJ : AbsolutelyContinuousOnInterval J a c)
    (hψ : AbsolutelyContinuousOnInterval ψ a c)
    (hψsupp : tsupport ψ ⊆ Ioo a c)
    (hψ0 : ∀ᵐ t ∂volume.restrict (Ioo (0 : ℝ) b), 0 ≤ ψ t)
    (hderiv : ∀ᵐ t ∂volume.restrict (Ioo (0 : ℝ) b),
      deriv J t ≤ h t * J t)
    (hint : IntegrableOn (fun t => h t * J t * ψ t) (Ioo (0 : ℝ) b)) :
    -(∫ t in Ioo (0 : ℝ) b, J t * deriv ψ t) ≤
      ∫ t in Ioo (0 : ℝ) b, h t * J t * ψ t := by
  have hsub : Icc a c ⊆ Ioo (0 : ℝ) b := by
    rintro t ⟨hat, htc⟩
  ...
  have hfinite :
    ...
    neg_mul_deriv_le hac hJ hψ (by simp only [hψa, hψc, mul_zero, le_rfl])
      hψ0' hderiv' hint'
  have hleft :
      (∫ t in Ioo (0 : ℝ) b, J t * deriv ψ t) =
    ...
    calc
      (∫ t in Ioo (0 : ℝ) b, J t * deriv ψ t) =
        ...
        apply setIntegral_eq_of_subset_of_forall_diff_eq_zero measurableSet_Ioo hsub
        intro t ht
...
/-- Weighted integration by parts on `(0, b)` when the test vanishes only near
the initial endpoint and the terminal boundary term is nonnegative. -/
theorem neg_mul_deriv_le_Ioo
    {a b : ℝ} (ha : 0 < a) (hab : a ≤ b) {J ψ h : ℝ → ℝ}
    (hJ : AbsolutelyContinuousOnInterval J a b)
    (hψ : AbsolutelyContinuousOnInterval ψ a b)
    (hψzero : EqOn ψ 0 (Ioc (0 : ℝ) a))
    (hbdry : 0 ≤ J b * ψ b)
    (hψ0 : ∀ᵐ t ∂volume.restrict (Ioo (0 : ℝ) b), 0 ≤ ψ t)
    (hderiv : ∀ᵐ t ∂volume.restrict (Ioo (0 : ℝ) b),
      deriv J t ≤ h t * J t)
    ...
    -(∫ t in Ioo (0 : ℝ) b, J t * deriv ψ t) ≤
      ∫ t in Ioo (0 : ℝ) b, h t * J t * ψ t := by
  have hopen : Ioo a b ⊆ Ioo (0 : ℝ) b :=
    fun _ ht => ⟨ha.trans ht.1, ht.2⟩
  have hhalf : Ico a b ⊆ Ioo (0 : ℝ) b :=
    fun _ ht => ⟨ha.trans_le ht.1, ht.2⟩
  have hψa : ψ a = 0 := hψzero ⟨ha, le_rfl⟩
  have hψderiv : EqOn (deriv ψ) 0 (Ioo (0 : ℝ) a) := by
  ...
  have hfinite :
      -(∫ t in a..b, J t * deriv ψ t) ≤
        ∫ t in a..b, h t * J t * ψ t :=
    neg_mul_deriv_le hab hJ hψ (by simpa only [hψa, mul_zero] using hbdry)
      hψ0' hderiv' hint'
  have hleft :
      (∫ t in Ioo (0 : ℝ) b, J t * deriv ψ t) =
    ...
    calc
      (∫ t in Ioo (0 : ℝ) b, J t * deriv ψ t) =
        ...
        apply setIntegral_eq_of_subset_of_forall_diff_eq_zero measurableSet_Ioo hhalf
        intro t ht
...
/-- Weighted integration by parts on `(0, b)` from absolutely continuous
restrictions to compact initial subintervals approaching `b`. -/
theorem neg_mul_deriv_le_lim
    {a b : ℝ} (ha : 0 < a) (hab : a < b) {J ψ h : ℝ → ℝ}
    ...
    (hderiv : ∀ᵐ t ∂volume.restrict (Ioo (0 : ℝ) b),
      deriv J t ≤ h t * J t)
    (hLint : IntegrableOn (fun t => J t * deriv ψ t) (Ioo (0 : ℝ) b))
    (hRint : IntegrableOn (fun t => h t * J t * ψ t) (Ioo (0 : ℝ) b)) :
    -(∫ t in Ioo (0 : ℝ) b, J t * deriv ψ t) ≤
      ∫ t in Ioo (0 : ℝ) b, h t * J t * ψ t := by
  ...
  have hψderiv : EqOn (deriv ψ) 0 (Ioo (0 : ℝ) a) := by
    simpa only [deriv_zero] using
      (hψzero.mono Ioo_subset_Ioc_self).deriv isOpen_Ioo
  have hlim (q : ℝ → ℝ) (hq : IntegrableOn q (Ioo (0 : ℝ) b)) :
  ...
  have hfinite (n : ℕ) :
    ...
    have hsub : Icc a (c n) ⊆ Ioo (0 : ℝ) b := by
      rintro t ⟨hat, htc⟩
    ...
    exact neg_mul_deriv_le (hc_mem n).1.le
