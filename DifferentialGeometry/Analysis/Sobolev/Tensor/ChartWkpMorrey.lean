...
private theorem weakGrad_real_le
    ...
    (eLpNorm (fun x => ‖hw.weakGrad x‖) p (volume.restrict Ω)).toReal ≤
      ∑ _i : Fin d, (iteratedWeakSobolevNorm (d := d) 1 p v Ω).toReal := by
  classical
  dsimp only
  ...
  have hterm : ∀ i : Fin d,
    ...
    simpa [iterWeakPartial_succ, iterWeakPartial_zero] using
      (eLpNorm_iterWeakPartial_le_wkpNorm (d := d) p v Ω 1 le_rfl
        (fun _ : Fin 1 => i))
  have hG : G ≤ ∑ _i : Fin d, iteratedWeakSobolevNorm (d := d) 1 p v Ω :=
    hmono.trans (hsum.trans (Finset.sum_le_sum fun i _ => hterm i))
  have hfinite : iteratedWeakSobolevNorm (d := d) 1 p v Ω ≠ ⊤ :=
    (wkpNorm_lt_top_of_memWkp hv).ne
  have hsum_finite : (∑ _i : Fin d, iteratedWeakSobolevNorm (d := d) 1 p v Ω) ≠ ⊤ :=
...
theorem w3p_morrey_c2
      ...
      ∀ β : Fin 2 → Fin d, ∃ C : ℝ, 0 ≤ C ∧
        ∀ x ∈ Metric.ball x₀ ((R / 4) / 4),
          ...
          ‖iterClassicalPartial (d := d) 2 β v x -
            ...
            C * (dist x y) ^ (1 - (d : ℝ) / p) *
              (iteratedWeakSobolevNorm (d := d) 3 (ENNReal.ofReal p) u
  ...
  have hae₁₆ : w =ᵐ[volume.restrict Ω₁₆]
    ...
    exact Filter.EventuallyEq.trans hw_ae htail
  ...
  have hG :
      (eLpNorm (fun x => ‖hw₂.weakGrad x‖) (ENNReal.ofReal p)
        ...
        ∑ _i : Fin d, (iteratedWeakSobolevNorm (d := d) 1 (ENNReal.ofReal p) w₂ Ω₄).toReal :=
  ...
  have hweak_real :
      (iteratedWeakSobolevNorm (d := d) 1 (ENNReal.ofReal p) w₂ Ω₄).toReal ≤
        (iteratedWeakSobolevNorm (d := d) 3 (ENNReal.ofReal p) u Ω).toReal :=
  ...
  have hG_total :
      (eLpNorm (fun x => ‖hw₂.weakGrad x‖) (ENNReal.ofReal p)
        (volume.restrict Ω₄)).toReal ≤
        D * (iteratedWeakSobolevNorm (d := d) 3 (ENNReal.ofReal p) u Ω).toReal := by
    refine hG.trans ?_
    calc
      ∑ _i : Fin d, (iteratedWeakSobolevNorm (d := d) 1 (ENNReal.ofReal p) w₂ Ω₄).toReal ≤
      ∑ _i : Fin d, (iteratedWeakSobolevNorm (d := d) 3 (ENNReal.ofReal p) u Ω).toReal :=
        Finset.sum_le_sum fun i _ => hweak_real
      _ = D * (iteratedWeakSobolevNorm (d := d) 3 (ENNReal.ofReal p) u Ω).toReal := by
        dsimp [D]
        rw [Finset.sum_mul]
        simp only [one_mul]
  ...
  calc
    ...
    _ = (C₀ * D) * (dist x y) ^ (1 - (d : ℝ) / p) *
        (iteratedWeakSobolevNorm (d := d) 3 (ENNReal.ofReal p) u Ω).toReal := by ring
end Tensor
end Sobolev
end Analysis
