...
private lemma engineRem_le_dLaGridWin (b : ℕ → ℝ) (hb : ∀ j, 0 ≤ b j) (j : ℕ) :
    ∑ k ∈ Finset.range j,
      ...
      (∑ k ∈ Finset.range j,
        Combinatorics.antidiagonalTupleGridCount
        (j - k) * Combinatorics.antidiagonalTupleGridCount (k + 1)) *
  ...
  have h3 : Combinatorics.antidiagonalTupleGrid b ((j - k) +
    (k + 1)) ≤ antidiagonalTupleGridPartialSum b (j + 2) :=
  ...
  calc b (j - k) * Combinatorics.antidiagonalTupleGrid b (k + 1)
    ...
    _ ≤ (Combinatorics.antidiagonalTupleGridCount
      (j - k) * Combinatorics.antidiagonalTupleGridCount (k + 1)) *
          Combinatorics.antidiagonalTupleGrid b ((j - k) + (k + 1)) := h2
    _ ≤ (Combinatorics.antidiagonalTupleGridCount
      (j - k) * Combinatorics.antidiagonalTupleGridCount
      (k + 1)) * antidiagonalTupleGridPartialSum b (j + 2) :=
  ...
  calc riemannianFiberNormSq (I := I) (M := M) g₀ 1 (2 + j) x
    ...
    _ ≤ 2 * (Kt0 * riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + (j + 1)) x
        ...
        2 * (Kc0 j * ((∑ k ∈ Finset.range j,
          Combinatorics.antidiagonalTupleGridCount
            (j - k) * Combinatorics.antidiagonalTupleGridCount
            (k + 1)) * antidiagonalTupleGridPartialSum b (j + 2))) := by
    ...
    _ = (2 * Kt0) * riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + (j + 1)) x
        ...
        (2 * Kc0 j * (∑ k ∈ Finset.range j,
          Combinatorics.antidiagonalTupleGridCount
            (j - k) * Combinatorics.antidiagonalTupleGridCount
            (k + 1))) * antidiagonalTupleGridPartialSum b (j + 2) := by ring
...
private theorem exists_rfns_dLaKernelRaised_topsep (g₀ g_bg : SmoothRiemannianMetric I M)
  ...
  set CQ1 : ℕ → ℝ := fun j => diagonalGridGrowthFactor (E := E) j * ∑ i' ∈ Finset.range (j + 1),
    (Module.finrank ℝ E : ℝ) * CA i' *
      ∑ l ∈ Finset.range (j + 1 - i'),
        CA l * antidiagonalTuplePairCount (i' + 2) (l + 2) with hCQ1_def
  set CQ2 : ℕ → ℝ := fun j => diagonalGridGrowthFactor (E := E) j * ∑ i' ∈ Finset.range (j + 1),
    (Module.finrank ℝ E : ℝ) * cc i' *
      ∑ l ∈ Finset.range (j + 1 - i'),
        CA l * antidiagonalTuplePairCount (i' + 2) (l + 2) with hCQ2_def
  set CQ3 : ℕ → ℝ := fun j => diagonalGridGrowthFactor (E := E) j * ∑ i' ∈ Finset.range (j + 1),
    (Module.finrank ℝ E : ℝ) * CA i' *
      ∑ l ∈ Finset.range (j + 1 - i'),
        cc l * antidiagonalTuplePairCount (i' + 2) (l + 2) with hCQ3_def
  ...
  have hrs_eq : ∀ (σ : Equiv.Perm (Fin 3)) (F : SmoothCcTensor g₀ 1 3),
    ...
    intro σ F
    exact riemannianFiberNormSq_iteratedCovGrad_rs_eq_of_section_domDomCongr (I := I)
      (M := M) g₀ 1 3 σ F
...
private lemma appCcGrid_le_dla (b : ℕ → ℝ) (hb : ∀ j, 0 ≤ b j) (i : ℕ)
    ...
    diagonalGridGrowthFactor (E := E) i * ∑ i' ∈ Finset.range (i + 1),
        pΦ i' * ∑ l ∈ Finset.range (i + 1 - i'), qW l ≤
      (diagonalGridGrowthFactor (E := E) i * ∑ i' ∈ Finset.range (i + 1),
        cΦ i' * ∑ l ∈ Finset.range (i + 1 - i'),
          cW l * antidiagonalTuplePairCount (i' + 1) (l + 3)) *
  ...
  have hcell : ∀ i' ∈ Finset.range (i + 1),
    ...
    rw [show (cΦ i' * ∑ l ∈ Finset.range (i + 1 - i'),
        cW l * antidiagonalTuplePairCount (i' + 1) (l + 3)) * antidiagonalTupleGridPartialSum b
          (i + 3) =
        ∑ l ∈ Finset.range (i + 1 - i'),
          (cΦ i' * (cW l * antidiagonalTuplePairCount (i' + 1)
            (l + 3))) * antidiagonalTupleGridPartialSum b (i + 3) from by
    ...
    calc cΦ i' * antidiagonalTupleGridPartialSum b (i' + 1) *
      (cW l * antidiagonalTupleGridPartialSum b (l + 3))
        = (cΦ i' * cW l) * (antidiagonalTupleGridPartialSum b
          (i' + 1) * antidiagonalTupleGridPartialSum b (l + 3)) := by ring
      _ ≤ (cΦ i' * cW l) * (antidiagonalTuplePairCount (i' + 1)
        (l + 3) * antidiagonalTupleGridPartialSum b (i + 3)) :=
          mul_le_mul_of_nonneg_left hpair (mul_nonneg (hcΦ_nn i') (hcW_nn l))
      _ = (cΦ i' * (cW l * antidiagonalTuplePairCount (i' + 1)
        (l + 3))) * antidiagonalTupleGridPartialSum b (i + 3) := by ring
...
private theorem exists_rfns_dLaLowered_topsep (g₀ g_bg : SmoothRiemannianMetric I M)
  ...
  have hbridge : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + i) x
    ...
    have h := riemannianFiberNormSq_iteratedCovGrad_cometricRaiseSlot0Field_eq (I := I)
      (M := M) g₀ 2
...
private theorem exists_rfns_dLaSym_topsep (g₀ g_bg : SmoothRiemannianMetric I M)
  ...
  set CLT : ℕ → ℝ := fun i => diagonalGridGrowthFactor (E := E) i * ∑ i' ∈ Finset.range (i + 1),
    CP i' * ∑ l ∈ Finset.range (i + 1 - i'),
      CL l * antidiagonalTuplePairCount (i' + 1) (l + 3) with hCLT_def
  ...
  have hPfac : ∀ i', i' ≤ i →
    ...
    | 0 =>
        ...
        have hwin1 : (1 : ℝ) ≤ antidiagonalTupleGridPartialSum b
          (0 + 1) := one_le_dLaGridWin b hb (by omega)
    ...
    | (m + 1) =>
        ...
        have hwin_nn : 0 ≤ antidiagonalTupleGridPartialSum b
          ((m + 1) + 1) := dLaGridWin_nonneg b hb _
  ...
  have hPer : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + i) x
    ...
    refine le_trans
      ...
      (I := I) (M := M) g₀ i 0 4 4
      (endoSlotZeroCcTensor (I := I) (M := M) g₀ 3 (dLaPerturbSharpEndoField (I := I)
        (M := M) g₀ T))
  ...
  have hG1 : riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + i) x
    ...
    calc 2 * riemannianFiberNormSq (I := I) (M := M) g₀ 0 (4 + i) x
      ...
      _ = (2 * KtopL + 2 * diagonalGridGrowthFactor (E := E) i * cPer * KtopL) *
            riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + (i + 2)) x
              ((iteratedCovGrad (I := I) g₀ 0 2 (i + 2) T).toSection x) +
          (2 * KcL i + 2 * diagonalGridGrowthFactor
            (E := E) i * cPer * KcL i + 2 * CLT i) * W := by ring
...
theorem riemannianFiberNormSq_iteratedCovGrad_deTurckLieConnDiffDerivCoeffField_topSeparated_le (g₀ g_bg : SmoothRiemannianMetric I M)
    ...
    have hperm : riemannianFiberNormSq (I := I) (M := M) g₀ 2 (6 + l) x
        ...
        riemannianFiberNormSq (I := I) (M := M) g₀ 2 (6 + l) x
          ((iteratedCovGrad (I := I) g₀ 2 6 l
            (slotExtendIter (I := I) (M := M) g₀ 0 4 2
              (dLaSymCc (I := I) (M := M) g₀ T g₁ g_bg))).toSection x) :=
      riemannianFiberNormSq_iteratedCovGrad_rs_eq_of_section_domDomCongr (I := I)
        (M := M) g₀ 2 6 sigmaE0dla
  ...
  have hPT0 : riemannianFiberNormSq (I := I) (M := M) g₀ 6 (2 + 0) x
    ...
    rwa [show antidiagonalTupleGridPartialSum b (0 + 1) = 1 from by
          rw [antidiagonalTupleGridPartialSum, Finset.sum_range_one,
            Combinatorics.antidiagonalTupleGrid_zero],
        mul_one] at h
