...
theorem markFold (g₀ : SmoothRiemannianMetric I M) {p a b : ℕ} (u v : ℕ)
    ...
    (hΦ : ∀ (i' : ℕ) (x : M),
      riemannianFiberNormSq (I := I) (M := M) g₀ a (b + i') x
        ...
        KΦ i' * Combinatorics.markGrid (gridBase (I := I) (M := M) g₀ P x) u i')
    (hW : ∀ (l : ℕ) (x : M),
      riemannianFiberNormSq (I := I) (M := M) g₀ p (a + l) x
          ((iteratedCovGrad (I := I) g₀ p a l W).toSection x) ≤
        KW l * Combinatorics.markGrid (gridBase (I := I) (M := M) g₀ P x) v l)
    (n : ℕ) (x : M) :
    riemannianFiberNormSq (I := I) (M := M) g₀ p (b + n) x
        ((iteratedCovGrad (I := I) g₀ p b n
          (appCcRS (I := I) (M := M) g₀ p a b Φ W)).toSection x) ≤
      foldConst (E := E) 0 0 KΦ KW n *
        Combinatorics.markGrid (gridBase (I := I) (M := M) g₀ P x) (u + v) n := by
  classical
...
theorem mkOfBnd (g₀ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
    ...
    HasMarkWin (I := I) (M := M) g₀ P X 0 S := by
  intro i x
...
theorem mkOfDP (g₀ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2) :
  ...
  set b : ℕ → ℝ := gridBase (I := I) (M := M) g₀ P x with hb_def
  have hb : ∀ j, 0 ≤ b j := gridBase_nn (I := I) (M := M) g₀ P x
  rw [rfns_iteratedCovGrad_covGrad_comm_rs (I := I) (M := M) g₀ 0 2 i P x, one_mul]
...
theorem mkOfTop (g₀ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
    ...
    (hX : ∀ (i : ℕ) (x : M),
      riemannianFiberNormSq (I := I) (M := M) g₀ r (c + i) x
          ((iteratedCovGrad (I := I) g₀ r c i X).toSection x) ≤
        Ktop * gridBase (I := I) (M := M) g₀ P x (i + 1) +
          Kc i * ∑ k ∈ Finset.range i,
            gridBase (I := I) (M := M) g₀ P x (i - k) *
              Combinatorics.antidiagonalTupleGrid
                (gridBase (I := I) (M := M) g₀ P x) (k + 1)) :
    HasMarkWin (I := I) (M := M) g₀ P X 1 (fun i => Ktop + Kc i * i) := by
  intro i x
  classical
  set b : ℕ → ℝ := gridBase (I := I) (M := M) g₀ P x with hb_def
  have hb : ∀ j, 0 ≤ b j := gridBase_nn (I := I) (M := M) g₀ P x
...
theorem mkApp (g₀ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
    ...
    (hΦ : HasMarkWin (I := I) (M := M) g₀ P Φ u KΦ)
    (hW : HasMarkWin (I := I) (M := M) g₀ P W v KW) :
    HasMarkWin (I := I) (M := M) g₀ P (appCcRS (I := I) (M := M) g₀ p a b Φ W)
      (u + v) (foldConst (E := E) 0 0 KΦ KW) := by
  intro n x
...
theorem mkMono (g₀ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
    ...
    HasMarkWin (I := I) (M := M) g₀ P X u K' := by
  intro i x
  exact le_trans (hX i x)
...
theorem mkAdd (g₀ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
    ...
    (hY : HasMarkWin (I := I) (M := M) g₀ P Y u KY) :
    HasMarkWin (I := I) (M := M) g₀ P (X + Y) u (fun i => 2 * KX i + 2 * KY i) := by
  intro i x
...
theorem mkSmul (g₀ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
    ...
    (hX : HasMarkWin (I := I) (M := M) g₀ P X u K) :
    HasMarkWin (I := I) (M := M) g₀ P (t • X) u (fun i => t ^ 2 * K i) := by
  intro i x
...
theorem mkDdc0 (g₀ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
    ...
    (hX : HasMarkWin (I := I) (M := M) g₀ P X u K) :
    HasMarkWin (I := I) (M := M) g₀ P (domDomCongrSection (I := I) g₀ σ X) u K := by
  intro i x
...
theorem mkSlotExt (g₀ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
    {r c u : ℕ} {X : SmoothCcTensor g₀ r c} {K : ℕ → ℝ}
    (hX : HasMarkWin (I := I) (M := M) g₀ P X u K) :
    HasMarkWin (I := I) (M := M) g₀ P (slotExtend (I := I) (M := M) g₀ r c X) u
...
theorem mkIter (g₀ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
    ...
    (hX : HasMarkWin (I := I) (M := M) g₀ P X u K) :
    HasMarkWin (I := I) (M := M) g₀ P (slotExtendIter (I := I) (M := M) g₀ r c w X) u
  ...
  | succ w ih =>
      ...
      have h := mkSlotExt (I := I) (M := M) g₀ P ih
      rw [← hrec] at h
      refine mkMono (I := I) (M := M) g₀ P (fun i => ?_) h
      simp only [pow_succ]
      ring_nf
      exact le_of_eq rfl
...
theorem gridIntHigh (g₀ : SmoothRiemannianMetric I M) :
    ∃ K : ℕ → ℝ, (∀ m, 0 ≤ K m) ∧
      ∀ (P : SmoothCcTensor g₀ 0 2) {Λ₀ Λ₁ : ℝ}, 0 ≤ Λ₀ → Λ₀ ≤ 1 → 0 ≤ Λ₁ →
        ...
        ∀ (m c₁ c₂ k n : ℕ) (e : Fin n → ℕ), 2 ≤ c₁ → 2 ≤ c₂ → 1 ≤ k → n ≤ k →
          ...
          (∫ x, gridBase (I := I) (M := M) g₀ P x c₁ *
                gridBase (I := I) (M := M) g₀ P x c₂ *
  ...
  have hint : ∀ x : M,
    ...
    intro x
    change _ = ∏ j : Fin (n + 2), gridBase (I := I) (M := M) g₀ P x (cc j)
...
theorem markMon (g₀ : SmoothRiemannianMetric I M) :
  ...
  obtain ⟨KH, hKH_nn, hHigh⟩ := gridIntHigh (I := I) (M := M) g₀
  ...
  · -- the top weight: `c₁ + c₂ + k = m + 1`
    ...
    · -- CLASS 1a: the first mark is a bare `∇P`
      ...
      have hfsum : (∑ q, f q) = m := by
        ...
        rw [he]; omega
      have hpt : ∀ x : M,
          gridBase (I := I) (M := M) g₀ P x 1 * gridBase (I := I) (M := M) g₀ P x c₂ *
              ∏ q : Fin n, gridBase (I := I) (M := M) g₀ P x (e q) =
    ...
    · by_cases h2 : c₂ = 1
      · -- CLASS 1b: the second mark is a bare `∇P`
        ...
        have hfsum : (∑ q, f q) = m := by
          ...
          rw [he]; omega
        have hpt : ∀ x : M,
            gridBase (I := I) (M := M) g₀ P x c₁ * gridBase (I := I) (M := M) g₀ P x 1 *
                ∏ q : Fin n, gridBase (I := I) (M := M) g₀ P x (e q) =
              riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + 1) x
                  ((iteratedCovGrad (I := I) g₀ 0 2 1 P).toSection x) *
                ∏ q : Fin (n + 1), riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + f q) x
      ...
      · -- both marks carry at least two derivatives
        ...
        · -- CLASS 2: the pure two-factor product
          ...
          have hpt : ∀ x : M,
              gridBase (I := I) (M := M) g₀ P x c₁ * gridBase (I := I) (M := M) g₀ P x c₂ *
                  ∏ q : Fin 0, gridBase (I := I) (M := M) g₀ P x (e q) =
        ...
        · have hk1 : 1 ≤ k := by omega
          ...
          · -- CLASS 1c: an unmarked factor is a bare `∇P`
            ...
            have hpt : ∀ x : M,
                gridBase (I := I) (M := M) g₀ P x c₁ *
                    gridBase (I := I) (M := M) g₀ P x c₂ *
                    ∏ q : Fin (n' + 1), gridBase (I := I) (M := M) g₀ P x (e q) =
                  riemannianFiberNormSq (I := I) (M := M) g₀ 0 (2 + 1) x
                      ((iteratedCovGrad (I := I) g₀ 0 2 1 P).toSection x) *
                    ∏ q : Fin (n' + 2),
              ...
              have hsplit := Fin.prod_univ_succAbove
                (fun q => gridBase (I := I) (M := M) g₀ P x (e q)) q₀
...
private lemma contGrid (g₀ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
  ...
  classical
  have heq : (fun x : M =>
        Combinatorics.antidiagonalTupleGrid (gridBase (I := I) (M := M) g₀ P x) k)
      = fun x : M => ∑ n' ∈ Finset.range (k + 1),
          ∑ e ∈ Finset.Nat.antidiagonalTuple n' k,
            ∏ q : Fin n', gridBase (I := I) (M := M) g₀ P x (e q) := rfl
  rw [heq]
...
private lemma contMk (g₀ : SmoothRiemannianMetric I M) (P : SmoothCcTensor g₀ 0 2)
  ...
  | zero =>
      have heq : (fun x : M =>
            Combinatorics.markGrid (gridBase (I := I) (M := M) g₀ P x) 0 w)
  ...
  | succ u ih =>
      have heq : (fun x : M =>
            Combinatorics.markGrid (gridBase (I := I) (M := M) g₀ P x) (u + 1) w)
          = fun x : M => ∑ c ∈ Finset.range (w + 1),
              gridBase (I := I) (M := M) g₀ P x (c + 1) *
                Combinatorics.markGrid (gridBase (I := I) (M := M) g₀ P x) u (w - c) := rfl
      rw [heq]
...
theorem markJet (g₀ : SmoothRiemannianMetric I M) :
            ...
            ‖iteratedCovGrad (I := I) g₀ r c n X‖ ^ 2 ≤
              K n * (K0 n * (1 + Λ₁ ^ 2)) *
                (1 + ∑ j ∈ Finset.range (n + 2),
                  ‖iteratedCovGrad (I := I) g₀ 0 2 j P‖ ^ 2) := by
  classical
  obtain ⟨Kmon, hKmon_nn, hmon⟩ := markMon (I := I) (M := M) g₀
  ...
  set b : M → ℕ → ℝ := fun x => gridBase (I := I) (M := M) g₀ P x with hb_def
