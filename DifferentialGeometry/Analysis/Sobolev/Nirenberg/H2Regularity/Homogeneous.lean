...
import DifferentialGeometry.Analysis.Sobolev.Nirenberg.H2Regularity.UniformDiffQuot
import DifferentialGeometry.Analysis.Sobolev.Nirenberg.SubstitutionIdentity.HomogeneousMaster
import DifferentialGeometry.Analysis.Sobolev.Nirenberg.TestFunction.StandardTestSquareBound
...
omit [NeZero d] in
private theorem one_set_sub_tsupp
    {V : Set E} {eta : E → ℝ}
...
/-- A local homogeneous weak solution has a uniform local `L²` bound for every
difference quotient of each displayed weak-gradient component. -/
theorem homSol_dq_bound
  ...
  obtain ⟨U, hU, _hU_eq, hgrad_eq, hmaster⟩ :=
    hom_master_nonsmooth (d := d) hOmega hu hsol B hrho heta heta_cpt
  ...
  have heta_in_Omega' : tsupport eta ⊆ Omega' := by
    exact Metric.self_subset_thickening (by linarith) _
  ...
  have hf_zero : ∀ {W : Set E}, IsCompact (closure W) →
      MemLp (0 : E → ℝ) 2 ((volume : Measure E).restrict W) := by
    intro W _
    exact MemLp.zero
  ...
  have hC : 0 ≤ C := by
    exact nirenbergMasterYoungConstant_nonneg (d := d) B hN hOmega'_compact k
  ...
  have hquant_h :=
    nirenberg_diffQuot_g_localL2_bound_quantitative
      (d := d) (Ω := Set.univ) (u := U) (f := (0 : E → ℝ)) (g := g)
      B hU_l2 hf_zero hg_l2 (fun j => hU.isWeakGrad j)
  ...
  have hV_tsupp : V ⊆ tsupport eta :=
    subset_closure.trans (one_set_sub_tsupp heta_one)
  have hdq_eq : diffQuot k h (g i) =ᵐ[(volume : Measure E).restrict V]
    ...
    exact dq_grad_eq_on (d := d) hh hh_le hV_tsupp
      (fun y hy => hgrad_eq y hy i) k x hx
  rw [← eLpNorm_congr_ae hdq_eq]
  exact hU_bound

/-- Every displayed weak-gradient component of a local homogeneous solution has
each second weak partial derivative locally in `L²`. -/
theorem homSol_second
    {Omega : Set E} (hOmega : IsOpen Omega)
    {A : DeGiorgi.EllipticCoeff d Omega} {u : E → ℝ}
    (hu : DeGiorgi.MemW1pWitness 2 u Omega)
    (hsol : DeGiorgi.IsSolution A u)
    ...
    ∃ G : E → ℝ,
      ...
      DeGiorgi.HasWeakPartialDeriv (d := d) k G
        (fun x => hu.weakGrad x i) V ∧
      ∃ M : ℝ, 0 ≤ M ∧
        eLpNorm G 2 ((volume : Measure E).restrict V) ≤ ENNReal.ofReal M := by
  obtain ⟨M, hM, hbound⟩ := homSol_dq_bound (d := d) hOmega hu hsol B hrho
    heta heta_cpt heta_range hR₀ hK_Omega hcoeff hc hV_open hV_compact
    heta_one hderiv i k
  have hV_tsupp : closure V ⊆ tsupport eta := one_set_sub_tsupp heta_one
  ...
  obtain ⟨G, hG_l2, hG_weak, hG_norm⟩ :=
  ...
  exact ⟨G, hG_l2, hG_weak, M, hM, hG_norm⟩

/-- A local homogeneous weak solution belongs to `W^{2,2}` on every inner open
set on which the cutoff equals one. -/
theorem homSol_memW2
    {Omega : Set E} (hOmega : IsOpen Omega)
    {A : DeGiorgi.EllipticCoeff d Omega} {u : E → ℝ}
    (hu : DeGiorgi.MemW1pWitness 2 u Omega)
    (hsol : DeGiorgi.IsSolution A u)
  ...
  intro k
  obtain ⟨G, hG_l2, hG_weak, _M, _hM, _hG_norm⟩ :=
    homSol_second (d := d) hOmega hu hsol B hrho heta heta_cpt heta_range
