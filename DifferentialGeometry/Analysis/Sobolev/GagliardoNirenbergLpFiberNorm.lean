...
private theorem riemannianFiberNormSq_covGradBundleEquiv_eq_sum_frame_rs
  ...
  -- Parseval-expand the `(r, s + 1)` fibre norm of the bundle image.
  rw [rfns_rs_eq_sum_fiberNormSqComponent_sq_of_orthoFrame (I := I) (M := M) g r (s + 1) x T e hn
    horth]
  -- Each per-direction fibre norm is the reading `(covGradBundleEquiv r s x).symm T (e a)`,
  -- Parseval-expanded in `e`, whose `(K, J)`-component is the `(K, Fin.cons a J)`-component of `T`.
  have hΦeq : ∀ a : Fin n,
  ...
  -- The `(a, J) ↦ Fin.cons a J` bijection reassembles the `∑_a ∑_J` into a single `∑_{J''}`.
  ...
  -- The per-`K` reassembly, stated as a universally-quantified identity to fix all motives.
    ...
    exact Fintype.sum_bijective _ hcons_bij
      ...
      (fun J'' : Fin (s + 1) → Fin n =>
        (Integral.Connection.fiberNormSqComponent (I := I) (M := M) g x r (s + 1) T
          n e K J'') ^ 2)
      (fun p => rfl)
  -- The goal is `∑_K ∑_{J''} f(K, J'') = ∑_a ∑_K ∑_J f(K, cons a J)`.  Commute the RHS's `∑_a ∑_K`
  -- to `∑_K ∑_a`, then discharge each `K`-slice by `hperK`.
...
/-- The directional covariant derivative `∇_Y W` of a smooth compactly-supported `(r, t)`-tensor, as
a bundled smooth section (inline mirror of the private `covApplyCcSec` in
`HomFieldCurvatureJetDecomposition`, smoothness from `covApplyRS_contMDiff`). -/
private noncomputable def covApplyCcSec_rs
    (g : SmoothRiemannianMetric I M) (r t : ℕ) (W : Integral.L2.SmoothCcTensor g r t)
    {Y : Π b : M, TangentSpace I b}
    (hY : ContMDiff I (I.prod 𝓘(ℝ, E)) ∞ (T% Y)) :
    Cₛ^∞⟮I; Tensor0SBundle.TensorRSModel r t ℝ E,
      (fun x : M => Tensor0SBundle.TensorRSSpace r t I x)⟯ :=
  letI : NormedAddCommGroup (Tensor0SBundle.TensorRSModel r t ℝ E) :=
    Tensor0SBundle.tensorRSModel_normedAddCommGroup r t
  letI : NormedSpace ℝ (Tensor0SBundle.TensorRSModel r t ℝ E) :=
    Tensor0SBundle.tensorRSModel_normedSpace r t
  { toFun := fun y : M =>
      covApply (tensorCov (I := I) g r t) Y (fun z : M => W.toSection z) y
    contMDiff_toFun :=
      Integral.Connection.covApplyRS_contMDiff (I := I) g r t W.toSection.contMDiff_toFun hY }

set_option backward.isDefEq.respectTransparency false in
set_option maxHeartbeats 3200000 in
set_option synthInstance.maxHeartbeats 1600000 in
/-- **The rank-`r` two-slot evaluation bridge.** The general first-valence `r` lift of the
contravariant-`0` `Integral.Connection.tensorSecondCovDeriv_eq_covGrad_succ_twoSlotEval_genVal`
(reproduced inline; the on-disk general-`r` version
`secondCovGrad_eval_eq_tensorSecondCovDeriv` is private in `HomFieldCurvatureJetDecomposition`):
the second iterated covariant gradient `∇²W = covGrad g r (t+1) (covGrad g r t W)` of a smooth
compactly-supported `(r, t)`-tensor `W`, evaluated at a lower input `D` and on the tuple
`(X x, Y x, m)` reading its two leading covariant slots along smooth vector fields `X, Y`, is the
genuine tensorial second covariant derivative `tensorSecondCovDeriv g r t X Y W (x)` evaluated at
`(D, m)`.  The two leading slots are read off as iterated directional covariant derivatives
(`covGrad_toSection_apply_eval`), the `(0, r)`-lower input is carried by a smooth section `w`
through `D` via the Hom-connection product rule `tensorRSCovariantDerivative_apply` (twice — at
valence `t + 1`, then at valence `t` over `∇_Y W`), the slot-`0` Christoffel correction of the
abstract `(0, t + 1)`-derivative is exposed by `abstract_succ_covDeriv_unfold_at_genVal`, and the two
`w`-correction terms cancel pairwise, leaving exactly the two terms of `tensorSecondCovDeriv_def`. -/
private theorem secondCovGrad_eval_eq_tensorSecondCovDeriv_rs
    (g : SmoothRiemannianMetric I M) (r t : ℕ) (W : Integral.L2.SmoothCcTensor g r t)
    {X Y : Π b : M, TangentSpace I b}
    (hX : ContMDiff I (I.prod 𝓘(ℝ, E)) ∞ (T% X))
    (hY : ContMDiff I (I.prod 𝓘(ℝ, E)) ∞ (T% Y))
    (x : M) (D : Tensor0SBundle.Tensor0SSpace r I x) (m : Fin t → TangentSpace I x) :
    Tensor0SBundle.Tensor0SSpace.toModel
        ((show Tensor0SBundle.Tensor0SSpace r I x →L[ℝ]
            Tensor0SBundle.Tensor0SSpace (t + 2) I x from
          (covGrad (I := I) (M := M) g r (t + 1)
            (covGrad (I := I) (M := M) g r t W)).toSection x) D)
        (Fin.cons (X x) (Fin.cons (Y x) m)) =
      Tensor0SBundle.Tensor0SSpace.toModel
        ((show Tensor0SBundle.Tensor0SSpace r I x →L[ℝ]
            Tensor0SBundle.Tensor0SSpace t I x from
          Integral.Connection.tensorSecondCovDeriv (I := I) g r t X Y
            (fun y : M => W.toSection y) x) D) m := by
  classical
  -- Lift the lower input `D` to a smooth `(0, r)`-section `w` through `D` (closed manifold).
  obtain ⟨w, hwx⟩ := ContMDiffSection.exists_eq_at (I := I)
    (F := Tensor0SBundle.Tensor0SModel r ℝ E)
    (V := fun z : M => Tensor0SBundle.Tensor0SSpace r I z) (n := (⊤ : ℕ∞)) x D
  subst hwx
  set GW : Integral.L2.SmoothCcTensor g r (t + 1) := covGrad (I := I) (M := M) g r t W with hGW_def
  ...
  rw [vecTail_cons_rs (X x) (Fin.cons (Y x) m)]
  -- STEP 2: Hom-connection product rule at valence `t + 1` against the section `w` through `D`.
  have happly₁ : (show Tensor0SBundle.Tensor0SSpace r I x →L[ℝ]
        Tensor0SBundle.Tensor0SSpace (t + 1) I x from
      tensorCovDerivAt (I := I) (M := M) g r (t + 1) GW x (X x)) (w x) =
      Tensor0SNabla.tensor0SCovariantDerivative I M (t + 1) (LeviCivita (I := I) g)
          (fun y : M =>
            (show Tensor0SBundle.Tensor0SSpace r I y →L[ℝ]
                Tensor0SBundle.Tensor0SSpace (t + 1) I y from
              GW.toSection y) (w y)) x (X x) -
        (show Tensor0SBundle.Tensor0SSpace r I x →L[ℝ]
            Tensor0SBundle.Tensor0SSpace (t + 1) I x from GW.toSection x)
  ...
  rw [happly₁, Tensor0SBundle.Tensor0SSpace.toModel_sub, ContinuousMultilinearMap.sub_apply]
  -- STEP 3: smoothness of the contracted `(0, t + 1)`-section and of its curried section.
  have hP_smooth : ContMDiff I (I.prod 𝓘(ℝ, Tensor0SBundle.Tensor0SModel (t + 1) ℝ E)) ∞
      (fun y : M => Bundle.TotalSpace.mk' (Tensor0SBundle.Tensor0SModel (t + 1) ℝ E)
        (E := fun z : M => Tensor0SBundle.Tensor0SSpace (t + 1) I z) y
        ((show Tensor0SBundle.Tensor0SSpace r I y →L[ℝ]
            Tensor0SBundle.Tensor0SSpace (t + 1) I y from
          GW.toSection y) (w y))) :=
    ContMDiff.clm_bundle_apply (b := id) GW.toSection.contMDiff w.contMDiff
  have hP_curried : ContMDiffAt I
      (I.prod 𝓘(ℝ, E →L[ℝ] Tensor0SBundle.Tensor0SModel t ℝ E)) ∞
      (fun y : M => Bundle.TotalSpace.mk' (E →L[ℝ] Tensor0SBundle.Tensor0SModel t ℝ E)
        (E := fun z : M => TangentSpace I z →L[ℝ] Tensor0SBundle.Tensor0SSpace t I z) y
  ...
  -- STEP 4: expose the `Y x`-slot through the curry, then unfold the slot-`0` Christoffel correction.
  ...
  have habs : (tensor0S_curry (I := I) (M := M) (𝕜 := ℝ) t x
    ...
    Integral.Connection.abstract_succ_covDeriv_unfold_at_genVal (I := I) (M := M) g t
      (fun y : M =>
        (show Tensor0SBundle.Tensor0SSpace r I y →L[ℝ]
            Tensor0SBundle.Tensor0SSpace (t + 1) I y from
          GW.toSection y) (w y))
  ...
  rw [habs, Tensor0SBundle.Tensor0SSpace.toModel_sub, ContinuousMultilinearMap.sub_apply]
  ...
  have hsec : (fun y : M => Tensor0SNabla.curriedSection I M
      ...
      (fun y : M =>
        (show Tensor0SBundle.Tensor0SSpace r I y →L[ℝ] Tensor0SBundle.Tensor0SSpace t I y from
    ...
    rw [TensorMultilinear.tensor0S_curry_apply_eval (I := I) (M := M)
      (T := (show Tensor0SBundle.Tensor0SSpace r I y →L[ℝ]
          Tensor0SBundle.Tensor0SSpace (t + 1) I y from
  ...
  have happly₂ : Tensor0SNabla.tensor0SCovariantDerivative I M t (LeviCivita (I := I) g)
      ...
      (show Tensor0SBundle.Tensor0SSpace r I x →L[ℝ] Tensor0SBundle.Tensor0SSpace t I x from
        TensorRSNabla.tensorRSCovariantDerivative I M r t (LeviCivita (I := I) g)
          (covApplyCcSec_rs (I := I) (M := M) g r t W hY) x (X x)) (w x) +
        (show Tensor0SBundle.Tensor0SSpace r I x →L[ℝ] Tensor0SBundle.Tensor0SSpace t I x from
          tensorCovDerivAt (I := I) (M := M) g r t W x (Y x))
          (Tensor0SNabla.tensor0SCovariantDerivative I M r (LeviCivita (I := I) g) w x (X x)) := by
    have h : (show Tensor0SBundle.Tensor0SSpace r I x →L[ℝ] Tensor0SBundle.Tensor0SSpace t I x from
        TensorRSNabla.tensorRSCovariantDerivative I M r t (LeviCivita (I := I) g)
          (covApplyCcSec_rs (I := I) (M := M) g r t W hY) x (X x)) (w x) =
        Tensor0SNabla.tensor0SCovariantDerivative I M t (LeviCivita (I := I) g)
          (fun y : M =>
      ...
      TensorRSNabla.tensorRSCovariantDerivative_apply (I := I) (M := M) r t
        (LeviCivita (I := I) g) (covApplyCcSec_rs (I := I) (M := M) g r t W hY) w x (X x)
    exact sub_eq_iff_eq_add.mp h.symm
  rw [happly₂, Tensor0SBundle.Tensor0SSpace.toModel_add, ContinuousMultilinearMap.add_apply]
  -- STEP 7: read the `Y x`-slot of the `w`-correction `(∇W)(x)(u₀)`.
  have hPb : Tensor0SBundle.Tensor0SSpace.toModel
      ((show Tensor0SBundle.Tensor0SSpace r I x →L[ℝ]
          Tensor0SBundle.Tensor0SSpace (t + 1) I x from GW.toSection x)
        (Tensor0SNabla.tensor0SCovariantDerivative I M r (LeviCivita (I := I) g) w x (X x)))
      (Fin.cons (Y x) m) =
    ...
    rw [hgw]
    simp only [Fin.cons_zero]
    rw [vecTail_cons_rs (Y x) m]
  rw [hPb]
  -- STEP 8: read the curried Christoffel correction `curry((∇W)(x)(w x))(∇_X Y (x))`.
  have hC₁ : Tensor0SBundle.Tensor0SSpace.toModel
      (Tensor0SNabla.curriedSection I M
        (fun y' : M =>
          (show Tensor0SBundle.Tensor0SSpace r I y' →L[ℝ]
              Tensor0SBundle.Tensor0SSpace (t + 1) I y' from
    ...
    rw [TensorMultilinear.tensor0S_curry_apply_eval (I := I) (M := M)
      (T := (show Tensor0SBundle.Tensor0SSpace r I x →L[ℝ]
          Tensor0SBundle.Tensor0SSpace (t + 1) I x from
    ...
    rw [hgw]
    simp only [Fin.cons_zero]
    rw [vecTail_cons_rs ((LeviCivita (I := I) g).toFun Y x (X x)) m]
  rw [hC₁]
  -- STEP 9: assemble; the `(∇_Y W)(u₀)`-pair cancels, leaving `tensorSecondCovDeriv_def`.
  have hSCD : (show Tensor0SBundle.Tensor0SSpace r I x →L[ℝ] Tensor0SBundle.Tensor0SSpace t I x from
      Integral.Connection.tensorSecondCovDeriv (I := I) g r t X Y
        (fun y : M => W.toSection y) x) (w x) =
      (show Tensor0SBundle.Tensor0SSpace r I x →L[ℝ] Tensor0SBundle.Tensor0SSpace t I x from
        TensorRSNabla.tensorRSCovariantDerivative I M r t (LeviCivita (I := I) g)
          (covApplyCcSec_rs (I := I) (M := M) g r t W hY) x (X x)) (w x) -
        (show Tensor0SBundle.Tensor0SSpace r I x →L[ℝ] Tensor0SBundle.Tensor0SSpace t I x from
  ...
  rw [hSCD, Tensor0SBundle.Tensor0SSpace.toModel_sub, ContinuousMultilinearMap.sub_apply]
  ring
...
private theorem secondCovDeriv_frame_diag_fiberNormSq_sum_le_rs
  ...
  have horth : ∀ a b : Fin n, g.inner x (e a) (e b) = if a = b then (1 : ℝ) else 0 := by
    ...
    rw [he_def]
    exact Integral.Connection.smoothOrthoFrame_orthonormal_at_center (I := I) g x a b
  -- The second iterated covariant gradient as an `(r, m + 1 + 1)`-tensor at `x`.
  set T2 : Tensor0SBundle.TensorRSSpace r (m + 1 + 1) I x :=
    (covGrad (I := I) (M := M) g r (m + 1)
      (covGrad (I := I) (M := M) g r m w)).toSection x with hT2_def
  -- The per-component identity: each diagonal second-derivative component reads off `T2` at the two
  -- leftmost slots `(e i, e i)` via the rank-`r` two-slot bridge.
  have hcomp : ∀ (i : Fin n) (K : Fin r → Fin n) (J : Fin m → Fin n),
    ...
    set ωK : Tensor0SBundle.Tensor0SSpace r I x :=
      (ContinuousMultilinearMap.mkPiAlgebra ℝ (Fin r) ℝ).compContinuousLinearMap
        (fun k => g.inner x (e (K k))) with hωK
    -- Tuple identity: `e ∘ (i :: i :: J) = e i :: e i :: (e ∘ J)`.
    have htuple : (fun k : Fin (m + 1 + 1) =>
          e ((Fin.cons i (Fin.cons i J) : Fin (m + 1 + 1) → Fin n) k)) =
        Fin.cons (e i) (Fin.cons (e i) (fun k : Fin m => e (J k))) := by
      funext k
      ...
      · intro j
        ...
        · simp
        · intro l; simp
    -- The two-slot bridge at the lower input `ωK` and tuple `(e i, e i, e ∘ J)`.
    have hbridge := secondCovGrad_eval_eq_tensorSecondCovDeriv_rs (I := I) (M := M) g r m w
      (Integral.Connection.smoothOrthoFrame_smooth (I := I) g x i)
      (Integral.Connection.smoothOrthoFrame_smooth (I := I) g x i) x ωK
      (fun k : Fin m => e (J k))
    -- Both `fiberNormSqComponent`s are `(· ωK)(tuple)`, i.e. `toModel (· ωK) tuple`; align via the
    -- bridge and the tuple identity.
  ...
  have hdiag_term : ∀ i : Fin n,
    ...
    refine Finset.sum_congr rfl (fun K _ => Finset.sum_congr rfl (fun J _ => ?_))
    rw [hcomp i K J]
  -- The full fibre norm of `T2` is the `(K, J)` double Parseval frame sum.
  ...
  -- Split the full `(m + 2)`-multi-index sum as `∑_a ∑_b ∑_{J'}` via two `Fin.consEquiv`, per `K`.
  have hfull : ∀ K : Fin r → Fin n,
    ...
    rw [← Fintype.sum_equiv (Fin.consEquiv (fun _ : Fin (m + 1) => Fin n))
          ...
          (fun pr => by simp [Fin.consEquiv])]
    rw [Fintype.sum_prod_type]
  -- The goal is `∑_i ∑_K ∑_J f(K, i::i::J) ≤ ∑_K ∑_{J''} f(K, J'')`.  Commute `∑_i ∑_K` to `∑_K ∑_i`
  -- on the LHS, then dominate each `K`-slice's diagonal `(a = b = i)` sub-sum by the full sum.
  conv_lhs => rw [Finset.sum_comm]
  refine Finset.sum_le_sum (fun K _ => ?_)
  rw [hfull K]
  -- `∑_i ∑_J f(K, i::i::J) ≤ ∑_a ∑_b ∑_J f(K, a::b::J)`: each `i`-term is the `b = i` slice.
  refine Finset.sum_le_sum (fun i _ => ?_)
...
private theorem rawConnLap_innerWith_sqrt_finrank_bound_rs
  ...
  have htrace : (rawTensorConnLapSmooth (I := I) g r m w).toFun x =
    ...
    rw [h1, Integral.Connection.rawTensorConnLapSmooth_toSection_apply (I := I) (M := M) g r m w x,
      Integral.Connection.rawTensorConnLap_eq_frame_trace_secondCovDeriv (I := I) g r m
        (fun y : M => w.toSection y) x]
  -- Distribute the pairing over the frame sum.
  have hsum_aux : ∀ (s' : Finset (Fin n)),
      Integral.L2.tensorInnerPointwise (I := I) (M := M) g r m x
          (Tensor0SBundle.TensorRSSpace.toModel (∑ i ∈ s', D i)) (w.toFun x) =
  ...
  have hsum : Integral.L2.tensorInnerPointwise (I := I) (M := M) g r m x
    ...
    rw [htrace]
    exact hsum_aux Finset.univ
  -- Per-direction pointwise Cauchy–Schwarz.
...
private theorem kato_mfderiv_riemannianFiberNormSq_frame_sum_le_rs
  ...
  -- Each directional differential squared is `≤ 4 · ss a · rQ`.
  have hterm : ∀ a : Fin n,
      (extDerivFun (I := I)
...
private theorem covDerivCrossLeft_weight_bound_rs
  ...
  -- Step 1 — the cross term as the genuine `(r, m+1)`-tensor pairing `⟨∇w, P⟩`.
...
private theorem weightedCovIBP_lpFiberJet_fin_regIneq_rs
  ...
  have hc_nonneg : ∀ y, 0 ≤ c y := fun y =>
    riemannianFiberNormSq_nonneg (I := I) (M := M) g r (m + 1 + 1) y (ggw.toSection y)
  -- `b` agrees pointwise with the smooth scalar `tensorInnerScalar g r (m+1) (∇w) (∇w)`.
  have hb_eq_scalar : b = Integral.Connection.tensorInnerScalar (I := I) (M := M) g r (m + 1)
      gw.toSection gw.toSection := by
    funext y
    simp only [hb, Integral.Connection.tensorInnerScalar_apply]
  ...
  have hcrossL_cont : Continuous (tensorCovDerivCrossLeft (I := I) (M := M) g r m ζ w w) := by
    have heq : tensorCovDerivCrossLeft (I := I) (M := M) g r m ζ w w =
      ...
      funext x; rw [hsplit x]; ring
    rw [heq]; exact htcdpi_cont.sub hζb_cont
  -- The diagonal inner product `dw = ⟨Δw, w⟩` is the smooth scalar `tensorInnerScalar g r m Δw w`.
  set dw : M → ℝ := fun x => Integral.L2.tensorInnerPointwise (I := I) (M := M) g r m x
    (Lw.toFun x) (w.toFun x) with hdw
  have hdw_eq : dw = Integral.Connection.tensorInnerScalar (I := I) (M := M) g r m
      Lw.toSection w.toSection := by
