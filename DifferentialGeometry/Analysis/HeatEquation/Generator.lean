...
/-! ## Bridge: `oneMinusLapHeat g 1 t = heatSemigroup g t + heatPower g 1 t`

The binomial sum collapses to the two-term identity for `k = 1`:
`C(1, 0) = 1`, `C(1, 1) = 1`, and `heatPower g 0 t = heatSemigroup g t`. -/
...
lemma oneMinusLapHeat_one_apply
  ...
  -- ∑ j ∈ range 2, C(1, j) • heatPower g j t u
  --   = C(1, 0) • heatPower g 0 t u + C(1, 1) • heatPower g 1 t u
  --   = 1 • heatSemigroup g t u + 1 • heatPower g 1 t u
  --   = heatSemigroup g t u + heatPower g 1 t u.
  rw [Finset.sum_range_succ, Finset.sum_range_succ, Finset.sum_range_zero]
  simp only [zero_add, Nat.choose_zero_right, Nat.choose_self,
    Nat.cast_one, one_smul]
  rw [heatPower_zero (I := I) (M := M) g t]

/-! ## Bridge: explicit lift at `k = 0` lies in `laplacianDomain g`

By construction `heatSemigroupExplicitLift g 0 t u_0 ∈ laplacianDomainPow g 1`,
which is the ordinary Laplacian domain. We package the membership directly. -/
...
lemma heatSemigroupExplicitLift_zero_mem_laplacianDomain
  ...
  -- The witness `oneMinusLapHeat g 1 t u_0` makes the lift the resolvent's image.
  rw [laplacianDomain_mem_iff]
  refine ⟨oneMinusLapHeat (I := I) (M := M) g 1 t u_0, ?_⟩
  -- heatSemigroupExplicitLift g 0 t u_0 = resolvent g (iteratedResolventL2 g 0 (oneMinusLapHeat g 1 t u_0))
  --                                    = resolvent g (oneMinusLapHeat g 1 t u_0).
  unfold heatSemigroupExplicitLift
  rw [iteratedResolventL2_zero_apply]

/-! ## Headline 1: operator identity

The explicit lift evaluates under `laplacianOp` to `-heatPower g 1 t u_0`. -/
...
theorem laplacianOp_heatSemigroupExplicitLift_zero_eq_neg_heatPower_one
  ...
  -- Set f := oneMinusLapHeat g 1 t u_0.
  set f : Lp ℝ 2 (riemannianVolumeMeasure (I := I) (M := M) g) :=
    oneMinusLapHeat (I := I) (M := M) g 1 t u_0 with hf_def
  -- The explicit lift unfolds to `resolvent g f`.
  have h_lift_eq :
      heatSemigroupExplicitLift (I := I) (M := M) g 0 t u_0 =
        resolvent (I := I) (M := M) g f := by
    unfold heatSemigroupExplicitLift
    rw [iteratedResolventL2_zero_apply]
  -- Convert the subtype to the canonical resolvent-image form.
  ...
  · -- Apply laplacianOp_resolvent and then identify H1ComplToLp (resolvent f) = heatSemigroup t u_0.
    rw [laplacianOp_resolvent (I := I) (M := M) g f]
    -- Show: H1ComplToLp (resolvent f) - f = -heatPower g 1 t u_0.
    -- Step 1: H1ComplToLp (resolvent (oneMinusLapHeat 1 t u_0)) = heatSemigroup t u_0.
    have h_H1 : H1ComplToLp (I := I) (M := M) g
          (resolvent (I := I) (M := M) g f) =
        heatSemigroup (I := I) (M := M) g t u_0 := by
      -- This is `H1ComplToLp_heatSemigroupExplicitLift g 0 ht u_0` rewritten via h_lift_eq.
      have := H1ComplToLp_heatSemigroupExplicitLift
        (I := I) (M := M) g 0 ht u_0
      rwa [h_lift_eq] at this
    rw [h_H1]
    -- Step 2: f = oneMinusLapHeat g 1 t u_0 = heatSemigroup g t u_0 + heatPower g 1 t u_0.
    rw [hf_def, oneMinusLapHeat_one_apply (I := I) (M := M) g t u_0]
    -- heatSemigroup g t u_0 - (heatSemigroup g t u_0 + heatPower g 1 t u_0) = -heatPower g 1 t u_0.
    abel
  · -- Subtype equality from underlying-element equality.
    apply Subtype.ext
    exact h_lift_eq
...
theorem hasDerivAt_heatSemigroup_eq_laplacianOp
      ...
      (laplacianOp (I := I) (M := M) g
        ⟨heatSemigroupExplicitLift (I := I) (M := M) g 0 t u_0,
          heatSemigroupExplicitLift_zero_mem_laplacianDomain
            (I := I) (M := M) g t u_0⟩)
      t := by
  -- The operator-norm derivative gives:
  -- HasDerivAt (fun s => heatSemigroup g s) (-heatPower g 1 t) t.
  have h_op := hasDerivAt_heatSemigroup (I := I) (M := M) g ht
  -- Composed with `applyL u_0`, evaluating at fixed `u_0`:
  -- HasDerivAt (fun s => heatSemigroup g s u_0) (-heatPower g 1 t u_0) t.
  -- The const `u_0` has HasDerivAt 0.
  have h_u_const : HasDerivAt (fun _ : ℝ => u_0) (0 :
      Lp ℝ 2 (riemannianVolumeMeasure (I := I) (M := M) g)) t :=
    hasDerivAt_const t u_0
  -- Use HasDerivAt.clm_apply: HasDerivAt (fun y => c y (u y)) (c' (u x) + c x u') x.
  ...
  -- Simplify the derivative: heatSemigroup g t 0 = 0 (linearity).
  have h_simp :
      ((-heatPower (I := I) (M := M) g 1 t) u_0 +
  ...
  -- Rewrite the target using Headline 1.
  rw [laplacianOp_heatSemigroupExplicitLift_zero_eq_neg_heatPower_one
    (I := I) (M := M) g ht u_0]
  exact h_apply

/-! ## Optional: identification with the `Classical.choose` lift

For the canonical `Classical.choose`-based lift
`heatSemigroupSpectralLift g ht u_0 1`, the analogous operator identity holds.
Both `heatSemigroupSpectralLift g ht u_0 1` and
`heatSemigroupExplicitLift g 0 t u_0` lie in `laplacianDomain g` and project
to the same `Lp` element under `H1ComplToLp`, hence agree by injectivity. -/
...
lemma heatSemigroupSpectralLift_one_eq_explicit_zero
  ...
  -- Both lifts lie in laplacianDomain g and project to heatSemigroup g t u_0
  -- under H1ComplToLp; injectivity of H1ComplToLp on laplacianDomain g gives
  -- equality.
  -- Canonical lift's domain membership: laplacianDomainPow g 1 = laplacianDomain g.
  ...
  have h_can_mem : heatSemigroupSpectralLift (I := I) (M := M) g ht u_0 1 ∈
    ...
    exact h_can_mem_pow
  -- Explicit lift's domain membership: from heatSemigroupExplicitLift_zero_mem_laplacianDomain.
  have h_exp_mem : heatSemigroupExplicitLift (I := I) (M := M) g 0 t u_0 ∈
      laplacianDomain (I := I) (M := M) g :=
    heatSemigroupExplicitLift_zero_mem_laplacianDomain
      (I := I) (M := M) g t u_0
  -- Both project to the same Lp element.
  have h_proj_eq :
  ...
  -- Apply injectivity of H1ComplToLp on laplacianDomain g.
        ...
        ⟨(heatSemigroupSpectralLift (I := I) (M := M) g ht u_0 1 :
          ...
          by
            ...
            exact heatSemigroupSpectralLift_mem
              (I := I) (M := M) g ht u_0 1⟩ =
      -(heatPower (I := I) (M := M) g 1 t u_0) := by
  -- Rewrite the canonical lift to the explicit lift via subtype equality, then apply Headline 1.
