...
/-!
# The Tensor Product Decomposition `T⁰ₛ₊ᵧ(E) ≃ T⁰ₛ(E) ⊗ T⁰ᵧ(E)`

For a `C^n` vector bundle `E` over `B` with model fiber `F`, the `(s+q)`-multilinear
bundle decomposes as a tensor product of the `s`- and `q`-multilinear bundles:

  `T⁰ₛ₊ᵧ(E) ≃ T⁰ₛ(E) ⊗ T⁰ᵧ(E)`

as a `C^n` vector bundle equivalence (`ContMDiffVectorBundleEquiv`). Fiberwise, an
`(s+q)`-multilinear map is identified with a tensor product of an `s`- and a
`q`-multilinear map by concatenating inputs via `uncurrySum`/`domDomCongr`.

## Main Definitions

* `Bundle.continuousMultilinearMap.product_fun` : pointwise tensor product of multilinear
  bundle fiber elements (concatenating inputs).
...
* `Bundle.continuousMultilinearMap.modelFromTensorEquiv` : model-fiber `LinearEquiv` from
  `MLF s ⊗ MLF q` to `MLF (s+q)`, via dimension counting.
* `multilinearTensorFiberwiseEquiv` : the fiberwise `LinearEquiv` between the
  `(s+q)`-multilinear fiber and the tensor product of `s`- and `q`-multilinear fibers.
* `multilinearBundle_tensorProduct_equiv` : the `C^n` vector bundle equivalence.

## Implementation Notes

The equivalence is built in four stages: (1) a model-fiber `LinearEquiv` via
`LinearEquiv.ofBijective` from the surjectivity of `modelFromTensor` and matching
dimensions; (2) fiberwise lift via the trivialization CLEs; (3) total-space smoothness
via trivialization compatibility (`triv_fromTensor_eq_modelFromTensor`,
`triv_toTensor_eq_modelFromTensorEquiv_symm`); (4) assembly via
