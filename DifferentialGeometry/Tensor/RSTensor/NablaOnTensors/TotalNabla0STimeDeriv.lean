...
If, near the base point `x₀`, the parameter derivative of the scalar field
`r ↦ α r (·)(V ·)` commutes with the directional derivative in `X` (hypothesis
`hswap`, an instance of `FixedBaseExtDerivTimeDerivativeOn`), and each correction
term `r ↦ α r x₀ (…)` has parameter derivative the corresponding `β t` term
(hypothesis `hpt`), then `r ↦ (∇_X (α r))(V)` is differentiable in `r` with
derivative `(∇_X (β t))(V)`. -/
theorem nabla0SFun_hasDerivWithinAt {s : ℕ}
  ...
  · exact hswap t x₀ (Set.mem_singleton x₀) (X x₀)
  · have key :
...
The `totalNabla0SFun` version of `nabla0SFun_hasDerivWithinAt`, obtained by
...
theorem totalNabla0SFun_hasDerivWithinAt {s : ℕ}
  ...
  simpa only [totalNabla0SFun_apply_section] using
    nabla0SFun_hasDerivWithinAt cov X V α β timeSet x₀ t hswap hpt
