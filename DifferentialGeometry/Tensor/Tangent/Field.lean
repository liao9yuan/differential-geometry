...
import DifferentialGeometry.Tensor.Multilinear.Product
...
abbrev TensorRSField (r s : ℕ) :=
  letI := Bundle.continuousMultilinearMap.mixedTopology (𝕜 := 𝕜) (E := TangentSpace I) r s
  ContMDiffSection I
...
def tensorRSField_smulByFun
  ...
  letI := Bundle.continuousMultilinearMap.mixedTopology (𝕜 := 𝕜) (E := TangentSpace I) r s
  ⟨fun x => φ x • α x, by
    letI := Bundle.continuousMultilinearMap.mixedFiberBundle (𝕜 := 𝕜) (E := TangentSpace I) r s
    letI := Bundle.continuousMultilinearMap.mixedVectorBundle (𝕜 := 𝕜) (E := TangentSpace I) r s
    intro x₀
...
noncomputable def Tensor0SField.fromScalarField [CompleteSpace 𝕜]
  ...
  letI := Bundle.continuousMultilinearMap.topologicalSpace_totalSpace 𝕜 _ E (TangentSpace I) 0
  ...
  ⟨fun x => ContinuousMultilinearMap.constOfIsEmpty 𝕜
    ...
    have hcoord : ∀ x, (continuousMultilinearMap_basis b 0).repr
        (trivializationAt (ContinuousMultilinearMap 𝕜 (fun _ : Fin 0 => E) 𝕜)
...
theorem Tensor0SField.toScalarField_contMDiff [CompleteSpace 𝕜]
  ...
  have hbase := (trivializationAt (ContinuousMultilinearMap 𝕜 (fun _ : Fin 0 => E) 𝕜)
...
theorem Tensor0SField.toScalarField_add [CompleteSpace 𝕜]
  ...
  rw [show (α + β) x = α x + β x from rfl, ContinuousMultilinearMap.add_apply]
...
theorem Tensor0SField.toScalarField_smulByFun [CompleteSpace 𝕜]
  ...
  simp only [Tensor0SField.toScalarField, tensor0SField_smulByFun_apply,
    ContinuousMultilinearMap.smul_apply, Pi.mul_apply, smul_eq_mul]
...
→L[𝕜] Tensor0SSpace s I x`) by sending `c ↦ (c Fin.elim0) • T`, i.e. extracting the
...
element: compose the identity CLE with evaluation at the empty tuple. -/
noncomputable def tensor0SSpace_evalScalar (x : M) :
  ...
  (ContinuousMultilinearMap.apply 𝕜 (fun _ : Fin 0 => E) 𝕜 Fin.elim0).comp
    (tensor0SSpace_continuousLinearEquiv 0 x).toContinuousLinearMap
...
noncomputable def Tensor0SField.toTensorRSField {s : ℕ} [CompleteSpace 𝕜]
  ...
  letI := Bundle.continuousMultilinearMap.mixedTopology (𝕜 := 𝕜) (E := TangentSpace I) 0 s
  letI := Bundle.continuousMultilinearMap.topologicalSpace_totalSpace 𝕜 _ E (TangentSpace I) s
...
noncomputable def tensor0SField_product
  ...
  letI := Bundle.continuousMultilinearMap.topologicalSpace_totalSpace 𝕜 _ E (TangentSpace I) (s + q)
  letI := Bundle.continuousMultilinearMap.topologicalSpace_totalSpace 𝕜 _ E (TangentSpace I) s
  letI := Bundle.continuousMultilinearMap.topologicalSpace_totalSpace 𝕜 _ E (TangentSpace I) q
