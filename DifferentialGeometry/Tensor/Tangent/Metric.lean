...
import DifferentialGeometry.Tensor.Tangent.Contract
...
def RiemannianMetric.to02Tensor {I : ModelWithCorners ℝ E H} {n : WithTop ℕ∞}
  ...
  letI := Bundle.continuousMultilinearMap.topologicalSpace_totalSpace ℝ 2 E (TangentSpace I)
  ...
  exact ⟨fun x => (eCLM.comp (gI x)).uncurryLeft, by
    ...
    haveI := SmoothVectorBundle.continuousMultilinearMap
      (𝕜 := ℝ) (s := 2) (F := E) (E := TangentSpace I) (IB := I) (n := n)
    intro x₀; rw [contMDiffAt_section]
    have triv_2 : ∀ (x : M) (f : Tensor0SBundle.Tensor0SSpace 2 I x),
        (trivializationAt (ContinuousMultilinearMap ℝ (fun _ : Fin 2 => E) ℝ)
