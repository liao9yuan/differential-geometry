...
theorem dualMultilinearSymmTransition {r : ℕ} (x₀ x : B)
  ...
  intro v
  have hx_dual :
      x ∈ (trivializationAt (F →L[𝕜] 𝕜) (Bundle.dual 𝕜 E) x₀).baseSet := by
  ...
  have hxx_dual :
      x ∈ (trivializationAt (F →L[𝕜] 𝕜) (Bundle.dual 𝕜 E) x).baseSet :=
  ...
  have hLHS :
    ...
    have hdual := Bundle.continuousMultilinearMap.dualBundle_triv_symmL_eq_comp
      (𝕜 := 𝕜) (F := F) (E := E) x x hxx
      ((trivializationAt (F →L[𝕜] 𝕜) (Bundle.dual 𝕜 E) x).continuousLinearMapAt
        𝕜 x (v i))
...
theorem dualMultilinearTrivTransition {r : ℕ} (x₀ x : B)
  ...
  have hx_dmr : x ∈ (trivializationAt
      (ContinuousMultilinearMap 𝕜 (fun _ : Fin r => F →L[𝕜] 𝕜) 𝕜)
      (fun x => Bundle.continuousMultilinearMap 𝕜 r (F →L[𝕜] 𝕜)
        (Bundle.dual 𝕜 E) x) x₀).baseSet := by
  ...
  change (trivializationAt
      (ContinuousMultilinearMap 𝕜 (fun _ : Fin r => F →L[𝕜] 𝕜) 𝕜)
      (fun x => Bundle.continuousMultilinearMap 𝕜 r (F →L[𝕜] 𝕜)
        (Bundle.dual 𝕜 E) x) x₀).continuousLinearMapAt 𝕜 x
      ((Bundle.continuousMultilinearMap.continuousLinearEquivAt
        (𝕜 := 𝕜) (F := F →L[𝕜] 𝕜) (E := Bundle.dual 𝕜 E) r x).symm M) =
    (ContinuousMultilinearMap.compContinuousLinearMapL
      (𝕜 := 𝕜) (E := fun _ : Fin r => F →L[𝕜] 𝕜)
  ...
  exact (trivializationAt
    (ContinuousMultilinearMap 𝕜 (fun _ : Fin r => F →L[𝕜] 𝕜) 𝕜)
    (fun x => Bundle.continuousMultilinearMap 𝕜 r (F →L[𝕜] 𝕜)
      (Bundle.dual 𝕜 E) x) x₀).continuousLinearMapAt_symmL hx_dmr
    ((ContinuousMultilinearMap.compContinuousLinearMapL
      (𝕜 := 𝕜) (E := fun _ : Fin r => F →L[𝕜] 𝕜)
      (E₁ := fun _ : Fin r => F →L[𝕜] 𝕜) (F := 𝕜)
      (fun _ => (ContinuousLinearMap.compL 𝕜 F F 𝕜).flip
